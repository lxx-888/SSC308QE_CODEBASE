...
static inline int fwfs_read_wrapper(const fwfs_t *fwfs, fwfs_block_t block,
                                    fwfs_off_t off, void *buffer, fwfs_size_t size) {
    if (fwfs_tftl_isenabled(fwfs->cfg))
	return fwfs_tftl_read(fwfs, block, off, buffer, size);
    else
	return fwfs->cfg->read(fwfs->cfg, block, off, buffer, size);
}
...
static int fwfs_prog_wrapper(fwfs_t *fwfs, fwfs_block_t block, fwfs_off_t off,
    ...
    if (fwfs->cfg->debug >= 2) {
        ...
        for (fwfs_off_t i = 0; i < size; i++) {
            ...
            if (data != 0xff) {
                FWFS_ERROR("Value 0x%"PRIx32" at offset 0x%"PRIx32" in block "
                           "0x%"PRIx32" isn't 0x%"PRIx32, (uint32_t)data,
                           off + i, block, (uint32_t)0xff);
                FWFS_ASSERT(false);
            }
        }
    }
    ...
    if (fwfs_tftl_isenabled(fwfs->cfg))
	return fwfs_tftl_prog(fwfs, block, off, buffer, size);
    else
	return fwfs->cfg->prog(fwfs->cfg, block, off, buffer, size);
}

static inline int fwfs_erase_wrapper(const fwfs_t *fwfs, fwfs_block_t block) {
    if (fwfs_tftl_isenabled(fwfs->cfg))
	return fwfs_tftl_erase(fwfs, block);
    else
	return fwfs->cfg->erase(fwfs->cfg, block);
}

static inline int fwfs_sync_wrapper(const fwfs_t *fwfs) {
    if (fwfs_tftl_isenabled(fwfs->cfg))
	return fwfs_tftl_sync(fwfs);
    else
...
static inline int fwfs_alloc_wrapper(fwfs_t *fwfs, fwfs_block_t *block, bool ctz) {
    if (fwfs_tftl_isenabled(fwfs->cfg)) {
        if (ctz)
            return fwfs_tftl_alloc(fwfs, block);
        else
            return fwfs_tftl_dir_alloc(fwfs, block, 1);
    } else {
...
static inline bool fwfs_block_check(fwfs_t *fwfs, fwfs_block_t block) {
    if (fwfs_tftl_block_id(fwfs, block) >= fwfs->cfg->block_count)
        return false;
    return true;
}
...
static int fwfs_bd_read(fwfs_t *fwfs,
 ...
 retry:
    while (size > 0) {
        ...
        if (fwfs->cfg->bypass(fwfs->cfg, data) && size >= hint &&
            ...
            // bypass cache?
            int err = fwfs_read_wrapper(fwfs, block, off - extra_size,
                                        data, diff);
            if (err > 0) {
        ...
        rcache->size = fwfs_min(
                ...
                - rcache->off,
                rcache->cache_size);
        int err = fwfs_read_wrapper(fwfs, rcache->block,
...
static int fwfs_bd_erase(fwfs_t *fwfs, fwfs_block_t block) {
    FWFS_ASSERT(fwfs_block_check(fwfs, block));
    int err = fwfs_erase_wrapper(fwfs, block);
    if (err > 0) {
...
static int fwfs_dir_alloc(fwfs_t *fwfs, fwfs_mdir_t *dir) {
    if (fwfs_tftl_isenabled(fwfs->cfg)) {
        fwfs_block_t blocks[2];
        int err;

        err = fwfs_tftl_dir_alloc(fwfs, blocks, 2);
        if (err) {
            return err;
        }
...
static int fwfs_dir_commit(fwfs_t *fwfs, fwfs_mdir_t *dir,
    ...
    if (!fwfs_force_compact(fwfs) && (dir->erased || dir->count >= 0xff)) {
        fwfs_size_t block_size = fwfs_block_size_wrapper(fwfs, dir->pair[0]);
...
int fwfs_dir_read(fwfs_t *fwfs, fwfs_dir_t *dir, struct fwfs_info *info) {
    ...
    while (true) {
        ...
        if (err != FWFS_ERR_NOENT) {
            if (err == 0 && strcmp(info->name, FWFS_TFTL_GC_FILE) == 0)
                continue;
            if (err == 0 && strcmp(info->name, FWFS_VERSION_FILE) == 0)
                continue;
            break;
        }
    }
...
int fwfs_file_opencfg(fwfs_t *fwfs, fwfs_file_t *file,
    ...
    if (tag == FWFS_ERR_NOENT) {
        ...
        if (strstr(path, FWFS_VERSION_FILE)) {
            err = FWFS_ERR_NOENT;
            goto cleanup;
        }
    } else {
    ...
    if (fwfs->cfg->debug >= 3)
        FWFS_INFO("fwfs_file_opencfg:%s => fid 0x%x\n", path, file->ctz.fid);
...
static int fwfs_file_outline(fwfs_t *fwfs, fwfs_file_t *file) {
    ...
    file->flags &= ~(FWFS_F_INLINE|FWFS_F_DIRTY);
...
int fwfs_remove(fwfs_t *fwfs, const char *path) {
    ...
    if (fwfs_tag_type3(tag) == FWFS_TYPE_REG) {
        if (strstr(path, FWFS_VERSION_FILE)) {
            return FWFS_ERR_NOENT;
        }
    }
...
static int fwfs_init(fwfs_t *fwfs, const struct fwfs_config *cfg) {
    ...
    for (uint8_t i = 0; i < fwfs->rcache_pool.size; i++) {
        ...
        if (!rcache) {
            if (i == 0 && (!fwfs->cfg->read_buffer)) {
                fwfs_free(rcache_buffer);
                rcache_buffer = NULL;
            }
            err = FWFS_ERR_NOMEM;
            goto cleanup;
...
static int fwfs_format_write_version(fwfs_t* fwfs) {
    fwfs_file_t file;
    char path[FWFS_NAME_MAX+1];
    int err;
    uint32_t version;

    snprintf(path, sizeof(path), "%s", FWFS_VERSION_FILE);
    ...
    version = FWFS_VERSION;
    err = fwfs_file_write(fwfs, &file, &version, 4);
    if (err != 4) {
        FWFS_ERROR("fwfs_file_write return %d(%d)\n", err,
                   (int)sizeof(version));
        fwfs_file_close(fwfs, &file);
        return FWFS_ERR_IO;
    }
...
cleanup:
    ...
    FWFS_TRACE("fwfs_format -> %d", err);
    {
        err = fwfs_mount(fwfs, cfg);
        if (err)
            return err;

        err = fwfs_format_write_version(fwfs);
        if (err) {
            FWFS_ERROR("failed to create version file!");
            return err;
        }

        err = fwfs_unmount(fwfs);
    }
    return err;
}
