...
#define ASCII_DECLARE_MASK(name) char name[256]; memset(name, 0, 256);
#define ASCII_MASK_SET(val, name) (name[val] = 1)
#define ASCII_MASK_GET(val, name) (name[val])
#define ASCII_MASK_INVERSE(name) do{    \
	int ascii_loop = 0; \
	for (ascii_loop = 0; ascii_loop < 128; ascii_loop++)    \
	name[ascii_loop] = !name[ascii_loop];  \
}while(0);
			...
			case '[':
				{
					...
					do{
						ASCII_MASK_SET((u8)*fmt, ascii_set);
						++fmt, ++len;
					}while (*fmt && *fmt != ']');
					...
					if (negate) {
						ASCII_MASK_INVERSE(ascii_set);
					}

					/* match must be non-empty */
					if (!ASCII_MASK_GET((u8)*str, ascii_set))
						return num;

					while (*str != '\0'
							&& ASCII_MASK_GET((u8)*str, ascii_set)
