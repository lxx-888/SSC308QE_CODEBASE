...
extern u8   spi_nand_is_match(u8 *sni_buf);
...
int _spinand_loadbin_ubi(PARTS_INFO_t *pst_PartInfo, u8 *pu8_data)
{
    ...
    for (i = 0; i < spinand_ubi->vtbl_slots; i++)
    {
        ...
        if (vol)
        {
            ...
            memcpy((void *)(g_spinand_ubi_dev[g_ubi_dev_index].spinand_ubivol[i].vol_name), (const void *)(vol->name),
                   vol->name_len);
            // g_spinand_ubi_dev[g_ubi_dev_index].spinand_ubivol[i].vol_size = vol->used_ebs * (nand->erasesize - (2 *
            // nand->writesize));
...
static int _spinand_loadbin(PARTS_INFO_t *pst_PartInfo, u8 *pu8_data)
{
    ...
    for (u32_offset = pst_PartInfo->u32_Offset, u32_size = pst_PartInfo->u32_Size; u32_offset < u32_size;
         u32_offset += nand->erasesize)
    {
    ...
    if (retlen < pst_PartInfo->u32_Size
        && nand->writesize == MDRV_PARTS_load_part(pst_PartInfo, 0, pu8_data, nand->writesize))
    {
        if (!strncmp((const char *)pu8_data, "UBI", 3))
        {
            return _spinand_loadbin_ubi(pst_PartInfo, pu8_data);
        }
        else if (nand->writesize == MDRV_PARTS_load_part(pst_PartInfo, nand->erasesize, pu8_data, nand->writesize)
    ...
    if (retlen != MDRV_PARTS_load_part(pst_PartInfo, 0, pu8_data, retlen))
    {
        return 1;
    }
...
static int _spinand_loadbin_cis(PARTS_INFO_t *pst_PartInfo, u8 *pu8_data)
{
    ...
    for (u32_offset = pst_PartInfo->u32_Offset; u32_offset < u32_end; u32_offset += nand->erasesize)
    {
        if (!(u32_offset & nand->erasesize) && !nand_block_isbad(nand, (loff_t)u32_offset)
            && nand->erasesize == MDRV_PARTS_load_part(pst_PartInfo, u32_offset, pu8_data, nand->erasesize))
        {
            ret = 1;
            break;
        }
...
static void _spinand_adjust_parts_tbl(void)
{
    ...
    for (u8_i = 0; u8_i < u32Count; u8_i++)
    {
        if (!strcmp((const char *)(pstTemp->au8_partName), "IPL"))
        {
            ...
            for (u32_offset = stOld.u32_Offset, u32_size = stOld.u32_Size; u32_offset < u32_size;
                 u32_offset += nand->erasesize)
            {
                if (nand_block_isbad(nand, (loff_t)u32_offset))
                {
            ...
            {
                memcpy((void *)(g_bin_buf + pstTemp->u32_Offset), (const void *)(g_bin_buf + stOld.u32_Offset),
                       pstTemp->u32_Size);
            }
        }
        else if (!strcmp((const char *)(pstTemp->au8_partName), "IPL_CUST")
                 || !strcmp((const char *)(pstTemp->au8_partName), "UBOOT"))
        {
    ...
    pst_partsTbl->u32_Checksum =
        _spinand_checksum((u32) & (pst_partsTbl->u32_Size), pst_partsTbl->u32_Size + sizeof(pst_partsTbl->u32_Size));
...
static int spinand_loadbin(void)
{
    ...
    do
    {
        ...
        if (!ret)
        {
            printf("[SPINAND] Load %s form 0x%x to 0x%lx\n", st_PartInfo.au8_partName, st_PartInfo.u32_Offset,
                   (unsigned long)(g_bin_buf + st_PartInfo.u32_Offset));
...
static int _spinand_savebin(PARTS_INFO_t *pst_PartInfo, u8 *pu8_data)
{
    ...
    if (retlen != MDRV_PARTS_program_part(pst_PartInfo, 0, pu8_data, retlen))
    {
        return 1;
    }
...
static int spinand_savebin(void)
{
    ...
    if (!spi_nand_is_match(g_bin_buf))
    {
        ...
        printk("[SPINAND] Please change to %s 0x%02x 0x%02x 0x%02x\n", sni_info->au8_partnumber,
               g_spinand_info->au8_ID[0], g_spinand_info->au8_ID[1], g_spinand_info->au8_ID[2]);
        return 0;
    }
    ...
    do
    {
        ...
        printf("[SPINAND] Save %s form 0x%lx to 0x%x\n", st_PartInfo.au8_partName,
               (unsigned long)(g_bin_buf + st_PartInfo.u32_Offset), st_PartInfo.u32_Offset);
        if (!ret)
        {
...
static int do_spinand(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
    const char *cmd;
    int         ret = -1;
    char *      endptr;
    ...
    if (g_pni_buf && strcmp(cmd, "savebin") == 0)
    {
        ret = spinand_savebin();
        goto done;
    }
    ...
    {
        if (!g_pni_buf && !(g_pni_buf = (u8 *)malloc(2048)))
        {
            goto done;
        }

        if (!MDRV_PARTS_init_tlb(g_pni_buf))
        {
...
U_BOOT_CMD(spinand, CONFIG_SYS_MAXARGS, 1, do_spinand, "spinand sub-system",
           "spinand probe\n"
           "spinand remove\n"
