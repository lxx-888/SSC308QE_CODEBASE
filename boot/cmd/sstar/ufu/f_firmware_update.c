...
#define FSG_DRIVER_VERSION "2022/02/18"
...
#ifdef CONFIG_CMD_SSTAR_VENDOR
#define SSTAR_USB_GET_VENDOR_INFO 0x07
#endif
/*-------------------------------------------------------------------------*/
...
static inline int __fsg_is_set(struct fsg_common *common, const char *func, unsigned line)
{
    if (common->fsg)
        return 1;
...
static void set_bulk_out_req_length(struct fsg_common *common, struct fsg_buffhd *bh, unsigned int length)
{
    unsigned int rem;
...
static int fsg_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
    ...
    VDBG(fsg,
         ...
         ctrl->bRequestType, ctrl->bRequest, get_unaligned_le16(&ctrl->wValue), w_index, w_length);
    return -EOPNOTSUPP;
}
...
#define START_TRANSFER_OR(common, ep_name, req, pbusy, state)                     \
    if (fsg_is_set(common))                                                       \
        start_transfer((common)->fsg, (common)->fsg->ep_name, req, pbusy, state); \
    else

#define START_TRANSFER(common, ep_name, req, pbusy, state) START_TRANSFER_OR(common, ep_name, req, pbusy, state)(void) 0
...
static int do_inquiry(struct fsg_common *common, struct fsg_buffhd *bh)
{
    ...
    static const char vendor_id[] = "GCREADERUBOOT";
    u8 *              buf         = (u8 *)bh->buf;
    ...
    buf[4] = 31; /* Additional length */
                 /* No special options */
    sprintf((char *)(buf + 8), "%s", (char *)vendor_id);
...
/* unused function to avoid:
 * warning: 'store_cdrom_address' defined but not used [-Wunused-function]
 */
int do_read_header(struct fsg_common *common, struct fsg_buffhd *bh)
{
...
/* unused function to avoid:
 * warning: ‘fsg_lun_fsync_sub’ defined but not used [-Wunused-function]
 */
int do_prevent_allow(struct fsg_common *common)
{
    struct fsg_lun *curlun = &common->luns[common->lun];
    int             prevent;
...
static int do_read_format_capacities(struct fsg_common *common, struct fsg_buffhd *bh)
{
    u8 *buf = (u8 *)bh->buf;
...
/*-------------------------------------------------------------------------*/
static int do_read(struct fsg_common *common, struct fsg_buffhd *bh)
{
    u8 *buf = (u8 *)bh->buf;
...
static int do_vendor_sstar_bulk_out(struct fsg_common *common, void *out)
{
    ...
    while (amount_left_to_write > 0)
    {
        ...
        if (bh->state == BUF_STATE_FULL)
        {
            ...
            //  buf[7]); debug("%2x %2x %2x %2x %2x %2x %2x %2x\n", buf[8], buf[9], buf[10], buf[11], buf[12], buf[13],
            //  buf[14], buf[15]);
...
#ifdef CONFIG_CMD_SSTAR_VENDOR
extern int usb_read_vendor_info(u8 *buf);
static int do_vendor_sstar_get_vendor_info(struct fsg_common *common, struct fsg_buffhd *bh)
{
    u8 *buf = (u8 *)bh->buf;

    return usb_read_vendor_info(buf);
}
#endif
...
static int do_vendor_sstar_end_dnl(struct fsg_common *common)
{
    ...
    do_vendor_sstar_bulk_out(common, ufu.cur);
    ufu.cur += common->data_size_from_cmnd;
...
static int do_vendor_sstar_loadinfo(struct fsg_common *common)
{
    ...
    ufu.cur = (u8 *)(ulong)ufu.loadinfo.addr;
    env_set_hex("filesize", ufu.loadinfo.size);
...
static int do_vendor_sstar_runcmd(struct fsg_common *common)
{
    ...
    printf("[UFU runcmd] %s\n", ufu.cmd);
    // run command
    if (memcmp(ufu.cmd, "reset", strlen("reset")) == 0)
    {
        need_reset = 1;
        result     = 0;
    }
...
static int do_vendor_sstar(struct fsg_common *common, struct fsg_buffhd *bh)
{
    ...
    switch (common->cmnd[1])
    {
        ...
        case SSTAR_USB_RUN_CMD:
            ...
            break;
#ifdef CONFIG_CMD_SSTAR_VENDOR
        case SSTAR_USB_GET_VENDOR_INFO:
            reply = do_vendor_sstar_get_vendor_info(common, bh);
            break;
#endif
        default:
            reply = -EINVAL;
...
static int throw_away_data(struct fsg_common *common)
{
    ...
    for (bh = common->next_buffhd_to_drain; bh->state != BUF_STATE_EMPTY || common->usb_amount_left > 0;
         bh = common->next_buffhd_to_drain)
    {
...
static int check_command(struct fsg_common *common, int cmnd_size, enum data_direction data_dir, unsigned int mask,
                         int needs_medium, const char *name)
{
    ...
    if (common->data_dir != DATA_DIR_UNKNOWN)
        sprintf(hdlen, ", H%c=%u", dirletter[(int)common->data_dir], common->data_size);
    ...
    if (cmnd_size != common->cmnd_size)
    {
        ...
        if (cmnd_size <= common->cmnd_size)
        {
            DBG(common,
                ...
                "but we got %d\n",
                name, cmnd_size, common->cmnd_size);
    ...
    else
    {
        ...
        if (common->cmnd[0] != SC_INQUIRY && common->cmnd[0] != SC_REQUEST_SENSE)
        {
...
static int do_scsi_command(struct fsg_common *common)
{
    ...
    switch (common->cmnd[0])
    {
        case SC_INQUIRY:
            ...
            reply                       = check_command(common, 6, DATA_DIR_TO_HOST, (1 << 4), 0, "INQUIRY");
            if (reply == 0)
        ...
        case SC_READ_CAPACITY:
            ...
            reply = check_command(common, 10, DATA_DIR_TO_HOST, (0xf << 2) | (1 << 8), 1, "READ CAPACITY");
            if (reply == 0)
        ...
        case SC_READ_FORMAT_CAPACITIES:
            ...
            reply = check_command(common, 10, DATA_DIR_TO_HOST, (3 << 7), 1, "READ FORMAT CAPACITIES");
            if (reply == 0)
                reply = do_read_format_capacities(common, bh);
            break;
        case SC_READ_10:
            common->data_size_from_cmnd = get_unaligned_be16(&common->cmnd[7]) << 9;
            reply = check_command(common, 10, DATA_DIR_TO_HOST, (1 << 1) | (0xf << 2) | (3 << 7), 1, "READ(10)");
            if (reply == 0)
                reply = do_read(common, bh);
            break;
        case SC_TEST_UNIT_READY:
            common->data_size_from_cmnd = 0;
            reply                       = check_command(common, 6, DATA_DIR_NONE, 0, 1, "TEST UNIT READY");
            break;

        case SC_REQUEST_SENSE:
            common->data_size_from_cmnd = common->cmnd[4];
            reply                       = check_command(common, 6, DATA_DIR_TO_HOST, (1 << 4), 0, "REQUEST SENSE");
            if (reply == 0)
...
static int received_cbw(struct fsg_dev *fsg, struct fsg_buffhd *bh)
{
    ...
    /* Is the CBW valid? */
    if (req->actual != USB_BULK_CB_WRAP_LEN || cbw->Signature != cpu_to_le32(USB_BULK_CB_SIG))
    {
        DBG(fsg, "invalid CBW: len %u %u sig 0x%x\n", req->length, req->actual, le32_to_cpu(cbw->Signature));
    ...
    if (cbw->Lun >= FSG_MAX_LUNS || cbw->Flags & ~USB_BULK_IN_FLAG || cbw->Length <= 0
...
static int enable_endpoint(struct fsg_common *common, struct usb_ep *ep, const struct usb_endpoint_descriptor *d)
{
    int rc;
...
static int alloc_request(struct fsg_common *common, struct usb_ep *ep, struct usb_request **preq)
{
...
reset:
    ...
    d  = fsg_ep_desc(common->gadget, &fsg_fs_bulk_in_desc, &fsg_hs_bulk_in_desc, &fsg_ss_bulk_in_desc);
    rc = enable_endpoint(common, fsg->bulk_in, d);
    if (rc)
        goto reset;
    fsg->bulk_in_enabled = 1;

    d  = fsg_ep_desc(common->gadget, &fsg_fs_bulk_out_desc, &fsg_hs_bulk_out_desc, &fsg_ss_bulk_out_desc);
...
static void handle_exception(struct fsg_common *common)
{
    ...
    switch (old_state)
    {
        ...
        case FSG_STATE_RESET:
            ...
            if (!fsg_is_set(common))
                break;
            if (test_and_clear_bit(IGNORE_BULK_OUT, &common->fsg->atomic_bitflags))
...
static struct fsg_common *fsg_common_init(struct fsg_common *common, struct usb_composite_dev *cdev)
{
    ...
    for (i = 0; i < nluns; i++)
    {
        ...
        rc = fsg_lun_open(&common->luns[i], 0, "");
        if (rc)
            goto error_luns;
    }
    common->lun = 0;
    ...
    snprintf(common->inquiry_string, sizeof common->inquiry_string, "%-8s%-16s%04x", "Linux   ", "File-Store Gadget",
             0xffff);
    ...
    common->thread_task = kthread_create(fsg_main_thread, common, OR(cfg->thread_name, "file-storage"));
...
static int fsg_bind(struct usb_configuration *c, struct usb_function *f)
{
    ...
    if (gadget_is_superspeed(gadget))
    {
        ...
        fsg_ss_bulk_in_comp_desc.bMaxBurst    = min_t(unsigned, FSG_BUFLEN / 1024, 15);
        fsg_ss_bulk_out_comp_desc.bMaxBurst   = min_t(unsigned, FSG_BUFLEN / 1024, 15);
