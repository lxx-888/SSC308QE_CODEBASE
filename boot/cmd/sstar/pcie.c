...
// Macro & Define
//=================================================================================================
/* [FIXME] number of MSI interrupt supported, just for example */
#define PCI_EPF_MSI_INTR_NUM 8
/* [FIXME] number of MSI-X interrupt supported, just for example */
#define PCI_EPF_MSIX_INTR_NUM 16

#define EPF_FUNC_ENTER() PCIE_VERBOSE("%s+\r\n", __FUNCTION__)
#define EPF_FUNC_EXIT()  PCIE_VERBOSE("%s-\r\n", __FUNCTION__)

#define IRQ_TYPE_LEGACY (0)
#define IRQ_TYPE_MSI    (1)
#define IRQ_TYPE_MSIX   (2)

#define CMD_RAISE_LEGACY_IRQ (0x0001)
#define CMD_RAISE_MSI_IRQ    (0x0002)
#define CMD_RAISE_MSIX_IRQ   (0x0004)
#define CMD_READ             (0x0008)
#define CMD_WRITE            (0x0010)
#define CMD_COPY             (0x0020)

#define STATUS_READ_OK          (0x0001)
#define STATUS_READ_NG          (0x0002)
#define STATUS_WRITE_OK         (0x0004)
#define STATUS_WRITE_NG         (0x0008)
#define STATUS_COPY_OK          (0x0010)
#define STATUS_COPY_NG          (0x0020)
#define STATUS_IRQ_RAISED       (0x0040)
#define STATUS_SRC_ADDR_INVALID (0x0080)
#define STATUS_DST_ADDR_INVALID (0x0100)

#define FLAG_USE_DMA (0x0001)
...
typedef struct
{
    ...
    u32 irq_number;
    u32 flags;
} __packed epf_test_reg;

//=================================================================================================
// Local Variable
//=================================================================================================
static struct pci_ep_header header = {
    .vendorid       = 0x104c, /* [FIXME] an example only */
    .deviceid       = 0xb500, /* [FIXME] an example only */
    .baseclass_code = 0xff,
    .interrupt_pin  = PCI_INTERRUPT_INTA,
};

static u8 dma_completed = 0;
/* ep function */
static PCIE_EPF *epf = NULL;

/* [FIXME] Using BAR0 for communicating with RC, just for example */
#define TEST_REG_BAR 0
/* [FIXME] BAR where the MSI-X table resides, just for example  */
#define MSIX_BAR 0
/* [FIXME] Offset pointing to the start of MSI-X table, just for example */
#define MSIX_TABLE_OFFSET (EPC_MEM_ALIGN(sizeof(epf_test_reg), 128))
/* [FIXME] only use BAR0 & BAR1, for example */
static u32   bar_size[6] = {1048576, 65536, 0, 0, 0, 0};
static void *bar_addr[6] = {NULL, NULL, NULL, NULL, NULL, NULL};

/* DMA timeout in unit of us, for example */
#define PCIE_DMA_TIMEOUT 5000000 // 5 sec
...
static u32 crc32_body(u32 crc, u8 *buf, size_t len, const u32 (*tab)[256])
{
...
static u32 crc32_le(const void *buf, size_t size)
{
    return crc32_body(~0, (u8 *)buf, size, (const u32(*)[256])crc32_table);
}

static void *alloc_space(PCIE_EPF *epf, u32 size, enum pci_barno bar)
{
    void *      addr;
    phys_addr_t phys;
    ...
    if (phys & (size - 1))
        phys = (phys + size) & ~(size - 1);

    /* keep the original physical addr, virtual addr, and size */
    epf->bar[bar].phys_addr = phys;
    epf->bar[bar].size      = size;
    epf->bar[bar].barno     = bar;
    epf->bar[bar].flags     = PCI_BASE_ADDRESS_SPACE_MEMORY;

    PCIE_DBG("BAR[%d]: phys %llx, size %d\r\n", bar, epf->bar[bar].phys_addr, size);
...
static int pcie_epf_alloc_space(PCIE_EPF *epf)
{
    u8    i;
    void *space;
    ...
    for (i = 0; i < 6; i++)
    {
        if (bar_size[i])
        {
            space = alloc_space(epf, bar_size[i], i);
            if (space == NULL)
                return -ENOMEM;
            if (i == TEST_REG_BAR)
                memset(phys_to_virt(epf->bar[i].phys_addr), 0, sizeof(epf_test_reg));
        }
    }
...
static void pcie_epf_free_space(PCIE_EPF *epf)
{
    u8 i;
...
static int pcie_epf_set_bar(struct udevice *dev, PCIE_EPF *epf)
{
    u8  i;
    int ret;
    ...
    for (i = 0; i < 6; i++)
    {
        if (bar_size[i])
        {
            ret = pci_ep_set_bar(dev, 0, &epf->bar[i]);
            if (ret)
                return ret;
        }
    }
...
static void pcie_epf_raise_irq(struct udevice *dev, epf_test_reg *reg)
{
    if (reg->irq_type == IRQ_TYPE_MSI)
    {
        if ((reg->irq_number >= PCI_EPF_MSI_INTR_NUM) || (reg->irq_number <= 0))
        {
            PCIE_ERR("epf msi invalid irq num %x\r\n", reg->irq_number);
            return;
        }
        reg->status |= STATUS_IRQ_RAISED;
        flush_cache((ulong)reg, sizeof(epf_test_reg));
        pci_ep_raise_irq(dev, 0, PCI_EP_IRQ_MSI, reg->irq_number);
    }
    else if (reg->irq_type == IRQ_TYPE_MSIX)
    {
        if ((reg->irq_number >= PCI_EPF_MSIX_INTR_NUM) || (reg->irq_number <= 0))
        {
            PCIE_ERR("epf msix invalid irq num %x\r\n", reg->irq_number);
            return;
        }
        reg->status |= STATUS_IRQ_RAISED;
        flush_cache((ulong)reg, sizeof(epf_test_reg));
        pci_ep_raise_irq(dev, 0, PCI_EP_IRQ_MSIX, reg->irq_number);
    }
    else
    {
        PCIE_ERR("Unsupported IRQ type %x\r\n", reg->irq_type);
        return;
    }
}

static void pcie_epf_dma_callback(EDMA_XFER_STATE state, void *data)
{
    dma_completed = 1;
    PCIE_DBG("epf dma callback %d\r\n", state);
}

static int pcie_epf_wait_dma(struct udevice *dev)
{
    ulong    start;
    PCIE_EP *ep = (PCIE_EP *)dev_get_priv(dev);

    start = timer_get_us();
    do
    {
        /* handle dma interrupt while it occurs */
        Drv_PCIE_EpcIrqHandler(ep);
        if (dma_completed)
            return 0;

        /* abort due to ctrl-c was pressed */
        if (ctrlc())
            return -ECANCELED;
        /* dma timeouts */
        if ((timer_get_us() - start) >= PCIE_DMA_TIMEOUT)
            return -ETIME;
    } while (1);
...
static int pcie_epf_copy(struct udevice *dev, epf_test_reg *reg)
{
    int         ret = 0;
    PCIE_EP *   ep  = (PCIE_EP *)dev_get_priv(dev);
    ...
    src_addr = Drv_PCIE_EpcMemAlloc(&ep->epc, reg->size, &src_phys);
    if (!src_addr)
    {
        PCIE_ERR("epf copy: failed to allocate src address\r\n");
        reg->status = STATUS_SRC_ADDR_INVALID;
        ret         = -ENOMEM;
        goto err;
    }

    ret = pci_ep_map_addr(dev, 0, src_phys, reg->src_addr, reg->size);
    if (ret)
    {
        PCIE_ERR("epf copy: failed to map src address\r\n");
        reg->status = STATUS_SRC_ADDR_INVALID;
        goto err_addr;
    }
    PCIE_NOTICE("epf copy: src phys x%llx, virt x%p\r\n", src_phys, src_addr);

    dst_addr = Drv_PCIE_EpcMemAlloc(&ep->epc, reg->size, &dst_phys);
    if (!dst_addr)
    {
        PCIE_ERR("epf copy: failed to allocate dst address\r\n");
        reg->status = STATUS_DST_ADDR_INVALID;
        ret         = -ENOMEM;
        goto err;
    }

    ret = pci_ep_map_addr(dev, 0, dst_phys, reg->dst_addr, reg->size);
    if (ret)
    {
        PCIE_ERR("test copy: failed to map dst address\r\n");
        reg->status = STATUS_DST_ADDR_INVALID;
        goto err_addr;
    }
    PCIE_NOTICE("test copy: dst phys x%llx, virt x%p\r\n", dst_phys, dst_addr);

    start = timer_get_us();
    memcpy(dst_addr, src_addr, reg->size);
    end = timer_get_us();

    PCIE_NOTICE("test copy: size %d consumes %d us\r\n", reg->size, (u32)(end - start));

    pci_ep_unmap_addr(dev, 0, src_phys);
    pci_ep_unmap_addr(dev, 0, dst_phys);

err_addr:
    // not support EpcMemFree

err:
    return ret;
}

static int pcie_epf_read(struct udevice *dev, epf_test_reg *reg)
{
    int         ret = 0;
    PCIE_EP *   ep  = (PCIE_EP *)dev_get_priv(dev);
    void *      buf;
    phys_addr_t buf_phys, buf_miu;
    u32         crc = 0;
    void *      src_addr;
    ...
    if (!buf)
    {
        PCIE_ERR("epf read: alloc %d failed\r\n", reg->size);
        ret = -ENOMEM;
        goto err;
    }
    ...
    {
        PCIE_EDMA *edma = &ep->edma;

        buf_miu = Drv_PCIE_Phys2Miu(buf_phys);
        PCIE_NOTICE("epf read: dma src miu x%llx, dst miu x%llx\r\n", reg->src_addr, buf_miu);
        flush_cache((ulong)buf, reg->size);
        dma_completed = 0;
        Drv_PCIE_DmaSubmit(edma, reg->src_addr, (u64)buf_miu, reg->size, EDMA_DIR_RD);
        Drv_PCIE_DmaIssuePending(edma, EDMA_DIR_RD, pcie_epf_dma_callback, NULL);
        start = timer_get_us();
        pcie_epf_wait_dma(dev);
        end = timer_get_us();
    }
    else
    {
        memset(buf, 0, reg->size);

        src_addr = Drv_PCIE_EpcMemAlloc(&ep->epc, reg->size, &src_phys);
        if (!src_addr)
        {
            PCIE_ERR("epf read: allocate mem failed\r\n");
            reg->status = STATUS_SRC_ADDR_INVALID;
            ret         = -ENOMEM;
            goto err;
        }
        ...
        if (ret)
        {
            PCIE_ERR("test read: failed to map address\n");
            reg->status = STATUS_SRC_ADDR_INVALID;
            goto err_addr;
        }
        PCIE_NOTICE("test read: src phys x%llx, dst phys x%llx\r\n", src_phys, buf_phys);
    ...
    PCIE_NOTICE("test read: dma %s size %d consumes %d us\r\n", reg->flags & FLAG_USE_DMA ? "on" : "off", reg->size,
                (u32)(end - start));

    crc = crc32_le(buf, reg->size);
    PCIE_NOTICE("test read: crc x%x v.s x%x\r\n", crc, reg->checksum);
    if (crc != reg->checksum)
        ret = -EFAULT;
...
static int pcie_epf_write(struct udevice *dev, epf_test_reg *reg)
{
    int         ret = 0;
    PCIE_EP *   ep  = (PCIE_EP *)dev_get_priv(dev);
    void *      buf;
    ...
    if (!buf)
    {
        PCIE_ERR("epf write: alloc %d failed\r\n", reg->size);
        ret = -ENOMEM;
        goto err;
    }
    ...
    reg->checksum = crc32_le(buf, reg->size);
    buf_phys      = virt_to_phys(buf);
    ...
    if (reg->flags & FLAG_USE_DMA)
    {
        ...
        buf_miu = Drv_PCIE_Phys2Miu(buf_phys);
        PCIE_NOTICE("test write: dma src miu x%llx, dst addr x%llx\r\n", buf_miu, reg->dst_addr);
        flush_cache((ulong)buf, reg->size);
        dma_completed = 0;
        Drv_PCIE_DmaSubmit(edma, (u64)buf_miu, reg->dst_addr, reg->size, EDMA_DIR_WR);
        Drv_PCIE_DmaIssuePending(edma, EDMA_DIR_WR, pcie_epf_dma_callback, dev);
        start = timer_get_us();
        pcie_epf_wait_dma(dev);
        end = timer_get_us();
    }
    else
    {
        ...
        if (!dst_addr)
        {
            PCIE_ERR("epf write: alloc mem failed\r\n");
            reg->status = STATUS_DST_ADDR_INVALID;
            ret         = -ENOMEM;
            goto err;
        }
        ...
        if (ret)
        {
            PCIE_ERR("epf write: failed to map address\r\n");
            reg->status = STATUS_DST_ADDR_INVALID;
            goto err_addr;
        }
        PCIE_NOTICE("test write: src phys x%llx, dst phys x%llx\r\n", buf_phys, dst_phys);
    ...
    PCIE_NOTICE("test write: dma %s size %d consumes %d us\r\n", reg->flags & FLAG_USE_DMA ? "on" : "off", reg->size,
                (u32)(end - start));
    PCIE_NOTICE("test write: crc x%x\r\n", reg->checksum);
...
static void pcie_epf_cmd_handler(struct udevice *dev)
{
    int           ret = 0;
    u32           cmd;
    epf_test_reg *reg = phys_to_virt(epf->bar[TEST_REG_BAR].phys_addr);

    invalidate_dcache_range((ulong)reg, sizeof(epf_test_reg));
    ...
    {
        case CMD_WRITE:
            ret = pcie_epf_write(dev, reg);
            if (ret)
                reg->status |= STATUS_WRITE_NG;
            else
                reg->status |= STATUS_WRITE_OK;

            pcie_epf_raise_irq(dev, reg);
            break;
        case CMD_READ:
            ret = pcie_epf_read(dev, reg);
            if (ret)
                reg->status |= STATUS_READ_NG;
            else
                reg->status |= STATUS_READ_OK;

            pcie_epf_raise_irq(dev, reg);
            break;
        case CMD_COPY:
            ret = pcie_epf_copy(dev, reg);
            if (ret)
                reg->status |= STATUS_COPY_NG;
            else
                reg->status |= STATUS_COPY_OK;

            pcie_epf_raise_irq(dev, reg);
            break;
        case CMD_RAISE_MSI_IRQ:
        case CMD_RAISE_MSIX_IRQ:
            reg->status = 0; // reset status first
            pcie_epf_raise_irq(dev, reg);
            break;
        default:
            PCIE_NOTICE("epf unsupported cmd %x\r\n", cmd);
            break;
    }
}

static int pcie_epf_bind_epc(struct udevice *dev, PCIE_EPF *epf)
{
    int       ret;
    PCIE_EP * ep  = (PCIE_EP *)dev_get_priv(dev);
    PCIE_EPC *epc = &ep->epc;
    u32       msix_tbl_size, pba_size;

    EPF_FUNC_ENTER();

    Drv_PCIE_EpcBindEpf(epc, epf);

    ret = pcie_epf_alloc_space(epf);
    if (ret)
    {
        PCIE_ERR("epf alloc space failed\r\n");
        return ret;
    }

    pci_ep_write_header(dev, 0, epf->header);

    ret = pcie_epf_set_bar(dev, epf);
    if (ret)
    {
        PCIE_ERR("epf set bar failed\r\n");
        return ret;
    }

    /* set msi
     * NOTICE: we don't use pci_ep-uclass API to set msix,
     *         thus we use epc driver API to set msi as well for the same behavior
     */
    Drv_PCIE_EpcSetMsi(epc, epf->msi_interrupts);

    /* set msix
     * NOTICE: we don't use pci_ep-uclass API to set msix
     *         since the API can't pass BIR & table offset to ep controller.
     *
     * MSI-X table BAR Indicator register (BIR) & Table Offset register are
     * predefined as MSIX_BAR & MSIX_TABLE_OFFSET,
     * please update them for your target epf driver.
     */
    msix_tbl_size = epf->msix_interrupts * PCI_MSIX_ENTRY_SIZE;
    pba_size      = EPC_MEM_ALIGN(((epf->msix_interrupts + 7) >> 3), 8);
    if ((MSIX_TABLE_OFFSET + msix_tbl_size + pba_size) > bar_size[MSIX_BAR])
    {
        PCIE_ERR("epf msi-x has no enough memory at BAR%d\r\n", MSIX_BAR);
        return -ENOMEM;
    }
    Drv_PCIE_EpcSetMsiX(epc, epf->msix_interrupts, MSIX_BAR, MSIX_TABLE_OFFSET);
...
int do_pcie(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    if (argc > 1)
        seq = simple_strtoul(argv[1], NULL, 10);

    ret = uclass_first_device_err(UCLASS_PCI_EP, &dev);
    if (ret)
        return ret;
    ret = uclass_get_device_by_seq(UCLASS_PCI_EP, seq, &dev);
    if (ret)
    {
        PCIE_ERR("epf: no ep found\r\n");
        return ret;
    }
    ep  = (PCIE_EP *)dev_get_priv(dev);
    epf = malloc(sizeof(PCIE_EPF));
    memset(epf, 0, sizeof(PCIE_EPF));

    /* Initial ep function */
    epf->header          = &header;
    epf->msi_interrupts  = PCI_EPF_MSI_INTR_NUM;
    epf->msix_interrupts = PCI_EPF_MSIX_INTR_NUM;

    /* bind ep function & ep controller */
    ret = pcie_epf_bind_epc(dev, epf);
    if (ret)
    {
        PCIE_ERR("epf: bind epc failed\r\n");
        return ret;
    }
    ...
    pcie_epf_free_space(epf);
    free(epf);

    return 0;
}

U_BOOT_CMD(pcie, 2, 0, do_pcie, "pcie endpoint verification",
           "<seq>\n"
           "  <seq>   = pcie port id as the endpoint\n");
