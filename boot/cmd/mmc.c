...
#ifdef CONFIG_SSTAR_MMC_HOST
extern void show_speed(unsigned long time,unsigned long bytes);
#endif
...
static void mmc_part_conf_setenv(u8 part)
{
	u8 boot_part;
	char boot_partition[2];

	sprintf(boot_partition, "%02x", part);
	env_set("partconf", boot_partition);
	...
	env_set("boot_part", boot_partition);
}

int mmc_get_curr_device(void)
{
	return curr_device;
}
#endif
...
#endif
#ifdef CONFIG_ARCH_SSTAR
	"mmc erase - erase all blocks in user area\n"
#endif
	"mmc erase blk# cnt\n"
	"mmc rescan [mode]\n"
