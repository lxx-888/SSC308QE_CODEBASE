...
extern struct spi_flash *spi_flash_probe_driver(unsigned long u32_address, unsigned int bus, unsigned int cs, unsigned int max_hz, unsigned int spi_mode);
...
static int do_spi_flash_probe(int argc, char *const argv[])
{
	...
	if (0 != address)
		new = spi_flash_probe_driver(address, bus, cs, speed, mode);
	else
...
static int do_spi_flash_read_write(int argc, char *const argv[])
{
	...
	if (strcmp(argv[0], "update") == 0) {
		...
		show_speed(timer_diff,(unsigned long)len);
#endif
	}
