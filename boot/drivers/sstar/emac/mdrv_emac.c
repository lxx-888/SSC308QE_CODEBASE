...
//-------------------------------------------------------------------------------------------------
static int MDrv_EMAC_open(struct eth_device *nic, struct bd_info *bis)
{
    // struct emac_hw *hw = (struct emac_hw *) nic->priv;
    u32 uRegVal;
...
static int MDrv_EMAC_rx(struct eth_device *nic)
{
    ...
    {
        flush_dcache_range((unsigned long)RBQP_BASE + RAM_ADDR_BASE,
                           (unsigned long)RBQP_BASE + RAM_ADDR_BASE + RBQP_SIZE);
        p_recv = (unsigned char *)((((dlist->descriptors[hw->rxBuffIndex].addr)) + RAM_ADDR_BASE)
                                   & ~(EMAC_DESC_DONE | EMAC_DESC_WRAP));
        pktlen = (u32)dlist->descriptors[hw->rxBuffIndex].size & 0x7ff; /* Length of frame including FCS */

        addr_start = ALIGN_DOWN((u32)p_recv, CONFIG_SYS_CACHELINE_SIZE);
        addr_end   = (u32)p_recv + pktlen;
        addr_end   = ALIGN_UP(addr_end, CONFIG_SYS_CACHELINE_SIZE);
        flush_dcache_range(addr_start, addr_end);
...
#endif
        if ((unsigned char *)(dlist->recv_buf + RX_BUFFER_SIZE) > (p_recv + ROUND_SUP_4(pktlen)))
        {
        ...
        else
        {
            ...
            int firstPacketPartSize = (unsigned char *)dlist->recv_buf + RX_BUFFER_SIZE - p_recv;
        ...
        */
        ...
        else if (ThisBCE.loopback == MAC_LOOPBACK)
        {
            ...
            net_process_received_packet((uchar *)packet, pktlen);
...
#define RXD_PER_CACHELINE (CONFIG_SYS_CACHELINE_SIZE / RXD_SIZE)

static int MDrv_EMAC_rx_cacheline(struct eth_device *nic)
{
    struct emac_hw *hw = (struct emac_hw *)nic->priv;
    struct recv_desc_bufs *dlist = hw->dlist;
    int rxBuffIndex = hw->rxBuffIndex;
    int rxd_end;
    int i;
    int rxd_start;
    ...
    rxd_start = ALIGN_DOWN(rxBuffIndex, RXD_PER_CACHELINE);
    line_start = (unsigned long)&(dlist->descriptors[rxd_start]);
    invalidate_dcache_range(line_start, line_start + CONFIG_SYS_CACHELINE_SIZE);

    rxd_end = rxd_start + RXD_PER_CACHELINE;

    for (i = rxBuffIndex; i < rxd_end; i++)
    {
        if (!((dlist->descriptors[i].addr) & EMAC_DESC_DONE))
        {
            break;
        }
        p_recv =
            (unsigned char *)((((dlist->descriptors[i].addr)) + RAM_ADDR_BASE) & ~(EMAC_DESC_DONE | EMAC_DESC_WRAP));
        pktlen = (u32)dlist->descriptors[i].size & 0x7ff; /* Length of frame including FCS */

        addr_start = ALIGN_DOWN((u32)p_recv, CONFIG_SYS_CACHELINE_SIZE);
        addr_end = (u32)p_recv + pktlen;
        addr_end = ALIGN_UP(addr_end, CONFIG_SYS_CACHELINE_SIZE);
        invalidate_dcache_range(addr_start, addr_end);
    ...
    for (i = rxd_start; i < rxd_end; i++)
    {
        dlist->descriptors[i].addr &= ~EMAC_DESC_DONE; /* reset ownership bit */
    }
    flush_dcache_range(line_start, line_start + CONFIG_SYS_CACHELINE_SIZE);
    Chip_Flush_Memory();
    return 1;
}
...
static int MDrv_EMAC_rx(struct eth_device *nic)
{
    while (MDrv_EMAC_rx_cacheline(nic))
        ;
...
static char t_packet[2048] __attribute__((aligned(CONFIG_SYS_CACHELINE_SIZE)));
...
static int MDrv_EMAC_tx(struct eth_device *nic, /*volatile*/ void *tx_packet, int length)
{
    ...
    // flush_dcache_range((unsigned long)ptr, (unsigned long)ptr + sizeof(t_packet));
    flush_dcache_range((unsigned long)ptr, (unsigned long)ptr + ALIGN_UP(length, CONFIG_SYS_CACHELINE_SIZE));
    Chip_Flush_Memory();
...
void MDrv_EMAC_close(struct eth_device *nic)
{
    ...
    uRegVal =
        EMAC_INT_RCOM | EMAC_INT_RBNA | EMAC_INT_TUND | EMAC_INT_RTRY | EMAC_INT_TCOM | EMAC_INT_ROVR | EMAC_INT_HRESP;
...
static int MDrv_EMAC_HW_init(const struct eth_device *nic)
{
    ...
    printf("MAC Address %02X:%02X:%02X:%02X:%02X:%02X\n", ThisBCE.sa1[0], ThisBCE.sa1[1], ThisBCE.sa1[2],
           ThisBCE.sa1[3], ThisBCE.sa1[4], ThisBCE.sa1[5]);
    // Set MAC address ------------------------------------------------------
    MHal_EMAC_Write_SA1_MAC_Address(ThisBCE.sa1[0], ThisBCE.sa1[1], ThisBCE.sa1[2], ThisBCE.sa1[3], ThisBCE.sa1[4],
                                    ThisBCE.sa1[5]);
    MHal_EMAC_Write_SA2_MAC_Address(ThisBCE.sa2[0], ThisBCE.sa2[1], ThisBCE.sa2[2], ThisBCE.sa2[3], ThisBCE.sa2[4],
                                    ThisBCE.sa2[5]);
    MHal_EMAC_Write_SA3_MAC_Address(ThisBCE.sa3[0], ThisBCE.sa3[1], ThisBCE.sa3[2], ThisBCE.sa3[3], ThisBCE.sa3[4],
                                    ThisBCE.sa3[5]);
    MHal_EMAC_Write_SA4_MAC_Address(ThisBCE.sa4[0], ThisBCE.sa4[1], ThisBCE.sa4[2], ThisBCE.sa4[3], ThisBCE.sa4[4],
                                    ThisBCE.sa4[5]);
...
#ifdef CONFIG_SSTAR_ETHERNET_FIXLINK
    ThisBCE.duplex = 2;
...
// EMAC init Variable
//-------------------------------------------------------------------------------------------------
static volatile u8 emac_var[EMAC_ABSO_MEM_SIZE] __attribute__((aligned(CONFIG_SYS_CACHELINE_SIZE)));
u32                MDrv_EMAC_VarInit(void)
{
    ...
    {
        printf("MMAP overflow: %x > %x\n", (RBQP_BASE + RBQP_SIZE + EMAC_MEM_GAP),
               (alloRAM_ADDR_BASE + EMAC_ABSO_MEM_SIZE));
        return 0;
    }
...
/**************************************************************************
  Divided PCLK to general MDC not exceed 2.5 MHZ
  call this function before phy scan
 ***************************************************************************/
void MDrv_EMAC_MdcStandardization(void)
{
    u32 word_ETH_CFG = 0x00000C00;
...
#ifdef CONFIG_SSTAR_ETHERNET_ICPLUS
extern int ip101a_g_config_init(void);
#endif

int MDrv_EMAC_initialize(const struct bd_info *bis)
{
    struct eth_device *nic           = NULL;
    struct emac_hw *   hw            = NULL;
    char *             phy_id_string = NULL;

    phy_id_string = env_get("PHY_ID");
    ...
    MDrv_EMAC_MdcStandardization();

#ifndef CONFIG_SSTAR_ETHERNET_FIXLINK
...
int board_eth_init(const struct bd_info *bis)
{
    if (env_get_yesno("autoestart") != 0)
    {
#ifdef CONFIG_SSTAR_EMAC1
        MHal_EMAC_PHYHW_reset();
#endif
