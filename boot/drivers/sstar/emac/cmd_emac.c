...
extern int  MDrv_EMAC_initialize(struct bd_info *bis);
...
int do_emac(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    struct bd_info *bis = NULL;
#if 0
	char * macaddr;

	if (   NULL != (macaddr = env_get(MACADDR_ENV))
...
int do_ewavetest(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
...
int do_setMac(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    sprintf(cmdline_buf, "env_set -f " MACADDR_ENV " %02X:%02X:%02X:%02X:%02X:%02X", MY_MAC[0], MY_MAC[1], MY_MAC[2],
            MY_MAC[3], MY_MAC[4], MY_MAC[5]);
    run_command(cmdline_buf, 0);
    free(cmdline_buf);
    // run_command("saveenv", 0);

    printf("MAC Address %02X:%02X:%02X:%02X:%02X:%02X\n", MY_MAC[0], MY_MAC[1], MY_MAC[2], MY_MAC[3], MY_MAC[4],
           MY_MAC[5]);
...
int        do_readPhy(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    unsigned char phyId;
    ...
    MHal_EMAC_read_phy(phyId, phyAddress, &phyValue);

    printf("phy read address[%x] value is %x\n", phyAddress, phyValue);
    return 0;
}

int do_writePhy(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    unsigned char phyId;
    unsigned char phyAddress;
    unsigned int  phyValue;
    ...
    MHal_EMAC_write_phy(phyId, phyAddress, phyValue);
    return 0;
}

extern void MDrv_EMAC_DumpReg(void);
int         do_dumpMac(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    if (!emac_init)
    {
        printf("Please start emac first!\n");
...
int        do_bistMac(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    else
    {
        u8   idx = 0;
        char buffer[256],
            ip_addr[][15] = {"10.0.0.253", "127.0.0.1", "172.16.90.254", "192.168.0.1", "224.30.0.253", "0"};
...
int do_Loopback(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    if (!emac_init)
    {
        printf("Please start emac first!\n");
...
int do_loadspi(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    ulong ram_start;
    ulong spi_start;
    ulong len, i;
...
int do_emacpowerdown(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
...
U_BOOT_CMD(estart, EMAC_INPUT_LEN, 1, do_emac, "EMAC start",
           "reset  - reset EMAC controller\n"
           "start  - start EMAC controller\n");

U_BOOT_CMD(macaddr, EMAC_ADDR_INPUT_LEN, 1, do_setMac, "setup EMAC MAC addr", "XX XX XX XX XX XX");

U_BOOT_CMD(phy_r, EMAC_ADDR_INPUT_LEN, 1, do_readPhy, "phy read", "phyAddress");

U_BOOT_CMD(phy_w, EMAC_ADDR_INPUT_LEN, 1, do_writePhy, "phy write", "phyAddress phyValue");

#ifdef CONFIG_MINIUBOOT
#else
U_BOOT_CMD(ewavetest, EMAC_INPUT_LEN, 1, do_ewavetest, "EMAC wave test",
           "1  - 100M\n"
           "2  - 10M\n");

U_BOOT_CMD(ebist, EMAC_INPUT_LEN, 1, do_bistMac, "PHY loopback test", "ebist - PHY loopback test\n");

U_BOOT_CMD(edump, EMAC_INPUT_LEN, 1, do_dumpMac, "EMAC Register settings dump",
           "edump - EMAC Register settings dump\n");

U_BOOT_CMD(eloopback, EMAC_INPUT_LEN, 1, do_Loopback, "Long loopback test", "eloopback - Long loopback test\n");

U_BOOT_CMD(loadspi, 4, 4, do_loadspi, "load data from SPI", "XX XX XX XX XX XX");

U_BOOT_CMD(epd, 4, 4, do_emacpowerdown, "emac power down", "XX XX XX XX XX XX");
#endif
