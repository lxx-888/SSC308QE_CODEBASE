...
s32 MHal_EMAC_CableConnection(void)
{
...
#else
    MHal_EMAC_read_phy(phy_id, PHY_REG_BASIC, &bcrm);
    speed = ((bcrm & BIT(6)) >> 5) | ((bcrm & BIT(13)) >> 13);
    // printf("[%s][%d] PHY_REG_BASIC = 0x%08x, 0x%x\n", __FUNCTION__, __LINE__, bcrm, speed);
    switch (speed)
    {
        default:
        case 0x3:
        case 0x2:
            printf("[%s][%d] speed not support %d\n", __FUNCTION__, __LINE__, speed);
        case 0x1:
            MHalThisBCE.speed = SPEED_100;
            break;
        case 0x0:
    ...
    MHalThisBCE.duplex = !!(bcrm & BIT(8));
    if (SPEED_100 == MHalThisBCE.speed)
        retValue = (MHalThisBCE.duplex) ? 4 : 3;
    else
        retValue = (MHalThisBCE.duplex) ? 2 : 1;
#endif
    return (retValue);
}
...
#ifdef CONFIG_SSTAR_VERSION_FPGA
    MHal_EMAC_read_phy(phy_id, MII_ADVERTISE, &adv);
    adv &= ~(ADVERTISE_100HALF | ADVERTISE_100FULL | ADVERTISE_100BASE4);
    MHal_EMAC_write_phy(phy_id, MII_ADVERTISE, adv);
    MHal_EMAC_write_phy(phy_id, MII_CTRL1000, 0x0000UL);
    mdelay(10);
#endif

    MHal_EMAC_write_phy(phy_id, PHY_REG_BASIC, 0x1200);
...
void MHal_EMAC_Set_Tx_Wrong_Packet_Fix(u32 xval)
{
    u32 value;
    value = MHal_EMAC_ReadReg32(0x138);
    value &= 0xfffffffd;
    value |= xval << 1;
    MHal_EMAC_WritReg32(0x138, value);

    value = MHal_EMAC_ReadReg32(0x1d2);
    value &= 0xfffffffe;
    value |= xval << 0;
    MHal_EMAC_WritReg32(0x1d2, value);
}
...
void MHal_EMAC_ClkGen(void)
{
    ...
    */
    ...
    uRegVal = MHal_EMAC_ReadReg8(REG_BANK_ALBANY2, 0xa1);
    if (uRegVal == 0x80)
    {
...
void MHal_EMAC_Power_On_Clk(void)
{
    ...
    // pad mux
    {
        ...
        // reg_bt1120_mode; reg[103CDC]#1~#0=2b
        uRegVal = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdc);
        uRegVal &= 0xfc;
        uRegVal |= 0x02;
        MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xdc, uRegVal);
...
void MHal_EMAC_HW_init(void)
{
    ...
    MHal_EMAC_Set_Tx_Wrong_Packet_Fix(1);
    ...
    // disable tx/rx mcg at bring up stage
    {
        u8 uRegVal;

        uRegVal = MHal_EMAC_ReadReg8(REG_BANK_EMAC1, 0xF2);
        uRegVal &= ~(0x5);
        MHal_EMAC_WritReg8(REG_BANK_EMAC1, 0xF2, uRegVal);
    }
}
