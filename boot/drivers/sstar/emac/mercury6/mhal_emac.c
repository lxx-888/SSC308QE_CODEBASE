...
#include "../../gpio/mercury6/gpio.h"
#include "../../gpio/mercury6/padmux.h"
...
#define EMAC_MII_MODE  1
#define EMAC_RMII_MODE 2

#ifdef CONFIG_SSTAR_ETHERNET_ALBANY
static u8 m_bus_mode = EMAC_MII_MODE;
#else
static u8 m_bus_mode = EMAC_RMII_MODE;
#endif
...
{
    if (m_bus_mode == EMAC_MII_MODE)
    {
        xval = JULIAN_100_VAL_MII;
    }
    else
    {
        xval = JULIAN_100_VAL_RMII;
    }
...
void MHal_EMAC_write_phy(unsigned char phy_addr, unsigned char address, u32 value)
{
    ...
    if (m_bus_mode == EMAC_MII_MODE)
    {
        uRegVal = (EMAC_HIGH | EMAC_CODE_802_3 | EMAC_RW_W) | ((phy_addr & 0x1f) << PHY_ADDR_SHIFT)
                  | (address << PHY_REG_SHIFT) | (value & 0xFFFF);
    }
    else
    {
...
void MHal_EMAC_read_phy(unsigned char phy_addr, unsigned char address, u32 *value)
{
    ...
    if (m_bus_mode == EMAC_MII_MODE)
    {
        uRegVal = (EMAC_HIGH | EMAC_CODE_802_3 | EMAC_RW_R) | ((phy_addr & 0x1f) << PHY_ADDR_SHIFT)
                  | (address << PHY_REG_SHIFT) | (0);
    }
    else
    {
        *value  = 0xffff;
        uRegVal = (EMAC_HIGH | EMAC_CODE_802_3 | EMAC_RW_R) | ((phy_addr & 0x1f) << PHY_ADDR_OFFSET)
                  | (address << PHY_REGADDR_OFFSET) | (0);
    }
...
void MHal_EMAC_ClkGen(void)
{
    ...
    // emac_clk gen
#ifdef CONFIG_SSTAR_EMAC1
#ifdef CONFIG_SSTAR_ETHERNET_RMII
    MHal_EMAC_WritReg8(REG_BANK_CLKGEN0, 0x84, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x54, 0x04);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x55, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x56, 0x04);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x57, 0x00);
#else
    MHal_EMAC_WritReg8(REG_BANK_CLKGEN0, 0x84, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x54, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x55, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x56, 0x00);
    MHal_EMAC_WritReg8(REG_BANK_SCGPCTRL, 0x57, 0x00);
#endif
#else
...
void MHal_EMAC_Power_On_Clk(void)
{
    ...
    if (m_bus_mode == EMAC_MII_MODE)
    {
        ...
        // pad mux
        {
            ...
            // reg_led0/1_mode; reg[103CC6]#6 ~ #4/#2 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xc6);
            uRegVal1 &= 0x77;
            uRegVal1 |= 0x22;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xc6, uRegVal1);
        }
    }
    else
    {
        // pad mux
        {
#ifdef CONFIG_SSTAR_QFN
            u8 uRegVal1;

            // all pad in
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_CHIPTOP, 0xa1);
            uRegVal1 &= 0x7f;
            MHal_EMAC_WritReg8(REG_BANK_CHIPTOP, 0xa1, uRegVal1);

            // reg_eth_mode; reg[103CDC]#3 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdc);
            uRegVal1 &= 0xF0;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xdc, uRegVal1);

            // reg_eth1_mode; reg[103CDC]#7 ~ #4
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdc);
            uRegVal1 &= 0x0F;
            uRegVal1 |= 0x30;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xdc, uRegVal1);
#elif CONFIG_SSTAR_LQFP
            u8 uRegVal1;
...
#else
#if 0
			printf("[%s][%d], Set IO MUX to PINMUX_FOR_ETH1_MODE_4!\n", __FUNCTION__, __LINE__);
			//sstar_gpio_padgroupmode_set(PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL18, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL19, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL20, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL21, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL22, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL23, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL24, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL25, PINMUX_FOR_ETH1_MODE_4);
			sstar_gpio_pad_val_set(PAD_TTL26, PINMUX_FOR_ETH1_MODE_4);
#else
            u8 uRegVal1, uRegVal2;
            ...
            // reg_ej_mode; reg[103CC1]#1 ~ reg[103CC0]#7
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xc0);
            uRegVal2 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xc1);
            if (((uRegVal1 & 0x80) == 0x80) && ((uRegVal2 & 0x03) == 0x02))
            {
                uRegVal1 &= 0x7F;
                uRegVal2 &= 0xFC;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xc0, uRegVal1);
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xc1, uRegVal2);
            }

            // reg_dla_ej_mode; reg[103CE2]#2 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xe2);
            if ((uRegVal1 & 0x07) == 0x03)
            {
                uRegVal1 &= 0xF8;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xe2, uRegVal1);
            }

            // reg_i2c0_mode; reg[103CDE]#2 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xde);
            if ((uRegVal1 & 0x07) == 0x05)
            {
                uRegVal1 &= 0xF8;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xde, uRegVal1);
            }

            // reg_i2c5_mode; reg[103CE7]#3 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xe7);
            if ((uRegVal1 & 0x0F) == 0x05)
            {
                uRegVal1 &= 0xF0;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xe7, uRegVal1);
            }

            // reg_spi2_mode; reg[103CD1]#2 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xd1);
            if ((uRegVal1 & 0x07) == 0x03)
            {
                uRegVal1 &= 0xF8;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xd1, uRegVal1);
            }

            // reg_fuart_mode; reg[103CDD]#3 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdd);
            if ((uRegVal1 & 0x0F) == 0x08)
            {
                uRegVal1 &= 0xF0;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xdd, uRegVal1);
            }

            // reg_uart1_mode; reg[103CDA]#6 ~ #4
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xda);
            if ((uRegVal1 & 0x70) == 0x50)
            {
                uRegVal1 &= 0x8F;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xda, uRegVal1);
            }

            // reg_uart2_mode; reg[103CDB]#2 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdb);
            if ((uRegVal1 & 0x07) == 0x05)
            {
                uRegVal1 &= 0xF8;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xdb, uRegVal1);
            }

            // reg_uart3_mode; reg[103CDB]#6 ~ #4
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xdb);
            if ((uRegVal1 & 0x70) == 0x50)
            {
                uRegVal1 &= 0x8F;
            ...
            // reg_emmc1_8b_mode; reg[103CC3]#1 ~ #0
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xc3);
            if ((uRegVal1 & 0x03) == 0x03)
            {
                uRegVal1 &= 0xFC;
                MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xc3, uRegVal1);
            }

            // disable pwm
            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xca);
            uRegVal1 &= 0x88;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xca, uRegVal1);

            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xcb);
            uRegVal1 &= 0x88;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xcb, uRegVal1);

            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xcc);
            uRegVal1 &= 0x88;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xcc, uRegVal1);

            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xcd);
            uRegVal1 &= 0x88;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xcd, uRegVal1);

            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xce);
            uRegVal1 &= 0x88;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xce, uRegVal1);

            uRegVal1 = MHal_EMAC_ReadReg8(REG_BANK_PADTOP, 0xe8);
            uRegVal1 &= 0xF8;
            MHal_EMAC_WritReg8(REG_BANK_PADTOP, 0xe8, uRegVal1);
...
#ifdef CONFIG_SSTAR_EMAC1
void MHal_EMAC_PHYHW_reset(void)
{
    printf("\n[%s][%d] - Use PAD_TTL27 to reset ePhy!\n", __FUNCTION__, __LINE__);
    sstar_gpio_pad_oen(PAD_TTL27);
    sstar_gpio_pull_high(PAD_TTL27);
    mdelay(1);
    sstar_gpio_pull_low(PAD_TTL27);
    mdelay(1);
    sstar_gpio_pull_high(PAD_TTL27);
    mdelay(1);
}
