...
const u16  u16PadmuxOff[PWM_NUM]   = {0x0065, 0x0065, 0x0065, 0x0065, 0x0066, 0x0066,
                                   0x0066, 0x0066, 0x0067, 0x0067, 0x0074, 0x0045};
const u16  u16PadmuxMask[PWM_NUM]  = {0x0007, 0x0070, 0x0700, 0x7000, 0x0007, 0x0070,
                                    0x0700, 0x7000, 0x0007, 0x0070, 0x0007, 0x0700};
const u16  u16PadmuxShift[PWM_NUM] = {0x0000, 0x0004, 0x0008, 0x000C, 0x0000, 0x0004,
                                     0x0008, 0x000C, 0x0000, 0x0004, 0x0000, 0x0008};
...
void HalPwmEnable(struct sstar_pwm_chip *chip, u8 u8Id, u8 u8Val)
{
    ...
    if (u8Val)
    {
        if (pwm_duty_ns[u8Id])
        {
            CLRREG16(ulPwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id));
        }
        else
        {
            SETREG16(ulPwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id));
        }
    }
    else
    {
