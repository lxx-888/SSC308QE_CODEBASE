...
int hal_pwm_set_div(struct hal_pwm_cfg *pwm_cfg, u64 period_wf, u8 sync)
{
    ...
    {
        period_max = (pwm_clk_div[i] * 262144 / pwm_cfg->clk_freq) * pwm_cfg->hz_to_ns;
...
period_set:
    ...
    {
#ifdef CONFIG_SSTAR_PWM_EXTEND
        duty_wf = (period_wf * (pwm_cfg->pwm_attr.duty_wf * 100 / pwm_cfg->pwm_attr.period_wf) / 100);
#else
    ...
    {
#ifdef CONFIG_SSTAR_PWM_EXTEND
        shift_wf = (period_wf * (pwm_cfg->pwm_attr.shift_wf * 100 / pwm_cfg->pwm_attr.period_wf) / 100);
#else
...
int hal_pwm_set_duty(struct hal_pwm_cfg *pwm_cfg, u64 duty_wf, u8 sync)
{
    ...
    else if (duty_reg >= (period_reg * div_reg))
    {
...
int hal_pmw_set_shift(struct hal_pwm_cfg *pwm_cfg, u64 shift_wf, u8 sync)
{
    ...
    else if (shift_reg >= (period_reg * div_reg))
    {
