...
static int hal_mspi_full_duplex_buf(struct hal_mspi *hal, u8 *rx_buff, u8 *tx_buff, u16 size)
{
    ...
    int ret        = HAL_MSPI_OK;
    u8  shift      = 0;
    u8  w_msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_RLSB_FIRST) & HAL_MSPI_REG_RLSB_FIRST_BIT);
    u8  r_msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_WLSB_FIRST) & HAL_MSPI_REG_WLSB_FIRST_BIT);
...
int hal_mspi_dma_write(u8 ch, struct hal_mspi *hal, u8 *data, u32 size)
{
    ...
    memset(&bdma_para, 0, sizeof(HalBdmaParam_t));
    ...
    if (HAL_BDMA_PROC_DONE != HalBdma_DoTransfer((u8)dma_ch, &bdma_para))
    {
