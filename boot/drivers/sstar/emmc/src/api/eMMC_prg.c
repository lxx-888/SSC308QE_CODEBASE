...
extern int add_emmc_partitions(struct blk_desc *dev_desc, struct disk_partition *info);

#if defined(UNIFIED_eMMC_DRIVER) && UNIFIED_eMMC_DRIVER

static U32 eMMC_ReadPartitionInfo_Ex(void);
static U32 eMMC_GetPartitionIndex(U16 u16_PartType, U32 u32_LogicIdx, volatile U16 *pu16_PartIdx);
...
LABEL_BOOT_MODE_START:
    ...
    while (u32_i < u32_ItemCnt)
    {
        ...
        u32_err = eMMC_FCIE_PollingEvents(FCIE_MIE_EVENT, BIT_MIU_LAST_DONE, HW_TIMER_DELAY_1s);
    ...
    u32_err = eMMC_FCIE_PollingEvents(FCIE_MIE_EVENT, BIT_CARD_BOOT_DONE, HW_TIMER_DELAY_1s);
    if (u32_err)
        goto LABEL_LOAD_IMAGE_END;
...
U32 eMMC_CheckCIS(eMMC_CIS_t *ptCISData)
{
    ...
    u32_ChkSum = eMMC_ChkSum((U8 *)&peMMCInfo->u16_SpareByteCnt,
                             (U16)((U32)(&peMMCInfo->u16_SeqAccessTime) - (U32)(&peMMCInfo->u16_SpareByteCnt)));
...
U32 eMMC_WriteCIS(eMMC_CIS_t *ptCISData)
{
    ...
    for (u16_i = 0; u16_i < eMMC_CIS_NNI_BLK_CNT; u16_i++)
    {
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: write NNI %u fail, %Xh\n", u16_i, u32_err);
            return u32_err;
        }
    }
    ...
    for (u16_i = 0; u16_i < eMMC_CIS_PNI_BLK_CNT; u16_i++)
    {
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: write PNI %u fail, %Xh\n", u16_i, u32_err);
            return u32_err;
        }
    }
...
U32 eMMC_FATAutoSize(void)
{
    ...
    for (u16_i = 0; u16_i < pPartInfo->u16_PartCnt; u16_i++)
    {
        if (eMMC_PART_FAT == pPartInfo->records[u16_i].u16_PartType)
        {
            pPartInfo->records[u16_i].u16_BlkCnt = g_eMMCDrv.u32_FATSectorCnt / pPartInfo->u16_BlkPageCnt;
...
static U32 eMMC_GetPartitionIndex(U16 u16_PartType, U32 u32_LogicIdx, volatile U16 *pu16_PartIdx)
{
    ...
    for (*pu16_PartIdx = 0; *pu16_PartIdx < pPartInfo->u16_PartCnt; *pu16_PartIdx += 1)
    {
        printf("pPartInfo->records[%X].u16_PartType : %X \r\n", *pu16_PartIdx,
               pPartInfo->records[*pu16_PartIdx].u16_PartType);
...
U32 eMMC_GetPartitionCapacity(U16 u16_PartType, U32 *pu32_Cap)
{
    ...
    {
        u32_err = eMMC_GetPartitionIndex(u16_PartType, 0, &u16_PartIdx);
        if (eMMC_ST_SUCCESS != u32_err)
            return u32_err;
...
U32 eMMC_ReadPartition(U16 u16_PartType, U8 *pu8_DataBuf, U32 u32_StartSector, U32 u32_SectorCnt, U32 u32_LogicIdx)
{
    ...
    u32_err = eMMC_GetPartitionIndex(u16_PartType, u32_LogicIdx, &u16_PartIdx);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
    ...
    // read data
    u32_err = eMMC_ReadData(pu8_DataBuf, u32_SectorCnt << eMMC_SECTOR_512BYTE_BITS,
...
U32 eMMC_WritePartition(U16 u16_PartType, U8 *pu8_DataBuf, U32 u32_StartSector, U32 u32_SectorCnt, U32 u32_LogicIdx)
{
    U32          u32_err, u32_PartSecCnt;
    volatile U16 u16_PartIdx;
    eMMC_PNI_t * pPartInfo = (eMMC_PNI_t *)gau8_eMMC_PartInfoBuf;
    ...
    U32 u32tmp;
    u32tmp = pPartInfo->records[u16_PartIdx].u16_StartBlk * pPartInfo->u16_BlkPageCnt + u32_StartSector;

    printf("u32_SectorCnt : %X, startSector : %X\r\n", u32_SectorCnt, u32tmp);

    u32_err =
        eMMC_WriteData(pu8_DataBuf, u32_SectorCnt << eMMC_SECTOR_512BYTE_BITS,
                       pPartInfo->records[u16_PartIdx].u16_StartBlk * pPartInfo->u16_BlkPageCnt + u32_StartSector);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_ErasePartition(U16 u16_PartType)
{
    ...
    {
        u32_err = eMMC_GetPartitionIndex(u16_PartType, u32_PartLogiIdx, &u16_PartIdx);
        if (eMMC_ST_SUCCESS != u32_err)
        {
...
U32 eMMC_EraseAllPartitions(void)
{
    ...
    while (1) // find all partition copies
    {
        u32_err = eMMC_GetPartitionIndex(eMMC_PART_NVRAMBAK, u32_PartLogiIdx, &u16_PartIdx_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
            break;
...
U32 eMMC_EraseBootPart(U32 u32_eMMCBlkAddr_start, U32 u32_eMMCBlkAddr_end, U8 u8_PartNo)
{
    ...
    else
    {
        ...
        for (u32_i = 0; u32_i < u32_SectorCnt; u32_i++)
        {
            ...
            u32_err = eMMC_WriteData(gau8_eMMC_SectorBuf, u32_j, u32_eMMCBlkAddr_start + u32_i);
            if (eMMC_ST_SUCCESS != u32_err)
            {
        ...
        // erase blocks
        u32_i = (u32_eMMCBlkAddr_end - (u32_eMMCBlkAddr_start + u32_SectorCnt)) / g_eMMCDrv.u32_EraseUnitSize;
        if (u32_i)
        {
            u32_err = eMMC_EraseBlock((u32_eMMCBlkAddr_start + u32_SectorCnt),
        ...
        u32_eMMCBlkAddr_start = (u32_eMMCBlkAddr_start + u32_SectorCnt) + u32_i * g_eMMCDrv.u32_EraseUnitSize;
...
U32 eMMC_WriteBootPart(U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    ...
    u32_err = eMMC_WriteData(pu8_DataBuf, u16_SecCnt << eMMC_SECTOR_512BYTE_BITS, u32_BlkAddr);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_ReadBootPart(U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    ...
    u32_err = eMMC_ReadData(pu8_DataBuf, u16_SecCnt << eMMC_SECTOR_512BYTE_BITS, u32_BlkAddr);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
int eMMC_mmc_Init(struct mmc *mmc)
{
    ...
    if (!mmc->ext_csd)
        mmc->ext_csd = (U8 *)malloc(eMMC_SECTOR_512BYTE);
    if (!mmc->ext_csd)
    {
        printf("Memory allocate for ext_csd failed\n");
        return -ENOMEM;
    }
    ...
    {
        mmc->slc_size =
            ((u64)(mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT_0] | ((u32)(mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT_1]) << 8)
                   | ((u32)(mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT_2]) << 16))
             << 19)
            * mmc->ext_csd[221] * mmc->ext_csd[224];
        mmc->max_slc_size =
            ((u64)(mmc->ext_csd[157] | ((u32)(mmc->ext_csd[158]) << 8) | ((u32)(mmc->ext_csd[159]) << 16)) << 19)
            * mmc->ext_csd[221] * mmc->ext_csd[224];
    ...
    if ((mmc->ext_csd[EXT_CSD_PARTITIONING_SUPPORT] & PART_SUPPORT) || mmc->ext_csd[EXT_CSD_BOOT_MULT])
        mmc->part_config = mmc->ext_csd[EXT_CSD_PART_CONF];

    printf("mmc->part_config : %Xh\n", mmc->part_config);
    {
        U16 u16_i;
        for (u16_i = 0; u16_i < 4; u16_i++)
        {
            if (u16_i < 3)
                mmc->cid[u16_i] = g_eMMCDrv.au8_CID[u16_i * 4 + 1] << 24 | g_eMMCDrv.au8_CID[u16_i * 4 + 2] << 16
    ...
    sprintf(mmc->block_dev.product, "%c%c%c%c%c", mmc->cid[0] & 0xff, (mmc->cid[1] >> 24), (mmc->cid[1] >> 16) & 0xff,
...
#endif
#else
    //    init_part(&mmc->block_dev);	//this will re-init dev,then status are error
#endif
...
static int eMMC_send_cmd(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
    ...
    switch (cmd->cmdidx)
    {
        case MMC_CMD_GO_IDLE_STATE:
            if (eMMC_CMD0(cmd->cmdarg) == eMMC_ST_SUCCESS)
                return 0;
            else
                return -1;
            break;
        ...
        case 8: /* SDIO */
            ...
            break;
        case 16:
            if (eMMC_CMD16(cmd->cmdarg) == eMMC_ST_SUCCESS)
                return 0;
            else
                return -1;
            break;
        case 17:
            if (!data || !(data->src))
                printf("[%s] param err !\n", __FUNCTION__);
            if (eMMC_CMD17(cmd->cmdarg, (U8 *)data->dest) == eMMC_ST_SUCCESS)
                return 0;
            else
                return -1;
            break;
        case 24:
            ...
            if (eMMC_CMD24(cmd->cmdarg, (U8 *)data->src) == eMMC_ST_SUCCESS)
                return 0;
            else
                return -1;
            break;
...
static const struct mmc_ops ms_eMMC_ops = {
    .send_cmd = eMMC_send_cmd, .set_ios = eMMC_set_ios, .init = eMMC_core_init,
    //  .getcd      = NULL
};

int board_emmc_init(struct bd_info *bis)
{
    struct mmc *       mmc     = NULL;
...
LABEL_INIT_END:
    ...
    g_eMMCDrv.au8_ID[eMMC_ID_FROM_CID_BYTE_CNT] = (g_eMMCDrv.u32_SEC_COUNT >> (1 + 10 + 10)) + 1;
...
U32 eMMC_EraseBlock(U32 u32_eMMCBlkAddr_start, U32 u32_eMMCBlkAddr_end)
{
    ...
    else
    {
        ...
        if (u32_i)
        {
            u32_err = eMMC_EraseCMDSeq((u32_eMMCBlkAddr_start + u32_SectorCnt),
                                       (u32_eMMCBlkAddr_start + u32_SectorCnt) + u32_i * g_eMMCDrv.u32_EraseUnitSize);
            if (eMMC_ST_SUCCESS != u32_err)
            {
