...
U32 eMMCTest_SgW_MIU(U32 u32_eMMC_Addr)
{
    ...
    REG_FCIE_W(FCIE_MIU_DMA_ADDR_15_0, u32_dmaaddr & 0xFFFF);
    REG_FCIE_W(FCIE_MIU_DMA_ADDR_31_16, (u32_dmaaddr >> 16) & 0XFFFF);
    ...
    if (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR))
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD25 Reg.12: %04Xh, Err: %Xh,  Arg: %Xh\r\n", u16_reg,
                   u32_err, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
    ...
    if (eMMC_ST_SUCCESS != u32_err)
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD25 check R1 error: %Xh\r\n", u32_err);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
    ...
    {
        u32_err = eMMC_ST_ERR_CMD25;
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 0, "eMMC Err: CMD25 Reg.12: %04Xh, Err: %Xh\r\n", u16_reg, u32_err);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
U32 eMMCTest_SgR_MIU(U32 u32_eMMC_Addr)
{
    ...
    zdec_top_fpga_feed(1, // last
                       p_ZDecDesc_st, sizeof(struct _ZDecDescriptor) * u32_i);
    ...
    if ((eMMC_ST_SUCCESS != u32_err) || (u16_reg & BIT_SD_FCIE_ERR_FLAGS))
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 0, "eMMC Err: CMD18 Reg.12: %04Xh, Err: %Xh\r\n", u16_reg, u32_err);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
    ...
    {
        u32_err = eMMC_ST_ERR_CMD18;
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD18 Reg.12: %04Xh, Err: %Xh\n", u16_reg, u32_err);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
    ...
    {
        u32_err = eMMC_ST_ERR_CMD18;
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD18 check R1 error: %Xh\r\n", u32_err);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
U32 eMMCTest_MultiBlkWRC_MIU(U32 u32_eMMC_Addr, U16 u16_BlkCnt, U32 u32_DataPattern)
{
    ...
    {
        u32_err = eMMC_CMD25_MIU(u32_eMMC_Addr + u32_i, gau8_WBuf + (u32_i << eMMC_SECTOR_512BYTE_BITS),
        ...
        u32_err = eMMC_CMD18_MIU(u32_eMMC_Addr + u32_i, gau8_RBuf + (u32_i << eMMC_SECTOR_512BYTE_BITS),
                                 u16_BlkCnt_tmp - u32_i);
        if (eMMC_ST_SUCCESS != u32_err)
        {
...
U32 eMMCTest_Write_Timeout_Error(void)
{
    ...
    REG_FCIE_W(FCIE_MIE_FUNC_CTL, 0x1);

    eMMC_CMD25_MIU(eMMC_TEST_BLK_0, gau8_WBuf, 2);

    REG_FCIE_W(FCIE_MIE_FUNC_CTL, 0x4);
    g_eMMCDrv.u8_make_sts_err = 0;
...
void eMMC_Prepare_Power_Saving_Mode_Queue(void)
{
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE, 0x1D), PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WINT);

    /* (16) STOP */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE, 0x1E), 0x0000);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE, 0x1F), PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_STOP);
...
#if (defined(WRITE_TEST_SQUASHFS_IMAGE) && WRITE_TEST_SQUASHFS_IMAGE) || (defined(FCIE_ZDEC_TEST) && FCIE_ZDEC_TEST)
...
U8 *squashfs_test_data[5] = {
    squashfs_test_1_96_35731_131072_bin,     squashfs_test_2_35827_19864_70326_bin,
    squashfs_test_3_72094_29571_110762_bin,  squashfs_test_4_107474_13776_72656_bin,
...
U32 eMMC_IPVerify_Main(void)
{
    ...
    while (1)
    {
...
#endif
        {
            ...
            {
                eMMC_pads_switch(FCIE_eMMC_BYPASS_8X);
            }

            eMMC_debug(eMMC_DEBUG_LEVEL, 1, "\033[7;33meMMC IP GPIO pad: %s Mode Bus Width: %d\033[m\r\n\r\n",
                       (g_eMMCDrv.u8_PadType == FCIE_eMMC_BYPASS || g_eMMCDrv.u8_PadType == FCIE_eMMC_BYPASS_8X)
                           ? "BYPASS"
                           : "SDR(GPIO)",
                       u8_BusWidth[u8_i]);
            ...
            eMMC_debug(eMMC_DEBUG_LEVEL, 1, "\033[7;32meMMC IP GPIO pad: %s Mode Bus Width:%d Verify ok\033[m\r\n\r\n",
                       (g_eMMCDrv.u8_PadType == FCIE_eMMC_BYPASS) ? "BYPASS" : "SDR(GPIO)", u8_BusWidth[u8_i]);
        }
...
#if ((defined(FCIE_eMMC5_1_FLASH_MACRO32_VERIFY) && FCIE_eMMC5_1_FLASH_MACRO32_VERIFY) \
     && (defined(ENABLE_AFIFO) && ENABLE_AFIFO))
#ifdef ONLY_4_BITS_MODE
        for (u8_i = 2; u8_i < 2; u8_i++)
#else
        for (u8_i = 2; u8_i < 3; u8_i++)
