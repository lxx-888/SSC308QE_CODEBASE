...
#define BASE_REG_NET_GP_CTRL_PA     (ulong) GET_BASE_ADDR_BY_BANK(SSTAR_BASE_REG_RIU_PA, 0x127200)
#define BASE_REG_X32_GMAC0_PA       (ulong) GET_BASE_ADDR_BY_BANK(SSTAR_BASE_REG_RIU_PA, 0x195000)
#define BASE_REG_X32_GMAC1_PA       (ulong) GET_BASE_ADDR_BY_BANK(SSTAR_BASE_REG_RIU_PA, 0x195A00)
#define BASE_REG_GMAC0_PA           (ulong) GET_BASE_ADDR_BY_BANK(SSTAR_BASE_REG_RIU_PA, 0x127000)
#define BASE_REG_GMAC1_PA           (ulong) GET_BASE_ADDR_BY_BANK(SSTAR_BASE_REG_RIU_PA, 0x127100)
...
u32 GMAC_Driving_Offset[GMAC_NUM][GMAC_MAX_IO] = {
    {MDRV_PUSE_ETH0_MDC, MDRV_PUSE_ETH0_MDIO, MDRV_PUSE_ETH0_TX_CLK, MDRV_PUSE_ETH0_TXD0, MDRV_PUSE_ETH0_TXD1,
     MDRV_PUSE_ETH0_TXD2, MDRV_PUSE_ETH0_TXD3, MDRV_PUSE_ETH0_TX_CTL},
    {MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA, MDRV_PUSE_NA}};
...
static void mhal_gmac_gpio_reset(uint8_t gmacId, int interface, int reset_io)
{
    if (gmacId == GMAC0)
    {
        ...
        {
            // Output Low
            sstar_gpio_set_low(PAD_RGMII0_RSTN);
            mdelay(20);
            // Output High
            sstar_gpio_set_high(PAD_RGMII0_RSTN);
            mdelay(50);
        }
        else
#endif
        ...
        {
            // Output Low
            sstar_gpio_pad_set(PAD_RGMII0_RSTN);
            sstar_gpio_set_low(PAD_RGMII0_RSTN);
            OUTREGMSK16(GET_REG_ADDR(BASE_REG_GMAC0_PA, 0x31), 0, BIT1); // free sw reset
            mdelay(20);
            // Output High
            sstar_gpio_set_high(PAD_RGMII0_RSTN);
            gmac_reset0 = 1;
...
static void mhal_gmac_update_driving(u8 gmacId, u8 io_idx)
{
    ...
    { // Update All IO
        ...
        {
            sstar_gpio_drv_set(drv_padmux_getpad(GMAC_Driving_Offset[gmacId][i]), GMAC_Driving_Setting[gmacId][i]);
        }
    }
    else
    { // Update one IO by io_idx
        sstar_gpio_drv_set(drv_padmux_getpad(GMAC_Driving_Offset[gmacId][io_idx]),
                           GMAC_Driving_Setting[gmacId][io_idx]);
    }
}
