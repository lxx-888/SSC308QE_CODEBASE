...
u8 mdrv_spinand_setup_by_volatile(struct spinand_handle *handle)
{
    ...
    if (profile->flags & SPINAND_VOLATILE)
    {
...
u8 mdrv_spinand_get_ecc_mode(struct spinand_handle *handle)
{
    ...
    if (ERR_SPINAND_SUCCESS
        != drv_spinand_complete_get_features(handle, SPI_NAND_CMD_GF,
                                             spinand_ext_info->profile.ext_config.ecc_enabled.address, &u8_eccmode,
                                             spinand_ext_info->profile.ext_config.ecc_enabled.data_bytes))
    {
        return ERR_SPINAND_DEVICE_FAILURE;
    }

    if (spinand_ext_info->profile.ext_config.ecc_enabled.value & u8_eccmode)
    {
        return 1;
    }
    else
...
u8 mdrv_spinand_get_ecc_status(struct spinand_handle *handle)
{
    ...
    if (ERR_SPINAND_SUCCESS
        != drv_spinand_complete_get_features(handle, SPI_NAND_CMD_GF, SPI_NAND_REG_STAT, &u8_ecc_status, 1))
    {
        return ERR_SPINAND_DEVICE_FAILURE;
    }

    printf("u8_ecc_status:0x%x\r\n", u8_ecc_status);
    if (spinand_ext_info->profile.ecc_config.ecc_en)
    {
        if (spinand_ext_info->profile.ecc_config.ecc_status_mask & u8_ecc_status)
        {
            return 1;
        }
        else
    ...
    {
        if (ECC_STATUS_MASK & u8_ecc_status)
        {
            return 1;
        }
        else
