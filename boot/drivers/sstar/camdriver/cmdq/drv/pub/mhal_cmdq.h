 ...
 */
#define MHAL_CMDQ_ERROR_STATUS \
    (MHAL_CMDQ_POLLNEQ_TIMEOUT | MHAL_CMDQ_POLLEQ_TIMEOUT | MHAL_CMDQ_WAIT_TIMEOUT | MHAL_CMDQ_WRITE_TIMEOUT)
...
#if 1 // RUN_CMD_INT
/**
 * MHAL_CMDQ_CmdRunningIntSel_e
 * @note Direct mode CMDQ cmd running int select
 */
typedef enum
{
    E_MHAL_CMDQ_RUN_CMD_INT_SEL0 = 0, /**< Run CMD INT select 0 */
    E_MHAL_CMDQ_RUN_CMD_INT_SEL1,     /**< Run CMD INT select 1 */
    E_MHAL_CMDQ_RUN_CMD_INT_SEL2,     /**< Run CMD INT select 2 */
    E_MHAL_CMDQ_RUN_CMD_INT_SEL3,     /**< Run CMD INT select 3 */
    E_MHAL_CMDQ_RUN_CMD_INT_MAX
} MHAL_CMDQ_RunCmdIntSel_e;
#endif
...
#if 1 // RUN_CMD_INT
typedef void (*MHAL_CMDQ_CmdqIsrRunCmdCallBack)(MS_U32 nCmdqId, MHAL_CMDQ_RunCmdIntSel_e eIntSel);
#endif

//------------------------------------------------------------------------------
/// @brief        cmdq ISR callback function
/// @param[in]    nCmdqId : CMDQ channel ID
/// @return       NULL
//------------------------------------------------------------------------------
typedef void (*MHAL_CMDQ_CmdqIsrCallBack)(MS_U32 nCmdqId);
...
struct MHAL_CMDQ_CmdqInterface_s
{
    ...
    /// @retval       MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_WriteRegCmdqMask)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Value, MS_U16 u16WriteMask);
    ...
    //------------------------------------------------------------------------------
    MS_U32(*MHAL_CMDQ_WriteRegCmdqMaskMulti)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MHAL_CMDQ_MultiCmdBufMask_t *u32MultiCmdBufMask, MS_U16 u16Size);
    ...
    /// @retval       Actual size already write to CMDQ buffer
    //------------------------------------------------------------------------------
    MS_U32(*MHAL_MemCMDQ_WriteRegCmdqMaskMulti)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MHAL_MemCMDQ_MultiCmdBufMask_t *u32MultiCmdBufMask, MS_U16 u16Size);
    ...
    /// @retval       Actual size already write to CMDQ buffer
    //------------------------------------------------------------------------------
    MS_U32(*MHAL_CMDQ_WriteRegCmdqMulti)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MHAL_CMDQ_MultiCmdBuf_t *u32MultiCmdBuf, MS_U16 u16Size);
    ...
    /// @retval       MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_CmdqPollRegBits)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Value, MS_U16 u16WriteMask, MS_BOOL bPollEq);
    ...
    //------------------------------------------------------------------------------
    /// @brief        Register cmdq ISR callback
    /// @param[in]    pCmdinf      : CMDQ interface
    /// @param[in]    pIsrCallBack : CMDQ callback func
    /// @return       integer
    /// @retval       MHAL_SUCCESS : is ok
    /// @retval       MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_CmdqRegisterIsrCallBack)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MHAL_CMDQ_CmdqIsrCallBack pIsrCallBack);

    //------------------------------------------------------------------------------
    /// @brief        Delete cmdq ISR callback
    /// @param[in]    pCmdinf : CMDQ interface
    /// @return       integer
    /// @retval       MHAL_SUCCESS : is ok
    /// @retval       MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32 (*MHAL_CMDQ_CmdqDeleteIsrCallBack)(MHAL_CMDQ_CmdqInterface_t *pCmdinf);
    ...
    /// @retval       MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_CmdqPollRegBits_ByTime)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Value, MS_U16 u16WriteMask, MS_BOOL bPollEq,
     MS_U32 uTimeNs);
...
#if 1 // RUN_CMD_INT
      //------------------------------------------------------------------------------
      /// @brief cmdq register RunCmd callback
      /// @param[in] MHAL_CMDQ_CmdqInterface_t *pCmdinf : CMDQ interface.
      /// @param[in] MHAL_CMDQ_CmdqIsrRunCmdCallBack pIsrRunCmdCallBack : CMDQ RunCmd callback func.
      /// @return MHAL_SUCCESS : is ok
      /// @return MHAL_FAILURE : is failed
      //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_CmdqRegisterRunCmdIsrCallBack)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MHAL_CMDQ_CmdqIsrRunCmdCallBack pIsrRunCmdCallBack);

    //------------------------------------------------------------------------------
    /// @brief cmdq Delete RunCmd callback
    /// @param[in] MHAL_CMDQ_CmdqInterface_t *pCmdinf : CMDQ interface.
    /// @return MHAL_SUCCESS : is ok
    /// @return MHAL_FAILURE : is failed
    //------------------------------------------------------------------------------
    MS_S32 (*MHAL_CMDQ_CmdqDeleteRunCmdIsrCallBack)(MHAL_CMDQ_CmdqInterface_t *pCmdinf);
#endif
    ...
    ///
    ///               for(i=0; i<0x20; i++)
    ///                   g_pMemScriptCmdq1->MHAL_CMDQ_WriteRegWithSeq(g_pMemScriptCmdq1, (nDumAddr-(2*i)), 1,
    ///                   0x2010+i);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_KickOffMemScriptCmdq(g_pSelectCmdq, g_pMemScriptCmdq1);
    /// @endcode
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_KickOffMemScriptCmdq)
    (MHAL_CMDQ_CmdqInterface_t *pHWCmdinf, MHAL_CMDQ_CmdqInterface_t *pMemscriptCmdinf);
    ...
    ///               g_pMemScriptCmdq1 = MHAL_CMDQ_CreateMemoryScriptCmdq(&cmdqMemScriptBufDesp1);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_MemCmdqAddWaitEventCmd(g_pMemScriptCmdq1, E_MHAL_CMDQEVE_DUMMY15,
    ///               u16Seq2);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_KickOffMemScriptCmdq(g_pSelectCmdq, g_pMemScriptCmdq1);
    /// @endcode
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_MemCmdqAddWaitEventCmd)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U16 u16Seq, MHAL_CMDQ_EventId_e eEvent);
    ...
    ///               g_pMemScriptCmdq1 = MHAL_CMDQ_CreateMemoryScriptCmdq(&cmdqMemScriptBufDesp1);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_MemCmdqPollRegBits(g_pMemScriptCmdq1, ((nDumAddr)), 0xff00, u16Seq,
    ///               0xff00, 1);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_KickOffMemScriptCmdq(g_pSelectCmdq, g_pMemScriptCmdq1);
    /// @endcode
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_MemCmdqPollRegBits)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Value, MS_U16 u16Seq, MS_U16 u16WriteMask,
     MS_BOOL bPollEq);
    ...
    /// @endcode
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_WriteRegWithSeq)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Seq, MS_U16 u16Value);
    ...
    ///               g_pMemScriptCmdq1 = MHAL_CMDQ_CreateMemoryScriptCmdq(&cmdqMemScriptBufDesp1);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_WriteRegWithSeqMask(g_pMemScriptCmdq1, ((nDumAddr)), u16Seq, 0xaa,
    ///               0xffff);
    ///
    ///               g_pMemScriptCmdq1->MHAL_CMDQ_KickOffMemScriptCmdq(g_pSelectCmdq, g_pMemScriptCmdq1);
    /// @endcode
    //------------------------------------------------------------------------------
    MS_S32(*MHAL_CMDQ_WriteRegWithSeqMask)
    (MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Seq, MS_U16 u16Value, MS_U16 u16WriteMask);
...
//------------------------------------------------------------------------------
MHAL_CMDQ_CmdqInterface_t *MHAL_CMDQ_GetSysCmdqService(MS_S32 nCmdqId, MHAL_CMDQ_BufDescript_t *pCmdqBufDesp,
                                                       MS_BOOL bForceRIU);

//------------------------------------------------------------------------------
/// @brief        Create ring mode MemCmdq service
/// @param[in]    pCmdqBufDesp : MemCMDQ buffer description structure
/// @return       MHAL_CMDQ_CmdqInterface_t
/// @retval       NULL                      : is failed
/// @retval       MI_ERR_INVALID_PARAMETER  : Null parameter
/// @retval       MHAL_CMDQ_CmdqInterface_t : point is success
//------------------------------------------------------------------------------
MHAL_CMDQ_CmdqInterface_t *MHAL_CMDQ_CreateMemoryScriptCmdq(MHAL_CMDQ_BufDescript_t *pCmdqBufDesp);

//------------------------------------------------------------------------------
/// @brief        Duplicate ring mode MemCmdq service
/// @param[in]    pCmdqSrcInf  : Duplicate MemCmdq service source
/// @param[in]    pCmdqBufDesp : Dup MemCMDQ buffer description structure
/// @return       MHAL_CMDQ_CmdqInterface_t
/// @retval       NULL                      : is failed
/// @retval       MHAL_CMDQ_CmdqInterface_t : point is success
//------------------------------------------------------------------------------
MHAL_CMDQ_CmdqInterface_t *MHAL_CMDQ_CreateDupMemCmdq(MHAL_CMDQ_CmdqInterface_t *pCmdqSrcInf,
                                                      MHAL_CMDQ_BufDescript_t *  pCmdqBufDesp);
...
/// @endcode
//------------------------------------------------------------------------------
MS_S32 MHAL_CMDQ_BeginPartialUpdate(MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Seq,
                                    MS_BOOL bLookupTable);
...
/// @retval       MHAL_FAILURE : is failed
//------------------------------------------------------------------------------
MS_S32 MHAL_CMDQ_HostControlSlave(MHAL_CMDQ_CmdqInterface_t *pCmdinf, void *pBufVir, ss_miu_addr_t nBufMiu,
                                  MS_U32 u32CmdNum, MS_S32 nSlaveCmdqId);
...
#if 1 // RUN_CMD_INT
//------------------------------------------------------------------------------
/// @brief        Enable or disable function that running specific cmd will issue a INT, then call a isr callback.
/// @param[in]    pCmdinf : CMDQ interface
/// @param[out]   bEnable : 0/1         \n
///                         0 : disable \n
///                         1 : enable
...
/// @retval       MHAL_FAILURE : is failed
//------------------------------------------------------------------------------
MS_S32 MHAL_CMDQ_EnableRunCmdInt(MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_BOOL bEnable);
...
/// @retval       MHAL_FAILURE : is failed
//------------------------------------------------------------------------------
MS_S32 MHAL_CMDQ_WriteRegCmdqMaskWithInt(MHAL_CMDQ_CmdqInterface_t *pCmdinf, MS_U32 u32RegAddr, MS_U16 u16Value,
                                         MS_U16 u16WriteMask, MHAL_CMDQ_RunCmdIntSel_e eIntSel);
#endif
