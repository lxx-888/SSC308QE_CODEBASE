    ...
    if (g_pHdmitxDeviceNode && pClkRate && u32ClkRateSize == HAL_HDMITX_CLK_NUM)
    {
        ...
        if (u32NumParents == HAL_HDMITX_CLK_NUM)
        {
            ...
            for (u32idx = 0; u32idx < u32NumParents; u32idx++)
            {
                ...
                HDMITX_DBG(HDMITX_DBG_LEVEL_DRV_IF, "%s %d, [Hdmitx] u32NumParents:%d-%d %15s En:%d, Rate:%d\n",
                           __FUNCTION__, __LINE__, u32NumParents, u32idx + 1,
                           CamOfClkGetParentName(g_pHdmitxDeviceNode, u32idx), p8clkenbuf[u32idx], u32Clkrate);
            ...
            for (u32idx = 0; u32idx < u32NumParents; u32idx++)
            {
                ...
                if (!IS_ERR(pstHdmitxClks[u32idx]))
                {
                    HDMITX_DBG(HDMITX_DBG_LEVEL_DRV_IF, "%s %d, [Hdmitx] %d-%d, %15s En:%d, \n", __FUNCTION__, __LINE__,
                               u32NumParents, u32idx + 1, CamOfClkGetParentName(g_pHdmitxDeviceNode, u32idx),
                               p32clkenbuf[u32idx]);
