...
#define PARSING_HAL_LINKTYPE(x)                                  \
    (x == E_HAL_PNL_LINK_TTL                ? "TTL"              \
     ...
     : x == E_HAL_PNL_LINK_BT656            ? "BT656"            \
     : x == E_HAL_PNL_LINK_BT601            ? "BT601"            \
     : x == E_HAL_PNL_LINK_BT1120           ? "BT1120"           \
     : x == E_HAL_PNL_LINK_BT1120_DDR       ? "BT1120_DDR"       \
     : x == E_HAL_PNL_LINK_MCU_TYPE         ? "MCU"              \
     : x == E_HAL_PNL_LINK_SRGB             ? "sRGB"             \
                                            : "UNKNOWN")

#define PARSING_HAL_TI_BIT(x)                   \
    (x == E_HAL_PNL_TI_10BIT_MODE  ? "TI_10BIT" \
...
#define PARSING_HAL_MIPI_DSI_CTRL(x)                     \
    (x == E_HAL_PNL_MIPI_DSI_CMD_MODE     ? "CMD_MODE"   \
...
#define PARSING_HAL_MIPI_DSI_FMT(x)                              \
    (x == E_HAL_PNL_MIPI_DSI_RGB565           ? "RGB565"         \
...
#define PARSING_HAL_QUERY_TYPE(x)                               \
    (x == E_HAL_PNL_QUERY_PARAM             ? "PARAM"           \
     ...
     : x == E_HAL_PNL_QUERY_UNIFIED_PARAM   ? "UNIFIED_PARAM"   \
                                            : "UNKNOWN")

#define PARSING_HAL_QUERY_RET(x)                               \
    (x == E_HAL_PNL_QUERY_RET_OK           ? "RET_OK"          \
...
#define PARSING_HAL_BITMODE(x)                      \
    (x == E_HAL_PNL_OUTPUT_10BIT_MODE    ? "10BIT"  \
     : x == E_HAL_PNL_OUTPUT_8BIT_MODE   ? "8BIT"   \
     : x == E_HAL_PNL_OUTPUT_6BIT_MODE   ? "6BIT"   \
     : x == E_HAL_PNL_OUTPUT_565BIT_MODE ? "565BIT" \
                                         : "UNKNOWN")

#define PARSING_HAL_SC_PIX_CLK(x)              \
    (x == E_HAL_PNL_SC_PIX_CLK_240M   ? "240M" \
...
#define PARSING_HAL_MIPI_DSI_PACKET(x)                          \
    (x == E_HAL_PNL_MIPI_DSI_PACKET_TYPE_DCS       ? "DCS"      \
