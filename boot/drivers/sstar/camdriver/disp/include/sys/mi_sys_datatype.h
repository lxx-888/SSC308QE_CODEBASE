...
#define MI_SYS_MAX_MBX_PARAM_SIZE 6
...
typedef struct MI_SYS_FrameData_s
{
    ...
    MI_U16
    u16RingBufRealTotalHeight; /// Valid in case RINGBUF_FRAME_DATA, u16RingBufRealTotalHeight must be LGE than
                               /// u16Height
...
/// Mail Message Define
typedef struct MI_SYS_MbxMsg_s
{
    /// mailbox direction
    MI_SYS_MbxDirect_e eDirect;
    /// Ctrl Byte in Mail Message
    MI_U8 u8Ctrl;
    /// Status  byte of Mail Message
    MI_U8 u8Status;
    /// @ref MBX_Class
    MI_U8 u8MsgClass;
    /// Parameter Count
    MI_U8 u8ParameterCount;
    /// Parameters, Max Number @ref MAX_MBX_PARAM_SIZE
    MI_U16 u16Parameters[MI_SYS_MAX_MBX_PARAM_SIZE];
} MI_SYS_MbxMsg_t;
