...
#if (EN_BIND_CARD_INT)
#include "hal_card_intr_v5.h" //inlcue but may not use
#endif
...
extern BOOL_T         gb_Sdio_Dis_Intr_By_IP[MMC_TOTAL];
// Reg Dynamic Variable
//-----------------------------------------------------------------------------------------------------------
static RspStruct       gst_RspStruct[MMC_TOTAL];
static volatile BOOL_T gb_StopWProc[MMC_TOTAL] = {0};
static volatile U16_T  gu16_WT_NRC[MMC_TOTAL]  = {0}; // Waiting Time for Nrc (us)

static volatile U16_T gu16_SD_MODE_DatLine[MMC_TOTAL] = {0};

static volatile U16_T gu16_DDR_MODE_REG[MMC_TOTAL]        = {0};
static volatile U16_T gu16_DDR_MODE_REG_ForR2N[MMC_TOTAL] = {0};

static volatile BOOL_T gb_SDIODevice[MMC_TOTAL]          = {0};
static volatile U16_T  gu16_SDIO_MODE_IntMode[MMC_TOTAL] = {0};

static volatile U8_T *gpu8Buf[MMC_TOTAL];

static volatile IpType geIpTypeIp[MMC_TOTAL] = {IP_0_TYPE, IP_1_TYPE, IP_2_TYPE, IP_3_TYPE};
...
static U16_T _REG_GetMIEFunCtlSetting(IpOrder eIP)
{
    ...
    {
        return R_EMMC_EN; // FCIE don't have SDIO function.
    }
...
static RspErrEmType _MMC_CheckR1Error(IpOrder eIP)
{
    ...
    if (gst_RspStruct[eIP].u8ArrRspToken[1] & (MMC_ERR_R1_31_24 >> 24))
    {
        sdmmc_print("\nMMC Err: MMC_ST_ERR_R1_31_24 \n ");
        eErr = EV_OTHER_ERR;
        goto LABEL_CHECK_R1_END;
    }

    if (gst_RspStruct[eIP].u8ArrRspToken[2] & (MMC_ERR_R1_23_16 >> 16))
    {
        sdmmc_print("\nMMC Err: MMC_ST_ERR_R1_23_16 \n ");
        eErr = EV_OTHER_ERR;
        goto LABEL_CHECK_R1_END;
    }

    if (gst_RspStruct[eIP].u8ArrRspToken[3] & (MMC_ERR_R1_15_8 >> 8))
    {
        sdmmc_print("\nMMC Err: MMC_ST_ERR_R1_15_8 \n ");
        eErr = EV_OTHER_ERR;
        goto LABEL_CHECK_R1_END;
    }

    if (gst_RspStruct[eIP].u8ArrRspToken[4] & (MMC_ERR_R1_7_0 >> 0))
    {
        sdmmc_print("\nMMC Err: MMC_ST_ERR_R1_7_0 \n ");
        eErr = EV_OTHER_ERR;
        goto LABEL_CHECK_R1_END;
    }
...
void Hal_SDMMC_SetBusTiming(IpOrder eIP, BusTimingEmType eBusTiming)
{
    switch (eBusTiming)
    {
        ...
        case EV_BUS_DEF:
        case EV_BUS_HS:
            gu16_DDR_MODE_REG[eIP] = R_PAD_IN_SEL | (Enable_C2 == 1 ? R_PAD_CLK_SEL : 0);
                                             ...
                                             SDMMCRspEmType eRspType, BOOL_T bCloseClk)
{
    ...
    {
        CARD_REG(A_SD_MODE_REG(eIP)) = (eTransType >> 8) | gu16_SD_MODE_DatLine[eIP];
        if (gst_RspStruct[eIP].u8Cmd == 18)
            CARD_REG(A_SD_MODE_REG(eIP)) |= ((U8_T)(eTransType & R_DMA_RD_CLK_STOP));
        else
            CARD_REG(A_SD_MODE_REG(eIP)) &= ~((U8_T)(eTransType & R_DMA_RD_CLK_STOP));
    }
    CARD_REG(A_SD_CTL_REG(eIP)) =
...
#endif
    CARD_REG_SETBIT(A_SD_MODE_REG(eIP), V_SD_MODE_INIT);
    ...
    if ((eRspType == EV_R3) || (eRspType == EV_R4))  // For IP CRC bug
    ...
    {
        if (gst_RspStruct[eIP].u8Cmd != 3 && ((eRspType == EV_R1) || (eRspType == EV_R1B)))
        {
...
int HAL_SDMMC_WaitDat0(IpOrder eIP, U8_T u8_state, U32_T u32WaitMs)
{
    ...
    do
    {
        if ((M_REG_GETDAT0(eIP) >> 8) == u8_state)
            break;
        Hal_Timer_uDelay(1);
        u32DiffTime++;
    } while (u32DiffTime <= (u32WaitMs * 1000));
    CARD_REG_CLRBIT(A_SD_MODE_REG(eIP), R_CLK_EN);
    if (u32DiffTime < u32WaitMs * 1000)
        return EV_OK;
    else
        return EV_FAIL;
}
