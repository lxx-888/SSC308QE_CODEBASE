...
U8 HAL_AESDMA_rng_init(void)
{
    if (((RIU[(AESDMA_BASE_ADDR + (0x3 << 1))] & 0x8000) != 0x8000))
    {
        RIU[(AESDMA2_BASE_ADDR + (0x09 << 1))] = 0x4321;
        RIU[(AESDMA2_BASE_ADDR + (0x0A << 1))] = 0xFF00; // clk rc =12M
        RIU[(AESDMA2_BASE_ADDR + (0x0D << 1))] = 0xFFFF;

        // HW init clk_rng=12M. SW set clk_rc set = 12M
        // so set 0x12A1_0[15]=1
        if ((RIU[(AESDMA_RNG_CLK_ADDR + (0x77 << 1))] & 0x40) == 0x40)
            RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] = 0x0070;
        else
            RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] = 0x8070;

        // rng restart
        RIU[(AESDMA_BASE_ADDR + (0x3 << 1))] = 0x0001;
        RIU[(AESDMA_BASE_ADDR + (0x3 << 1))] = 0x0000;

        while ((RIU[(AESDMA_BASE_ADDR + (0x3 << 1))] & 0x8000) != 0x8000)
            ;
    }
...
void HAL_AESDMA_SCA(U8 enbale)
{
    ...
    else
    {
        ...
        RIU[(AESDMA2_BASE_ADDR + (0x20 << 1))] = 0x8000;
    }
}
...
BOOL HAL_AESDMA_CheckAesKey(U16 u32KeySel)
{
    ...
    return (u32KeySel < E_AESDMA_KEY_NUM ? TRUE : FALSE);
}

void HAL_AESDMA_UseAesKey256(U16 u32KeySel)
{
    RIU[(AESDMA_BASE_ADDR + (0x77 << 1))] = ((RIU[(AESDMA_BASE_ADDR + (0x77 << 1))]) | (1 << 13));
}
...
void HAL_AESDMA_UseAesKey(U16 u32KeySel)
{
    ...
    else if (u32KeySel == AESDMA_KEY_OTP_EFUSE_KEY8)
    {
        RIU[(AESDMA_BASE_ADDR + (0x79 << 1))] =
            (((RIU[(AESDMA_BASE_ADDR + (0x79 << 1))]) & (~AESDMA_USE_SECRET_KEY_MASK)) | AESDMA_KEY_OTP_EFUSE_USE_KEY8);
    }
    else if (u32KeySel > AESDMA_KEY_OTP_EFUSE_KEY8)
    {
        RIU[(AESDMA_BASE_ADDR + (0x79 << 1))] =
            (((RIU[(AESDMA_BASE_ADDR + (0x79 << 1))]) & (~(AESDMA_USE_SECRET_KEY_MASK | AESDMA_KEY_OTP_EFUSE_USE_KEY8)))
             | AESDMA_USE_KEY((((u32KeySel - AESDMA_KEY_OTP_EFUSE_KEY8) * 2) - 1)));
    }
}
...
#ifdef CONFIG_SRAM_DUMMY_ACCESS_RSA
__attribute__((optimize("O0"))) void HAL_RSA_LoadKeyNInverse(U32 *ptr_N, U8 length)
#else
void HAL_RSA_LoadKeyNInverse(U32 *ptr_N, U8 length)
#endif
{
    U32 i;
...
#endif
{
    ...
    for (index = 0; index < 16; index++)
    {
        ...
        *((U16 *)(unsigned long)u32Buf + index) = (RIU[(SHARNG_BASE_ADDR + (0x10 << 1) + index * 2)]);
    }
}
