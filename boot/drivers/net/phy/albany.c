...
*/
    ...
    uRegVal = MHal_EMAC_ReadReg8(REG_BANK_ALBANY1, 0x5a * 2);
    uRegVal = uRegVal | 0x02;
    MHal_EMAC_WritReg8(REG_BANK_ALBANY1, 0x5a * 2, uRegVal);

    b_albany_config_init = 1;
    return 0;
}

static int sstar_probe(struct phy_device *phydev)
{
    return 0;
}

static int sstar_config(struct phy_device *phydev)
{
    albany_config_init();
...
static int sstar_startup(struct phy_device *phydev)
{
    int ret;
...
static struct phy_driver SSTAR_PHY_driver = {
    .name = "Sigmastar 10/100Mbps Ethernet",
    .uid = 0x11112222,
    .mask = 0xffffffff,
    .features = PHY_BASIC_FEATURES,
    .probe = &sstar_probe,
    .config = &sstar_config,
    .startup = &sstar_startup,
    .shutdown = &genphy_shutdown,
};

int phy_sstar_init(void)
{
    //create phy id for albany
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0x04, 0x11);
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0x05, 0x11);
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0x06, 0x22);
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0x07, 0x22);

    phy_register(&SSTAR_PHY_driver);
