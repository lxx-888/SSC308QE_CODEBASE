...
int mmc_uda_wp_op(struct mmc *mmc, u32 u32_addr, u8 u8_mode)
{
    struct mmc_cmd cmd;
    unsigned int   status, start, send_status = 1;
    int            timeout_ms = 500;
    int            ret;
    cmd.cmdidx    = u8_mode ? 28 : 29;
    cmd.resp_type = MMC_RSP_R1b;
    if (mmc->high_capacity)
        cmd.cmdarg = u32_addr;
    else
        cmd.cmdarg = u32_addr * mmc->read_bl_len;
    ...
    do
    {
        ...
        if (!ret && (status & MMC_STATUS_SWITCH_ERROR))
        {
            pr_debug("write protect switch failed mode:%#x, address:%#x !\n", u8_mode, u32_addr);
            return -EIO;
        }
        if (!ret && (status & MMC_STATUS_RDY_FOR_DATA) && (status & MMC_STATUS_CURR_STATE) == MMC_STATE_TRANS)
            return 0;
        udelay(100);
    } while (get_timer(start) < timeout_ms);
...
int mmc_get_uda_wp_status(struct mmc *mmc, u8 *pu8_buf, u32 u32_addr, u8 u8_mode)
{
    struct mmc_cmd  cmd;
    struct mmc_data data;
    int             err;
    ...
    data.dest      = (char *)pu8_buf;
    data.blocks    = 1;
    data.blocksize = u8_mode ? 4 : 8;
    data.flags     = MMC_DATA_READ;
...
static int sd_get_capabilities(struct mmc *mmc)
{
	int err;
	struct mmc_cmd cmd;
	ALLOC_CACHE_ALIGN_BUFFER(__be32, scr, 16);
