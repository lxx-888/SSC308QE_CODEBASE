...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_MMC_BOOT_PART)
#define FASTBOOT_FLASH_MMC_BOOT_PARTITION_ID  CONFIG_FASTBOOT_SSTAR_MMC_BOOT_PART
#define FASTBOOT_FLASH_MMC_USER_PARTITION_ID  0
#endif
				       ...
				       char *response)
{
	...
	hdr_sectors = DIV_ROUND_UP(sizeof(struct andr_boot_info), sector_size);
	if (hdr_sectors == 0) {
				...
				char *response)
{
	...
	struct andr_boot_info *hdr;		/* boot image header */
	...
	hdr = (struct andr_boot_info *)hdr_addr;
	...
	ramdisk_sector_start += DIV_ROUND_UP(hdr->kernel_size, hdr->page_size) *
					     sectors_per_page;
	ramdisk_sectors = DIV_ROUND_UP(hdr->boot_ramdisk_size, hdr->page_size) *
				       sectors_per_page;
...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_MMC_BOOT_PART)
static int fb_mmc_switch_hwpar(int dev, int part, struct disk_partition *info)
{
	...
	/*
	* For partition config details, please refer to the sswiki article about
	* extcsd settings. Note: don't clear the boot enable setting or it will
	* fail to boot from mmc.
	*/
	printf("Flash target is mmc:%d boot_partition_eanble:%d partition_access:%d\n", dev, \
				FASTBOOT_FLASH_MMC_BOOT_PARTITION_ID, part);
	if (target_mmc->part_config != MMCPART_NOAVAILABLE)
		sprintf(mmc_dev, "mmc partconf %x 0 %x %x", dev, /*slot no*/
				FASTBOOT_FLASH_MMC_BOOT_PARTITION_ID, /* boot partition eanble */
				part/* partition access */);

	if (info && part == FASTBOOT_FLASH_MMC_BOOT_PARTITION_ID) {
		info->start = 0;
		info->blksz = 0x200;
		info->size = target_mmc->capacity_boot;
	}

	return run_command(mmc_dev, 0);
}
#endif
...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_MMC_BOOT_PART)
	if (strncmp(cmd, "bootloader", 10) == 0) {
		dev_desc = fastboot_mmc_get_dev(response);
		if (!dev_desc)
			return;
		if (fb_mmc_switch_hwpar(CONFIG_FASTBOOT_FLASH_MMC_DEV,
								FASTBOOT_FLASH_MMC_BOOT_PARTITION_ID, &info)) {
			fastboot_fail("Switch to boot part fail", response);
			return;
		}
		...
		if (fb_mmc_switch_hwpar(CONFIG_FASTBOOT_FLASH_MMC_DEV,
								FASTBOOT_FLASH_MMC_USER_PARTITION_ID, NULL)) {
			fastboot_fail("Switch to user part fail", response);
			return;
		}
		return;
	}
