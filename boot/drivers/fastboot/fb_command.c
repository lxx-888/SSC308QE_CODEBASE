...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_CMDS)
#include "sstar_fastboot.h"
#endif
#if CONFIG_IS_ENABLED(FASTBOOT_FLASH_SSTAR_CUS)
...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_CMDS)
	printf("Receive fastboot cmd %s\n", cmd_string);
#endif
	strsep(&cmd_parameter, ":");

#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_CMDS)
	/* find cmd_string from sstar_commands frist */
	for (i = 0; sstar_commands[i].command != NULL; i++) {
		if (!strcmp(sstar_commands[i].command, cmd_string)) {
			if (sstar_commands[i].dispatch) {
				sstar_commands[i].dispatch(cmd_parameter,
							response);
				return FASTBOOT_COMMAND_COUNT + i;
			} else {
				break;
			}
		}
...
#endif
#if CONFIG_IS_ENABLED(FASTBOOT_FLASH_CUS)
...
static void reboot_fastbootd(char *cmd_parameter, char *response)
{
	...
	extern int bcb_write_recovery_cmd(int devnum, char *partp, const char *cmdp);
	if (bcb_write_recovery_cmd(CONFIG_FASTBOOT_FLASH_MMC_DEV,
				"misc",
				"recovery\n--fastboot"))
		fastboot_fail("Cannot set recovery cmd", response);
#else
