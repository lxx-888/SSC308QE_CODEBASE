...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_GETVAR)
 ...
 */
int getvar_get_part_info(const char *part_name, char *response,
				size_t *size)
{
	int r;
# if CONFIG_IS_ENABLED(FASTBOOT_FLASH_MMC)
...
static void getvar_serialno(char *var_parameter, char *response)
{
	...
	if (tmp)
		...
		fastboot_okay("1234567890", response);
#else
...
#if CONFIG_IS_ENABLED(FASTBOOT_SSTAR_GETVAR)
		/* find cmd_parameter from sstar_getvar_dispatch frist */
		for (i = 0; sstar_getvar_dispatch[i].variable != NULL; ++i) {
			if (!strcmp(sstar_getvar_dispatch[i].variable,
				    cmd_parameter)) {
				sstar_getvar_dispatch[i].dispatch(var_parameter,
							    response);
				return;
			}
		}
