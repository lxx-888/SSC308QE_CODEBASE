...
/**
 * fastboot_sendinfo_callback - callback for send info
 */
void (*fastboot_sendinfo_callback)(const char *msg);
#endif
...
#ifdef CONFIG_ARCH_SSTAR
/**
 * fastboot_set_sendinfo_callback() - set sendinfo callback
 *
 * @progress: Pointer to send info callback
 *
 * Set a callback which is invoked when a message need to be sent multiple times
 * (getvar all).
 */
void fastboot_set_sendinfo_callback(void (*progress)(const char *msg))
{
	fastboot_sendinfo_callback = progress;
}
#endif
...
#ifdef CONFIG_ARCH_SSTAR
	fastboot_set_sendinfo_callback(NULL);
#endif
}
