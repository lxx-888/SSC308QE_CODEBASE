			...
			void *buffer, struct devrequest *req)
{
	...
	case USB_REQ_SET_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		...
		case USB_PORT_FEAT_RESET:
			debug("USB_PORT_FEAT_RESET, port(%d)sc = 0x%x.\r\n", le16_to_cpu(req->index) - 1, reg);
		#ifdef CONFIG_ARCH_SSTAR
			if (XDEV_FS == (reg & DEV_SPEED_MASK)) {
				volatile uint32_t portsc;
				portsc = xhci_readl((volatile uint32_t *) (&hcor->portregs[1].or_portsc));
				if (((reg & PORT_PLS_MASK) == XDEV_POLLING) && !(portsc & PORT_CONNECT)) {
					debug("USB_PORT_FEAT_RESET, port(1)sc = 0x%x\r\n", portsc);
					portsc = xhci_port_state_to_neutral(portsc);
					portsc |= PORT_CSC | PORT_PEC | PORT_WRC |
							PORT_OCC | PORT_RC | PORT_PLC |
							PORT_CEC | PORT_PE;
					xhci_writel((volatile uint32_t *) (&hcor->portregs[1].or_portsc), portsc);
					portsc = xhci_readl((volatile uint32_t *) (&hcor->portregs[1].or_portsc));
				}
			}
		#endif
	...
	case USB_REQ_CLEAR_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		...
		case USB_PORT_FEAT_C_RESET:
		#ifdef CONFIG_ARCH_SSTAR
			if (XDEV_HS >= (reg & DEV_SPEED_MASK)) 	{
				if ((reg & PORT_PLS_MASK) == XDEV_U0) {
					volatile uint32_t portsc;
					portsc = xhci_readl((volatile uint32_t *) (&hcor->portregs[1].or_portsc));
					if (((portsc & PORT_PLS_MASK) == XDEV_DISABLED) && !(portsc & PORT_CONNECT)) {
						debug("USB_PORT_FEAT_C_RESET, port(1)sc = 0x%x\r\n", portsc);
						portsc = xhci_port_state_to_neutral(portsc);
