...
#ifdef CONFIG_SSTAR_USB_EHCI
#define ehci_readl(x) sstar_ehci_readl(x)
#define ehci_writel(a, b) sstar_ehci_writel(a, b)
#define readl(addr)       *((volatile u32 *)(addr))
#define writel(val, addr) (*((volatile u32 *)(addr)) = (u32)val)

static inline void *phys_to_virt(phys_addr_t paddr)
{
  return (void *)(unsigned long)(paddr + 0x20000000);
}

static inline phys_addr_t virt_to_phys(void *vaddr)
{
  return (phys_addr_t)(unsigned long)(vaddr - 0x20000000);
}
#else
...
#define QT_TOKEN_TOTALBYTES(x)                                                 \
  (((x)&0x7fff) << 16) /* Total Bytes to Transfer */
