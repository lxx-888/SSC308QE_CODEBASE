...
#endif
#ifdef CONFIG_SSTAR_USB_EHCI
unsigned int sstar_ehci_readl(uint32_t *regs) {
  unsigned int result;
  regs = (u32 *)(((uintptr_t)regs & ~(0xffUL)) +
                 (((uintptr_t)regs & 0xffUL) << 1));
  result = (readl((void *)regs) & 0xffffU) |
           ((readl((void *)((uintptr_t)regs + 4)) << 16) & (0xffffU << 16));
  return result;
}

void sstar_ehci_writel(uint32_t *regs, const unsigned int val) {
  regs = (u32 *)(((uintptr_t)regs & ~(0xffUL)) +
                 (((uintptr_t)regs & 0xffUL) << 1));
...
static int ehci_common_init(struct ehci_ctrl *ctrl, uint tweaks)
{
	...
	for (i = 0; i < 1024; i++) {
#ifdef CONFIG_SSTAR_USB_EHCI
		ctrl->periodic_list[i] = cpu_to_hc32((unsigned long)virt_to_phys(periodic)
						| QH_LINK_TYPE_QH);
#else
	...
	/* Set periodic list base address */
        ...
        ehci_writel(&ctrl->hcor->or_periodiclistbase,
                    virt_to_phys((unsigned long)ctrl->periodic_list));
#else
        ehci_writel(&ctrl->hcor->or_periodiclistbase,
...
#ifdef CONFIG_SSTAR_USB_EHCI
		qh->qh_link = cpu_to_hc32((unsigned long)(virt_to_phys(qh)+1) | QH_LINK_TYPE_QH);
#else
...
#ifdef CONFIG_SSTAR_USB_EHCI
		qh->qh_overlay.qt_next = cpu_to_hc32((unsigned long)virt_to_phys(td));
#else
...
#ifdef CONFIG_SSTAR_USB_EHCI
		td->qt_buffer[0] =
		    cpu_to_hc32((unsigned long)virt_to_phys(buffer) + i * elementsize);
#else
		td->qt_buffer[0] =
...
#ifdef CONFIG_SSTAR_USB_EHCI
	list->qh_link = cpu_to_hc32((unsigned long)virt_to_phys(result->first) | QH_LINK_TYPE_QH);
#else
...
#ifdef CONFIG_SSTAR_USB_EHCI
		if (NEXT_QH(cur) == virt_to_phys(queue->first)) {
#else
