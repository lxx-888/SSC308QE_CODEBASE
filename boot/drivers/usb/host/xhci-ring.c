...
static void xhci_queue_reset_ep(struct xhci_ctrl *ctrl, int slot_id,
			unsigned int ep_index)
{
	...
	fields[1] = 0;
	fields[2] = 0;
	fields[3] = trb_slot_id | trb_ep_index | type | ctrl->cmd_ring->cycle_state;
	queue_trb(ctrl, ctrl->cmd_ring, false, fields);
...
static __maybe_unused void xhci_queue_set_deq(struct xhci_ctrl *ctrl, u8 *ptr, int cycle, int slot_id,
			unsigned int ep_index)
{
	u32 fields[4];
	u64 val_64 = 0;
	...
	fields[0] = lower_32_bits(val_64) | cycle;
	fields[1] = upper_32_bits(val_64);
	fields[2] = 0;
	fields[3] = SLOT_ID_FOR_TRB(slot_id) | EP_ID_FOR_TRB(ep_index) | TRB_TYPE(TRB_SET_DEQ);
	queue_trb(ctrl, ctrl->cmd_ring, false, fields);
...
retry:
	...
	if (GET_COMP_CODE(le32_to_cpu(event->trans_event.transfer_len))
			== COMP_TX_ERR) {
		if (3 > retry_cnt) {
			xhci_queue_reset_ep(ctrl, slot_id, ep_index);
			event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
