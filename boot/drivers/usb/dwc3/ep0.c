				...
				u32 len, u32 type, unsigned chain)
{
	...
	dma_addr_t buf_miu_addr = buf_dma - dev_get_dma_offset(dwc->dev);
	dma_addr_t ep0_trb_miu_addr = dwc->ep0_trb_addr - dev_get_dma_offset(dwc->dev);
#endif
	...
	if (chain)
		dep->free_slot++;
#if CONFIG_IS_ENABLED(ARCH_SSTAR)
	trb->bpl = lower_32_bits(buf_miu_addr);
	trb->bph = upper_32_bits(buf_miu_addr);
#else
...
#if CONFIG_IS_ENABLED(ARCH_SSTAR)
	params.param0 = upper_32_bits(ep0_trb_miu_addr);
	params.param1 = lower_32_bits(ep0_trb_miu_addr);
#else
