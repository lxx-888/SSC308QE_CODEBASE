...
#include "../drivers/sstar/rpmb/sstar_rpmb.h"
#endif
...
#ifdef CONFIG_SSTAR_RPMB
	int ret = 0;
	ret = sstar_rpmb_get_rollback_index(rollback_index_slot, out_rollback_index);
	if (ret) {
		printf("ANDROID:Read rollback index loacation %ld fail: %d\n", rollback_index_slot, ret);
		return AVB_IO_RESULT_ERROR_NO_SUCH_VALUE;
	}
	return AVB_IO_RESULT_OK;
#else
...
#ifdef CONFIG_SSTAR_RPMB
	int ret = 0;
	ret = sstar_rpmb_set_rollback_index(rollback_index_slot, rollback_index);
	if (ret) {
		printf("ANDROID:Read rollback index loacation %ld fail: %d\n", rollback_index_slot, ret);
		return AVB_IO_RESULT_ERROR_IO;
	}
...
static AvbIOResult read_is_device_unlocked(AvbOps *ops, bool *out_is_unlocked)
{
	...
	*out_is_unlocked = true;
	...
	if (ret)
	{
		printf("ANDROID: Fail to get fastboot flags\n");
		return AVB_IO_RESULT_ERROR_IO;
	}

	*out_is_unlocked = lock_flags ? true : false;
