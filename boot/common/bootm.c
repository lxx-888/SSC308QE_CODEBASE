...
#ifdef CONFIG_SSTAR_BOOTING_PERF
...
static image_header_t *image_get_kernel(ulong img_addr, int verify)
{
	...
	ulong rtos_base = env_get_hex("rtos_base", 0);
#endif
...
#ifdef CONFIG_BOOTM_SS_RTOS
	if (image_check_os(hdr, IH_OS_SS_RTOS) && rtos_base)
	{
		hdr->ih_load = cpu_to_be32(rtos_base);
#ifdef CONFIG_SS_SMF_LOAD_64_KERNEL_ENTRY
		hdr->ih_ep = cpu_to_be64(rtos_base);
#else
		hdr->ih_ep = cpu_to_be32(rtos_base);
#endif
	}
#endif
