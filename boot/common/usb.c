...
static int usb_hub_port_reset(struct usb_device *dev, struct usb_device *hub)
{
	...
	printf("%s hub:%p dev:%p %s reset\n", __func__, hub, dev, !hub ? "root port" : "port");
	...
	return usb_control_msg(hub, usb_sndctrlpipe(hub, 0),
				...
				dev->portnr, NULL, 0, USB_CNTL_TIMEOUT);
#else
	if (!hub)
...
static int usb_setup_descriptor(struct usb_device *dev, bool do_read)
{
	...
	if (do_read && dev->speed <= USB_SPEED_HIGH) {
		int err;
...
#ifdef CONFIG_SSTAR_USB_EHCI
	/* Some devices times out at the first time and need a
	 * second reset.This is what Linux does and it does work.
	 */
	for (retries = 0; retries < 2; (++retries, mdelay(200))) {
		for (operations = 0; operations < 3; ++operations) {
			err = usb_setup_descriptor(dev, do_read);
			if (err) {
				printf("%s retry %d operation %d\n", __func__, retries, operations);
				if (err && retries == 0) {
					break;
				}
			} else {
