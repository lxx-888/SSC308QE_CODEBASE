...
#ifdef CONFIG_AUTOBOOT_RUN_FASTBOOT_MODE
	printf("Run into fastboot mode\n");
	env_set("bootcmd", "fastboot 0");
#endif

#ifdef CONFIG_CMD_SSTAR_UFU
#ifdef CONFIG_SSTAR_AUTOBOOT_RUN_UFU_ALWAYS
    printf("ufu always\n");
    run_command("ufu", 0);
#endif
#ifdef CONFIG_SSTAR_AUTOBOOT_RUN_UFU_BY_ENV
    s = env_get("ota_upgrade_status");
    if((s && !strcmp(s, "1")))
    {
        printf("ufu by env\n");
        run_command("ufu", 0);
...
#endif
#ifdef CONFIG_SSTAR_AUTOBOOT_RUN_UFU_BY_GPIO
    extern bool sstar_force_enter_ufu(void);
    if(sstar_force_enter_ufu())
    {
        printf("ufu by gpio\n");
        run_command("ufu", 0);
    }
...
#ifdef CONFIG_CMD_SSTAR_USBSTAR
#ifdef CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_BY_GPIO
    #include "../drivers/sstar/gpio/drv_gpio.h"
    char level;
    sstar_gpio_pad_set(CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_GPIO_NUM);
    sstar_gpio_pad_odn(CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_GPIO_NUM);
    sstar_gpio_pad_read(CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_GPIO_NUM, &level);
    if (level)
    {
        printf("usbstar by gpio:%d\n", CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_GPIO_NUM);
        run_command("usbstar", 0);
    }

#endif
#ifdef CONFIG_SSTAR_AUTOBOOT_RUN_USBSTAR_BY_ENV
    s = env_get("auto_usbstar");
    if((s && !strcmp(s, "1")))
    {
        printf("usbstar by env\n");
        run_command("usbstar", 0);
    }
#endif
#endif
