...
static void _populate_boot_info(const struct boot_img_hdr_v4* boot_hdr,
	...
	/* Set kernel load addr and ramdisk start addr
	 * follow vndboot image setting
	 */
	if (vboot_hdr->kernel_addr)
		boot_info->kernel_addr = vboot_hdr->kernel_addr;
	else
		boot_info->kernel_addr = (ulong)load_addr;

	if (vboot_hdr->ramdisk_addr)
		boot_info->vendor_ramdisk_addr = vboot_hdr->ramdisk_addr;
	else
		boot_info->vendor_ramdisk_addr = boot_info->kernel_addr +
			ALIGN(boot_info->kernel_size, SZ_64M);

	env_set_hex("kernel_comp_size", boot_info->kernel_size);
#else
	boot_info->kernel_addr = (ulong)load_addr;
...
/*
 * By default, we get env "fdt_addr" in
 * android_bootloader_boot_kernel(), but
 * we change to read fdt from storage and
 * set env "fdt_addr".
 */
#ifdef CONFIG_SSTAR_ANDROID_BOOTLOADER
#include <linux/libfdt.h>
#include <dt_table.h>
static bool _read_in_dtb(struct blk_desc *dev_desc,
		const struct disk_partition *vendor_boot_img,
		const struct andr_boot_info *boot_info,
		const AvbPartitionData *verified_vendor_boot_img) {
	/* in vndboot.img, vendor fdt is next to vendor ramdisk */
	size_t page = boot_info->page_size;
	size_t offset = ALIGN(sizeof(struct vendor_boot_img_hdr_v4), page)
						+ ALIGN(boot_info->vendor_ramdisk_size, page);
	size_t size = ALIGN(boot_info->dtb_size, page);
	void *dtb = boot_info->dtb_addr;
	void *dtb_addr = NULL;
	uint32_t dtb_size = 0;

	if (!android_read_data("dtb", dev_desc, vendor_boot_img,
			       verified_vendor_boot_img, offset, dtb, size)) {
	      return false;
	}

	if (be32_to_cpu(((struct dt_table_header *)dtb)->magic) == DT_TABLE_MAGIC) {
		struct dt_table_header *dt_table_hdr = (struct dt_table_header *)dtb;
		struct dt_table_entry *dt_entry = (struct dt_table_entry *)((uint32_t)dt_table_hdr +
				be32_to_cpu(dt_table_hdr->dt_entries_offset));
		dtb_addr = (void *)((uint32_t)dt_table_hdr + be32_to_cpu(dt_entry->dt_offset));
		dtb_size = be32_to_cpu(dt_entry->dt_size);
	} else if (be32_to_cpu(((struct fdt_header *)dtb)->magic) == FDT_MAGIC) {
		struct fdt_header *ftd_hdr = (struct fdt_header *)dtb;
		dtb_addr = (void *)ftd_hdr;
		dtb_size = be32_to_cpu(ftd_hdr->totalsize);
	} else {
		printf("ANDROID: Unrecognized dtb format magic:%#x\n", *(uint32_t *)dtb);
		return false;
	}

	printf("ANDROID: loading dtb to 0x%llx~0x%x\n", boot_info->dtb_addr, dtb_size);
	memmove((void *)(boot_info->dtb_addr), dtb_addr, dtb_size);
	env_set_hex("fdtaddr", boot_info->dtb_addr);
...
struct andr_boot_info* android_image_load(struct blk_desc *dev_desc,
	...
	if(!_read_in_kernel(dev_desc, boot_img, boot_info, verified_boot_img)
		...
		|| !_read_in_boot_ramdisk(dev_desc, boot_img, boot_info,
					  verified_boot_img)
#ifdef CONFIG_SSTAR_ANDROID_BOOTLOADER
		|| !_read_in_dtb(dev_desc, vendor_boot_img, boot_info,
					  verified_vendor_boot_img)
#endif
