...
#define RECORD_BL_ADDR  0xA000F800 // BL
...
void recode_show(void)
{
    ...
    if (tc->count < MAX_RECORD && tc->count > 0)
    {
        ...
        for (i = 0; i < tc->count; i++)
        {
            ...
            printk("%03d time:%8u, diff:%8u, %s, %d\n", i, tc->tt[i].timestamp_us,
                   tc->tt[i].timestamp_us - tc->tt[i ? i - 1 : i].timestamp_us, tc->tt[i].name, tc->tt[i].mark);
        }
...
static int do_bootingtime(struct cmd_tbl *cmdtp, int flag, int argc, char *const argv[])
{
    recode_show();
    return 0;
}
U_BOOT_CMD(btime, 2, 1, do_bootingtime, "Show booting time", "Show booting time\n");
#else
void         recode_timestamp(int mark, const char* name) {}
void         recode_show(void) {}
unsigned int read_timestamp(void)
