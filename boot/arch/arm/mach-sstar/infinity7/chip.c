...
void Chip_Set_RebootType(uint16_t type)
{
    if (type == SSTAR_REBOOT_BY_SW_RST)
    {
        // set as software reboot
        SETREG16(REG_ADDR_BASE_PMPOR + REG_ID_01, BIT0);
        // clear watchdog reset status
...
{
    if (INREG16(REG_ADDR_BASE_WDT + REG_ID_02) & 0x01)
        return SSTAR_REBOOT_BY_WDT_RST;

    if (INREG16(REG_ADDR_BASE_PMPOR + REG_ID_01) & 0x01)
        return SSTAR_REBOOT_BY_SW_RST;
    else
        return SSTAR_REBOOT_BY_HW_RST;
}
