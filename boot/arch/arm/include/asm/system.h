...
#define ATTR_TARGET_ARM         __attribute__((target("arm")))

ATTR_TARGET_ARM static inline void arch_change_to_mon_mode_int_off(void)
{
	/* Change to MON_MODE, FIQ/IRQ disabled */
	__asm__ __volatile__("MOV    r0, #0xD6\n"
						"MSR    cpsr_cxsf, r0\n");
}

ATTR_TARGET_ARM static inline void arch_change_to_non_secure_world(void)
{
	/* Switch to NS world, must be called in secure world */
	__asm__ __volatile__("MRC    p15, 0, r0, c1, c1, 0\n"
						"ORR    r0, r0, #1\n"
						"MCR    p15, 0, r0, c1, c1, 0\n"
						"ISB");

	/* Switch to SVC mode for boot OS */
	__asm__ __volatile__("CPSID   if, #0x13");
}
#endif
/**
