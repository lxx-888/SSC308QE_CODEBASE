...
loff_t ss_nand_env_offset = 0;
loff_t ss_nand_env_part_size = 0;
#if defined(CONFIG_ENV_OFFSET_REDUND)
loff_t ss_nand_env_redund_offset = 0;
#endif
#endif
...
static int env_nand_save(void)
{
	...
	if (!mtdparts_init() && !find_dev_and_part(ENV_PART_NAME, &dev, &pnum, &part)) {
		...
		CONFIG_ENV_RANGE = (loff_t)(part->size);
	} else if (sstar_part_get_active((u8 *)ENV_PART_NAME, &sstar_part)) {
		CONFIG_ENV_OFFSET = (loff_t)(sstar_part.offset);
		CONFIG_ENV_RANGE = (loff_t)(sstar_part.size);
	}

	printf("\n%s: offset = 0x%llx size = 0x%llx\r\n", ENV_PART_NAME, CONFIG_ENV_OFFSET, CONFIG_ENV_RANGE);
	...
	printf("\n%s: offset = 0x%llx size = 0x%llx\r\n", ENV_REDUND_PART_NAME, CONFIG_ENV_OFFSET_REDUND, CONFIG_ENV_RANGE);
