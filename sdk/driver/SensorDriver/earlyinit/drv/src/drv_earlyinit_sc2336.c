...
#include "SC2336_init_table.h"         /* Sensor initial table */
/* Sensor EarlyInit implementation */
SENSOR_EARLYINIY_ENTRY_IMPL_BEGIN(SC2336)

static EarlyInitSensorInfo_t _gSC2336Info =
{
...
*/
static unsigned int SC2336EarlyInitShutterAndFps( unsigned int nFpsX1000, unsigned int nShutterUs, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
...
unsigned int vts_30fps = 1125;
    ...
    _gSC2336Info.u32ShutterUs = nShutterUs;
    _gSC2336Info.u32FpsX1000 = nFpsX1000;
...
static unsigned int SC2336EarlyInitGain( unsigned int u32GainX1024, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
...
#define SENSOR_MAXGAIN     (32000*4 / 1000) // (32768*15 / 1000)//32XAGAIN 15XDGAIN
    ...
    _gSC2336Info.u32GainX1024 = u32GainX1024;
...
static unsigned int SC2336EarlyInitGetSensorInfo(EarlyInitSensorInfo_t* pSnrInfo)
{
    if(pSnrInfo)
    {
        pSnrInfo->eBayerID      = _gSC2336Info.eBayerID;
        pSnrInfo->ePixelDepth   = _gSC2336Info.ePixelDepth;
        pSnrInfo->eIfBusType    = _gSC2336Info.eIfBusType;
        pSnrInfo->u32FpsX1000   = _gSC2336Info.u32FpsX1000;
        pSnrInfo->u32Width      = _gSC2336Info.u32Width;
        pSnrInfo->u32Height     = _gSC2336Info.u32Height;
        pSnrInfo->u32GainX1024  = _gSC2336Info.u32GainX1024;
        pSnrInfo->u32ShutterUs  = _gSC2336Info.u32ShutterUs;
        pSnrInfo->u32ShutterShortUs = _gSC2336Info.u32ShutterShortUs;
        pSnrInfo->u32GainShortX1024 = _gSC2336Info.u32GainShortX1024;
        pSnrInfo->u8ResId       = _gSC2336Info.u8ResId;
        pSnrInfo->u8HdrMode     = _gSC2336Info.u8HdrMode;
        pSnrInfo->u32TimeoutMs  = 200;
        pSnrInfo->u32CropX       = _gSC2336Info.u32CropX;
        pSnrInfo->u32CropY       = _gSC2336Info.u32CropY;
    }
    return 0;
}

/* Sensor EarlyInit implementation end*/
SENSOR_EARLYINIY_ENTRY_IMPL_END( SC2336,
                                 Sensor_init_table,
                                 SC2336EarlyInitShutterAndFps,
                                 SC2336EarlyInitGain,
                                 SC2336EarlyInitGetSensorInfo
                                );
