...
typedef enum
{
    ...
    E_EARLY_INIT_CMD_ORITATION      = 23,
    E_EARLY_INIT_CMD_HDR_SHUTTER_FPS= 24,
    E_EARLY_INIT_CMD_HDR_GAIN       = 25,
    E_EARLY_INIT_CMD_STREAM_ONOFF   = 26,
    E_EARLY_INIT_CMD_CSI_CONFIG     = 27,
}EarlyInitCmdID_e;
...
{
    EarlyInitCmdHead_t tHead;
    unsigned int nLefShutterUs;
    unsigned int nSefShutterUs;
    unsigned int nMefShutterUs;
    unsigned int nFpsX1000;
    unsigned char nSlaveAddr;
    unsigned char nI2cFmt;
}EarlyInitHDRShutterFps_t;
...
typedef struct __attribute__ ((packed))
{
    EarlyInitCmdHead_t tHead;
    unsigned int nGainX1024;
    unsigned int nSefGainX1024;
    unsigned int nMefGainX1024;
    unsigned char nSlaveAddr;
    unsigned char nI2cFmt;
}EarlyInitHDRGain_t;
...
typedef struct __attribute__ ((packed))
{
    ...
    unsigned char nSlaveAddr;
    unsigned char nI2cFmt;
}EarlyInitOrientation_t;

typedef struct __attribute__ ((packed))
{
    EarlyInitCmdHead_t tHead;
    unsigned char nStreamOnoff;
    unsigned char nSlaveAddr;
    unsigned char nI2cFmt;
}EarlyInitStreamOnoff_t;

typedef struct __attribute__ ((packed))
{
    EarlyInitCmdHead_t tHead;
    unsigned int u32Clk;
    unsigned int u32Pack;
    unsigned int u32Pack2;
    unsigned char u8Lane;
    unsigned char u8Hdr;
}EarlyInitCsiCfg_t;
