...
#endif
static int OS04c10_SetAEGain(ss_cus_sensor *handle, u32 gain);
static int OS04c10_SetAEUSecs(ss_cus_sensor *handle, u32 us);
static int OS04c10_SetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit);
static int OS04c10_SetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 us);
static int OS04c10_SetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 us);
...
static int OS04c10_poweron(ss_cus_sensor *handle, u32 idx)
{
...
static int OS04c10_poweroff(ss_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
...
static int OS04c10_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{

    return SUCCESS;
}

static int OS04c10_SetFPS(ss_cus_sensor *handle, u32 fps);
static int OS04c10_AEStatusNotify(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status);

static int OS04c10_init_DCG_3p6m30(ss_cus_sensor *handle)
{
    int i;
    os04c10_params *params = (os04c10_params *)handle->private_data;
...
static int OS04c10_3p6m30_init(ss_cus_sensor *handle)
{
    int i;
    os04c10_params *params = (os04c10_params *)handle->private_data;
...
static int OS04c10_GetVideoResNum( ss_cus_sensor *handle, u32 *ulres_num)
{
    *ulres_num = handle->video_res_supported.num_res;
    return SUCCESS;
}

static int OS04c10_GetVideoRes(ss_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
...
static int OS04c10_GetCurVideoRes(ss_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS04c10_SetVideoRes(ss_cus_sensor *handle, u32 res_idx)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS04c10_SetVideoRes_HDR_DCG(ss_cus_sensor *handle, u32 res_idx)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS04c10_GetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit) {
    os04c10_params *params = (os04c10_params *)handle->private_data;
...
static int OS04c10_GetFPS(ss_cus_sensor *handle)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
...
static int OS04c10_GetFPS_HDR_lef(ss_cus_sensor *handle)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS04c10_SetFPS_HDR_lef(ss_cus_sensor *handle, u32 fps)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 min_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
static int OS04c10_GetFPS_HDR_sef(ss_cus_sensor *handle)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS04c10_SetFPS_HDR_sef(ss_cus_sensor *handle, u32 fps)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 min_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
static int OS04c10_AEStatusNotify_HDR_lef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    switch(status)
    {
...
static int OS04c10_AEStatusNotify_HDR_sef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    switch(status)
    {
...
static int OS04c10_GetAEUSecs(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS04c10_GetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS04c10_GetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
// Gain: 1x = 1024
static int OS04c10_GetAEGain(ss_cus_sensor *handle, u32* gain)
{
...
static int OS04c10_SetAEGain_HDR_sef(ss_cus_sensor *handle, u32 gain)
{
    os04c10_params *params = (os04c10_params *)handle->private_data;
    u32 input_gain = 0;
    u16 gain16 = 0;
...
static int OS04c10_SetPatternMode_hdr_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}

static int OS04c10_poweron_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}

static int OS04c10_poweroff_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}


static int OS04c10_init_hdr_lef(ss_cus_sensor *handle)
{
    return SUCCESS;
}
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"OS04c10_MIPI");
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width         = os04c10_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = os04c10_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = os04c10_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = os04c10_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = os04c10_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = os04c10_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = os04c10_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = os04c10_mipi_linear[res].senout.height;
...
int cus_camsensor_init_handle_hdr_dcg_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"OS04c10_MIPI_HDR_SEF");
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res+1;
        handle->video_res_supported.res[res].u16width         = os04c10_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = os04c10_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = os04c10_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = os04c10_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = os04c10_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = os04c10_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = os04c10_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = os04c10_mipi_hdr[res].senout.height;
...
int cus_camsensor_init_handle_hdr_dcg_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"OS04c10_MIPI_HDR_LEF");
