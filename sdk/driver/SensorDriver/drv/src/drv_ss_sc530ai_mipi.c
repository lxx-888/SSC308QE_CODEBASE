...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(SC530ai);
...
//AE info
#define SENSOR_MAX_GAIN                       (323 * 1024)
...
{
//cleaned_0x01_FT_SC530AI_27Minput_396Mbps_4lane_10bit_2880x1620_30fps   update for danny 20220427
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
{
    //cleaned_0x03_SC530AI_27Minput_792Mbps_4lane_10bit_2880x1620_SHDR_30fps update for danny 20220427
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
const static I2C_ARRAY max_short_exp_reg[] = {
    ...
    {0x3e24, 0xc8}, // expo[7:0], [3:0] fraction of line
};
...
{
    SC530ai_params *params = (SC530ai_params *)handle->private_data;
...
static int SC530ai_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_SetVideoRes(ss_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        case 0: //"2880x1620@30fps"
            ...
            params->expo.fps = 30;
            Preview_line_period  = 20202;//1sec/(vts*fps)=1sec/(1650*30)
            break;
...
static int pCus_SetVideoRes_HDR(ss_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        case 0:
            ...
            params->expo.fps = 30;
            Preview_line_period_HDR = 10101;//1sec/(vts*fps)=1sec/(3300*30)
            params->expo.max_short_exp = 200;//3e23 3e24
            break;
        default:
            break;
    }
...
static int pCus_AEStatusNotify_HDR_SEF(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    ...
    switch(status)
    {
        ...
        break;
        case CUS_FRAME_ACTIVE:
            /*CamOsPrintf(KERN_DEBUG "[%s] set  lsh:(0x%x%x%x) ssh:(0x%x%x) lgain:(%x,%x,%x)(%d) sgain:(%x,%x,%x)(%d) vts_reg value:(0x%x%x) upreg:%d upvts:%d\n", __FUNCTION__,
                params->tExpo_reg[0].data,params->tExpo_reg[1].data,params->tExpo_reg[2].data>>4,
                params->tExpo_reg_HDR_SEF[0].data,params->tExpo_reg_HDR_SEF[1].data>>4,
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain) {
    ...
    else if ((gain >= 83559)&&(gain < 167117)) //40.8 *2 ~ 40.8 *4
    {
        dcg = 83559;
        reg_gain = 0x5f;
        DIG_gain = 1;
    }
    else if (gain >= 167117) //81.6 *2 ~ 323
    {
        dcg = 83559;
        reg_gain = 0x5f;
        DIG_gain = 2;
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
    ...
    params = (SC530ai_params *)handle->private_data;
    ...
    // Sensor stream name
    ///////////////////////////////////////////////////////
    sprintf(handle->strSensorStreamName,"SC530ai_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode                            = SC530ai_SetPatternMode;
...
int cus_camsensor_init_handle_HDR_SEF(ss_cus_sensor* drv_handle)
{
    ...
    ////////////////////////////////////////////////////
    sprintf(handle->strSensorStreamName,"SC530ai_MIPI_HDR_SEF");
...
int cus_camsensor_init_handle_HDR_LEF(ss_cus_sensor* drv_handle) {
    ...
    // Sensor Stream Name info
    ////////////////////////////////////////////////////
    sprintf(handle->strSensorStreamName,"SC530ai_MIPI_HDR_LEF");
    memcpy(params->tVts_reg, vts_reg, sizeof(vts_reg));
    memcpy(params->tGain_reg, gain_reg, sizeof(gain_reg));
    memcpy(params->tExpo_reg, expo_reg, sizeof(expo_reg));
    memcpy(params->tMirror_reg, mirror_reg, sizeof(mirror_reg));
...
SENSOR_DRV_ENTRY_IMPL_END_EX( SC530ai,
