...
u32 vts_30fps;
u32 vts_30fps_DCG;
u32 Preview_line_period = 14405;
u32 Preview_line_period_DCG = 14405;
...
#endif
static int OS08A10_SetAEGain(ss_cus_sensor *handle, u32 gain);
static int OS08A10_SetAEUSecs(ss_cus_sensor *handle, u32 us);
static int OS08A10_SetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit);
static int OS08A10_SetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 us);
static int OS08A10_SetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 us);
...
static struct {     // LINEAR
    ...
    {LINEAR_RES_6, {3840, 2160, 3, 60}, {0, 0, 3840, 2160}, {"3840x2160@60fps"}}, // Modify it
};
...
//Res 3840X2160 4lane MIPI960Mbps HDRVC10 20fps MCLK24M
    ...
    {0x3797, 0x04},
    {0x3762, 0x11},
    {0x3800, (12>>8)&0xFF}, //x start
    {0x3801, (12)&0xFF},
    {0x3802, 0x00}, //y start
    {0x3803, 0x0c},
    {0x3804, ((3839+12+4)>>8)&0xFF}, //x end
    {0x3805, (3839+12+4)&0xFF},
    {0x3806, 0x08},
    {0x3807, 0x6f},
    {0x3808, 0x0f}, //x size
    {0x3809, 0x00+4},
...
//100 99 3840 2160
    ...
    {0x3806, 0x08}, //y end
    {0x3807, 0x6f},
    {0x3808, 0x0f}, //x output size
    {0x3809, 0x00+4},
    {0x380a, 0x08}, //y output size
    {0x380b, 0x70},
...
const static I2C_ARRAY Sensor_8m60_init_table[] =
{
//@@ 0 2 Res 3840X2160 4lane MIPI1440Mbps Linear10 60fps MCLK24M
    {0x0100, 0x00},
    {0x0103, 0x01},
    {0x0303, 0x01},
    {0x0305, 0x5a},
...
//100 99 2688 1520
    ...
    {0x380a, 0x06},//;08
    {0x380b, 0x78},//;70
    {0x380c, 0x09},//;  for 30fps
    {0x380d, 0xbc},//;  for 30fps
    {0x380e, 0x84},
    {0x380f, 0x07},
    {0x3813, 0x10},
    {0x3814, 0x01},
...
#define SENSOR_DMSG1(_fmt, _args...)       CamOsPrintf(KERN_DEBUG _fmt, ##_args)
#elif SENSOR_DBG == 0
//#define SENSOR_DMSG(args...)
#define SENSOR_DMSG1(_fmt, _args...)
#endif
#undef SENSOR_NAME
...
static int pCus_sensor_GetAEInfo(ss_cus_sensor *handle, CUS_SENSOR_AE_INFO_t *info)
{
    ...
    {
        info->u32AEShutter_min                   = Preview_line_period_DCG*4;
        info->u32AEShutter_step                  = Preview_line_period_DCG*2;
        if (handle->interface_attr.attr_mipi.mipi_hdr_virtual_channel_num == 1)
        {
            info->u32AEShutter_max                   = Preview_line_period_DCG * params->expo.max_short_exp;
        }
        else
        {
            info->u32AEShutter_max                   = handle->sensor_ae_info_cfg.u32AEShutter_max;
...
static int OS08A10_poweron(ss_cus_sensor *handle, u32 idx)
{
    ...
    SENSOR_DMSG1("[%s] reset low\n", __FUNCTION__);
    sensor_if->Reset(idx, SENSOR_RST_POL);
    SENSOR_UDELAY(20);
    SENSOR_DMSG1("[%s] power low\n", __FUNCTION__);
    sensor_if->PowerOff(idx, SENSOR_PWDN_POL);
    SENSOR_UDELAY(20);
    ///////////////////

    // power -> high, reset -> high
    SENSOR_DMSG1("[%s] power high\n", __FUNCTION__);
    sensor_if->PowerOff(idx, !SENSOR_PWDN_POL);
    SENSOR_UDELAY(20);
    SENSOR_DMSG1("[%s] reset high\n", __FUNCTION__);
    sensor_if->Reset(idx, !SENSOR_RST_POL);
    SENSOR_USLEEP(6000);
...
static int OS08A10_poweroff(ss_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    SENSOR_DMSG1("?????[%s]  \n", __FUNCTION__);
...
static int OS08A10_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{

    return SUCCESS;
}

static int OS08A10_init_DCG_8m20(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_init_DCG_8m30(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_init_DCG_5m30(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_init_DCG_4p8m30(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_init_DCG_2m30(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_8m60_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
    for(i=0;i< ARRAY_SIZE(Sensor_8m60_init_table);i++)
    {
        if(Sensor_8m60_init_table[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_8m60_init_table[i].data);
        }
        if(SensorReg_Write(Sensor_8m60_init_table[i].reg,Sensor_8m60_init_table[i].data) != SUCCESS)
        {
           SENSOR_DMSG1("[%s] I2C write fail\n", __FUNCTION__);
            return FAIL;
        }
...
static int OS08A10_8m30_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
...
static int OS08A10_8m20_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
...
static int OS08A10_4p8m30_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
...
static int OS08A10_5m30_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
...
static int OS08A10_2m60_init(ss_cus_sensor *handle)
{
    int i;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
...
static int OS08A10_GetVideoResNum( ss_cus_sensor *handle, u32 *ulres_num)
{
    *ulres_num = handle->video_res_supported.num_res;
    return SUCCESS;
}

static int OS08A10_GetVideoRes(ss_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
...
static int OS08A10_GetCurVideoRes(ss_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS08A10_SetVideoRes(ss_cus_sensor *handle, u32 res_idx)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 num_res = handle->video_res_supported.num_res;
    ...
    switch (res_idx) {
        ...
        case 4://3840x2160@30fps
			...
			handle->data_prec = CUS_DATAPRECISION_12;
            break;
        case 5://3840x2160@60fps
            handle->video_res_supported.ulcur_res = 5;
            handle->pCus_sensor_init = OS08A10_8m60_init;
            vts_30fps = 2314;
            params->expo.vts = vts_30fps;
            params->expo.fps = 60;
            Preview_line_period = 7202;
...
static int OS08A10_SetVideoRes_HDR_DCG(ss_cus_sensor *handle, u32 res_idx)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS08A10_GetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit) {
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_SetOrien_HDR(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
...
static int OS08A10_GetFPS(ss_cus_sensor *handle)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
...
static int OS08A10_SetFPS(ss_cus_sensor *handle, u32 fps)
{
    u32 vts = 0;
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    ...
    if(fps>=min_fps && fps <= max_fps){
        ...
        SENSOR_DMSG1("[%s] FPS %d out of range.\n",__FUNCTION__,fps);
        return FAIL;
    }
...
static int OS08A10_GetFPS_HDR_lef(ss_cus_sensor *handle)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS08A10_SetFPS_HDR_lef(ss_cus_sensor *handle, u32 fps)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 min_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
static int OS08A10_GetFPS_HDR_sef(ss_cus_sensor *handle)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS08A10_SetFPS_HDR_sef(ss_cus_sensor *handle, u32 fps)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 min_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
static int OS08A10_AEStatusNotify(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    SENSOR_DMSG1("?????[%s] %d @:%llu\n", __FUNCTION__,status,CamOsGetTimeInMs());
    switch(status)
    {
...
static int OS08A10_AEStatusNotify_HDR_lef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    //os08A10_params *params = (os08A10_params *)handle->private_data;
    switch(status)
    {
...
static int OS08A10_AEStatusNotify_HDR_sef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    switch(status)
    {
...
static int OS08A10_GetAEUSecs(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS08A10_SetAEUSecs(ss_cus_sensor *handle, u32 us)
{
    ...
    SENSOR_DMSG1("[%s] us %ld, lines %ld, vts %ld @:%llu\\n", __FUNCTION__,
                us,
                lines,
                params->expo.vts,CamOsGetTimeInMs()
                );
    // lines <<= 4;
...
static int OS08A10_GetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS08A10_SetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 us)
{
    ...
    SENSOR_DMSG1("[%s] us %ld, lines %ld, vts %ld\n", __FUNCTION__,
                us,
                lines,
                params->expo.vts
                );
...
static int OS08A10_GetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
// Gain: 1x = 1024
static int OS08A10_GetAEGain(ss_cus_sensor *handle, u32* gain)
{

    //SENSOR_DMSG1("[%s] get gain/reg0/reg1 (1024=1X)= %d/0x%x/0x%x\n", __FUNCTION__, *gain,params->tGain_reg[0].data,params->tGain_reg[1].data);
    return SUCCESS;
}
...
static int OS08A10_SetAEGain_HDR_sef(ss_cus_sensor *handle, u32 gain)
{
    os08A10_params *params = (os08A10_params *)handle->private_data;
    u32 input_gain = 0;
    u16 gain16 = 0;
...
static int OS08A10_SetPatternMode_hdr_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}

static int OS08A10_poweron_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}

static int OS08A10_poweroff_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}

static int OS08A10_init_hdr_lef(ss_cus_sensor *handle)
{
    return SUCCESS;
}
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle)
{
    ...
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
    if (!handle) {
        SENSOR_DMSG1("[%s] not enough memory!\n", __FUNCTION__);
        return FAIL;
    }
    SENSOR_DMSG1("[%s] \n", __FUNCTION__);
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName ,"OS08A10_MIPI");
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width         = os08a10_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = os08a10_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = os08a10_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = os08a10_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = os08a10_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = os08a10_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = os08a10_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = os08a10_mipi_linear[res].senout.height;
...
int cus_camsensor_init_handle_hdr_dcg_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"OS08A10_MIPI_HDR_SEF");
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res+1;
        handle->video_res_supported.res[res].u16width         = os08a10_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = os08a10_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = os08a10_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = os08a10_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = os08a10_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = os08a10_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = os08a10_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = os08a10_mipi_hdr[res].senout.height;
    ...
    params->expo.vts=vts_30fps_DCG;
    params->expo.fps = 20;
    params->expo.max_short_exp=70;
    Preview_line_period_DCG = 14404;
    handle->sensor_ae_info_cfg.u8AEGainDelay                      = SENSOR_GAIN_DELAY_FRAME_COUNT_HDR;
    handle->sensor_ae_info_cfg.u8AEShutterDelay                   = SENSOR_SHUTTER_DELAY_FRAME_COUNT_HDR;
    ...
    handle->sensor_ae_info_cfg.u32AEGain_min                      = SENSOR_MIN_GAIN;
    handle->sensor_ae_info_cfg.u32AEGain_max                      = SENSOR_MAX_GAIN;
    handle->sensor_ae_info_cfg.u32AEShutter_min                   = Preview_line_period_DCG*4;
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = Preview_line_period_DCG * params->expo.max_short_exp;
    handle->sensor_ae_info_cfg.u32AEShutter_step                  = Preview_line_period_DCG*2;
    return SUCCESS;
}
...
int cus_camsensor_init_handle_hdr_dcg_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"OS08A10_MIPI_HDR_LEF");
