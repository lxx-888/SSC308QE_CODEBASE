...
static struct {     // LINEAR
    ...
    {LINEAR_RES_1, {3864, 2192, 3, 30}, {0, 4, 3840, 2160}, {"3840x2160@30fps"}},  // table 1
    {LINEAR_RES_2, {2616, 1974, 3, 30}, {0, 4, 2592, 1944}, {"2592x1944@30fps"}},  // table 2
    {LINEAR_RES_3, {2592, 1470, 3, 30}, {0, 4, 2560, 1440}, {"2560x1440@30fps"}},  // table 3
    {LINEAR_RES_4, {1932, 1090, 3, 60}, {0, 4, 1920, 1080}, {"1920x1080@60fps"}},  // table 4
    {LINEAR_RES_5, {1932, 1090, 3, 60}, {0, 4, 1920, 1080}, {"Cropping2m@60fps"}}, // table 7
};
...
static struct {     // HDR
    ...
    {HDR_RES_1, {3864, 2192, 3, 30}, {0, 4, 3840, 2160}, {"3840x2160@30fps_HDR"}}, // Modify it
};
...
static int pCus_sensor_GetAEInfo(ss_cus_sensor *handle, CUS_SENSOR_AE_INFO_t *info)
{
    ...
    else
    {
        ...
        {
            info->u32AEShutter_max                   = Preview_line_period_HDR_DOL * params->max_rhs1;
        }
        else
        {
            info->u32AEShutter_max                   = handle->sensor_ae_info_cfg.u32AEShutter_max;
...
static int pCus_poweron(ss_cus_sensor *handle, u32 idx)
{
    ...
    sensor_if->PowerOff(idx, CUS_CLK_POL_NEG);//!handle->pwdn_POLARITY); It should set off
    sensor_if->Reset(idx, CUS_CLK_POL_NEG);//!handle->reset_POLARITY); It should set off
...
static int imx334_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
    int i;
    switch(mode)
    {
...
static int pCus_init_mipi4lane_5m30fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane_4m30fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane_binning_60fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane_cropping_2m60fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
#if 0
static int pCus_init_mipi4lane_2m30fps_HDR_DOL(ss_cus_sensor *handle)
{
    int i,cnt=0;
    SENSOR_DMSK("[%s]\n", __FUNCTION__);
...
static int pCus_init_mipi4lane_8m30fps_HDR_DOL(ss_cus_sensor *handle)
{
    int i,cnt=0;
    SENSOR_DMSK("[%s]\n", __FUNCTION__);
...
static int pCus_SetVideoRes_HDR_DOL(ss_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        case 0:
            ...
            Preview_MAX_FPS = 30;
            params->expo.fps=Preview_MAX_FPS;
            Preview_line_period_HDR_DOL = 13154;  // 33.3/2250
...
static int IMX334_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int IMX334_GetShutterInfoHDR_DOL_SEF1(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    imx334_params *params = (imx334_params *)handle->private_data;
...
int cus_camsensor_init_handle_linear(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID             //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX334_MIPI_Linear");
   ...
   //handle->video_res_supported.num_res = LINEAR_RES_END;
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width         = imx334_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = imx334_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = imx334_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = imx334_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = imx334_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = imx334_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = imx334_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = imx334_mipi_linear[res].senout.height;
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000000/imx334_mipi_linear[0].senout.min_fps;
...
int cus_camsensor_init_handle_hdr_dol_sef1(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID             //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX334_MIPI_HDR_SEF");
    ...
    handle->interface_attr.attr_mipi.mipi_hdr_mode                = SENSOR_HDR_MODE; //CUS_HDR_MODE_SONY_DOL;
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res+1;
        handle->video_res_supported.res[res].u16width         = imx334_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = imx334_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = imx334_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = imx334_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = imx334_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = imx334_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = imx334_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = imx334_mipi_hdr[res].senout.height;
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = Preview_line_period_HDR_DOL * params->max_rhs1;
    handle->sensor_ae_info_cfg.u32AEShutter_step                  = Preview_line_period_HDR_DOL;
...
#endif
static int imx334_SetPatternMode_hdr_dol_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}
...
static int IMX334_GetShutterInfo_hdr_dol_lef(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    info->max = 1000000000/imx334_mipi_linear[0].senout.min_fps;
    info->min = (Preview_line_period_HDR_DOL * 4);
    info->step = Preview_line_period_HDR_DOL;
...
static int cus_camsensor_init_handle_hdr_dol_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID             //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX334_MIPI_HDR_LEF");
