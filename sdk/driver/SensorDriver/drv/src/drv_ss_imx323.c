...
static int Imx323SetAEGain(ss_cus_sensor *handle, u32 gain);
static int Imx323SetAEUSecs(ss_cus_sensor *handle, u32 us);
...
const I2C_ARRAY Sensor_init_table[] =
{
    ...
    //{0x0344, 0x00},//??remove by eddie
    //{0x0345, 0x00},//??remove by eddie
    //{0x0346, 0x00},//??remove by eddie
    //{0x0347, 0x00},//??remove by eddie

    //{0x034C, 0x07}, //??remove by eddie
    //{0x034D, 0xC0}, // 1984??remove by eddie
    //{0x034E, 0x04}, //??remove by eddie
    //{0x034F, 0xC9}, // 1225??remove by eddie
    {0x3000, 0x31},
    {0x3001, 0x00},
    {0x3002, 0x0F},
    //{0x300D, 0x00}, //remove by eddie
    //{0x300E, 0x00}, //remove by eddie
    //{0x300F, 0x00}, //remove by eddie
    //{0x3010, 0x00}, //remove by eddie
    {0x3011,   0x00},
    //{0x3011, 0x01}, //FPGA, FRSEL

    {0x3012, 0x82},
    {0x3013, 0x40}, // add by eddie
    ...
    //{0x3022, 0x00},
    {0x3021, 0x00}, // 6clk 0x00, // XHSLNG// add by eddie
    {0x3022, 0x40},// add by eddie

    {0x3027, 0x20},
    {0x302C, 0x00}, // change by eddie
    {0x303B, 0xE1},
    ...
    //{0x30CE, 0x16},//remove by eddie
    //{0x30CF, 0x82},//remove by eddie
    //{0x30D0, 0x00},//remove by eddie
    {0x3117, 0x0D},
...
static int Imx323SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
	int i;
	switch(mode)
	{
...
static int Imx323Init(ss_cus_sensor *handle)
{
    int i,cnt=0;
    //ISensorIfAPI *sensor_if = &(handle->sensor_if_api);
...
static int Imx323GetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit)
{
	char sen_data;
...
static int Imx323SetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
	char index=0;
...
static int Imx323GetFPS(ss_cus_sensor *handle)
{
	imx323_params *params = (imx323_params *)handle->private_data;
...
static int Imx323SetFPS(ss_cus_sensor *handle, u32 fps)
{
	imx323_params *params = (imx323_params *)handle->private_data;
	//SENSOR_DMSG("[%s]\n", __FUNCTION__);
	if(fps>=3 && fps <= 30)
...
static int Imx323AEStatusNotify(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
#if __ENABLE_AE__
    imx323_params *params = (imx323_params *)handle->private_data;
    //SENSOR_DMSG("[%s]\n", __FUNCTION__);
...
static int Imx323GetAEUSecs(ss_cus_sensor *handle, u32 *us)
{
    imx323_params *params = (imx323_params *)handle->private_data;
    u32 lines = 0;
...
// Gain: 1x = 1024
static int Imx323GetAEGain(ss_cus_sensor *handle, u32* gain)
{
    //u16 temp_gain;
...
int Imx323InitHandle(ss_cus_sensor* handle)
{
    ...
    sprintf(handle->strSensorStreamName,"IMX323_PARL");
    ...
    handle->video_res_supported.res[0].u16max_fps       = 30;
    handle->video_res_supported.res[0].u16min_fps       = 3;
    ...
    handle->video_res_supported.res[0].u16OutputWidth  = 0x882;
    handle->video_res_supported.res[0].u16OutputHeight = 0x464;
