...
#define SENSOR_MAXGAIN          955           // max sensor gain, a-gain*conversion-gain*d-gain  60.5*15.8
#define SENSOR_MINGAIN          1024
...
static int sc450ai_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain) {
    ...
    if(gain < 61976)
    {
        ...
        DIG_Fine_gain_reg = abs(8192ULL * gain /(Dcg_gainx1024 * Coarse_gain * ANA_Fine_gain_reg));
    }else{
        DIG_Fine_gain_reg = abs(8192ULL * gain /(Dcg_gainx1024 * Coarse_gain * DIG_gain) / ANA_Fine_gainx64);
    }
...
static int pCus_sensor_GetAEInfo(ss_cus_sensor *handle, CUS_SENSOR_AE_INFO_t *info)
{
    ...
    if(handle->interface_attr.attr_mipi.mipi_hdr_mode == CUS_HDR_MODE_NONE) //linear
    {
    ...
    else
    {
        ...
        if (handle->interface_attr.attr_mipi.mipi_hdr_virtual_channel_num == 1)
        {
            info->u32AEShutter_max           = Preview_line_period_HDR * (2 * (params->expo.max_short_exp)-2);
        }
        else
        {
            info->u32AEShutter_max           = 1000000000/handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width         = sc450ai_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = sc450ai_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = sc450ai_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = sc450ai_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = sc450ai_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = sc450ai_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth   = sc450ai_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight  = sc450ai_mipi_linear[res].senout.height;
        strcpy(handle->video_res_supported.res[res].strResDesc, sc450ai_mipi_linear[res].senstr.strResDesc);
    }
    ...
    handle->sensor_ae_info_cfg.u32AEGain_min                      = SENSOR_MINGAIN;
...
int cus_camsensor_init_handle_HDR_SEF(ss_cus_sensor* drv_handle)
{
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res+1;
        handle->video_res_supported.res[res].u16width         = sc450ai_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height        = sc450ai_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps       = sc450ai_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps       = sc450ai_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x  = sc450ai_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y  = sc450ai_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth   = sc450ai_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight  = sc450ai_mipi_hdr[res].senout.height;
        strcpy(handle->video_res_supported.res[res].strResDesc, sc450ai_mipi_hdr[res].senstr.strResDesc);
    }
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = Preview_line_period_HDR * (2 * (params->expo.max_short_exp)-2);
