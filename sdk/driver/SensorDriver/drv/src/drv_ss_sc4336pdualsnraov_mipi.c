...
const I2C_ARRAY Sensor_init_table_4M15fps_slave[] =    //Cleaned_0x01_FT_SC4336P_MIPI_27Minput_2lane_10bit_630Mbps_2560x1440_15fps_LDO_1.35v
{
    {0x0103,0x01},
    {0x36e9,0x80},
    {0x37f9,0x80},
...
static int pCus_Suspend(struct __ss_cus_sensor *handle, u32 idx)
{
    ...
    sensor_if->MCLK(idx, 0, UseParaMclk(SENSOR_DRV_PARAM_MCLK())); // handle->mclk);
#endif
    params->g_u8SuspendFlag = false;

    SENSOR_EMSG("[%s] sensor%d suspend!\n", __FUNCTION__, idx);
    return SUCCESS;
}
...
static int pCus_Resume(struct __ss_cus_sensor *handle, u32 idx)
{
    ...
    sensor_if->SetCSI_LongPacketType(idx, 0, 0x1C00, 0);
    sensor_if->MCLK(idx, 1, UseParaMclk(SENSOR_DRV_PARAM_MCLK())); //handle->mclk);
    CamOsMsDelay(4);//or ae notify i2c err

    _pCus_SleepMode(handle, SENSOR_SLEEP_MODE_DISABLE, idx);

    pCus_AEStatusNotify(handle, idx, CUS_FRAME_ACTIVE);
...
SENSOR_DRV_ENTRY_IMPL_END_EX( SC4336P_DUALSNR_AOV,
