...
//AE info
#define SENSOR_MAX_GAIN                       128                 // max sensor gain 128x
#define SENSOR_MIN_GAIN                       (1)                 // min gain 1x
...
{0x12, 0x80},
{0x13, 0x07},//LWC
{0x15, 0x10},//[1:0]clk_lane_mode; 0x10:mipi no continue mode;0x12:mipi continue mode;
{0xfe, 0x00},
...
#endif
static int gc2053_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_GetFPS(ss_cus_sensor *handle)
{
    ...
    u32 tVts = vts_reg[1].data << 8 | vts_reg[0].data ;
...
static int gc2053_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"gc2053_MIPI_Linear");
