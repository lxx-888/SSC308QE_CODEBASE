...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(SC431HAI_HDR);
...
static struct {     // HDR
   ...
   {HDR_RES_1, {2560, 1440, 3, 25}, {0, 0, 2560, 1440}, {"2560x1440@25fps_HDR"}}, // Modify it
};
...
{
  //Cleaned_0x01_sc431hai_MIPI_27Minput_2Lane_10bit_630Mbps_2560x1440_30fps_20240115
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x3018,0x3a},
...
const static I2C_ARRAY Sensor_init_table_HDR_2lane[] =
{
	//cleaned_0x09_sc431hai_MIPI_27Minput_2Lane_10bit_1080Mbps_2560x1440_30fps_SHDR_VC_20240625
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
	{0x3018,0x3a},
...
static int pCus_init_mipi4lane_HDR(ss_cus_sensor *handle)
{
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_HDR_2lane);i++)
    {
        if(Sensor_init_table_HDR_2lane[i].reg==0xffff)
        {
            //MsSleep(RTK_MS_TO_TICK(1));//usleep(1000*Sensor_init_table_2560_1440_30_HDR[i].data);
            SENSOR_MSLEEP(Sensor_init_table_HDR_2lane[i].data);
        }else
        {
            cnt = 0;
            SENSOR_DMSG("reg =  %x, data = %x\n", Sensor_init_table_HDR_2lane[i].reg, Sensor_init_table_HDR_2lane[i].data);
            while(SensorReg_Write(Sensor_init_table_HDR_2lane[i].reg,Sensor_init_table_HDR_2lane[i].data) != SUCCESS)
            {
                cnt++;
                 SENSOR_DMSG("Sensor_init_table_HDR_2lane -> Retry %d...\n",cnt);
                if(cnt>=10)
                {
...
SENSOR_DRV_ENTRY_IMPL_END_EX(SC431HAI_HDR,
