...
static int OS05A20_SetAEGain(ss_cus_sensor *handle, u32 gain);
static int OS05A20_SetAEUSecs(ss_cus_sensor *handle, u32 us);

u32 vts_DCG = 2500;
u32 Preview_line_period = 16801;
u32 Preview_line_period_DCG = 16000;
...
static int pCus_sensor_GetAEInfo(ss_cus_sensor *handle, CUS_SENSOR_AE_INFO_t *info)
{
    ...
    {
        info->u32AEShutter_min                   = Preview_line_period_DCG*3;
...
static int OS05A20_poweron(ss_cus_sensor *handle, u32 idx)
{
...
static int OS05A20_poweroff(ss_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    SENSOR_DMSG("[%s] reset low\n", __FUNCTION__);
...
static int OS05A20_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
/*     int i;
    switch(mode)
    {
...
static int OS05A20_init(ss_cus_sensor *handle)
{
    int i;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_init_DCG(ss_cus_sensor *handle)
{
    int i;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_init_DCG_1(ss_cus_sensor *handle)
{
    int i;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_GetVideoResNum( ss_cus_sensor *handle, u32 *ulres_num)
{
    *ulres_num = handle->video_res_supported.num_res;
    return SUCCESS;
}

static int OS05A20_GetVideoRes(ss_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
...
static int OS05A20_GetCurVideoRes(ss_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS05A20_GetVideoResNum_LEF( ss_cus_sensor *handle, u32 *ulres_num)
{
    *ulres_num = handle->video_res_supported.num_res;
    return SUCCESS;
}

static int OS05A20_GetVideoRes_LEF(ss_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS05A20_GetCurVideoRes_LEF(ss_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS05A20_SetVideoRes(ss_cus_sensor *handle, u32 res_idx)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 num_res = handle->video_res_supported.num_res;
...
static int OS05A20_SetVideoRes_HDR_DCG(ss_cus_sensor *handle, u32 res_idx)
{
    u32 num_res = handle->video_res_supported.num_res;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_SetVideoRes_HDR_DCG_LEF(ss_cus_sensor *handle, u32 res_idx)
{
    u32 num_res = handle->video_res_supported.num_res;
    //os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_GetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit) {
    os05a20_params *params = (os05a20_params *)handle->private_data;
    return params->mirror_flip.cur;
}

static int OS05A20_SetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    switch(orit)
    {
...
static int OS05A20_SetOrien_HDR(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit)
{
...
static int OS05A20_GetFPS(ss_cus_sensor *handle)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_SetFPS(ss_cus_sensor *handle, u32 fps)
{
    u32 vts = 0;
    os05a20_params *params = (os05a20_params *)handle->private_data;
    SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
...
static int OS05A20_GetFPS_HDR_LEF(ss_cus_sensor *handle)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS05A20_SetFPS_HDR_lef(ss_cus_sensor *handle, u32 fps)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
...
static int OS05A20_GetFPS_HDR_SEF(ss_cus_sensor *handle)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);
...
static int OS05A20_SetFPS_HDR_sef(ss_cus_sensor *handle, u32 fps)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 max_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16max_fps;
    u32 min_fps = handle->video_res_supported.res[handle->video_res_supported.ulcur_res].u16min_fps;
...
static int OS05A20_AEStatusNotify(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    switch(status)
    {
...
static int OS05A20_AEStatusNotify_HDR_lef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    //os05a20_params *params = (os05a20_params *)handle->private_data;
    switch(status)
    {
...
static int OS05A20_AEStatusNotify_HDR_sef(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_GetAEUSecs(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS05A20_GetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS05A20_SetAEUSecs_HDR_lef(ss_cus_sensor *handle, u32 us)
{
    u32 lines = 0, vts = 0;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
static int OS05A20_GetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 *us)
{
    int rc = SUCCESS;
    u32 lines = 0;
...
static int OS05A20_SetAEUSecs_HDR_sef(ss_cus_sensor *handle, u32 us)
{
    u32 lines = 0, vts = 0;
    os05a20_params *params = (os05a20_params *)handle->private_data;
...
// Gain: 1x = 1024
static int OS05A20_GetAEGain(ss_cus_sensor *handle, u32* gain)
{
...
static int OS05A20_SetAEGain_HDR_sef(ss_cus_sensor *handle, u32 gain)
{
    os05a20_params *params = (os05a20_params *)handle->private_data;
    u32 input_gain = 0;
    u16 gain16 = 0;
...
static int OS05A20_SetPatternMode_hdr_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}

static int OS05A20_poweron_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}

static int OS05A20_poweroff_hdr_lef(ss_cus_sensor *handle, u32 idx)
{
    return SUCCESS;
}

static int OS05A20_init_hdr_lef(ss_cus_sensor *handle)
{
    return SUCCESS;
}
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"OS05A20_MIPI");
    ...
    params->mirror_flip.cur      = SENSOR_ORIT;      //CUS_ORIT_M1F1;
    ...
    handle->video_res_supported.res[0].u16OutputWidth= 0xA20;
    handle->video_res_supported.res[0].u16OutputHeight= 0x798;
...
int cus_camsensor_init_handle_hdr_dcg_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"OS05A20_MIPI_HDR_SEF");
    ...
    handle->video_res_supported.res[0].u16width = Preview_WIDTH;
    handle->video_res_supported.res[0].u16height = Preview_HEIGHT;
    handle->video_res_supported.res[0].u16max_fps= 25;
    handle->video_res_supported.res[0].u16min_fps= 5;
    handle->video_res_supported.res[0].u16crop_start_x= 0;
    handle->video_res_supported.res[0].u16crop_start_y= 0;
    handle->video_res_supported.res[0].u16OutputWidth= 0xA20;
    handle->video_res_supported.res[0].u16OutputHeight= 0x798;
    ...
    handle->video_res_supported.res[1].u16min_fps= 5;
    handle->video_res_supported.res[1].u16crop_start_x= 0;
    handle->video_res_supported.res[1].u16crop_start_y= 0;
    handle->video_res_supported.res[1].u16OutputWidth= 0xA20;
    handle->video_res_supported.res[1].u16OutputHeight= 0x798;
    ...
    handle->sensor_ae_info_cfg.u32AEGain_min                      = SENSOR_MIN_GAIN;
    handle->sensor_ae_info_cfg.u32AEGain_max                      = SENSOR_MAX_GAIN;
    handle->sensor_ae_info_cfg.u32AEShutter_min                   = Preview_line_period_DCG*3;
...
int cus_camsensor_init_handle_hdr_dcg_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"OS05A20_MIPI_HDR_LEF");
