...
#define SENSOR_MAX_GAIN                             (80 * 1024)        // max sensor again, a-gain * conversion-gain*d-gain
...
static struct {     // LINEAR
    ...
    {LINEAR_RES_2, {1920, 1080, 5, 30}, {0, 0, 1920, 1080}, {"HDR 1920x1080@30fps"}},
    {LINEAR_RES_3, {1920, 1080, 5, 30}, {0, 0, 1920, 1080}, {"14BIT 1920x1080@30fps"}},
};
...
u16 gu16DgainLBH = 0, gu16DgainLBL = 0, u16GainMin = 0;	// For Min Gain by Temp
#define	SENSOR_GAIN_MIN		(2)	// For Min Gain by Temp
//static int  drv_Fnumber = 22;
...
typedef struct {
    ...
    I2C_ARRAY tVts_reg[2];	// Fix bug of #
    I2C_ARRAY tGain_reg[2];	// Fix bug of #
    I2C_ARRAY tExpo_reg[6];	// Fix bug of #
#if ENABLE_ver
    I2C_ARRAY tNr_reg[6];
#endif
    int sen_init;
...
static int ps5260_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_GetFPS(ss_cus_sensor *handle)
{
    ...
    u32 tVts = (params->tVts_reg[0].data << 8) | (params->tVts_reg[1].data << 0);	// Fix bug of #
...
static int pCus_SetFPS(ss_cus_sensor *handle, u32 fps)
{
    ...
    u32Temp_Manual = (u32Temp_Manual*params->expo.vts)/((params->tVts_reg[0].data<<8)+params->tVts_reg[1].data);	// Fix bug of #
...
static int pCus_AEStatusNotify(ss_cus_sensor *handle, u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status){
    ...
    switch(status)
    {
        ...
        case CUS_FRAME_ACTIVE:
            ...
            line_write|= (u32)(params->tExpo_reg[0].data&0xff)<<8;
            line_write|= (u32)(params->tExpo_reg[1].data&0xff)<<0;
...
//#endif
            ...
            if((params->nCompanding)&&(framecount > ((params->expo.fps)/1000)/4))	// run at every 0.25s
			{
				...
				u16GainMin = (u16GainMin > 4)?4:u16GainMin;
				if(params->tGain_reg[0].data < (SENSOR_GAIN_MIN+u16GainMin))	// For Min Gain by Temp
				{
					params->tGain_reg[0].data = (SENSOR_GAIN_MIN+u16GainMin);	// For Min Gain by Temp
					params->dirty = true;
				}
			}
#endif
...
#if 1//PS5260 needn't SW ne_patch
    params->tExpo_reg[2].data = (u16)((ne >>8) & 0x001f);
    params->tExpo_reg[3].data = (u16)((ne >>0) & 0x00ff);
    params->tExpo_reg[4].data = (u16)(((ne_patch & 0x0100) >>8)<<2);
    params->tExpo_reg[5].data = (u16)((ne_patch >>0) & 0x00ff);
#endif

    u32Temp_Manual = (u32Temp_Manual*vts)/((params->tVts_reg[0].data<<8)+params->tVts_reg[1].data);	// Fix bug of #
...
static int pCus_GetAEGain(ss_cus_sensor *handle, u32* gain) {
  ps5260_params *params = (ps5260_params *)handle->private_data;
    ...
    *gain=params->expo.final_gain;
    SENSOR_DMSG("[%s] set gain/reg=%d/0x%x\n", __FUNCTION__, gain,params->tGain_reg[0].data);	// Fix bug of #
...
static int pCus_SetAEGain_cal(ss_cus_sensor *handle, u32 gain) {
    ...
    for(i = (SENSOR_GAIN_MIN+u16GainMin);i < 96;i++)	// For Min Gain by Temp
    {
        if(gain < gain_table[i].total_gain)    break;
    }
    params->tGain_reg[0].data = (params->tGain_reg[1].data)?i:(i-16);	// Fix bug of #
    SENSOR_DMSG("[%s] set gain/reg=%d/0x%x\n", __FUNCTION__, gain,params->tGain_reg[0].data);	// Fix bug of #
    return SUCCESS;
}*/
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain) {
    ...
    params->tGain_reg[0].data = sens ?i:(i-16);    // Davis 20181101 -- 20200513 for normal usage
    params->tGain_reg[1].data = sens;
...
int cus_camsensor_init_handle_linear(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"ps5260_MIPI");
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width             = ps5260_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height            = ps5260_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps           = ps5260_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps           = ps5260_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x      = ps5260_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y      = ps5260_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth       = ps5260_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight      = ps5260_mipi_linear[res].senout.height;
        sprintf(handle->video_res_supported.res[res].strResDesc, ps5260_mipi_linear[res].senstr.strResDesc);
    }
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000000/ps5260_mipi_linear[0].senout.min_fps;
