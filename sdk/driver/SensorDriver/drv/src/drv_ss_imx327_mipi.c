...
#define SENSOR_CHANNEL_imx327_ISP_CALIBRATION_ENABLE (1)
...
//For MCLK = 37.125Mhz
u32 Preview_line_period = 29629;       // hts=33.333/1125=25629,
#if HDR_DOL_10bit
//For MCLK = 36Mhz
...
#if HDR_DOL_10bit
#define vts_30fps_HDR_DOL_4lane                1220//1125   //for 29.091fps @ MCLK=36MHz //VMAX
#endif
#if HDR_DOL_12bit
#define vts_30fps_HDR_DOL_4lane                1158//1125   //for 29.091fps @ MCLK=36MHz //VMAX
#define vts_30fps_HDR_DOL_2lane                1158
#endif
#define MAX_DOL_RHS1                           ((1158 * 2) - (1109 * 2) - 21) //must 1158
...
#define SENSOR_I2C_SPEED                       20000                   //I2C speed, 60000~320000
...
const static I2C_ARRAY Sensor_init_table_4lane_60fps[] =
{
    {0x3002, 0x01},   //Master mode stop
    {0xffff, 0x14},//delay
    {0x3000, 0x01},   // standby
...
const static I2C_ARRAY Sensor_init_table_2lane_60fps[] =
{
    {0x3002, 0x01},   //Master mode stop
    {0xffff, 0x14},//delay
    {0x3000, 0x01},   // standby
...
static int imx327_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
    int i;
  switch(mode)
  {
...
static int pCus_init_mipi4lane_linear_60fps(ss_cus_sensor *handle)
{
    imx327_params *params = (imx327_params *)handle->private_data;
    int i,cnt=0;
    //u16 sen_data;
    ...
    //UartSendTrace("IMX327 Sensor_init_table_4lane_60fps\n");
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_4lane_60fps);i++)
    {
        if(Sensor_init_table_4lane_60fps[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_init_table_4lane_60fps[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_init_table_4lane_60fps[i].reg,Sensor_init_table_4lane_60fps[i].data) != SUCCESS)
            {
                cnt++;
                //printf("Sensor_init_table_4lane -> Retry %d...\n",cnt);
                if(cnt>=10)
            ...
            //SensorReg_Read( Sensor_init_table_4lane_60fps[i].reg, &sen_data );
            //SENSOR_EMSG("IMX327 reg: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_init_table_4lane_60fps[i].reg, Sensor_init_table_4lane_60fps[i].data, sen_data);
        }
    }
...
static int pCus_init_mipi2lane_HDR_DOL(ss_cus_sensor *handle)
{
    ...
    //UartSendTrace("IMX327 Sensor_init_table_HDR_DOL_2lane\n");
...
static int pCus_init_mipi2lane_linear_60fps(ss_cus_sensor *handle)
{
    int i,cnt=0;
    imx327_params *params = (imx327_params *)handle->private_data;
    SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_2lane_60fps);i++)
    {
		...
		{
			SENSOR_MSLEEP_(Sensor_init_table_2lane_60fps[i].data);
		}
		else
		{
			cnt = 0;
			...
			//SensorReg_Read( Sensor_init_table_2lane_60fps[i].reg, &sen_data );
			//UartSendTrace("IMX327 reg: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_init_table_2lane_60fps[i].reg, Sensor_init_table_2lane_60fps[i].data, sen_data);
		}
	}
...
static int pCus_SetVideoRes(ss_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        ...
        case 1:
            ...
            if(lane_num == 2)
                handle->pCus_sensor_init = pCus_init_mipi2lane_linear_60fps;
            else if (lane_num == 4)
                handle->pCus_sensor_init = pCus_init_mipi4lane_linear_60fps;
            else
                handle->pCus_sensor_init = pCus_init_mipi2lane_linear_60fps;
...
static int pCus_SetFPS(ss_cus_sensor *handle, u32 fps)
{
    ...
    if (max_fps == 30) {
        ...
        if(fps>=3 && fps <= 60){
            if (CUS_CMU_CLK_36MHZ == handle->mclk)
                fps = fps>58?58:fps;//limit fps at 58 fps due to MCLK=36MHz
            params->expo.fps = fps;
            if (CUS_CMU_CLK_36MHZ == handle->mclk)
                params->expo.vts=  (vts_30fps*58182 + fps * 500)/(fps * 1000);
            else
                params->expo.vts=  (vts_30fps*60000 + fps * 500)/(fps * 1000);
            params->dirty = true;
        }else if(fps>=3000 && fps <= 60000){
            if (CUS_CMU_CLK_36MHZ == handle->mclk)
                fps = fps>58182?58182:fps;//limit fps at 29.091 fps due to MCLK=36MHz
            params->expo.fps = fps;
            if (CUS_CMU_CLK_36MHZ == handle->mclk)
                params->expo.vts=  (vts_30fps*58182 + (fps>>1))/fps;
            else
                params->expo.vts=  (vts_30fps*60000 + (fps>>1))/fps;
            params->dirty = true;
        }else{
          //params->expo.vts=vts_30fps;
          //params->expo.fps=30;
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain)
{
   ...
   // u32 times = log2((double)gain/1024.0f)/log(2);
    ...
    params->tGain_reg[1].data &= ~0x3;

    if (max_fps == 60) {
        params->tGain_reg[1].data |= 0x1;
    } else {
        params->tGain_reg[1].data |= 0x2;
    }

    if(gain>=22925)//if gain exceed 2x , enable high conversion gain, >27DB, enable HCG
    {

        if ((params->tGain_reg[1].data & 0x10) == 0x0) {
            // params->change = true;
        ...
        if((params->tGain_reg[1].data & 0x10) == 0x10){
            // params->change = true;
            //  gain_reg[2].data=0x08;
        }
        else{
...
static int pCus_GetDGainRemainder(ss_cus_sensor *handle, u32 *dgain_remainder)
{
...
static int IMX327_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int IMX327_GetShutterInfo_hdr_dol_sef1(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX327_MIPI_Linear");
    ...
    handle->video_res_supported.res[0].u16OutputWidth = 0x79C;
    handle->video_res_supported.res[0].u16OutputHeight = 0x449;
    sprintf(handle->video_res_supported.res[0].strResDesc, "1920x1080@30fps");

    handle->video_res_supported.num_res = 2;
    handle->video_res_supported.ulcur_res = 0; //default resolution index is 0.
    handle->video_res_supported.res[1].u16width = Preview_WIDTH;
    handle->video_res_supported.res[1].u16height = Preview_HEIGHT;
    handle->video_res_supported.res[1].u16max_fps= 60;
    handle->video_res_supported.res[1].u16min_fps= Preview_MIN_FPS;
    handle->video_res_supported.res[1].u16crop_start_x= Preview_CROP_START_X;
    handle->video_res_supported.res[1].u16crop_start_y= Preview_CROP_START_Y;
    handle->video_res_supported.res[1].u16OutputWidth = 0x79C;
    handle->video_res_supported.res[1].u16OutputHeight = 0x449;
    sprintf(handle->video_res_supported.res[1].strResDesc, "1920x1080@60fps");
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_step                  = Preview_line_period*1;
...
int cus_camsensor_init_handle_hdr_dol_sef1(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"IMX327_MIPI_HDR_SEF");
    ...
    handle->video_res_supported.ulcur_res = 0; //default resolution index is 0.
    handle->video_res_supported.res[0].u16width = Preview_WIDTH;
    handle->video_res_supported.res[0].u16height = Preview_HEIGHT_HDR_DOL; //TBD. Workaround for Sony DOL HDR mode
    handle->video_res_supported.res[0].u16max_fps= Preview_MAX_FPS;
    handle->video_res_supported.res[0].u16min_fps= Preview_MIN_FPS;
    handle->video_res_supported.res[0].u16crop_start_x= Preview_CROP_START_X;
    handle->video_res_supported.res[0].u16crop_start_y= Preview_CROP_START_Y;
    ...
    handle->pCus_sensor_GetAEInfo                                 = pCus_sensor_GetAEInfo;
    //handle->pCus_sensor_GetShutterInfo = IMX327_GetShutterInfo_hdr_dol_sef1;
...
static int pCus_init_mipi4lane_linear_isp_calibration(ss_cus_sensor *handle)
{
    return SUCCESS;
}
#endif
#if 1//(SENSOR_MIPI_LANE_NUM == 2)
static int pCus_init_mipi2lane_linear_isp_calibration(ss_cus_sensor *handle)
{
    return SUCCESS;
}
#endif
static int pCus_poweron_isp_calibration( ss_cus_sensor *handle, u32 idx)
{
    //ISensorIfAPI *sensor_if = handle->sensor_if_api;
    //SENSOR_DMSG( "[%s] ", __FUNCTION__ );
...
static int pCus_poweroff_isp_calibration( ss_cus_sensor *handle, u32 idx)
{
    //ISensorIfAPI *sensor_if = handle->sensor_if_api;
...
#if 0
static int pCus_GetSensorID_isp_calibration( ss_cus_sensor *handle, u32 *id )
{
    *id = 0;
     return SUCCESS;
}
#endif
static int imx327_SetPatternMode_isp_calibration( ss_cus_sensor *handle, u32 mode )
{
    return SUCCESS;
}
#if 0
static int pCus_I2CWrite_isp_calibration(ss_cus_sensor *handle, unsigned short usreg, unsigned short usval)
{
    return SUCCESS;
}

static int pCus_I2CRead_isp_calibration(ss_cus_sensor *handle, unsigned short usreg)
{
    return SUCCESS;
}

static int pCus_init_isp_calibration( ss_cus_sensor *handle )
{
    return SUCCESS;
}
#endif
static int pCus_GetVideoRes_isp_calibration( ss_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res )
{
    *res = &handle->video_res_supported.res[res_idx];
    return SUCCESS;
}

static int pCus_SetVideoRes_isp_calibration( ss_cus_sensor *handle, u32 res )
{
    handle->video_res_supported.ulcur_res = 0; //TBD
    return SUCCESS;
}

static int pCus_GetOrien_isp_calibration( ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT *orit )
{
    //imx327_params *params = ( imx327_params * ) handle->private_data;
...
static int pCus_SetOrien_isp_calibration( ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit )
{
    return SUCCESS;
}

static int pCus_GetFPS_isp_calibration( ss_cus_sensor *handle )
{
    return SUCCESS;
}

static int pCus_SetFPS_isp_calibration( ss_cus_sensor *handle, u32 fps )
{
    return SUCCESS;
}
#if 0
static int pCus_GetSensorCap_isp_calibration( ss_cus_sensor *handle, CUS_CAMSENSOR_CAP *cap )
{
    if( cap )
        memcpy( cap, &sensor_cap, sizeof(CUS_CAMSENSOR_CAP) );
    else
...
#endif
static int pCus_AEStatusNotify_isp_calibration( ss_cus_sensor *handle,  u32 idx, CUS_CAMSENSOR_AE_STATUS_NOTIFY status )
{
    return SUCCESS;
}

static int pCus_GetAEUSecs_isp_calibration( ss_cus_sensor *handle, u32 *us )
{
    *us = 0;
    return SUCCESS;
}

static int pCus_SetAEUSecs_isp_calibration( ss_cus_sensor *handle, u32 us )
{
    return SUCCESS;
}

// Gain: 1x = 1024
static int pCus_GetAEGain_isp_calibration( ss_cus_sensor *handle, u32* gain )
{
    *gain = 0;
    return SUCCESS;
}
...
static int pCus_SetAEGain_isp_calibration( ss_cus_sensor *handle, u32 gain )
{
    return SUCCESS;
}
...
#if 0
static int pCus_SetAEGain_cal_isp_calibration( ss_cus_sensor *handle, u32 gain )
{
    return SUCCESS;
}

static int pCus_setCaliData_gain_linearity_isp_calibration( ss_cus_sensor* handle, CUS_GAIN_GAP_ARRAY* pArray, u32 num )
{
    return SUCCESS;
}

static int pCus_GetAEMinMaxUSecs_isp_calibration( ss_cus_sensor *handle, u32 *min, u32 *max )
{
    *min = 1;
    *max = 1000000 / 30;
...
static int pCus_GetAEMinMaxGain_isp_calibration( ss_cus_sensor *handle, u32 *min, u32 *max )
{
    *min = 1024;            //1024*1.1*2
    *max = 1024*8;
...
static int IMX327_GetShutterInfo_isp_calibration( struct __ss_cus_sensor* handle, CUS_SHUTTER_INFO *info )
{
...
static int pCus_GetDGainRemainder_isp_calibration(ss_cus_sensor *handle, u32 *dgain_remainder)
{
   return SUCCESS;
}
#endif

int cus_camsensor_init_handle_linear_isp_calibration(ss_cus_sensor* drv_handle)
{
    ss_cus_sensor *handle = drv_handle;
    imx327_params *params;
    ...
    //private data allocation & init
#if defined(__RTK_OS__)
    sprintf((char*)str, "IMX327linear");
    eCamOsRet = CamOsDirectMemAlloc((char*)str, sizeof(imx327_params), &pvBufVirt, (void**)&u64BufMiu, (void**)&u64BufPhy);
    if (eCamOsRet != CAM_OS_OK) {
        //UartSendTrace("[%s:%d] fail!\n", __FUNCTION__, __LINE__);
        return FAIL;
    }
    handle->private_data = (imx327_params *)pvBufVirt;
    memset(handle->private_data, 0, sizeof(imx327_params));
#else
    if (handle->private_data == NULL) {
        SENSOR_EMSG("[%s] Private data is empty!\n", __FUNCTION__);
        return FAIL;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX327_MIPI");
    ...
    handle->sensor_ae_info_cfg.u32AEGain_max                      = 1024*8;
    handle->sensor_ae_info_cfg.u32AEShutter_min                   = 1;
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000 / 30;
    ...
    handle->pCus_sensor_SetPatternMode = imx327_SetPatternMode_isp_calibration;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    //handle->pCus_sensor_GetShutterInfo = IMX327_GetShutterInfo_isp_calibration;
#if 0//defined(__MV5_FPGA__)
...
#if 0
static int pCus_GetSensorCap_hdr_dol_lef(ss_cus_sensor *handle, CUS_CAMSENSOR_CAP *cap)
{
    if (cap)
        memcpy(cap, &sensor_cap, sizeof(CUS_CAMSENSOR_CAP));
    else     return FAIL;
...
#endif
static int imx327_SetPatternMode_hdr_dol_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}
...
static int pCus_SetAEGain_cal_hdr_dol_lef(ss_cus_sensor *handle, u32 gain)
{
    return SUCCESS;
}

static int pCus_setCaliData_gain_linearity_hdr_dol_lef(ss_cus_sensor* handle, CUS_GAIN_GAP_ARRAY* pArray, u32 num)
{
    return SUCCESS;
}

static int IMX327_GetShutterInfo_hdr_dol_lef(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    info->max = 1000000000/Preview_MIN_FPS;
...
static int pCus_I2CWrite_hdr_dol_lef(ss_cus_sensor *handle, unsigned short usreg, unsigned short usval)
{
    return SUCCESS;
}

static int pCus_I2CRead_hdr_dol_lef(ss_cus_sensor *handle, unsigned short usreg)
{
    return SUCCESS;
}
#endif
...
static int cus_camsensor_init_handle_hdr_dol_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"IMX327_MIPI_HDR_LEF");
