...
{
   //cleaned_0x01_FT_SC2336_MIPI_27Minput_2Lane_371.25Mbps_10bit_1920x1080_30fps-20230911
	{0x0103,0x01},
	{0x0100,0x00},
	{0x36e9,0x80},
	{0x37f9,0x80},
...
static int SC2336_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
  return SUCCESS;
}
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width        = SC2336_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height       = SC2336_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps      = SC2336_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps      = SC2336_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x = SC2336_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y = SC2336_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth  = SC2336_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight = SC2336_mipi_linear[res].senout.height;
        strcpy(handle->video_res_supported.res[res].strResDesc, SC2336_mipi_linear[res].senstr.strResDesc);
    }
