...
/* Sensor Porting on Master V4
   Porting owner :  yuantian.cai, yuantian.cai
   Date :           9/26/2023, 10/31/2023
   Build on :       Master_V4 I6C
   Verified on :    Master_V4 I6C, pass
   Remark :
*/
...
static int sc4238_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_init_4M30fps(ss_cus_sensor *handle)
{
    sc4238_params *params = (sc4238_params *)handle->private_data;
    int i,cnt=0;
    //ISensorIfAPI *sensor_if = handle->sensor_if_api;
...
static int pCus_init_4M30fps_2lane(ss_cus_sensor *handle)
{
    sc4238_params *params = (sc4238_params *)handle->private_data;
    int i,cnt=0;
    //ISensorIfAPI *sensor_if = handle->sensor_if_api;
...
static int pCus_init_HDR_DOL_4lane(ss_cus_sensor *handle)
{
    sc4238_params *params = (sc4238_params *)handle->private_data;
    //ISensorIfAPI *sensor_if = handle->sensor_if_api;
    int i,cnt=0;
...
static int pCus_GetAEUSecsHDR_DOL_SEF(ss_cus_sensor *handle, u32 *us) {
  int rc=0;
  u32 lines = 0;
  lines |= (u32)(expo_reg_HDR_DOL_SEF[0].data&0x0f)<<8;
  lines |= (u32)(expo_reg_HDR_DOL_SEF[1].data&0xff)<<0;
...
static int sc4238_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle_linear(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"sc4238_MIPI");
...
if (lane_num == 4)
{
    ...
    handle->video_res_supported.ulcur_res = 0;
    handle->video_res_supported.res[0].u16width = 2560;
    handle->video_res_supported.res[0].u16height = 1440;
    handle->video_res_supported.res[0].u16max_fps= Preview_MAX_FPS;
    handle->video_res_supported.res[0].u16min_fps= Preview_MIN_FPS;
    handle->video_res_supported.res[0].u16crop_start_x= 64;
    handle->video_res_supported.res[0].u16crop_start_y= 40;
    ...
    handle->video_res_supported.res[1].u16max_fps= Preview_MAX_FPS;
    ...
    handle->video_res_supported.res[1].u16OutputWidth= 2688;
    handle->video_res_supported.res[1].u16OutputHeight= 1520;
...
int cus_camsensor_init_handle_hdr_dol_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"sc4238_MIPI_HDR_SEF");
...
int cus_camsensor_init_handle_hdr_dol_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"SC4238_MIPI_HDR_LEF");
    ...
    // AE parameters
    ////////////////////////////////////////////////////
    handle->sensor_ae_info_cfg.u8AEGainDelay        = 2;//0;//1;
    handle->sensor_ae_info_cfg.u8AEShutterDelay    = 2;//1;//2;
    handle->sensor_ae_info_cfg.u8AEGainCtrlNum = 1;
    handle->sensor_ae_info_cfg.u8AEShutterCtrlNum = 2;
