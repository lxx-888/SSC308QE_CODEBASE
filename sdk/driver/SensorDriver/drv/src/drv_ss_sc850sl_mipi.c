...
/* Sensor Porting on Master V4
   Porting owner :  Lance.Lan, yuantian.cai
   Date :           9/26/2023, 10/31/2023
   Build on :       Master_V4 I6C
   Verified on :    Master_V4 I6C, pass
   Remark :         i6c not support 4k resolution, verified on crop 1080p.
*/
...
#define SENSOR_MAX_GAIN         50790                   // max sensor Again
...
static int pCus_sensor_GetAEInfo(ss_cus_sensor *handle, CUS_SENSOR_AE_INFO_t *info)
{
    ...
    if(handle->interface_attr.attr_mipi.mipi_hdr_mode == CUS_HDR_MODE_NONE)
    {
        ...
        info->u32AEShutter_max                   = 1000000000/Preview_MIN_FPS-4*Preview_line_period;
    }
    else
    {
...
static int pCus_poweron(ss_cus_sensor *handle, u32 idx)
{
    ...
    SENSOR_USLEEP(2000);
    sensor_if->Reset(idx, !SENSOR_PWDN_POL);
    SENSOR_USLEEP(1000);

    SENSOR_DMSG("[%s] pwd high\n", __FUNCTION__);
    sensor_if->PowerOff(idx, !SENSOR_RST_POL);
    SENSOR_USLEEP(2000);
...
static int sc850sl_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
  int i;
  SENSOR_DMSG("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
...
static int pCus_init_mipi4lane_HDR_8M15fps(ss_cus_sensor *handle)
{
    sc850sl_params *params = (sc850sl_params *)handle->private_data;
    int i,cnt=0;
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain) {
    ...
    {
        Coarse_gain = 1;        Dcg_gainx1024 = 1024;
        Coarse_gain_reg = 0x03;
    }
    else if (gain < 3200) // 3.125 * 1024
    {
        Coarse_gain = 2;        Dcg_gainx1024 = 1024;
        Coarse_gain_reg = 0x07;
    }
    else if (gain < 6400) // 6.25 * 1024
    {
        Coarse_gain = 1;        Dcg_gainx1024 = 3200;
        Coarse_gain_reg = 0x23;
    }
    else if (gain < 12800)// 12.5 * 1024
    {
        Coarse_gain = 2;        Dcg_gainx1024 = 3200;
        Coarse_gain_reg = 0x27;
    }
    else if (gain < 25600)// 25 * 1024
    {
        Coarse_gain = 4;        Dcg_gainx1024 = 3200;
        Coarse_gain_reg = 0x2f;
    }
    else if (gain <= SENSOR_MAX_GAIN)// 49.6 * 1024 // end again
    {
        Coarse_gain = 8;        Dcg_gainx1024 = 3200;
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"sc850sl_MIPI");
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000000/Preview_MIN_FPS-4*Preview_line_period;
...
int cus_camsensor_init_handle_HDR_SEF(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"sc850sl_MIPI_HDR_SEF");
...
int cus_camsensor_init_handle_HDR_LEF(ss_cus_sensor* drv_handle) {
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"sc850sl_MIPI_HDR_LEF");
