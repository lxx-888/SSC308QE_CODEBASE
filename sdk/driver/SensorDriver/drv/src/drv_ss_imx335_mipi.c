...
u32 Preview_line_period_HDR_DOL = 9411;
...
/*"IMX335LQN All-pixel scan CSI-2_4lane 1188Mbps AD:10bit Output:10bit 27MHz Master Mode DOL HDR 2frameLI 30fps"*/
    ...
    {0x300C,0x42},
    {0x300D,0x2E},
    {0x3030,0x9A},  //VMAX 4506 x119A //4096 1000h
    {0x3031,0x11},
    {0x3032,0x00},
    {0x3034,0x13},  //HMAX 275 x113 //300 : 12Ch
    {0x3035,0x01},
    {0x3048,0x01},
    {0x3049,0x01},
    {0x304A,0x04},
...
static int imx335_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
    int i;
    switch(mode)
    {
...
static int pCus_init_mipi2lane_5m25fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi2lane_5m30fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane_5m25fps_linear(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane5m25fps_HDR_DOL(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane5m20fps_HDR_DOL(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_init_mipi4lane5m30fps_HDR_DOL(ss_cus_sensor *handle)
{
    int i,cnt=0;
...
static int pCus_SetVideoRes_HDR_DOL(ss_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        ...
        case 2:
            ...
            vts_30fps_HDR_DOL = 4506; // R0x3030 ~ R0x3032
            params->expo.vts = vts_30fps_HDR_DOL;
            Preview_MAX_FPS = 30;
            params->expo.fps= Preview_MAX_FPS;
            Preview_line_period_HDR_DOL = 7398; // (1/30)/4506
            params->max_rhs1 = 290;
            break;
        default:
            break;
...
static int imx335_SetPatternMode_hdr_dol_lef(ss_cus_sensor *handle,u32 mode)
{
    return SUCCESS;
}
...
static int IMX335_GetShutterInfo_hdr_dol_lef(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle_linear(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"IMX335_MIPI");
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        ...
        handle->video_res_supported.res[res].u16width             = imx335_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height            = imx335_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps           = imx335_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps           = imx335_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x      = imx335_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y      = imx335_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth       = imx335_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight      = imx335_mipi_linear[res].senout.height;
    ...
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000000/imx335_mipi_linear[0].senout.min_fps;
...
int cus_camsensor_init_handle_hdr_dol_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"IMX335_MIPI_HDR_SEF");
    ...
    for (res = 0; res < HDR_RES_END; res++) {
        handle->video_res_supported.num_res = res+1;
        handle->video_res_supported.res[res].u16width             = imx335_mipi_hdr[res].senif.preview_w;
        handle->video_res_supported.res[res].u16height            = imx335_mipi_hdr[res].senif.preview_h;
        handle->video_res_supported.res[res].u16max_fps           = imx335_mipi_hdr[res].senout.max_fps;
        handle->video_res_supported.res[res].u16min_fps           = imx335_mipi_hdr[res].senout.min_fps;
        handle->video_res_supported.res[res].u16crop_start_x      = imx335_mipi_hdr[res].senif.crop_start_X;
        handle->video_res_supported.res[res].u16crop_start_y      = imx335_mipi_hdr[res].senif.crop_start_y;
        handle->video_res_supported.res[res].u16OutputWidth       = imx335_mipi_hdr[res].senout.width;
        handle->video_res_supported.res[res].u16OutputHeight      = imx335_mipi_hdr[res].senout.height;
...
static int cus_camsensor_init_handle_hdr_dol_lef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"IMX335_MIPI_HDR_LEF");
    ...
    handle->sensor_ae_info_cfg.u32AEGain_max                      = SENSOR_MAX_GAIN;
    handle->sensor_ae_info_cfg.u32AEShutter_min                   = Preview_line_period_HDR_DOL * 4;
    handle->sensor_ae_info_cfg.u32AEShutter_max                   = 1000000000/imx335_mipi_hdr[0].senout.min_fps;
    handle->sensor_ae_info_cfg.u32AEShutter_step                  = Preview_line_period_HDR_DOL;
