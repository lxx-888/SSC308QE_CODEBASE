...
static int tp2852b_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
#if 0
    int i;
        SENSOR_DMSG("[%s]\n", __FUNCTION__);
...
//std: STD_TVI/STD_HDA
////////////////////////////////
static void TP2852B_decoder_init(ss_cus_sensor *handle, unsigned char ch, unsigned char fmt, unsigned char std)
{
...
static void TP2852B_mipi_out(ss_cus_sensor *handle, unsigned char output)
{
    //mipi setting
...
static int pCus_BT656_GetSrcAttr(ss_cus_sensor *handle, u32 plane_id, CUS_SNR_Anadec_SrcAttr_t *psrcAttr)
{
...
int tp2852b_cus_camsensor_init_handle_ch0(ss_cus_sensor* handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"tp2852b_MIPI_CH0");
    ...
    handle->video_res_supported.res[0].u16width = Preview_WIDTH_CH0;
    handle->video_res_supported.res[0].u16height = Preview_HEIGHT_CH0; // Fixme : TOTAL_LINE_CNT not enough to 1080
    ...
    handle->video_res_supported.res[0].u16OutputWidth = Preview_WIDTH_CH0;
    handle->video_res_supported.res[0].u16OutputHeight = Preview_HEIGHT_CH0;
    ...
    //Src type
    handle->pCus_sensor_GetSrcAttr       = pCus_BT656_GetSrcAttr;
...
int tp2852b_cus_camsensor_init_handle_ch1(ss_cus_sensor* handle)
{
    tp2852b_params *params;
    if (!handle)
    {
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName, "tp2852b_MIPI_CH1");
    ...
    handle->video_res_supported.num_res = 1;
    handle->video_res_supported.ulcur_res = 0; //default resolution index is 0.
    handle->video_res_supported.res[0].u16width = Preview_WIDTH_CH1;
    handle->video_res_supported.res[0].u16height = Preview_HEIGHT_CH1; // Fixme : TOTAL_LINE_CNT not enough to 1080
    handle->video_res_supported.res[0].u16max_fps = Preview_MAX_FPS;
    handle->video_res_supported.res[0].u16min_fps = Preview_MIN_FPS;
    handle->video_res_supported.res[0].u16crop_start_x = Preview_CROP_START_X;
    handle->video_res_supported.res[0].u16crop_start_y = Preview_CROP_START_Y;
    handle->video_res_supported.res[0].u16OutputWidth = Preview_WIDTH_CH1;
    handle->video_res_supported.res[0].u16OutputHeight = Preview_HEIGHT_CH1;
    sprintf(handle->video_res_supported.res[0].strResDesc, "1920x1080");
...
SENSOR_DRV_ENTRY_3CHAHD_IMPL_END_EX(TP2852B_MIPI,
