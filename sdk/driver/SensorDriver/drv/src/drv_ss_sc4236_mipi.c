...
static int pCus_poweron(ss_cus_sensor *handle, u32 idx)
{
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    SENSOR_DMSG("[%s] ", __FUNCTION__);
    sensor_if->PowerOff(idx, CUS_CLK_POL_POS);////pwd always high
...
static int sc4236_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_init_linear_3p6m_30fps(ss_cus_sensor *handle)
{
    sc4236_params *params = (sc4236_params *)handle->private_data;
    //SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    int i,cnt;
...
static int pCus_init_linear_1920x1536m_25fps(ss_cus_sensor *handle)
{
    sc4236_params *params = (sc4236_params *)handle->private_data;
    //SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    int i,cnt;
...
static int pCus_init_linear_1344x1344m_25fps(ss_cus_sensor *handle)
{
    sc4236_params *params = (sc4236_params *)handle->private_data;
    //SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    int i,cnt;
...
static int sc4236_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    ////////////////////////////////////////////////////
    sprintf(handle->strSensorStreamName,"sc4236_MIPI");
    ...
    handle->video_res_supported.res[0].u16OutputWidth= 0;
    handle->video_res_supported.res[0].u16OutputHeight= 0;
    ...
    handle->video_res_supported.res[1].u16height = 1536;
    handle->video_res_supported.res[1].u16max_fps= 25;
    ...
    handle->video_res_supported.res[1].u16OutputWidth= 0;
    handle->video_res_supported.res[1].u16OutputHeight= 0;
    sprintf(handle->video_res_supported.res[1].strResDesc, "1920x1536@25fps");

    handle->video_res_supported.num_res = 3;
    handle->video_res_supported.ulcur_res = 0; //default resolution index is 0.
    handle->video_res_supported.res[2].u16width = 1344;
    handle->video_res_supported.res[2].u16height = 1344;
    handle->video_res_supported.res[2].u16max_fps= 25;
    ...
    handle->video_res_supported.res[2].u16OutputWidth= 0;
    handle->video_res_supported.res[2].u16OutputHeight= 0;
