...
#define Preview_MCLK_SPEED                    CUS_CMU_CLK_36MHZ
...
#define Preview_line_period                   29625
...
#endif
#if SENSOR_MIPI_HDR_DOL_12bit
#define Preview_line_period_HDR_DOL_4LANE     (29550 / 2)
#define Preview_line_period_HDR_DOL_2LANE     (29550 / 2)

#define vts_30fps_HDR_DOL_4lane               1128
#define vts_30fps_HDR_DOL_2lane               1128
#endif
#define MAX_DOL_RHS1                          ((1128 * 2) - (1097 * 2) - 21)
...
const static I2C_ARRAY Sensor_init_table_2lane[] =
{
    ...
    {0x3018, 0x65},   //VMAX
    {0x3019, 0x04},
    {0x301c, 0xAA},   //0x1167 HMAX,for 25fps
    {0x301d, 0x10},
    {0x305C, 0x18},   //INCK
    {0x305D, 0x03},
    {0x305E, 0x20},
...
static int pCus_init_mipi4lane_linear(ss_cus_sensor *handle)
{
    ...
    pCus_SetOrien(handle, params->res.orit);
    return SUCCESS;
}
...
static int pCus_SetAEUSecsHDR_DOL_SEF1(ss_cus_sensor *handle, u32 us)
{
    ...
    if(params->expo.fps > 3000){ //3fps*1000
        if(params->expo.fps > 25000)
            ...
            params->max_rhs1 = (MAX_DOL_RHS1+110);
    }
    else {
...
static int pCus_GetAEGainHDR_DOL_LEF(ss_cus_sensor *handle, u32* gain)
{
    ...
    *gain = params->expo.expo_lef_gain;
...
static int pCus_SetAEGainHDR_DOL_LEF(ss_cus_sensor *handle, u32 gain)
{
    ...
    params->expo.expo_lef_gain = gain;
    pCus_SetAEGainHDR_DOL_Calculate(gain, &gain_reg);
