...
static int F23_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int pCus_SetOrien(ss_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit) {
    ...
    switch(orit) {
        ...
        case CUS_ORIT_M0F1:
            ...
            params->tMirror_reg[1].data = 0x28;
            params->tMirror_reg[2].data = 0x00;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M1F1:
...
static int pCus_SetAEGain(ss_cus_sensor *handle, u32 gain) {
  ...
  //gain_double=(double)gain;
    ...
    if(params->tGain_reg[0].data < 0x30)
    {
        SensorReg_Write(0x66,0x04);
        SensorReg_Write(0x0c,0x00);
    }else{
        SensorReg_Write(0x0c,0x40);
        SensorReg_Write(0x66,0x44);
    }
...
#if 0
static int F23_GetShutterInfo(struct __ss_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    // Sensor stream name
    ///////////////////////////////////////////////////////
    sprintf(handle->strSensorStreamName,"F23_MIPI_LINEAR");
