...
//    sensor_if->PowerOff(idx, handle->pwdn_POLARITY);
    ...
    if (handle->interface_attr.attr_mipi.mipi_hdr_mode == SENSOR_HDR_MODE) {
                sensor_if->SetCSI_hdr_mode(idx, handle->interface_attr.attr_mipi.mipi_hdr_mode, SENSOR_HDR_MODE);
    }
...
static int jxf37_SetPatternMode(ss_cus_sensor *handle,u32 mode)
{
...
static int cus_camsensor_init_handle(ss_cus_sensor* drv_handle) {
   ss_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"JXF37_MIPI_Linear");
...
static int cus_camsensor_init_handle_hdr_sef(ss_cus_sensor* drv_handle)
{
    ...
    sprintf(handle->strSensorStreamName,"JXF37_MIPI_HDR_SEF");
    ...
    handle->video_res_supported.res[0].u16OutputWidth = 1920;//0x79C;
    handle->video_res_supported.res[0].u16OutputHeight = 1080;//0x449;
...
static int cus_camsensor_init_handle_hdr_lef(ss_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->strSensorStreamName,"JXF37_MIPI_HDR_LEF");
