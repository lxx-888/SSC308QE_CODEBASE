...
inline std::string md5_hash_buffer(unsigned char const * data, size_t nbytes) {
    char digest[16];
    std::string hex;

    md5_state_t state;
    md5_init(&state);
    md5_append(&state, (md5_byte_t const *)data, nbytes);
    md5_finish(&state, (md5_byte_t*)digest);
...
inline std::string md5_hash_buffer_blocks(struct md5_blocks_s *blocks, size_t block_count) {
    char digest[16];
    std::string hex;
    ...
    for (size_t i = 0; i < block_count; i++) {
        md5_append(&state, (md5_byte_t const *)blocks[i].data, blocks[i].nbytes);
    }
    md5_finish(&state, (md5_byte_t*)digest);
    for (size_t i = 0; i < 16; i++) {
        hex.push_back(hexval[((digest[i] >> 4) & 0xF)]);
