...
{
    struct MST_ONVIF_Descs *pDescs = (struct MST_ONVIF_Descs *)soap->user;
    ...
    if (!pDescs)
    {
        printf("[%d] %s ERR desc null!\n", __LINE__,__func__);
        return SOAP_ERR;
    }

    sscanf(trt__GetProfile->ProfileToken, "Profile_%d", &idx);

    // profile_ idx
    if (idx >= pDescs->count)
    {
        printf("[%d] %s ERR desc count %d/%d!\n", __LINE__,__func__, idx, pDescs->count);
        return SOAP_ERR;
    }
    ...
    strncpy(tProfile->Name, pDescs->descs[idx].name, SMALL_INFO_LEN);

    tProfile->token= (char *)soap_malloc(soap,sizeof(char)*SMALL_INFO_LEN);
    strcpy(tProfile->token, trt__GetProfile->ProfileToken);
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetProfiles(struct soap* soap, struct _trt__GetProfiles *trt__GetProfiles, struct _trt__GetProfilesResponse *trt__GetProfilesResponse)
{
    ...
    {
        // main stream profiles
        soap_default_tt__Profile(soap, &tProfile[i]);

        tProfile[i].Name = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strncpy(tProfile[i].Name, pDescs->descs[i].name, SMALL_INFO_LEN);

        tProfile[i].token = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        sprintf(tProfile[i].token, "Profile_%d", i);

        tProfile[i].fixed  = (enum xsd__boolean *)soap_malloc(soap, sizeof(enum xsd__boolean));
        *tProfile[i].fixed = xsd__boolean__true_;

        // video source configuration
        tProfile[i].VideoSourceConfiguration =
            (struct tt__VideoSourceConfiguration *)soap_malloc(soap, sizeof(struct tt__VideoSourceConfiguration));
        soap_default_tt__VideoSourceConfiguration(soap, tProfile[i].VideoSourceConfiguration);

        tProfile[i].VideoSourceConfiguration->Name = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].VideoSourceConfiguration->Name, "VideoSourceConfig");

        tProfile[i].VideoSourceConfiguration->token = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].VideoSourceConfiguration->token, "VideoSourceToken");

        tProfile[i].VideoSourceConfiguration->SourceToken = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].VideoSourceConfiguration->SourceToken,
               "VideoSource_1"); // must same to __tmd__GetVideoSources's token

        tProfile[i].VideoSourceConfiguration->UseCount = 1;

        tProfile[i].VideoSourceConfiguration->Bounds =
            (struct tt__IntRectangle *)soap_malloc(soap, sizeof(struct tt__IntRectangle));
        tProfile[i].VideoSourceConfiguration->Bounds->x      = 0;
        tProfile[i].VideoSourceConfiguration->Bounds->y      = 0;
        tProfile[i].VideoSourceConfiguration->Bounds->width  = 1920;
        tProfile[i].VideoSourceConfiguration->Bounds->height = 1080;

        // audio source configuration

        tProfile[i].AudioSourceConfiguration =
            (struct tt__AudioSourceConfiguration *)soap_malloc(soap, sizeof(struct tt__AudioSourceConfiguration));
        soap_default_tt__AudioSourceConfiguration(soap, tProfile[i].AudioSourceConfiguration);

        tProfile[i].AudioSourceConfiguration->Name = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].AudioSourceConfiguration->Name, "AudioSourceConfig");

        tProfile[i].AudioSourceConfiguration->token = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].AudioSourceConfiguration->token, "AudioSourceToken");

        tProfile[i].AudioSourceConfiguration->SourceToken = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].AudioSourceConfiguration->SourceToken, "AudioSourceChannel1");

        tProfile[i].AudioSourceConfiguration->UseCount = 1;

        // video encoder configuration
        tProfile[i].VideoEncoderConfiguration =
            (struct tt__VideoEncoderConfiguration *)soap_malloc(soap, sizeof(struct tt__VideoEncoderConfiguration));
        soap_default_tt__VideoEncoderConfiguration(soap, tProfile[i].VideoEncoderConfiguration);

        tProfile[i].VideoEncoderConfiguration->Name = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].VideoEncoderConfiguration->Name, "VideoEncoder_1");

        tProfile[i].VideoEncoderConfiguration->token = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].VideoEncoderConfiguration->token, "VideoEncoder_Token");

        tProfile[i].VideoEncoderConfiguration->UseCount = 1;
        tProfile[i].VideoEncoderConfiguration->Quality  = 3.0;
        tProfile[i].VideoEncoderConfiguration->Encoding = tt__VideoEncoding__H264; // JPEG = 0, MPEG4 = 1, H264 = 2;

        tProfile[i].VideoEncoderConfiguration->Resolution =
            (struct tt__VideoResolution *)soap_malloc(soap, sizeof(struct tt__VideoResolution));
        tProfile[i].VideoEncoderConfiguration->Resolution->Width  = 1920;
        tProfile[i].VideoEncoderConfiguration->Resolution->Height = 1080;

        tProfile[i].VideoEncoderConfiguration->RateControl =
            (struct tt__VideoRateControl *)soap_malloc(soap, sizeof(struct tt__VideoRateControl));
        tProfile[i].VideoEncoderConfiguration->RateControl->FrameRateLimit   = 30;
        tProfile[i].VideoEncoderConfiguration->RateControl->EncodingInterval = 1;
        tProfile[i].VideoEncoderConfiguration->RateControl->BitrateLimit     = 4096;

        // audio encoder configuration
        tProfile[i].AudioEncoderConfiguration =
            (struct tt__AudioEncoderConfiguration *)soap_malloc(soap, sizeof(struct tt__AudioEncoderConfiguration));
        soap_default_tt__AudioEncoderConfiguration(soap, tProfile[i].AudioEncoderConfiguration);

        tProfile[i].AudioEncoderConfiguration->Name = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].AudioEncoderConfiguration->Name, "AudioEncoder_1");

        tProfile[i].AudioEncoderConfiguration->token = (char *)soap_malloc(soap, sizeof(char) * SMALL_INFO_LEN);
        strcpy(tProfile[i].AudioEncoderConfiguration->token, "AudioEncoder_Token");

        tProfile[i].AudioEncoderConfiguration->UseCount = 1;
        tProfile[i].AudioEncoderConfiguration->Encoding = tt__AudioEncoding__G711;

        tProfile[i].AudioEncoderConfiguration->Bitrate    = 64;
        tProfile[i].AudioEncoderConfiguration->SampleRate = 8;

        // tProfile[i].AudioEncoderConfiguration->SessionTimeout = 3000;
    }
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetStreamUri(struct soap* soap, struct _trt__GetStreamUri *trt__GetStreamUri, struct _trt__GetStreamUriResponse *trt__GetStreamUriResponse)
{
    ...
    sscanf(trt__GetStreamUri->ProfileToken, "Profile_%d", &idx);
    ...
    mst_GetV4Ip(ETH_NAME, &V4Addr);
    sprintf(IPAddr, "rtsp://%s/%s", inet_ntoa(*((struct in_addr *)&V4Addr)), pDescs->descs[idx].name);
