...
struct MST_ONVIF_Descs g_descs;
...
void MST_ONVIF_Init()
{
    ...
    g_descs.count = 1;
    strcpy(g_descs.descs[0].name, "main_stream");
...
void MST_ONVIF_SetDescs(const struct MST_ONVIF_Descs *pDescs)
{
    ...
    if (pDescs->count >= MST_ONVIF_DESC_MAX_NUM)
    {
        return;
    }
    memcpy(&g_descs, pDescs, sizeof(struct MST_ONVIF_Descs));
}
