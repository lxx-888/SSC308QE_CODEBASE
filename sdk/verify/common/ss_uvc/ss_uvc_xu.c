...
extern int32_t UVC_DFU_Init(SS_UVC_Device_t *pdev, size_t fw_size);
extern int32_t UVC_DFU_Update(SS_UVC_Device_t *pdev, u_int8_t *data, size_t size);
extern int32_t UVC_DFU_DnlEnd(SS_UVC_Device_t *pdev);

#if AIT_XU_DEF
FW_RELEASE_VERSION    gbFwVersion = { 0x01, 0x00, FW_VER_BCD }; // Add FW version.

uint8_t pu_brightness_data[2];
uint8_t ct_roi_data[10];
...
static int usb_vc_eu1_mmp_cmd(SS_UVC_Device_t *pdev, unsigned char* cmd)//cmd -> 8byte
{
    int ret = 0;
#if (ISP_IQ_SERVER)
...
//MICROSOFT EU
unsigned int face_authentication_length = 9;
unsigned int metadata_control_length = 4;

MSX_face_authentication fa_dwValue_res = {0,3,2,0,0,0,0,0,0};
MSX_face_authentication fa_dwValue_def = {1,3,2,0,0,0,0,0,0};
MSX_face_authentication fa_dwValue_min = {0,3,2,0,0,0,0,0,0};
MSX_face_authentication fa_dwValue_max = {1,3,2,0,0,0,0,0,0};
MSX_face_authentication fa_dwValue_cur = {1,3,2,0,0,0,0,0,0};

MSX_metadata_control meta_dwValue_res = {0};
MSX_metadata_control meta_dwValue_def = {16};
MSX_metadata_control meta_dwValue_min = {16};
MSX_metadata_control meta_dwValue_max = {16};
MSX_metadata_control meta_dwValue_cur = {0};

int8_t UVC_GetRoiData(SS_UVC_Device_t *pdev, SS_UVC_XuRoiNode_t *pRoiReadListNode)
{
    int Ret =-1;
    SS_UVC_XuRoiNode_t *tmpListNode;
    pthread_mutex_lock(&pdev->ListMutexRoi);
    if(!(list_empty(&pdev->ListHeadRoi)))
    {
        tmpListNode = list_first_entry(&pdev->ListHeadRoi, SS_UVC_XuRoiNode_t, list);
        pRoiReadListNode->u16Roix       = tmpListNode->u16Roix;
        pRoiReadListNode->u16Roiy       = tmpListNode->u16Roiy;
        pRoiReadListNode->u16RoiWidth   = tmpListNode->u16RoiWidth;
        pRoiReadListNode->u16RoiHeight  = tmpListNode->u16RoiHeight;

        list_del(&(tmpListNode->list));
        free(tmpListNode);
        Ret = SS_UVC_SUCCESS;
    }
    pthread_mutex_unlock(&pdev->ListMutexRoi);
...
int8_t usb_vc_eu3_cs(uint8_t cs, uint8_t req, struct uvc_request_data *resp)
{
    switch (cs)
    {
        case MSXU_CONTROL_FACE_AUTHENTICATION:
        {
            XU_Print("MSXU_CONTROL_FACE_AUTHENTICATION\r\n");
            switch (req)
            {
            case UVC_SET_CUR:
                resp->length = face_authentication_length;
                break;
            case UVC_GET_CUR:
                resp->length = face_authentication_length;
                memcpy(resp->data, &fa_dwValue_cur, face_authentication_length);
                break;
            case UVC_GET_MIN:
                resp->length = face_authentication_length;
                memcpy(resp->data, &fa_dwValue_min, face_authentication_length);
                break;
            case UVC_GET_MAX:
                resp->length = face_authentication_length;
                memcpy(resp->data, &fa_dwValue_max, face_authentication_length);
                break;
            case UVC_GET_RES:
                resp->length = face_authentication_length;
                memcpy(resp->data, &fa_dwValue_res, face_authentication_length);
                break;
            case UVC_GET_INFO:
                resp->length = 1;
                resp->data[0] = 0x03;
            ...
            case UVC_GET_DEF:
                resp->length = face_authentication_length;
                memcpy(resp->data, &fa_dwValue_def, face_authentication_length);
                break;
            case UVC_GET_LEN:
                resp->length = sizeof(unsigned short);
                memcpy(resp->data, &face_authentication_length, sizeof(unsigned short));
                break;
            }
            break;
        }

        case MSXU_CONTROL_METADATA:
        {
            XU_Print("MSXU_CONTROL_METADATA\r\n");
            switch (req)
            {
            case UVC_GET_CUR:
                resp->length = metadata_control_length;
                memcpy(resp->data, &meta_dwValue_cur, metadata_control_length);
                break;
            case UVC_GET_MIN:
                resp->length = metadata_control_length;
                memcpy(resp->data, &meta_dwValue_min, metadata_control_length);
                break;
            case UVC_GET_MAX:
                resp->length = metadata_control_length;
                memcpy(resp->data, &meta_dwValue_max, metadata_control_length);
                break;
            case UVC_GET_RES:
                resp->length = metadata_control_length;
                memcpy(resp->data, &meta_dwValue_res, metadata_control_length);
                break;
            case UVC_GET_INFO:
                resp->length = 1;
                resp->data[0] = 0x01;
            ...
            case UVC_GET_DEF:
                resp->length = metadata_control_length;
                memcpy(resp->data, &meta_dwValue_def, metadata_control_length);
                break;
            case UVC_GET_LEN:
                resp->length = sizeof(unsigned short);
                memcpy(resp->data, &metadata_control_length, sizeof(unsigned short));
                break;
            }
            break;
        }
...
int8_t usb_vc_eu3_cs_out(uint8_t entity_id, uint8_t cs, uint32_t len, struct uvc_request_data *data)
{
    switch (cs)
    {
        ...
        {
            XU_Print("MSXU_CONTROL_FACE_AUTHENTICATION\r\n");
            fa_dwValue_cur.bNumEntries = data->data[0];
            fa_dwValue_cur.bInterfaceNumber = data->data[1];
            fa_dwValue_cur.bmControlFlags0 = data->data[2];
            printf("set data to %d, %d, %d\n", fa_dwValue_cur.bNumEntries
                , fa_dwValue_cur.bInterfaceNumber, fa_dwValue_cur.bmControlFlags0);
        }
        break;
...
int8_t usb_vc_ct_cs_out(SS_UVC_Device_t *pdev, uint8_t entity_id, uint8_t cs, uint32_t len, struct uvc_request_data *data)
{
    ...
    case UVC_CT_REGION_OF_INTEREST_CONTROL:
        memcpy(ct_roi_data, data->data, 10);
        // printf("ROI DATA (%d, %d, %d, %d)\n", ct_roi_data[0] | ct_roi_data[1] << 8,
        //         ct_roi_data[2] | ct_roi_data[3] << 8,
        //         ct_roi_data[4] | ct_roi_data[5] << 8,
        //         ct_roi_data[6] | ct_roi_data[7] << 8);

        tmpRoix             = ct_roi_data[2] | ct_roi_data[3] << 8;
        tmpRoiy             = ct_roi_data[0] | ct_roi_data[1] << 8;
        tmpRoix1            = ct_roi_data[6] | ct_roi_data[7] << 8;
        tmpRoiy1            = ct_roi_data[4] | ct_roi_data[5] << 8;

        if((tmpRoix1 - tmpRoix) && (tmpRoiy1 - tmpRoiy))
        {
            SS_UVC_XuRoiNode_t *ListRoiToIve;
            ListRoiToIve = (SS_UVC_XuRoiNode_t*)malloc(sizeof(SS_UVC_XuRoiNode_t));

            ListRoiToIve->u16Roix        = tmpRoix;
            ListRoiToIve->u16Roiy        = tmpRoiy;
            ListRoiToIve->u16RoiWidth    = tmpRoix1 - tmpRoix;
            ListRoiToIve->u16RoiHeight   = tmpRoiy1 - tmpRoiy;

            pthread_mutex_lock(&pdev->ListMutexRoi);
            list_add_tail(&ListRoiToIve->list, &pdev->ListHeadRoi);
            pthread_mutex_unlock(&pdev->ListMutexRoi);
        }
        break;
