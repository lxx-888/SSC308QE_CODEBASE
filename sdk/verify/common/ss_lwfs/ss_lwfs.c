...
int SS_LWFS_List(const s8 *s8MtdPath)
{
    s32 s32Fd = 0;
    s32 s32Ret = 0;
    ...
    for (i = 0; i < stBlockHeader.u32FileCount; i++)
    {
        ...
        printf("%10d    %8d    %7d    %10s\n", uFileHeadeData.stData.u32FileLen, uFileHeadeData.stData.u32FilePaddingLen,
               uFileHeadeData.stData.u32FilePosition, uFileHeadeData.stData.s8FileName);
    }
ERR:
    _FileClose(s32Fd);
...
SS_LWFS_FD SS_LWFS_Open(const s8 *s8MtdPath, const s8 *s8FileName, SS_LWFS_FileOpertion_e enFileOpt)
{
    ...
    for (i = 0; i < stBlockHeader.u32FileCount; i++)
    {
        ...
        PRINT("File name %s, pos 0x%x file size 0x%x padding 0x%x crc 0x%x\n", uFileHeadeData.stData.s8FileName,
              uFileHeadeData.stData.u32FilePosition, uFileHeadeData.stData.u32FileLen, uFileHeadeData.stData.u32FilePaddingLen,
              uFileHeadeData.stData.u32CrcCheck);
        if (pstFileDescN)
        {
...
int SS_LWFS_Read(SS_LWFS_FD lwfsFd, void *buf, int size)
{
    ...
    pstFileDesc = (SS_LWFS_FileDesc_t *)lwfsFd;
    if (!pstFileDesc)
        return -1;
    if (!size)
        return 0;
...
int SS_LWFS_Sync(SS_LWFS_FD lwfsFd)
{
    ...
    for (i = 0; i < stBlockHeader.u32FileCount; i++)
    {
        ...
        PRINT("File name %s, file size 0x%x padding 0x%x crc 0x%x\n", uFileHeadeData.stData.s8FileName,
              uFileHeadeData.stData.u32FileLen, uFileHeadeData.stData.u32FilePaddingLen, uFileHeadeData.stData.u32CrcCheck);
    }
    if (u32CurPos)
    {
        u32TempPos = 0;
        ...
        PRINT("File name %s, pos 0x%x, file size 0x%x padding 0x%x crc 0x%x\n", uFileHeadeData.stData.s8FileName, uFileHeadeData.stData.u32FilePosition,
              uFileHeadeData.stData.u32FileLen, uFileHeadeData.stData.u32FilePaddingLen, uFileHeadeData.stData.u32CrcCheck);
