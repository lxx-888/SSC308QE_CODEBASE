...
static Boolean sendToAgain(int socket, unsigned char *buffer, unsigned bufferSize,
                           struct sockaddr_in *dest, int dest_len,
                           int retVal)
{
  while (retVal == -1 && (errno == EAGAIN || errno == EINTR || errno == EWOULDBLOCK))
  {
    usleep(1000);
    retVal = sendto(socket, (char*)buffer, bufferSize, 0, (struct sockaddr*)dest, dest_len);
  }
  return (retVal == (int)bufferSize) ? True : False;
}
...
Boolean writeSocket(UsageEnvironment& env,
  ...
  do {
    ...
    if (bytesSent != (int)bufferSize && !sendToAgain(socket, buffer, bufferSize, &dest, sizeof dest, bytesSent)) {
      char tmpBuf[100];
...
unsigned increaseSendBufferTo(UsageEnvironment& env,
			      int socket, unsigned requestedSize) {
  unsigned size = 0x0;
  size = increaseBufferTo(env, SO_SNDBUF, socket, requestedSize);
    ...
    if (badAddressForUs(from)) {
	  ...
	  if (-1 != sock)
	  {
		  ...
		  if (-1 == ret)
		  {
			...
			// try all possible methods to retrieve the default network interface IP, but to no avail.
			// In the end, the only option left was to iterate through all network interfaces and obtain
			// the corresponding valid IPv4 addresses as the final addresses.
			if (badAddressForUs(from))
			{
				...
				if (getifaddrs(&ifaddr) != -1)
				{
					for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
					{
						...
						addr = *(uint32_t*)&(((struct sockaddr_in*)ifa->ifa_addr)->sin_addr);

						// ipv4 and valid addr
						if (ifa->ifa_addr->sa_family == AF_INET && !badAddressForUs(addr))
						{
							from = addr;
							break;
						}
				...
				{
					env.setResultErrMsg("getifaddrs");
				}
			}
	  }
	  else
