...
#include "AV1VideoRTPSource.hh"

////////// AV1BufferedPacket and AV1BufferedPacketFactory //////////

class AV1BufferedPacket: public BufferedPacket {
public:
...
class AV1BufferedPacketFactory: public BufferedPacketFactory {
private: // redefined virtual functions
  virtual BufferedPacket* createNewPacket(MultiFramedRTPSource* ourSource);
};


////////// AV1VideoRTPSource implementation //////////
...
AV1VideoRTPSource*
AV1VideoRTPSource::createNew(UsageEnvironment& env, Groupsock* RTPgs,
			      unsigned char rtpPayloadFormat,
			      unsigned rtpTimestampFrequency,
			      unsigned defaultWidth, unsigned defaultHeight) {
  return new AV1VideoRTPSource(env, RTPgs, rtpPayloadFormat,
				rtpTimestampFrequency, defaultWidth, defaultHeight);
}

AV1VideoRTPSource::AV1VideoRTPSource(UsageEnvironment& env,
				       Groupsock* RTPgs,
				       unsigned char rtpPayloadFormat,
				       unsigned rtpTimestampFrequency,
				       unsigned defaultWidth, unsigned defaultHeight)
  : MultiFramedRTPSource(env, RTPgs,
			 ...
			 new AV1BufferedPacketFactory),
    fDefaultWidth(defaultWidth), fDefaultHeight(defaultHeight) {
}

AV1VideoRTPSource::~AV1VideoRTPSource() {
}
...
static unsigned computeAV1HeaderSize(unsigned qtlen, unsigned dri) {
...
static void createAV1Header(unsigned char* buf, unsigned type,
...
*/
  ...
  // If this is the first (or only) fragment of a AV1 frame, then we need
  // to synthesize a AV1 header, and prepend it to the incoming data.
  // Hack: We can do this because we allowed space for it in
  // our special "AV1BufferedPacket" subclass.  We also adjust
  ...
  if (Offset == 0) {
    ...
    if (qtlen == 0) {
      // A quantization table was not present in the RTP AV1 header,
    ...
    unsigned hdrlen = computeAV1HeaderSize(qtlen, dri);
    resultSpecialHeaderSize -= hdrlen; // goes negative
    headerStart += (int)resultSpecialHeaderSize; // goes backward
    createAV1Header(headerStart, type, width, height, qtables, qtlen, dri);
  }

  fCurrentPacketBeginsFrame = (Offset == 0);

  // The RTP "M" (marker) bit indicates the last fragment of a frame:
  ((AV1BufferedPacket*)packet)->completesFrame
   = fCurrentPacketCompletesFrame = packet->rtpMarkerBit();

  return True;
}

char const* AV1VideoRTPSource::MIMEtype() const {
  return "video/AV1";
}

////////// AV1BufferedPacket and AV1BufferedPacketFactory implementation

void AV1BufferedPacket::reset() {
  BufferedPacket::reset();

  // Move our "fHead" and "fTail" forward, to allow space for a synthesized
  // AV1 header to precede the RTP data that comes in over the network.
  unsigned offset = MAX_AV1_HEADER_SIZE;
  if (offset > fPacketSize) offset = fPacketSize; // shouldn't happen
  fHead = fTail = offset;
}

unsigned AV1BufferedPacket
...
BufferedPacket* AV1BufferedPacketFactory
::createNewPacket(MultiFramedRTPSource* /*ourSource*/) {
  return new AV1BufferedPacket;
}
