...
int sdp_av1(uint8_t *data, int bytes, const char* proto, unsigned short port, int payload, int frequence, const void* extra, int extra_size)
{
    ...
    // TODO: should br 98, Canlet
    if (payload == -1)
        payload = 35; // https://bugs.chromium.org/p/webrtc/issues/detail?id=11042
    n = snprintf((char*)data, bytes, pattern,
                port, proto && *proto ? proto : "RTP/AVPF", payload, payload, payload,
                (unsigned int)av1.seq_profile,
                (unsigned int)av1.seq_level_idx_0,
                (unsigned int)av1.seq_tier_0);
