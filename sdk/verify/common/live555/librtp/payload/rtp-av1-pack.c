...
static int rtp_av1_pack_obu(struct rtp_encode_av1_t *packer, const uint8_t* obu, int64_t bytes)
{
    ...
    fprintf(stderr, "%s write %d data %02x %02x %02x %02x\n", __func__, (int)bytes,
                        *obu, *(obu+1), *(obu+2), *(obu+3));
	while (bytes > 0)
	{
		if (NULL == packer->ptr)
		{
        ...
        fprintf(stderr, "-----> extra hader size %d\n", (int)(ptr - tmp));
		...
		{
            verbose(stderr, "%s %d - flush %d\n", __func__, __LINE__, packer->size);
...
static int rtp_av1_pack_input_obu(void* pack, const void* data, int bytes, uint32_t timestamp)
{
    ...
    verbose(stderr, "%s %d timestamp %u size %d: %02x %02x %02x %02x\n", __func__, __LINE__,
            timestamp, bytes, *raw, *(raw+1), *(raw+2), *(raw+3));
