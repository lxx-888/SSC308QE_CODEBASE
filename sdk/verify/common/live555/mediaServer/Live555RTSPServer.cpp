...
int Live555RTSPServer::Start(int priority)
{
	m_iPauseFlag = 0;
	if ( m_iStartFlag ) {
		return -2;
	...
	m_iStartFlag = 1;
    if ( pthread_create(&m_Thread_handle, &m_Thread_attr, SingleStepThread, this) < 0) {
...
int Live555RTSPServer::Join()
{
	...
	m_iPauseFlag = 1;
	m_iStartFlag = 0;
	int iRet = pthread_join(m_Thread_handle, NULL);
    pthread_attr_destroy(&m_Thread_attr);
	m_Thread_handle = -1;
	return iRet;
}
