...
static int _PTREE_RTOS_ParseCmd(void *data);
...
static int _PTREE_RTOS_Init(int argc, char **argv)
{
    int s32Ret          = 0;
    ...
    PTREE_DBG("-------------->ready to _PTREE_RTOS_Init");
    BootTimestampRecord(__LINE__, "_PTREE_RTOS_Init");
    PTREE_PRELOAD_Setup();
    ...
    MI_DEVICE_RegMiSysExitCall(_PTREE_RTOS_Deint);
    MI_DEVICE_RegMiCliGetDataL2R(MI_CLICMD_PTREE_CMD, _PTREE_RTOS_ParseCmd);
    ...
    application_selector_retreat();
    PTREE_DBG("-------------->end to _PTREE_RTOS_Init");
    return 0;
}
...
static int _PTREE_RTOS_DeinitCmd(CLI_t *pCli, char *p)
{
    int cnt = 0;
...
SS_RTOS_CLI_CMD(ptree_deinit, "get mixer4.0 To ptree deinit pipeline cmd",
                "Usage: ptree_deinit_cmd [will deinit all]\n", _PTREE_RTOS_DeinitCmd);

rtos_application_selector_initcall(ptree, _PTREE_RTOS_Init);
