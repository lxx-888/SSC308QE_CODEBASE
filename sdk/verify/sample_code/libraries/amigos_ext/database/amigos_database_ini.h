...
*/
#ifndef __AMIGOS_DATABASE_INI_H__
#define __AMIGOS_DATABASE_INI_H__

#include <map>
#include "amigos_database.h"
#include "iniparser.h"

class AmigosDatabaseIni : public AmigosDatabase
{
    public:
        explicit AmigosDatabaseIni(const std::string &section);
        virtual ~AmigosDatabaseIni();
    ...
    private:
        std::map<unsigned int, std::string> mapStrRootBlock;
        std::map<unsigned int, unsigned int> mapStrRootBlockChipId;
        std::map<unsigned int, bool> mapStrRootBlockDelayPass;
        std::map<unsigned int, std::string> mapStrInSection;
        std::map<unsigned int, std::string> mapStrOutSection;
        std::string strTraverse;
};
#endif //__AMIGOS_DATABASE_INI_H__
