...
/*****************************************************************************
*   Prototype    : MI_IVE_Create_handle
*   Description  : Create IVE handle index
*   Parameters   : Null
...
*                  MI_IVE_DstImage_t       *pstDst          Output result, of same format with the input.
...
*   Parameters   : MI_IVE_HANDLE           hHandle        Handle ID of a task
*                  MI_IVE_SrcImage_t      *pstSrc         Input source data. Only the U8C1 input format is supported.
*                  MI_IVE_DstImage_t      *pstDst         Output result. Can be U32C1 or U64C1 format, relied on the control parameter.
...
*   Parameters   : MI_IVE_HANDLE               hHandle              Handle ID of a task
*                  MI_IVE_SrcImage_t          *pstSrc               Input source data. Only U16C1\S16C1 input format is supported.
*                  MI_IVE_DstImage_t          *pstDst               Output result
...
*   Parameters   : MI_IVE_HANDLE           hHandle     Handle ID of a task
*                  MI_IVE_SrcImage_t      *pstSrc1     Input source1. Only the U8C1 input format is supported.
*                  MI_IVE_SrcImage_t      *pstSrc2     Input source2. Must be of the same format and size with source1.
*                  MI_IVE_DstMemInfo_t    *pstDst      Output result
*                  MI_BOOL                 bInstant    Reserved
*   Return Value : MI_SUCCESS: Success;Error codes: Failure.
*   Spec         : The size of the input data ranges from 64x64 pixels to 1920x1024 pixels.
...
*   Description  : Connected Component Labeling. 4-Connected & 8-Connected method is supported.
*   Parameters   : MI_IVE_HANDLE          hHandle         Handle ID of a task
*                  MI_IVE_SrcImage_t     *pstSrcDst       Input source. Only the U8C1 format is supported.
*                  MI_IVE_DstMemInfo_t   *pstBlob         Output result of detected region.
*                  MI_IVE_CclCtrl_t      *pstCclCtrl      CCL control parameter
*                  MI_BOOL                bInstant        Reserved
*   Return Value : MI_SUCCESS: Success;   Error codes: Failure.
*   Spec         : The size of the input data ranges from 16x4 pixels to 720x640 pixels.
*                  The physical addresses of the input data and output data must be 16-byte-aligned.
*                  The stride must be 16-pixel-aligned.
*   History:
*****************************************************************************/
...
*   Parameters   : MI_IVE_HANDLE                hHandle             Handle ID of a task
*                  MI_IVE_SrcImage_t           *pstSrcPre           Pre-frame input source. Must be U8C1 format.
*                  MI_IVE_SrcImage_t           *pstSrcCur           Cur-frame input source. Of the same size and format with pstPreSrc.
...
*                  MI_BOOL                bInstant         Reserved
*   Return Value : MI_SUCCESS: Success;Error codes: Failure.
*   Spec         : The size of the input data ranges from 16x4 pixels to 1920x1280 pixels,
*                  and the resize factor should be ranges from 1/16 to 16.
*                  The input and output stride must be 16-pixel-aligned.
*   History:
*****************************************************************************/
...
/*****************************************************************************
*   Prototype    : MI_IVE_BGBlur
*   Description  : Used to do background blur/mosaic/blur+mosaic on the basis of input mask of Y/UV component.
*   Parameters   : MI_IVE_HANDLE          hHandle         Handle ID of a task.
*                  MI_IVE_SrcImage_t      pstSrcYMask     Input mask of Y component and only the U8C1 format is supported.
*                  MI_IVE_SrcImage_t      pstSrcUvMask    Input mask of UV component and only the U8C1 format is supported.
*                  MI_IVE_SrcImage_t      pstSrcOri       Input original image and only the YUV420SP format is supported.
*                  MI_IVE_SrcImage_t      pstSrcRepBg     Input small original image and only the YUV420SP format is supported.
*                  MI_IVE_DstImage_t      pstDst          Output result, the supported format is the same with pstSrcOri.
*   Return Value : MI_SUCCESS: Success;   Error codes:    Failure.
*   Spec         : The width of the mask image of Y component support 640 pixels when the width of the original image is bigger than its height.
*                  The width of the mask image of Y component support 480 pixels when the width of the original image is less   than its height.
*                  The size of the small original image should the same with the mask image of Y component.
*                  The size of the input original image ranges from 640x360 pixels to 2688x2048 pixels.
*                  The physical addresses of the input data and output data must be 16-byte-aligned.
*                  The stride of the maks&input&output image must be 16-pixel-aligned.
*   History:
*****************************************************************************/
