...
#define JPD_MAJOR_VERSION   2
#define JPD_SUB_VERSION     1
#define MACRO_TO_STR(macro) #macro
#define JPD_VERSION_STR(major_version, sub_version)                                                                 \
    (                                                                                                               \
        {                                                                                                           \
            char *tmp =                                                                                             \
                sub_version / 100  ? "mi_jpd_version_" MACRO_TO_STR(major_version) "." MACRO_TO_STR(sub_version)    \
                : sub_version / 10 ? "mi_jpd_version_" MACRO_TO_STR(major_version) ".0" MACRO_TO_STR(sub_version)   \
                                   : "mi_jpd_version_" MACRO_TO_STR(major_version) ".00" MACRO_TO_STR(sub_version); \
            tmp;                                                                                                    \
        })
...
extern "C"
{
    ...
    MI_S32 MI_JPD_GetStreamBuf(MI_JPD_DEV JpdDev, MI_JPD_CHN JpdChn, MI_U32 u32RequiredLength,
                               MI_JPD_StreamBuf_t *pstRetStreamBuf, MI_S32 s32MilliSecToWait);
    ...
    MI_S32 MI_JPD_QueryStreamInfo(MI_JPD_DEV JpdDev, MI_JPD_CHN JpdChn, MI_JPD_DirectInputBuf_t *pstInputBuf,
                                  MI_JPD_StreamInfo_t *pstStreamInfo);
    MI_S32 MI_JPD_DirectBufDecode(MI_JPD_DEV JpdDev, MI_JPD_CHN JpdChn, MI_JPD_DirectInputBuf_t *pstInputBuf,
                                  MI_JPD_DirectOutputBuf_t *pstOutputBuf);
