...
#
include $(PROJ_ROOT)/release/chip/$(CHIP)/$(PRODUCT)/$(BOARD)/bootscript/$(FLASH_TYPE)/boot_sequence.mk
...
DEMO=$(LIB_DIR_PATH)/bin/sample_code/bin/prog_vif_sensor_demo
DEMO_RELEASE_DIR=$(OUTPUTDIR)/rootfs/
DEMO_RUN_PATH=
DEMO_RUN_OPT=0 index 6
...
CONFIG_FILES_DIR=$(OUTPUTDIR)/rootfs/config
...
rootfs_$(PRODUCT):
	...
	if [ -d $(PROJ_ROOT)/board/$(CHIP)/nir_file ]; then \
		cp -rf $(PROJ_ROOT)/board/$(CHIP)/nir_file $(CONFIG_FILES_DIR); \
	fi;
	...
	sed -i 's/mi_common.ko/mi_common.ko g_ModParamPath=\/$(notdir $(CONFIG_FILES_DIR))\/modparam.json/g' $(INIT_FILE);
	sed -i 's/sstar_mi.ko/sstar_mi.ko common_init.g_ModParamPath=\/$(notdir $(CONFIG_FILES_DIR))\/modparam.json/g' $(INIT_FILE);
