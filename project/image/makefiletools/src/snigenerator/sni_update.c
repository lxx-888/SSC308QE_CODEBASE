...
#define NEW_SNI_SIZE 1024
#define ROM_SNI_SIZE 256
#define SNI_SIZE 512
#define FLASH_SNI_SIZE 768
...
typedef struct
{
    ...
    u8  boot_id_loc;
    u8  reserved[24]; // just for aligning to 64bytes + magic[16] = 80bytes
} spinand_info_t;
...
int main(int argc, char **argv)
{
	...
	char *rom_sni_filename               = NULL;
	char rom_sni_Buf[ROM_SNI_SIZE]       = {0};
	char sni_Buf[SNI_SIZE]               = {0};
	char sni_change_Buf[FLASH_SNI_SIZE]  = {0};
	char ouput_Buf[NEW_SNI_SIZE]         = {0};

    spinand_sni_t *spinand_sni           = NULL;
    spinand_info_t *spinand_info          = NULL;

	while ((opt = getopt(argc, argv, "r:o:i:")) != -1) {
        switch (opt) {
			case 'r':
                rom_sni_filename = optarg;
                break;	
            case 'o':
    ...
    if(!output_filename || !input_filename || !rom_sni_filename)
    {
        goto err;
    }

    fd = open(input_filename, O_RDONLY);
	fd1 = open(rom_sni_filename, O_RDONLY);
    stream = fopen(output_filename, "w+");

    spinand_sni  = (spinand_sni_t *)sni_change_Buf;
    spinand_info = &spinand_sni->spinand_info;

    if(fd < 0 || fd1 < 0 || !stream)
    {
        goto err;
    }

    if (ROM_SNI_SIZE != read(fd1, rom_sni_Buf, ROM_SNI_SIZE))
    {
        goto err;
    }
    ...
    do
	{
	    if (SNI_SIZE != read(fd, sni_Buf, SNI_SIZE))
        {
            break;
        }

        spinand_sni->magic[0] = 0x53;
		spinand_sni->magic[1] = 0x4e;
		spinand_sni->magic[2] = 0x49;
		spinand_sni->magic[3] = 0x46;
		memcpy(&sni_change_Buf[8], &sni_Buf[16], sizeof(spinand_info_t));
        memcpy(&sni_change_Buf[8 + sizeof(spinand_info_t)], &sni_Buf[36 + sizeof(spinand_info_t)], sizeof(spinand_ext_profile_t));
        memcpy(&sni_change_Buf[32 + sizeof(spinand_info_t) + sizeof(spinand_ext_profile_t)], &sni_Buf[44 + sizeof(spinand_info_t) + sizeof(spinand_ext_profile_t)], 32);

        spinand_sni->crc32 = calc_crc32(0, &spinand_info->id_byte_cnt, (U32)(FLASH_SNI_SIZE - 8));

	    memcpy(ouput_Buf, rom_sni_Buf, ROM_SNI_SIZE);
		memcpy(&ouput_Buf[ROM_SNI_SIZE], sni_change_Buf, FLASH_SNI_SIZE);
	    fwrite(ouput_Buf, 1, NEW_SNI_SIZE, stream);
	} while(1);
