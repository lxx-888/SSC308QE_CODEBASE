						  ...
						  char ownername[])
{
	...
	if (err) {
		printk("%s: verify symbol (%s), return %d.\n",
			mod->name, kernel_symbol_name(sym), err);
		sym = ERR_PTR(err);
		goto getname;
	}
...
static void free_module(struct module *mod)
{
	...
	trace_android_vh_set_memory_nx((unsigned long)mod->init_layout.base,
		(mod->init_layout.size)>>PAGE_SHIFT);
#if defined(CONFIG_ARCH_SSTAR) && defined(CONFIG_ARM) && defined(CONFIG_KASAN)
	kasan_unpoison_module(mod->init_layout.base, mod->init_layout.size);
#endif
	...
	trace_android_vh_set_memory_nx((unsigned long)mod->core_layout.base,
		(mod->core_layout.size)>>PAGE_SHIFT);
#if defined(CONFIG_ARCH_SSTAR) && defined(CONFIG_ARM) && defined(CONFIG_KASAN)
	kasan_unpoison_module(mod->core_layout.base, mod->core_layout.size);
#endif
