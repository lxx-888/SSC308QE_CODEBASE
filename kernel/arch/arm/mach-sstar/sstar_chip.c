 ...
 * dmac_map_area is private to the dma-mapping API,
 * and can't be used directly.
 * We use dmac_map_area to invalidate cache and need the
 * private function to be exported here.
 * Rules are made to be broken... It's so bad.
 */
...
extern struct device *parent ;

const struct SS_BIN_OPTION ms_image_size __attribute__ ((aligned (16))) =
{
	{'#','I','M','G','_','S','Z','#'},
	{0,0,0,0,0,0,0,0}
};
...
#define BUILTIN_UNFDT_SZ (192*1024)
...
void Chip_Flush_CacheAllOnAllCPUS(void)
{
	chip_funcs.cache_flush_all_on_all_cpus();
}
...
static void _default_Flush_Cache_Range_VA_PA(void* pVAddr, unsigned long long u64PAddr,unsigned long u32Size)
{
	...
	//Clean & Invalid inner cache by range
	dmac_flush_range(pVAddr, pVAddr + u32Size);

#ifdef CONFIG_OUTER_CACHE
#ifdef CONFIG_MS_L2X0_PATCH
	if (Chip_Cache_Outer_Is_Enabled()) //check if outer cache is enabled
#endif
	{
		//Clean&Inv outer cache by range
...
static void _default_Flush_Cache_Range(void* pAddr, unsigned long u32Size)
{
	...
	//Clean & Inv inner cache by range
	dmac_flush_range(pAddr, pAddr + u32Size);
...
static void _default_Clean_Cache_Range(void* pAddr, unsigned long u32Size)
{
	...
	//Clean inner cache by range
	dmac_map_area((const void *)pAddr, (size_t)u32Size, DMA_TO_DEVICE);
	//dma_sync_single_for_cpu(NULL, virt_to_dma(NULL, pAddr), u32Size, DMA_TO_DEVICE);
#ifdef CONFIG_OUTER_CACHE
#ifdef CONFIG_MS_L2X0_PATCH
	if (Chip_Cache_Outer_Is_Enabled()) //check if outer cache is enabled
#endif
...
static void _default_Inv_Cache_Range(void* pAddr, unsigned long u32Size)
{
...
#endif
	{
		if (!virt_addr_valid(pAddr) || !virt_addr_valid(pAddr+ u32Size - 1)) {
			...
			//Inv outer cache by range
			outer_cache.inv_range(__pa(pAddr) , __pa(pAddr)+ u32Size);
		}
	}
#endif
	//Inv inner cache by range
	dmac_map_area((const void *)pAddr, (size_t)u32Size, DMA_FROM_DEVICE);
	//dma_sync_single_for_cpu(parent, virt_to_dma(NULL, pAddr), u32Size, DMA_FROM_DEVICE);
}
...
static inline void ipi_flush_cache_all(void *ignored)
{
        _default_Flush_CacheAll();
}

static void _default_Flush_CacheAllOnAllCPUS(void)
{
...
void __init sstar_chip_init_default(void)
{
	...
	chip_funcs.cache_flush_all_on_all_cpus = _default_Flush_CacheAllOnAllCPUS;
...
EXPORT_SYMBOL(Chip_Flush_CacheAllOnAllCPUS);
