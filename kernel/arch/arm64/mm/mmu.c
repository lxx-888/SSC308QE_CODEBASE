...
#define PGAE_DEVICE __pgprot(PROT_DEVICE_nGnRE)
extern void map_device(pgd_t *pgd);
void __init __map_device(pgd_t *pgd, struct map_desc_aarch64 *des, int nr)
{
 	struct map_desc_aarch64 *md;
 	struct vm_struct *vm;
	...
	for (md = des; nr; md++, nr--) {
		/* The virt address should not overlap with kernel image
		*/
		BUG_ON(((md->virt >= (unsigned long)_text) && (md->virt <= (unsigned long)_end)));

		/* Create the page table for SSTAR's device
	 	* with property of device, because they are
	 	* register likely.
	 	*/
		__create_pgd_mapping(pgd, md->phys, md->virt,
			md->size, PGAE_DEVICE,
			early_pgtable_alloc,
			!debug_pagealloc_enabled());

		vm = &md->vm;
		vm->addr = (void *)(md->virt & PAGE_MASK);
		vm->size = PAGE_ALIGN(md->size + (md->virt & ~PAGE_MASK));
		vm->phys_addr = md->phys;
		vm->flags = VM_IOREMAP;
		vm->caller = __map_device;
...
void __init paging_init(void)
{
	...
	pgd_t *pgdp;
	pgd_phys = early_pgtable_alloc(0);
...
#ifdef CONFIG_ARCH_SSTAR
	/* This map the RIU, GIC, SPI, IMI registers area.
	* These area are mapped as static-mapping in aarch32,
	* Inorder to reuse the SSTAR's old drivers, a simple way
	* is to map these zone here. The same as kernel image,
	* they are in vmalloc.
	*/
