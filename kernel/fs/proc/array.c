			...
			struct pid *pid, struct task_struct *task, int whole)
{
	...
	if (lock_task_sighand(task, &flags)) {
		...
		if (whole) {
			...
			maj_flt += sig->maj_flt;
#if defined(CONFIG_PARAVIRT) && (defined(CONFIG_LH_RTOS) || defined(CONFIG_LINUX_ON_SS_HYPERVISOR))
			/* Replace thread_group_cputime_adjusted with thread_group_cputime
			   because cputime_adjust proportionally allocates sum_exec_runtime
			   to utime and stime, which includes steal time. When steal time is
			   non-zero, cputime_adjust may load to an overestimation of CPU
			   usage for the task.
			*/
...
#if defined(CONFIG_PARAVIRT) && (defined(CONFIG_LH_RTOS) || defined(CONFIG_LINUX_ON_SS_HYPERVISOR))
		/* Replace task_cputime_adjusted with task_cputime
		   because cputime_adjust proportionally allocates sum_exec_runtime
		   to utime and stime, which includes steal time. When steal time is
		   non-zero, cputime_adjust may load to an overestimation of CPU
		   usage for the task.
