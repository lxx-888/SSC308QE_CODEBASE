...
#define PSTORE_KMSG_HEADER_MAGIC 0x4dfc3ae5 /* Just a random number */
	uint32_t magic;
#ifdef CONFIG_PSTORE_ZONE_RECORD_ORDER_BY_COUNTER
...
#ifndef CONFIG_PSTORE_ZONE_RECORD_ORDER_BY_COUNTER
	struct timespec64 time = { };
#endif
	unsigned long i;
	/*
...
#else
		if (hdr->total_counter >= cxt->total_counter) {
			cxt->total_counter = hdr->total_counter;
			cxt->kmsg_write_cnt = (i + 1) % cxt->kmsg_max_cnt;
		}
#endif
		...
		struct pstore_record *record)
{
	...
	if (hdr->reason == KMSG_DUMP_OOPS)
		...
		hdr->counter = 0;
#ifdef CONFIG_PSTORE_ZONE_RECORD_ORDER_BY_COUNTER
	hdr->total_counter = ++cxt->total_counter;
#endif
