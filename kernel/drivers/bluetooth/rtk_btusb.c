...
#define RTKBT_RELEASE_NAME "20210702_BT_ANDROID_11.0"
...
{ 0x7392, 0xC611, 0x8761, 0, 0, "mp_rtl8761b_fw", "rtl8761b_fw", "rtl8761b_config", NULL, 0 ,CONFIG_MAC_OFFSET_GEN_4PLUS, MAX_PATCH_SIZE_40K}, /* RTL8761BU */
...
static void set_wakeup_device_from_conf(firmware_info *fw_info)
{
    ...
    if (!IS_ERR(fp)) {
        ...
        set_fs(KERNEL_DS);
        pos = 0;
        //read number change vfs_read to kernel_read
        kernel_read(fp, &num, 1, &pos);
        RTKBT_DBG("read number = %d", num);
        if(num) {
            for(i = 0; i < num; i++) {
            //change vfs_read to kernel_read
            kernel_read(fp, paired_wakeup_bdaddr, 7, &pos);
