...
#if defined(CONFIG_SSTAR_GMAC_EXTRA_STATS)
	STMMAC_STAT(napi_poll_tx),
	STMMAC_STAT(napi_poll_rx),
	STMMAC_STAT(last_tx_dma_en_jiffies),
	STMMAC_STAT(last_rx_dma_en_jiffies),
	STMMAC_STAT(last_hw_irq_jiffies),
	STMMAC_STAT(last_hw_irq_tx_jiffies),
	STMMAC_STAT(last_hw_irq_rx_jiffies),
	STMMAC_STAT(last_napi_poll_tx_jiffies),
	STMMAC_STAT(last_napi_poll_rx_jiffies),
	STMMAC_STAT(max_napi_poll_tx_time),
	STMMAC_STAT(max_napi_poll_rx_time),
#endif

#if defined(CONFIG_SSTAR_SNPS_GMAC_RXIC)
	STMMAC_STAT(napi_poll_rx),
	STMMAC_STAT(napi_poll_rx_irq),
	STMMAC_STAT(napi_poll_rx_tmr),
	STMMAC_STAT(tx_normal_ti_irq_n),
	STMMAC_STAT(tx_normal_tbu_irq_n),
#endif
...
static void stmmac_get_per_qstats(struct stmmac_priv *priv, u64 *data)
{
	...
	for (q = 0; q < tx_cnt; q++) {
		p = (char *)priv +
		    offsetof(struct stmmac_priv, xstats.txq_stats[q].tx_pkt_n);
	...
	for (q = 0; q < rx_cnt; q++) {
		p = (char *)priv +
		    offsetof(struct stmmac_priv, xstats.rxq_stats[q].rx_pkt_n);
...
static int stmmac_get_sset_count(struct net_device *netdev, int sset)
{
	...
	case ETH_SS_STATS:
#ifdef CONFIG_ARCH_SSTAR
		len = STMMAC_STATS_LEN + STMMAC_TXQ_STATS * tx_cnt +
				     ...
				     struct ethtool_eee *edata)
{
	...
	priv->eee_force_disable = !edata->eee_enabled;
#endif
