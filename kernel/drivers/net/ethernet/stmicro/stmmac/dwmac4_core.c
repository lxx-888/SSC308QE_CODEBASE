...
static void dwmac4_flow_ctrl_tx(struct mac_device_info *hw, u32 chan, u32 pause_time)
{
	...
	mtl_txop = readl(ioaddr + MTL_CHAN_TX_OP_MODE(chan)) | MTL_OP_MODE_FTQ/* Flush transmit queue */;
	writel(mtl_txop, ioaddr + MTL_CHAN_TX_OP_MODE(chan));
}
#endif
