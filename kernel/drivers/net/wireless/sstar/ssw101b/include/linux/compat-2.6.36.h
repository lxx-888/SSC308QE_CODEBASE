...
#define usb_scuttle_anchored_urbs(anchor)                                      \
	compat_usb_scuttle_anchored_urbs(anchor)
 ...
 * attribute memory.
 */
static inline int pcmcia_read_config_byte(struct pcmcia_device *p_dev,
					  off_t where, u8 *val)
{
	int ret;
 ...
 * attribute memory.
 */
static inline int pcmcia_write_config_byte(struct pcmcia_device *p_dev,
					   off_t where, u8 val)
{
...
#define pm_qos_remove_request(_req) pm_qos_remove_request((_req)->request)
 ...
 */
static inline __attribute__((format(printf, 1, 2))) int no_printk(const char *s,
								  ...)
{
	return 0;
}
