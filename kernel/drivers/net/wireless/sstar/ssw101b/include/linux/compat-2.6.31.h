...
#define rfkill_destroy backport_rfkill_destroy
	...
	!defined(CONFIG_PARISC) && !defined(CONFIG_SPARC)
...
static inline void compat_irq_work(struct work_struct *work)
{
...
static inline irqreturn_t compat_irq_dispatcher(int irq, void *dev_id)
{
...
static inline int
compat_request_threaded_irq(struct compat_threaded_irq *comp, unsigned int irq,
			    ...
			    unsigned long flags, const char *name, void *dev_id)
{
	...
	if (!comp->wq) {
		snprintf(comp->wq_name, sizeof(comp->wq_name), "compirq/%u-%s",
			 irq, name);
		comp->wq = create_singlethread_workqueue(comp->wq_name);
		if (!comp->wq) {
			printk(KERN_ERR
			       "Failed to create compat-threaded-IRQ workqueue %s\n",
			       comp->wq_name);
			return -ENOMEM;
		}
	}
...
static inline void compat_free_threaded_irq(struct compat_threaded_irq *comp)
{
	free_irq(comp->irq, comp);
}

static inline void compat_destroy_threaded_irq(struct compat_threaded_irq *comp)
{
	if (comp->wq)
...
static inline void
compat_synchronize_threaded_irq(struct compat_threaded_irq *comp)
{
