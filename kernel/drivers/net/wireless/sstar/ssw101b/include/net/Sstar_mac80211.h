...
#else
#define Sstar_workqueue_struct workqueue_struct
#define Sstar_delayed_work delayed_work
#define Sstar_work_struct work_struct
#endif
...
extern int cfg80211_altmtest_reply(struct wiphy *wiphy, const void *data,
				   int len);
#define DBUG_AUTHEN(fc)                                                        \
	if (ieee80211_is_auth(fc))                                             \
		Sstar_printk_mgmt("%s:%d,fc(%x)\n", __func__, __LINE__, fc);
static inline unsigned Sstar_compare_ether_addr(const u8 *addr1,
						const u8 *addr2)
{
...
enum Sstar_ieee80211_category {
	SSTAR_WLAN_CATEGORY_SPECTRUM_MGMT = 0,
	SSTAR_WLAN_CATEGORY_QOS = 1,
	SSTAR_WLAN_CATEGORY_DLS = 2,
	SSTAR_WLAN_CATEGORY_BACK = 3,
	SSTAR_WLAN_CATEGORY_PUBLIC = 4,
	SSTAR_WLAN_CATEGORY_RADIO_MEASUREMENT = 5,
	SSTAR_WLAN_CATEGORY_HT = 7,
	SSTAR_WLAN_CATEGORY_SA_QUERY = 8,
	SSTAR_WLAN_CATEGORY_PROTECTED_DUAL_OF_ACTION = 9,
	SSTAR_WLAN_CATEGORY_WNM = 10,
	SSTAR_WLAN_CATEGORY_WNM_UNPROTECTED = 11,
	SSTAR_WLAN_CATEGORY_TDLS = 12,
	SSTAR_WLAN_CATEGORY_MESH_ACTION = 13,
	SSTAR_WLAN_CATEGORY_MULTIHOP_ACTION = 14,
	SSTAR_WLAN_CATEGORY_SELF_PROTECTED = 15,
	SSTAR_WLAN_CATEGORY_DMG = 16,
	SSTAR_WLAN_CATEGORY_WMM = 17,
	SSTAR_WLAN_CATEGORY_FST = 18,
	SSTAR_WLAN_CATEGORY_UNPROT_DMG = 20,
	SSTAR_WLAN_CATEGORY_VHT = 21,
	SSTAR_WLAN_CATEGORY_VENDOR_SPECIFIC_PROTECTED = 126,
	SSTAR_WLAN_CATEGORY_VENDOR_SPECIFIC = 127,
};
...
#define SSTAR_MICHAEL_MIC_LEN 8
					      ...
					      size_t len)
{
	...
	if (!ieee80211_is_action(hdr->frame_control))
		return false;
	return mgmt->u.action.category == SSTAR_WLAN_CATEGORY_PUBLIC;
}
#endif
...
enum ieee80211_bss_change {
	...
	BSS_CHANGED_PS = 1 << 16,
	BSS_CHANGED_CHANNEL =
		1 << 17, // XXX: COMBO: should this be merged with _HT?
...
#define IEEE80211_TX_TEMPORARY_FLAGS                                           \
	(IEEE80211_TX_CTL_NO_ACK | IEEE80211_TX_CTL_CLEAR_PS_FILT |            \
	 IEEE80211_TX_CTL_FIRST_FRAGMENT | IEEE80211_TX_CTL_SEND_AFTER_DTIM |  \
	 IEEE80211_TX_CTL_AMPDU | IEEE80211_TX_STAT_TX_FILTERED |              \
	 IEEE80211_TX_STAT_ACK | IEEE80211_TX_STAT_AMPDU |                     \
	 IEEE80211_TX_STAT_AMPDU_NO_BACK | IEEE80211_TX_CTL_RATE_CTRL_PROBE |  \
	 IEEE80211_TX_CTL_POLL_RESPONSE | IEEE80211_TX_CTL_MORE_FRAMES |       \
...
#define IEEE80211_SPECIAL_FILTER_MASK                                          \
	(SPECIAL_F_FLAGS_FRAME_TYPE | SPECIAL_F_FLAGS_FRAME_OUI |              \
...
struct ieee80211_ops {
	...
	void (*get_ringparam)(struct ieee80211_hw *hw, u32 *tx, u32 *tx_max,
			      u32 *rx, u32 *rx_max);
	bool (*tx_frames_pending)(struct ieee80211_hw *hw);
	int (*set_bitrate_mask)(struct ieee80211_hw *hw,
				struct ieee80211_vif *vif,
	...
	void (*allow_buffered_frames)(struct ieee80211_hw *hw,
				      ...
				      bool more_data);
	void (*release_buffered_frames)(
		struct ieee80211_hw *hw, struct ieee80211_sta *sta, u16 tids,
		int num_frames, enum ieee80211_frame_release_type reason,
		bool more_data);
	...
	int (*sta_triger_listen)(struct ieee80211_hw *hw,
				 struct ieee80211_vif *vif,
				 struct ieee80211_channel *chan);
	int (*sta_stop_listen)(struct ieee80211_hw *hw,
			       struct ieee80211_vif *vif);
#ifdef CONFIG_IEEE80211_SPECIAL_FILTER
	int (*set_frame_filter)(struct ieee80211_hw *hw,
 ...
 */
__le16
ieee80211_ctstoself_duration(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			     size_t frame_len,
			     const struct ieee80211_tx_info *frame_txctl);
 ...
 */
void ieee80211_get_tkip_p1k_iv(struct ieee80211_key_conf *keyconf, u32 iv32,
			       u16 *p1k);
 ...
 */
void ieee80211_get_key_rx_seq(struct ieee80211_key_conf *keyconf, int tid,
			      struct ieee80211_key_seq *seq);
/**
 ...
 */
enum rate_control_changed { IEEE80211_RC_HT_CHANGED = BIT(0) };
...
bool rate_control_send_low(struct ieee80211_sta *sta, void *priv_sta,
			   struct ieee80211_tx_rate_control *txrc);
static inline s8 rate_lowest_index(struct ieee80211_supported_band *sband,
				   struct ieee80211_sta *sta)
{
	int i;
...
static inline bool
rate_usable_index_exists(struct ieee80211_supported_band *sband,
			 struct ieee80211_sta *sta)
{
	unsigned int i;
...
static inline bool conf_is_ht(struct ieee80211_channel_conf *conf)
{
	return conf->channel_type != NL80211_CHAN_NO_HT;
}

static inline enum nl80211_iftype ieee80211_iftype_p2p(enum nl80211_iftype type,
						       bool p2p)
{
	if (p2p) {
		switch (type) {
...
static inline void Sstar_wdev_lock(struct wireless_dev *wdev) __acquires(wdev)
{
	mutex_lock(&wdev->mtx);
	__acquire(wdev->mtx);
}

static inline void Sstar_wdev_unlock(struct wireless_dev *wdev) __releases(wdev)
{
	__release(wdev->mtx);
...
static inline void Sstar_ewma_init(struct Sstar_ewma *avg, unsigned long factor,
				   unsigned long weight)
{
 ...
 */
static inline struct Sstar_ewma *Sstar_ewma_add(struct Sstar_ewma *avg,
						unsigned long val)
{
	avg->internal =
		avg->internal ?
...
static inline const u8 *Sstar_ieee80211_find_vendor_ie(unsigned int oui,
						       u8 oui_type,
						       const u8 *ies, int len)
{
	...
	while (pos < end) {
		pos = Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_VENDOR_SPECIFIC,
					      pos, end - pos);
		if (!pos)
			return NULL;
		...
		BUILD_BUG_ON(offsetof(struct Sstar_ieee80211_vendor_ie, len) !=
			     1);
...
static inline u8 *Sstar_ieee80211_find_p2p_ie(const u8 *ie_start, size_t ie_len)
{
	if ((ie_start == NULL) || (ie_len == 0))
		return NULL;
	return (u8 *)Sstar_ieee80211_find_vendor_ie(
		SSTAR_OUI_WFA, SSTAR_P2P_OUI_TYPE, ie_start, ie_len);
}

static inline u8 *Sstar_ieee80211_find_p2p_attr(u8 *attr_start,
						ssize_t attr_len, u8 attr_id)
{
	...
	while (attr_len > 2 && attr_start[0] != attr_id) {
		attr_len -=
			SSTAR_WPA_GET_LE16((const u8 *)(&attr_start[1])) + 3;
		attr_start +=
			SSTAR_WPA_GET_LE16((const u8 *)(&attr_start[1])) + 3;
	}
...
/*
*Sstar timer function
*/
struct Sstar_timer_list {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
static inline void Sstar_timer_handle(struct timer_list *in_timer)
#else
static inline void Sstar_timer_handle(unsigned long data)
#endif
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	struct Sstar_timer_list *Sstar_timer =
		from_timer(Sstar_timer, in_timer, timer);
#else
	struct Sstar_timer_list *Sstar_timer = (struct Sstar_timer_list *)data;
#endif
	BUG_ON(Sstar_timer->function == NULL);
	Sstar_timer->function(Sstar_timer->data);
}

static inline void Sstar_init_timer(struct Sstar_timer_list *Sstar_timer)
{
	Sstar_timer->expires = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	timer_setup(&Sstar_timer->timer, Sstar_timer_handle, 0);
#else
	/* setup_timer(ptimer, pfunc,(u32)cntx);	 */
	Sstar_timer->timer.function = Sstar_timer_handle;
	Sstar_timer->timer.data = (unsigned long)Sstar_timer;
	init_timer(&Sstar_timer->timer);
#endif
}

static inline void Sstar_setup_timer(struct Sstar_timer_list *Sstar_timer,
				     void (*function)(unsigned long data),
				     unsigned long data)
{
	Sstar_timer->expires = 0;
	Sstar_timer->function = function;
	Sstar_timer->data = data;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0))
	timer_setup(&Sstar_timer->timer, Sstar_timer_handle, 0);
#else
...
static inline int Sstar_del_timer_sync(struct Sstar_timer_list *Sstar_timer)
{
	return del_timer_sync(&Sstar_timer->timer);
}
static inline int Sstar_mod_timer(struct Sstar_timer_list *Sstar_timer,
				  unsigned long expires)
{
	int ret = 0;
	ret = mod_timer(&Sstar_timer->timer, expires);
	Sstar_timer->expires = Sstar_timer->timer.expires;

	return ret;
}

static inline void Sstar_add_timer(struct Sstar_timer_list *Sstar_timer)
{
	Sstar_timer->timer.expires = Sstar_timer->expires;
	add_timer(&Sstar_timer->timer);
}

static inline int Sstar_del_timer(struct Sstar_timer_list *Sstar_timer)
{
	return del_timer(&Sstar_timer->timer);
}
static inline int Sstar_timer_pending(struct Sstar_timer_list *Sstar_timer)
{
	return timer_pending(&Sstar_timer->timer);
}
extern struct cfg80211_bss *
ieee80211_Sstar_get_bss(struct wiphy *wiphy, struct ieee80211_channel *channel,
			const u8 *bssid, const u8 *ssid, size_t ssid_len,
			u16 capa_mask, u16 capa_val);
extern void ieee80211_Sstar_put_authen_bss(struct ieee80211_vif *vif,
					   struct cfg80211_bss *bss);
extern void __ieee80211_Sstar_put_authen_bss(struct ieee80211_vif *vif,
					     struct cfg80211_bss *bss);

extern void ieee80211_Sstar_put_bss(struct wiphy *wiphy,
				    struct cfg80211_bss *pub);
extern int ieee80211_Sstar_handle_bss(struct wiphy *wiphy,
				      struct cfg80211_bss *pub);
extern void ieee80211_Sstar_release_bss(struct wiphy *wiphy,
					struct cfg80211_bss *pub);
extern struct cfg80211_bss *ieee80211_Sstar_get_authen_bss(
	struct ieee80211_vif *vif, struct ieee80211_channel *channel,
	const u8 *bssid, const u8 *ssid, size_t ssid_len);
extern struct cfg80211_bss *__ieee80211_Sstar_get_authen_bss(
	struct ieee80211_vif *vif, struct ieee80211_channel *channel,
	const u8 *bssid, const u8 *ssid, size_t ssid_len);
extern char *ieee80211_alloc_name(struct ieee80211_hw *hw, const char *name);
extern bool Sstar_ieee80211_is_robust_mgmt_frame(struct sk_buff *skb);

#define IEEE80211_CHANNLE_SPECIAL_FREQ_FLAG BIT(15)
#define IEEE80211_CHANNLE_CCA_RUNNING_FLAG BIT(14)

#define IEEE80211_CHANNLE_HW_VALUE_MASK                                        \
	(IEEE80211_CHANNLE_SPECIAL_FREQ_FLAG |                                 \
	 IEEE80211_CHANNLE_CCA_RUNNING_FLAG)
...
{
	return (channel->hw_value & IEEE80211_CHANNLE_SPECIAL_FREQ_FLAG) ?
			     true :
			     false;
}
static inline bool channel_in_cca(struct ieee80211_channel *channel)
{
	return (channel->hw_value & IEEE80211_CHANNLE_CCA_RUNNING_FLAG) ? true :
										false;
}
...
static inline void Sstar_common_hash_list_init(struct hlist_head *hlist,
					       size_t size)
{
	size_t index = 0;
...
Sstar_hash_index(const char *string, unsigned int len, unsigned int hash_bit)
{
...
static inline struct hlist_head *Sstar_hash_list(const char *string,
						 unsigned int len,
						 struct hlist_head *hlist_table,
...
ieee8011_chnum_to_channel(struct ieee80211_hw *hw, int channel)
{
	int freq;
	if (channel <= 0)
		return NULL;
...
#define SSTAR_PRINTK_MASK_WSM BIT(21)
...
#define SSTAR_PRINTK_LEVEL1                                                    \
	(SSTAR_PRINTK_MASK_ERR | SSTAR_PRINTK_MASK_WARN |                      \
	 SSTAR_PRINTK_MASK_INIT | SSTAR_PRINTK_MASK_EXIT |                     \
	 SSTAR_PRINTK_MASK_SCAN | SSTAR_PRINTK_MASK_LMAC |                     \
	 SSTAR_PRINTK_MASK_PM | SSTAR_PRINTK_MASK_WEXT)
#define SSTAR_PRINTK_LEVEL2                                                    \
	(SSTAR_PRINTK_LEVEL1 | SSTAR_PRINTK_MASK_P2P |                         \
	 SSTAR_PRINTK_MASK_MGMT | SSTAR_PRINTK_MASK_AP |                       \
	 SSTAR_PRINTK_MASK_STA | SSTAR_PRINTK_MASK_CFG80211 |                  \
	 SSTAR_PRINTK_MASK_PLATFROM)

#define SSTAR_PRINTK_LEVEL3                                                    \
	(SSTAR_PRINTK_LEVEL2 | SSTAR_PRINTK_MASK_BH | SSTAR_PRINTK_MASK_TX |   \
	 SSTAR_PRINTK_MASK_RX | SSTAR_PRINTK_MASK_AGG | SSTAR_PRINTK_MASK_WSM)
#define SSTAR_PRINTK_LEVEL4                                                    \
	(SSTAR_PRINTK_LEVEL3 | SSTAR_PRINTK_MASK_BUS |                         \
	 SSTAR_PRINTK_MASK_DEBUG | SSTAR_PRINTK_MASK_SMARTCONFIG)

#define SSTAR_PRINTK_DEFAULT_MASK SSTAR_PRINTK_LEVEL1

extern const char *Sstar_log;

#ifdef CONFIG_SSTAR_MOULE_FS
extern u32 Sstar_printk_mask;
#else
#define Sstar_printk_mask SSTAR_PRINTK_DEFAULT_MASK
#endif

#define SSTAR_PRINTK_ALL ((u32)(-1))
#define SSTAR_PRINTK_CLEAR (0)
#define SSTAR_TAG "[Sstar_log]:"
#define Sstar_printk(_level, fmt, arg...)                                      \
	do {                                                                   \
		if (Sstar_printk_mask & (_level))                              \
			printk(KERN_ERR "%s" fmt, Sstar_log, ##arg);           \
	} while (0)
...
#define Sstar_printk_smt(...)                                                  \
	Sstar_printk(SSTAR_PRINTK_MASK_SMARTCONFIG, __VA_ARGS__)
...
#define Sstar_printk_platform(...)                                             \
	Sstar_printk(SSTAR_PRINTK_MASK_PLATFROM, __VA_ARGS__)
#define Sstar_printk_bh(...) Sstar_printk(SSTAR_PRINTK_MASK_BH, __VA_ARGS__)
#define Sstar_printk_cfg(...)                                                  \
	Sstar_printk(SSTAR_PRINTK_MASK_CFG80211, __VA_ARGS__)
#define Sstar_printk_wsm(...) Sstar_printk(SSTAR_PRINTK_MASK_WSM, __VA_ARGS__)

#define Sstar_printk_debug(...)                                                \
	Sstar_printk(SSTAR_PRINTK_MASK_DEBUG, __VA_ARGS__)

#define Sstar_printk_always(fmt, arg...)                                       \
	printk(KERN_ERR "%s" fmt, Sstar_log, ##arg)
...
static inline const char *Sstar_skip_space(const char *target, ssize_t len)
{
	...
	while ((*target == SSTAR_SPACE) || (*target == SSTAR_LINEF) ||
	       (*target == SSTAR_ENTER)) {
		target++;
		if (target == pos_end) {
			break;
...
#define ieee80211_chw_is_ht40(chtype)                                          \
	(((chtype) == NL80211_CHAN_HT40PLUS) ||                                \
	 ((chtype) == NL80211_CHAN_HT40MINUS))
#define LIGHT "\e[1m"
#define NORMAL "\e[0m"
#define ENTER "\n"
