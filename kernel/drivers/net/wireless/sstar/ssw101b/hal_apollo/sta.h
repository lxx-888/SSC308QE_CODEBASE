...
int Sstar_change_interface(struct ieee80211_hw *dev, struct ieee80211_vif *vif,
			   enum nl80211_iftype new_type, bool p2p);
void Sstar_configure_filter(struct ieee80211_hw *dev, struct ieee80211_vif *vif,
...
void Sstar_keep_alive_work(struct Sstar_work_struct *work);
...
void Sstar_iterate_vifs(void *data, u8 *mac, struct ieee80211_vif *vif);
...
void Sstar_device_power_calc(struct Sstar_common *priv, s16 max_output_power,
			     s16 fe_cor, u32 band);
int Sstar_testmode_cmd(struct ieee80211_hw *hw, void *data, int len);
int Sstar_tesmode_event(struct wiphy *wiphy, const u32 msg_id, const void *data,
			int len, gfp_t gfp);
