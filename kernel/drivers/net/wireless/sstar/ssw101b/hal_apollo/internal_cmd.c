...
static struct Sstar_gpio_config Sstar_gpio_table[]=
{
	{
		.gpio = 4,
		.flags = 0,
...
#define SSTAR_WSM_TXPWR "set_txpower" SSTAR_SPACE_STR
...
#define SSTAR_WSM_SET_RATE_POWER                                               \
	"set_spec_rate_txpower_mode" SSTAR_SPACE_STR "%d" SSTAR_SPACE_STR "%d"

#ifdef CONFIG_SSTAR_MONITOR_SPECIAL_MAC
#define SSTAR_WSM_MONITOR_MAC                                                  \
	"set_sta_monitor" SSTAR_SPACE_STR "%d" SSTAR_SPACE_STR                 \
	"%d" SSTAR_SPACE_STR "%x,%x,%x,%x,%x,%x"
#endif
...
int str2mac(char *dst_mac, char *src_str)
{
	...
	for (i = 0; i < 6; i++) {
		dst_mac[i] = (char2Hex(src_str[i * 3]) << 4) +
			     (char2Hex(src_str[i * 3 + 1]));
...
u32 GetChipCrystalType(struct Sstar_common *hw_priv)
{
	...
	Sstar_direct_read_reg_32(hw_priv, 0x17400000, &pin_reg17400000);
	Sstar_direct_write_reg_32(hw_priv, 0x17400000,
				  pin_reg17400000 | BIT(8));
	Sstar_direct_read_reg_32(hw_priv, 0x17400000, &pin_reg17400000);
	if (pin_reg17400000 & BIT(17)) {
		etf_config.chip_crystal_type = 1;
	}
	Sstar_direct_read_reg_32(hw_priv, 0x16101010, &pin_reg);
	if (pin_reg & BIT(5)) {
	...
	Sstar_direct_write_reg_32(hw_priv, 0x17400000, pin_reg17400000);

	Sstar_printk_always("crystal:%d\n", etf_config.chip_crystal_type);
...
int ieee80211_set_channel(struct wiphy *wiphy, struct net_device *netdev,
			  struct ieee80211_channel *chan,
			  enum nl80211_channel_type channel_type);

static void
Sstar_internal_cmd_scan_dump(struct ieee80211_internal_scan_request *scan_req)
{
	int i;
	if (scan_req->n_ssids) {
				    ...
				    struct ieee80211_internal_scan_request *req)
{
	...
	scan_req = Sstar_kzalloc(
		sizeof(*scan_req) + sizeof(*scan_req->ssids) * req->n_ssids +
			sizeof(*scan_req->channels) * n_channels + req->ie_len +
			req->n_channels +
			sizeof(struct ieee80211_internal_mac) * req->n_macs,
		GFP_KERNEL);
	...
	*/
	if (req->n_ssids) {
		...
		for (i = 0; i < req->n_ssids; i++) {
			...
			memcpy(scan_req->ssids[i].ssid, req->ssids[i].ssid,
			       req->ssids[i].ssid_len);
		}
	...
	*/
	...
	if (req->n_macs) {
		...
		memcpy(local->internal_scan.req.macs, req->macs,
		       sizeof(struct ieee80211_internal_mac) * req->n_macs);
	...
	*/
	if (req->channels) {
		...
		memcpy(local->internal_scan.req.channels, req->channels,
		       req->n_channels);
	...
	rcu_assign_pointer(local->internal_scan.req.result_handle,
			   req->result_handle);
	rcu_assign_pointer(local->internal_scan.req.priv, req->priv);

	Sstar_common_hash_list_init(local->internal_scan.mac_hash_list,
				    IEEE80211_INTERNAL_SCAN_HASHENTRIES);

	for (index = 0; index < local->internal_scan.req.n_macs; index++) {
		int hash_index = Sstar_hash_index(
			local->internal_scan.req.macs[index].mac, 6,
			IEEE80211_INTERNAL_SCAN_HASHBITS);
		struct hlist_head *hlist =
			&local->internal_scan.mac_hash_list[hash_index];
		hlist_add_head(&local->internal_scan.req.macs[index].hnode,
			       hlist);
	}
	...
	if (ieee80211_internal_scan_triger(sdata, scan_req) == false) {
		...
		for (index = 0; index < local->internal_scan.req.n_macs;
		     index++) {
		...
		memset(&local->internal_scan.req, 0,
		       sizeof(struct ieee80211_internal_scan_sta));
		goto err;
	}
	...
	status =
		wait_event_timeout(local->internal_scan_wq,
				   atomic_read(&local->internal_scan_status) !=
					   IEEE80211_INTERNAL_SCAN_STATUS__WAIT,
				   status);
	...
	if (atomic_read(&local->internal_scan_status) ==
	    IEEE80211_INTERNAL_SCAN_STATUS__ABORT)
		return false;
				...
				struct ieee80211_internal_sta_req *sta_req)
{
	...
	bool(__rcu * sta_handle)(struct ieee80211_internal_sta_info * stainfo,
				 void *priv);
	...
	Sstar_common_hash_list_init(Sstar_sta_mac_hlist,
				    SSTAR_COMMON_HASHENTRIES);

	for (index = 0; index < sta_req->n_macs; index++) {
		hash_index = Sstar_hash_index(sta_req->macs[index].mac, 6,
					      SSTAR_COMMON_HASHBITS);

		hhead = &Sstar_sta_mac_hlist[hash_index];
		hlist_add_head(&sta_req->macs[index].hnode,
			       &Sstar_sta_mac_hlist[hash_index]);
	}
	...
	list_for_each_entry_rcu (sta, &local->sta_list, list) {
		struct ieee80211_channel_state *chan_state =
			ieee80211_get_channel_state(local, sta->sdata);
		...
		if (sta_req->n_macs) {
			...
			hash_index = Sstar_hash_index(sta->sta.addr, 6,
						      SSTAR_COMMON_HASHBITS);
			hhead = &Sstar_sta_mac_hlist[hash_index];
			hlist_for_each (node, hhead) {
				mac_node = hlist_entry(
					node, struct ieee80211_internal_mac,
					hnode);
				if (memcmp(mac_node->mac, sta->sta.addr, 6) ==
				    0) {
					sta_needed = true;
					break;
				}
		...
		if (sta_req->req_flag & IEEE80211_INTERNAL_STA_FLAGS_SIGNAL) {
			...
			stainfo.avg_signal =
				(s8)-Sstar_ewma_read(&sta->avg_signal2);
		}
		...
		if (sta_req->req_flag & IEEE80211_INTERNAL_STA_FLAGS_TOPRATE) {
			struct Sstar_common *hw_priv =
				(struct Sstar_common *)local->hw.priv;
			struct Sstar_vif *priv =
				(struct Sstar_vif *)sta->sdata->vif.drv_priv;
			if (sta->sdata->vif.type == NL80211_IFTYPE_STATION) {
				wsm_read_mib(hw_priv, WSM_MIB_ID_GET_RATE,
					     &stainfo.top_rate,
					     sizeof(unsigned int), priv->if_id);
			} else if (sta->sdata->vif.type == NL80211_IFTYPE_AP) {
				struct Sstar_sta_priv *sta_priv =
					(struct Sstar_sta_priv *)&sta->sta
						.drv_priv;
		...
		if (sta_req->req_flag & IEEE80211_INTERNAL_STA_FLAGS_SSID) {
			...
			if (sta->sdata->vif.type == NL80211_IFTYPE_STATION) {
				...
				struct ieee80211_bss_conf *bss_conf =
					&sta->sdata->vif.bss_conf;
				stainfo.ssid_len = bss_conf->ssid_len;
				if (stainfo.ssid_len)
					memcpy(stainfo.ssid, bss_conf->ssid,
					       stainfo.ssid_len);
	...
	struct ieee80211_internal_monitor_req *monitor_req)
{
	...
	rcu_assign_pointer(local->internal_monitor.req.monitor_rx,
			   monitor_req->monitor_rx);
	rcu_assign_pointer(local->internal_monitor.req.priv, monitor_req->priv);

	Sstar_printk_debug("%s:[%s] channel %d\n", __func__, sdata->name,
			   local->internal_monitor.req.ch);
	if (ieee80211_if_change_type(sdata, NL80211_IFTYPE_MONITOR)) {
		res = false;
		goto err;
	}

	if (ieee80211_set_channel(local->hw.wiphy, sdata->dev, chan,
				  monitor_req->chtype)) {
		goto err;
	}
				   ...
				   struct ieee80211_internal_iftype_req *req)
{
	...
	ASSERT_RTNL();
	Sstar_printk_debug("%s:type(%d),channel(%d)\n", __func__, req->if_type,
			   req->channel);
	...
	if (change_channel == true) {
		struct ieee80211_channel *chan = ieee8011_chnum_to_channel(
			&sdata->local->hw, req->channel);
	...
	struct ieee80211_internal_wsm_adaptive *adaptive)
{
	...
	len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_ADAPTIVE "%d",
		       adaptive->enable);
	...
	struct ieee80211_internal_wsm_txpwr_dcxo *txpwr_dcxo)
{
	...
	len = snprintf(cmd, SSTAR_WSM_CMD_LEN,
		       "set_txpwr_and_dcxo,%d,%d,%d,%d ", txpwr_dcxo->txpwr_L,
		       txpwr_dcxo->txpwr_M, txpwr_dcxo->txpwr_H,
		       txpwr_dcxo->dcxo);
			      ...
			      struct ieee80211_internal_wsm_txpwr *txpwr)
{
	...
	*0,3,15,63
	*/
	if (txpwr->txpwr_indx != 0 && txpwr->txpwr_indx != 3 &&
	    txpwr->txpwr_indx != 15 && txpwr->txpwr_indx != 63) {
	...
	len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_TXPWR "%d",
		       txpwr->txpwr_indx);
				 ...
				 struct ieee80211_internal_rate_req *req)
{
	...
	if (req->flags & IEEE80211_INTERNAL_RATE_FLAGS_CLEAR_TX_RATE) {
		...
		if (wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, cmd, len + 1, 0) <
		    0) {
			res = false;
			goto err;
		}
	...
	if (req->flags & IEEE80211_INTERNAL_RATE_FLAGS_SET_TX_RATE) {
		len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_FIX_RATE,
			       req->rate);
	...
	if (req->flags & IEEE80211_INTERNAL_RATE_FLAGS_SET_TOP_RATE) {
		len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_TOP_RATE,
			       req->rate);
	...
	if (req->flags & IEEE80211_INTERNAL_RATE_FLAGS_SET_MIN_RATE) {
		len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_MIN_RATE,
			       req->rate);
...
#define MAX_POWER (16)
	...
	if ((req->rate_index < MIN_RATE_INDEX) ||
	    (req->rate_index > MAX_RATE_INDEX)) {
		ret = false;
		goto exit;
	}
	...
	len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_SET_RATE_POWER,
		       req->rate_index, req->power);
...
#define SSTAR_SPECIAL_FREQ_MAX_LEN 128
static char wifi_freq_buf[SSTAR_SPECIAL_FREQ_MAX_LEN] = { 0 };
...
void Sstar_set_freq(struct ieee80211_local *local)
{
	...
	memset(freq_show, 0, SSTAR_SPECIAL_FREQ_MAX_LEN);

	for (hash_index = 0; hash_index < SSTAR_COMMON_HASHENTRIES;
	     hash_index++) {
		hlist = &local->special_freq_list[hash_index];
		hlist_for_each_safe (node, node_temp, hlist) {
			freq_node = hlist_entry(
				node, struct ieee80211_special_freq, hnode);
			n_freqs++;
			len = scnprintf(freq_show + total_len,
					SSTAR_SPECIAL_FREQ_MAX_LEN - total_len,
					"ch:%d, freq:%d \n",
					channel_hw_value(freq_node->channel),
					freq_node->freq);
			total_len += len;
		}
	}
			     ...
			     struct ieee80211_internal_set_freq_req *req)
{
	...
	len = snprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_SET_FREQ,
		       req->channel_num, (int)req->freq);
	...
	if (channel_center_freq(channel) != req->freq) {
		if (ieee80211_special_freq_update(local, &special_req) ==
		    false) {
			res = false;
			goto out;
		}
...
static bool Sstar_internal_channel_auto_select_results_handle(
	struct ieee80211_hw *hw, struct Sstar_internal_scan_results_req *req,
	struct ieee80211_internal_scan_sta *sta_info)
{
	struct ieee80211_internal_channel_auto_select_results *cca_results =
	...
	*/
	...
	for (index = 0; index < IEEE80211_SSTAR_MAX_SCAN_CHANNEL_INDEX;
	     index++) {
		Sstar_printk_debug("weight[%d]=[%d]\n", index,
				   cca_results->weight[index]);
	}
	return true;
}
...
#define SSTAR_BUSY_RATIO_MIN 100
	...
	results_req.result_handle =
		Sstar_internal_channel_auto_select_results_handle;
	...
	if (results->n_channels) {
		...
		for (i = 0; i < results->n_channels; i++) {
			...
			if (ieee8011_channel_valid(&local->hw,
						   results->channels[i]) ==
			    false) {
				goto err;
			}
	...
	for (band = 0; band < IEEE80211_NUM_BANDS; band++) {
		...
		*/
		for (i = 0; i < sband->n_channels; i++) {
			...
			*/
			if (busy_ratio[channel_hw_value(&sband->channels[i]) -
				       1] == 0) {
				continue;
			}

			if (ignor_channel_mask[channel_hw_value(
						       &sband->channels[i]) -
					       1] == 1) {
				continue;
			}

			if (channel_mask[channel_hw_value(&sband->channels[i]) -
					 1] == 0) {
				continue;
			}
			/*
			...
			*/
			//	Sstar_printk_err("\n");
			//		Sstar_printk_err("channel[%d] min_ap_num [%d]  min_ap_num_ration[%d] min_busy_ratio[%d] \n",channel,min_ap_num,min_ap_num_ration,min_busy_ratio);
			//		Sstar_printk_err("i = %d , busy_ratio[%d] = %d \n",i,channel_hw_value(&sband->channels[i])-1,busy_ratio[channel_hw_value(&sband->channels[i])-1]);

			if (busy_ratio[channel_hw_value(&sband->channels[i]) -
				       1] < SSTAR_BUSY_RATIO_MIN) {
				if (results->weight[channel_hw_value(
							    &sband->channels[i]) -
						    1] <= min_ap_num) {
					if (results->weight
						    [channel_hw_value(
				...
				if (busy_ratio[channel_hw_value(
						       &sband->channels[i]) -
					       1] < min_busy_ratio) {
					min_busy_ratio = busy_ratio
						[channel_hw_value(
							 &sband->channels[i]) -
						 1];
					channel = channel_hw_value(
	...
	if (channel == 0) {
		//WARN_ON(channel == 0);
		Sstar_printk_err("auto select fail! \n");
		goto err;
	}
	Sstar_printk_debug("auto_select channel %d\n", channel);
	memcpy(results->busy_ratio, busy_ratio,
				...
				struct ieee80211_internal_mac_monitor *monitor)
{
	...
	if (monitor->flags & (IEEE80211_INTERNAL_MAC_MONITOR_START |
			      IEEE80211_INTERNAL_MAC_MONITOR_STOP)) {
		Sstar_printk_err("mac_monitor:enable(%d),mac[%pM]\n", __func__,
				 !!(monitor->flags &
				    IEEE80211_INTERNAL_MAC_MONITOR_START),
				 monitor->mac);
		len = scnprintf(cmd, SSTAR_WSM_CMD_LEN, SSTAR_WSM_MONITOR_MAC,
				monitor->index,
				!!(monitor->flags &
				   IEEE80211_INTERNAL_MAC_MONITOR_START),
	...
	if (monitor->flags & IEEE80211_INTERNAL_MAC_MONITOR_RESULTS) {
		int i = 0;
		if (wsm_read_mib(hw_priv, WSM_MIB_ID_GET_MONITOR_MAC_STATUS,
				 cmd, SSTAR_WSM_CMD_LEN, 0) != 0) {
			ret = false;
			goto exit;
		}
		...
		for (i = 0; i < IEEE80211_INTERNAL_MAC_MONITOR_RESULTS; i++) {
			...
			memcpy(monitor->reults[i].mac, cmd, 6);
			cmd += 6;
			monitor->reults[i].delta_time =
				__le32_to_cpu(*((u32 *)cmd));
			cmd += 4;
				     ...
				     struct ieee80211_internal_req_chip *req)
{
	...
	if (req->flags & IEEE80211_INTERNAL_REQ_CHIP_FLAGS__CHIP_VER) {
		...
		} else if (hw_priv->wsm_caps.firmwareCap &
			   CAPABILITIES_EFUSEI) {
		} else if (hw_priv->wsm_caps.firmwareCap &
			   CAPABILITIES_EFUSEB) {
				   ...
				   bool clear)
{
	...
	if (conf_req && conf_req->channel) {
		if (ieee8011_channel_valid(&sdata->local->hw,
					   (int)conf_req->channel) == false) {
			goto err;
		}
	}
...
#endif
int Sstar_internal_addr_read_bit(struct Sstar_common *hw_priv, u32 addr,
				 u8 endBit, u8 startBit, u32 *data)
{
	...
	ret = Sstar_direct_read_reg_32(hw_priv, addr, &reg_val);
	if (ret < 0) {
		goto rw_end;
	}
...
int Sstar_internal_addr_write_bit(struct Sstar_common *hw_priv, u32 addr,
				  u8 endBit, u8 startBit, u32 data)
{
	u32 reg_val = 0;
	u32 regmask = 0;
	...
	if (ret < 0) {
		Sstar_printk_err("%s:read err\n", __func__);
		goto rw_end;
	}
	Sstar_printk_err("%s:ret(%d)\n", __func__, ret);
	regmask = ~((1 << startBit) - 1);
	regmask &= ((1 << endBit) - 1) | (1 << endBit);
	reg_val &= ~regmask;
	reg_val |= (data << startBit) & regmask;
	ret = Sstar_direct_write_reg_32(hw_priv, addr, reg_val);

	if (ret < 0) {
		Sstar_printk_err("%s:write err\n", __func__);
		goto rw_end;
	}
...
static int Sstar_internal_gpio_set(struct Sstar_common *hw_priv,
				   struct Sstar_ctr_addr *gpio_addr)
{
	unsigned int status = -1;

	if (Sstar_bh_is_term(hw_priv)) {
		Sstar_printk_err("%s:Sstar term\n", __func__);
		goto exit;
	}

	status = Sstar_internal_addr_write_bit(
		hw_priv, gpio_addr->base_addr,
		gpio_addr->start_bit + gpio_addr->width, gpio_addr->start_bit,
		gpio_addr->val);
exit:
	return status;
}

static int Sstar_internal_gpio_get(struct Sstar_common *hw_priv,
				   struct Sstar_ctr_addr *gpio_addr)
{
	unsigned int status = -1;
	...
	status = Sstar_internal_addr_read_bit(
		hw_priv, gpio_addr->base_addr,
		gpio_addr->start_bit + gpio_addr->width - 1,
		gpio_addr->start_bit, &gpio_addr->val);
exit:
	return status;
}

static struct Sstar_gpio_config *
Sstar_internal_gpio_reqest(struct Sstar_common *hw_priv, int gpio)
{
	int i = 0;
	struct Sstar_gpio_config *gpio_dev = NULL;

	for (i = 0; i < ARRAY_SIZE(Sstar_gpio_table); i++) {
		gpio_dev = &Sstar_gpio_table[i];
				...
				bool dir, bool pu, bool default_val)
{
	...
	gpio_dev = Sstar_internal_gpio_reqest(hw_priv, gpio);

	if (gpio_dev == NULL) {
		Sstar_printk_err("%s:gpio (%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}
	...
	status = Sstar_internal_gpio_set(hw_priv, &gpio_dev->fun_ctrl);

	if (status) {
		Sstar_printk_err("%s:gpio function(%d)(%d) is err\n", __func__,
				 gpio, status);
		ret = false;
		goto exit;
	}

	status = Sstar_internal_gpio_set(hw_priv, &gpio_dev->dir_ctrl);

	if (status) {
		Sstar_printk_err("%s:gpio dir(%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}

	status = Sstar_internal_gpio_set(hw_priv, &gpio_dev->pup_ctrl);

	if (status) {
		Sstar_printk_err("%s:gpio pup(%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}
	...
	if (status) {
		Sstar_printk_err("%s:gpio pdu(%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}
	...
	if (dir == true) {
		...
		status = Sstar_internal_gpio_set(hw_priv, &gpio_dev->out_val);
		if (status) {
			Sstar_printk_err("%s:gpio out(%d) is err\n", __func__,
					 gpio);
			ret = false;
			goto exit;
		}
	...
	gpio_dev->flags = SSTAR_GPIO_CONFIG__FUNCTION_CONFIGD;

	if (dir == true)
		gpio_dev->flags |= SSTAR_GPIO_CONFIG__OUTPUT;
	else
		gpio_dev->flags |= SSTAR_GPIO_CONFIG__INPUT;

	if (pu)
		gpio_dev->flags |= SSTAR_GPIO_CONFIG__PUP;
	else
		gpio_dev->flags |= SSTAR_GPIO_CONFIG__PUD;
exit:
	return ret;
}
				...
				bool set)
{
	...
	if (!(gpio_dev->flags & SSTAR_GPIO_CONFIG__FUNCTION_CONFIGD)) {
		Sstar_printk_err("%s:gpio (%d) is not configed\n", __func__,
				 gpio);
		ret = false;
		goto exit;
	}

	if (!(gpio_dev->flags & SSTAR_GPIO_CONFIG__OUTPUT)) {
		Sstar_printk_err("%s:gpio (%d) is not output mode\n", __func__,
				 gpio);
		ret = false;
		goto exit;
	}
	...
	if (Sstar_internal_gpio_set(hw_priv, &gpio_dev->out_val)) {
		Sstar_printk_err("%s:gpio out(%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}
			       ...
			       bool *set)
{
	...
	if (!(gpio_dev->flags & SSTAR_GPIO_CONFIG__INPUT)) {
		Sstar_printk_err("%s:gpio (%d) is not input mode\n", __func__,
				 gpio);
		ret = false;
		goto exit;
	}

	if (Sstar_internal_gpio_get(hw_priv, &gpio_dev->in_val)) {
		Sstar_printk_err("%s:gpio out(%d) is err\n", __func__, gpio);
		ret = false;
		goto exit;
	}
				...
				int aifs, int cw_win, int cw_max, int txop)
{
	...
	if (atomic_read(&priv->enabled) == 0) {
		Sstar_printk_err("%s:disabled\n", __func__);
		goto exit;
	}

	WSM_EDCA_SET(&priv->edca, queue, aifs, cw_win, cw_max, txop, 0xc8,
		     priv->edca.params[queue].uapsdEnable);
	ret = wsm_set_edca_params(priv->hw_priv, &priv->edca, priv->if_id);
	if (ret) {
		Sstar_printk_err("%s:wsm_set_edca_params\n", __func__);
		goto exit;
	}
...
{
	if (recv_ie) {
		if (memcmp(recv_ie, &private_ie,
			   sizeof(struct Sstar_vendor_cfg_ie))) {
			memcpy(&private_ie, recv_ie,
			       sizeof(struct Sstar_vendor_cfg_ie));
		}
	}
	return 0;
...
{
	struct Sstar_vendor_cfg_ie ie;
	memset(&ie, 0, sizeof(struct Sstar_vendor_cfg_ie));
	if (memcmp(&ie, &private_ie, sizeof(struct Sstar_vendor_cfg_ie)) == 0)
		return NULL;
