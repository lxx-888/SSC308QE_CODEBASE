...
#define bh_printk(...)
#endif
static int spi_Sstar_bh(void *arg);
...
int Sstar_register_bh(struct Sstar_common *hw_priv)
{
	...
	hw_priv->bh_thread = kthread_create(&spi_Sstar_bh, hw_priv, ieee80211_alloc_name(hw_priv->hw,"spi_Sstar_bh"));
...
int Sstar_spi_rx_bh_cb(struct Sstar_common *hw_priv,struct sk_buff *skb)
{
	struct wsm_hdr *wsm;
	u32 wsm_len;
	int wsm_id;
	...
	if (WARN_ON(wsm_seq != hw_priv->wsm_rx_seq)) {
		Sstar_printk_err("%s wsm_seq error %d %d \n",__func__,wsm_seq,hw_priv->wsm_rx_seq);
		goto __free;
	}
...
static struct sk_buff *Sstar_spi_get_skb(struct Sstar_common *hw_priv, u32 len)
{
	struct sk_buff *skb;
	u32 alloc_len = (len > SPI_READ_BLOCK_SIZE) ? len : SPI_READ_BLOCK_SIZE;

	skb = Sstar_dev_alloc_skb(alloc_len
...
static void Sstar_spi_put_skb(struct Sstar_common *hw_priv, struct sk_buff *skb)
{
	Sstar_dev_kfree_skb(skb);
}


int Sstar_spi_rx_bh(struct Sstar_common *hw_priv)
{
	...
	//printk("Sstar_spi_rx_bh start\n");
	ret = Sstar_read_status_ready(hw_priv, &ready);
...
nextfrag:
	ret = Sstar_read_status(hw_priv, &status);
	if ((ret != 0))//rx_ready
	{
		Sstar_printk_err( "[BH] read status rx not ready\n");
		goto out;
	}
	bh_printk( "%s %d\n", __func__, __LINE__);
	...
	if (WARN_ON(read_len < sizeof(struct wsm_hdr))) {
		Sstar_printk_err( "Invalid read len: %d",
			read_len);
		goto out;
	}
	if (totalLen == 0)
	{
		...
		skb = Sstar_spi_get_skb(hw_priv, alloc_len);
		if (WARN_ON(!skb))
			goto error;

		Sstar_skb_trim(skb, 0);
		Sstar_skb_put(skb, read_len);
		data = skb->data;
		if (WARN_ON(!data))
			goto error;
	...
	//printk("Sstar_spi_rx_bh start read_len:%d\n", read_len);
	if (WARN_ON(Sstar_read_data(hw_priv, data + totalLen, read_len)))
		goto error;
	if (totalLen == 0)
	{
	...
	Sstar_printk_bus("Sstar_spi_rx_bh start wsm_len:%d, read_len:%d\n", wsm_len, read_len);
{
	int i;
	for (i = 0; i < 28; i++)
	{
		Sstar_printk_bus("%02x ", data[i]);
	}
	Sstar_printk_bus("\n");
}
...
readyflagchange:
		...
		if (ret !=0)
		{
			Sstar_printk_bus( "[BH]RX SPI read status error.\n");
			goto error;
		}
		if (ready == 0)
		{
			Sstar_printk_bus("Sstar_spi_rx_bh start readyflagchange cout=%d\n", count);
			msleep(100);
			count++;
			if (count > 100)
			{
				Sstar_printk_err("[BH]RX SPI read status ready count > 100 error.\n");
				goto error;
			}
		...
		{
			msleep(600);
			Sstar_printk_bus("Sstar_spi_rx_bh start nextfrag\n");
			goto nextfrag;
		}
	}
	//printk("Sstar_spi_rx_bh_cb start \n");

	Sstar_spi_rx_bh_cb(hw_priv,skb);
out:
	return 0;
error:
	if (!skb)
		Sstar_spi_put_skb(hw_priv,skb);
	return 0;
}

static int Sstar_spi_xmit_data(struct Sstar_common *hw_priv)
{
	int status=0;
	int tx_burst=0;
	int vif_selected;
	...
	ret = Sstar_read_status_channelflag(hw_priv, &chanflag);
	if ((ret !=0) || (chanflag == 0))
	{
		goto error;
	}
...
nextflag:
		...
		ret = Sstar_update_status_channelflag(hw_priv);
		if ((ret !=0))
		{
			wsm_release_tx_buffer(hw_priv, 1);
			status=-4;
		...
		if (WARN_ON(Sstar_write_data(hw_priv,data + totalLen, SPI_WRITE_BLOCK_SIZE))) {
			wsm_release_tx_buffer(hw_priv, 1);
			goto error;
		}
		{
...
flagchange:
			...
			if ((ret !=0))
			{
				Sstar_printk_bus( "[BH] SPI read_status_channelflag error.\n");
				wsm_release_tx_buffer(hw_priv, 1);
				status=-5;
				goto error;
			}
			if (chanflag == 0)
			{
				...
				if (count > 10)
				{
					wsm_release_tx_buffer(hw_priv, 1);
					status=-6;
					Sstar_printk_bus( "[BH]TX SPI channelflag count > 10 error.\n");
					goto error;
				}
				msleep(600);
				goto flagchange;
...
static int spi_Sstar_bh(void *arg)
{
	...
	if (status <= 0 || term || hw_priv->bh_error){
		Sstar_printk_bus("%s BH thread break %ld %d %d\n",__func__,status,term,hw_priv->bh_error);
		goto out;
	}
...
#define __ALL_HW_BUFS_USED (hw_priv->hw_bufs_used)
	...
	while (1) {
		...
		//printk("spi_Sstar_bh Sstar_spi_read_rdy_start++\n");
		Sstar_spi_read_rdy_start();

		//printk("spi_Sstar_bh Sstar_spi_read_rdy_start 2 ++%d\n",status/HZ);
		status = wait_event_interruptible_timeout(hw_priv->bh_wq, ({
		...
		Sstar_spi_read_rdy_end();
		//printk("spi_Sstar_bh Sstar_spi_read_rdy_start end\n");
		...
		if (!status && __ALL_HW_BUFS_USED)
		{
			...
			if (pending && timeout < 0) {
				...
				prink_test++;
				if(prink_test <10){
					Sstar_printk_bus("Sstar_bh: Timeout waiting for TX confirm. hw_bufs_used %d\n",hw_priv->hw_bufs_used);
				}
				//break;
			}
		} else if (!status) {
			Sstar_printk_bus("%s %d ++ !!\n",__func__,__LINE__);
			if (!hw_priv->device_can_sleep
					&& !atomic_read(&hw_priv->recent_scan)) {
			...
			Sstar_printk_bus( "[BH] Device suspend.\n");
			Sstar_printk_bus("%s %d ++ !!\n",__func__,__LINE__);
			powersave_enabled = 1;
			Sstar_hw_vif_read_lock(&hw_priv->vif_list_lock);
			Sstar_for_each_vif_safe(hw_priv, priv, i) {
			...
			Sstar_printk_bus( "[BH] Device resume.\n");
			atomic_set(&hw_priv->bh_suspend, SSTAR_APOLLO_BH_RESUMED);
			//wake_up(&hw_priv->bh_evt_wq);
		...
		if (rx){
			Sstar_spi_rx_bh(hw_priv);
		}
		if (tx){
			Sstar_spi_xmit_data(hw_priv);
	    }
	}
out:
	if (!term) {
	...
	Sstar_printk_bus("Sstar_wifi_BH_thread stop ++\n");
	/*
