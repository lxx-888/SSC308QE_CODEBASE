...
#define ap_printk(...) Sstar_printk_always(__VA_ARGS__)
#else
#define ap_printk(...)
#endif
...
#define SSTAR_APOLLO_LINK_ID_GC_TIMEOUT ((unsigned long)(10 * HZ))

#define SSTAR_APOLLO_ENABLE_ARP_FILTER_OFFLOAD 3
/*For Samsung, it is defined as 4*/
#define SSTAR_APOLLO_KEEP_ALIVE_PERIOD (4)
...
#define SSTAR_APOLLO_ENABLE_NDP_FILTER_OFFLOAD 3
#endif /*IPV6_FILTERING*/

static int Sstar_upload_beacon(struct Sstar_vif *priv);
#ifdef SSTAR_PROBE_RESP_EXTRA_IE
static int Sstar_upload_proberesp(struct Sstar_vif *priv);
#endif
#ifdef CONFIG_SSTAR_BT_COMB
static int Sstar_upload_pspoll(struct Sstar_vif *priv);
static int Sstar_upload_null(struct Sstar_vif *priv);
#endif
static int Sstar_upload_qosnull(struct Sstar_vif *priv);
static int Sstar_start_ap(struct Sstar_vif *priv);
static int Sstar_update_beaconing(struct Sstar_vif *priv);
/*
static int Sstar_enable_beaconing(struct Sstar_vif *priv,
				   bool enable);
*/
static void __Sstar_sta_notify(struct Sstar_vif *priv,
			       enum sta_notify_cmd notify_cmd, int link_id);

/* ******************************************************************** */
/* AP API								*/
static int __Sstar_find_link_id(struct Sstar_vif *priv, const u8 *mac)
{
	int i, ret = 0;
		  ...
		  struct ieee80211_sta *sta)
{
	struct Sstar_sta_priv *sta_priv =
		(struct Sstar_sta_priv *)&sta->drv_priv;
	struct Sstar_vif *priv = ABwifi_get_vif_from_ieee80211(vif);
	struct Sstar_link_entry *entry;
	struct sk_buff *skb;
	...
	if (priv->mode != NL80211_IFTYPE_AP)
		return 0;
	Sstar_printk_err("sta(%pM) add\n", sta->addr);
	sta_priv->priv = priv;
	sta_priv->link_id = __Sstar_find_link_id(priv, sta->addr);
	if (!sta_priv->link_id) {
		/* Impossible error */
		sta_priv->link_id = Sstar_alloc_link_id(priv, sta->addr);
		if (sta_priv->link_id) {
			Sstar_printk_err("realloc link id(%d) mac[%pM]\n",
	...
	entry->status = SSTAR_APOLLO_LINK_HARD;
	entry->timestamp = jiffies;
	while ((skb = Sstar_skb_dequeue(&entry->rx_queue)))
		Sstar_ieee80211_rx(priv->hw, skb);
	spin_unlock_bh(&priv->ps_state_lock);
	hw_priv->connected_sta_cnt++;
#ifdef CONFIG_SSTAR_AP_SUPPORT_ONE_AMPDU
	if (hw_priv->connected_sta_cnt > 1) {
		wsm_lock_tx(hw_priv);
		WARN_ON(wsm_set_block_ack_policy(
			hw_priv, SSTAR_APOLLO_TX_BLOCK_ACK_DISABLED_FOR_ALL_TID,
			SSTAR_APOLLO_RX_BLOCK_ACK_DISABLED_FOR_ALL_TID,
			priv->if_id));
		     ...
		     struct ieee80211_sta *sta)
{
	...
	if (priv->mode != NL80211_IFTYPE_AP || !sta_priv->link_id)
		return 0;
	Sstar_printk_err("sta(%pM) remove\n", sta->addr);
	entry = &priv->link_id_db[sta_priv->link_id - 1];
#ifdef SSTAR_PKG_REORDER
	Sstar_reorder_func_reset(priv, sta_priv->link_id - 1);
#endif
	/*
	...
	*/
	...
	entry->status = SSTAR_APOLLO_LINK_SOFT;
	smp_mb();
	entry->timestamp = jiffies - SSTAR_APOLLO_LINK_ID_GC_TIMEOUT -
			   msecs_to_jiffies(10);
	//}else {
	//	entry->status = SSTAR_APOLLO_LINK_RESERVE;
	//	smp_mb();
	//	entry->timestamp = jiffies;
	//	}
	wsm_lock_tx_async(hw_priv);
	if (Sstar_hw_priv_queue_work(hw_priv, &priv->link_id_work) <= 0)
		wsm_unlock_tx(hw_priv);
	spin_unlock_bh(&priv->ps_state_lock);
	Sstar_flush_workqueue(hw_priv->workqueue);
	hw_priv->connected_sta_cnt--;
	...
	if (hw_priv->connected_sta_cnt <= 1) {
		if ((priv->if_id != 1) || ((priv->if_id == 1)
			...
			WARN_ON(wsm_set_block_ack_policy(
				hw_priv,
				SSTAR_APOLLO_TX_BLOCK_ACK_ENABLED_FOR_ALL_TID,
				SSTAR_APOLLO_RX_BLOCK_ACK_ENABLED_FOR_ALL_TID,
				priv->if_id));
			wsm_unlock_tx(hw_priv);
		}
	}
		...
		if (!prev) {
			if (priv->buffered_multicasts && !priv->sta_asleep_mask)
				Sstar_hw_priv_queue_work(
					hw_priv, &priv->multicast_start_work);
	...
	case STA_NOTIFY_AWAKE:
		if (prev) {
			...
			if (priv->tx_multicast && link_id &&
				...
				Sstar_hw_priv_queue_work(
					hw_priv, &priv->multicast_stop_work);
...
#endif
	spin_lock_bh(&priv->ps_state_lock);
	__Sstar_sta_notify(priv, notify_cmd, sta_priv->link_id);
	spin_unlock_bh(&priv->ps_state_lock);
}

static void Sstar_ps_notify(struct Sstar_vif *priv, int link_id, bool ps)
{
	if (link_id > SSTARWIFI_MAX_STA_IN_AP_MODE)
		return;

	txrx_printk("%s for LinkId: %d. STAs asleep: %.8X\n",
		    ps ? "Stop" : "Start", link_id, priv->sta_asleep_mask);

	/* TODO:COMBO: __Sstar_sta_notify changed. */
	__Sstar_sta_notify(priv, ps ? STA_NOTIFY_SLEEP : STA_NOTIFY_AWAKE,
			   link_id);
}

static int Sstar_set_tim_impl(struct Sstar_vif *priv, bool aid0_bit_set)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	struct sk_buff *skb;
...
#endif
	skb = ieee80211_beacon_get_tim(priv->hw, priv->vif, &tim_offset,
				       &tim_length);
	if (!skb) {
		Sstar_printk_err("%s : if_id[%d],if_name[%s] alloc skb err \n",
				 __func__, priv->if_id,
				 vif_to_sdata(priv->vif)->name);
#if 0
		if (!__Sstar_flush(hw_priv, false, priv->if_id))
			wsm_unlock_tx(hw_priv);
#endif
		return -ENOENT;
	}
...
void Sstar_set_tim_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, set_tim_work);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		return;
	}
	(void)Sstar_set_tim_impl(priv, priv->aid0_bit_set);
}

int Sstar_set_tim(struct ieee80211_hw *dev, struct ieee80211_sta *sta, bool set)
{
	struct Sstar_sta_priv *sta_priv =
		(struct Sstar_sta_priv *)&sta->drv_priv;
	struct Sstar_vif *priv = sta_priv->priv;
	...
	Sstar_hw_priv_queue_work(priv->hw_priv, &priv->set_tim_work);
	return 0;
}
#if 0
void Sstar_set_cts_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, set_cts_work.work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);

	u8 erp_ie[3] = {SSTAR_WLAN_EID_ERP_INFO, 0x1, 0};
	struct wsm_update_ie update_ie = {
		.what = WSM_UPDATE_IE_BEACON,
		.count = 1,
		.ies = erp_ie,
	...
	{
		extern int Sstar_tool_use_cts_prot;
		Sstar_tool_use_cts_prot = use_cts_prot;
	}
#endif
...
#endif
#ifdef CONFIG_SSTAR_BT_COMB
static int Sstar_set_btcoexinfo(struct Sstar_vif *priv)
{
	...
	if (priv->mode == NL80211_IFTYPE_STATION) {
		...
		WARN_ON(Sstar_upload_pspoll(priv));
		WARN_ON(Sstar_upload_null(priv));
	} else {
		return 0;
	}
	...
	ap_printk("[STA] BTCOEX_INFO"
		  ...
		  priv->mode, arg.internalTxRate, arg.nonErpInternalTxRate);

	ret = WARN_ON(wsm_write_mib(ABwifi_vifpriv_to_hwpriv(priv),
				    WSM_MIB_ID_OVERRIDE_INTERNAL_TX_RATE, &arg,
				    sizeof(arg), priv->if_id));

	return ret;
}
#endif
#ifdef CONFIG_SSTAR_SUPPORT_IBSS
void Sstar_ibss_join_work(struct Sstar_vif *priv)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	const u8 *bssid;
	...
	bss = ieee80211_Sstar_get_bss(hw_priv->hw->wiphy, hw_priv->channel,
				      bssid, NULL, 0, 0, 0);
	if (!bss) {
		wsm_unlock_tx(hw_priv);
		return;
	...
	if (priv->tmpframe_probereq_set == 0) {
		...
		Sstar_printk_ap(
			"<Sstar_WIFI>because not scan before join, probeForJoin =0 hw_value %d\n",
	...
	if (priv->vif->p2p) {
		...
		join.flags |= (1 << 6);
#endif
		join.basicRateSet = Sstar_rate_mask_to_wsm(hw_priv, 0xFF0);
	}
#endif
#ifdef SSTAR_SUPPORT_WIDTH_40M
	if (priv->if_id && (priv->vif->p2p == true)) {
		join.channel_type =
			(u32)(hw_priv->channel_type > NL80211_CHAN_NO_HT ?
					    NL80211_CHAN_HT20 :
					    NL80211_CHAN_NO_HT);
	} else {
	...
	Sstar_hw_priv_queue_delayed_work(hw_priv, &priv->join_timeout, 3 * HZ);
#ifdef CONFIG_PM
	/*Stay Awake for Join Timeout*/
	Sstar_pm_stay_awake(&hw_priv->pm_state, 3 * HZ);
#endif
#if define(CONFIG_SSTAR_STA_LISTEN) || defined(CONFIG_SSTAR_SUPPORT_P2P)
	...
	WARN_ON(wsm_set_block_ack_policy(hw_priv,
					 ...
					 hw_priv->ba_tid_rx_mask, priv->if_id));
	...
	if (wsm_join(hw_priv, &join, priv->if_id)) {
		...
		/* Upload keys */
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_IBSS;
		Sstar_upload_keys(priv);
	...
	ieee80211_Sstar_put_bss(hw_priv->hw->wiphy, bss);
	wsm_unlock_tx(hw_priv);
}
#endif
void Sstar_bss_info_changed(struct ieee80211_hw *dev, struct ieee80211_vif *vif,
			    struct ieee80211_bss_conf *info, u32 changed)
{
	...
	if (changed & BSS_CHANGED_STA_RESTART) {
		...
		if (sdata->u.mgd.associated) {
			Sstar_restart_join_bss(priv, sdata->u.mgd.associated);
		}
	}
	...
	if (changed & BSS_CHANGED_ARP_FILTER) {
		...
		if (info->arp_filter_enabled) {
			if (vif->type == NL80211_IFTYPE_STATION)
				filter.enable = (u32)
					SSTAR_APOLLO_ENABLE_ARP_FILTER_OFFLOAD;
		...
		Sstar_printk_ap("[STA] arp ip filter enable: %d\n",
				__le32_to_cpu(filter.enable));
		...
		if (priv->filter4.enable &&
...
#ifdef SSTAR_WIFI_NO_P2P_PS
				if (priv->vif->p2p)
					priv->powersave_mode.pmMode =
						WSM_PSM_ACTIVE;
#endif
				priv->powersave_mode.pmMode = WSM_PSM_ACTIVE;
				Sstar_printk_err("%s %d ,pmMode = %d \n",
						 __func__, __LINE__,
						 priv->powersave_mode.pmMode);
			...
			priv->powersave_mode.pmMode = WSM_PSM_ACTIVE;
			Sstar_printk_err("%s %d : pmMode(%d) \n", __func__,
					 __LINE__, priv->powersave_mode.pmMode);
		}
	}
#ifdef CONFIG_SSTAR_LMAC_FILTER_IP_FRAME
#ifdef IPV6_FILTERING
	if (changed & BSS_CHANGED_NDP_FILTER) {
		...
		if (info->ndp_filter_enabled) {
			if (vif->type == NL80211_IFTYPE_STATION)
				filter.enable = (u32)
					SSTAR_APOLLO_ENABLE_NDP_FILTER_OFFLOAD;
		...
		if (filter.enable && (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA) && (priv->vif->p2p) &&
...
#endif
		WARN_ON(Sstar_upload_beacon(priv));
		WARN_ON(Sstar_update_beaconing(priv));
	}
#ifdef CONFIG_SSTAR_SUPPORT_IBSS
	if (changed & BSS_CHANGED_IBSS) {
		priv->beacon_int = info->beacon_int;
		Sstar_upload_beacon(priv);
		WARN_ON(Sstar_update_beaconing(priv));
	}
#endif
...
#endif
	if (changed & (BSS_CHANGED_ASSOC | BSS_CHANGED_BASIC_RATES |
		       BSS_CHANGED_ERP_PREAMBLE | BSS_CHANGED_HT |
		       BSS_CHANGED_ERP_SLOT | BSS_CHANGED_IBSS)) {
		int is_combo = 0;
		int i;
		struct Sstar_vif *tmp_priv;
		Sstar_printk_ap("BSS_CHANGED_ASSOC.changed(%x)\n", changed);
		if (info->assoc || info->ibss_joined) { /* TODO: ibss_joined */
			...
			mutex_unlock(&hw_priv->conf_mutex);
			Sstar_hw_cancel_delayed_work(&priv->join_timeout,
						     false);
			mutex_lock(&hw_priv->conf_mutex);
			...
			if (sta) {
				...
				* mac80211 changes are available */
				...
				priv->bss_params.operationalRateSet =
					__cpu_to_le32(Sstar_rate_mask_to_wsm(
						hw_priv,
						sta->supp_rates[hw_priv->channel
									->band]));
			...
			rcu_read_unlock();
			priv->htcap = (sta && Sstar_is_ht(&hw_priv->ht_info));
			Sstar_for_each_vif(hw_priv, tmp_priv, i)
			{
#ifdef P2P_MULTIVIF
				if ((i == (SSTAR_WIFI_MAX_VIFS - 1)) ||
				    !tmp_priv)
#else
				if (!tmp_priv)
#endif
			...
			if (is_combo > 1) {
				...
				Sstar_printk_ap("ASSOC is_combo %d\n",
						hw_priv->vif0_throttle);
			} else if ((priv->join_status ==
				    SSTAR_APOLLO_JOIN_STATUS_STA) &&
				   priv->htcap) {
				...
				hw_priv->vif1_throttle =
					SSTAR_WIFI_HOST_VIF1_11BG_THROTTLE;
				Sstar_printk_ap("ASSOC not_combo 11BG %d\n",
						hw_priv->vif0_throttle);
			}
			...
			priv->association_mode.greenfieldMode =
				Sstar_ht_greenfield(&hw_priv->ht_info);
...
#if defined(CONFIG_NL80211_TESTMODE) || defined(CONFIG_SSTAR_IOCTRL)
			{
				extern int Sstar_tool_use_short_preamble;
			...
			priv->association_mode.mpduStartSpacing =
				Sstar_ht_ampdu_density(&hw_priv->ht_info);
			...
			Sstar_hw_cancel_delayed_work(
				&priv->connection_loss_work, false);
#endif
			...
			WARN_ON(wsm_set_association_mode(
				hw_priv, &priv->association_mode, priv->if_id));
			if (!info->ibss_joined) {
				WARN_ON(wsm_keep_alive_period(
					hw_priv,
					SSTAR_APOLLO_KEEP_ALIVE_PERIOD /* sec */,
					priv->if_id));
...
#ifdef CONFIG_SSTAR_SUPPORT_P2P
			if (priv->vif->p2p) {
				ap_printk("[STA] Setting p2p powersave "
					  "configuration.\n");
				WARN_ON(wsm_set_p2p_ps_modeinfo(
					hw_priv, &priv->p2p_ps_modeinfo,
					priv->if_id));
				Sstar_notify_noa(
					priv,
					SSTAR_APOLLO_NOA_NOTIFICATION_DELAY);
			}
#endif
			if (priv->mode == NL80211_IFTYPE_STATION)
				WARN_ON(Sstar_upload_qosnull(priv));
#ifdef CONFIG_SSTAR_BT_COMB
			if (hw_priv->is_BT_Present)
				WARN_ON(Sstar_set_btcoexinfo(priv));
#endif
		} else {
		...
		if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_AP) {
			...
			if (prev_erp_info != priv->erp_info) {
...
#endif
				u8 erp_ie[3] = { SSTAR_WLAN_EID_ERP_INFO, 0x1,
						 0 };
				struct wsm_update_ie update_ie = {
					.what = WSM_UPDATE_IE_BEACON,
					.count = 1,
				...
				WARN_ON(wsm_write_mib(
					hw_priv, WSM_MIB_ID_NON_ERP_PROTECTION,
					&use_cts_prot, sizeof(use_cts_prot),
					priv->if_id));
#if defined(CONFIG_NL80211_TESTMODE) || defined(CONFIG_SSTAR_IOCTRL)
				{
...
#endif
				WARN_ON(wsm_update_ie(hw_priv, &update_ie,
						      priv->if_id));
			}
		}
	}
	...
	if (changed & (BSS_CHANGED_ASSOC | BSS_CHANGED_ERP_SLOT)) {
		__le32 slot_time = info->use_short_slot ? __cpu_to_le32(9) :
								__cpu_to_le32(20);
		...
		Sstar_printk_ap("[STA] Slot time :%d us.\n",
				__le32_to_cpu(slot_time));
...
#if defined(CONFIG_NL80211_TESTMODE) || defined(CONFIG_SSTAR_IOCTRL)
		{
			extern int Sstar_tool_use_short_slot;
	...
	if (changed & BSS_CHANGED_P2P_PS) {
		...
		Sstar_printk_ap("[AP] Opportunistic: %d\n",
				info->p2p_ps.opp_ps);
		...
		Sstar_printk_ap("[AP] NOA: %d, %d, %d, %d\n",
...
#define NOA_DELAY_START_MS 300
		 ...
		 * notification yet */
		Sstar_notify_noa(priv, 10);
	}
#endif
	mutex_unlock(&hw_priv->conf_mutex);
}

void Sstar_multicast_start_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, multicast_start_work);
	long tmo = priv->join_dtim_period * (priv->beacon_int + 20) * HZ / 1024;

	Sstar_hw_cancel_queue_work(&priv->multicast_stop_work, true);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		return;
	}
	if (!priv->aid0_bit_set) {
		wsm_lock_tx(priv->hw_priv);
		Sstar_set_tim_impl(priv, true);
		priv->aid0_bit_set = true;
		Sstar_mod_timer(&priv->mcast_timeout, jiffies + tmo);
		wsm_unlock_tx(priv->hw_priv);
	}
}

void Sstar_multicast_stop_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, multicast_stop_work);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		return;
	}
	if (priv->aid0_bit_set) {
...
{
	struct Sstar_vif *priv = (struct Sstar_vif *)arg;

	Sstar_printk_warn("Multicast delivery timeout.\n");
	spin_lock_bh(&priv->ps_state_lock);
	priv->tx_multicast = priv->aid0_bit_set && priv->buffered_multicasts;
	if (priv->tx_multicast)
		Sstar_bh_wakeup(ABwifi_vifpriv_to_hwpriv(priv));
	spin_unlock_bh(&priv->ps_state_lock);
}

int Sstar_ampdu_action(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
...
/* Firmware sends this indication every DTIM if there
...
#ifdef CONFIG_PM
			Sstar_pm_stay_awake(&hw_priv->pm_state,
					    priv->join_dtim_period *
						    (priv->beacon_int + 20) *
						    HZ / 1024);
#endif
		...
		spin_lock_bh(&priv->ps_state_lock);
		Sstar_ps_notify(priv, arg->link_id, arg->stop);
		spin_unlock_bh(&priv->ps_state_lock);
		if (!arg->stop)
			Sstar_bh_wakeup(hw_priv);
	}
...
static int Sstar_upload_beacon(struct Sstar_vif *priv)
{
...
#ifdef CONFIG_SSTAR_AP_CHANNEL_CHANGE_EVENT
	u8 *ds_params;
#endif
	u32 ies_len;
	...
	if (!frame.skb) {
		Sstar_printk_err("upload_beacon NULL\n");
		return -ENOMEM;
	}
...
#ifdef SSTAR_SUPPORT_WIDTH_40M
	//#ifdef P2P_MULTIVIF
	Sstar_printk_ap("Sstar_clear_wpas_p2p_40M_ie\n");
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	if (priv->if_id && (priv->vif->p2p == true))
...
//#endif
#endif
	ht_info = (u8 *)Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_HT_INFORMATION,
						ies, ies_len);
	if (ht_info) {
		/* Enable RIFS*/
		ht_info[3] |= 8;
	}
	erp_inf = (u8 *)Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_ERP_INFO, ies,
						ies_len);
	if (erp_inf) {
		if (erp_inf[ERP_INFO_BYTE_OFFSET] & WLAN_ERP_BARKER_PREAMBLE)
			priv->erp_info |= WLAN_ERP_BARKER_PREAMBLE;
		else
			priv->erp_info &= ~WLAN_ERP_BARKER_PREAMBLE;

		if (erp_inf[ERP_INFO_BYTE_OFFSET] & WLAN_ERP_NON_ERP_PRESENT) {
			priv->erp_info |= WLAN_ERP_USE_PROTECTION;
	...
	ds_params = (u8 *)Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_DS_PARAMS, ies,
						  ies_len);
	if (ds_params) {
		ds_params[2] = channel_hw_value(hw_priv->channel);
		Sstar_printk_err("change ds_params channel %d \n",
				 channel_hw_value(hw_priv->channel));
	}
#endif //#ifdef CONFIG_SSTAR_AP_CHANNEL_CHANGE_EVENT
	...
	if (priv->hidden_ssid) {
		...
		Sstar_printk_ap("hidden_ssid set\n");
		ssid_ie = (u8 *)Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_SSID,
							ies, ies_len);
		WARN_ON(!ssid_ie);
		ssid_len = ssid_ie[1];
		if (ssid_len) {
			Sstar_printk_ap("%s: hidden_ssid with zero content "
					"ssid\n",
					__func__);
			ssid_ie[1] = 0;
			...
			Sstar_printk_ap("%s: hidden ssid with ssid len 0"
					" not supported",
					__func__);
...
#ifdef SSTAR_PROBE_RESP_EXTRA_IE
		if (priv->mode == NL80211_IFTYPE_AP) {
			ret = Sstar_upload_proberesp(priv);
...
static int Sstar_upload_proberesp(struct Sstar_vif *priv)
{
	...
	if (priv->hidden_ssid) {
		...
		offset = offsetof(struct Sstar_ieee80211_mgmt,
				  u.probe_resp.variable);
		ssid_ie =
			(u8 *)Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_SSID,
		...
		if (ssid_len && (ssid_len == priv->ssid_length)) {
			...
			Sstar_printk_err(
				"hidden ssid with mismatched ssid_len %d\n",
				ssid_len);
			Sstar_dev_kfree_skb(frame.skb);
			return -1;
		}
...
int Sstar_upload_beacon_private(struct Sstar_vif *priv)
{
	return Sstar_upload_beacon(priv);
}
int Sstar_upload_proberesp_private(struct Sstar_vif *priv)
{
#ifdef SSTAR_PROBE_RESP_EXTRA_IE
	return Sstar_upload_proberesp(priv);
#else
	return 0;
#endif
}
...
static int Sstar_start_ap(struct Sstar_vif *priv)
{
	...
	struct wsm_start start = {
		...
		.probeDelay = 100,
		.basicRateSet =
			Sstar_rate_mask_to_wsm(hw_priv, conf->basic_rates),
#ifdef P2P_MULTIVIF
	...
	offset = offsetof(struct Sstar_ieee80211_mgmt, u.beacon.variable);
	ssidie = Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_SSID,
					 skb->data + offset, skb->len - offset);
	...
	if (start_choff <= NL80211_CHAN_HT40PLUS) {
		Sstar_printk_ap("%s:fix chatype(%d)\n", __func__, start_choff);
	...
	Sstar_printk_ap("%s:start.channel_type,(%d),channelNumber(%d)\n",
			__func__, start.channel_type, start.channelNumber);
#endif

	Sstar_printk_ap("[AP] ch: %d(%d), bcn: %d(%d), "
			"brt: 0x%.8X, ssid: %.*s.\n",
			start.channelNumber, start.band, start.beaconInterval,
			start.DTIMPeriod, start.basicRateSet, start.ssidLength,
			start.ssid);
	ret = WARN_ON(wsm_start(hw_priv, &start, priv->if_id));
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	if (!ret && priv->vif->p2p) {
		ap_printk("[AP] Setting p2p powersave "
			  "configuration.\n");
		WARN_ON(wsm_set_p2p_ps_modeinfo(hw_priv, &priv->p2p_ps_modeinfo,
						priv->if_id));
		Sstar_notify_noa(priv, SSTAR_APOLLO_NOA_NOTIFICATION_DELAY);
	}
	...
	if (!ret) {
		...
		if ((priv->if_id == 1) && !hw_priv->is_go_thru_go_neg)
			WARN_ON(wsm_set_block_ack_policy(
				hw_priv,
				SSTAR_APOLLO_TX_BLOCK_ACK_DISABLED_FOR_ALL_TID,
				SSTAR_APOLLO_RX_BLOCK_ACK_DISABLED_FOR_ALL_TID,
				priv->if_id));
		else
#endif
			WARN_ON(wsm_set_block_ack_policy(
				hw_priv, hw_priv->ba_tid_tx_mask,
				hw_priv->ba_tid_rx_mask, priv->if_id));
	...
	WARN_ON(wsm_set_operational_mode(hw_priv, &mode, priv->if_id));
	hw_priv->vif0_throttle = SSTAR_WIFI_HOST_VIF0_11N_THROTTLE;
	hw_priv->vif1_throttle = SSTAR_WIFI_HOST_VIF0_11N_THROTTLE;
...
#endif
	Sstar_printk_ap("AP/GO mode BG THROTTLE %d,cipherType %x\n",
...
int Sstar_alloc_link_id(struct Sstar_vif *priv, const u8 *mac)
{
	...
	for (i = 0;
	     i <
	     hw_priv->wsm_caps.NumOfStations /*SSTARWIFI_MAX_STA_IN_AP_MODE*/;
	     ++i) {
	...
	if (ret) {
		struct Sstar_link_entry *entry = &priv->link_id_db[ret - 1];
		ap_printk("[AP] STA added, link_id: %d\n", ret);
		Sstar_printk_err("alloc mac[%pM],link_id(%d)\n", mac, ret);
		entry->timestamp = jiffies;
		entry->status = SSTAR_APOLLO_LINK_RESERVE;
		memcpy(&entry->mac, mac, ETH_ALEN);
		memset(&entry->buffered, 0, SSTAR_APOLLO_MAX_TID);
		Sstar_skb_queue_head_init(&entry->rx_queue);
		wsm_lock_tx_async(hw_priv);
		if (Sstar_hw_priv_queue_work(hw_priv, &priv->link_id_work) <= 0)
			wsm_unlock_tx(hw_priv);
	} else {
		for (i = 0; i < hw_priv->wsm_caps.NumOfStations; i++) {
			Sstar_printk_err("dammp mac[%pM],status[%d]\n",
...
void Sstar_link_id_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, link_id_work);
...
void Sstar_link_id_gc_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, link_id_gc_work.work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	...
	for (i = 0; i < SSTARWIFI_MAX_STA_IN_AP_MODE; ++i) {
		need_reset = false;
		mask = BIT(i + 1);
		if (priv->link_id_db[i].status == SSTAR_APOLLO_LINK_RESERVE ||
		    (priv->link_id_db[i].status == SSTAR_APOLLO_LINK_HARD &&
			...
			spin_unlock_bh(&priv->ps_state_lock);
			if (need_reset) {
				WARN_ON(wsm_map_link(hw_priv, &unmap_link,
						     priv->if_id));
			}
			map_link.link_id = i + 1;
			WARN_ON(wsm_map_link(hw_priv, &map_link, priv->if_id));
			next_gc = min(next_gc, SSTAR_APOLLO_LINK_ID_GC_TIMEOUT);
			spin_lock_bh(&priv->ps_state_lock);
		} else if (priv->link_id_db[i].status ==
			   SSTAR_APOLLO_LINK_SOFT) {
			ttl = priv->link_id_db[i].timestamp - now +
			      SSTAR_APOLLO_LINK_ID_GC_TIMEOUT;
			if (ttl <= 0) {
				need_reset = true;
				priv->link_id_db[i].status =
...
#if 0
			else if (priv->link_id_db[i].status == SSTAR_APOLLO_LINK_RESET ||
				priv->link_id_db[i].status ==
				SSTAR_APOLLO_LINK_RESET_REMAP) {
			int status = priv->link_id_db[i].status;
			if(priv->link_id_db[i].status == SSTAR_APOLLO_LINK_RESET){
				priv->link_id_db[i].status = SSTAR_APOLLO_LINK_OFF;
				priv->link_id_map &= ~mask;
				priv->sta_asleep_mask &= ~mask;
				priv->pspoll_mask &= ~mask;
			}else {
			...
			if (status == SSTAR_APOLLO_LINK_RESET_REMAP) {
...
#endif
		if (need_reset) {
			Sstar_skb_queue_purge(&priv->link_id_db[i].rx_queue);
	...
	if (next_gc != -1)
		Sstar_hw_priv_queue_delayed_work(
			hw_priv, &priv->link_id_gc_work, next_gc);
	wsm_unlock_tx(hw_priv);
}
#ifdef CONFIG_SSTAR_SUPPORT_P2P
...
#endif /* CONFIG_SSTAR_APOLLO_STA_DEBUG */
#if 0
...
int ABwifi_unmap_link(struct Sstar_vif *priv, int link_id)
{
	...
	if (link_id)
		memcpy(&maplink.mac_addr[0], priv->link_id_db[link_id - 1].mac,
		       ETH_ALEN);
	return wsm_map_link(hw_priv, &maplink, priv->if_id);
}
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 3, 0))
void Sstar_ht_info_update_work(struct Sstar_work_struct *work)
{
	struct sk_buff *skb;
	struct Sstar_ieee80211_mgmt *mgmt;
	u8 *ht_info, *ies;
	u32 ies_len;
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, ht_info_update_work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	struct wsm_update_ie update_ie = {
		.what = WSM_UPDATE_IE_BEACON,
		.count = 1,
			     ...
			     struct ieee80211_channel *chan)
{
	...
	*/
	...
	if (hw_priv->chip_version == ARES_6012B) {
		...
		if ((channel_hw_value(chan) >= 9) &&
		    (channel_hw_value(chan) <= 14))
			start.channel_type = (u32)(NL80211_CHAN_HT40MINUS);
		else if ((channel_hw_value(chan) > 0) &&
			 (channel_hw_value(chan) < 9))
	...
	start.channel_type = (u32)(NL80211_CHAN_HT20);
#endif
	Sstar_printk_ap("%s:if_id(%d),channel(%d)\n", __func__, priv->if_id,
			channel_hw_value(chan));
	wsm_start(hw_priv, &start, SSTAR_WIFI_GENERIC_IF_ID);
