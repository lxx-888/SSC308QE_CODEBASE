...
#else
#define WIFI_PLFDEVNAME "Sstarusbwifi"
#endif
...
void Sstar_usb_urb_put(struct sbus_priv *self, unsigned long *bitmap, int id,
		       int tx);
int Sstar_usb_urb_get(struct sbus_priv *self, unsigned long *bitmap,
		      int max_urb, int tx);
...
extern int wsm_release_tx_buffer_NoLock(struct Sstar_common *hw_priv,
					int count);
int wsm_release_vif_tx_buffer_Nolock(struct Sstar_common *hw_priv, int if_id,
				     int count);
void Sstar_usb_release_tx_err_urb(struct sbus_priv *self, unsigned long *bitmap,
				  int tx);
...
#define Sstar_usb_buffer_alloc(dev, size, dma)                                 \
	usb_alloc_coherent((dev), (size),                                      \
			   (in_interrupt() ? GFP_ATOMIC : GFP_KERNEL), (dma))
#define Sstar_usb_buffer_free(dev, size, addr, dma)                            \
	usb_free_coherent((dev), (size), (addr), (dma))
#else //(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,35))
#define Sstar_usb_buffer_alloc(dev, size, dma)                                 \
	usb_buffer_alloc((dev), (size),                                        \
			 (in_interrupt() ? GFP_ATOMIC : GFP_KERNEL), (dma))
#define Sstar_usb_buffer_free(dev, size, addr, dma)                            \
...
#else
#define Sstar_usb_buffer_alloc(_dev, _size, _dma)                              \
	Sstar_kmalloc(_size, GFP_ATOMIC)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 5, 0))
#define SSTAR_USB_FILL_HTTX_BULK_URB(pUrb, pUsb_Dev, uEndpointAddress,         \
				     pTransferBuf, BufSize, Complete,          \
				     pContext, TransferDma)                    \
	do {                                                                   \
		usb_fill_bulk_urb(pUrb, pUsb_Dev,                              \
...
static int Sstar_usb_receive_data(struct sbus_priv *self, unsigned int addr,
				  void *dst, int count,
				  sbus_callback_handler hander);
static int Sstar_usb_xmit_data(struct sbus_priv *self, unsigned int addr,
...
#define Sstar_wifi_set_status(status)                                          \
	SSTAR_VOLATILE_SET(&wifi_module_exit, status)
...
#ifdef CONFIG_USE_DMA_ADDR_BUFFER
	Sstar_dma_addr_t
		dma_transfer_addr; /* (in) dma addr for transfer_buffer */
...
#else
	struct Sstar_workqueue_struct *tx_workqueue;
	struct Sstar_workqueue_struct *rx_workqueue;
	struct Sstar_work_struct rx_complete_work;
	struct Sstar_work_struct tx_complete_work;
#endif
	bool rx_running;
	void *tx_data;
	int tx_vif_selected;
...
static void Sstar_usb_fw_sync(struct Sstar_common *hw_priv,
...
void Sstar_tx_complete_work(struct Sstar_work_struct *work)
{
	struct sbus_priv *self =
...
void Sstar_rx_complete_work(struct Sstar_work_struct *work)
{
	struct sbus_priv *self =
...
static int Sstar_usb_xmit_init(struct sbus_priv *self)
{
	...
	Sstar_printk_init("Sstarwifi USB_USE_TASTLET_TXRX enable (%p)\n",
			  self->core);
	tasklet_init(&self->tx_cmp_tasklet, Sstar_tx_tasklet,
		     (unsigned long)self->core);
#else
	struct Sstar_common *hw_priv = self->core;
	Sstar_printk_init("Sstarwifi INIT_WORK enable\n");
	SSTAR_INIT_WORK(&self->tx_complete_work, Sstar_tx_complete_work);
	self->tx_workqueue =
		Sstar_create_singlethread_workqueue(ieee80211_alloc_name(
			hw_priv->hw, "tx_workqueue") /*"tx_workqueue"*/);
...
static int Sstar_usb_xmit_deinit(struct sbus_priv *self)
{
	...
	if (self->tx_workqueue) {
		Sstar_flush_workqueue(self->tx_workqueue);
		Sstar_destroy_workqueue(self->tx_workqueue);
	}
...
static int Sstar_usb_rev_init(struct sbus_priv *self)
{
	...
	Sstar_printk_init("Sstarwifi USB_USE_TASTLET_TXRX enable (%p)\n",
			  self->core);
	tasklet_init(&self->rx_cmp_tasklet, Sstar_rx_tasklet,
		     (unsigned long)self->core);
#else
	struct Sstar_common *hw_priv = self->core;
	Sstar_printk_init("Sstarwifi INIT_WORK enable\n");
	SSTAR_INIT_WORK(&self->rx_complete_work, Sstar_rx_complete_work);
	self->rx_workqueue =
		Sstar_create_singlethread_workqueue(ieee80211_alloc_name(
			hw_priv->hw, "rx_workqueue") /*"rx_workqueue"*/);
...
static int Sstar_usb_rev_deinit(struct sbus_priv *self)
{
	...
	if (self->rx_workqueue) {
		Sstar_flush_workqueue(self->rx_workqueue);
		Sstar_destroy_workqueue(self->rx_workqueue);
	}
...
static int Sstar_usb_xmit_schedule(struct sbus_priv *self)
{
	...
	if (atomic_read(&hw_priv->bh_term) || hw_priv->bh_error ||
	...
	Sstar_queue_work(self->tx_workqueue, &self->tx_complete_work);
#endif
	return 0;
}
static int Sstar_usb_rev_schedule(struct sbus_priv *self)
{
	...
	Sstar_queue_work(self->rx_workqueue, &self->rx_complete_work);
#endif
	return 0;
}
...
static int Sstar_usbctrl_vendorreq_sync(struct sbus_priv *self, u8 request,
					u8 b_write, u16 value, u16 index,
					void *pdata, u16 len)
{
	unsigned int pipe;
	int status;
	...
	if (len > SSTAR_USB_EP0_MAX_SIZE) {
		Sstar_dbg(
			SSTAR_APOLLO_DBG_MSG,
			"usbctrl_vendorreq request 0x%x, b_write %d! len:%d >%d too long \n",
	...
	do {
		...
		if (status < 0) {
			Sstar_printk_err(
				"vendorreq:err(%d)addr[%x] len[%d],b_write %d request %d\n",
				status, value | (index << 16), len, b_write,
				request);
		} else if (status != len) {
			Sstar_printk_err("vendorreq:len err(%d)\n", status);
		} else {
			break;
		}
...
#ifdef HW_DOWN_FW
static int Sstar_usb_hw_read_port(struct sbus_priv *self, u32 addr, void *pdata,
				  int len)
{
	int ret = 0;
	...
	ret = Sstar_usbctrl_vendorreq_sync(self, request, 0, wvalue, index,
					   pdata, len);
	if (ret < 0) {
...
static int Sstar_usb_hw_write_port(struct sbus_priv *self, u32 addr,
				   const void *pdata, int len)
{
	...
	ret = Sstar_usbctrl_vendorreq_sync(self, request, 1, wvalue, index,
					   (void *)pdata, len);
	if (ret < 0) {
...
{
	struct Sstar_usb_ctrlrequest *ctrl =
		(struct Sstar_usb_ctrlrequest *)urb->context;
	struct sbus_priv *self = ctrl->self;

	BUG_ON(self == NULL);

	if (urb->status) {
		Sstar_printk_err("ctrl req urb err[%d]\n", urb->status);
				   ...
				   const void *pdata, int len)
{
	...
	usb_fill_control_urb(urb, self->drvobj->pusbdev,
			     usb_sndctrlpipe(self->drvobj->pusbdev, 0),
...
#endif
int Sstar_usb_sw_read_port(struct sbus_priv *self, u32 addr, void *pdata,
			   int len)
{
	...
	return Sstar_usbctrl_vendorreq_sync(self, request, 0, wvalue, index,
					    pdata, len);
}

#else
static int Sstar_usb_sw_read_port(struct sbus_priv *self, u32 addr, void *pdata,
				  int len)
{
	u8 request = VENDOR_SW_READ; //SW
	u16 wvalue = (u16)(addr & 0x0000ffff);
...
//#ifndef HW_DOWN_FW
static int Sstar_usb_sw_write_port(struct sbus_priv *self, u32 addr,
				   const void *pdata, int len)
{
	...
	WARN_ON(len > SSTAR_USB_EP0_MAX_SIZE);
	return Sstar_usbctrl_vendorreq_sync(self, request, 1, wvalue, index,
					    (void *)pdata, len);
}
#endif
int Sstar_lmac_start(struct sbus_priv *self)
{
	...
	return Sstar_usbctrl_vendorreq_sync(self, request, 1, 0, 0, &tmpdata,
					    0);
}
...
static int Sstar_usb_device_reset(struct sbus_priv *self)
{
	...
	*reset usb
	*/
	ret = usb_lock_device_for_reset(self->drvobj->pusbdev,
					self->drvobj->pusbintf);
	if (ret == 0) {
...
static int Sstar_usb_wait_anchor_empty_timeout(struct sbus_priv *self,
					       int timeout)
{
	int urb_index = 0;
	struct Sstar_common *hw_priv = self->core;
	...
	ret = usb_wait_anchor_empty_timeout(
		&hw_priv->sbus_priv->drvobj->rx_submitted, timeout);
	if (ret == 0) {
		Sstar_printk_err(
			"rx_submitted cancle timeout (%d)\n",
			usb_anchor_empty(
				&hw_priv->sbus_priv->drvobj->rx_submitted));
		goto exit;
	}
	...
	ret = usb_wait_anchor_empty_timeout(
		&hw_priv->sbus_priv->drvobj->tx_submitted, timeout);
	if (ret == 0) {
		Sstar_printk_err(
			"tx_submitted cancle timeout(%d) \n",
			usb_anchor_empty(
				&hw_priv->sbus_priv->drvobj->tx_submitted));
	}
...
int Sstar_usb_ep0_hw_reset_cmd(struct sbus_priv *self, enum HW_RESET_TYPE type,
			       bool irq_lmac)
{
	...
	if (type == HW_RESET_HIF) {
		...
		tmpdata =
			HW_RESET_REG_CPU | HW_RESET_REG_HIF | HW_RESET_REG_SYS;
	}
	if (irq_lmac) {
...
#ifdef CONFIG_USB_AGGR_URB_TX
	Sstar_usb_free_txDMABuf_all(self, tx_urb->pallocated_buf,
...
void Sstar_usb_free_err_data(struct sbus_priv *self, struct sbus_urb *tx_urb)
{
	...
	if (!WARN_ON(
		    Sstar_queue_get_skb(queue, wsm->packetID, &skb, &txpriv))) {
		...
		wsm_release_vif_tx_buffer_Nolock(
			hw_priv, Sstar_queue_get_if_id(wsm->packetID), 1);
	}
}
			       ...
			       sbus_callback_handler hander)
{
	...
	urb_id = Sstar_usb_urb_get(self, self->drvobj->tx_urb_map, TX_URB_NUM,
				   1);
	if (urb_id < 0) {
	...
	do {
		...
		ret = wsm_get_tx(hw_priv, &data, &actual_len, &tx_burst,
				 &vif_selected);
		if (ret <= 0) {
			wsm_release_tx_buffer_NoLock(hw_priv, 1);
			//Sstar_usb_urb_put(self,self->drvobj->tx_urb_map,urb_id,1);
			...
			usb_printk("%s flag=%x, tx_len %d seq %d len %d\n",
				   __func__, wsm->flag, tx_len,
				   hw_priv->wsm_tx_seq, wsm->len);
			...
			usb_printk("tx_seq %d\n", self->tx_hwChanId);
			hw_priv->wsm_tx_seq =
				(hw_priv->wsm_tx_seq + 1) & WSM_TX_SEQ_MAX;

			if (vif_selected != -1) {
				hw_priv->hw_bufs_used_vif[vif_selected]++;
			}
			//memcpy(txdmabuff,txdata,actual_len);
			Sstar_xmit_linearize(hw_priv, (struct wsm_tx *)wsm,
					     txdmabuff, actual_len);
	...
	//the last dma buffer
			...
			if (tx_urb->frame_cnt >=
			    (PER_BUFF_AGGR_NUM * tx_urb->dma_buff_alloced)) {
				if (usb_aggr_buff >=
				    self->drvobj->tx_dma_addr_buffer_end)
					break;
	...
	if (!Sstar_wifi_get_status()) {
		...
		tx_urb->dma_transfer_addr =
			self->drvobj->tx_dma_addr +
			((Sstar_dma_addr_t)tx_urb->pallocated_buf -
			 (Sstar_dma_addr_t)self->drvobj->tx_dma_addr_buffer);
		SSTAR_USB_FILL_HTTX_BULK_URB(tx_urb->test_urb,
					     self->drvobj->pusbdev,
					     self->drvobj->ep_out,
					     tx_urb->pallocated_buf,
					     tx_urb->pallocated_buf_len,
					     Sstar_usb_xmit_data_complete,
					     tx_urb, tx_urb->dma_transfer_addr);
	...
	if (status && tx_urb->frame_cnt) {
		...
		for (i = 0; i < tx_urb->frame_cnt; i++) {
			wsm_txd = (struct wsm_tx *)(tx_urb->pallocated_buf +
						    i * PER_PACKET_LEN);
...
#ifdef CONFIG_TX_NO_CONFIRM
				if (!(wsm_txd->htTxParameters &
				      __cpu_to_le32(WSM_NEED_TX_CONFIRM))) {
					continue;
				}
#endif
				tx_urb->data = wsm_txd;
			       ...
			       sbus_callback_handler hander)
{
	...
	/*if (atomic_read(&self->tx_lock)==0)*/ {
		...
		ret = wsm_get_tx(hw_priv, &data, &actual_len, &tx_burst,
				 &vif_selected);
		if (ret <= 0) {
			Sstar_usb_urb_put(self, self->drvobj->tx_urb_map,
					  urb_id, 1);
			...
			usb_anchor_urb(tx_urb->test_urb,
				       &self->drvobj->tx_submitted);
			if (!Sstar_wifi_get_status()) {
#ifdef CONFIG_USE_DMA_ADDR_BUFFER
				///self->drvobj->tx_urb->transfer_flags |= URB_ZERO_PACKET;
				//memcpy(tx_urb->pallocated_buf,txdata,tx_len);
				Sstar_xmit_linearize(hw_priv,
						     (struct wsm_tx *)txdata,
						     tx_urb->pallocated_buf,
						     tx_len);
				SSTAR_USB_FILL_HTTX_BULK_URB(
					tx_urb->test_urb, self->drvobj->pusbdev,
					self->drvobj->ep_out,
					tx_urb->pallocated_buf, tx_len,
					Sstar_usb_xmit_data_complete, tx_urb,
					tx_urb->dma_transfer_addr);
				//usb_anchor_urb(self->drvobj->tx_urb,
				status = usb_submit_urb(tx_urb->test_urb,
							GFP_ATOMIC);
	...
	//tx_urb->data = NULL;
				...
				Sstar_xmit_linearize(hw_priv,
						     (struct wsm_tx *)txdata,
						     submit_buff, actual_len);
				pipe = usb_sndbulkpipe(self->drvobj->pusbdev,
						       self->drvobj->ep_out);
				//printk("%s:tx_urb->data[%p]\n",__func__,tx_urb->data);
				///self->drvobj->tx_urb->transfer_flags |= URB_ZERO_PACKET;
				usb_fill_bulk_urb(tx_urb->test_urb,
						  self->drvobj->pusbdev, pipe,
						  submit_buff, tx_len,
						  Sstar_usb_xmit_data_complete,
						  tx_urb);
				//usb_anchor_urb(self->drvobj->tx_urb,
			...
			if (status) {
				usb_unanchor_urb(tx_urb->test_urb);
				tx_urb->test_urb->status = 0;
				if (__le16_to_cpu(wsm->id) !=
				    WSM_FIRMWARE_CHECK_ID) {
					if (vif_selected != -1) {
						Sstar_usb_free_err_data(self,
									tx_urb);
					} else {
						Sstar_usb_free_err_cmd(self);
					}
					//atomic_xchg(&self->tx_lock, 0);
					wsm_release_tx_buffer_NoLock(hw_priv,
								     1);
				}
				Sstar_usb_urb_put(self,
						  self->drvobj->tx_urb_map,
						  urb_id, 1);
				tx_urb->data = NULL;
				status = 1;
				printk_once(KERN_ERR
					    "tx:Sstar_usb_urb_put %d\n",
					    urb_id);
				//msleep(1000);
				goto error;
			}
...
void Sstar_usb_urb_free(struct sbus_priv *self, struct sbus_urb *pUrb,
			int max_num)
{
	int i = 0;
#ifdef CONFIG_USE_DMA_ADDR_BUFFER
	if (self->drvobj->tx_dma_addr) {
		Sstar_usb_buffer_free(self->drvobj->pusbdev,
				      self->drvobj->tx_dma_addr_buffer_len,
				      self->drvobj->tx_dma_addr_buffer,
				      self->drvobj->tx_dma_addr);
...
char *Sstar_usb_pick_txDMABuf(struct sbus_priv *self)
{
	...
	if (self->drvobj->NextAllocPost == self->drvobj->NextFreePost) {
		if (self->drvobj->tx_dma_addr_buffer_full) {
			usb_printk(
				"Sstar_usb_pick_txDMABuf:tx_dma_addr_buffer_full %d \n",
...
char *Sstar_usb_get_txDMABuf(struct sbus_priv *self)
{
	...
	if (self->drvobj->free_dma_buffer_cnt < 0) {
		...
		Sstar_printk_err(
			"free_dma_buffer_cnt ERR,NextAllocPost %p drvobj->NextFreePost %p\n",
...
void Sstar_usb_free_txDMABuf(struct sbus_priv *self)
{
	if (self->drvobj->NextAllocPost == self->drvobj->NextFreePost) {
		if (self->drvobj->tx_dma_addr_buffer_full == 0) {
			Sstar_printk_err(
				"self->drvobj->free_dma_buffer_cnt %d\n",
				self->drvobj->free_dma_buffer_cnt);
	...
	self->drvobj->free_dma_buffer_cnt++;
	if (self->drvobj->total_dma_buffer_cnt <
	    self->drvobj->free_dma_buffer_cnt) {
		Sstar_printk_err(
			"<WARNING> Sstar_usb_free_txDMABuf (buffer(%p)  NextFreePost(%p))free_dma_buffer_cnt %d \n",
...
void Sstar_usb_free_txDMABuf_all(struct sbus_priv *self, u8 *buffer, int cnt)
{
	...
	if ((char *)buffer != self->drvobj->NextFreePost) {
		Sstar_printk_err(
			"<WARNING> Sstar_usb_free_txDMABuf_all (buffer(%p) != NextFreePost(%p))free_dma_buffer_cnt %d cnt %d\n",
			buffer, self->drvobj->NextFreePost,
			self->drvobj->free_dma_buffer_cnt, cnt);
		BUG_ON(1);
...
#endif
#endif //#ifdef CONFIG_USE_DMA_ADDR_BUFFER
int Sstar_usb_urb_malloc(struct sbus_priv *self, struct sbus_urb *pUrb,
			 int max_num, int len, int b_skb, int b_dma_buffer)
{
	int i = 0;
#ifdef CONFIG_USE_DMA_ADDR_BUFFER
...
#ifdef CONFIG_USB_AGGR_URB_TX
		self->drvobj->tx_dma_addr_buffer_len =
			len * max_num * URB_AGGR_NUM; //ALIGN(len*max_num,4096);
#else
		self->drvobj->tx_dma_addr_buffer_len =
			len * max_num; //ALIGN(len*max_num,4096);
#endif //CONFIG_USB_AGGR_URB_TX
		Sstar_printk_init(
			"Sstar_usb_urb_malloc CONFIG_USE_DMA_ADDR_BUFFER max_num %d, total %d\n",
			max_num, (int)self->drvobj->tx_dma_addr_buffer_len);
		self->drvobj->tx_dma_addr_buffer = Sstar_usb_buffer_alloc(
			self->drvobj->pusbdev,
			self->drvobj->tx_dma_addr_buffer_len,
			&self->drvobj->tx_dma_addr);
		if (self->drvobj->tx_dma_addr_buffer == NULL) {
			Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
				  "Can't allocate tx_dma_addr_buffer.");
			goto __free_urb;
		}
	...
	self->drvobj->tx_dma_addr_buffer_full = 0;
	self->drvobj->free_dma_buffer_cnt =
		self->drvobj->tx_dma_addr_buffer_len / BUFF_ALLOC_LEN;
	self->drvobj->total_dma_buffer_cnt =
		self->drvobj->tx_dma_addr_buffer_len / BUFF_ALLOC_LEN;
	Sstar_printk_init(
		"CONFIG_USB_AGGR_URB_TX enable cnt tx_dma_addr_buffer_end(%p)tx_dma_addr_buffer(%p),%d\n",
		self->drvobj->tx_dma_addr_buffer,
		self->drvobj->tx_dma_addr_buffer_end,
		(int)self->drvobj->tx_dma_addr_buffer_len / BUFF_ALLOC_LEN);
#endif //CONFIG_USB_AGGR_URB_TX
#endif //#ifdef CONFIG_USE_DMA_ADDR_BUFFER
...
#else
			pUrb[i].pallocated_buf =
				self->drvobj->tx_dma_addr_buffer + i * len;
			pUrb[i].test_skb = NULL;
			pUrb[i].pallocated_buf_len = len;
			pUrb[i].dma_transfer_addr =
				self->drvobj->tx_dma_addr + i * len;
#endif //USB_ATHENAB_AGGR
		} else
#endif //#ifdef CONFIG_USE_DMA_ADDR_BUFFER
			if (b_skb) {
			pUrb[i].test_skb =
				__Sstar_dev_alloc_skb(len, GFP_KERNEL);
			if (!pUrb[i].test_skb) {
				Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
					  "Can't allocate test_skb.");
				goto __free_skb;
			}
			pUrb[i].pallocated_buf = NULL;
		} else {
...
static int Sstar_usb_data_send(struct sbus_priv *self)
{
	...
	do {
		status = hw_priv->sbus_ops->sbus_memcpy_toio(
			hw_priv->sbus_priv, 0x1, NULL, TX_BUFFER_SIZE);
	} while (status > 0);
...
static int __Sstar_usb_wsm_send(struct sbus_priv *self)
{
	...
	if (!Sstar_wifi_get_status() && (!Sstar_bh_is_term(hw_priv)) &&
	    txdata && wsm_urb->pallocated_buf) {
		unsigned int pipe;
		...
		pipe = usb_sndbulkpipe(self->drvobj->pusbdev,
				       self->drvobj->ep_out);
		usb_fill_bulk_urb(wsm_urb->test_urb, self->drvobj->pusbdev,
				  pipe, submit_buff, tx_len,
		...
		*/
		...
		if (status != 0) {
			...
			*/
			usb_kill_urb(wsm_urb->test_urb);
			wsm_release_tx_buffer_NoLock(hw_priv, 1);
			if (usb_wait_anchor_empty_timeout(wsm_anchor, 20000) ==
			    0) {
				WARN_ON(1);
				usb_unanchor_urb(wsm_urb->test_urb);
				usb_unanchor_urb(self->drvobj->wait_urb);
			...
			if (cmd_recovery == 0) {
				int recovery = 0;
				Sstar_printk_err(
					"%s:wsm urb not send,try to recovery\n",
					__func__);
				recovery = wsm_recovery(hw_priv);
				if ((recovery != RECOVERY_ERR) &&
				    (recovery != RECOVERY_BH_HALT)) {
					spin_lock_bh(&hw_priv->wsm_cmd.lock);
					hw_priv->wsm_cmd.ptr =
						hw_priv->wsm_cmd_buf.begin;
					hw_priv->wsm_cmd.len =
						hw_priv->wsm_cmd_buf.data -
						hw_priv->wsm_cmd_buf.begin;
					cmd_recovery = 1;
					spin_unlock_bh(&hw_priv->wsm_cmd.lock);
					goto cmd_retry;
...
static int Sstar_usb_wait_submitted_xmited(struct sbus_priv *self)
{
	...
	status = usb_wait_anchor_empty_timeout(&self->drvobj->tx_submitted,
					       10000);
...
#ifdef CONFIG_WSM_CMD_XMIT_DIRECTLY
	status = usb_wait_anchor_empty_timeout(&self->drvobj->wsm_submitted,
					       10000);
...
#ifdef CONDIF_SSTAR_CTRL_REQUEST_ASYNC
	status = usb_wait_anchor_empty_timeout(&self->drvobj->ctrl_submitted,
					       10000);
	if (status == 0)
		return -1;
...
int Sstar_usb_free_tx_wsm(struct sbus_priv *self, struct sbus_urb *tx_urb)
{
	...
	if ((wsm) &&
		...
		if (!WARN_ON(Sstar_queue_get_skb(queue, wsm->packetID, &skb,
						 &txpriv))) {
			struct ieee80211_tx_info *tx = IEEE80211_SKB_CB(skb);
			//int tx_count = 0;
			int i;

			wsm_release_vif_tx_buffer_Nolock(hw_priv, txpriv->if_id,
							 1);
			wsm_release_tx_buffer_NoLock(hw_priv, 1);
			...
			wsm_release_vif_tx_buffer_Nolock(
				hw_priv, Sstar_queue_get_if_id(wsm->packetID),
				1);
			wsm_release_tx_buffer_NoLock(hw_priv, 1);
		}
		tx_urb->data = NULL;
				  ...
				  int tx)
{
	...
	for (; (clear_id >= 0) && (clear_id < TX_URB_NUM);
	     clear_id =
		     find_first_bit(self->drvobj->tx_err_urb_map, TX_URB_NUM)) {
		      ...
		      int max_urb, int tx)
{
	...
	if (tx) {
		int clear_id;
		struct sbus_urb *tx_urb = NULL;
		clear_id = find_first_bit(self->drvobj->txpending_urb_map,
					  TX_URB_NUM);
		for (; (clear_id >= 0) && (clear_id < TX_URB_NUM);
		     clear_id = find_first_bit(self->drvobj->txpending_urb_map,
					       TX_URB_NUM)) {
...
extern bool Sstar_rx_directly(struct Sstar_common *hw_priv, struct sk_buff *skb,
			      int (*rx_func)(struct Sstar_common *hw_priv,
					     struct sk_buff *skb));
static void Sstar_usb_submit_rev_skb(struct sbus_priv *self,
				     struct sk_buff *skb, int urb_id)
{
	struct Sstar_common *hw_priv = self->core;
	unsigned long flags;
	bool rx_running;
...
static void Sstar_usb_receive_data_complete(struct urb *urb)
{
	...
	case -ESHUTDOWN:
		Sstar_printk_bus(
			"Sstar_usb_rx_complete1 error status=%d len %d\n",
			urb->status, RecvLength);
		goto __free;
	case -EPROTO:
		Sstar_printk_bus(
			"Sstar_usb_rx_complete3 error status=%d len %d\n",
			urb->status, RecvLength);
		if (RecvLength != 0) {
			break;
		}
	case -EOVERFLOW:
		...
		if (RecvLength)
			break;
		Sstar_printk_bus(
			"<ERROR>:Sstar_usb_rx_complete status=%d len %d\n",
			urb->status, RecvLength);
		goto __free;
	default:
		Sstar_printk_bus(
			"Sstar_usb_rx_complete2 error status=%d len %d\n",
			urb->status, RecvLength);
		goto resubmit;
	}

	if ((self->drvobj->suspend_skb != NULL) &&
	    (self->drvobj->suspend_skb_len != 0)) {
		if (Sstar_skb_tailroom(self->drvobj->suspend_skb) <
		    self->drvobj->suspend_skb_len + RecvLength) {
			struct sk_buff *long_suspend_skb = NULL;
			BUG_ON(self->drvobj->suspend_skb_len + RecvLength >
			       RX_BUFFER_SIZE);
			Sstar_printk_err(
				"suspend skb len is not enough(%d),(%ld)\n",
				Sstar_skb_tailroom(self->drvobj->suspend_skb),
				self->drvobj->suspend_skb_len + RecvLength);

			long_suspend_skb =
				Sstar_dev_alloc_skb(RX_BUFFER_SIZE + 64);
			BUG_ON(!long_suspend_skb);
			Sstar_skb_reserve(long_suspend_skb, 64);
			memcpy((u8 *)long_suspend_skb->data,
			       self->drvobj->suspend_skb->data,
			       self->drvobj->suspend_skb_len);
			Sstar_dev_kfree_skb(self->drvobj->suspend_skb);
			self->drvobj->suspend_skb = long_suspend_skb;
		}
		memcpy((u8 *)self->drvobj->suspend_skb->data +
			       self->drvobj->suspend_skb_len,
		       skb->data, RecvLength);
		RecvLength += self->drvobj->suspend_skb_len;
		memcpy(skb->data, (u8 *)self->drvobj->suspend_skb->data,
		       RecvLength);
		self->drvobj->suspend_skb_len = 0;
	...
	if (wsm->len != RecvLength) {
		...
		if (((wsm->len % 512) == 0) && ((wsm->len + 1) == RecvLength)) {
			...
			if ((urb->actual_length > RX_BUFFER_SIZE) ||
			    (wsm->len < urb->actual_length)) {
				int actual_length = urb->actual_length;
				u8 *data = skb->data;
				Sstar_printk_err(
					"actual_length(%d),max_len(%d),transfer_buffer_length(%d)\n",
					urb->actual_length, RX_BUFFER_SIZE,
				...
				*/
				if(urb->actual_length < urb->transfer_buffer_length){
					Sstar_printk_err("%s:need reload wifi driver! \n",__FUNCTION__);
					if(local->upper_pid != 0)
						send_signal(SIGUSR1,local->upper_pid);
					else
						Sstar_printk_err("%s:not set upper process pid , not send signal\n",__FUNCTION__);
				}
				/*
					end add
				*/
			...
			wsm = (struct wsm_hdr *)skb->data;
			Sstar_printk_err(
				"rx rebulid usbsuspend  id %d wsm->len %d,RecvLength %d\n",
	...
	if (0) {
		...
		extern int Sstar_rx_single_channel_bh_cb(
			struct Sstar_common * hw_priv, struct sk_buff * skb);
		int (*cb_handle)(struct Sstar_common * hw_priv,
				 struct sk_buff * skb);
#if 0
		...
		if (Sstar_rx_directly(hw_priv, skb, cb_handle) == true) {
			...
			usb_fill_bulk_urb(rx_urb->test_urb,
					  self->drvobj->pusbdev,
					  usb_rcvbulkpipe(self->drvobj->pusbdev,
							  self->drvobj->ep_in),
					  skb->data, RX_BUFFER_SIZE,
					  Sstar_usb_receive_data_complete,
					  rx_urb);
			usb_anchor_urb(rx_urb->test_urb,
				       &self->drvobj->rx_submitted);
			status = usb_submit_urb(rx_urb->test_urb, GFP_ATOMIC);
			if (status) {
				usb_unanchor_urb(rx_urb->test_urb);
				Sstar_printk_err(
					"receive_data usb_submit_urb ++ ERR %d\n",
					status);
				goto __free;
			}
			return;
				  ...
				  sbus_callback_handler hander)
{
	...
	if (Sstar_wifi_get_status() >= 2) {
		Sstar_printk_err(
			"Sstar_usb_receive_data drop urb req because rmmod driver\n");
		status = 3;
		goto system_err;
	}

	urb_id = Sstar_usb_urb_get(self, self->drvobj->rx_urb_map, RX_URB_NUM,
				   0);
	if (urb_id < 0) {
		status = -4;
		goto system_err;
	...
	if (rx_urb->test_skb == NULL) {
		if (dst) {
			...
			if (!skb) {
				status = -1;
				Sstar_printk_err(
					"Sstar_usb_receive_data++ Sstar_dev_alloc_skb %p ERROR\n",
					skb);
				goto __err_skb;
			}
	...
	usb_fill_bulk_urb(rx_urb->test_urb, self->drvobj->pusbdev, pipe,
			  skb->data, count, Sstar_usb_receive_data_complete,
			  rx_urb);
...
static int Sstar_usb_memcpy_fromio(struct sbus_priv *self, unsigned int addr,
				   void *dst, int count)
{
	int i = 0;
...
#endif
static int Sstar_usb_memcpy_toio(struct sbus_priv *self, unsigned int addr,
...
static int Sstar_usb_memcpy_fromio_async(struct sbus_priv *self,
					 unsigned int addr, void *dst,
					 int count, sbus_callback_handler func)
{
	int ret = 0;
...
static int Sstar_usb_memcpy_toio_async(struct sbus_priv *self,
				       unsigned int addr, const void *src,
				       int count, sbus_callback_handler func)
{
	int ret = 0;
...
static int Sstar_usb_lock_reset(struct sbus_priv *self)
{
	...
	result = usb_lock_device_for_reset(
		interface_to_usbdev(self->drvobj->pusbintf),
		self->drvobj->pusbintf);
	if (result < 0) {
		Sstar_printk_err("unable to lock device for reset :%d\n",
				 result);
	} else {
		result = usb_reset_device(
			interface_to_usbdev(self->drvobj->pusbintf));
...
#endif
static int Sstar_usb_irq_subscribe(struct sbus_priv *self,
...
static int Sstar_wtd_process(void *arg)
{
	...
	while (1) {
		...
		while (waittime-- > 0) {
			...
			if (wtd_probe != 0) {
				Sstar_printk_exit(
					"[Sstar_wtd]:wtd_probe(%d) have done\n",
					wtd_probe);
				break;
			}
		}
...
static void Sstar_wtd_init(void)
{
	...
	g_wtd.wtd_thread =
		kthread_create(&Sstar_wtd_process, &g_wtd, "Sstar_wtd");
...
#else
		sched_setscheduler(g_wtd.wtd_thread, SCHED_FIFO, &param);
#endif
...
static struct dvobj_priv *usb_dvobj_init(struct usb_interface *usb_intf)
{
	...
	for (i = 0; i < pdvobjpriv->nr_endpoint; i++) {
		...
		if (phost_endp) {
			...
			if (usb_endpoint_is_bulk_in(pendp_desc)) {
				Sstar_dbg(SSTAR_APOLLO_DBG_MSG,
					  "usb_endpoint_is_bulk_in = %x\n",
					  usb_endpoint_num(pendp_desc));
				pdvobjpriv->ep_in_size =
					le16_to_cpu(pendp_desc->wMaxPacketSize);
				pdvobjpriv->ep_in =
					usb_endpoint_num(pendp_desc);
			} else if (usb_endpoint_is_bulk_out(pendp_desc)) {
				Sstar_dbg(SSTAR_APOLLO_DBG_MSG,
					  "usb_endpoint_is_bulk_out = %x\n",
					  usb_endpoint_num(pendp_desc));
				pdvobjpriv->ep_out_size =
					le16_to_cpu(pendp_desc->wMaxPacketSize);
				pdvobjpriv->ep_out =
					usb_endpoint_num(pendp_desc);
			}
	...
	Sstar_dbg(SSTAR_APOLLO_DBG_MSG,
		  ...
		  pdvobjpriv->nr_endpoint, pdvobjpriv->ep_in,
		  pdvobjpriv->ep_out);
exit:
	return pdvobjpriv;
}
					...
					enum Sstar_system_action action)
{
	...
	case SSTAR_SYSTEM_RMMOD:
		...
		Sstar_mod_timer(&local->resume_timer,
					     ...
					     enum Sstar_system_action action)
{
	...
	*/
	...
	ieee80211_iterate_active_interfaces_atomic(
		hw, Sstar_usb_sta_deauthen_iterate_atomic_cb, (void *)hw_priv);
	/*
	*make sure beacon_connection_loss_work has been processed
	*/
	Sstar_flush_workqueue(local->workqueue);
	/*
...
{
	struct sbus_priv *self =
		container_of(h, struct sbus_priv, Sstar_early_suspend);
...
static void Sstar_enable_early_suspend(struct sbus_priv *self)
{
	...
	down(&self->early_suspend_lock);
	atomic_set(&self->early_suspend_state, 0);
	self->Sstar_early_suspend.level =
		EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 100;
			     ...
			     const struct usb_device_id *id)
{
	...
	if (Sstar_wifi_get_status()) {
		Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
			  "[Sstar]Can't probe USB. when wifi_module_exit");
	...
	self->drvobj = dvobj;
	/*2---alloc rx_urb*/
	dvobj->suspend_skb = __Sstar_dev_alloc_skb(
		RX_BUFFER_SIZE + 64,
		GFP_KERNEL); //dev_alloc_skb(RX_BUFFER_SIZE+64);
...
#endif //CONFIG_USE_DMA_ADDR_BUFFER
	status = Sstar_usb_urb_malloc(self, dvobj->rx_urb, RX_URB_NUM,
				      RX_BUFFER_SIZE, 1, 0);
	if (status != 0) {
	...
	/*3---alloc tx_urb*/
	status = Sstar_usb_urb_malloc(self, dvobj->tx_urb, TX_URB_NUM,
				      TX_BUFFER_SIZE, 0, 1);
...
#else
	self->drvobj->pusbdev->autosuspend_disabled =
		0; //autosuspend disabled by the user
#endif
#ifdef CONFIG_PM
	...
	//initial wifi mac
	status = Sstar_core_probe(&Sstar_usb_sbus_ops, self, &intf->dev,
				  &self->core);
...
static void __Sstar_usb_disconnect(struct usb_interface *intf)
{
	...
	if (dvobj) {
		...
		if (g_rebootSystem)
			...
			if (interface_to_usbdev(intf)->state !=
...
wow_resume:
	if (ret != 0) {
		...
		*/
		...
		highlight_debug(
			"[Sstar usb wifi] resume fail,waitting disconnect.....");
	} else {
...
static int Sstar_usb_pre_reset(struct usb_interface *intf)
{
	...
	*/
	...
	Sstar_printk_init("%s:%d\n", __func__,
			  interface_to_usbdev(intf)->state);
error:
	return ret;
}
...
{
	struct Sstar_usb_driver_ref *driver_ref =
		container_of(kref, struct Sstar_usb_driver_ref, ref);
...
static int Sstar_usb_pm_notifier(struct notifier_block *b,
				 unsigned long pm_event, void *d)
{
	switch (pm_event) {
...
static int Sstar_process_each_dev(struct usb_device *usb_dev, void *data)
{
	...
	if ((usb_dev->descriptor.idVendor != WIFI_USB_VID) ||
	    (usb_dev->descriptor.idProduct != WIFI_USB_PID)) {
		goto exit;
	}
retry:
	...
	if (intf && (interface_to_usbdev(intf) == usb_dev)) {
		...
		for_each_netdev (&init_net, dev) {
			...
			if (dev->ieee80211_ptr->wiphy->privid !=
...
{
	int loop = 0;
	Sstar_printk_exit("wifi_tx_urb_pending(%d)\n", wifi_tx_urb_pending);
			      ...
			      struct dvobj_priv *dvobj)
{
	...
	*/
	...
	if (atomic_read(&g_wtd.wtd_probe) &&
	    ((hw_priv->wsm_caps.HiHwCnfBufaddr & 0xFFF00000) == 0x9000000)) {
		Sstar_printk_exit("Sstar_usb_hw_write_port  0x87690121\n");

		regdata = 0x87690121;
		Sstar_usb_hw_write_port(dvobj->self,
					hw_priv->wsm_caps.HiHwCnfBufaddr,
					&regdata, 4);
		loop = 0;
		...
		while (regdata != 0x87690122) {
			mdelay(10);
			Sstar_usb_hw_read_port(
				dvobj->self,
				hw_priv->wsm_caps.HiHwCnfBufaddr + 4, &regdata,
				4);
			if (regdata == 0x87690122) {
				Sstar_printk_exit(
					"wait usb lmac rmmod ok !!!!!!!!\n");
				break;
			}
			if (loop++ > 100) {
				Sstar_printk_exit(
					"wait usb lmac rmmod fail !!!!!!!!\n");
				break;
			}
		}
	}
...
{
	Sstar_usb_module_lock();
	Sstar_usb_station_diconnect_sync(Sstar_hw_priv_dereference(),
					 SSTAR_SYSTEM_RMMOD);
	Sstar_usb_fw_sync(Sstar_hw_priv_dereference(),
			  Sstar_usb_dvobj_dereference());
	Sstar_usb_module_unlock();
}
static void Sstar_usb_module_exit(void)
{
	Sstar_printk_exit(
		"Sstar_usb_module_exit g_dvobj %p wifi_usb_probe_doing %d\n",
