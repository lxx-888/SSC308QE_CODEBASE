...
#endif
extern void
ieee80211_ap_setchannel_fail(struct ieee80211_sub_if_data *sdata,
...
extern int ieee80211_set_channel(struct wiphy *wiphy, struct net_device *netdev,
				 struct ieee80211_channel *chan,
				 enum nl80211_channel_type channel_type);
int cfg80211_altmtest_reply(struct wiphy *wiphy, const void *data, int len)
{
	return 0;
}
		    ...
		    struct vif_params *params)
{
	...
	Sstar_printk_init("add iftype(%d),if_cnt(%d),flags(%p)\n", type,
			  interface_cnt, flags);
	if (interface_cnt >= 2) {
		if (type != NL80211_IFTYPE_MONITOR) {
			err = -ENOMEM;
			Sstar_printk_err("cannot add interface type(%d)\n",
					 type);
			return ERR_PTR(err);
		}
		...
		if (!flags || !(*flags & MONITOR_FLAG_COOK_FRAMES)) {
			err = -ENOMEM;
			Sstar_printk_err("cannot add interface monitor\n");
			return ERR_PTR(err);
		}
	}
	if (local->only_monitors) {
		if (local->monitor_sdata) {
			Sstar_printk_err(
				"[%s] in monitor mode,so can not add sdata\n",
				  ...
				  struct vif_params *params)
{
	...
	if (type == NL80211_IFTYPE_AP_VLAN && params && params->use_4addr == 0)
		RCU_INIT_POINTER(sdata->u.vlan.sta, NULL);
	else if (type == NL80211_IFTYPE_STATION && params &&
		 params->use_4addr >= 0)
...
int Sstar_change_iface_to_monitor(struct net_device *dev)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4, 13, 0))
	return ieee80211_change_iface(hw->wiphy, dev, NL80211_IFTYPE_MONITOR,
				      NULL, NULL);
#else
	return ieee80211_change_iface(hw->wiphy, dev, NL80211_IFTYPE_MONITOR,
				      NULL);
			     ...
			     struct key_params *params)
{
	...
	if (mac_addr) {
		...
		if (!sta) {
			Sstar_printk_err("%s:[%pM] is not in sta list\n",
					 __func__, mac_addr);
...
static int
ieee80211_get_key(struct wiphy *wiphy, struct net_device *dev, u8 key_idx,
		  ...
		  void (*callback)(void *cookie, struct key_params *params))
{
	...
	case WLAN_CIPHER_SUITE_TKIP:
		...
		if (key->flags & KEY_FLAG_UPLOADED_TO_HARDWARE)
			drv_get_tkip_seq(sdata->local, key->conf.hw_key_idx,
					 &iv32, &iv16);
...
static void rate_idx_to_bitrate(struct rate_info *rate, struct sta_info *sta,
				int idx)
{
	struct ieee80211_channel_state *chan_state =
		ieee80211_get_channel_state(sta->local, sta->sdata);
	if (!(rate->flags & RATE_INFO_FLAGS_MCS)) {
		struct ieee80211_supported_band *sband;
		sband = sta->local->hw.wiphy
				->bands[chan_state->conf.channel->band];
...
static void sta_set_sinfo(struct sta_info *sta, struct station_info *sinfo)
{
	...
	sinfo->filled = STATION_INFO_INACTIVE_TIME | STATION_INFO_RX_BYTES |
			STATION_INFO_TX_BYTES | STATION_INFO_RX_PACKETS |
			STATION_INFO_TX_PACKETS | STATION_INFO_TX_RETRIES |
			STATION_INFO_TX_FAILED | STATION_INFO_TX_BITRATE |
			STATION_INFO_RX_BITRATE | STATION_INFO_RX_DROP_MISC |
			STATION_INFO_BSS_PARAM | STATION_INFO_CONNECTED_TIME
	...
	if ((sta->local->hw.flags & IEEE80211_HW_SIGNAL_DBM) ||
		...
		sinfo->signal_avg = (s8)-Sstar_ewma_read(&sta->avg_signal);
	}
				   ...
				   struct beacon_parameters *params)
{
	...
	if (params->interval &&
		...
		sdata->vif.bss_conf.beacon_int = params->interval;
		ieee80211_bss_info_change_notify(sdata, BSS_CHANGED_BEACON_INT);
	}
	...
	new = Sstar_kzalloc(size, GFP_KERNEL);
	if (!new)
		return -ENOMEM;
				      ...
				      struct beacon_parameters *params)
{
	...
	size = sizeof(*new) + new_head_len + new_proberesp_data_ies_len +
	       new_tail_len;
	...
	*(__le16 *)new->head = __cpu_to_le16(IEEE80211_FTYPE_MGMT |
	...
	if (params->tail) {
		...
		while (tail_len >= 6) {
			ie_len = p[1] + 2;
			tail_len -= ie_len;
			if ((p[0] == SSTAR_WLAN_EID_VENDOR_SPECIFIC)
	...
	if (proberesp_ies)
		memcpy(new->proberesp_data_ies, proberesp_ies,
		       new_proberesp_data_ies_len);
	else if (old)
		memcpy(new->proberesp_data_ies, old->proberesp_data_ies,
		       new_proberesp_data_ies_len);
					 ...
					 struct beacon_parameters *params)
{
	...
	new = Sstar_kzalloc(sizeof(struct associate_data) +
	...
	memcpy(new->associate_ies, params->assocresp_ies,
	       params->assocresp_ies_len);
			      ...
			      struct cfg80211_ap_settings *settings)
{
	...
	memcpy(&params.crypto, &settings->crypto,
	...
	if (ieee80211_set_channel(wiphy, dev, chan, chtype)) {
		Sstar_printk_err("set channel err\n");
		return -EBUSY;
	}
#endif
...
static void ieee80211_send_layer2_update(struct sta_info *sta)
{
	...
	skb = Sstar_dev_alloc_skb(sizeof(*msg));
	if (!skb)
		return;
	msg = (struct iapp_layer2_update *)Sstar_skb_put(skb, sizeof(*msg));
				 ...
				 struct station_parameters *params)
{
	...
	if (Sstar_compare_ether_addr(mac, sdata->vif.addr) == 0)
		return -EINVAL;
...
)
{
	...
	if (mac) {
		...
		if (sta) {
			Sstar_printk_cfg(
				"%s:mac(%pM),AUTH(%d),DEAUTHENNING(%d)\n",
				__func__, mac,
		...
		if (deauthen_sta == true) {
			ieee80211_ap_sme_queue_event(
				sdata, STA_EVENT__SEND_DEAUTHEN, mac);
			ieee80211_ap_sme_sta_sync_lock(sdata);
		}

		if (free_sta == true)
			ieee80211_ap_sme_queue_event(sdata, STA_EVENT__FREE,
						     mac);
	} else {
		Sstar_printk_cfg("%s:deauthen all sta\n", __func__);
	}
#endif
	return 0;
}

static int ieee80211_change_station(struct wiphy *wiphy, struct net_device *dev,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 18, 31))
				    u8 *mac,
#else
				    const u8 *mac,
			    ...
			    struct mpath_info *pinfo)
{
	...
	pinfo->filled = MPATH_INFO_FRAME_QLEN | MPATH_INFO_SN |
			MPATH_INFO_METRIC | MPATH_INFO_EXPTIME |
			MPATH_INFO_DISCOVERY_TIMEOUT |
			MPATH_INFO_DISCOVERY_RETRIES | MPATH_INFO_FLAGS;
			   ...
			   const struct mesh_setup *setup)
{
	...
	if (setup->ie_len) {
		new_ie = kmemdup(setup->ie, setup->ie_len, GFP_KERNEL);
		if (!new_ie)
			return -ENOMEM;
	}
					...
					const struct mesh_config *nconf)
{
	...
	if (_chg_mesh_attr(NL80211_MESHCONF_HWMP_NET_DIAM_TRVS_TIME, mask))
...
#endif
static int ieee80211_change_bss(struct wiphy *wiphy, struct net_device *dev,
				struct bss_parameters *params)
{
	...
	if (params->ht_opmode >= 0) {
		sdata->vif.bss_conf.ht_operation_mode = (u16)params->ht_opmode;
				    ...
				    struct ieee80211_txq_params *params)
{
	...
	Sstar_printk_cfg("ieee80211_set_txq_params\n");
	...
	if (sdata == NULL) {
		Sstar_printk_err("txq_params:sdata == NULL\n");
		return -EOPNOTSUPP;
	}
#endif
				   ...
				   enum nl80211_channel_type channel_type)
{
	...
	if (netdev) {
		...
		Sstar_printk_cfg("%s:set monitor interface channel\n",
				 __func__);
		list_for_each_entry (sdata, &local->interfaces, list) {
			if ((sdata->vif.type == NL80211_IFTYPE_MONITOR) &&
			    !(sdata->u.mntr_flags & MONITOR_FLAG_COOK_FRAMES)) {
	...
	case CHAN_MODE_FIXED:
		...
		if (chan_state->_oper_channel_type != channel_type) {
			...
			char oper_type = ieee80211_chw_is_ht40(
	...
	Sstar_printk_cfg("%s:old_oper(%p),oper_channel(%p),old_oper_type %d\n",
			 __func__, old_oper, chan_state->oper_channel,
			 old_oper_type);
			  ...
			  struct cfg80211_scan_request *req)
{
	...
	if (ETF_Test_is_Start()) {
		Sstar_printk_err("ieee80211_scan drop ETF_bStartTx/Rx\n");
		return -EBUSY;
	}
	...
	if (sdata->local->adaptive_started == true &&
	    time_is_after_jiffies(sdata->local->adaptive_started_time +
				  (5 * 60 * HZ))) {
		Sstar_printk_err("%s:adaptive runing\n", __func__);
		return -EBUSY;
	}
	...
	if (sdata->local->only_monitors) {
		if (sdata->local->monitor_sdata) {
			Sstar_printk_err("[%s] in monitor mode,cannot scan\n",
				      ...
				      struct cfg80211_sched_scan_request *req)
{
	...
	if (ETF_Test_is_Start()) {
		Sstar_printk_err(
			"ieee80211_sched_scan_start drop ETF_bStartTx/Rx\n");
		return -EOPNOTSUPP;
	}
				     ...
				     struct net_device *dev)
{
	...
	if (ETF_Test_is_Start()) {
		Sstar_printk_err(
			"ieee80211_sched_scan_stop drop ETF_bStartTx/Rx\n");
		return -EOPNOTSUPP;
	}
...
void ieee80211_start_ap_changechannel_work(
	struct ieee80211_sub_if_data *ap_sdata, int channel, u8 chann_type)
{
...
#define WLAN_REASON_STA_CHANNEL_CHANGE 9999
	...
	Sstar_printk_err(
		"ieee80211_start_ap_changechannel_work chann_type %d channel %d\n",
		chann_type, channel);

	skb = Sstar_dev_alloc_skb(sizeof(struct Sstar_ieee80211_mgmt) + 64);
	Sstar_skb_reserve(skb, 64);
	deauth = (struct Sstar_ieee80211_mgmt *)Sstar_skb_put(
		skb, sizeof(struct Sstar_ieee80211_mgmt));
	WARN_ON(!deauth);
...
#endif
	Sstar_dev_kfree_skb_any(skb);
}
/*
change hostapd channel
*/
void ieee80211_start_ap_changechannel(struct wiphy *wiphy,
				      struct ieee80211_sub_if_data *sdata,
				      struct cfg80211_auth_request *req)
{
	...
	struct ieee80211_channel_state *chan_state =
		ieee80211_get_channel_state(local, sdata);
	struct ieee80211_supported_band *sband =
		local->hw.wiphy->bands[chan_state->conf.channel->band];
	const u8 *ht_information_ie = NULL;
	struct ieee80211_ht_info *hti;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 0))
	if (req->bss->len_proberesp_ies)
		ht_information_ie =
			Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_HT_INFORMATION,
						req->bss->proberesp_ies,
						req->bss->len_proberesp_ies);
	else
		ht_information_ie =
			Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_HT_INFORMATION,
						req->bss->beacon_ies,
						req->bss->len_beacon_ies);
#else

	if (req->bss->proberesp_ies && req->bss->proberesp_ies->len)
		ht_information_ie =
			Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_HT_INFORMATION,
						req->bss->proberesp_ies->data,
						req->bss->proberesp_ies->len);
	else if (req->bss->beacon_ies && req->bss->beacon_ies->len)
		ht_information_ie =
			Sstar_ieee80211_find_ie(SSTAR_WLAN_EID_HT_INFORMATION,
						req->bss->beacon_ies->data,
						req->bss->beacon_ies->len);
	...
	if (ht_information_ie) {
		...
		Sstar_printk_err("ieee80211_start_ap_changechannel no HT\n");
	}
	...
	list_for_each_entry_rcu (ap_sdata, &local->interfaces, list) {
		...
		Sstar_printk_err("ieee80211_start_ap_changechannel_tmp\n");
		...
		ieee80211_start_ap_changechannel_work(
			ap_sdata, channel_hw_value(chan), channel_type);
		break;
	}
}
...
change hostapd channel
*/
void ieee80211_ap_setchannel_fail(struct ieee80211_sub_if_data *ap_sdata,
				  struct ieee80211_channel *chan,
				  enum nl80211_channel_type channel_type)
{
	//find AP interface , change channel
	do {
		...
		Sstar_printk_err("ieee80211_ap_setchannel_fail\n");
		//set channel change to hostapd
		ieee80211_start_ap_changechannel_work(
			ap_sdata, channel_hw_value(chan), channel_type);
		break;
...
//send AP channel event , this is delay work ,because hostapd may have not been setup, we must wait it
void ieee80211_ap_channel_event_work(struct Sstar_work_struct *work)
{
	struct Sstar_delayed_work *delayed_work = Sstar_to_delayed_work(work);
	struct ieee80211_sub_if_data *sdata =
		container_of(delayed_work, struct ieee80211_sub_if_data,
			     ap_channel_event_work);
	//struct ieee80211_sub_if_data *sdata =
	...
	Sstar_printk_err("ieee80211_ap_channel_event_work\n");
	ieee80211_ap_setchannel_fail(sdata, chan_state->oper_channel,
				     chan_state->_oper_channel_type);
}
#endif //#ifdef CONFIG_SSTAR_AP_CHANNEL_CHANGE_EVENT
			  ...
			  struct cfg80211_auth_request *req)
{
	...
	struct ieee80211_channel_state *chan_state =
		ieee80211_get_channel_state(local, sdata);
	if (Sstar_ieee80211_suspend(local) == true) {
		Sstar_printk_err("ieee80211_auth drop:suspend\n");
		return -EBUSY;
	}
	switch (ieee80211_get_channel_mode(local, sdata)) {
	case CHAN_MODE_HOPPING:
		Sstar_printk_err("(%s) authen err CHAN_MODE_HOPPING\n",
				 sdata->name);
		return -EBUSY;
	case CHAN_MODE_FIXED:
		if (chan_state->oper_channel == req->bss->channel)
			break;
		Sstar_printk_err("(%s) authen err CHAN_MODE_FIXED %d  %d\n",
				 sdata->name,
		...
		if (chan_state->oper_channel == req->bss->channel)
			break;
		Sstar_printk_err("(%s) authen  CHAN_CHANGE_OK \n", sdata->name);
#endif //#ifdef CONFIG_SSTAR_AP_CHANNEL_CHANGE_EVENT
		return -EBUSY;
	case CHAN_MODE_UNDEFINED:
		break;
			   ...
			   struct cfg80211_assoc_request *req)
{
	...
	if (Sstar_ieee80211_suspend(local) == true) {
		Sstar_printk_err("ieee80211_assoc drop:suspend\n");
		return -EBUSY;
	}
	switch (ieee80211_get_channel_mode(local, sdata)) {
	case CHAN_MODE_HOPPING:
...
static void ieee80211_get_ringparam(struct wiphy *wiphy, u32 *tx, u32 *tx_max,
				    u32 *rx, u32 *rx_max)
{
	struct ieee80211_local *local = wiphy_priv(wiphy);
...
#else
int ieee80211_altmtest_cmd(struct wiphy *wiphy, struct wireless_dev *wdev,
			   void *data, int len)
{
	int err = 0;
	struct ieee80211_local *local = wiphy_priv(wiphy);
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 1, 0))
static int ieee80211_testmode_dump(struct wiphy *wiphy, struct sk_buff *skb,
			     ...
			     enum ieee80211_smps_mode smps_mode)
{
	...
	if (old_req == smps_mode && smps_mode != IEEE80211_SMPS_AUTOMATIC)
		return 0;
	...
	err = ieee80211_send_smps_action(sdata, smps_mode, ap, ap);
	if (err)
...
static int ieee80211_set_cqm_rssi_config(struct wiphy *wiphy,
					 struct net_device *dev, s32 rssi_thold,
					 u32 rssi_hyst)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	struct ieee80211_local *local = wdev_priv(dev->ieee80211_ptr);
				    ...
				    struct sk_buff *txskb)
{
...
#endif
	if (Sstar_ieee80211_suspend(local) == true) {
		Sstar_printk_err("ieee80211_start_roc_work drop:suspend\n");
		return -EBUSY;
	}
	roc = Sstar_kzalloc(sizeof(*roc), GFP_KERNEL);
	if (!roc)
		return -ENOMEM;
	...
	roc->sdata = sdata;
	SSTAR_INIT_DELAYED_WORK(&roc->work, ieee80211_sw_roc_work);
	...
	Sstar_printk_cfg("%s:roc->cookie(%llx)\n", __func__, roc->cookie);

#ifdef CONFIG_SSTAR_STA_LISTEN
	local->hw_roc_channel = channel;
	__ieee80211_recalc_idle(local);
#endif
	ret = drv_remain_on_channel(
		local, sdata, channel, NL80211_CHAN_NO_HT, duration,
...
out_check_combine:
	list_for_each_entry (tmp, &local->roc_list, list) {
		...
		if (!tmp->started) {
			...
			queued = true;
			Sstar_printk_cfg("%s:roc not start add dependents\n",
					 __func__);
			break;
		}
		...
		if (local->ops->remain_on_channel) {
			...
			if (!tmp->hw_begun) {
				...
				queued = true;
				Sstar_printk_cfg(
					"%s:hw roc not started add dependents\n",
					__func__);
				break;
			}

			if (time_before(
				    j + IEEE80211_ROC_MIN_LEFT,
				    tmp->hw_start_time +
					    msecs_to_jiffies(
						    tmp->duration +
						    tmp->hw_extend_roc_time))) {
				int new_dur;
				...
				queued = true;
				Sstar_printk_cfg(
					"%s:hw roc has been started add dependents\n",
					__func__);
			} else {
				Sstar_printk_cfg(
					"%s:remain on channel time duration is not enough\n",
					__func__);
			}
		}
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
		else if (Sstar_del_timer_sync(&tmp->work.timer)) {
			...
			if (time_after(new_end, tmp->work.timer.expires))
				Sstar_mod_timer(&tmp->work.timer, new_end);
			else
				Sstar_add_timer(&tmp->work.timer);
...
out_queue:
	if ((!queued) && (!work_on))
		...
		if ((local->roc_pendding == NULL) &&
		    (local->roc_pendding_sdata == NULL)) {
			local->roc_pendding = roc;
			local->roc_pendding_sdata = sdata;
			Sstar_printk_cfg(
				"%s:work_list is not empty, pendding remain on channel skb(%p),ch(%d)\n",
			...
			Sstar_printk_cfg(
				"%s:work_list is not empty, has one pending,so cancle skb(%p),ch(%d)\n",
				__func__, txskb, channel_hw_value(channel));
			Sstar_kfree(roc);
			return -EBUSY;
		}
	} else if (!queued) {
		Sstar_printk_cfg("%s:work_list(%d), ,cancle skb(%p),ch(%d)\n",
				 __func__, work_on, txskb,
...
#endif
	if (pended == true)
		Sstar_printk_cfg("%s:pended cookie(%x)\n", __func__,
				 (unsigned int)(*cookie));

	Sstar_printk_cfg("%s:return cookie(%llx)\n", __func__, *cookie);
	return 0;
}
...
static int ieee80211_cancel_roc(struct ieee80211_local *local, u64 cookie,
				bool mgmt_tx)
{
	...
	while (local->roc_pendding && local->roc_pendding_sdata) {
		...
		mutex_unlock(&local->mtx);
		Sstar_printk_cfg("%s:cancle pendding roc\n", __func__);
	...
	if (local->ops->remain_on_channel) {
		...
		Sstar_flush_delayed_work(&found->work);
	}
			     ...
			     u64 *cookie)
{
	...
	const struct Sstar_ieee80211_mgmt *mgmt = (void *)buf;
	...
	if (chan != chan_state->tmp_channel && chan != chan_state->oper_channel)
		is_offchan = true;
	...
	skb = Sstar_dev_alloc_skb(local->hw.extra_tx_headroom + len);
	if (!skb) {
		return -ENOMEM;
	}
	Sstar_skb_reserve(skb, local->hw.extra_tx_headroom);

	memcpy(Sstar_skb_put(skb, len), buf, len);
	...
	if (!list_empty(&local->roc_list)) {
		roc = list_first_entry(&local->roc_list,
				       struct ieee80211_roc_work, list);
		j = jiffies;
		if (roc->started)
			rem = jiffies_to_msecs(
				roc->hw_start_time +
				msecs_to_jiffies(roc->duration +
						 roc->hw_extend_roc_time) -
				j);
	}
...
#endif
#ifdef SSTAR_POWERSAVE_SAVE_GROUP_KEY
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 0, 8))
static int ieee80211_set_rekey_data(struct wiphy *wiphy, struct net_device *dev,
...
{
	u8 *pos = (void *)Sstar_skb_put(skb, 7);

	*pos++ = SSTAR_WLAN_EID_EXT_CAPABILITY;
	*pos++ = 5; /* len */
	*pos++ = 0x0;
	*pos++ = 0x0;
	*pos++ = 0x0;
				       ...
				       u8 *peer, u8 *bssid)
{
	...
	lnkid = (void *)Sstar_skb_put(skb, sizeof(struct ieee80211_tdls_lnkie));
	lnkid->ie_type = SSTAR_WLAN_EID_LINK_ID;
					  ...
					  u16 status_code, struct sk_buff *skb)
{
	...
	case WLAN_TDLS_SETUP_REQUEST:
		tf->category = SSTAR_WLAN_CATEGORY_TDLS;
		tf->action_code = WLAN_TDLS_SETUP_REQUEST;

		Sstar_skb_put(skb, sizeof(tf->u.setup_req));
	...
	case WLAN_TDLS_SETUP_RESPONSE:
		...
		Sstar_skb_put(skb, sizeof(tf->u.setup_resp));
	...
	case WLAN_TDLS_SETUP_CONFIRM:
		...
		Sstar_skb_put(skb, sizeof(tf->u.setup_cfm));
	...
	case WLAN_TDLS_TEARDOWN:
		...
		Sstar_skb_put(skb, sizeof(tf->u.teardown));
	...
	case WLAN_TDLS_DISCOVERY_REQUEST:
		...
		Sstar_skb_put(skb, sizeof(tf->u.discover_req));
...
#endif
	mgmt = (void *)Sstar_skb_put(skb, 24);
	memset(mgmt, 0, 24);
...
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 1, 0))
		Sstar_skb_put(skb, 1 + sizeof(struct tdls_discover_resp_s));
#else
		Sstar_skb_put(skb,
			       ...
			       size_t extra_ies_len)
{
...
#ifdef CONFIG_MAC80211_SSTAR_VERBOSE_TDLS_DEBUG
	Sstar_printk_cfg("TDLS mgmt action %d peer %pM\n", action_code, peer);
#endif

	skb = Sstar_dev_alloc_skb(local->hw.extra_tx_headroom +
				  max(sizeof(struct Sstar_ieee80211_mgmt),
	...
	if (extra_ies_len)
		memcpy(Sstar_skb_put(skb, extra_ies_len), extra_ies,
		       extra_ies_len);
	...
	case WLAN_TDLS_SETUP_REQUEST:
	case WLAN_TDLS_SETUP_RESPONSE:
		Sstar_skb_set_queue_mapping(skb, IEEE80211_AC_BK);
		skb->priority = 2;
		break;
	default:
...
#ifdef CONFIG_MAC80211_SSTAR_VERBOSE_TDLS_DEBUG
	Sstar_printk_cfg("TDLS oper %d peer %pM\n", oper, peer);
#endif
...
)
{
	...
	if (sdata->radar_required == true) {
		Sstar_printk_err("radar cac is running\n");
		return -EBUSY;
	}
