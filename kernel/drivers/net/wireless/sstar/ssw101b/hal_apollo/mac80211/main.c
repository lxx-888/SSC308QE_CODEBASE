...
ieee80211_recalc_channel(struct ieee80211_local *local,
			 struct ieee80211_sub_if_data *sdata, u32 *changed)
{
...
int ieee80211_hw_config(struct ieee80211_local *local, u32 changed)
{
	struct ieee80211_channel *chan =
		NULL; /* Only used when multi channel is off */
	...
	if (local->hw.flags & IEEE80211_HW_SUPPORTS_MULTI_CHANNEL) {
		...
		list_for_each_entry_rcu (sdata, &local->interfaces, list)
			power = max(power,
				    sdata->chan_state.conf.channel->max_power);
		list_for_each_entry_rcu (sdata, &local->interfaces, list)
			power = min(power,
				    (sdata->chan_state.conf.channel->max_power -
...
restart:
	...
	if (queue_len >= 1000) {
		Sstar_printk_err("process %d skbs\n", queue_len);
	}
#if 0
...
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
static void ieee80211_restart_work(struct Sstar_work_struct *work)
{
...
void ieee80211_restart_hw(struct ieee80211_hw *hw)
{
	...
	Sstar_schedule_work(&local->restart_work);
}
...
int ieee80211_restart_hw_sync(struct ieee80211_hw *hw)
{
	...
	list_for_each_entry (sta, &local->sta_list, list) {
		if (sta->uploaded) {
			...
			if (sdata->vif.type == NL80211_IFTYPE_AP) {
				WARN_ON(ieee80211_rx_sta_cook_deauthen(sta) ==
					false);
				WARN_ON(ieee80211_tx_sta_deauthen(sta) ==
					false);
			}
...
static void ieee80211_recalc_smps_work(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local =
...
#endif
#ifdef CONFIG_INET
static int ieee80211_ifa_changed(struct notifier_block *nb, unsigned long data,
				 void *arg)
{
	struct in_ifaddr *ifa = arg;
	struct ieee80211_local *local =
		container_of(nb, struct ieee80211_local, ifa_notifier);
	...
	if (sdata->vif.type == NL80211_IFTYPE_STATION) {
		if (sta_associated &&
			...
			Sstar_printk_debug("(%s):IPv4 enable,end_time(%d)\n",
					   sdata->name,
...
#ifdef IPV6_FILTERING
static void ieee80211_ifa6_changed_work(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local =
	...
	if (sdata->vif.type == NL80211_IFTYPE_STATION) {
		if (ifmgd->associated &&
			...
			mutex_lock(&local->mtx);
			ieee80211_cancle_connecting_work(
				sdata, ifmgd->associated->bssid, false);
			mutex_unlock(&local->mtx);
		}
	}
}

static int ieee80211_ifa6_changed(struct notifier_block *nb, unsigned long data,
				  void *arg)
{
	...
	list_for_each_entry (ifa, &idev->addr_list, if_list) {
		...
		if (c > IEEE80211_BSS_NDP_ADDR_LIST_LEN)
			break;
		memcpy(&bss_conf->ndp_addr_list[c - 1], &ifa->addr,
	...
	if (sdata->vif.type == NL80211_IFTYPE_STATION) {
		...
		Sstar_schedule_work(&local->ifa6_changed_work);
#if 0
                /* Configure driver only if associated */
                if (ifmgd->associated) {
                        bss_conf->ndp_filter_enabled = sdata->ndp_filter_state;
...
void ieee80211_resume_timer(unsigned long data)
{
	...
	Sstar_printk_pm("end resume\n");
}

bool ieee80211_clear_extra_ie(struct ieee80211_sub_if_data *sdata,
			      enum ieee80211_special_work_type type)
{
	...
	case IEEE80211_SPECIAL_AP_SPECIAL_PROBRSP:
		...
		if (type == IEEE80211_SPECIAL_AP_SPECIAL_BEACON) {
			old_extra = (union iee80211_extra_ie *)rtnl_dereference(
				sdata->u.ap.beacon_extra);

		} else {
			old_extra = (union iee80211_extra_ie *)rtnl_dereference(
				sdata->u.ap.probe_response_extra);
		}
	...
	case IEEE80211_SPECIAL_STA_SPECIAL_PROBR:
		...
		old_extra = (union iee80211_extra_ie *)rtnl_dereference(
			sdata->u.mgd.probe_request_extra);
	...
	new_extra = Sstar_kzalloc(sizeof(union iee80211_extra_ie), GFP_KERNEL);
	...
	case IEEE80211_SPECIAL_AP_SPECIAL_PROBRSP:
		...
		if (type == IEEE80211_SPECIAL_AP_SPECIAL_BEACON)
			rcu_assign_pointer(sdata->u.ap.beacon_extra,
					   &new_extra->beacon);
		else
			rcu_assign_pointer(sdata->u.ap.probe_response_extra,
					   &new_extra->proberesponse);

		synchronize_rcu();

		ieee80211_bss_info_change_notify(
			sdata, BSS_CHANGED_BEACON_ENABLED | BSS_CHANGED_BEACON |
				       BSS_CHANGED_SSID);

		break;
	case IEEE80211_SPECIAL_STA_SPECIAL_PROBR:
		rcu_assign_pointer(sdata->u.mgd.probe_request_extra,
				   &new_extra->proberequest);
		synchronize_rcu();
		break;
	default:
		break;
			       ...
			       union iee80211_extra_ie *extra)
{
	...
	new_extra = Sstar_kzalloc(sizeof(union iee80211_extra_ie) +
	...
	new_extra->extra.extra =
		((u8 *)new_extra) + (sizeof(union iee80211_extra_ie));
	new_extra->extra.extra_len = extra->extra.extra_len;

	memcpy(new_extra->extra.extra, extra->extra.extra,
	       extra->extra.extra_len);
				...
				bool clear)
{
	...
	if (clear == false) {
		new_conf = Sstar_kzalloc(
			sizeof(struct ieee80211_internal_ap_conf), GFP_KERNEL);

		if (new_conf == NULL) {
			return false;
		}

		memcpy(new_conf, conf,
		       sizeof(struct ieee80211_internal_ap_conf));
		Sstar_printk_debug("%s:ssid[%s],bssid[%pM],channel[%d]\n",
				   __func__, conf->ssid, conf->bssid,
				   conf->channel);
	} else {
		new_conf = NULL;
	}
...
#endif
static bool ieee80211_special_work_scan_cb(
	struct ieee80211_sub_if_data *sdata, void *scan_store,
...
static void ieee80211_special_work(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local =
	...
	while ((skb = Sstar_skb_dequeue(&local->special_req_list))) {
		...
		*/
		...
		case IEEE80211_SPECIAL_STA_SPECIAL_PROBR: {
			...
			special_update =
				(struct ieee80211_update_special *)skb->cb;
			/*
				*special ie err
				*/
			if ((special_update->special_len == 0) ||
			    (special_update->special_ie == NULL)) {
				ieee80211_clear_extra_ie(
					sdata,
			...
			ieee80211_updata_extra_ie(
				sdata,
				(enum ieee80211_special_work_type)skb->pkt_type,
				&extra);
			break;
		}
		...
		case IEEE80211_SPECIAL_STA_POSITIVE_SCAN: {
			...
			special_scan =
				(struct ieee80211_special_work_scan *)skb->cb;
			memset(&req, 0,
			...
			if (special_scan->bssid) {
				...
				Sstar_printk_err("[Scan Bssid][%pM]\n",
						 req.bssid);
			}
			...
			rcu_assign_pointer(req.result_handle,
					   ieee80211_special_work_scan_cb);
				   ...
				   struct ieee80211_special_freq *special)
{
	...
	hash_index = Sstar_hash_index((u8 *)(&special->channel), sizeof(void *),
	...
	hlist_for_each (node, hlist) {
		freq_node =
			hlist_entry(node, struct ieee80211_special_freq, hnode);
	...
	if (freq_node_target == NULL) {
		freq_node_target = Sstar_kzalloc(
			sizeof(struct ieee80211_special_freq), GFP_ATOMIC);
		...
		Sstar_printk_debug("%s:channel[%d] freq[%d]->freq[%d]\n",
				   __func__, channel_hw_value(special->channel),
				   channel_center_freq(special->channel),
				   special->freq);
	}
...
static void ieee80211_name_free(struct ieee80211_local *local)
{
	...
	while (!list_empty(&local->ieee80211_name_list)) {
		def = list_first_entry(&local->ieee80211_name_list,
				       struct ieee80211_name_def, list);
		list_del(&def->list);
		Sstar_kfree(def);
	}
					...
					const struct ieee80211_ops *ops)
{
	...
	wiphy->flags |= WIPHY_FLAG_NETNS_OK
			...
			| WIPHY_FLAG_4ADDR_AP | WIPHY_FLAG_4ADDR_STATION
#endif
	...
	SSTAR_INIT_DELAYED_WORK(&local->scan_work, ieee80211_scan_work);

	ieee80211_work_init(local);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	SSTAR_INIT_WORK(&local->restart_work, ieee80211_restart_work);
#endif
	SSTAR_INIT_WORK(&local->special_work, ieee80211_special_work);
	Sstar_skb_queue_head_init(&local->special_req_list);
	Sstar_common_hash_list_init(local->special_freq_list,
				    SSTAR_COMMON_HASHENTRIES);

#ifdef CONFIG_INET
#ifdef IPV6_FILTERING
	SSTAR_INIT_WORK(&local->ifa6_changed_work, ieee80211_ifa6_changed_work);
#endif /*IPV6_FILTERING*/
#endif
#ifdef CONFIG_SSTAR_SMPS
	SSTAR_INIT_WORK(&local->recalc_smps, ieee80211_recalc_smps_work);
#endif
	local->smps_mode = IEEE80211_SMPS_OFF;
#ifdef CONFIG_SSTAR_SUPPORT_SCHED_SCAN
	SSTAR_INIT_WORK(&local->sched_scan_stopped_work,
...
#endif
	atomic_set(&local->resume_timer_start, 0);
	Sstar_setup_timer(&local->resume_timer, ieee80211_resume_timer,
			  (unsigned long)local);
...
#endif
static char *init_dev_name = "SIGMASTAR";
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
	 ...
	 */
	local->scan_ies_len =
		4 + max_bitrates /* (ext) supp rates */ + 3 /* DS Params */;
	if (supp_ht)
	...
	local->workqueue =
		Sstar_alloc_ordered_workqueue(wiphy_name(local->hw.wiphy), 0);
	if (!local->workqueue) {
		result = -ENOMEM;
		goto fail_workqueue;
	}
...
#endif
	if (result < 0)
		Sstar_printk_err("Failed to initialize wep: %d\n", result);
#endif
	rtnl_lock();
	...
	(NEED_P2P0_INTERFACE == 1) //CDLINUX no need p2p0
...
#endif
	sta_info_stop(local);
	Sstar_destroy_workqueue(local->workqueue);
fail_workqueue:
...
void ieee80211_unregister_hw(struct ieee80211_hw *hw)
{
	...
	atomic_set(&local->resume_timer_start, 0);
	Sstar_del_timer_sync(&local->resume_timer);
	/*
	 * Now all work items will be gone, but the
	 * timer might still be armed, so delete it
	 */
	Sstar_del_timer_sync(&local->work_timer);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	Sstar_cancel_work_sync(&local->restart_work);
#endif
	Sstar_cancel_work_sync(&local->special_work);
#ifdef IPV6_FILTERING
#ifdef CONFIG_INET
	Sstar_cancel_work_sync(&local->ifa6_changed_work);
#endif
#endif /*IPV6_FILTERING*/
	...
	if (Sstar_skb_queue_len(&local->skb_queue) ||
		...
		Sstar_printk_warn("skb_queue not empty\n");
