...
static void ieee80211_key_disable_hw_accel(struct ieee80211_key *key)
{
	...
	ret = drv_set_key(key->local, DISABLE_KEY, sdata, sta, &key->conf);
...
static void __ieee80211_key_replace(struct ieee80211_sub_if_data *sdata,
				    struct sta_info *sta, bool pairwise,
					  ...
					  const u8 *seq)
{
	...
	case WLAN_CIPHER_SUITE_AES_CMAC:
		 ...
		 * it does not need to be initialized for every packet.
		 */
		key->u.aes_cmac.tfm = ieee80211_aes_cmac_key_setup(key_data);
 ...
 */
static void ieee80211_key_free_rcu(struct rcu_head *rcu_h)
{
	struct ieee80211_key *key =
