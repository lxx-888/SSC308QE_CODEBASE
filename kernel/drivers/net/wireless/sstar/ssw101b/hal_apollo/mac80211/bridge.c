 ...
 ****************************************************************************************
 *
 * Copyright (C)  sigmastar 2015-2018
 *
...
typedef struct list_head Sstar_qentry;
...
struct NET_BR0_INFO_ENTRY {
	Sstar_qentry br0_list;
	Sstar_qentry br0_hash;
...
struct NETWIFI_S_BRIDGE {
	...
	int as_policy;
	Sstar_qentry as_list;
	Sstar_qentry as_hash[BR0_HASHSIZE];
	unsigned long ageing_timer;
...
#if defined(CONFIG_SSTAR_APOLLO_BR0_DEBUG)
#define br_printk Sstar_printk_always
#else
#define br_printk(...)
#endif
/**
 * br0_en_queue - add a new Sstar_qentry in queue tail
 * @list: new entry to be added
 * @head: list head to add it before
 *
 * Insert a new Sstar_qentry in queue tail.
 */
static inline void br0_en_queue(Sstar_qentry *list, Sstar_qentry *head)
{
	list_add_tail(list, head);
}

static inline void br0_en_queue_head(Sstar_qentry *list, Sstar_qentry *head)
{
	list_add(list, head);
}
/**
 * br0_de_queue - deletes Sstar_qentry from queue and reinitialize it.
 * @list: the element to delete from the queue.
 */

static inline void br0_de_queue(Sstar_qentry *list)
{
	list_del_init(list);
}
 ...
 */
static inline int br0_queue_empty(Sstar_qentry *head)
{
...
int br0_attach(struct ieee80211_sub_if_data *sdata)
{
	...
	br0_priv = Sstar_kmalloc(sizeof(struct NETWIFI_S_BRIDGE), GFP_KERNEL);
	if (br0_priv == NULL) {
		Sstar_printk_err("ERROR br0_attach\n\n");
		return 0;
	}
...
static void _br0_free(struct NETWIFI_S_BRIDGE *br0_priv,
		      struct NET_BR0_INFO_ENTRY *br_info)
{
...
static int br0_add(struct ieee80211_sub_if_data *sdata, const u8 *mac,
		   const u8 *ipaddr)
{
	struct NETWIFI_S_BRIDGE *br0_priv = sdata->bridge_priv;
	...
	new = Sstar_kmalloc(sizeof(struct NET_BR0_INFO_ENTRY), GFP_KERNEL);
	if (new == NULL) {
		Sstar_printk_err("ERROR br0_add\n\n");
		return 0;
	}
	...
	BR0_LOCK(br0_priv);
	hash = BR0_HASH(ipaddr);
	br_printk("SstarBr0:add new node %x:%x:%x:%x:%x:%x, ip %d.%d.%d.%d\n",
		  mac[0], mac[1], mac[2], mac[3], mac[4], mac[5], ipaddr[0],
...
static void br0_free_all_locked(struct NETWIFI_S_BRIDGE *br0_priv)
{
	...
	while (!br0_queue_empty(&br0_priv->as_list)) {
		br_info = list_first_entry(&br0_priv->as_list,
					   struct NET_BR0_INFO_ENTRY, br0_list);
		_br0_free(br0_priv, br_info);
	}
...
void br0_info_expire(struct ieee80211_sub_if_data *sdata)
{
	...
	BR0_LOCK(br0_priv);
	list_for_each_entry_safe (br_info, br_info_next, &br0_priv->as_list,
				  br0_list) {
		if (__br0_has_expired(br_info)) {
			u8 *mac = br_info->u.info.macaddr;
			u8 *ipaddr = br_info->u.info.ipaddr;
			Sstar_printk_bh(
				"SstarBr0:<WARNING> br0_info_expire %x:%x:%x:%x:%x:%x, ip %d.%d.%d.%d\n",
...
#endif
static int
ieee80211_brigde_network_find_and_replace(struct ieee80211_sub_if_data *sdata,
					  struct sk_buff *skb,
					  unsigned char *ipaddr)
{
	...
	if (entry) {
		if (!__br0_has_expired(entry)) {
			...
			br_printk(
				"[%s]:SstarBr0:rx change node [%x:%x:%x:%x:%x:%x]->[%x:%x:%x:%x:%x:%x], ip %d.%d.%d.%d\n",
				sdata->name, skb->data[0], skb->data[1],
				skb->data[2], skb->data[3], skb->data[4],
				skb->data[5], entry->u.info.macaddr[0],
				entry->u.info.macaddr[1],
				entry->u.info.macaddr[2],
				entry->u.info.macaddr[3],
				entry->u.info.macaddr[4],
				entry->u.info.macaddr[5], ipaddr[0], ipaddr[1],
				ipaddr[2], ipaddr[3]);
			entry->ageing_timer = jiffies;
		} else {
			Sstar_printk_err("<WARNING>__br0_has_expired\n");
		}
		ret = 1;
	}
	BR0_UNLOCK(br0_priv);
...
int __ieee80211_brigde_change_rxhdr(struct ieee80211_sub_if_data *sdata,
				    struct sk_buff *skb)
{
	...
	//        Sstar_printk_err("rxhdr in (%x),daddr[%pM],saddr[%pM]\n",ehdr->h_proto,skb->data,skb->data+6);
	switch (ehdr->h_proto) {
	case __constant_htons(ETH_P_IP): {
		struct iphdr *iph = (struct iphdr *)(ehdr + 1);

		if (!ieee80211_brigde_network_find_and_replace(
			    sdata, skb, (unsigned char *)&iph->daddr)) {
			if (*((unsigned char *)&iph->daddr + 3) == 0xff) {
				...
				br_printk(
					"Sstar_br0: Replace DA with BR's MAC [%x:%x:%x:%x:%x:%x]->[%x:%x:%x:%x:%x:%x], ip %d.%d.%d.%d\n",
					skb->data[0], skb->data[1],
					skb->data[2], skb->data[3],
					skb->data[4], skb->data[5],
					priv->br_mac[0], priv->br_mac[1],
					priv->br_mac[2], priv->br_mac[3],
					priv->br_mac[4], priv->br_mac[5],
	...
	case __constant_htons(ETH_P_ARP): {
		...
		Sstar_printk_err(
			"arp:ehdr[%pM][%pM],src_devaddr[%pM],tgt_devaddr[%pM],src_ipaddr[%d.%d.%d.%d],tgt_ipaddr[%d.%d.%d.%d]\n",
			skb->data, skb->data + 6, src_devaddr, tgt_devaddr,
			sip[0], sip[1], sip[2], sip[3], tip[0], tip[1], tip[2],
			tip[3]);
		ieee80211_brigde_network_find_and_replace(
			sdata, skb, (unsigned char *)&tgt_ipaddr);
...
int ieee80211_brigde_hash_update(struct ieee80211_sub_if_data *sdata,
				 struct sk_buff *skb)
{
	struct ethhdr *ehdr = (struct ethhdr *)skb->data;
	...
	case __constant_htons(ETH_P_IP): {
		...
		if (((unsigned char *)(iph) + (iph->ihl << 2)) >=
			...
			Sstar_printk_err(" IP packet len error!!\n");
			return -1;
		}
		...
		ieee80211_brigde_network_insert(sdata, skb->data + ETH_ALEN,
						(u8 *)&iph->saddr);
	...
	case __constant_htons(ETH_P_ARP): {
		...
		if (arp->ar_pro != __constant_htons(ETH_P_IP)) {
			Sstar_printk_err("BR0: arp protocol unknown (%4x)!\n",
					 htons(arp->ar_pro));
			return -1;
		}
		...
		ieee80211_brigde_network_insert(sdata, src_devaddr,
						(const u8 *)&src_ipaddr);

		Sstar_printk_err("%s 2: src_devaddr[%pM]tgt_devaddr[%pM] \n",
...
int ieee80211_brigde_change_rxhdr(struct ieee80211_sub_if_data *sdata,
				  struct sk_buff *skb)
{
	struct ethhdr *ehdr = (struct ethhdr *)skb->data;
	int b_vlan_frame = 0;
	...
	if (br_port && ((*(u32 *)br0_priv->br_mac) == 0 &&
			(*(u16 *)(br0_priv->br_mac + 4)) == 0)) {
		Sstar_printk_bh(
			"Re-init br0_netdev_open() due to br_mac==0!\n");
		...
		if ((sdata->vif.type == NL80211_IFTYPE_STATION) &&
			...
			if (ehdr->h_proto == __constant_htons(ETH_P_IP)) {
				...
				if (br0_priv->fast_entry &&
				    !memcmp(br0_priv->fast_ip, &iph->daddr,
					    4)) {
					memcpy(skb->data, br0_priv->fast_mac,
					       ETH_ALEN);
					br0_priv->fast_entry->ageing_timer =
						jiffies;
					need_look = 0;
				}
			}
			if (need_look) {
				ret = __ieee80211_brigde_change_rxhdr(sdata,
								      skb);
			}
	...
	if (memcmp(br0_priv->br_mac, ehdr->h_source, 6) == 0) {
#if 0
		{
			...
			if (deliver_skb) {
				...
				Sstar_dev_queue_xmit(deliver_skb);
			}
		}

#endif
		Sstar_printk_err(
			"BR0[%s]:  sourceHw==br0Hw (%2x:%2x)!h_proto %x,iftype %d\n",
			sdata->name, br0_priv->br_mac[4], br0_priv->br_mac[5],
			ehdr->h_proto, sdata->vif.type);
...
void *ieee80211_brigde_updata_fast_info(struct ieee80211_sub_if_data *sdata,
					unsigned char *mac,
					unsigned char *ipaddr)
{
	struct NETWIFI_S_BRIDGE *br0_priv = sdata->bridge_priv;
...
void ieee80211_tx_set_dhcp_bcast_flag(struct ieee80211_sub_if_data *sdata,
				      struct sk_buff *skb)
{
	//struct ethhdr *ehdr = (struct ethhdr *)skb->data;
	if (skb == NULL)
	...
	if (ieee80211_is_dhcp_frame(skb)) // DHCP request
	{
		...
		struct dhcpMessage *dhcph =
			(struct dhcpMessage *)((u8 *)udph +
					       sizeof(struct udphdr));

		if (dhcph->cookie ==
		    __constant_htonl(DHCP_MAGIC)) // match magic word
		{
			if (!(dhcph->flags &
			      htons(BROADCAST_FLAG))) // if not broadcast
			{
				register int sum = 0;
...
ieee80211_brigde_sdata_check(struct ieee80211_local *local,
			     struct sk_buff **pskb,
			     struct ieee80211_sub_if_data *source_sdata)
{
	...
	if (Sstar_skb_shared(skb) || Sstar_skb_cloned(skb)) {
		skb = Sstar_skb_copy(*pskb, GFP_ATOMIC);
		if (skb == NULL) {
			Sstar_printk_err("%s:skb cope err\n", __func__);
			goto exit_rcu;
		}
		dev_kfree_skb(*pskb);
		*pskb = skb;
	...
	if (ehdr->h_proto == __constant_htons(ETH_P_IP)) {
		...
		if (!is_multicast_ether_addr(ehdr->h_dest)) {
			entry = _br0_find_netinfo_ip(br0_priv,
						     (const u8 *)daip);

			if (entry) {
				sta = rcu_dereference(local->sta_hash[STA_HASH(
					entry->u.info.macaddr)]);
				while (sta) {
					if (memcmp(sta->sta.addr,
						   entry->u.info.macaddr,
						   ETH_ALEN) == 0)
						break;
			...
			if (sta == NULL) {
				sta = rcu_dereference(
					local->sta_hash[STA_HASH(ehdr->h_dest)]);
				while (sta) {
					if (memcmp(sta->sta.addr, ehdr->h_dest,
						   ETH_ALEN) == 0)
						break;
					sta = rcu_dereference(sta->hnext);
				}
		...
		Sstar_printk_debug("%s:dmac[%pM],smac[%pM]\n", __func__,
				   ehdr->h_dest, ehdr->h_source);
		Sstar_printk_debug("%s:dip[%d:%d:%d:%d],sip[%d:%d:%d:%d]\n",
				   __func__, daip[0], daip[1], daip[2], daip[3],
				   saip[0], saip[1], saip[2], saip[3]);
		Sstar_printk_debug("%s:[%s]->[%s],[%pM],sta[%p],entry[%p]\n",
				   __func__, source_sdata->name, sdata->name,
				   ehdr->h_dest, sta, entry);
		...
		struct NET_BR0_INFO_ENTRY *entry =
			_br0_find_netinfo_ip(br0_priv, (const u8 *)daip);
		...
		*/
		if (entry) {
			sta = rcu_dereference(local->sta_hash[STA_HASH(
				entry->u.info.macaddr)]);
			while (sta) {
				if (memcmp(sta->sta.addr, entry->u.info.macaddr,
					   ETH_ALEN) == 0)
					break;
				sta = rcu_dereference(sta->hnext);
			}
		...
		*/
		...
		if (sta == NULL) {
			...
			Sstar_printk_err("%s:mac[%pM] not in sta list\n",
					 __func__, damac);
		} else {
			sdata = sta->sdata;
			damac = sta->sta.addr;
		...
		Sstar_printk_debug("arp:dip[%d:%d:%d:%d],mac[%pM]\n", daip[0],
				   daip[1], daip[2], daip[3], damac);
...
int ieee80211_brigde_change_txhdr(struct ieee80211_sub_if_data *sdata,
				  struct sk_buff **pskb)
{
	...
	if (br_port) {
		...
		*/
...
#endif //0
		{
			...
			if (ehdr->h_proto == __constant_htons(ETH_P_IP)) {
				struct iphdr *iph = (struct iphdr *)(ehdr + 1);
				if ((memcmp(ehdr->h_source, br0_priv->fast_mac,
					    ETH_ALEN) == 0) &&
				    (memcmp(br0_priv->fast_ip, &iph->saddr,
					    4) == 0)) {
					if (br0_priv->fast_entry) {
						br0_priv->fast_entry
							->ageing_timer =
						...
						memset(br0_priv->fast_mac, 0,
						       ETH_ALEN);
						memset(br0_priv->fast_ip, 0, 4);
					}
				} else {
					br0_priv->fast_entry = (struct NET_BR0_INFO_ENTRY
									*)
						ieee80211_brigde_updata_fast_info(
							sdata, ehdr->h_source,
							(unsigned char
								 *)(&iph->saddr));
					if (br0_priv->fast_entry != NULL) {
						memcpy(br0_priv->fast_mac,
						       ehdr->h_source,
						       ETH_ALEN);
						memcpy(br0_priv->fast_ip,
						       &iph->saddr, 4);
						br0_priv->fast_entry
							->ageing_timer =
							jiffies;
						Sstar_printk_debug(
							"ieee80211_brigde_change_txhdr: fast_mac [%x:%x:%x:%x:%x:%x], ip %d.%d.%d.%d\n",
							br0_priv->fast_mac[0],
							br0_priv->fast_mac[1],
							br0_priv->fast_mac[2],
							br0_priv->fast_mac[3],
			...
			if (need_insert) {
				/*
					¸üÐÂ¹þÏ£±í
				*/
				if (ieee80211_brigde_hash_update(sdata, skb) <
				    0) {
					Sstar_printk_err(
						"ieee80211_brigde_change_txhdr: ieee80211_brigde_change_txhdr fail!\n");
					return -1;
				}
			}
			__vlan_hdr_add();
...
void br0_netdev_open(struct net_device *netdev)
{
...

	{
...
#endif // (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 35))
		{
			...
			if (br_netdev == NULL) {
				Sstar_printk_err("br_netdev failed!");
			} else {
				memcpy(br0_priv->br_mac, br_netdev->dev_addr,
				       ETH_ALEN);
			}
#else
			...
			br_netdev =
				dev_get_by_name(devnet, CONFIG_BR_EXT_BRNAME);
			...
			if (br_netdev) {
				...
				printk("%s()-%d: dev_get_by_name(%s) failed!",
				       __FUNCTION__, __LINE__,
				       CONFIG_BR_EXT_BRNAME);
			}
#endif
		} else {
			Sstar_printk_err("%s()-%d: dev_get_by_name(%s)\n",
					 __FUNCTION__, __LINE__,
					 CONFIG_BR_EXT_BRNAME);
		}
