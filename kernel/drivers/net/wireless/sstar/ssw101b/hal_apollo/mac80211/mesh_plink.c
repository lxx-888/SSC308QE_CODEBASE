...
#define mod_plink_timer(s, t) (Sstar_mod_timer(&s->plink_timer, \
				jiffies + HZ * t / 1000))
...
static int mesh_plink_frame_tx(struct ieee80211_sub_if_data *sdata,
	...
	mgmt->u.action.category = SSTAR_WLAN_CATEGORY_SELF_PROTECTED;
...
static void mesh_plink_timer(unsigned long data)
{
	...
	case NL80211_PLINK_HOLDING:
		/* holding timer */
		Sstar_del_timer(&sta->plink_timer);
...
{
	if (Sstar_del_timer_sync(&sta->plink_timer))
		sta->plink_timer_was_running = true;
}
...
void mesh_plink_restart(struct sta_info *sta)
{
	if (sta->plink_timer_was_running) {
		Sstar_add_timer(&sta->plink_timer);
			 ...
			 size_t len, struct ieee80211_rx_status *rx_status)
{
	...
	case NL80211_PLINK_HOLDING:
		switch (event) {
		case CLS_ACPT:
			if (Sstar_del_timer(&sta->plink_timer))
