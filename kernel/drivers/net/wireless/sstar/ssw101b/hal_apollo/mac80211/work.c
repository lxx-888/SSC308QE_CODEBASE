...
static void run_again(struct ieee80211_local *local, unsigned long timeout)
{
	...
	if (!Sstar_timer_pending(&local->work_timer) ||
	    time_before(timeout, local->work_timer.expires))
		Sstar_mod_timer(&local->work_timer, timeout);
}
#if ((LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 40)) ||                        \
     (defined(SSTAR_ALLOC_MEM_DEBUG)))
				...
				enum ieee80211_smps_mode smps)
{
	...
	case IEEE80211_SMPS_STATIC:
		cap |= WLAN_HT_CAP_SM_PS_STATIC << IEEE80211_HT_CAP_SM_PS_SHIFT;
		break;
	...
	pos = Sstar_skb_put(skb, sizeof(struct ieee80211_ht_cap) + 2);
				 ...
				 struct ieee80211_work *wk)
{
	...
	/* SSID */
	pos = Sstar_skb_put(skb, 2 + wk->assoc.ssid_len);
	...
	*pos++ = wk->assoc.ssid_len;
	...
	pos = Sstar_skb_put(skb, supp_rates_len + 2);
	...
	*pos++ = supp_rates_len;
	...
	if (rates_len > count) {
		pos = Sstar_skb_put(skb, rates_len - count + 2);
	...
	if (capab & WLAN_CAPABILITY_SPECTRUM_MGMT) {
		...
		pos = Sstar_skb_put(skb, 4);
		*pos++ = SSTAR_WLAN_EID_PWR_CAPABILITY;
		*pos++ = 2;
		*pos++ = 0; /* min tx power */
		*pos++ = wk->chan->max_power; /* max tx power */
		...
		pos = Sstar_skb_put(skb, 2 * sband->n_channels + 2);
		*pos++ = SSTAR_WLAN_EID_SUPPORTED_CHANNELS;
	...
	if (wk->ie_len) {
		static const u8 before_ht[] = {
			...
			SSTAR_WLAN_EID_PWR_CAPABILITY,
			SSTAR_WLAN_EID_SUPPORTED_CHANNELS,
			SSTAR_WLAN_EID_RSN,
			SSTAR_WLAN_EID_QOS_CAPA,
			SSTAR_WLAN_EID_RRM_ENABLED_CAPABILITIES,
			SSTAR_WLAN_EID_MOBILITY_DOMAIN,
			SSTAR_WLAN_EID_SUPPORTED_REGULATORY_CLASSES,
		};
		noffset =
			Sstar_ieee80211_ie_split(wk->ie, wk->ie_len, before_ht,
						 ARRAY_SIZE(before_ht), offset);
		pos = Sstar_skb_put(skb, noffset - offset);
		memcpy(pos, wk->ie + offset, noffset - offset);
		offset = noffset;
	}

	if (wk->assoc.use_11n && wk->assoc.wmm_used && local->hw.queues >= 4)
		ieee80211_add_ht_ie(skb, wk->assoc.ht_information_ie, sband,
				    wk->chan, wk->assoc.smps);
				      ...
				      struct ieee80211_work *wk)
{
	...
	cbss = ieee80211_Sstar_get_bss(
		local->hw.wiphy, wk->chan, wk->filter_ta, wk->probe_auth.ssid,
		wk->probe_auth.ssid_len,
		WLAN_CAPABILITY_ESS | WLAN_CAPABILITY_PRIVACY, capa_val);
	if (!cbss)
		return;
...
ieee80211_direct_probe(struct ieee80211_work *wk)
{
	...
	if (wk->probe_auth.tries > IEEE80211_AUTH_MAX_TRIES) {
		Sstar_printk_err("%s: direct probe to %pM timed out\n",
	...
	Sstar_printk_mgmt("%s: direct probe to %pM (try %d/%i)\n", sdata->name,
			  wk->filter_ta, wk->probe_auth.tries,
	...
	ieee80211_send_probe_req(sdata, NULL, wk->probe_auth.ssid,
				 wk->probe_auth.ssid_len, NULL, 0, (u32)-1,
				 true, false);
...
/* HACK!!! apollo device requires SSID to be available at AUTH stage.
	 ...
	 * bss struct returned by ieee80211_Sstar_get_bss() has random SSID if BSS
...
#if 0
	while (true) {
		bss = ieee80211_Sstar_get_bss(local->hw.wiphy,
		...
		if (bss == wk->probe_auth.bss) {
			ieee80211_Sstar_put_bss(local->hw.wiphy,bss);
			break;
		}
	...
	bss = __ieee80211_Sstar_get_authen_bss(&sdata->vif,
					       wk->probe_auth.bss->channel,
					       wk->probe_auth.bss->bssid, NULL,
					       0);
	if (bss)
		__ieee80211_Sstar_put_authen_bss(&sdata->vif, bss);
	else
		WARN_ON(1);
#endif
	/* End of the hack */
	...
	wk->probe_auth.tries++;
	if (wk->probe_auth.tries > IEEE80211_AUTH_MAX_TRIES) {
		Sstar_printk_err("%s: authentication with %pM"
				 " timed out\n",
				 sdata->name, wk->filter_ta);
	...
	Sstar_printk_mgmt("%s: authenticate with %pM (try %d)\n", sdata->name,
			  wk->filter_ta, wk->probe_auth.tries);
...
#endif
#endif
	ieee80211_send_auth(sdata, trans, wk->probe_auth.algorithm, status,
			    wk->ie, wk->ie_len, wk->filter_ta, NULL, 0, 0);
	//	wk->probe_auth.transaction = 2;
...
ieee80211_associate(struct ieee80211_work *wk)
{
	...
	if (wk->assoc.tries > IEEE80211_ASSOC_MAX_TRIES) {
		Sstar_printk_err("%s: association with %pM"
				 " timed out\n",
				 sdata->name, wk->filter_ta);
	...
	Sstar_printk_mgmt("%s: associate with %pM (try %d)\n", sdata->name,
			  wk->filter_ta, wk->assoc.tries);
...
ieee80211_assoc_beacon_wait(struct ieee80211_work *wk)
{
	 ...
	 */
	Sstar_printk_mgmt("%s: waiting for beacon from %pM\n", wk->sdata->name,
			  wk->filter_ta);
...
ieee80211_wk_connecting(struct ieee80211_work *wk)
{
	...
	lockdep_assert_held(&local->mtx);
	wk->connecting.tries++;
	Sstar_printk_mgmt("%s:connecting.tries(%d)\n", __func__,
			  wk->connecting.tries);
	if (wk->connecting.tries > wk->connecting.retry_max) {
		Sstar_printk_err("%s: time out\n", __func__);
		return WORK_ACT_TIMEOUT;
	}

	if (ifmgd->associated &&
	    (sdata->vif.bss_conf.arp_filter_enabled == true) &&
	    (sdata->vif.bss_conf.arp_addr_cnt > 0)) {
		Sstar_printk_mgmt("%s:arp_filter_state\n", __func__);
		return WORK_ACT_TIMEOUT;
	}
#ifdef IPV6_FILTERING
	if (ifmgd->associated &&
	    (sdata->vif.bss_conf.ndp_filter_enabled == true) &&
		       ...
		       struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (auth_alg != wk->probe_auth.algorithm ||
		...
		Sstar_printk_mgmt(
			"%s: %pM denied authentication (auth_alg[%d,%d],transaction[%d:%d])\n",
			wk->sdata->name, mgmt->sa, auth_alg,
			wk->probe_auth.algorithm, auth_transaction,
			wk->probe_auth.transaction);
		ieee80211_free_authen_bss(sdata);
		return WORK_ACT_NONE;
	}
	if (status_code != WLAN_STATUS_SUCCESS) {
		Sstar_printk_err("%s: %pM denied authentication (status %d)\n",
				 wk->sdata->name, mgmt->sa, status_code);
#ifdef CONFIG_MAC80211_SSTAR_ROAMING_CHANGES
		wk->sdata->queues_locked = 0;
#endif
		if (status_code == WLAN_STATUS_NOT_SUPPORTED_AUTH_ALG) {
			Sstar_printk_err("%s:auhen log err(%d)(%d)\n", __func__,
					 auth_alg, wk->probe_auth.algorithm);
			/*
			*triger wpa_supplicant to retransmit an authen frame with the rignt algorithm
			*/
			mgmt->u.auth.auth_alg =
				cpu_to_le16(wk->probe_auth.algorithm);
		}
		ieee80211_free_authen_bss(sdata);
	...
	Sstar_printk_always("%s: authenticated\n", wk->sdata->name);
	return WORK_ACT_DONE;
}

static enum work_action __must_check ieee80211_rx_mgmt_assoc_resp(
	struct ieee80211_work *wk, struct Sstar_ieee80211_mgmt *mgmt,
	size_t len, bool reassoc)
{
	...
	Sstar_printk_mgmt("%s: RX %sssocResp from %pM (capab=0x%x "
			  "status=%d aid=%d)\n",
			  sdata->name, reassoc ? "Rea" : "A", mgmt->sa,
			  capab_info, status_code,
			  (u16)(aid & ~(BIT(15) | BIT(14))));
	...
	if (status_code == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY &&
		...
		ms = tu * 1024 / 1000;
		Sstar_printk_err("%s: %pM rejected association temporarily; "
	...
	if (status_code != WLAN_STATUS_SUCCESS)
		Sstar_printk_err("%s: %pM denied association (code=%d)\n",
				 sdata->name, mgmt->sa, status_code);
	else
		Sstar_printk_always("%s: associated\n", sdata->name);
#ifdef CONFIG_MAC80211_SSTAR_ROAMING_CHANGES
	sdata->queues_locked = 0;
#endif
...
static enum work_action __must_check ieee80211_rx_mgmt_probe_resp(
	struct ieee80211_work *wk, struct Sstar_ieee80211_mgmt *mgmt,
	size_t len, struct ieee80211_rx_status *rx_status)
{
	...
	Sstar_printk_mgmt("%s: direct probe responded\n", sdata->name);
	return WORK_ACT_DONE;
}
			 ...
			 struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	Sstar_printk_mgmt("%s: beacon received\n", sdata->name);
	return WORK_ACT_DONE;
}
					  ...
					  struct sk_buff *skb)
{
	...
	list_for_each_entry (wk, &local->work_list, list) {
		 ...
		 */
		if (Sstar_compare_ether_addr(bssid, mgmt->bssid))
			continue;
		...
		case IEEE80211_STYPE_ASSOC_RESP:
			rma = ieee80211_rx_mgmt_assoc_resp(wk, mgmt, skb->len,
							   false);
			break;
...
static void ieee80211_work_empty_start_pendding(struct ieee80211_local *local)
{
	...
	if (local->roc_pendding && local->roc_pendding_sdata) {
		...
		Sstar_printk_mgmt(
			"%s:roc_pendding,roc_duration_left(%d),init_duration(%d)\n",
			__func__, roc_duration_left, init_duration);
		roc_timeout =
			!time_is_after_jiffies(roc->pending_start_time +
					       (roc_duration_left * HZ) / 1000);
		...
		if (roc_timeout || list_empty(&local->work_list)) {
			...
			if (roc_timeout) {
				...
				roc_duration = ((roc->pending_start_time +
						 (init_duration * HZ) / 1000 -
						 jiffies) *
						1000) /
					       HZ;

				if (roc_duration > 70)
					ret = ieee80211_start_pending_roc_work(
						local, roc->hw_roc_dev,
						roc->sdata, roc->chan,
						roc->chan_type, roc_duration,
						&roc->cookie, roc->frame);
				else {
					ret = -1;
				}
			} else {
...
#else
				Sstar_printk_mgmt(
					"%s:pennding roc err roc_timeout(%d),roc_duration(%d),cookie(%llx)\n",
					__func__, roc_timeout, roc_duration,
					roc->mgmt_tx_cookie ?
						      roc->mgmt_tx_cookie :
						      roc->cookie);
	...
	if (local->scan_req && !local->scanning) {
		...
		scan_timeout = !time_is_after_jiffies(
			local->pending_scan_start_time + (4 * HZ));

		if (scan_timeout || list_empty(&local->work_list)) {
			if (scan_timeout) {
				if (local->scan_req != local->int_scan_req) {
					Sstar_printk_mgmt(
						"%s(%s):cancle scan,scan_timeout(%d),work_list(%d)\n",
						__func__,
						local->scan_sdata->name,
						scan_timeout,
						list_empty(&local->work_list));
					Sstar_notify_scan_done(
						local, local->scan_req, false);
					local->scanning = 0;
					local->scan_req = NULL;
				...
				Sstar_printk_mgmt(
					"%s(%s):start delayed scan opration\n",
					__func__, local->scan_sdata->name);
				ieee80211_queue_delayed_work(
					&local->hw, &local->scan_work,
...
#endif
static void ieee80211_work_work(struct Sstar_work_struct *work)
{
	...
	while ((skb = Sstar_skb_dequeue(&local->work_skb_queue)))
	...
	list_for_each_entry_safe (wk, tmp, &local->work_list, list) {
		...
		if (ieee80211_work_need_sync_channel(wk) == true) {
			...
			if (!started && chan_state->tmp_channel &&
				...
				if ((wk->type != IEEE80211_WORK_CONNECTTING) &&
				    (atomic_read(&local->connectting) == 0)) {
					started = true;
					wk->timeout = jiffies;
				}
			}
			if (!started && !chan_state->tmp_channel) {
				 ...
				 */
				Sstar_printk_mgmt("%s:start work ch(%d)(%d)\n",
						  __func__,
						  channel_hw_value(wk->chan),
						  wk->type);
				...
				if (wk->type != IEEE80211_WORK_CONNECTTING) {
					...
					atomic_set(&wk->sdata->connectting,
						   IEEE80211_SSTAR_CONNECT_RUN);
		...
		if (!started) {
			Sstar_printk_mgmt("%s:not start work ch(%d)\n",
					  __func__, wk->type);
			continue;
		}
	...
	if (!remain_off_channel && chan_state->tmp_channel) {
		 ...
		 */
		if (ieee80211_get_channel_mode(local, NULL) !=
		    CHAN_MODE_UNDEFINED) {
			Sstar_printk_mgmt(
				"%s:reset work ch(%d)\n", __func__,
...
#ifndef CONFIG_SSTAR_STA_DYNAMIC_PS
	if (WARN_ON(!wk->chan))
		return;
#endif
...
void ieee80211_work_init(struct ieee80211_local *local)
{
	...
	Sstar_setup_timer(&local->work_timer, ieee80211_work_timer,
			  (unsigned long)local);
	SSTAR_INIT_WORK(&local->work_work, ieee80211_work_work);
	Sstar_skb_queue_head_init(&local->work_skb_queue);
}
					   ...
					   struct sk_buff *skb)
{
	...
	if (skb->len < 24) {
		Sstar_printk_mgmt("%s:skb->len < 24\n", __func__);
	...
	list_for_each_entry_rcu (wk, &local->work_list, list) {
		if (sdata != wk->sdata)
			continue;
		if (Sstar_compare_ether_addr(wk->filter_ta, mgmt->sa))
			continue;
		if (Sstar_compare_ether_addr(wk->filter_ta, mgmt->bssid))
			continue;
		...
		case IEEE80211_STYPE_REASSOC_RESP:
		case IEEE80211_STYPE_BEACON:
			Sstar_skb_queue_tail(&local->work_skb_queue, skb);
			ieee80211_queue_work(&local->hw, &local->work_work);
			return RX_QUEUED;
		}
	}
...
ieee80211_connecting_work_done(struct ieee80211_work *wk, struct sk_buff *skb)
{
	struct ieee80211_sub_if_data *sdata = wk->sdata;
	struct ieee80211_local *local = sdata->local;
	Sstar_printk_mgmt("%s\n", __func__);
	if (atomic_read(&sdata->connectting) == IEEE80211_SSTAR_CONNECT_RUN)
		atomic_set(&local->connectting, 0);
...
{
	struct ieee80211_work *wk;
	wk = Sstar_kzalloc(sizeof(*wk), GFP_KERNEL);
	if (WARN_ON(!wk))
		return;
	...
	Sstar_printk_debug("%s:bssid(%pM)\n", __func__, bssid);
	atomic_set(&sdata->connectting, IEEE80211_SSTAR_CONNECT_SET);
...
void ieee80211_start_ps_recal_work(struct ieee80211_sub_if_data *sdata)
{
	...
	wk = Sstar_kzalloc(sizeof(*wk), GFP_ATOMIC);
	if (WARN_ON(!wk))
		return;
				 ...
				 struct cfg80211_bss *pub)
{
	...
	Sstar_printk_mgmt("%s:assign authen bss[%p]\n", sdata->name, pub);
	lockdep_assert_held(&local->mtx);
	/*
	*hold our bss for later use
	*/
	if (ieee80211_Sstar_handle_bss(local->hw.wiphy, pub) == 0) {
		/*
		*save pub
		*/
	} else {
		Sstar_printk_mgmt("%s:get bss err\n", sdata->name);
		pub = NULL;
	}
...
void ieee80211_free_authen_bss(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_printk_always("%s:free authen bss ++\n", sdata->name);
	if (sdata->vif.type != NL80211_IFTYPE_STATION) {
		Sstar_printk_err("%s: is not sta mode\n", sdata->name);
		return;
	}
	free_bss = rcu_dereference_protected(ifmgd->authen_bss,
	...
	Sstar_printk_always("%s:free authen bss --\n", sdata->name);
	Sstar_printk_mgmt("%s:free authen bss[%p]\n", sdata->name, free_bss);
	ieee80211_Sstar_release_bss(local->hw.wiphy, free_bss);
}
