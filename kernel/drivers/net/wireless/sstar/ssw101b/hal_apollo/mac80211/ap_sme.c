...
static void ieee80211_ap_sme_work(struct Sstar_work_struct *work);
static void ieee80211_ap_sme_event_work(struct Sstar_work_struct *work);
...
#define SSTAR_WLAN_AUTH_CHALLENGE_LEN	128
#define SSTAR_WLAN_EID_CHALLENGE 		16
#define SSTAR_WLAN_AUTH_OPEN				0
#define SSTAR_WLAN_AUTH_SHARED_KEY		1
#define SSTAR_WLAN_AUTH_NOT_SUPORT		0

#define SSTAR_WLAN_AUTH_OPEN_BIT			(BIT(SSTAR_WLAN_AUTH_OPEN))
#define SSTAR_WLAN_AUTH_SHARED_KEY_BIT	(BIT(SSTAR_WLAN_AUTH_SHARED_KEY))

#define SSTAR_WLAN_AUTH_TYPE_TO_BIT(type) (BIT(type))

#define SSTAR_WLAN_SDATA_AUTHEN_BIT(sdata)	(sdata->vif.bss_conf.auth_type_bit)

#define SSTAR_WLAN_AUTH_OPEN_SUPPORT(sdata,rx_athen)		\
	((SSTAR_WLAN_SDATA_AUTHEN_BIT(sdata)&SSTAR_WLAN_AUTH_TYPE_TO_BIT(rx_athen))==SSTAR_WLAN_AUTH_OPEN_BIT)

#define SSTAR_WLAN_AUTH_SHARED_SUPPORT(sdata,rx_athen)	\
	((SSTAR_WLAN_SDATA_AUTHEN_BIT(sdata)&SSTAR_WLAN_AUTH_TYPE_TO_BIT(rx_athen))==SSTAR_WLAN_AUTH_SHARED_KEY_BIT)

#define SSTAR_GET_IE_FORM_SRC_IE(eid,des_ie,des_off,src_ie,src_len)		\
	do{																	\
		u8* wanted_ie = NULL;											\
		wanted_ie = (u8*)Sstar_ieee80211_find_ie(eid, src_ie,src_len);				\
		if(wanted_ie){													\
			memcpy(des_ie+des_off,wanted_ie,wanted_ie[1]+2);			\
			des_off += wanted_ie[1]+2;									\
			Sstar_printk_ap( "%s:add (%s)\n",__func__,#eid);				\
		}																\
	}while(0)

#define SSTAR_GET_VENDOR_IE_FORM_SRC_IE(oui,oui_type,des_ie,des_off,src_ie,src_len)		\
	do{ 																				\
			u8* wanted_ie = NULL;															\
			wanted_ie = (u8*)Sstar_ieee80211_find_vendor_ie(oui,oui_type,src_ie,src_len);				\
			if(wanted_ie){																\
				memcpy(des_ie+des_off,wanted_ie,wanted_ie[1]+2);					\
				des_off += wanted_ie[1]+2;												\
				Sstar_printk_ap("%s:add (%s)\n",__func__,#oui_type);						\
...
#define SSTAR_SET_SUPP_RATE_IE(des_ie,des_off,src_ie,src_len)				\
	SSTAR_GET_IE_FORM_SRC_IE(SSTAR_WLAN_EID_SUPP_RATES,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_EXT_RATE_IE(des_ie,des_off,src_ie,src_len)				\
	SSTAR_GET_IE_FORM_SRC_IE(SSTAR_WLAN_EID_EXT_SUPP_RATES,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_HT_CAP_IE(des_ie,des_off,src_ie,src_len)				\
	SSTAR_GET_IE_FORM_SRC_IE(SSTAR_WLAN_EID_HT_CAPABILITY,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_HT_INFO_IE(des_ie,des_off,src_ie,src_len)					\
	SSTAR_GET_IE_FORM_SRC_IE(SSTAR_WLAN_EID_HT_INFORMATION,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_VENDOR_WPS_IE(des_ie,des_off,src_ie,src_len)					\
	SSTAR_GET_VENDOR_IE_FORM_SRC_IE(SSTAR_OUI_MICROSOFT,4,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_VENDOR_P2P_IE(des_ie,des_off,src_ie,src_len)					\
	SSTAR_GET_VENDOR_IE_FORM_SRC_IE(SSTAR_OUI_WFA,SSTAR_P2P_OUI_TYPE,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_VENDOR_WFD_IE(des_ie,des_off,src_ie,src_len)					\
	SSTAR_GET_VENDOR_IE_FORM_SRC_IE(SSTAR_OUI_WFA,SSTAR_WFD_OUI_TYPE,des_ie,des_off,src_ie,src_len)

#define SSTAR_SET_VENDOR_WMM_IE(des_ie,des_off,src_ie,src_len)					\
	SSTAR_GET_VENDOR_IE_FORM_SRC_IE(SSTAR_OUI_MICROSOFT,SSTAR_WMM_OUI_TYPE,des_ie,des_off,src_ie,src_len)
/*
...
*/
#define SSTAR_AP_SME_CLAER_STA_ASSOC_INFO(_sta)   \
	do{											\
...
#define RSN_SELECTOR_GET(s) SSTAR_WPA_GET_BE32((const u8 *)s)
...
void ieee80211_ap_sme_mlme_purge(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_printk_ap("%s\n",__func__);
	Sstar_skb_queue_purge(&sdata->ap_sme_skb_queue);
...
void ieee80211_ap_sme_sta_sync_unlock(struct ieee80211_sub_if_data *sdata)
{
	...
	unlock_cnt++;
	Sstar_printk_ap("%s:unlock_cnt(%d)\n",__func__,unlock_cnt);
...
{
	Sstar_printk_ap("%s\n",__func__);
	Sstar_skb_queue_head_init(&sdata->ap_sme_skb_queue);
	SSTAR_INIT_WORK(&sdata->ap_sme_work, ieee80211_ap_sme_work);
...
void ieee80211_ap_sme_event_init(struct ieee80211_sub_if_data *sdata)
{
	...
	SSTAR_INIT_WORK(&sdata->ap_sme_event_work, ieee80211_ap_sme_event_work);
	mutex_lock(&sdata->ap_sme_event_lock);
											  ...
											  enum ap_sme_event_id event_id,u8* addr)
{
	...
	if (!ieee80211_sdata_running(sdata)){
		Sstar_printk_ap("%s:sdata is not running\n",__func__);
		goto queue_event_err;
	}
	...
	event = Sstar_kzalloc(sizeof(struct ap_sme_event),GFP_KERNEL);
	...
	mutex_unlock(&sdata->ap_sme_event_lock);
	Sstar_printk_ap("%s,event(%d),mac(%pM)\n",__func__,event_id,addr);
	if(first)
		Sstar_queue_work(sdata->local->workqueue, &sdata->ap_sme_event_work);
					   ...
					   struct sk_buff *skb)
{
	...
	case cpu_to_le16(IEEE80211_STYPE_DEAUTH):
		Sstar_printk_ap("%s:stype(%x),sdata(%x)\n",__func__,stype,(int)sdata);
		break;
	default:
		return -1;
	}
	...
	Sstar_skb_queue_tail(&sdata->ap_sme_skb_queue, skb);
	...
	struct beacon_parameters *ap_info)
{
	...
	struct Sstar_wpa_ie_data *wpa_data = &bss_conf->wpa_ap_data;
	...
	Sstar_printk_ap( "ap_info:auth_type(%d),privacy(%d),auth_type_bit(%x)\n",bss_conf->auth_type,
		bss_conf->privacy,bss_conf->auth_type_bit);
	memset(wpa_data,0,sizeof(struct Sstar_wpa_ie_data));
	if(ap_info->crypto.wpa_versions&NL80211_WPA_VERSION_1){
		Sstar_printk_ap("ap_info: support WPA\n");
		wpa_data->proto |= SSTAR_WPA_PROTO_WPA;
	}
	if(ap_info->crypto.wpa_versions&NL80211_WPA_VERSION_2){
		Sstar_printk_ap("ap_info: support RSN\n");
		wpa_data->proto |= SSTAR_WPA_PROTO_RSN;
	}
	...
	for(i = 0;i < ap_info->crypto.n_ciphers_pairwise;i++)
	{
		...
		case WLAN_CIPHER_SUITE_CCMP:
			Sstar_printk_ap("ap_info: support CCMP\n");
			wpa_data->pairwise_cipher |= SSTAR_WPA_CIPHER_CCMP;
			break;
		#if 0
		case WLAN_CIPHER_SUITE_GCMP:
			Sstar_printk_ap( "ap_info: support GCMP\n");
			wpa_data->pairwise_cipher |= SSTAR_WPA_CIPHER_GCMP;
			break;
		#endif
		case WLAN_CIPHER_SUITE_TKIP:
			Sstar_printk_ap("ap_info: support TKIP\n");
			wpa_data->pairwise_cipher |= SSTAR_WPA_CIPHER_TKIP;
			break;
		case WLAN_CIPHER_SUITE_WEP104:
			Sstar_printk_ap("ap_info: support WEP104\n");
			wpa_data->pairwise_cipher |= SSTAR_WPA_CIPHER_WEP104;
			break;
		case WLAN_CIPHER_SUITE_WEP40:
			Sstar_printk_ap("ap_info: support WEP40\n");
			wpa_data->pairwise_cipher |= SSTAR_WPA_CIPHER_WEP40;
			break;
		default:
			break;
		}
	...
	for (i = 0; i < ap_info->crypto.n_akm_suites; i++) {
		...
		case WLAN_AKM_SUITE_8021X:
			wpa_data->key_mgmt |= SSTAR_WPA_KEY_MGMT_IEEE8021X;
			Sstar_printk_ap("key_mgmt: support IEEE8021X\n");
			break;
		case WLAN_AKM_SUITE_PSK:
			wpa_data->key_mgmt |= SSTAR_WPA_KEY_MGMT_PSK;
			Sstar_printk_ap("key_mgmt: support PSK\n");
			break;
		default:
			break;
		}
	...
	case WLAN_CIPHER_SUITE_WEP40:
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_WEP40;
		Sstar_printk_ap("group_cipher:WEP40\n");
		break;
	case WLAN_CIPHER_SUITE_WEP104:
		Sstar_printk_ap("group_cipher:WEP104\n");
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_WEP104;
		break;
	case WLAN_CIPHER_SUITE_TKIP:
		Sstar_printk_ap("group_cipher:TKIP\n");
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_TKIP;
		break;
	case WLAN_CIPHER_SUITE_CCMP:
		Sstar_printk_ap("group_cipher:CCMP\n");
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_CCMP;
		break;
	case WLAN_CIPHER_SUITE_SMS4:
		Sstar_printk_ap("group_cipher:SMS4\n");
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_SMS4;
		break;
	default:
		Sstar_printk_ap("group_cipher:NONE\n");
		wpa_data->group_cipher = SSTAR_WPA_CIPHER_NONE;
		break;
	}
	/*
	*rsn: get mgmt protect
	*/
	while(wpa_data->proto&SSTAR_WPA_PROTO_RSN)
	{
		u8* rsn_ie = NULL;
		size_t rsn_ie_len = 0;
		struct Sstar_wpa_ie_data rsn_data;
		rsn_ie = (u8*)Sstar_ieee80211_find_ie(WLAN_EID_RSN, ap_info->tail, ap_info->tail_len);
	...
	struct beacon_parameters *ap_info)
{
	struct ieee80211_bss_conf *bss_conf = &sdata->vif.bss_conf;
	struct Sstar_wpa_ie_data *wpa_data = &bss_conf->wpa_ap_data;
	struct Sstar_ieee80211_mgmt *beacon = (struct Sstar_ieee80211_mgmt *)ap_info->head;
	struct ieee802_Sstar_11_elems ap_elems;
	struct Sstar_wpa_ie_data ap_wpa_data;
	struct Sstar_wpa_ie_data ap_rsn_data;
	...
	memset(wpa_data,0,sizeof(struct Sstar_wpa_ie_data));
	memset(&ap_wpa_data,0,sizeof(struct Sstar_wpa_ie_data));
	memset(&ap_rsn_data,0,sizeof(struct Sstar_wpa_ie_data));
	bss_conf->privacy = SSTAR_WLAN_CAPABILITY_PRIVACY&beacon->u.beacon.capab_info ? 1 : 0;
	ieee802_11_parse_elems((u8*)ap_info->tail,ap_info->tail_len,&ap_elems);

	if(ap_elems.wpa || ap_elems.rsn){
		bss_conf->auth_type_bit = SSTAR_WLAN_AUTH_OPEN_BIT;
	}else if(bss_conf->privacy)
		bss_conf->auth_type_bit = SSTAR_WLAN_AUTH_SHARED_KEY_BIT;
	else
		bss_conf->auth_type_bit = SSTAR_WLAN_AUTH_OPEN_BIT;
...
{
	Sstar_init_timer(&sta->sta_session_timer);
...
static int ieee80211_ap_sme_auth_alg(struct ieee80211_sub_if_data *sdata)
{
	if(sdata->vif.bss_conf.auth_type == NL80211_AUTHTYPE_OPEN_SYSTEM)
		return SSTAR_WLAN_AUTH_OPEN_BIT;
	else if(sdata->vif.bss_conf.auth_type == NL80211_AUTHTYPE_SHARED_KEY)
		return SSTAR_WLAN_AUTH_SHARED_KEY_BIT;
	else if(sdata->vif.bss_conf.auth_type == NL80211_AUTHTYPE_AUTOMATIC)
		return SSTAR_WLAN_AUTH_SHARED_KEY_BIT | SSTAR_WLAN_AUTH_OPEN_BIT;
	return SSTAR_WLAN_AUTH_NOT_SUPORT;
}
static void ieee80211_ap_sme_timer(unsigned long arg)
{
...
static int ieee80211_pre_athen(struct ieee80211_sub_if_data *sdata,
	...
	sta = NULL;
	if (err) {
		Sstar_printk_ap("%s: failed to insert Dummy STA entry for"
		       " the AP (error %d)\n", sdata->name, err);
		return err;
	}
	return 0;
			...
			struct sta_info *sta,u16 transaction,const u8 *challenge)
{
	if (transaction == 1) {
		if (!sta->challenge) {
			sta->challenge = Sstar_kmalloc(SSTAR_WLAN_AUTH_CHALLENGE_LEN,GFP_KERNEL);
	...
	if (!sta->challenge || !challenge ||
	    memcmp(sta->challenge, challenge, SSTAR_WLAN_AUTH_CHALLENGE_LEN)) {
		Sstar_printk_ap("%s:challenge err\n",__func__);
...
static void ieee80211_ap_sme_tx_mgmt_authen_status(struct ieee80211_sub_if_data *sdata,struct sk_buff *skb)
{
	...
	Sstar_printk_ap("%s:sta(%pM),auth_alg(%d),auth_transaction(%d),status_code(%d),ack(%d)\n",__func__,
	...
	if(
		(SSTAR_WLAN_AUTH_OPEN_SUPPORT(sdata,auth_alg)&&(auth_transaction == 2))
		||
		(SSTAR_WLAN_AUTH_SHARED_SUPPORT(sdata,auth_alg)&&(auth_transaction == 4))
	  )
	{
				     ...
				     struct ieee80211_rx_status *rx_status,u16 *status)
{
	...
	if(sta == NULL)
	{
		...
		{
			Sstar_printk_ap("%s:sta_info_alloc err\n",__func__);
			return RX_MGMT_NONE;
		}
		rcu_read_lock();
		sta = sta_info_get_rx(sdata, mgmt->sa);
		...
		if(sta == NULL)
		{
			Sstar_printk_ap("%s:sta_info_get_bss err\n",__func__);
	...
	if(test_sta_flag(sta, WLAN_STA_AUTHORIZED) || test_sta_flag(sta,WLAN_STA_ASSOCED)){
		Sstar_printk_ap("%s:sta(%pM) has been athened or assoced,what should we do\n",__func__,mgmt->sa);
		SSTAR_AP_SME_CLAER_STA_ASSOC_INFO(sta);
	}
	...
	if (len >= 24 + sizeof(mgmt->u.auth) +
	    2 + SSTAR_WLAN_AUTH_CHALLENGE_LEN &&
	    mgmt->u.auth.variable[0] == SSTAR_WLAN_EID_CHALLENGE &&
	    mgmt->u.auth.variable[1] == SSTAR_WLAN_AUTH_CHALLENGE_LEN)
		challenge = &mgmt->u.auth.variable[2];

	if(
		!((SSTAR_WLAN_AUTH_OPEN_SUPPORT(sdata,auth_alg))
		||
		(SSTAR_WLAN_AUTH_SHARED_SUPPORT(sdata,auth_alg)))
	  )
	{
		Sstar_printk_ap("%s:auth_alg(%d) bss_conf.auth_type(%d) err \n",__func__,auth_alg,sdata->vif.bss_conf.auth_type);
	...
	if (!((auth_transaction == 1 ) ||
		  (auth_alg == SSTAR_WLAN_AUTH_SHARED_KEY && auth_transaction == 3))) {
			Sstar_printk_ap("Unknown authentication transaction number (%d)\n",
				   auth_transaction);
	...
	*status = WLAN_STATUS_SUCCESS;
	if (!Sstar_timer_pending(&sta->sta_session_timer) ||
	    !time_is_after_jiffies(sta->sta_session_timer.expires))
		Sstar_mod_timer(&sta->sta_session_timer, jiffies + 3*HZ);
			 ...
			 u8 *extra, size_t extra_len, const u8 *da,u16 status)
{
	...
	skb = Sstar_dev_alloc_skb(local->hw.extra_tx_headroom +
	...
	mgmt = (struct Sstar_ieee80211_mgmt *) Sstar_skb_put(skb, 24 + 6);
	memset(mgmt, 0, 24 + 6);
	...
	if (extra)
		memcpy(Sstar_skb_put(skb, extra_len), extra, extra_len);

 	Sstar_printk_ap("%s %d transaction =%d \n",__func__,__LINE__,transaction);
	...
	Sstar_printk_ap("%s:sta(%pM),transaction(%d),auth_alg(%d),status(%d)\n",__func__,
	...
	const u8 *da,struct sta_info * sta)
{
	...
	skb = Sstar_dev_alloc_skb(local->hw.extra_tx_headroom + sizeof(*mgmt));
	if (!skb)
		return;
	...
	Sstar_skb_put(skb, 2);
	Sstar_printk_ap("%s:sta(%pM),reason_code(%u)\n",__func__,da,mgmt->u.deauth.reason_code);
	IEEE80211_SKB_CB(skb)->flags |= IEEE80211_TX_INTFL_DONT_ENCRYPT;
	IEEE80211_SKB_CB(skb)->flags |= IEEE80211_TX_AP_HANDLE_STATUS;
	IEEE80211_SKB_CB(skb)->flags |= IEEE80211_TX_CTL_USE_MINRATE;
	if(sdata->vif.p2p)
	...
	u16 status,struct sk_buff *skb)
{
	...
	if(!test_sta_flag(sta, WLAN_STA_AUTH)){
		Sstar_printk_ap("%s:not in authen state\n",__func__);
		goto tx_mgmt_authen_err;
	}

	auth_alg = mgmt->u.auth.auth_alg;
	auth_transaction = mgmt->u.auth.auth_transaction;
	Sstar_printk_ap("%s:status(%d)\n",__func__,status);
	if(status == WLAN_STATUS_SUCCESS)
	{
		if(SSTAR_WLAN_AUTH_OPEN_SUPPORT(sdata,auth_alg)){
			Sstar_printk_ap("%s:open alog\n",__func__);
			set_sta_flag(sta,WLAN_STA_AUTH_STATUS);
			goto tx_mgmt_authen;
		}
		else if(SSTAR_WLAN_AUTH_SHARED_SUPPORT(sdata,auth_alg))
		{
			const u8 *challenge = NULL;
			if (skb->len >= 24 + sizeof(mgmt->u.auth) +
	   			 ...
	   			 mgmt->u.auth.variable[1] == SSTAR_WLAN_AUTH_CHALLENGE_LEN)
			   challenge = &mgmt->u.auth.variable[2];
			Sstar_printk_ap("%s:share key alog\n",__func__);
			...
			{
				resp_ies = Sstar_kmalloc(SSTAR_WLAN_AUTH_CHALLENGE_LEN+2,GFP_KERNEL);
				if(resp_ies == NULL)
				{
					status = WLAN_STATUS_UNSPECIFIED_FAILURE;
					break;
				}
				resp_ies[0] = SSTAR_WLAN_EID_CHALLENGE;
				resp_ies[1] = SSTAR_WLAN_AUTH_CHALLENGE_LEN;
				memcpy(resp_ies + 2, sta->challenge,
				  	SSTAR_WLAN_AUTH_CHALLENGE_LEN);
				resp_ies_len = 2 + SSTAR_WLAN_AUTH_CHALLENGE_LEN;
			}
			 ...
			 struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	Sstar_printk_ap("%s: deauthenticated from %pM (Reason: %u)\n",
...
static u16 ieee80211_ap_sme_check_ssid(struct ieee80211_sub_if_data *sdata,u8 *ssid,u8 ssid_len)
{
	if(ssid == NULL)
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	Sstar_printk_ap("%s:bss_conf.ssid_len(%d),ssid_len(%d)\n",__func__,sdata->vif.bss_conf.ssid_len,
		ssid_len);
	if(sdata->vif.bss_conf.ssid_len)
		Sstar_printk_ap("%s:bss_conf.ssid(%s)\n",__func__,sdata->vif.bss_conf.ssid);
	if(ssid_len)
		Sstar_printk_ap("%s:ssid(%s)\n",__func__,ssid);
	...
	struct ieee802_Sstar_11_elems *ap_elems)
{
	...
	sband = local->hw.wiphy->bands[chan_state->oper_channel->band];
	Sstar_printk_ap("%s:ap_elems->ht_cap_elem(%x),sta_elems->ht_cap_elem(%x)\n",__func__,(int)ap_elems->ht_cap_elem
...
static int rsn_selector_to_bitfield(const u8 *s)
{
	...
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_NONE)
		return SSTAR_WPA_CIPHER_NONE;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_WEP40)
		return SSTAR_WPA_CIPHER_WEP40;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_TKIP)
		return SSTAR_WPA_CIPHER_TKIP;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_CCMP)
		return SSTAR_WPA_CIPHER_CCMP;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_WEP104)
		return SSTAR_WPA_CIPHER_WEP104;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_AES_128_CMAC)
		return SSTAR_WPA_CIPHER_AES_128_CMAC;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_CIPHER_SUITE_GCMP)
		return SSTAR_WPA_CIPHER_GCMP;
	return 0;
}

static int rsn_key_mgmt_to_bitfield(const u8 *s)
{
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_UNSPEC_802_1X)
		return SSTAR_WPA_KEY_MGMT_IEEE8021X;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X)
		return SSTAR_WPA_KEY_MGMT_PSK;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_FT_802_1X)
		return SSTAR_WPA_KEY_MGMT_FT_IEEE8021X;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_FT_PSK)
		return SSTAR_WPA_KEY_MGMT_FT_PSK;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_802_1X_SHA256)
		return SSTAR_WPA_KEY_MGMT_IEEE8021X_SHA256;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_PSK_SHA256)
		return SSTAR_WPA_KEY_MGMT_PSK_SHA256;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_SAE)
		return SSTAR_WPA_KEY_MGMT_SAE;
	if (RSN_SELECTOR_GET(s) == SSTAR_RSN_AUTH_KEY_MGMT_FT_SAE)
		return SSTAR_WPA_KEY_MGMT_FT_SAE;
	return 0;
}

static int wpa_selector_to_bitfield(const u8 *s)
{
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_CIPHER_SUITE_NONE)
		return SSTAR_WPA_CIPHER_NONE;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_CIPHER_SUITE_WEP40)
		return SSTAR_WPA_CIPHER_WEP40;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_CIPHER_SUITE_TKIP)
		return SSTAR_WPA_CIPHER_TKIP;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_CIPHER_SUITE_CCMP)
		return SSTAR_WPA_CIPHER_CCMP;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_CIPHER_SUITE_WEP104)
		return SSTAR_WPA_CIPHER_WEP104;
	return 0;
}
...
{
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_AUTH_KEY_MGMT_UNSPEC_802_1X)
		return SSTAR_WPA_KEY_MGMT_IEEE8021X;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_AUTH_KEY_MGMT_PSK_OVER_802_1X)
		return SSTAR_WPA_KEY_MGMT_PSK;
	if (RSN_SELECTOR_GET(s) == SSTAR_WPA_AUTH_KEY_MGMT_NONE)
		return SSTAR_WPA_KEY_MGMT_WPA_NONE;
	return 0;
}
			 ...
			 struct Sstar_wpa_ie_data *data)
{
	...
	data->proto = SSTAR_WPA_PROTO_RSN;
	data->pairwise_cipher = SSTAR_WPA_CIPHER_CCMP;
	data->group_cipher = SSTAR_WPA_CIPHER_CCMP;
	data->key_mgmt = SSTAR_WPA_KEY_MGMT_IEEE8021X;
	...
	if (left >= SSTAR_RSN_SELECTOR_LEN) {
		data->group_cipher = rsn_selector_to_bitfield(pos);

		if (data->group_cipher == SSTAR_WPA_CIPHER_AES_128_CMAC) {
			Sstar_printk_ap("%s: AES-128-CMAC used as group "
				   "cipher", __func__);
			return -1;
		}
		pos += SSTAR_RSN_SELECTOR_LEN;
		left -= SSTAR_RSN_SELECTOR_LEN;
	} else if (left > 0) {
		Sstar_printk_ap("%s: ie length mismatch, %u too much",
			   __func__, left);
		return -3;
	}
	...
	if (left >= 2) {
		...
		count = SSTAR_WPA_GET_LE16(pos);
		pos += 2;
		left -= 2;
		if (count == 0 || left < count * SSTAR_RSN_SELECTOR_LEN) {
			Sstar_printk_ap("%s: ie count botch (pairwise), "
		...
		if (data->pairwise_cipher & SSTAR_WPA_CIPHER_AES_128_CMAC) {
			Sstar_printk_ap("%s: AES-128-CMAC used as "
				   "pairwise cipher", __func__);
			return -1;
		}
	} else if (left == 1) {
		Sstar_printk_ap("%s: ie too short (for key mgmt)",
			   __func__);
		return -5;
	}
	...
	if (left >= 2) {
		...
		left -= 2;
		if (count == 0 || left < count * SSTAR_RSN_SELECTOR_LEN) {
			Sstar_printk_ap("%s: ie count botch (key mgmt), "
				   "count %u left %u", __func__, count, left);
			return -6;
		}
		for (i = 0; i < count; i++) {
		...
		Sstar_printk_ap("%s: ie too short (for capabilities)",
			   __func__);
		return -7;
	}

	if (left >= 2) {
		data->rsn_capabilities = SSTAR_WPA_GET_LE16(pos);
		pos += 2;
		left -= 2;
	}

	if (left >= 2) {
		data->num_pmkid = SSTAR_WPA_GET_LE16(pos);
		pos += 2;
		left -= 2;
		if (left < (int) data->num_pmkid * SSTAR_PMKID_LEN) {
			Sstar_printk_ap("%s: PMKID underflow "
				   "(num_pmkid=%lu left=%d)",
			...
			data->pmkid = pos;
			pos += data->num_pmkid * SSTAR_PMKID_LEN;
			left -= data->num_pmkid * SSTAR_PMKID_LEN;
		}
	}

	if (left >= 4) {
		data->mgmt_group_cipher = rsn_selector_to_bitfield(pos);
		if (data->mgmt_group_cipher != SSTAR_WPA_CIPHER_AES_128_CMAC) {
			Sstar_printk_ap("%s: Unsupported management "
	...
	if (left > 0) {
		Sstar_printk_ap("%s: ie has %u trailing bytes - ignored",
			   __func__, left);
	}
			 ...
			 struct Sstar_wpa_ie_data *data)
{
	...
	memset(data, 0, sizeof(*data));
	data->proto = SSTAR_WPA_PROTO_WPA;
	data->pairwise_cipher = SSTAR_WPA_CIPHER_TKIP;
	data->group_cipher = SSTAR_WPA_CIPHER_TKIP;
	data->key_mgmt = SSTAR_WPA_KEY_MGMT_IEEE8021X;
	...
	if (left >= SSTAR_WPA_SELECTOR_LEN) {
		data->group_cipher = wpa_selector_to_bitfield(pos);
		pos += SSTAR_WPA_SELECTOR_LEN;
		left -= SSTAR_WPA_SELECTOR_LEN;
	} else if (left > 0) {
		Sstar_printk_ap("%s: ie length mismatch, %u too much",
			   __func__, left);
		return -3;
	...
	if (left >= 2) {
		...
		pos += 2;
		left -= 2;
		if (count == 0 || left < count * SSTAR_WPA_SELECTOR_LEN) {
			Sstar_printk_ap("%s: ie count botch (pairwise), "
				   "count %u left %u", __func__, count, left);
			return -4;
		}
	...
	if (left >= 2) {
		data->wpa_capabilities = SSTAR_WPA_GET_LE16(pos);
		pos += 2;
		left -= 2;
	}
	...
	struct ieee802_Sstar_11_elems *ap_elems)
{
	...
	u8* ap_wpa_ie = NULL;
	u8  ap_wpa_ie_len = 0;
	struct Sstar_wpa_ie_data *ap_wpa = &sdata->vif.bss_conf.wpa_ap_data;
	struct Sstar_wpa_ie_data *sta_wpa = &sta->sta.wpa_sta_data;
	int res = -1;
	if(ap_elems->wpa&&sta_elems->wpa){
		...
		Sstar_printk_ap("%s:use rsn\n",__func__);
	}
	Sstar_printk_ap("%s:sta_wpa_ie(%x),sta_wpa_ie_len(%x)\n",__func__,(int)sta_wpa_ie,(int)sta_wpa_ie_len);
	if((sta_wpa_ie==NULL)&&(ap_wpa_ie==NULL)){

		if((sta_elems->wpa || sta_elems->rsn)&&(ap_elems->wpa || ap_elems->rsn))
		{
			Sstar_printk_ap("%s:wpa rsn err\n",__func__);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}

	}else if(sta_wpa_ie&&ap_wpa_ie){

		Sstar_printk_ap("%s:ap_wpa_ie(%x),ap_elems->wpa(%x),ap_elems->rsn(%x)\n",__func__,
		...
		if(res)
		{
			Sstar_printk_ap("%s:parse_rsn or parse_wpa err\n",__func__);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}

		if(sta_wpa->group_cipher != ap_wpa->group_cipher)
		{
			Sstar_printk_ap("%s:group_cipher(%d),(%d)\n",__func__,sta_wpa->group_cipher,ap_wpa->group_cipher);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}

		if (!(sta_wpa->key_mgmt & ap_wpa->key_mgmt)) {
			Sstar_printk_ap("%s:key_mgmt(%x),(%x)\n",__func__,sta_wpa->key_mgmt,ap_wpa->key_mgmt);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}

		if (!(sta_wpa->pairwise_cipher& ap_wpa->pairwise_cipher)) {
			Sstar_printk_ap("%s:pairwise_cipher(%x),(%x)\n",__func__,sta_wpa->pairwise_cipher,ap_wpa->pairwise_cipher);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}

		if(ap_wpa->rsn_capabilities & SSTAR_WPA_CAPABILITY_MFPC)
		{
			if(sta_wpa->rsn_capabilities&SSTAR_WPA_CAPABILITY_MFPC){
				if (sta_wpa->pairwise_cipher & SSTAR_WPA_CIPHER_TKIP) {
					Sstar_printk_ap("Management frame protection "
						   "cannot use TKIP");
					return WLAN_STATUS_UNSPECIFIED_FAILURE;
				}

				if (sta_wpa->mgmt_group_cipher != SSTAR_WPA_CIPHER_AES_128_CMAC) {
					Sstar_printk_ap("Unsupported management group "
	...
	struct ieee802_Sstar_11_elems *ap_elems)
{
	...
	if(ap_elems->wps_ie&&ap_elems->wps_ie_len){
		Sstar_printk_ap( "%s:ap support wps\n",__func__);
		ap_wps = true;
	}
	if(sta_elems->wps_ie&&sta_elems->wps_ie_len){
		Sstar_printk_ap( "%s:sta support wps\n",__func__);
		sta_wps = true;
	}

	if(test_sta_flag(sta, WLAN_STA_WPA_RSN)){
		if(sta_wps==true){
			Sstar_printk_ap( "%s:sta support both rsn and wps\n",__func__);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
	}else if(sta_wps == true){
		if(ap_wps == false){
			Sstar_printk_ap( "%s:sta support wps but ap not support\n",__func__);
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
		else
			   ...
			   const u8 *ies, size_t ies_len, int reassoc,const u8 *ap_ie,size_t ap_ie_len)
{
	...
	memset(&sta_elems,0,sizeof(struct ieee802_Sstar_11_elems));
	memset(&ap_elems,0,sizeof(struct ieee802_Sstar_11_elems));
	...
	*/
	...
	if(status != WLAN_STATUS_SUCCESS)
	{
		Sstar_printk_ap( "%s:supp_rates err\n",__func__);
		goto assoc_ies_end;
	}
	/*
	*check ht cap
	*/
	...
	if(status != WLAN_STATUS_SUCCESS)
	{
		Sstar_printk_ap( "%s:check_ht_cap err\n",__func__);
		goto assoc_ies_end;
	}
	/*
	*check vht cap
	...
	*/
	...
	if(status != WLAN_STATUS_SUCCESS)
	{
		Sstar_printk_ap( "%s:check_wpa err\n",__func__);
		goto assoc_ies_end;
	}
	...
	if(status != WLAN_STATUS_SUCCESS)
	{
		Sstar_printk_ap( "%s:check_wps err\n",__func__);
		goto assoc_ies_end;
	}
assoc_ies_end:
...
static u16 ieee80211_ap_sme_alloc_aid(struct ieee80211_sub_if_data *sdata, struct sta_info *sta)
{
	...
	if(sdata->vif.bss_conf.aid_map >= (u8)(-1)){
		...
		Sstar_printk_ap( "%s:aid_map err(%x)\n",__func__,sta->sta.aid);
...
ieee80211_ap_sme_rx_mgmt_assoc(struct ieee80211_sub_if_data *sdata,
			   struct Sstar_ieee80211_mgmt *mgmt, size_t len,struct ieee80211_rx_status *rx_status,int reassoc,u16 *status)
{
	struct sta_info *sta = NULL;
	...
	**/
	...
	if(test_sta_flag(sta, WLAN_STA_ASSOC_AP))
	{
		Sstar_printk_ap( "%s:sta has connected with ap , so return err\n",__func__);
		goto mgmt_assoc_err;
	}
	...
	if (reassoc) {
		...
		left = len - offsetof(struct Sstar_ieee80211_mgmt, u.reassoc_req.variable);
		pos = mgmt->u.reassoc_req.variable;
	} else {
		capab_info = le16_to_cpu(mgmt->u.assoc_req.capab_info);
		listen_interval = le16_to_cpu(
			mgmt->u.assoc_req.listen_interval);
		left = len - offsetof(struct Sstar_ieee80211_mgmt, u.assoc_req.variable);
	...
	if (capab_info & SSTAR_WLAN_CAPABILITY_SHORT_PREAMBLE)
	...
	if(capab_info & SSTAR_WLAN_CAPABILITY_SHORT_SLOT_TIME)
	...
	{
		sta->associate_ie = Sstar_kmalloc(left,GFP_KERNEL);
		...
		Sstar_printk_ap( "%s:associate_ie(%x),associate_ie_len(%d)\n",
...
{
	struct Sstar_ieee80211_mgmt *reply;
	struct sk_buff *assoc_skb;

	assoc_skb = Sstar_dev_alloc_skb(sdata->local->hw.extra_tx_headroom +
			    sizeof(struct Sstar_ieee80211_mgmt) + 6 + extra_len);
	if(assoc_skb == NULL){
		Sstar_printk_ap( "%s:assoc_skb == NULL\n",__func__);
		return;
	}

	Sstar_skb_reserve(assoc_skb, sdata->local->hw.extra_tx_headroom);

	reply = (struct Sstar_ieee80211_mgmt *) Sstar_skb_put(assoc_skb, 24 + 6);;
	...
	if (extra)
		memcpy(Sstar_skb_put(assoc_skb, extra_len), extra, extra_len);

 	Sstar_printk_ap( "%s %d aid =%d,capab_info %d\n",__func__,__LINE__,aid,capab_info);
	...
	u16 status,struct sk_buff *skb,u8 reassoc)
{
	...
	**/
	...
	rep_ie = Sstar_kmalloc(1024,GFP_KERNEL);

	if(rep_ie == NULL)
	{
		Sstar_printk_ap( "%s:rep_ie == NULL\n",__func__);
		goto tx_mgmt_assoc_err;
	}

	if (sdata->vif.bss_conf.privacy)
		capab_info |= SSTAR_WLAN_CAPABILITY_PRIVACY;
	if(sdata->vif.bss_conf.use_short_preamble&&
		(sdata->vif.bss_conf.num_sta_no_short_preamble == 0))
		capab_info |= SSTAR_WLAN_CAPABILITY_SHORT_PREAMBLE;
	if(sdata->vif.bss_conf.use_short_slot&&
		(sdata->vif.bss_conf.num_sta_no_short_slot_time==0))
		capab_info |= SSTAR_WLAN_CAPABILITY_SHORT_SLOT_TIME;
	...
	if(beacon)
	{
		...
		*/
		SSTAR_SET_SUPP_RATE_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
		SSTAR_SET_EXT_RATE_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
		SSTAR_SET_HT_CAP_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
		SSTAR_SET_HT_INFO_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);

		if(test_sta_flag(sta, WLAN_STA_WME))
		{
			SSTAR_SET_VENDOR_WMM_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
		}
		...
		if(assoc_ie){
			...
			*/
			Sstar_printk_ap( "%s:set assoc_ie\n",__func__);
		...
		else
		{
			...
			*/
			SSTAR_SET_VENDOR_WPS_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
			SSTAR_SET_VENDOR_P2P_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
			SSTAR_SET_VENDOR_WFD_IE(rep_ie,rep_ie_len,beacon_ie,beacon_ie_len);
		}
	}
	else
	{
		Sstar_printk_ap( "%s:beacon==NULL\n",__func__);
	}

	Sstar_printk_ap( "%s:sta(%pM),aid(%d),cap(%x),req_ie(%x),rep_ie_len(%d),status(%d)\n",__func__,
...
static void ieee80211_ap_sme_tx_mgmt_assoc_status(struct ieee80211_sub_if_data *sdata,struct sk_buff *skb)
{
	...
	if(status_code != WLAN_STATUS_SUCCESS)
	{
		...
		clear_sta_flag(sta, WLAN_STA_AUTH_STATUS);
		clear_sta_flag(sta, WLAN_STA_AUTHORIZED);
		Sstar_printk_ap( "%s:status_code != WLAN_STATUS_SUCCESS\n",__func__);
		goto assoc_status_err;
	}
	...
	{
		clear_sta_flag(sta, WLAN_STA_ASSOC);
		Sstar_printk_ap( "%s:no ack\n",__func__);
		goto assoc_status_err;
	}

	if(!test_sta_flag(sta,WLAN_STA_ASSOC))
	{
		Sstar_printk_ap( "%s:not assoc\n",__func__);
		goto assoc_status_err;
	}

	if(!sta->associate_ie)
	{
		Sstar_printk_ap( "%s:!sta->associate_ie\n",__func__);
		clear_sta_flag(sta, WLAN_STA_ASSOC);
		goto assoc_status_err;
	}
	...
	*/
	...
	if(!sta_info_reinsert(sta))	{
		Sstar_printk_ap( "%s:STA_EVENT__ASSOCED\n",__func__);
		ieee80211_ap_sme_queue_event(sdata,STA_EVENT__ASSOCED,mgmt->da);
	}
	else{
		Sstar_printk_ap( "%s:STA_EVENT__SEND_DEAUTHEN\n",__func__);
			   ...
			   struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	if(!test_sta_flag(sta, WLAN_STA_AUTH))
	{
		Sstar_printk_ap( "rx_mgmt_disassoc sa(%pM),not auten\n",mgmt->sa);
	}

	if(!test_sta_flag(sta, WLAN_STA_AUTHORIZED))
	{
		Sstar_printk_ap( "rx_mgmt_disassoc sa(%pM),not autened\n",mgmt->sa);
	}

	if(!test_sta_flag(sta, WLAN_STA_ASSOC))
	{
		Sstar_printk_ap( "rx_mgmt_disassoc sa(%pM),not assoc\n",mgmt->sa);
	}

	if(!test_sta_flag(sta, WLAN_STA_ASSOC_AP))
	{
		Sstar_printk_ap( "rx_mgmt_disassoc sa(%pM),not assoced ap\n",mgmt->sa);
	}
	else
	{
		assoced = true;
	...
	Sstar_printk_ap( "rx_mgmt_disassoc sa(%pM),reason_code(%d)\n",mgmt->sa,reason_code);
	rcu_read_unlock();
	...
	struct ieee80211_sub_if_data *sdata,struct sk_buff *skb)
{
	...
	Sstar_printk_ap( "%s:sta(%pM) rx deauthen or disassoc status\n",__func__,mgmt->da);
...
static void ieee80211_ap_sme_work(struct Sstar_work_struct *work)
{
	...
	if (!ieee80211_sdata_running(sdata)){
		Sstar_printk_ap( "%s:sdata not running\n",__func__);
		return;
	}

	mutex_lock(&sdata->ap_sme_mlme_lock);
	while ((skb = Sstar_skb_dequeue(&sdata->ap_sme_skb_queue))) {
		struct Sstar_ieee80211_mgmt *mgmt = (void *)skb->data;
	...
	list_for_each_entry(event, &sdata->ap_sme_event, list) {
		switch(event->event){
			case STA_EVENT__ASSOCED:
			{
				...
				associate_ie = Sstar_kmalloc(sta->associate_ie_len,GFP_KERNEL);
				...
				Sstar_printk_ap( "%s:notify 80211 new sta(%pM)\n",__func__,event->sta_addr);
			...
			sta_free:
			{
				Sstar_printk_ap( "STA_EVENT__FREE+++++++\n");
