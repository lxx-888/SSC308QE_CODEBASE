...
static void ieee80211_hw_roc_start(struct Sstar_work_struct *work)
{
...
void ieee80211_start_next_roc(struct ieee80211_local *local)
{
	...
	if (local->ops->remain_on_channel) {
		...
		if (ret) {
			Sstar_printk_warn("failed to start next HW ROC (%d)\n",
					  ret);
			/*
			...
			ieee80211_remain_on_channel_expired(
				&local->hw,
				(roc->mgmt_tx_cookie ? roc->mgmt_tx_cookie :
							     roc->cookie));
		} else
...
void ieee80211_roc_notify_destroy(struct ieee80211_roc_work *roc)
{
	...
	if (!roc->mgmt_tx_cookie)
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0))
		cfg80211_remain_on_channel_expired(roc->hw_roc_dev, roc->cookie,
						   roc->chan, roc->chan_type,
						   GFP_KERNEL);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 0))
		...
		cfg80211_remain_on_channel_expired(
			&roc->sdata->wdev, roc->cookie, roc->chan, GFP_KERNEL);
#endif
...
static void ieee80211_hw_roc_done(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local =
	...
	Sstar_printk_mgmt("%s:cookie(%x),roc_cookie(%x)\n", __func__,
			  (int)cookie, (int)local->roc_cookie);
...
{
	SSTAR_INIT_WORK(&local->hw_roc_start, ieee80211_hw_roc_start);
	SSTAR_INIT_WORK(&local->hw_roc_done, ieee80211_hw_roc_done);
...
void ieee80211_roc_purge(struct ieee80211_sub_if_data *sdata)
{
	...
	list_for_each_entry_safe (roc, tmp, &tmp_list, list) {
		if (local->ops->remain_on_channel) {
			...
			Sstar_flush_delayed_work(&roc->work);
		}
	}
