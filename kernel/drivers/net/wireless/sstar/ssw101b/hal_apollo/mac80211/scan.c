					 ...
					 struct ieee80211_scan_req_wrap *req)
{
	...
	*/
	...
	if (!(req->flags & IEEE80211_SCAN_REQ_NEED_LISTEN)) {
		...
		Sstar_printk_debug("%s:filter\n", __func__);
	} else {
#ifndef CONFIG_SSTAR_STA_LISTEN
		return -1;
#else
		Sstar_printk_debug("%s:listen\n", __func__);
		if (local->ops->sta_triger_listen)
			local->ops->sta_triger_listen(&local->hw, &sdata->vif,
						      req->req->channels[0]);
		else
			return -1;
#endif
	}
	skb = ieee80211_probereq_get(
		&local->hw, &sdata->vif, NULL, 0, req->req->ie,
		req->req->ie_len,
	...
	local->scan_idle_time = 300 * req->req->n_channels > 10000 ?
					      10000 :
					      100 * req->req->n_channels;
...
struct country_chan country_t[4] = { { "CN", 13 },
				     { "JP", 14 },
				     { "US", 11 },
				     { NULL, 0 } };
#endif
...
*/
static int
ieee80211_check_country_limit_scan_2_4G_chan(struct ieee80211_local *local,
					     struct cfg80211_scan_request *req)
{
	...
	*/
	...
	if ((country_chan > 0) && (req != NULL)) {
		Sstar_printk_err("%s : scan_n_channals = %d ++++++++ \n",
	...
	*/
		...
		for (j = 0; j < req->n_channels; j++) {
			if (req->channels[j] == NULL) {
				Sstar_printk_err(
					"%s : channals is NULL ! not allow scan\n",
					__func__);
				return -1;
			}
		...
		*/
		if (scan_n_chans <= 0) {
			Sstar_printk_err("%s : not allow scan ! \n", __func__);
			return -1;
		}
					     ...
					     struct ieee80211_channel *channel)
{
	...
	if ((country_chan > 0) && (skb != NULL)) {
		...
		//Sstar_printk_err("skb : country_chan = %d curr_chan = %d\n",country_chan,curr_chan);
		if (curr_chan > country_chan) {
			Sstar_printk_err(
				"%s : current scan results channals = %d,limit channel=%d ++++++++ \n",
		...
		curr_chan = ieee80211_frequency_to_channel(freq);
		//Sstar_printk_err("channel : country_chan = %d curr_chan = %d\n",country_chan,curr_chan);
		if (curr_chan > country_chan) {
			Sstar_printk_err(
				"%s : current scan results channals = %d,limit channel=%d ++++++++ \n",
				__func__, curr_chan, country_chan);
		...
		Sstar_printk_debug(
			"ieee80211_2_4G_scan_results_limit:%d :country_chan = %d \n",
...
static void ieee80211_scan_try_split(struct ieee80211_local *local)
{
	...
	Sstar_printk_cfg("%s:tp(%ld)(%d)\n", __func__, total_tp,
			 req->n_channels);
	if (total_tp <= IEEE80211_SCAN_SPIT_TP_500K) {
		local->scan_idle_time = 0;
		local->scan_channel_space =
			req->n_channels - local->scan_n_channels;
	...
	Sstar_printk_cfg("%s:space(%d),idle(%ld)\n", __func__,
			 local->scan_channel_space, local->scan_idle_time);
}
struct ieee80211_bss *ieee80211_rx_bss_get(struct ieee80211_local *local,
					   u8 *bssid, int freq, u8 *ssid,
					   u8 ssid_len)
{
	struct cfg80211_bss *cbss;

	cbss = ieee80211_Sstar_get_bss(
		local->hw.wiphy, ieee80211_get_channel(local->hw.wiphy, freq),
			  ...
			  struct ieee80211_bss *bss)
{
	if (!bss)
		return;
	ieee80211_Sstar_put_bss(local->hw.wiphy,
...
/*
	...
	*/
#define SSTAR_BOOT_TIME                                                        \
	0x7fffffffffffffffLL //((u64)(ktime_to_ns(ktime_get_boottime())>>10))
#endif
...
#undef SSTAR_BOOT_TIME
	...
	cbss = cfg80211_inform_bss_frame(local->hw.wiphy, channel,
					 (struct ieee80211_mgmt *)mgmt, len,
					 signal, GFP_ATOMIC);
	...
	bss->noise_floor =
		local->noise_floor[(channel_hw_value(channel) - 1) % 13]; //-92;
...
bool ieee80211_scan_internal_req_results(
	...
	struct Sstar_internal_scan_results_req *req)
{
	...
	for (hash_index = 0; hash_index < SSTAR_COMMON_HASHENTRIES;
		...
		hlist_for_each_safe (node, node_temp, hlist) {
			sta_node =
				hlist_entry(node,
					    struct Sstar_internal_scan_sta_node,
					    hnode);
...
void ieee80211_scan_internal_int(struct ieee80211_local *local)
{
	...
	atomic_set(&local->internal_scan_status,
		   IEEE80211_INTERNAL_SCAN_STATUS__IDLE);
	spin_lock_bh(&local->internal_scan_list_lock);
	Sstar_common_hash_list_init(local->internal_scan_list,
				    SSTAR_COMMON_HASHENTRIES);
	spin_unlock_bh(&local->internal_scan_list_lock);
}
static void
ieee80211_scan_rx_internal_update(struct ieee80211_local *local,
				  struct ieee80211_internal_scan_result *result)
{
	...
	Sstar_printk_debug(
		"%s:ssid[%s],mac[%pM],channel[%d],signal[%d],beacon[%d]\n",
		__func__, result->sta.ssid, result->sta.bssid,
		result->sta.channel, result->sta.signal, result->sta.beacon);
	spin_lock_bh(&local->internal_scan_list_lock);
	/*
	...
	*/
	if (result->sta.ssid_len)
		hash_index =
			Sstar_hash_index(result->sta.ssid, result->sta.ssid_len,
	...
	hlist_for_each (node, hlist) {
		sta_node = hlist_entry(
			node, struct Sstar_internal_scan_sta_node, hnode);
		if ((result->sta.ssid_len == sta_node->sta.ssid_len) &&
		    (!memcmp(result->sta.bssid, sta_node->sta.bssid, 6))) {
			/*
			*hidden ssid
			*/
			if ((result->sta.ssid_len == 0) ||
			    (!memcmp(sta_node->sta.ssid, result->sta.ssid,
				     sta_node->sta.ssid_len)))
	...
	*/
	if (sta_node_target == NULL) {
		sta_node_target = Sstar_kzalloc(
			sizeof(struct Sstar_internal_scan_sta_node),
			GFP_ATOMIC);
	...
	*/
	...
	memcpy(&sta_node_target->sta, &result->sta,
	       sizeof(struct ieee80211_internal_scan_sta));
	sta_node_target->sta.ssid_len = result->sta.ssid_len;
	if (result->sta.ssid_len)
		memcpy(sta_node_target->sta.ssid, result->sta.ssid,
		       result->sta.ssid_len);
	if (sta_node_target->sta.ie)
		Sstar_printk_debug("%s:ie(%s)\n", __func__,
				   sta_node_target->sta.ie);
...
bool ieee80211_scan_rx_internal_default(
	struct ieee80211_sub_if_data *sdata,
...
void ieee80211_scan_cca_notify(struct ieee80211_hw *hw,
			       struct ieee80211_internal_scan_notity *notify)
{
	...
	*/
	...
	for (index = 0; index < req->n_channels; index++) {
		...
		req_wrap->cca_val[channel_hw_value(req->channels[index]) - 1] =
			cca_val[index];
	}
}
...
{
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_NONE)
		return SSTAR_WPA_CIPHER_NONE;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_WEP40)
		return SSTAR_WPA_CIPHER_WEP40;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_TKIP)
		return SSTAR_WPA_CIPHER_TKIP;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_CCMP)
		return SSTAR_WPA_CIPHER_CCMP;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_WEP104)
		return SSTAR_WPA_CIPHER_WEP104;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_AES_128_CMAC)
		return SSTAR_WPA_CIPHER_AES_128_CMAC;
	if (SSTAR_WPA_GET_BE32(s) == SSTAR_RSN_CIPHER_SUITE_GCMP)
		return SSTAR_WPA_CIPHER_GCMP;
	return 0;
}

int get_ap_encryption_algorithm(char *ie, int ie_len, u8 *encry_info,
				enum ieee80211_enc_type enc_type)
{
	...
	case IEEE80211_ENC_WPA: {
		...
		if (ie == NULL || ie_len <= 0) {
			Sstar_printk_scan(
				"get_ap_encryption_algorithm : parameters err! %s,ie_len(%d) \n",
		...
		if (left >= 4) {
			...
			Sstar_printk_err(
				"get_ap_encryption_algorithm : ie too short \n");
			break;
		}
		if (left >= 2) {
			/* Number of Unicasts: */
			encry_type_count = SSTAR_WPA_GET_LE16(pos);
			pos += 2;
			left -= 2;
			if ((encry_type_count == 0) ||
			    (left < encry_type_count * 4)) {
				Sstar_printk_err(
					"%s: ie count botch (pairwise), "
					"count %u left %u",
					__func__, encry_type_count, left);
				break;
			}
			...
			for (alg_i = 0; alg_i < encry_type_count; alg_i++) {
				if (left < 0) {
					Sstar_printk_err(
						"%s ,out of range IEEE80211_ENC_WPA\n",
						__func__);
					break;
				}
	...
	case IEEE80211_ENC_WPA2: {
		...
		if (ie == NULL || ie_len <= 0) {
			Sstar_printk_scan(
				"get_ap_encryption_algorithm IEEE80211_ENC_WPA2: parameters err! %s,ie_len(%d) \n",
				ie == NULL ? "ie is null" : "", ie_len);
			return -1;
		}
		/* jump Version */
		...
		if (left >= 4) {
			...
			Sstar_printk_err(
				"get_ap_encryption_algorithm IEEE80211_ENC_WPA2 : ie too short \n");
			break;
		}
		if (left >= 2) {
			...
			if ((encry_type_count == 0) ||
			    (left < encry_type_count * 4)) {
				Sstar_printk_err(
					"%s: IEEE80211_ENC_WPA2 ie count botch (pairwise), "
					"count %u left %u",
					__func__, encry_type_count, left);
				break;
			}
			for (alg_i = 0; alg_i < encry_type_count; alg_i++) {
				if (left < 0) {
					Sstar_printk_err(
						"%s IEEE80211_ENC_WPA2,out of range IEEE80211_ENC_WPA2\n",
						__func__);
					return -1;
				}
				encry_flag |= get_selector_to_bitfield(pos);
...
ieee80211_scan_rx_internal_sta_info(struct ieee80211_sub_if_data *sdata,
				    struct sk_buff *skb)
{
	...
	bool presp, beacon = false;
	struct ieee802_Sstar_11_elems elems;
	bool (*func)(struct ieee80211_sub_if_data * sdata, void *fs_data,
		     struct ieee80211_internal_scan_result *result,
		     bool finish);
	void *scan_data;
	...
	if (elems.rsn && elems.rsn_len && elems.wpa && elems.wpa_len) {
		...
		/*get WPA2 encryption algorithm */
		get_ap_encryption_algorithm(elems.rsn, elems.rsn_len,
					    &elems.encry_info,
					    IEEE80211_ENC_WPA2);
		/*get WPA encryption algorithm */
		get_ap_encryption_algorithm(elems.wpa, elems.wpa_len,
					    &elems.encry_info,
					    IEEE80211_ENC_WPA);
		...
		get_ap_encryption_algorithm(elems.wpa, elems.wpa_len,
					    &elems.encry_info,
					    IEEE80211_ENC_WPA);
	} else if (((presp == true) && (mgmt->u.probe_resp.capab_info &
					SSTAR_WLAN_CAPABILITY_PRIVACY)) ||
		   ((beacon == true) && (mgmt->u.beacon.capab_info &
					 SSTAR_WLAN_CAPABILITY_PRIVACY))) {
	...
	if (ieee80211_get_channel(sdata->local->hw.wiphy, freq))
		scan_info.sta.channel = channel_hw_value(
			ieee80211_get_channel(sdata->local->hw.wiphy, freq));
	else {
		Sstar_printk_err("%s %d  freq = %d channel_ptr = NULL\n",
	...
	if (elems.Sstar_special_len && elems.Sstar_special) {
		scan_info.sta.ie =
			Sstar_kzalloc(elems.Sstar_special_len, GFP_ATOMIC);

		if (scan_info.sta.ie == NULL) {
			handle = RX_DROP_MONITOR;
			goto err;
		}
		memcpy(scan_info.sta.ie, elems.Sstar_special,
		       elems.Sstar_special_len);
	...
	if (func) {
		if (func(sdata->local->scan_sdata, scan_data, &scan_info,
			 false) == false) {
			handle = RX_DROP_MONITOR;
			if (scan_info.sta.ie)
				Sstar_kfree(scan_info.sta.ie);
	...
	} else if (sdata->local->scan_req_wrap.flags &
		   IEEE80211_SCAN_REQ_RESULTS_HANDLE) {
		ieee80211_scan_rx_internal_default(sdata->local->scan_sdata,
						   &scan_info, false);
	} else {
		if (scan_info.sta.ie)
			Sstar_kfree(scan_info.sta.ie);
...
static ieee80211_rx_result
ieee80211_scan_rx_internal_skb(struct ieee80211_sub_if_data *sdata,
			       struct sk_buff *skb)
{
	struct Sstar_ieee80211_mgmt *mgmt;
	ieee80211_rx_result handle = RX_QUEUED;
	...
	if (func(sdata->local->scan_sdata, scan_data, &scan_info, false) ==
	    false) {
		handle = RX_CONTINUE;
		goto err;
	}
...
ieee80211_rx_result
ieee80211_scan_rx_internal(struct ieee80211_sub_if_data *sdata,
			   struct sk_buff *skb)
{
	ieee80211_rx_result handle = RX_QUEUED;
	...
	if (sdata->local->scan_req_wrap.flags &
	    IEEE80211_SCAN_REQ_RESULTS_SKB) {
				      ...
				      struct sk_buff *skb)
{
	...
	bss = ieee80211_bss_info_update(sdata->local, rx_status, mgmt, skb->len,
					&elems, channel, beacon);
	if (bss)
...
static u16 ieee80211_scan_oper_channel(struct ieee80211_local *local)
{
	...
	struct ieee80211_channel_state *chan_state =
		ieee80211_get_channel_state(local, local->scan_sdata);

	switch (ieee80211_get_channel_mode(local, NULL)) {
	case CHAN_MODE_HOPPING:
	case CHAN_MODE_FIXED:
		oper_channel = channel_hw_value(chan_state->conf.channel);
		Sstar_printk_debug("%s:current op channel[%d]\n", __func__,
				   oper_channel);
		break;
	...
	*/
	...
	if (oper_channel && local->scan_sdata &&
	    !local->scan_sdata->u.mgd.associated) {
#ifndef CONFIG_SSTAR_STA_LISTEN
		oper_channel = 0;
#else
...
static bool ieee80211_prep_hw_scan(struct ieee80211_local *local)
{
	...
	while (conf) {
		struct ieee80211_channel *ap_channel =
			ieee8011_chnum_to_channel(&local->hw, conf->channel);
	...
	do {
		...
		for (; i < req->n_channels; i++) {
			...
			*/
			...
			if (n_chans && (oper_channel ==
					channel_hw_value(req->channels[i]))) {
				goto start_scan;
			}
			...
			if (oper_channel ==
			    channel_hw_value(req->channels[i])) {
				Sstar_printk_debug("%s:only send probe\n",
						   __func__);
				local->scan_req_wrap.flags |=
					IEEE80211_SCAN_REQ_ONLY_PROB;
				BUG_ON(n_chans != 1);
				goto start_scan;
			}
...
start_scan:
	...
	Sstar_printk_err("%s:n_chans(%d),space(%d),index(%d),scaned(%d)\n",
			 __func__, n_chans, local->scan_channel_space,
			 local->scan_channel_idx, local->scan_n_channels);
...
void ieee80211_run_pending_scan(struct ieee80211_local *local)
{
	...
	if (local->scanning) {
		Sstar_printk_scan(
			"%s:scanning,so cannot active pennding scan\n",
			__func__);
		WARN_ON(1);
		return;
	}
	...
	if (local->pending_scan_req && local->pending_scan_sdata) {
		if (ieee80211_sdata_running(local->pending_scan_sdata)) {
			...
			Sstar_printk_scan(
				"%s:sdata is not running,but scan,err!!!!!!\n",
				__func__);
			Sstar_notify_scan_done(local, local->pending_scan_req,
					       true);
			local->pending_scan_req = NULL;
			local->pending_scan_sdata = NULL;
		}
...
#endif
static void ieee80211_internal_scan_completed(struct ieee80211_hw *hw,
					      bool aborted)
{
	struct ieee80211_local *local = hw_to_local(hw);
	...
	if (func)
	...
	else if (local->scan_req_wrap.flags &
		 IEEE80211_SCAN_REQ_RESULTS_HANDLE) {
		WARN_ON(atomic_read(&local->internal_scan_status) !=
		...
		if (aborted == true)
			...
			atomic_set(&local->internal_scan_status,
				   IEEE80211_INTERNAL_SCAN_STATUS__FINISHED);
				       ...
				       bool was_hw_scan)
{
	...
	*/
	if (local->scan_req_wrap.flags & IEEE80211_SCAN_REQ_ONLY_PROB) {
		...
		if (!(local->scan_req_wrap.flags &
		      IEEE80211_SCAN_REQ_NEED_LISTEN))
			ieee80211_configure_filter(local->scan_sdata);
		else {
#ifdef CONFIG_SSTAR_STA_LISTEN
			local->scan_req_wrap.flags &=
				~IEEE80211_SCAN_REQ_NEED_LISTEN;
			local->ops->sta_stop_listen(&local->hw,
						    &local->scan_sdata->vif);
			Sstar_printk_debug("%s:exit listen\n", __func__);
#endif
		}
	}
	if (was_hw_scan && !aborted && ieee80211_prep_hw_scan(local)) {
		int rc = ieee80211_drv_hw_scan(local, local->scan_sdata,
					       &local->scan_req_wrap);
		if (rc == 0) {
...
void ieee80211_scan_completed(struct ieee80211_hw *hw, bool aborted)
{
	...
	ieee80211_queue_delayed_work(&local->hw, &local->scan_work,
				     msecs_to_jiffies(local->scan_idle_time));
}
				  ...
				  struct cfg80211_scan_request *req)
{
	...
	if (!list_empty(&local->roc_list)) {
		...
		*/
		...
		if ((local->pending_scan_req != NULL) ||
		    (local->pending_scan_sdata != NULL)) {
			Sstar_printk_scan(
				"%s:only can pending one scan request\n",
				__func__);
			return -EBUSY;
		} else {
			...
			Sstar_printk_scan(
				"%s:%d: offch runing ,so delay scanning\n",
				__func__, __LINE__);
		}
		return 0;
	}
	...
	if (local->ops->hw_scan) {
		...
		local->hw_scan_req = Sstar_kmalloc(
			sizeof(*local->hw_scan_req) +
				req->n_channels * sizeof(req->channels[0]) + 2 +
				IEEE80211_MAX_SSID_LEN + local->scan_ies_len +
				req->ie_len,
			GFP_KERNEL);
		...
		ies = (u8 *)local->hw_scan_req + sizeof(*local->hw_scan_req) +
	...
	*/
	...
	if (local->ops->hw_scan) {
		...
		if (sdata->last_scan_ie_len < local->hw_scan_req->ie_len) {
			...
			sdata->last_scan_ie = Sstar_kmalloc(
				local->hw_scan_req->ie_len, GFP_KERNEL);
			WARN_ON(sdata->last_scan_ie == NULL);
		}
		if (sdata->last_scan_ie && local->hw_scan_req->ie) {
			memcpy(sdata->last_scan_ie, local->hw_scan_req->ie,
			       local->hw_scan_req->ie_len);
...
void ieee80211_scan_work(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local =
...
bool ieee80211_internal_scan_triger(struct ieee80211_sub_if_data *sdata,
				    struct cfg80211_scan_request *req)
{
	struct ieee80211_local *local = sdata->local;
	int rc;
	...
	if (local->internal_scan.req.req_flags &
	    IEEE80211_INTERNAL_SCAN_FLAGS__CCA) {
		Sstar_printk_err("%s:hw.conf.flags(%x)\n", __func__,
				 local->hw.conf.flags);
		if (!!(local->hw.conf.flags & IEEE80211_CONF_IDLE) == 0) {
			//			Sstar_printk_err("now running sta or ap,not idle, maybe have some queue! \n");
			//			return false;
		}
	} else {
		if ((!!(local->hw.conf.flags & IEEE80211_CONF_IDLE) == 0) &&
		    req->n_channels > 2) {
			Sstar_printk_debug("%s:running split scan\n", __func__);
			local->internal_scan.req.req_flags |=
				IEEE80211_INTERNAL_SCAN_FLAGS__SCAN_SPLIT;
		}
	}
	ieee80211_scan_internal_list_flush(local);
	memset(&local->scan_info, 0, sizeof(struct cfg80211_scan_info));
	...
	memset(&local->scan_req_wrap, 0,
	       sizeof(struct ieee80211_scan_req_wrap));
	...
	if (local->internal_scan.req.req_flags &
	    IEEE80211_INTERNAL_SCAN_FLAGS__PASSAVI_SCAN)
		local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_PASSIVE_SCAN;
	if (local->internal_scan.req.req_flags &
	    IEEE80211_INTERNAL_SCAN_FLAGS__NEED_SKB)
	...
	if (local->internal_scan.req.req_flags &
	    IEEE80211_INTERNAL_SCAN_FLAGS__NEED_BSSID) {
		local->scan_req_wrap.flags |= IEEE80211_SCAN_REQ_NEED_BSSID;
		memcpy(local->scan_req_wrap.bssid,
		       local->internal_scan.req.bssid, 6);
	}
	local->scan_req_wrap.req = local->hw_scan_req;
	WARN_ON(!ieee80211_prep_hw_scan(local));
	rc = ieee80211_drv_hw_scan(local, sdata, &local->scan_req_wrap);
...
void ieee80211_scan_cancel(struct ieee80211_local *local)
{
	 ...
	 */
	Sstar_cancel_delayed_work(&local->scan_work);
	/* and clean up */
	__ieee80211_scan_completed(
		&local->hw, true,
		test_bit(SCAN_HW_SCANNING, &local->scanning) ? true : false);
out:
	mutex_unlock(&local->mtx);
}
				       ...
				       struct cfg80211_sched_scan_request *req)
{
	...
	for (i = 0; i < IEEE80211_NUM_BANDS; i++) {
		...
		local->sched_scan_ies.ie[i] =
			Sstar_kzalloc(2 + IEEE80211_MAX_SSID_LEN +
		...
		local->sched_scan_ies.len[i] = ieee80211_build_preq_ies(
			local, local->sched_scan_ies.ie[i], req->ie,
			req->ie_len, i, (u32)-1, 0);
	}

	ret = drv_sched_scan_start(local, sdata, req, &local->sched_scan_ies);
	if (ret == 0) {
...
void ieee80211_sched_scan_stopped_work(struct Sstar_work_struct *work)
{
	struct ieee80211_local *local = container_of(
		work, struct ieee80211_local, sched_scan_stopped_work);
	int i;
