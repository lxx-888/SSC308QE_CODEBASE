...
void ieee80211_mesh_root_setup(struct ieee80211_if_mesh *ifmsh)
{
	if (ifmsh->mshcfg.dot11MeshHWMPRootMode)
		...
		/* stop running timer */
		Sstar_del_timer_sync(&ifmsh->mesh_path_root_timer);
	}
}
			   ...
			   struct ieee80211_if_mesh *ifmsh)
{
	...
	Sstar_mod_timer(&ifmsh->housekeeping_timer,
...
static void ieee80211_mesh_rootpath(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_mod_timer(&ifmsh->mesh_path_root_timer,
...
void ieee80211_mesh_quiesce(struct ieee80211_sub_if_data *sdata)
{
	...
	if (Sstar_del_timer_sync(&ifmsh->housekeeping_timer))
		set_bit(TMR_RUNNING_HK, &ifmsh->timers_running);
	if (Sstar_del_timer_sync(&ifmsh->mesh_path_timer))
		set_bit(TMR_RUNNING_MP, &ifmsh->timers_running);
	if (Sstar_del_timer_sync(&ifmsh->mesh_path_root_timer))
		set_bit(TMR_RUNNING_MPR, &ifmsh->timers_running);
}
...
void ieee80211_mesh_restart(struct ieee80211_sub_if_data *sdata)
{
	...
	if (test_and_clear_bit(TMR_RUNNING_HK, &ifmsh->timers_running))
		Sstar_add_timer(&ifmsh->housekeeping_timer);
	if (test_and_clear_bit(TMR_RUNNING_MP, &ifmsh->timers_running))
		Sstar_add_timer(&ifmsh->mesh_path_timer);
	if (test_and_clear_bit(TMR_RUNNING_MPR, &ifmsh->timers_running))
		Sstar_add_timer(&ifmsh->mesh_path_root_timer);
...
void ieee80211_stop_mesh(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_del_timer_sync(&sdata->u.mesh.housekeeping_timer);
	Sstar_del_timer_sync(&sdata->u.mesh.mesh_path_root_timer);
	/*
...
void ieee80211_mesh_init_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_setup_timer(&ifmsh->housekeeping_timer,
	...
	if (!mesh_allocated)
		ieee80211s_init();
	Sstar_setup_timer(&ifmsh->mesh_path_timer,
		    ieee80211_mesh_path_timer,
		    (unsigned long) sdata);
	Sstar_setup_timer(&ifmsh->mesh_path_root_timer,
