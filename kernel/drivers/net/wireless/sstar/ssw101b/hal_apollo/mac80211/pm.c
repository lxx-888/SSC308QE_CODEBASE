...
#endif
static void
ieee80211_suspend_sta_disconnect(struct ieee80211_sub_if_data *sdata)
{
...
int __ieee80211_suspend(struct ieee80211_hw *hw, struct cfg80211_wowlan *wowlan)
{
	...
	if (mutex_trylock(&local->mtx))
		...
		Sstar_printk_warn(
			"Suspend when operation is in progress. Suspend aborted.\n");
		return -EBUSY;
	}
	...
	Sstar_del_timer_sync(&local->sta_cleanup);
	...
	Sstar_printk_pm("enable wowlan(%d)\n", local->wowlan);
	...
	if (local->wowlan) {
		...
		if (err < 0) {
			...
			ieee80211_wake_queues_by_reason(
				hw, IEEE80211_QUEUE_STOP_REASON_SUSPEND);
			return err;
		} else if (err > 0) {
			WARN_ON(err != 1);
			local->wowlan = false;
