...
static void ieee80211_sdata_listen_recalc(struct ieee80211_local *local,
					  bool start, bool flush);
#endif
/**
...
static void ieee80211_medium_traffic_timer(unsigned long data)
{
	...
	current_tx_bytes =
		sdata->dev->stats.tx_bytes - sdata->traffic.last_tx_bytes;
	current_rx_bytes =
		sdata->dev->stats.rx_bytes - sdata->traffic.last_rx_bytes;
	...
	Sstar_printk_debug("%s:tx_tp(%ld),rx_tp(%ld)\n", sdata->name,
			   sdata->traffic.current_tx_tp,
			   sdata->traffic.current_rx_tp);
	Sstar_mod_timer(
		&sdata->traffic.traffic_timer,
		jiffies + msecs_to_jiffies(IEEE80211_MEDIUM_TRAFFIC_PRIOD));
#ifdef CONFIG_SSTAR_STA_DYNAMIC_PS
	rcu_read_lock();
	if (sdata->vif.type == NL80211_IFTYPE_STATION &&
	    sdata->u.mgd.associated) {
		u32 need_ps = !!(sdata->traffic.current_tx_tp +
					 sdata->traffic.current_rx_tp <
				 IEEE80211_KEEP_WAKEUP_TP_PER_SECOND);
		u32 current_ps = !!(sdata->ps_allowed == true &&
				    sdata->vif.bss_conf.ps_enabled == true);
		Sstar_printk_debug(
			"%s:need_ps(%d),current_ps(%d),ps_allowed(%d),ps_enabled(%d)\n",
			__func__, need_ps, current_ps, sdata->ps_allowed,
			sdata->vif.bss_conf.ps_enabled);
		if (need_ps != current_ps) {
			struct sk_buff *skb = Sstar_dev_alloc_skb(0);

			if (skb) {
				skb->pkt_type = IEEE80211_SDATA_IDLE_RECAL;
				Sstar_skb_queue_tail(&sdata->skb_queue, skb);
...
{
	Sstar_del_timer_sync(&sdata->traffic.traffic_timer);
	sdata->traffic.current_rx_tp = 0;
	sdata->traffic.current_tx_tp = 0;
	sdata->traffic.last_tx_bytes = sdata->dev->stats.tx_bytes;
	sdata->traffic.last_rx_bytes = sdata->dev->stats.rx_bytes;
...
void ieee80211_medium_traffic_init(struct ieee80211_sub_if_data *sdata)
{
	...
	sdata->traffic.last_tx_bytes = sdata->dev->stats.tx_bytes;
	sdata->traffic.last_rx_bytes = sdata->dev->stats.rx_bytes;
	Sstar_setup_timer(&sdata->traffic.traffic_timer,
			  ieee80211_medium_traffic_timer, (unsigned long)sdata);
}
...
static int ieee80211_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	Sstar_printk_debug("%s: setting MTU %d\n", dev->name, new_mtu);
#endif /* CONFIG_MAC80211_SSTAR_VERBOSE_DEBUG */
	dev->mtu = new_mtu;
	return 0;
}
#ifdef CONFIG_SSTAR_SUPPORT_REALTIME_CHANGE_MAC
static int ieee80211_change_mac(struct net_device *dev, void *addr)
{
	...
	for (i = 0; i < hw->wiphy->n_addresses; i++) {
		if (memcmp(sdata->vif.addr, hw->wiphy->addresses[i].addr,
			   ETH_ALEN))
			continue;
		found = i;
		break;
	...
	if (found != 0) {
		Sstar_printk_err("[%s] do not support change mac\n",
				 sdata->name);
		return -EBUSY;
	}
	list_for_each_entry (other_sdata, &local->interfaces, list) {
		if (memcmp(local->hw.wiphy->addresses[1].addr,
			...
			if (eth_mac_addr(other_sdata->dev, &other_sa) == 0) {
				memcpy(other_sdata->vif.addr, other_sa.sa_data,
				       ETH_ALEN);
				call_netdevice_notifiers(NETDEV_CHANGEADDR,
							 other_sdata->dev);
			}
			Sstar_printk_always("[%s] change mac[%pM]\n",
					    other_sdata->name,
					    other_sa.sa_data);
			break;
		}
...
static inline int identical_mac_addr_allowed(int type1, int type2)
{
	return type1 == NL80211_IFTYPE_MONITOR ||
		...
		(type2 == NL80211_IFTYPE_WDS || type2 == NL80211_IFTYPE_AP)) ||
	       (type1 == NL80211_IFTYPE_AP &&
					    ...
					    enum nl80211_iftype iftype)
{
	...
	list_for_each_entry (nsdata, &local->interfaces, list) {
		...
		if (ndev != dev && ieee80211_sdata_running(nsdata)) {
			Sstar_printk_debug(
				"%s:[%s] iftype(%d),[%s] iftype (%d)\n",
				__func__, sdata->name, iftype, nsdata->name,
				nsdata->vif.type);

#ifndef CONFIG_SSTAR_SUPPORT_MULTIAP
			/*
			...
			if ((iftype == NL80211_IFTYPE_MONITOR) &&
				...
				*/
				if (nsdata->vif.type ==
					...
					Sstar_printk_err(
						"only support one monitor mode[%s],[%s]\n",
						sdata->name, nsdata->name);
					return -EBUSY;
				}
				/*
				...
				*/
				if (nsdata->vif.type ==
				    ...
				    nsdata->u.mgd.associated) {
					Sstar_printk_err(
						"[%s] is associated ,[%s] can not run into monior\n",
						nsdata->name, sdata->name);
					return -EBUSY;
				}
				if (nsdata->vif.type == NL80211_IFTYPE_AP &&
				    nsdata->u.ap.beacon) {
					Sstar_printk_err(
						"[%s] is ap mode ,[%s] can not run into monior\n",
						nsdata->name, sdata->name);
					return -EBUSY;
				}
			}
			...
			*/
			if ((nsdata->vif.type == NL80211_IFTYPE_MONITOR) &&
			    (!(nsdata->u.mntr_flags &
			       MONITOR_FLAG_COOK_FRAMES))) {
				Sstar_printk_err(
					"[%s] in monitor mode,so [%s] can not up\n",
					nsdata->name, sdata->name);
				return -EBUSY;
			}
			 ...
			 */
			if (Sstar_compare_ether_addr(dev->dev_addr,
						     ndev->dev_addr))
				continue;
			      ...
			      bool going_down)
{
	...
	else if (local->pending_scan_sdata &&
	...
	Sstar_del_timer_sync(&sdata->dynamic_ps_timer);
	Sstar_cancel_work_sync(&sdata->dynamic_ps_enable_work);
#endif
#ifdef CONFIG_SSTAR_RADAR_DETECT
	...
	if (sdata->vif.type == NL80211_IFTYPE_AP) {
		...
		struct beacon_extra *old_extra =
			rtnl_dereference(sdata->u.ap.beacon_extra);
		struct probe_response_extra *old_proberesp_extra =
			rtnl_dereference(sdata->u.ap.probe_response_extra);
		...
		synchronize_rcu();
		Sstar_kfree(old_beacon);
		if (old_extra)
			Sstar_kfree(old_extra);
#ifdef SSTAR_PROBE_RESP_EXTRA_IE
		if (old_proberesp)
			Sstar_kfree(old_proberesp);
#endif
		if (old_proberesp_extra)
			Sstar_kfree(old_proberesp_extra);
		...
		Sstar_skb_queue_purge(&sdata->u.ap.ps_bc_buf);
	}
	if (sdata->vif.type == NL80211_IFTYPE_STATION) {
		struct probe_request_extra *extra =
			rtnl_dereference(sdata->u.mgd.probe_request_extra);
	...
	case NL80211_IFTYPE_MONITOR:
		...
		rcu_assign_pointer(
			sdata->local->internal_monitor.req.monitor_rx, NULL);
		rcu_assign_pointer(sdata->local->internal_monitor.req.priv,
				   NULL);
		synchronize_rcu();
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 60))
...
#ifdef CONFIG_SSTAR_SUPPORT_MULTI_CHANNEL
	...
	for (i = 0; i < IEEE80211_MAX_QUEUES; i++) {
		Sstar_skb_queue_walk_safe(&local->pending[i], skb, tmp)
		{
			struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
			if (info->control.vif == &sdata->vif) {
				__Sstar_skb_unlink(skb, &local->pending[i]);
...
#else
	__dev_addr_sync(&sdata->mc_list, &sdata->mc_count, &dev->mc_list,
			&dev->mc_count);
#endif
	spin_unlock_bh(&local->filter_lock);
...
static void ieee80211_teardown_sdata(struct net_device *dev)
{
	...
	for (i = 0; i < IEEE80211_FRAGMENT_MAX; i++)
		__Sstar_skb_queue_purge(&sdata->fragments[i].skb_list);
...
static void ieee80211_sdata_uninit(struct net_device *dev)
{
	...
	struct ieee80211_local *local = sdata->local;
	mutex_lock(&local->mtx);
	Sstar_printk_err("%s:listen_sdata(%p)\n", __func__,
			 local->listen_sdata);
	if (local->listen_sdata == sdata)
...
/*
*CONFIG_SSTAR_IOCTRL : use net ioctrl to process some cmd
*/
#if defined(CONFIG_SSTAR_IOCTRL)
...
#define IEEE80211_CMD_GET(cmdname, index)                                      \
	(cmdname##_buff[index] == NULL ? cmdname##_default :                   \
					       cmdname##_buff[index])

#ifdef CONFIG_SSTAR_IOCTRL_VENDOR_CMD
...
static const char *android_wifi_cmd_str[ANDROID_WIFI_CMD_MAX] = {
	...
	"RXFILTER-START", "RXFILTER-STOP", "RXFILTER-ADD", "RXFILTER-REMOVE",
	"BTCOEXSCAN-START", "BTCOEXSCAN-STOP", "BTCOEXMODE", "SETSUSPENDOPT",
	"P2P_DEV_ADDR", "SETFWPATH", "SETBAND", "GETBAND", "COUNTRY",
	"P2P_SET_NOA", "P2P_GET_NOA", "P2P_SET_PS", "SET_AP_WPS_P2P_IE",
#ifdef CONFIG_PNO_SUPPORT
	"PNOSSIDCLR", "PNOSETUP", "PNOFORCE", "PNODEBUG",
#endif

	"MACADDR",

	"BLOCK", "WFD-ENABLE", "WFD-DISABLE", "WFD-SET-TCPPORT",
	"WFD-SET-MAXTPUT", "WFD-SET-DEVTYPE", "SET_DTIM",
	"HOSTAPD_SET_MACADDR_ACL", "HOSTAPD_ACL_ADD_STA",
...
typedef int (*android_cmd_handle)(struct ieee80211_sub_if_data *sdata,
...
{
	int ret = 0;
	Sstar_printk_debug("%s:android cmd-->(%s) unhandle\n", __func__, cmd);
...
*/
int ieee80211_netdev_process_android_cmd(struct net_device *dev,
					 struct ifreq *rq)
{
#if 0
	...
	android_cmd = Sstar_kmalloc(android_cmd_struct.total_len,GFP_KERNEL);

	if(android_cmd == NULL)
	{
		printk(KERN_ERR "Sstar_kmalloc err\n");
		ret = -EFAULT;
		goto exit;
	}
...
#ifdef CONFIG_SSTAR_DRIVER_RATE_CTRL
extern int rate_altm_control_test(struct wiphy *wiphy, void *data, int len);
#endif

/*
*vendor cmd ----> Sstar_tool
*/
int ieee80211_netdev_process_vendor_cmd(struct net_device *dev,
					struct ifreq *rq)
{
	...
	if (!rq->ifr_data) {
		Sstar_printk_err("%s:ifr->ifr_data == NULL\n", __func__);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (copy_from_user(&vendor_msg, rq->ifr_data,
			   sizeof(struct altm_msg))) {
		Sstar_printk_err("copy_from_user err\n");
		ret = -EFAULT;
		goto exit;
	}
	len = sizeof(struct altm_msg);
	Sstar_printk_debug("type = %d, value = %d\n", vendor_msg.type,
			   vendor_msg.value);
	for (i = 0; i < 30; i++)
		Sstar_printk_debug("%d ", vendor_msg.externData[i]);
	...
	ret |= Sstar_altmtest_cmd(&sdata->local->hw, &vendor_msg, len);
#endif
	if ((ret > 0) && (ret <= len)) {
		if (copy_to_user(rq->ifr_data,
				 (u8 *)(&sdata->local->hw.vendreturn),
				 sizeof(struct response))) {
			Sstar_printk_err("copy_to_user err\n");
			ret = -EFAULT;
		} else
			ret = 0;
		goto exit;
...
ioctl_list_t list_data[] = {
	{ 0x8B00, "SIOCSIWNAME" },   { 0x8B01, "SIOCGIWNAME" },
	{ 0x8B02, "SIOCSIWNWID" },   { 0x8B03, "SIOCGIWNWID" },
	{ 0x8B04, "SIOCSIWFREQ" },   { 0x8B05, "SIOCGIWFREQ" },
	{ 0x8B06, "SIOCSIWMODE" },   { 0x8B07, "SIOCGIWMODE" },
	{ 0x8B08, "SIOCSIWSENS" },   { 0x8B09, "SIOCGIWSENS" },
	{ 0x8B0A, "SIOCSIWRANGE" },  { 0x8B0B, "SIOCGIWRANGE" },
	{ 0x8B0C, "SIOCSIWPRIV" },   { 0x8B0D, "SIOCGIWPRIV" },
	{ 0x8B10, "SIOCSIWSPY" },    { 0x8B11, "SIOCGIWSPY" },
	{ 0x8B14, "SIOCSIWAP" },     { 0x8B15, "SIOCGIWAP" },
	{ 0x8B17, "SIOCGIWAPLIST" }, { 0x8B1A, "SIOCSIWESSID" },
	{ 0x8B1B, "SIOCGIWESSID" },  { 0x8B1C, "SIOCSIWNICKN" },
	{ 0x8B1D, "SIOCGIWNICKN" },  { 0x8B20, "SIOCSIWRATE" },
	{ 0x8B21, "SIOCGIWRATE" },   { 0x8B22, "SIOCSIWRTS" },
	{ 0x8B23, "SIOCGIWRTS" },    { 0x8B24, "SIOCSIWFRAG" },
	{ 0x8B25, "SIOCGIWFRAG" },   { 0x8B26, "SIOCSIWTXPOW" },
	{ 0x8B27, "SIOCGIWTXPOW" },  { 0x8B2A, "SIOCSIWENCODE" },
	{ 0x8B2B, "SIOCGIWENCODE" }, { 0x8B2C, "SIOCSIWPOWER" },
	{ 0x8B2D, "SIOCGIWPOWER" },  { -1, NULL }
};
...
#if defined(CONFIG_NL80211_TESTMODE) && defined(CONFIG_SSTAR_TEST_TOOL)
extern int Sstar_altmtest_cmd(struct ieee80211_hw *hw, void *data, int len);
#endif
#ifdef CONFIG_SSTAR_DEV_IOCTL
extern int Sstar_wext_cmd(struct net_device *dev, void *data, int len);
#endif
/*
...
int ieee80211_netdev_process_wext_cmd(struct net_device *dev, struct ifreq *rq)
{
	...
	if (dev == NULL) {
		Sstar_printk_err("Sstar_wext_cmd() dev NULL\n");
		return -1;
	}

	sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	if (sdata == NULL) {
		Sstar_printk_err("error, netdev sdata NULL\n");
		return -1;
	}

	local = sdata->local;
	if (local == NULL) {
		Sstar_printk_err("error, netdev local NULL\n");
		return -1;
	}

	hw = &local->hw;
	if (hw == NULL) {
		Sstar_printk_err("error, netdev hw NULL\n");
		return -1;
	}
	...
	if (priv == NULL) {
		Sstar_printk_err("error, netdev priv NULL\n");
		ret = -EINVAL;
		goto exit;
	}

	if (!rq->ifr_data) {
		Sstar_printk_err("error, netdev ifr->ifr_data == NULL\n");
		ret = -EINVAL;
		goto exit;
	}
	...
	if (copy_from_user(&vendor_msg, rq->ifr_data,
			   sizeof(struct altm_msg))) {
		Sstar_printk_err("error, netdev copy_from_user\n");
		ret = -EFAULT;
		goto exit;
	}

	Sstar_printk_debug("1,type = %d, value = %d\n", vendor_msg.type,
			   vendor_msg.value);

	sdata->local->hw.vendcmd_nl80211 = 1;

	ret = Sstar_wext_cmd(dev, &vendor_msg, sizeof(struct altm_wext_msg));
	Sstar_printk_debug("2,type = %d, value = %d\n", vendor_msg.type,
			   vendor_msg.value);
	if (ret != 0) {
		Sstar_printk_err("error, netdev Sstar_wext_cmd\n");
		ret = -EFAULT;
		goto exit;
	}

	if (copy_to_user(rq->ifr_data, (u8 *)(&vendor_msg),
			 sizeof(vendor_msg))) {
		Sstar_printk_err("error, netdev copy_to_user err\n");
		ret = -EFAULT;
		goto exit;
	}

	Sstar_printk_debug("3,type = %d, value = %d\n", vendor_msg.type,
			   vendor_msg.value);
...
typedef int (*ieee80211_netdev_cmd_handle)(struct net_device *dev,
					   struct ifreq *rq);
...
int ieee80211_netdev_ioctrl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	call_func = IEEE80211_CMD_GET(ieee80211_netdev_cmd_handle,
				      cmd - IEEE80211_NETDEV_BASE_CMD);
...
#endif
#ifdef CONFIG_SSTAR_SUPPORT_NET_TIMEOUT
static void ieee80211_tx_timeout(struct net_device *dev)
{
	...
	for (i = 0; i < local->hw.queues; i++)
		Sstar_printk_debug("%s:queue[%d],reason[%ld]\n", __func__, i,
...
static void ieee80211_iface_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata =
	...
	while ((skb = Sstar_skb_dequeue(&sdata->skb_queue))) {
		...
		if (0) {
			...
			u8 i;
			unsigned long flags;
			Sstar_printk_debug("%s:flush queue\n", __func__);
			...
			*/
			...
			case NL80211_IFTYPE_P2P_GO:
				...
				if (ieee80211_is_beacon(mgmt->frame_control)) {
					...
					ieee80211_bss_info_change_notify(
						sdata, bss_info_changed);
				} else
#endif
					ieee80211_ap_rx_queued_mgmt_special(
						sdata, skb);
				break;
			case NL80211_IFTYPE_STATION:
...
static void ieee80211_reconfig_filter(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata = container_of(
		work, struct ieee80211_sub_if_data, reconfig_filter);
				  ...
				  enum nl80211_iftype type)
{
	...
	atomic_set(&sdata->connectting, IEEE80211_SSTAR_CONNECT_DONE);
	...
	Sstar_skb_queue_head_init(&sdata->skb_queue);
	SSTAR_INIT_WORK(&sdata->work, ieee80211_iface_work);
	SSTAR_INIT_WORK(&sdata->reconfig_filter, ieee80211_reconfig_filter);
#ifdef SSTAR_AP_SME
	...
	sdata->vif.bss_conf.rts_threshold = sdata->wdev.wiphy->rts_threshold =
		(u32)-1;
...
#endif
	case NL80211_IFTYPE_AP:
		Sstar_skb_queue_head_init(&sdata->u.ap.ps_bc_buf);
	...
	case NL80211_IFTYPE_MONITOR: {
		...
		list_for_each_entry (attach_sdata, &sdata->local->interfaces,
				     list) {
			if (attach_sdata == sdata)
				continue;
			if (strstr(sdata->name, attach_sdata->name) == NULL)
				continue;
			Sstar_printk_debug("%s:add[%s],attach[%s]\n", __func__,
					   sdata->name, attach_sdata->name);
			memcpy(sdata->vif.hw_queue, attach_sdata->vif.hw_queue,
			       IEEE80211_NUM_ACS);
		}
#endif
		/*
		...
		if (sdata->local->hw.flags &
		    IEEE80211_HW_MONITOR_NEED_PRISM_HEADER) {
...
static void ieee80211_delete_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	case NL80211_IFTYPE_P2P_CLIENT:
	case NL80211_IFTYPE_STATION: {
#if defined(CONFIG_SSTAR_SUPPORT_CHANSWITCH) ||                                \
	defined(CONFIG_SSTAR_MAC80211_NO_USE)
		struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
		Sstar_del_timer_sync(&ifmgd->bcn_mon_timer);
		Sstar_del_timer_sync(&ifmgd->conn_mon_timer);
		Sstar_del_timer_sync(&ifmgd->timer);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
		Sstar_del_timer_sync(&ifmgd->chswitch_timer);
#endif
	} break;
	default:
		break;
			     ...
			     enum nl80211_iftype type)
{
	...
	Sstar_printk_err("[%s] change to [%d]\n", sdata->name, type);
	...
	if (sdata->local->only_monitors) {
		...
		list_for_each_entry (other_sdata, &sdata->local->interfaces,
			...
			if ((other_sdata->vif.type != NL80211_IFTYPE_MONITOR) &&
				...
				Sstar_printk_err(
					"and try again later to set[%s] to monitor mode\n",
					sdata->name);
				return -EOPNOTSUPP;
			}
		}
	...
	sdata->vif.bss_conf.basic_rates = ieee80211_Sstar_mandatory_rates(
		sdata->local, chan_state->conf.channel->band);
				       ...
				       enum nl80211_iftype type)
{
	...
	default:
				...
				if (memcmp(local->hw.wiphy->addresses[i].addr,
					...
					if ((sdata->vif.type ==
					    ...
					    (sdata->u.mntr_flags &
					     MONITOR_FLAG_COOK_FRAMES)) {
						Sstar_printk_err(
							"[%s] in cook monitor mode,we can use that addr\n",
							sdata->name);
						continue;
					}
					used = true;
		...
		if (__ffs64(mask) + hweight64(mask) != fls64(mask)) {
			...
			Sstar_printk_err("not contiguous\n");
			break;
		}
...
//#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,18,25))
	...
	ndev = alloc_netdev_mqs(sizeof(*sdata) + local->hw.vif_data_size, name,
				NET_NAME_UNKNOWN, ieee80211_if_setup, txqs, 1);
#else
	ndev = alloc_netdev_mqs(sizeof(*sdata) + local->hw.vif_data_size, name,
...
/* This is an optimization, just ignore for older kernels */
	...
	for (i = 0; i < IEEE80211_FRAGMENT_MAX; i++)
		Sstar_skb_queue_head_init(&sdata->fragments[i].skb_list);
	...
	for (i = 0; i < IEEE80211_NUM_BANDS; i++) {
		...
		if (!sdata->chan_state.oper_channel) {
			...
			sdata->chan_state.conf.channel =
				...
				sdata->chan_state.oper_channel =
					&sband->channels[2];
			sdata->chan_state.conf.channel_type =
				NL80211_CHAN_NO_HT;
		}
	}

#ifdef CONFIG_SSTAR_AP_CHANNEL_CHANGE_EVENT
	SSTAR_INIT_DELAYED_WORK(&sdata->ap_channel_event_work,
				ieee80211_ap_channel_event_work);
#endif

	sdata->dynamic_ps_forced_timeout = -1;
#ifdef CONFIG_SSTAR_RADAR_DETECT
#ifdef CONFIG_SSTAR_5G_PRETEND_2G
	SSTAR_INIT_DELAYED_WORK(&sdata->dfs_cac_timer_work,
				ieee80211_dfs_cac_timer_work);
#endif
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	SSTAR_INIT_WORK(&sdata->dynamic_ps_enable_work,
			ieee80211_dynamic_ps_enable_work);
	SSTAR_INIT_WORK(&sdata->dynamic_ps_disable_work,
			ieee80211_dynamic_ps_disable_work);
	Sstar_setup_timer(&sdata->dynamic_ps_timer, ieee80211_dynamic_ps_timer,
			  (unsigned long)sdata);
#endif
	/*
	*medium traffic init
...
#ifdef CONFIG_WIRELESS_EXT
	ndev->wireless_handlers = &Sstar_handlers_def;
#endif
#ifdef CONFIG_SSTAR_SUPPORT_NET_TIMEOUT
...
void ieee80211_if_remove(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_cancel_work_sync(&sdata->reconfig_filter);
	Sstar_cancel_work_sync(&sdata->work);
...
void ieee80211_remove_interfaces(struct ieee80211_local *local)
{
	...
	list_for_each_entry_safe (sdata, tmp, &wdev_list, list) {
       ...
       LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 65))
void ieee80211_remove_interfaces(struct ieee80211_local *local)
{
	struct ieee80211_sub_if_data *sdata, *tmp;
	LIST_HEAD(unreg_list);
...
#endif
#define IEEE80211_SSTAR_IDLE_OFF_AUTHENING BIT(0)
#define IEEE80211_SSTAR_IDLE_OFF_REMAIN_ON_CHANNEL BIT(1)
#define IEEE80211_SSTAR_IDLE_OFF_WORKING BIT(2)
#define IEEE80211_SSTAR_IDLE_OFF_SCANNING BIT(3)
#define IEEE80211_SSTAR_IDLE_OFF_OTHERS BIT(4)

static u32 ieee80211_idle_off(struct ieee80211_local *local, int reason)
{
					  ...
					  bool start, bool flush)
{
	...
	if (!local->listen_sdata || !local->listen_channel) {
		Sstar_printk_err(
			"%s,not found listen channel or listen sdata\n",
			__func__);
		return;
	}
	...
	if (start == true) {
		if ((local->listen_started == false) && running) {
			...
			if (local->ops->sta_triger_listen)
				local->ops->sta_triger_listen(
					&local->hw, &local->listen_sdata->vif,
...
int ieee80211_set_sta_channel(struct ieee80211_sub_if_data *sdata, int channel)
{
	...
	if (sdata->vif.type != NL80211_IFTYPE_STATION) {
		Sstar_printk_err("%s,interface not station! \n", __func__);
		return -EINVAL;
	}
	if (channel != 0) {
		if (channel < 14) {
		...
		if (chan == NULL) {
			Sstar_printk_err("%s,chan invaid! \n", __func__);
			return -EINVAL;
		}
...
u32 __ieee80211_recalc_idle(struct ieee80211_local *local)
{
	...
	list_for_each_entry (sdata, &local->interfaces, list) {
		...
		if (sdata->vif.type == NL80211_IFTYPE_STATION &&
		    rcu_dereference_protected(sdata->u.mgd.authen_bss,
...
#ifdef CONFIG_SSTAR_STA_LISTEN
	if (local->listen_channel && local->listen_sdata &&
	    ieee80211_sdata_running(local->listen_sdata)) {
		/*
		*idle ,start listen mode
		*/
		if ((working == true) || (scanning == true) ||
		    (hw_roc == true) || (authening == true) ||
	...
	if (authening) {
		return ieee80211_idle_off(local,
					  IEEE80211_SSTAR_IDLE_OFF_AUTHENING);
	}
	if (hw_roc)
		return ieee80211_idle_off(
			local, IEEE80211_SSTAR_IDLE_OFF_REMAIN_ON_CHANNEL);
	if (working)
		return ieee80211_idle_off(local,
					  IEEE80211_SSTAR_IDLE_OFF_WORKING);
	if (scanning)
		return ieee80211_idle_off(local,
					  IEEE80211_SSTAR_IDLE_OFF_SCANNING);
	if (!count)
		return ieee80211_idle_on(local);
	else
		return ieee80211_idle_off(local,
					  IEEE80211_SSTAR_IDLE_OFF_OTHERS);
...
static int netdev_notify(struct notifier_block *nb, unsigned long state,
			 void *ndev)
{
