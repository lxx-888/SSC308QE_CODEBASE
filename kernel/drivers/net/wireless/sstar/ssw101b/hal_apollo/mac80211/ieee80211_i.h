...
enum SSTAR_TEST_TYPE {
	TXRX_TEST_REQ,
	TXRX_TEST_RSP,
	TXRX_TEST_RESULT,
};

enum SSTAR_TEST_RESULT {
	TXRX_TEST_NONE,
	TXRX_TEST_PASS,
	TXRX_TEST_FAIL,
};

struct SSTAR_TEST_IE {
	u8 ie_id; //D11_WIFI_ELT_ID
	u8 len;
	u8 oui[3]; //SSTAR_OUI
	u8 oui_type; //WIFI_SSTAR_IE_OUI_TYPE
	u8 test_type;
	u8 resverd;
...
#define WIFI_SSTAR_IE_OUI_TYPE 0x0a
...
struct Sstar_wpa_eapol_key {
	u8 type;
	/* Note: key_info, key_length, and key_data_length are unaligned */
	u8 key_info[2]; /* big endian */
	u8 key_length[2]; /* big endian */
	u8 replay_counter[SSTAR_WPA_REPLAY_COUNTER_LEN];
	u8 key_nonce[SSTAR_WPA_NONCE_LEN];
	u8 key_iv[16];
	u8 key_rsc[SSTAR_WPA_KEY_RSC_LEN];
...
struct ieee80211_roc_work {
	...
	struct Sstar_delayed_work work;
...
struct ieee80211_if_managed {
	...
	struct Sstar_timer_list conn_mon_timer;
	struct Sstar_timer_list bcn_mon_timer;
	struct Sstar_work_struct monitor_work;
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
	struct Sstar_timer_list chswitch_timer;
	//	struct timer_list scan_delay_timer;
	struct Sstar_work_struct chswitch_work;
#endif
	struct Sstar_work_struct beacon_connection_loss_work;
	...
	struct Sstar_work_struct request_smps_work;
#endif
	unsigned int flags;
...
struct ieee80211_if_mesh {
	struct Sstar_timer_list housekeeping_timer;
	struct Sstar_timer_list mesh_path_timer;
	struct Sstar_timer_list mesh_path_root_timer;
...
#define IEEE80211_SSTAR_CONNECT_DONE (0)
#define IEEE80211_SSTAR_CONNECT_SET (1)
#define IEEE80211_SSTAR_CONNECT_RUN (2)
...
struct ieee80211_medium_traffic {
	struct Sstar_timer_list traffic_timer;
...
struct ieee80211_sub_if_data {
	...
	bool ps_allowed;
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	struct Sstar_work_struct dynamic_ps_enable_work;
	struct Sstar_work_struct dynamic_ps_disable_work;
	struct Sstar_timer_list dynamic_ps_timer;
#endif
	...
	struct ieee80211_key __rcu
		*keys[NUM_DEFAULT_KEYS + NUM_DEFAULT_MGMT_KEYS];
	...
	struct Sstar_work_struct reconfig_filter;
	...
	struct Sstar_work_struct ap_sme_event_work;
	struct sk_buff_head ap_sme_skb_queue;
	struct Sstar_work_struct ap_sme_work;
	...
	bool radar_required;
	struct Sstar_delayed_work dfs_cac_timer_work;
	...
	struct Sstar_delayed_work ap_channel_event_work;
#endif
	...
	struct Sstar_work_struct special_filter_work;
...
#define IEEE80211_INTERNAL_SCAN_HASHENTRIES                                    \
	(1 << IEEE80211_INTERNAL_SCAN_HASHBITS)
...
struct Sstar_internal_scan_results_req {
	bool(__rcu *result_handle)(struct ieee80211_hw *hw,
...
struct ieee80211_internal_scan_request {
	bool(__rcu *result_handle)(
		struct ieee80211_sub_if_data *sdata, void *priv,
...
struct ieee80211_internal_channel_auto_select_results {
	unsigned int n_aps[IEEE80211_SSTAR_MAX_SCAN_CHANNEL_INDEX];
	unsigned int weight[IEEE80211_SSTAR_MAX_SCAN_CHANNEL_INDEX];
	u8 busy_ratio[IEEE80211_SSTAR_MAX_SCAN_CHANNEL_INDEX];
	u8 susgest_channel;
	u8 *ignore_channels;
...
struct ieee80211_internal_monitor_req {
	...
	void *priv;
	bool(__rcu *monitor_rx)(struct ieee80211_sub_if_data *sdata, void *priv,
				struct sk_buff *skb);
};
...
struct ieee80211_internal_mac_monitor {
	...
	u8 index;
	struct ieee80211_internal_mac_monitor_results
		reults[IEEE80211_INTERNAL_MAX_STA_MONITOR_SUPPORT];
};
...
struct ieee80211_local {
	...
	struct Sstar_timer_list work_timer;
	struct Sstar_work_struct work_work;
	 ...
	 */
	struct Sstar_workqueue_struct *workqueue;
	...
	struct Sstar_work_struct recalc_smps;
...
#define IEEE80211_IRQSAFE_QUEUE_LIMIT 128
	...
	struct Sstar_timer_list resume_timer;
	struct Sstar_timer_list sta_cleanup;
	struct Sstar_work_struct sta_finish_work;
	int sta_generation;
	...
	struct hlist_head internal_scan_list[SSTAR_COMMON_HASHENTRIES];
	...
	struct Sstar_work_struct sched_scan_stopped_work;
#endif
	char country_code[2];
	unsigned long leave_oper_channel_time;
	enum mac80211_scan_state next_scan_state;
	struct Sstar_delayed_work scan_work;
...
#else /* CONFIG_MAC80211_SSTAR_DEBUG_COUNTERS */
#define I802_DEBUG_INC(c)                                                      \
	do {                                                                   \
	} while (0)
#endif /* CONFIG_MAC80211_SSTAR_DEBUG_COUNTERS */
...
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	struct Sstar_work_struct restart_work;
#endif

#ifdef IPV6_FILTERING
#ifdef CONFIG_INET
	struct Sstar_work_struct ifa6_changed_work;
	...
	struct Sstar_work_struct hw_roc_start, hw_roc_done;
...
#endif
#ifdef SSTAR_CONNECT_WITHOUT_SCAN
	atomic_t connectting;
	...
	*/
	struct Sstar_work_struct special_work;
	struct sk_buff_head special_req_list;
	/*
	*special channel params
	*/
	struct hlist_head special_freq_list[SSTAR_COMMON_HASHENTRIES];
...
{
	return Sstar_compare_ether_addr(raddr, addr) == 0 ||
	       is_broadcast_ether_addr(raddr);
}

//#define SSTAR_CONNECT_WITHOUT_SCAN
#ifdef SSTAR_CONNECT_WITHOUT_SCAN
...
#define LOCAL_IS_CONNECTTING(local)                                            \
	((atomic_read(&(local)->connectting) &&                                \
	  time_is_after_jiffies((local)->con_start_time + 10 * HZ)) ?          \
		       1 :                                                           \
		       (atomic_xchg(&(local)->connectting, 0), 0))
#else
...
bool ieee80211_update_ap_config(struct ieee80211_sub_if_data *sdata,
				struct ieee80211_internal_ap_conf *conf,
				bool clear);
#endif
...
void ieee80211_sw_roc_work(struct Sstar_work_struct *work);
...
void ieee80211_adjust_monitor_flags(struct ieee80211_sub_if_data *sdata,
				    const int offset);
static inline u8 *ieee8011_get_channel_list(struct ieee80211_hw *hw,
					    size_t *n_channels)
{
	...
	*n_channels = i;
	...
	channels = Sstar_kzalloc(i, GFP_ATOMIC);
...
static inline struct ieee80211_channel_state *
ieee80211_get_channel_state(struct ieee80211_local *local,
			    struct ieee80211_sub_if_data *sdata)
{
...
static inline bool Sstar_ieee80211_suspend(struct ieee80211_local *local)
{
	if ((local->quiescing == true) ||
	    (local->suspended && !local->resuming) ||
	    (atomic_read(&local->resume_timer_start))) {
		pr_warn("Sstar_ieee80211_suspend can not process cfg cmd\n");
		return true;
	}
	return false;
}
...
void mac80211_ev_michael_mic_failure(struct ieee80211_sub_if_data *sdata,
				     int keyidx, struct ieee80211_hdr *hdr,
...
void ieee80211_send_auth(struct ieee80211_sub_if_data *sdata, u16 transaction,
			 ...
			 const u8 *bssid, const u8 *key, u8 key_len,
			 u8 key_idx);
...
void ieee80211_start_connecting_work(struct ieee80211_sub_if_data *sdata,
				     struct ieee80211_channel *chan, u8 *bssid);
void ieee80211_cancle_connecting_work(struct ieee80211_sub_if_data *sdata,
				      u8 *bssid, bool delayed);
...
int ieee80211_wk_cancel_remain_on_channel(struct ieee80211_sub_if_data *sdata,
					  u64 cookie);
void ieee80211_assign_authen_bss(struct ieee80211_sub_if_data *sdata,
...
bool ieee80211_updata_extra_ie(struct ieee80211_sub_if_data *sdata,
			       enum ieee80211_special_work_type type,
			       union iee80211_extra_ie *extra);
bool ieee80211_special_freq_update(struct ieee80211_local *local,
				   struct ieee80211_special_freq *special);
void ieee80211_special_freq_clear(struct ieee80211_local *local,
				  struct ieee80211_special_freq *special);
#ifdef CONFIG_SSTAR_STA_LISTEN
...
#ifdef CONFIG_MAC80211_SSTAR_NOINLINE
...
u16 ieee80211_ap_sme_free_aid(struct ieee80211_sub_if_data *sdata,
			      struct sta_info *sta);
...
#else
static inline void
ieee80211_special_filter_init(struct ieee80211_sub_if_data *sdata)
{
}
static inline void
ieee80211_special_filter_exit(struct ieee80211_sub_if_data *sdata)
{
}
#endif
