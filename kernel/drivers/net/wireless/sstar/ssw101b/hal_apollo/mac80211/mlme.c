...
unsigned int Sstar_wifi_status_get(void)
{
	return wifi_status;
}
static void Sstar_wifi_status_set(unsigned int state)
{
	wifi_status = state;
	return;
}
...
#endif
#if defined(CONFIG_SSTAR_MAC80211_NO_USE) || defined(CONFIG_PM)
...
static void run_again(struct ieee80211_if_managed *ifmgd, unsigned long timeout)
{
	...
	if (!Sstar_timer_pending(&ifmgd->timer) ||
	    time_before(timeout, ifmgd->timer.expires))
		Sstar_mod_timer(&ifmgd->timer, timeout);
}
			     ...
			     struct ieee80211_sub_if_data *sdata, int powersave)
{
	...
	if (ifmgd->flags &
	    (IEEE80211_STA_BEACON_POLL | IEEE80211_STA_CONNECTION_POLL))
...
static void ieee80211_mgd_probe_ap_send(struct ieee80211_sub_if_data *sdata)
{
	...
	if (sdata->local->hw.flags & IEEE80211_HW_REPORTS_TX_ACK_STATUS) {
		...
		ssid = ieee80211_bss_get_ie(ifmgd->associated,
					    SSTAR_WLAN_EID_SSID);
				   ...
				   bool beacon)
{
	...
	if (beacon && net_ratelimit())
		Sstar_printk_debug("%s: detected beacon loss from AP "
...
static void ieee80211_sta_monitor_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata = container_of(
		work, struct ieee80211_sub_if_data, u.mgd.monitor_work);
...
static u32 ieee80211_enable_ht(struct ieee80211_sub_if_data *sdata,
			       struct ieee80211_ht_info *hti, const u8 *bssid,
			       u16 ap_ht_cap_flags, bool beacon_htcap_ie)
{
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_channel_state *chan_state =
	...
	if (enable_ht) {
		...
		if (channel_center_freq(chan_state->conf.channel) !=
		    hti_cfreq) {
			...
			Sstar_printk_err(
				...
				hti_cfreq, hti->control_chan, sband->band);
			enable_ht = false;
		}
	}
					   ...
					   bool send_frame)
{
	...
	if (stype == IEEE80211_STYPE_DEAUTH)
		if (cookie)
			__cfg80211_send_deauth(sdata->dev, (u8 *)mgmt,
					       skb->len);
		else
	...
	*/
	if (cookie)
		...
		Sstar_wdev_lock(wdev);
		cfg80211_tx_mlme_mgmt(sdata->dev, (u8 *)mgmt, skb->len);
		Sstar_wdev_unlock(wdev);
	}
#endif
					  ...
					  struct ieee80211_sub_if_data *sdata)
{
	...
	skb = Sstar_dev_alloc_skb(local->hw.extra_tx_headroom + 30);
	if (!skb)
		return;

	Sstar_skb_reserve(skb, local->hw.extra_tx_headroom);

	nullfunc = (struct ieee80211_hdr *)Sstar_skb_put(skb, 30);
...
static void ieee80211_chswitch_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata = container_of(
		work, struct ieee80211_sub_if_data, u.mgd.chswitch_work);
	struct ieee80211_channel_state *chan_state =
		ieee80211_get_channel_state(sdata->local, sdata);
	struct ieee80211_if_managed *ifmgd = &sdata->u.mgd;
	...
	if (!sdata->local->ops->channel_switch) {
		...
		*/
		if (prev_channel_type !=
		    sdata->local->chan_state.conf.channel_type) {
			struct sta_info *sta;
			struct ieee80211_supported_band *sband;
			sband = sdata->local->hw.wiphy
			...
			if (sta)
				rate_control_rate_update(
					sdata->local, sband, sta,
					IEEE80211_RC_HT_CHANGED,
					sdata->local->chan_state.conf
						.channel_type);
#endif
			rcu_read_unlock();
		}
...
void ieee80211_sta_process_chanswitch(
	struct ieee80211_sub_if_data *sdata,
	struct Sstar_ieee80211_channel_sw_packed_ie *sw_packed_ie,
	struct ieee80211_bss *bss, u64 timestamp)
{
	...
	struct Sstar_ieee80211_ext_chansw_ie *ex_sw_elem =
		sw_packed_ie->ex_chan_sw_ie;
	struct Sstar_ieee80211_sec_chan_offs_ie *sec_chan_ie =
		sw_packed_ie->sec_chan_offs_ie;
...
#define CHANGE_CHANNEL_TYPE_BIT (1 << 1)
	...
	if (count <= 1) {
		...
		ifmgd->flags |= IEEE80211_STA_CSA_RECEIVED;
		Sstar_mod_timer(
			&ifmgd->chswitch_timer,
			jiffies +
				msecs_to_jiffies(mode * cbss->beacon_interval));
	}
}
#else
				      ...
				      struct ieee80211_bss *bss, u64 timestamp)
{
	...
	if (sw_elem->count <= 1) {
		...
		Sstar_mod_timer(
			&ifmgd->chswitch_timer,
			jiffies + msecs_to_jiffies(sw_elem->count *
				...
				struct ieee80211_sub_if_data *sdata)
{
	...
	if (conf->dynamic_ps_timeout > 0 &&
		...
		Sstar_mod_timer(
			...
			jiffies + msecs_to_jiffies(conf->dynamic_ps_timeout));
	} else {
...
static bool ieee80211_powersave_allowed(struct ieee80211_sub_if_data *sdata)
{
	...
	if (sdata->traffic.current_tx_tp + sdata->traffic.current_rx_tp >=
	    IEEE80211_KEEP_WAKEUP_TP_PER_SECOND) {
		Sstar_printk_debug("%s:keep wakeup\n", __func__);
		return false;
	}
#endif
...
void ieee80211_recalc_ps_vif(struct ieee80211_local *local, s32 latency)
{
	...
	list_for_each_entry (sdata, &local->interfaces, list) {
		...
		beaconint_us =
			ieee80211_tu_to_usec(sdata->vif.bss_conf.beacon_int);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
#ifdef CONFIG_SSTAR_PM_QOS
		if (latency < 0)
...
#else
		timeout = (latency > 0 && latency < beaconint_us) ?
					latency :
					beaconint_us;
		...
		Sstar_printk_debug("%s:beaconint_us(%x)\n", __func__,
				   beaconint_us);
		{
			struct ieee80211_bss *bss;
			int maxslp = 1;
...
void ieee80211_dynamic_ps_disable_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata = container_of(
		work, struct ieee80211_sub_if_data, dynamic_ps_disable_work);
...
void ieee80211_dynamic_ps_enable_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata = container_of(
		work, struct ieee80211_sub_if_data, dynamic_ps_enable_work);
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_if_managed *ifmgd;
	unsigned long flags;
	int q;
					...
					u8 *p2p_ie, size_t p2p_ie_len)
{
	...
	while (left >= 2) {
		...
		if (noa_len >= (sizeof(struct noa_desc) + 2)) {
			...
			p2p_ps.start = __le32_to_cpu(noa_attr->dsc[0].start);
			p2p_ps.duration =
...
out:
	...
	if (memcmp(&conf->p2p_ps, &p2p_ps, sizeof(p2p_ps))) {
		...
		conf->p2p_ps = p2p_ps;
		Sstar_printk_debug("ieee80211_sta_p2p_noa_check\n");
				     ...
				     u8 *p2p_ie, size_t p2p_ie_len)
{
	...
	if (p2p_ie_len != ifmgd->p2p_last_ie_len ||
		...
		/* BSS_CHANGED_P2P_PS */
		ieee80211_sta_p2p_noa_check(local, sdata, p2p_ie, p2p_ie_len);
	}
				     ...
				     u8 *wmm_param, size_t wmm_param_len)
{
	...
	for (; left >= 4; left -= 4, pos += 4) {
		...
		wiphy_debug(local->hw.wiphy,
			    ...
			    params.cw_max, params.txop, params.uapsd);
#endif
...
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	Sstar_del_timer_sync(&sdata->u.mgd.conn_mon_timer);
	Sstar_del_timer_sync(&sdata->u.mgd.bcn_mon_timer);
	Sstar_del_timer_sync(&sdata->u.mgd.timer);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
	Sstar_del_timer_sync(&sdata->u.mgd.chswitch_timer);
#endif
}
					  ...
					  struct ieee80211_vif *vif)
{
	...
	ssid = ieee80211_bss_get_ie(ifmgd->associated, SSTAR_WLAN_EID_SSID);
	skb = ieee80211_build_probe_req(sdata, ifmgd->associated->bssid,
					(u32)-1, ssid + 2, ssid[1], NULL, 0,
					true);
...
static void __ieee80211_connection_loss(struct ieee80211_sub_if_data *sdata)
{
	...
	Sstar_printk_always("<WARNING>%s:AP %pM lost.....\n", sdata->name,
			    bssid);
...
void ieee80211_beacon_connection_loss_work(struct Sstar_work_struct *work)
{
	struct ieee80211_sub_if_data *sdata =
	...
	if (sdata->local->adaptive_started == true) {
		Sstar_printk_debug(
			"adaptive_started! ieee80211_beacon_connection_loss_work stop");
		return;
	}
			 ...
			 struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	Sstar_printk_always("%s: deauthenticated from %pM (Reason: %u)\n",
			   ...
			   struct Sstar_ieee80211_mgmt *mgmt, size_t len)
{
	...
	Sstar_printk_always("%s: disassociated from %pM (Reason: %u)\n",
				    ...
				    size_t len)
{
	...
	if (aid == 0 || aid > IEEE80211_MAX_AID) {
		Sstar_printk_err("%s: invalid AID value %d\n", sdata->name,
				 aid);
		aid = 0;
	...
	if (!elems.supp_rates) {
		Sstar_printk_err("%s: no SuppRates element in AssocResp\n",
				 sdata->name);
		//	return false;
	}
	...
	if (unlikely(!basic_rates) && min_rate_index >= 0) {
		Sstar_printk_err("%s: No basic rates in AssocResp. "
	...
	if (wk->chan->band == IEEE80211_BAND_2GHZ && have_higher_than_11mbit)
	...
	sta = NULL;
	if (err) {
		Sstar_printk_err("%s: failed to insert STA entry for"
				 " the AP (error %d)\n",
				 sdata->name, err);
		return false;
	}
	...
	*/
	while (sdata->vif.p2p) {
		if (!(elems.p2p_ie && elems.p2p_ie_len)) {
			Sstar_printk_err(
				"is p2p station,but p2p go do not send p2p ie\n");
			break;
		}
				  ...
				  bool beacon)
{
	...
	if (/*(sdata->u.mgd.associated) && */
	    (channel_hw_value(chan_state->conf.channel) !=
	     channel_hw_value(channel))) {
		Sstar_printk_err(
			"rx_bss_info channel change %d,deauth it [%pM][%pM][%pM][%x] seq=%d\n",
			channel_hw_value(channel), mgmt->da, mgmt->sa,
			mgmt->bssid, mgmt->frame_control, mgmt->seq_ctrl);
		if (elems->ssid && elems->ssid_len) {
			u8 *ssid =
				Sstar_kzalloc(elems->ssid_len + 1, GFP_KERNEL);

			if (ssid) {
				memcpy(ssid, elems->ssid, elems->ssid_len);
				ssid[elems->ssid_len] = 0;
				Sstar_printk_err("%s:channel change ssid(%s)\n",
						 __func__, ssid);
				Sstar_kfree(ssid);
			}
		}
	...
	if (elems->ch_switch_elem && (elems->ch_switch_elem_len == 3) &&
	    (memcmp(mgmt->bssid, sdata->u.mgd.associated->bssid, ETH_ALEN) ==
	     0)) {
		...
		sw_packed_ie.chan_sw_ie =
			(struct ieee80211_channel_sw_ie *)elems->ch_switch_elem;
		ieee80211_sta_process_chanswitch(sdata, &sw_packed_ie, bss,
						 rx_status->mactime);
#endif
	}
#endif
 ...
 */
static const u64 care_about_ies = (1ULL << SSTAR_WLAN_EID_COUNTRY) |
				  (1ULL << SSTAR_WLAN_EID_ERP_INFO) |
				  (1ULL << SSTAR_WLAN_EID_CHANNEL_SWITCH) |
				  (1ULL << SSTAR_WLAN_EID_PWR_CONSTRAINT) |
				  (1ULL << SSTAR_WLAN_EID_HT_CAPABILITY) |
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
				  (1ULL << WLAN_EID_HT_OPERATION);
#else
				  (1ULL << SSTAR_WLAN_EID_HT_INFORMATION);
#endif
				     ...
				     struct ieee80211_rx_status *rx_status)
{
...
#ifdef CONFIG_MAC80211_SSTAR_VERBOSE_DEBUG
		if (net_ratelimit()) {
			Sstar_printk_debug("%s: cancelling probereq poll due "
	...
	ncrc = Sstar_ieee802_11_parse_elems_crc(mgmt->u.beacon.variable,
						len - baselen, &elems,
						care_about_ies, ncrc);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	if (local->hw.flags & IEEE80211_HW_PS_NULLFUNC_STACK)
	...
	if (elems.ch_switch_elem) {
		...
		ieee80211_sta_process_chanswitch(
			sdata, &sw_packed_ie, (void *)ifmgd->associated->priv,
			rx_status->mactime);
	}
	...
	changed |= ieee80211_handle_bss_capability(
		sdata, le16_to_cpu(mgmt->u.beacon.capab_info), erp_valid,
		erp_value);
	...
	if (elems.ht_cap_elem && elems.ht_info_elem && elems.wmm_param &&
		...
		if (WARN_ON(!sta)) {
			Sstar_printk_err(
				"%s() %d## ifname[%s] sdata->vif.addr:%pM MAC:%pM \n",
				__func__, __LINE__, sdata->name,
				sdata->vif.addr, bssid);
			rcu_read_unlock();
			return;
		}
		...
		changed |= ieee80211_enable_ht(sdata, elems.ht_info_elem, bssid,
					       ap_ht_cap_flags, true);
	}
				  ...
				  struct sk_buff *skb)
{
	...
	if (ifmgd->associated &&
		...
		case IEEE80211_STYPE_ACTION: {
			...
			case SSTAR_WLAN_CATEGORY_SPECTRUM_MGMT:
				...
				if (WLAN_ACTION_SPCT_CHL_SWITCH ==
				    mgmt->u.action.u.chan_switch.action_code) {
					sw_packed_ie.chan_sw_ie =
					...
					if ((skb->len -
						...
						u8 *tmp_ie =
							((u8 *)sw_packed_ie
								 .chan_sw_ie) +
							3; //sizeof(struct ieee80211_channel_sw_ie)

						if ((tmp_ie[0] ==
						     SSTAR_WLAN_EID_SECONDARY_CH_OFFSET) &&
						    (tmp_ie[1] == 1)) {
							sw_packed_ie
								.sec_chan_offs_ie =
								(struct Sstar_ieee80211_sec_chan_offs_ie
			...
			case SSTAR_WLAN_CATEGORY_PUBLIC:
				...
				if (SSTAR_WLAN_PUB_ACTION_EX_CHL_SW_ANNOUNCE ==
				    mgmt->u.action.u.ext_chan_switch
					    .action_code) {
					sw_packed_ie.ex_chan_sw_ie =
						&mgmt->u.action.u
							 .ext_chan_switch
				...
				break;
			case SSTAR_WLAN_CATEGORY_HT: {
				//add 8.5.12.2 Notify Channel Width frame format
				if (WLAN_HT_ACTION_NOTIFY_CHANWIDTH ==
				    mgmt->u.action.u.notify_chan_width
					    .action_code) {
					break;
				}
			}
	...
	if (skb->len >= 24 + 2 /* mgmt + deauth reason */ &&
		...
		*/
		if ((list_empty(&local->work_list)) &&
		    (local->roc_pendding ||
		     (local->scan_req && !local->scanning))) {
			Sstar_printk_mgmt(
				"%s:queue work_work for pending scan or listenning",
				__func__);
		...
		*/
		...
		if ((sdata->vif.p2p == true) && (pre_reason != 23)) {
			...
			Sstar_printk_debug(
				"%s(%s):IEEE80211_STYPE_DEAUTH,reason(%d),pre_reason(%d)\n",
				__func__, sdata->name,
				le16_to_cpu(mgmt->u.deauth.reason_code),
				pre_reason);
		}
		...
		if (sdata->vif.p2p == true) {
			...
			{
				Sstar_wdev_lock(wdev);
				cfg80211_rx_mlme_mgmt(sdata->dev, (u8 *)mgmt,
						      skb->len);
				Sstar_wdev_unlock(wdev);
			}
#endif
		...
		Sstar_printk_err("##### %s:%d -- da:%pM sa:%pM bssid:%pM \n",
				 __func__, __LINE__, mgmt->da, mgmt->sa,
				 mgmt->bssid);
		mutex_lock(&local->mtx);
		ieee80211_recalc_idle(sdata->local);
		mutex_unlock(&local->mtx);
...
void Sstar_send_deauth_disossicate(struct ieee80211_sub_if_data *sdata,
				   u8 *bssid)
{
...
void ieee80211_sta_quiesce(struct ieee80211_sub_if_data *sdata)
{
	 ...
	 */
#ifdef CONFIG_SSTAR_SMPS
	Sstar_cancel_work_sync(&ifmgd->request_smps_work);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	Sstar_cancel_work_sync(&ifmgd->monitor_work);
#endif
	Sstar_cancel_work_sync(&ifmgd->beacon_connection_loss_work);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	if (Sstar_del_timer_sync(&ifmgd->timer))
		set_bit(TMR_RUNNING_TIMER, &ifmgd->timers_running);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
	Sstar_cancel_work_sync(&ifmgd->chswitch_work);
	if (Sstar_del_timer_sync(&ifmgd->chswitch_timer))
		set_bit(TMR_RUNNING_CHANSW, &ifmgd->timers_running);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
...
void ieee80211_sta_restart(struct ieee80211_sub_if_data *sdata)
{
	...
	if (test_and_clear_bit(TMR_RUNNING_TIMER, &ifmgd->timers_running))
		Sstar_add_timer(&ifmgd->timer);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
	if (test_and_clear_bit(TMR_RUNNING_CHANSW, &ifmgd->timers_running))
		Sstar_add_timer(&ifmgd->chswitch_timer);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	ieee80211_sta_reset_beacon_monitor(sdata);
...
void ieee80211_sta_setup_sdata(struct ieee80211_sub_if_data *sdata)
{
	...
	ifmgd = &sdata->u.mgd;
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	SSTAR_INIT_WORK(&ifmgd->monitor_work, ieee80211_sta_monitor_work);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_CHANSWITCH
	SSTAR_INIT_WORK(&ifmgd->chswitch_work, ieee80211_chswitch_work);
	Sstar_setup_timer(&ifmgd->chswitch_timer, ieee80211_chswitch_timer,
			  (unsigned long)sdata);
#endif
	SSTAR_INIT_WORK(&ifmgd->beacon_connection_loss_work,
			ieee80211_beacon_connection_loss_work);
#ifdef CONFIG_SSTAR_SMPS
	SSTAR_INIT_WORK(&ifmgd->request_smps_work, ieee80211_request_smps_work);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	Sstar_setup_timer(&ifmgd->timer, ieee80211_sta_timer,
			  (unsigned long)sdata);
	Sstar_setup_timer(&ifmgd->bcn_mon_timer, ieee80211_sta_bcn_mon_timer,
			  (unsigned long)sdata);
	Sstar_setup_timer(&ifmgd->conn_mon_timer, ieee80211_sta_conn_mon_timer,
			  (unsigned long)sdata);
#endif
	/*
...
#ifdef CONFIG_SSTAR_PM_QOS
int ieee80211_max_network_latency(struct notifier_block *nb, unsigned long data,
				  void *dummy)
{
	s32 latency_usec = (s32)data;
	struct ieee80211_local *local = container_of(nb, struct ieee80211_local,
...
ieee80211_probe_auth_done(struct ieee80211_work *wk, struct sk_buff *skb)
{
	...
	if (wk->type == IEEE80211_WORK_AUTH) {
		...
		{
			Sstar_wdev_lock(wdev);
			cfg80211_rx_mlme_mgmt(wk->sdata->dev, skb->data,
					      skb->len);
			Sstar_wdev_unlock(wdev);
		}
#endif
		       ...
		       struct cfg80211_auth_request *req)
{
	...
	case NL80211_AUTHTYPE_NETWORK_EAP:
		...
		break;
#ifdef CONFIG_SSTAR_SUPPORT_SAE
...
#endif
#endif
	wk = Sstar_kzalloc(sizeof(*wk) + ie_len, GFP_KERNEL);
	if (!wk)
		return -ENOMEM;
...
//#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 0, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0))
	...
	ssid = ieee80211_bss_get_ie(req->bss, SSTAR_WLAN_EID_SSID);
	if (ssid) {
		memcpy(wk->probe_auth.ssid, ssid + 2, ssid[1]);
		wk->probe_auth.ssid_len = ssid[1];
	} else {
		Sstar_printk_mgmt("%s:no ssid\n", __func__);
	...
	Sstar_printk_always("authen:(%pM),ssid(%s)\n", wk->filter_ta,
			    wk->probe_auth.ssid);
...
static int ieee80211_pre_assoc(struct ieee80211_sub_if_data *sdata, u8 *bssid)
{
	...
	sta = NULL;
	if (err) {
		Sstar_printk_err("%s: failed to insert Dummy STA(%d)\n",
				 sdata->name, err);
		return err;
	}
						  ...
						  struct sk_buff *skb)
{
...
#endif
		mutex_lock(&local->mtx);
		Sstar_printk_err("ieee80211_assoc_done err\n");
		ieee80211_cancle_connecting_work(wk->sdata, wk->filter_ta,
						 true);
		mutex_unlock(&local->mtx);
		goto destroy;
	}
	...
	if (wk->type == IEEE80211_WORK_ASSOC_BEACON_WAIT) {
		...
		ieee802_11_parse_elems(skb->data + 24 + 12, skb->len - 24 - 12,
				       &elems);
		ieee80211_rx_bss_info(wk->sdata, (void *)skb->data, skb->len,
				      rx_status, &elems, true);
		mutex_unlock(&wk->sdata->u.mgd.mtx);
			...
			struct cfg80211_assoc_request *req)
{
	...
	wk = Sstar_kzalloc(sizeof(*wk) + req->ie_len, GFP_KERNEL);
	if (!wk)
		return -ENOMEM;
	...
	wk->assoc.wmm_used =
		bss->wmm_used && (local->hw.queues >= IEEE80211_NUM_ACS);
	wk->assoc.supp_rates = bss->supp_rates;
	wk->assoc.supp_rates_len = bss->supp_rates_len;
	wk->assoc.ht_information_ie =
		ieee80211_bss_get_ie(req->bss, SSTAR_WLAN_EID_HT_INFORMATION);
			 ...
			 struct cfg80211_deauth_request *req, void *cookie)
{
	...
	*/
	...
	if (ifmgd->associated &&
	    (Sstar_compare_ether_addr(ifmgd->associated->bssid, req->bssid) ==
	     0))
#endif
	{
...
void ieee80211_dfs_cac_timer_work(struct Sstar_work_struct *work)
{
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 9, 0))
	struct Sstar_delayed_work *delayed_work = Sstar_to_delayed_work(work);
	struct ieee80211_sub_if_data *sdata =
		container_of(Sstar_delayed_work, struct ieee80211_sub_if_data,
			     dfs_cac_timer_work);
	mutex_lock(&sdata->local->mtx);
...
{
	Sstar_cancel_delayed_work_sync(&sdata->dfs_cac_timer_work);
	sdata->radar_required = false;
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 9, 0))
	if (sdata->wdev.cac_started) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
