...
* unless specifically authorized in writing by sigmastar
******************************************************************************/
...
extern int smartconfig_scan(struct Sstar_common *priv);
extern 	int fun_recv_magic(struct Sstar_common *hw_priv,short rxdata);
extern int fun_recv_payload(short rxdata, int turn_id);
extern int Sstar_smartconfig_stop(struct Sstar_common *hw_priv);
int Sstar_smart_scan_start(struct Sstar_vif *priv, struct wsm_scan *scan)
{
        int ret, i;
        int tmo = 5000;
...
void Sstar_smart_scan_work(struct Sstar_work_struct *work)
{
	...
	if (!priv) {
	 	Sstar_printk_scan("%s:[SCAN] interface removed\n",__func__);
	    return ;
	}
	...
	scan.ch = Sstar_kzalloc(sizeof(struct wsm_scan_ch[scan.numOfChannels]),GFP_KERNEL);
	if (!scan.ch) {
	...
	for (i = 0; i < scan.numOfChannels; i++) {
		...
		scan.ch[i].number =i+1;//Sstar_band_2ghz.channels[i].hw_value;
		scan.ch[i].txPowerLevel =30; //Sstar_band_2ghz.channels[i].max_power;
	}

	hw_priv->scan.status = Sstar_smart_scan_start(priv, &scan);
...
fail:
	mutex_unlock(&hw_priv->conf_mutex);
        Sstar_hw_priv_queue_work(hw_priv, &hw_priv->scan.smartwork);
        return ;
}
void Sstar_smart_setchan_work(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv = container_of(work,
                                                struct Sstar_common,
		...
		Sstar_printk_smt("hw_priv->st_configchannel = %d flag %x\n", hw_priv->st_configchannel,arg.flag);
		wsm_set_chantype_func(hw_priv,&arg,0);
}


void Sstar_smart_stop_work(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv = container_of(work,
                                                struct Sstar_common,
                                                scan.smartstopwork);
	...
	Sstar_del_timer_sync(&hw_priv->smartconfig_expire_timer);
	if(st_payload_buf){
		 Sstar_kfree(st_payload_buf);
		 st_payload_buf= NULL;
	}
...
void smartconfig_scan_start(struct Sstar_common *hw_priv)
{
	//printk("%s %d\n", __FUNCTION__, __LINE__);
	struct Sstar_vif *priv = ABwifi_hwpriv_to_vifpriv(hw_priv,hw_priv->scan.if_id);
	if (unlikely(!priv)) {
		WARN_ON(1);
		return;
	}
	...
	if(st_payload_buf==NULL){
		st_payload_buf = Sstar_kmalloc(sizeof(short)*ST_PAYLOAD_BUF_SZ,GFP_KERNEL | GFP_DMA);
	}
	else {
...
int smartconfig_start(struct Sstar_common *hw_priv,struct smartconfig_config * st_cfg)
{
	...
	//int Istate=0;
	if(st_cfg == NULL){
		Sstar_printk_err("unsupport smartconfig st_cfg NULL!");
		return CONFIG_ST_STARTCONFIG_ERR;
	}
	if((st_cfg->magic_cnt<1)||(st_cfg->magic_cnt>10)){
		Sstar_printk_err("unsupport smartconfig imagic_cnt %d!",st_cfg->magic_cnt);
		return CONFIG_ST_STARTCONFIG_ERR;
	}
	if((st_cfg->magic_time<20)||(st_cfg->magic_time>500)){
		Sstar_printk_err("unsupport smartconfig magic_time!");
		return CONFIG_ST_STARTCONFIG_ERR;
	}
	if((st_cfg->payload_time<500)||(st_cfg->payload_time>30000)){
		Sstar_printk_err("unsupport smartconfig payload_time!");
		return CONFIG_ST_STARTCONFIG_ERR;
	}
	 memcpy(&hw_priv->st_cfg,st_cfg,sizeof(struct smartconfig_config ));
	 /*disconnect AP*/
	// AT_WDisConnect(NULL);
	 Sstar_mod_timer(&hw_priv->smartconfig_expire_timer,st_cfg->payload_time + st_cfg->magic_time*14);
	 Sstar_printk_err("st_cfg->payload_time + st_cfg->magic_time*14 = %d\n", (st_cfg->payload_time + st_cfg->magic_time*14));
...
int Sstar_smartconfig_start(struct Sstar_common *hw_priv,int enable)
{
	if (enable){
		...
		if(hw_priv->scan.scan_smartconfig){
			Sstar_printk_smt("scan_smartconfig now!please try later!\n");
			return 0 ;
	   }else
	   {
		struct smartconfig_config st_cfg = {
		.type = CONFIG_TP_SSTAR_SMART,
		.magic_cnt = 1,
		.magic_time = 50,
		.payload_time = 8000,
		};
...
{
	Sstar_printk_smt("%s:%d\n",__func__,__LINE__);
	Sstar_hw_priv_queue_work(hw_priv, &hw_priv->scan.smartstopwork);
...
int smartconfig_start_timer_func(struct Sstar_common *hw_priv )
{
	Sstar_printk_smt("smartconfig timeout\n");
...
int smartconfig_magic_scan_done(struct Sstar_common *hw_priv)
{
	//printk("smartconfig_magic_scan_done %d\n",hw_priv->st_status);

	if(hw_priv->st_status == CONFIG_ST_SWITCH_PAYLOAD_CH ){
		Sstar_printk_smt("st_configchannel %d\n",hw_priv->st_configchannel);
		Sstar_hw_priv_queue_work(hw_priv, &hw_priv->scan.smartsetChanwork);
...
const u8 Sstar_smart_config_magic_mac[4][6] ={
...
int Sstar_ieee80211_build_preq_ies(struct Sstar_vif *priv,u8 *buffer,
...
struct sk_buff *Sstar_ieee80211_build_probe_req(struct Sstar_vif *priv,
					  u8 *dst,
					  const u8 *ie, size_t ie_len)
{
	...
	//buf_len = Sstar_ieee80211_build_preq_ies(priv, buf, ie, ie_len,
	//				  hw_priv->st_configchannel);

//	Sstar_skb_put(skb, buf_len);
...
struct sk_buff * Sstar_ieee80211_send_probe_req(struct Sstar_vif *priv, u8 *dst,
			      const u8 *ie, size_t ie_len, bool no_cck)
{
	struct sk_buff *skb;

	skb = Sstar_ieee80211_build_probe_req(priv, dst,ie, ie_len);
	if (skb) {
		if (no_cck)
...
int smartconfig_scan(struct Sstar_common *hw_priv)
{
	//printk("%s:%d\n",__func__,__LINE__);
	int ret;
	struct Sstar_vif *priv;
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA)
		return -EOPNOTSUPP;
	frame.skb = Sstar_ieee80211_send_probe_req(priv,NULL,NULL,0,0);
	//printk("%s:%d\n",__func__,__LINE__);
...
#if 0
int start_connect_ap(struct Sstar_vif*priv,u8 *essid,int essid_len,u8 *key,int key_len,int key_mgmt,int keyid)
{
...
#endif
int smartconfig_step_1(struct Sstar_common *hw_priv,struct sk_buff *skb,int channel )
{
	int len  = skb->len;
	char * data  = skb->data;
	...
	if((memcmp(Sstar_smart_config_magic_mac[0],&data[4+12],6))
		&& (memcmp(Sstar_smart_config_magic_mac[0],&data[4],6))){
	...
	if((flag == FLAG_ORIGINAL_VALUE) && (!memcmp(Sstar_smart_config_magic_mac[0],&data[4],6))){
		flag = FLAG_RECEIVE_BROADCAST;
		CNT_BROAD = ORIGINAL_NUM_OF_CNT;
		Sstar_printk_smt("smartconfig:FLAG_RECEIVE_BROADCAST \n");
	}
	else if((flag == FLAG_ORIGINAL_VALUE) && (CNT_BROAD > WAIT_NUM_OF_CNT)){
		flag = FLAG_RECEIVE_MULTICAST;
		CNT_BROAD = ORIGINAL_NUM_OF_CNT;
		Sstar_printk_smt("smartconfig:FLAG_RECEIVE_MULTICAST \n");
	}
	...
	if((flag == FLAG_RECEIVE_BROADCAST) && (!memcmp(Sstar_smart_config_magic_mac[0],&data[4],6))){

		if(fun_recv_magic(hw_priv,len)/*magic done*/){
			hw_priv->st_status = CONFIG_ST_SWITCH_PAYLOAD_CH;
			hw_priv->st_configchannel = channel;
			Sstar_printk_smt("FLAG_RECEIVE_BROADCAST\n");
		}
	}
	else if((flag == FLAG_RECEIVE_MULTICAST) && (!memcmp(Sstar_smart_config_magic_mac[0],&data[4+12],6))){
...
int smartconfig_step_2(struct Sstar_common *hw_priv,struct sk_buff *skb)
{
	int turn_id = 0;
	...
	if((memcmp(Sstar_smart_config_magic_mac[1],&data[4+12],5))
		&& (memcmp(Sstar_smart_config_magic_mac[1],&data[4],5))){
			//printk("rxmac %x %x %x %x %x %x\n",data[4+12],data[4+13],data[4+14],data[4+15],data[4+16],data[4+17]);
			return -1;
	}
	if((!memcmp(Sstar_smart_config_magic_mac[3], &data[4+12], 6))){
		//printk("rxmac %x %x %x %x %x %x\n",data[4+12],data[4+13],data[4+14],data[4+15],data[4+16],data[4+17]);
		return -1;
	}

	if((!memcmp(Sstar_smart_config_magic_mac[3], &data[4], 6)))
		return -1;

	Sstar_printk_smt("data: %x %x %x %x %x %x\n",data[4],data[4+1],data[4+2],data[4+3],data[4+4],data[4+5]);	
	//printk("data: %x %x %x %x %x %x\n",data[4+12],data[4+13],data[4+14],data[4+15],data[4+16],data[4+17]);
	//printk("step_2_rx len %x \n",len);
#if 1
	if(flag == FLAG_RECEIVE_BROADCAST){
		if(memcmp(Sstar_smart_config_magic_mac[2],&data[4],5))
			return -1;
		if((!memcmp(Sstar_smart_config_magic_mac[2],&data[4],5))
			&& (data[4+5] >= Sstar_smart_config_magic_mac[2][5]))
		{
			turn_id = data[4+5] - Sstar_smart_config_magic_mac[2][5];
			//iot_printf("1\n");
			Sstar_printk_smt("receive broadcast\n");
		}
	}
	else if(flag == FLAG_RECEIVE_MULTICAST){
		if(memcmp(Sstar_smart_config_magic_mac[2],&data[4+12],5))
			return -1;
		if((!memcmp(Sstar_smart_config_magic_mac[2],&data[4+12],5))
			&& (data[4+17] >= Sstar_smart_config_magic_mac[2][5]))
		{
			turn_id = data[4+17] - Sstar_smart_config_magic_mac[2][5];
			//iot_printf("2\n");
			Sstar_printk_smt("receive multicast\n");
		}
	}
#else
	if(data[4+17] >= Sstar_smart_config_magic_mac[2][5]){
		turn_id = data[4+17] - Sstar_smart_config_magic_mac[2][5];
		if(turn_id >= 10)
			return -1;
	}
...
int smartconfig_start_rx(struct Sstar_common *hw_priv,struct sk_buff *skb,int channel )
{
	if(hw_priv->st_status == CONFIG_ST_GET_MAGIC){
