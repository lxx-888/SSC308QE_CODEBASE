...
#define SSTAR_MAX_OVERFLOW_SIZE	(64)
...
void Sstar_unregister_bh(struct Sstar_common *hw_priv)
{
	...
	hw_priv->bh_thread = NULL;
	Sstar_printk_exit( "[BH] unregister.\n");
...
static int Sstar_rx_directly(struct Sstar_common *hw_priv,struct sk_buff *skb)
{
	u8 *data;
	struct wsm_hdr *wsm;
	u32 wsm_len;
...
static int Sstar_sdio_submit_skb(struct Sstar_common *hw_priv,struct sk_buff *skb)
{
	...
	queue_len = Sstar_skb_queue_len(&hw_priv->rx_frame_queue);
	...
	return (queue_len >= SSTAR_MAX_OVERFLOW_SIZE) && (bh_running == false);
}
static int Sstar_sdio_submit_skb_in_thread(struct Sstar_common *hw_priv,struct sk_buff *skb)
{
	__u32 queue_len = 0;
	...
	return queue_len >= SSTAR_MAX_OVERFLOW_SIZE;
}
#endif

static int Sstar_sdio_process_read_data(struct Sstar_common *hw_priv,reed_ctrl_handler_t read_ctrl_func,
	reed_data_handler_t read_data_func ,int (*rx_handle)(struct Sstar_common *hw_priv,struct sk_buff *skb),enum Sstar_rx_frame_type frame_type)
{
...
rx_check:
	if (read_ctrl_func(
			hw_priv, &ctrl_reg)){
			//Sstar_printk_err("read ctrl func error\n");
			goto err;
	}
rx_continue:
...
#define MAX_LOOP_POLL_CNT  (2*3000)
	...
	while(enough == false){
		...
		if(Sstar_bh_is_term(hw_priv)){
			Sstar_printk_err("%s:bh term\n",__func__);
			return false;
		}
#ifdef CONFIG_TX_NO_CONFIRM
#ifndef CONFIG_SSTAR_SDIO_TX_HOLD
		...
		if(enough == false){
			...
			if(loop>=MAX_LOOP_POLL_CNT)
				break;
			if((loop >= 3)&&(print == 0)){
				Sstar_printk_debug("%s:n_xmits(%d),hw_xmited(%d),need(%d)\n",__func__,
...
static void Sstar_sdio_force_free_wsm(struct Sstar_common	*hw_priv,struct wsm_tx *wsm)
{
	int wsm_id;
	...
	case WSM_TRANSMIT_REQ_MSG_ID:
		Sstar_sdio_release_err_data(hw_priv,wsm);
		break;
	default:
		wsm_release_tx_buffer(hw_priv, 1);
...
#endif
#define SSTAR_SDIO_FREE_BUFF_ERR(condition,free,prev_free,xmiteds,hw_xmiteds)	\
	do{																			\
		if(condition)	{																\
			Sstar_printk_err("%s[%d]:free(%x),prev_free(%x),xmiteds(%x),hw_xmiteds(%x)\n",__func__,__LINE__,free,prev_free,xmiteds,hw_xmiteds);	\
			BUG_ON(1);			\
		}\
	}while(0)
#ifdef CONFIG_TX_NO_CONFIRM
...
xmit_continue:
	...
	do {
		...
		spin_lock_bh(&hw_priv->tx_com_lock);
		SSTAR_SDIO_FREE_BUFF_ERR(hw_priv->hw_bufs_free <= 0,hw_priv->hw_bufs_free,hw_priv->hw_bufs_free_init,hw_priv->n_xmits,hw_priv->hw_xmits);
		hw_priv->n_xmits ++;
		...
		if(wsm_txed(hw_priv, data)){
			...
			if(Sstar_sdio_free_tx_wsm(hw_priv,(struct wsm_tx *)data) == 0){
				need_confirm = data;
				Sstar_printk_debug("%s:confirm free(%d),used(%d)\n",__func__,hw_priv->hw_bufs_free,hw_priv->hw_bufs_used);
				break;
			}
#else
			need_confirm = data;
	...
	if(Sstar_bh_is_term(hw_priv)){
		Sstar_printk_err("%s:bh term\n",__func__);
		Sstar_sdio_force_free_wsm(hw_priv,(struct wsm_tx *)need_confirm);
		goto xmit_continue;
	}
#ifdef CONFIG_SSTAR_SDIO_TX_HOLD
	if (WARN_ON(Sstar_data_write_unlock(hw_priv,hw_priv->xmit_buff, putLen))) {
...
restart:
		...
		*/
		...
		if(ctrl_reg & SSTAR_HIFREG_CONT_NEXT_LEN_MASK){
			Sstar_sdio_process_read_data(hw_priv,Sstar_bh_read_ctrl_reg_unlock,
										Sstar_data_read_unlock,Sstar_sdio_submit_skb_in_thread,SSTAR_RX_DERICTLY_DATA_FRAME);
		}
		hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
...
//	hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
	Sstar_sdio_process_read_data(hw_priv,Sstar_bh_read_ctrl_reg,Sstar_data_read,Sstar_rx_directly,SSTAR_RX_RAW_FRAME);
//	hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
}
...
void Sstar_irq_handler(struct Sstar_common *hw_priv)
{
	...
	if(hw_priv->hard_irq == false)	{
		...
		__Sstar_irq_enable(hw_priv,0);
rx_continue:
#ifdef CONFIG_SDIO_IRQ_THREAD_PROCESS_DATA
		rx_counter = Sstar_sdio_process_read_data(hw_priv,Sstar_bh_read_ctrl_reg_unlock,Sstar_data_read_unlock,
									Sstar_sdio_submit_skb,SSTAR_RX_DERICTLY_DATA_FRAME);
#else
		rx_counter = 0;
#endif
		if(rx_counter == 0){
			...
			*/
			Sstar_printk_debug("%s:no package triger rx\n",__func__);
			goto triger_rx;
		}else if(rx_counter >= SSTAR_MAX_OVERFLOW_SIZE){
			hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
			Sstar_printk_debug("%s:over flow\n",__func__);
...
int Sstar_bh_suspend(struct Sstar_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		Sstar_printk_warn("BH error -- can't suspend\n");
		return -EINVAL;
	}
#ifdef MCAST_FWDING
...
int Sstar_bh_resume(struct Sstar_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		Sstar_printk_warn("BH error -- can't resume\n");
		return -EINVAL;
	}
...
#endif
		Sstar_skb_reserve(skb, ALIGN(WSM_TX_EXTRA_HEADROOM
   			+ 8 /* TKIP IV */
	...
	while (retry <= MAX_RETRY) {
		...
		if(!ret){
			...
			if(hw_priv->sbus_ops->abort(hw_priv->sbus_priv) != 0){
				//Sstar_printk_err("Sstar_bh_read_ctrl_reg , abort fail ! \n");
				break;
			}
			retry++;
			mdelay(retry);
					  ...
					  u16 *ctrl_reg)
{
	...
	while (retry <= MAX_RETRY) {
		...
		if(!ret){
			...
			//WARN_ON(hw_priv->sbus_ops->abort(hw_priv->sbus_priv));
			if(hw_priv->sbus_ops->abort(hw_priv->sbus_priv) != 0){
				//Sstar_printk_err("Sstar_bh_read_ctrl_reg_unlock , abort fail ! \n");
				break;
			}
			retry++;
			mdelay(retry);
...
#define RX_ALLOC_BUFF_OFFLOAD (  (40+16)/*RX_DESC_OVERHEAD*/ -16 /*WSM_HI_RX_IND*/)
		...
		if((wsm_id == WSM_MULTI_RECEIVE_INDICATION_ID)||
			...
			Sstar_bh_multrx_trace(hw_priv,RxFrameNum);
...
#define __ALL_HW_BUFS_USED (hw_priv->hw_bufs_used)
	while (1) {
		...
		if (0)
		{
			...
			Sstar_printk_warn("Missed interrupt Status =%d, buffused=%d\n",(int)status,(int)__ALL_HW_BUFS_USED);
			rx = 1;
			Sstar_printk_debug("[bh] next wsm_rx_seq %d wsm_tx_seq %d\n",hw_priv->wsm_rx_seq,hw_priv->wsm_tx_seq);
			Sstar_printk_debug("[bh] wsm_hiftx_cmd_num %d wsm_hif_cmd_conf_num %d\n",hw_priv->wsm_hiftx_num,hw_priv->wsm_hifconfirm_num);
			Sstar_printk_debug("[bh] wsm_txframe_num %d wsm_txconfirm_num %d\n",hw_priv->wsm_txframe_num,hw_priv->wsm_txconfirm_num);
			Sstar_printk_debug("[bh] num_pending[0]=%d num_pending[1]=%d pending[2]=%d pending[3]=%d\n",
			...
			status = wait_event_interruptible(hw_priv->bh_wq,
					SSTAR_APOLLO_BH_RESUME == atomic_read(
						&hw_priv->bh_suspend));
			if (status < 0) {
				Sstar_printk_err("%s: Failed to wait for resume: %ld.\n",
					__func__, status);
				break;
			}
...
//			ieee80211_driver_hang_notify(priv->vif, GFP_KERNEL);
		...
		#endif
		Sstar_dbg(SSTAR_APOLLO_DBG_ERROR, "[BH] Fatal error, exitting.1\n");
		...
		for (;;) {
			...
			Sstar_dbg(SSTAR_APOLLO_DBG_ERROR, "[BH] Fatal error, exitting.2\n");
			if (status || term)
				break;
		}
#endif
	...
	if(term)
	{
		...
		*/
		...
		if(atomic_read(&hw_priv->scan.in_progress))
		{
			...
			scanto_running = Sstar_hw_cancel_delayed_work(&hw_priv->scan.timeout,true);
			mutex_lock(&hw_priv->conf_mutex);
			if(scanto_running>0)
			{
		...
		mutex_unlock(&hw_priv->conf_mutex);
		{
			...
			if(Sstar_hw_cancel_delayed_work(&hw_priv->scan.probe_work,true))
				Sstar_probe_work(&hw_priv->scan.probe_work.work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
			if(Sstar_hw_cancel_delayed_work(&hw_priv->rem_chan_timeout,true))
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
				if(Sstar_hw_cancel_delayed_work(&priv->chantype_change_work,true))
...
#ifndef CONFIG_TX_NO_CONFIRM
				if(Sstar_hw_cancel_delayed_work(&priv->bss_loss_work,true))
					Sstar_bss_loss_work(&priv->bss_loss_work.work);
				if(Sstar_hw_cancel_delayed_work(&priv->connection_loss_work,true))
...
#endif
				if(Sstar_hw_cancel_delayed_work(&priv->link_id_gc_work,true))
					Sstar_link_id_gc_work(&priv->link_id_gc_work.work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
				if(Sstar_hw_cancel_delayed_work(&priv->pending_offchanneltx_work,true))
					Sstar_pending_offchanneltx_work(&priv->pending_offchanneltx_work.work);
#endif
				if(Sstar_hw_cancel_delayed_work(&priv->join_timeout,true))
