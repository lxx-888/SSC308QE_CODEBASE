...
#ifndef SSTAR_APOLLO_WSM_H_INCLUDED
#define SSTAR_APOLLO_WSM_H_INCLUDED
...
/* Key indexes */
#define SSTARWIFI_WSM_KEY_MAX_INDEX (15)
...
int wsm_read_mib(struct Sstar_common *hw_priv, u16 mibId, void *buf,
		 size_t buf_size, int if_id);

/* 3.7 */
#define WSM_WRITE_MIB_REQ_ID 0x0006
#define WSM_WRITE_MIB_RESP_ID 0x0406
int wsm_write_mib(struct Sstar_common *hw_priv, u16 mibId, void *buf,
		  size_t buf_size, int if_id);
...
typedef void (*wsm_scan_complete_cb)(struct Sstar_common *hw_priv,
...
/* 3.15 */
typedef void (*wsm_tx_confirm_cb)(struct Sstar_common *hw_priv,
 ...
 * It is filled partly in Sstar_tx, partly in low-level WSM code.
 * Please pay attention once again: ideology is different.
 *
 * Legend:
 * - [in]: Sstar_tx must fill this field.
...
typedef void (*wsm_rx_cb)(struct Sstar_vif *priv, struct wsm_rx *arg,
...
typedef void (*wsm_event_cb)(struct Sstar_common *hw_priv,
...
typedef void (*wsm_set_pm_complete_cb)(struct Sstar_common *hw_priv,
...
#define WSM_TX_QUEUE_SET(queue_params, queue, ack_policy, allowed_time,        \
			 max_life_time)                                        \
	do {                                                                   \
		struct wsm_set_tx_queue_params *p =                            \
...
int wsm_set_uapsd_param(struct Sstar_common *hw_priv,
...
typedef void (*wsm_channel_switch_cb)(struct Sstar_common *hw_priv);
...
int wsm_beacon_transmit(struct Sstar_common *hw_priv,
...
typedef void (*wsm_find_complete_cb)(struct Sstar_common *hw_priv, u32 status);
...
typedef void (*wsm_suspend_resume_cb)(struct Sstar_vif *priv,
...
int wsm_init_release_buffer_request(struct Sstar_common *priv, u8 index);
...
static inline int wsm_set_output_power(struct Sstar_common *hw_priv,
...
static inline int wsm_set_beacon_wakeup_period(struct Sstar_common *hw_priv,
...
static inline int
wsm_set_rcpi_rssi_threshold(struct Sstar_common *hw_priv,
...
static inline int wsm_get_counters_table(struct Sstar_common *hw_priv,
					 struct wsm_counters_table *arg)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_COUNTERS_TABLE, arg,
			    sizeof(*arg), -1);
}

static inline int wsm_get_statistics_table(struct Sstar_common *hw_priv,
					   struct wsm_statistics_table *arg)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_STATISTICS_TABLE, arg,
			    sizeof(*arg), -1);
}

static inline int wsm_get_station_id(struct Sstar_common *hw_priv, u8 *mac)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_DOT11_STATION_ID, mac, ETH_ALEN,
			    -1);
}

static inline int wsm_get_mac_address(struct Sstar_common *hw_priv, u8 *mac)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_MAC_ADDRESS_FROM_EFUSE, mac,
			    ETH_ALEN, -1);
}

static inline int wsm_set_efuse_mac_address(struct Sstar_common *hw_priv,
					    u8 *mac)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_EFUSE_MAC, mac, ETH_ALEN,
			     -1);
}
//notice!!!this function no write data to efuse register, but write data to variable
static inline int wsm_set_efuse_data(struct Sstar_common *hw_priv, u8 *efuse,
				     int len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_EFUSE_DATA_ZHUANG, efuse,
			     len, -1);
}
static inline int wsm_get_SIGMSTAR_256BITSEFUSE(struct Sstar_common *hw_priv,
						u8 *efuse, int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_SIGMSTAR_256BITSEFUSE,
			    efuse, len, -1);
}
static inline int wsm_set_SIGMSTAR_256BITSEFUSE(struct Sstar_common *hw_priv,
						u8 *efuse, int len)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_SIGMSTAR_256BITSEFUSE,
			     efuse, len, -1);
}
...
static inline int wsm_get_tx_power_status(struct Sstar_common *hw_priv,
					  void *pwrStatus, int len)
{
...
static inline int wsm_get_efuse_data(struct Sstar_common *hw_priv, void *efuse,
				     int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_DATA_FROM_EFUSE, efuse, len,
			    -1);
}

static inline int wsm_get_cfo_ppm_correction_value(struct Sstar_common *hw_priv,
						   void *buf, int len)
{
	if (buf == NULL)
		return -1;
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_CFO_PPM_CORRECTION_VALUE,
			    buf, len, -1);
}

static inline int wsm_get_efuse_first_data(struct Sstar_common *hw_priv,
					   void *efuse, int len)
{
...
static inline int wsm_get_efuse_remain_bit(struct Sstar_common *hw_priv,
					   void *remainBit, int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_EFUSE_CUR_STATUS, remainBit,
			    len, -1);
}

static inline int wsm_get_efuse_last_mac(struct Sstar_common *hw_priv, u8 *mac)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_EFUSE_LAST_MAC, mac,
			    ETH_ALEN, -1);
}

static inline int wsm_get_efuse_first_mac(struct Sstar_common *hw_priv, u8 *mac)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_EFUSE_FIRST_MAC, mac,
			    ETH_ALEN, -1);
}

static inline int wsm_get_Tjroom_temperature(struct Sstar_common *hw_priv,
					     void *buf, int len)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_TEMP_TX_REQ_ID, buf, len,
			    -1);
}

static inline int wsm_get_efuse_all_data(struct Sstar_common *hw_priv,
					 void *buf, int len)
{
	if (buf == NULL)
		return -1;
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_EFUSE_ALL_DATA, buf, len,
			    -1);
}

static inline int wsm_get_cfg_txpower(struct Sstar_common *hw_priv, void *buf,
				      int len)
{
	if (buf == NULL)
		return -1;
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_CONFIG_TX_POWER, buf, len,
			    -1);
}
...
static inline int wsm_set_rx_filter(struct Sstar_common *hw_priv,
...
static inline int
wsm_set_beacon_filter_table(struct Sstar_common *hw_priv,
...
#define WSM_BEACON_FILTER_AUTO_ERP                                             \
	BIT(1) /* If 1 FW will handle ERP IE changes internally */
...
static inline int
wsm_beacon_filter_control(struct Sstar_common *hw_priv,
...
static inline int
wsm_set_operational_mode(struct Sstar_common *hw_priv,
...
static inline int wsm_set_inactivity(struct Sstar_common *hw_priv,
...
static inline int wsm_set_template_frame(struct Sstar_common *hw_priv,
					 struct wsm_template_frame *arg,
					 int if_id)
{
	int ret;
	u8 *p = Sstar_skb_push(arg->skb, 4);
	p[0] = arg->frame_type;
	p[1] = arg->rate;
...
static inline int
wsm_set_protected_mgmt_policy(struct Sstar_common *hw_priv,
...
static inline int wsm_set_block_ack_policy(struct Sstar_common *hw_priv,
					   ...
					   u8 blockAckRxTidPolicy, int if_id)
{
	...
	Sstar_printk_mgmt(
		"%s:blockAckTxTidPolicy(%x),blockAckRxTidPolicy(%x),if_id(%d)\n",
...
static inline int wsm_set_association_mode(struct Sstar_common *hw_priv,
...
static inline int
wsm_set_tx_rate_retry_policy(struct Sstar_common *hw_priv,
...
static inline int
wsm_set_ether_type_filter(struct Sstar_common *hw_priv,
...
static inline int wsm_set_udp_port_filter(struct Sstar_common *hw_priv,
...
static inline int wsm_keep_alive_period(struct Sstar_common *hw_priv,
					int period, int if_id)
{
	struct wsm_keep_alive_period arg = {
		.keepAlivePeriod = __cpu_to_le16(period),
	};
	return wsm_write_mib(hw_priv, WSM_MIB_ID_KEEP_ALIVE_PERIOD, &arg,
			     sizeof(arg), if_id);
};
...
static inline int wsm_set_bssid_filtering(struct Sstar_common *hw_priv,
					  bool enabled, int if_id)
{
	struct wsm_set_bssid_filtering arg = {
		.filter = !enabled,
	};
	return wsm_write_mib(hw_priv, WSM_MIB_ID_DISABLE_BSSID_FILTER, &arg,
			     sizeof(arg), if_id);
}
...
static inline int wsm_set_multicast_filter(struct Sstar_common *hw_priv,
...
static inline int wsm_set_arp_ipv4_filter(struct Sstar_common *hw_priv,
					  struct wsm_arp_ipv4_filter *fp,
					  int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_ARP_IP_ADDRESSES_TABLE, fp,
			     sizeof(*fp), if_id);
}

#ifdef IPV6_FILTERING
static inline int wsm_set_ndp_ipv6_filter(struct Sstar_common *priv,
					  struct wsm_ndp_ipv6_filter *fp,
					  int if_id)
{
	return wsm_write_mib(priv, WSM_MIB_ID_NS_IP_ADDRESSES_TABLE, fp,
			     sizeof(*fp), if_id);
}
#endif /*IPV6_FILTERING*/
#endif
...
static inline int wsm_set_p2p_ps_modeinfo(struct Sstar_common *hw_priv,
					  struct wsm_p2p_ps_modeinfo *mi,
					  int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_P2P_PS_MODE_INFO, mi,
			     sizeof(*mi), if_id);
}

static inline int wsm_get_p2p_ps_modeinfo(struct Sstar_common *hw_priv,
					  struct wsm_p2p_ps_modeinfo *mi,
					  int if_id)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_P2P_PS_MODE_INFO, mi,
			    sizeof(*mi), if_id);
}
#endif
/* UseMultiTxConfMessage */

static inline int wsm_use_multi_tx_conf(struct Sstar_common *hw_priv,
					bool enabled, int if_id)
{
	__le32 arg = enabled ? __cpu_to_le32(1) : 0;

	return wsm_write_mib(hw_priv, WSM_MIB_USE_MULTI_TX_CONF, &arg,
			     sizeof(arg), if_id);
}
...
static inline int wsm_set_uapsd_info(struct Sstar_common *hw_priv,
				     struct wsm_uapsd_info *arg, int if_id)
{
	/* TODO:COMBO:UAPSD will be supported only on one interface */
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_UAPSD_INFORMATION, arg,
			     sizeof(*arg), if_id);
}
...
static inline int
wsm_set_override_internal_txrate(struct Sstar_common *hw_priv,
				 struct wsm_override_internal_txrate *arg,
				 int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_OVERRIDE_INTERNAL_TX_RATE, arg,
			     sizeof(*arg), if_id);
}
#ifdef MCAST_FWDING
...
static inline int wsm_set_forwarding_offlad(struct Sstar_common *hw_priv,
					    struct wsm_forwarding_offload *arg,
					    int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_FORWARDING_OFFLOAD, arg,
			     sizeof(*arg), if_id);
}

#endif
#ifdef IPC_AP_USED_11G_NO_RTS
static inline int wsm_set_rts_threshold(struct Sstar_common *hw_priv, int if_id)
{
	int ret;
	int val32 = 2347;
...
int wsm_generic_req(struct Sstar_common *hw_priv, const struct wsm_gen_req *arg,
		    void *_buf, size_t buf_size, int if_id);
static inline int Sstar_req_sta_info(struct Sstar_common *hw_priv,
				     struct wsm_sta_info_req *req,
				     struct wsm_sta_info *info, int if_id)
{
	...
	return wsm_generic_req(hw_priv, &arg, (void *)info,
			       sizeof(struct wsm_sta_info), if_id);
}
/* ******************************************************************** */
/* WSM TX port control							*/
...
extern int wsm_req_chtype_change_func(struct Sstar_common *hw_priv,
				      struct wsm_req_chtype_change *arg,
				      int if_id);
int wsm_req_chtype_indication(struct Sstar_common *hw_priv,
			      struct wsm_buf *buf);
#endif
void wsm_sync_channl_reset(struct Sstar_work_struct *work);
int sdio_sync_channle_process(struct Sstar_common *hw_priv);
#endif
#ifdef SSTAR_SDIO_PATCH
...
typedef enum { OUT_BH = 1, IN_BH } BH;
