...
#if defined(CONFIG_SSTAR_APOLLO_STA_DEBUG)
#define sta_printk(...) Sstar_printk_always(__VA_ARGS__)
#else
#define sta_printk(...)
#endif
...
#if defined(CONFIG_NL80211_TESTMODE) && defined(CONFIG_SSTAR_TEST_TOOL)
static u8 ETF_bStart_Tx = 0;
static u8 ETF_bStart_Rx = 0;
static char ch_and_type[20] = { 0 };
#endif
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
const int Sstar_1d_to_ac[8] = { IEEE80211_AC_BE, IEEE80211_AC_BK,
				IEEE80211_AC_BK, IEEE80211_AC_BE,
				IEEE80211_AC_VI, IEEE80211_AC_VI,
				IEEE80211_AC_VO, IEEE80211_AC_VO };

/**
 * enum Sstar_ac_numbers - AC numbers as used in apollo
 * @SSTAR_APOLLO_AC_VO: voice
 * @SSTAR_APOLLO_AC_VI: video
 * @SSTAR_APOLLO_AC_BE: best effort
 * @SSTAR_APOLLO_AC_BK: background
 */
enum Sstar_ac_numbers {
	SSTAR_APOLLO_AC_VO = 0,
	SSTAR_APOLLO_AC_VI = 1,
	SSTAR_APOLLO_AC_BE = 2,
	SSTAR_APOLLO_AC_BK = 3,
};
#endif /*CONFIG_SSTAR_APOLLO_TESTMODE*/
...
static inline void __Sstar_free_event_queue(struct list_head *list)
{
	while (!list_empty(list)) {
		struct Sstar_wsm_event *event =
			list_first_entry(list, struct Sstar_wsm_event, link);
		list_del(&event->link);
		Sstar_kfree(event);
	}
}
...
const int Sstar_priority_to_queueId[8] = {
	WSM_QUEUE_BEST_EFFORT, WSM_QUEUE_BACKGROUND, WSM_QUEUE_BACKGROUND,
	WSM_QUEUE_BEST_EFFORT, WSM_QUEUE_VIDEO,	     WSM_QUEUE_VIDEO,
	WSM_QUEUE_VOICE,       WSM_QUEUE_VOICE
};
#endif /*CONFIG_SSTAR_APOLLO_TESTMODE*/
static inline void __Sstar_bf_configure(struct Sstar_vif *priv)
{
	priv->bf_table.numOfIEs = __cpu_to_le32(3);
	priv->bf_table.entry[0].ieId = SSTAR_WLAN_EID_VENDOR_SPECIFIC;
	...
	priv->bf_table.entry[1].ieId = SSTAR_WLAN_EID_ERP_INFO;
	priv->bf_table.entry[1].actionFlags =
		WSM_BEACON_FILTER_IE_HAS_CHANGED |
		WSM_BEACON_FILTER_IE_NO_LONGER_PRESENT |
		WSM_BEACON_FILTER_IE_HAS_APPEARED;

	priv->bf_table.entry[2].ieId = SSTAR_WLAN_EID_HT_INFORMATION;
...
int Sstar_start(struct ieee80211_hw *dev)
{
	...
	memset(&hw_priv->tsm_stats, 0, sizeof(struct Sstar_tsm_stats));
	memset(hw_priv->Sstar_tsm_stats, 0, 4 * sizeof(struct Sstar_tsm_stats));
	memset(&hw_priv->tsm_info, 0, sizeof(struct Sstar_tsm_info));
	spin_unlock_bh(&hw_priv->tsm_lock);
#endif /*CONFIG_SSTAR_APOLLO_TESTMODE*/
	memcpy(hw_priv->mac_addr, dev->wiphy->perm_addr, ETH_ALEN);
	hw_priv->softled_state = 0;

	ret = Sstar_setup_mac(hw_priv);
	if (WARN_ON(ret))
		goto out;
...
void Sstar_stop(struct ieee80211_hw *dev)
{
	...
	Sstar_hw_cancel_delayed_work(&hw_priv->scan.probe_work, true);
	Sstar_hw_cancel_delayed_work(&hw_priv->scan.timeout, true);
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
	Sstar_hw_cancel_delayed_work(&hw_priv->advance_scan_timeout, true);
#endif
	Sstar_flush_workqueue(hw_priv->workqueue);
#ifdef CONFIG_SSTAR_BA_STATUS
	Sstar_del_timer_sync(&hw_priv->ba_timer);
#endif

	mutex_lock(&hw_priv->conf_mutex);

	hw_priv->softled_state = 0;
	/* Sstar_set_leds(hw_priv); */

	spin_lock_bh(&hw_priv->event_queue_lock);
	list_splice_init(&hw_priv->event_queue, &list);
	spin_unlock_bh(&hw_priv->event_queue_lock);
	__Sstar_free_event_queue(&list);
...
#endif
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_PASSIVE;
		Sstar_hw_cancel_delayed_work(&priv->join_timeout, false);
#ifndef CONFIG_TX_NO_CONFIRM
		Sstar_hw_cancel_delayed_work(&priv->bss_loss_work, false);
		Sstar_hw_cancel_delayed_work(&priv->connection_loss_work,
					     false);
#endif
		Sstar_hw_cancel_delayed_work(&priv->link_id_gc_work, false);
		Sstar_hw_cancel_delayed_work(&priv->dhcp_retry_work, false);
		Sstar_del_timer_sync(&priv->mcast_timeout);
	}
...
int Sstar_add_interface(struct ieee80211_hw *dev, struct ieee80211_vif *vif)
{
	int ret;
	struct Sstar_common *hw_priv = dev->priv;
	struct Sstar_vif *priv;
	struct Sstar_vif **drv_priv = (void *)vif->drv_priv;
#ifndef P2P_MULTIVIF
	int i;

	if (atomic_read(&hw_priv->num_vifs) >= SSTAR_WIFI_MAX_VIFS)
		return -EOPNOTSUPP;
#endif
	if (Sstar_bh_is_term(hw_priv)) {
		return -EOPNOTSUPP;
	}
	Sstar_printk_sta("%s: vif->type %d, vif->p2p %d, addr %pM\n", __func__,
			 vif->type, vif->p2p, vif->addr);
	...
	*drv_priv = priv;
	...
	Sstar_hw_vif_write_lock(&hw_priv->vif_list_lock);
	if (atomic_read(&hw_priv->num_vifs) < SSTAR_WIFI_MAX_VIFS) {
#ifdef P2P_MULTIVIF
		...
		for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++)
			if (!memcmp(vif->addr, hw_priv->addresses[i].addr,
				    ETH_ALEN))
				break;
		if (i == SSTAR_WIFI_MAX_VIFS) {
			Sstar_hw_vif_write_unlock(&hw_priv->vif_list_lock);
			mutex_unlock(&hw_priv->conf_mutex);
			return -EINVAL;
		}
		priv->if_id = i;
...
#ifdef SSTAR_WIFI_QUEUE_LOCK_BUG
		{
			...
			*/
			...
			for (index = 0; index < IEEE80211_NUM_ACS; index++) {
				...
				Sstar_printk_sta("%s[%d],hw_queue[%d]=[%d]\n",
						 __func__, priv->if_id, index,
						 vif->hw_queue[index]);
			}
		}
#endif
		...
		*/
		...
		Sstar_reorder_func_init(priv);
#endif
		ret = Sstar_debug_init_priv(hw_priv, priv);
		WARN_ON(ret);
		Sstar_vif_setup_params(priv);
		WARN_ON(SSTAR_HW_VIF_GET(hw_priv->vif_list[priv->if_id]) !=
			NULL);
		SSTAR_HW_VIF_SET(hw_priv->vif_list[priv->if_id], vif);
	...
	*/
	...
	if (vif->type == NL80211_IFTYPE_MONITOR) {
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_SIMPLE_MONITOR;
		Sstar_printk_sta("%s:start monitor\n", __func__);
	}
	mutex_unlock(&hw_priv->conf_mutex);

	Sstar_vif_setup(priv);
	ret = WARN_ON(Sstar_setup_mac_pvif(priv));

	return ret;
}

void Sstar_remove_interface(struct ieee80211_hw *dev, struct ieee80211_vif *vif)
{
	struct Sstar_common *hw_priv = dev->priv;
	...
	struct Sstar_vif *tmp_priv;
	struct wsm_operational_mode mode = {
		.power_mode = wsm_power_mode_quiescent,
		.disableMoreFlagUsage = true,
	};

	Sstar_printk_sta(" !!! %s: enter, priv=%p type %d p2p %d addr %pM\n",
			 __func__, priv, vif->type, vif->p2p, vif->addr);

	if ((atomic_read(&priv->enabled) == 0) || (priv->hw_priv != hw_priv)) {
		Sstar_printk_err("[%s] has been removed,dont remove again\n",
	...
	*/
	...
	case SSTAR_APOLLO_JOIN_STATUS_IBSS:
		wsm_reset(hw_priv, &reset, priv->if_id);
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 10, 60))

		fallthrough;
#endif
	case SSTAR_APOLLO_JOIN_STATUS_STA:
		wsm_lock_tx(hw_priv);
		//if (Sstar_hw_priv_queue_work(hw_priv, &priv->unjoin_work) <= 0)
		//	wsm_unlock_tx(hw_priv);
		mutex_unlock(&hw_priv->conf_mutex);
		Sstar_unjoin_work(&priv->unjoin_work);
		mutex_lock(&hw_priv->conf_mutex);
		break;
	case SSTAR_APOLLO_JOIN_STATUS_AP:
		...
		Sstar_for_each_vif(hw_priv, tmp_priv, i)
		{
#ifdef P2P_MULTIVIF
			if ((i == (SSTAR_WIFI_MAX_VIFS - 1)) || !tmp_priv)
#else
			if (!tmp_priv)
#endif
				continue;
		...
		if (is_htcapie) {
			hw_priv->vif0_throttle =
				SSTAR_WIFI_HOST_VIF0_11N_THROTTLE;
			hw_priv->vif1_throttle =
				SSTAR_WIFI_HOST_VIF1_11N_THROTTLE;
			sta_printk(KERN_ERR "AP REMOVE HTCAP 11N %d\n",
				   hw_priv->vif0_throttle);
		} else {
			hw_priv->vif0_throttle =
				SSTAR_WIFI_HOST_VIF0_11BG_THROTTLE;
			hw_priv->vif1_throttle =
				SSTAR_WIFI_HOST_VIF1_11BG_THROTTLE;
			sta_printk(KERN_ERR "AP REMOVE 11BG %d\n",
				   hw_priv->vif0_throttle);
		}
		if (priv->if_id != 0) {
			struct Sstar_vif *priv_wlan;

			if ((priv_wlan = __ABwifi_hwpriv_to_vifpriv(
				     hw_priv, 0)) != NULL) {
				if (hw_priv->roc_wlan_pm.pmMode !=
				    WSM_PSM_ACTIVE)
					memcpy(&priv_wlan->powersave_mode,
					       &hw_priv->roc_wlan_pm,
					       sizeof(struct wsm_set_pm));
				Sstar_set_pm(priv_wlan,
					     &priv_wlan->powersave_mode);
			}
		}
		break;
	case SSTAR_APOLLO_JOIN_STATUS_MONITOR:
#if defined(CONFIG_SSTAR_STA_LISTEN) || defined(CONFIG_SSTAR_SUPPORT_P2P)
		Sstar_disable_listening(priv);
#endif
		break;
	case SSTAR_APOLLO_JOIN_STATUS_SIMPLE_MONITOR:
		Sstar_printk_sta("%s:SSTAR_APOLLO_JOIN_STATUS_SIMPLE_MONITOR\n",
				 __func__);
		Sstar_stop_monitor_mode(priv);
		break;

#ifdef CONFIG_SSTAR_STA_LISTEN
	case SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN:
		WARN_ON(1);
		break;
#endif
	default:
...
reset_priv:
#ifdef SSTAR_PKG_REORDER
	Sstar_reorder_func_reset(priv, 0xff);
#endif
	/* TODO:COMBO: Change Queue Module */
	sta_printk("%s:priv->if_id(%d)\n", __func__, priv->if_id);
	if (!__Sstar_flush(hw_priv, true, priv->if_id))
		wsm_unlock_tx(hw_priv);
	Sstar_hw_cancel_delayed_work(&priv->dhcp_retry_work, true);

	spin_lock_bh(&priv->dhcp_retry_spinlock);
	if (priv->dhcp_retry_skb) {
		Sstar_dev_kfree_skb(priv->dhcp_retry_skb);
		priv->dhcp_retry_skb = NULL;
	}
	spin_unlock_bh(&priv->dhcp_retry_spinlock);
#ifndef CONFIG_TX_NO_CONFIRM
	Sstar_hw_cancel_delayed_work(&priv->bss_loss_work, false);
	Sstar_hw_cancel_delayed_work(&priv->connection_loss_work, false);
#endif
	Sstar_hw_cancel_delayed_work(&priv->link_id_gc_work, false);
	Sstar_hw_cancel_delayed_work(&priv->join_timeout, false);
#if 0
	Sstar_hw_cancel_delayed_work(&priv->set_cts_work,false);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	Sstar_hw_cancel_delayed_work(&priv->pending_offchanneltx_work, false);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	Sstar_hw_cancel_queue_work(&priv->update_filtering_work, true);
#endif
	Sstar_hw_cancel_queue_work(&priv->set_beacon_wakeup_period_work, true);
	Sstar_del_timer_sync(&priv->mcast_timeout);
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_AP) {
		int link_id = 0;
		for (link_id = 0; link_id < SSTARWIFI_MAX_STA_IN_AP_MODE;
		     ++link_id) {
			if (priv->link_id_db[link_id].status !=
			    SSTAR_APOLLO_LINK_OFF) {
				Sstar_skb_queue_purge(
					&priv->link_id_db[link_id].rx_queue);
			}
		}
	}
	...
	if ((priv->if_id == 1) && (vif->p2p)) {
		Sstar_printk_sta("%s:reset p2p go save\n", __func__);
		...
		Sstar_printk_sta("%s:disable combination mode\n", __func__);
		atomic_set(&hw_priv->combination, 0);
	}
#endif
#endif
#ifdef SSTAR_WIFI_QUEUE_LOCK_BUG
	Sstar_clear_priv_queue_cap(priv);
#endif

	Sstar_hw_vif_write_lock(&hw_priv->vif_list_lock);
	Sstar_priv_vif_list_write_lock(&priv->vif_lock);
	SSTAR_HW_VIF_SET(hw_priv->vif_list[priv->if_id], NULL);
	hw_priv->if_id_slot &= (~BIT(priv->if_id));
	atomic_dec(&hw_priv->num_vifs);
	if (atomic_read(&hw_priv->num_vifs) == 0) {
		Sstar_free_keys(hw_priv);
		memset(hw_priv->mac_addr, 0, ETH_ALEN);
	}
	Sstar_priv_vif_list_write_unlock(&priv->vif_lock);
	Sstar_hw_vif_write_unlock(&hw_priv->vif_list_lock);
	priv->listening = false;
	...
	*/
	Sstar_flush_workqueue(hw_priv->workqueue);
	memset(priv, 0, sizeof(struct Sstar_vif));
}

int Sstar_change_interface(struct ieee80211_hw *dev, struct ieee80211_vif *vif,
			   enum nl80211_iftype new_type, bool p2p)
{
	int ret = 0;
	Sstar_printk_sta("Sstar_change_interface: type %d (%d), p2p %d (%d)\n",
			 new_type, vif->type, p2p, vif->p2p);
	if (new_type != vif->type || vif->p2p != p2p) {
		Sstar_remove_interface(dev, vif);
		vif->type = new_type;
		vif->p2p = p2p;
		ret = Sstar_add_interface(dev, vif);
	}
...
int Sstar_config(struct ieee80211_hw *dev, u32 changed)
{
	...
	u32 support_changed =
		...
		IEEE80211_CONF_CHANGE_POWER | IEEE80211_CONF_CHANGE_CHANNEL;

	/* TODO:COMBO: adjust to multi vif interface
	 * IEEE80211_CONF_CHANGE_IDLE is still handled per Sstar_vif*/
	int if_id = 0;
	struct Sstar_vif *priv;
	if (Sstar_bh_is_term(hw_priv)) {
		return 0;
	...
	if ((!!(support_changed & changed)) == 0) {
		Sstar_printk_debug("%s:changed(%x)\n", __func__, changed);
		return ret;
	}
	while (changed &
	       (IEEE80211_CONF_CHANGE_MONITOR | IEEE80211_CONF_CHANGE_IDLE)) {
		...
		if (i == SSTAR_WIFI_MAX_VIFS) {
			Sstar_printk_debug("%s: no priv is moniter\n",
					   __func__);
			break;
		}
		down(&hw_priv->scan.lock);
		...
		if (hw_priv->monitor_if_id != -1)
			Sstar_stop_monitor_mode(priv);
		if (hw_priv->monitor_if_id == -1) {
			Sstar_start_monitor_mode(priv, hw_priv->channel);
			mutex_unlock(&hw_priv->conf_mutex);
			up(&hw_priv->scan.lock);
			return ret;
		}
		mutex_unlock(&hw_priv->conf_mutex);
		up(&hw_priv->scan.lock);
		sta_printk("ignore IEEE80211_CONF_CHANGE_MONITOR (%d)"
	...
	if (changed & IEEE80211_CONF_CHANGE_POWER) {
		...
		WARN_ON(wsm_set_output_power(
			hw_priv, hw_priv->output_power * 10, if_id));
	}
	...
	if ((changed & IEEE80211_CONF_CHANGE_CHANNEL) &&
		...
		/* Earlier there was a call to __Sstar_flush().
...
#define CHTYPE_IS_20M(chtype)                                                  \
	((chtype == NL80211_CHAN_HT20) || (chtype == NL80211_CHAN_NO_HT))
		u8 new_type;
		enum nl80211_channel_type pre_hw_channel_type =
			hw_priv->channel_type;
		struct ieee80211_channel *pre_hw_channel = hw_priv->channel;
		struct ieee80211_sub_if_data *sdata;
		struct ieee80211_local *local = hw_to_local(dev);
		struct ieee80211_channel_state *chan_state =
			ieee80211_get_channel_state(local, NULL);
		...
		*/
		if ((chan_state->tmp_channel == conf->chan_conf->channel) &&
		    (chan_state->tmp_channel_type ==
		     conf->chan_conf->channel_type) &&
		    (chan_state->conf.offchannel == true)) {
			Sstar_printk_sta("%s:tmp_channel action\n", __func__);
			hw_priv->channel = conf->chan_conf->channel;
			if (chan_state->_oper_channel_type ==
			    conf->chan_conf->channel_type)
				hw_priv->channel_type =
					conf->chan_conf->channel_type;
		...
		*/
		if ((chan_state->oper_channel == conf->chan_conf->channel) &&
			...
			Sstar_printk_sta("%s:oper_channel action\n", __func__);
			hw_priv->channel = conf->chan_conf->channel;
			hw_priv->channel_type = conf->chan_conf->channel_type;
		}
		/*
		...
		*/
		if (hw_priv->channel != conf->chan_conf->channel) {
			Sstar_printk_sta("%s:channel maybe err,must updata\n",
					 __func__);
			hw_priv->channel = conf->chan_conf->channel;
			hw_priv->channel_type = conf->chan_conf->channel_type;
			break;
		...
		if ((pre_hw_channel == hw_priv->channel) &&
		    (pre_hw_channel_type == hw_priv->channel_type)) {
			Sstar_printk_sta("%s:channel not chage(%p)(%d)\n",
					 __func__, hw_priv->channel,
					 hw_priv->channel_type);
			break;
		}
		/*
		...
		*/
		list_for_each_entry (sdata, &local->interfaces, list) {
			...
			*/
			if (vif_chw(&sdata->vif) !=
			    conf->chan_conf->channel_type) {
				Sstar_printk_sta("[%s]:channel_type(%d),(%d)\n",
						 sdata->name,
						 vif_chw(&sdata->vif),
						 conf->chan_conf->channel_type);
				continue;
			...
			if (priv == NULL) {
				Sstar_printk_sta("%s:priv == NULL\n", __func__);
				continue;
			}
			if (atomic_read(&priv->enabled) == 0) {
				Sstar_printk_sta("%s:priv->enabled = 0\n",
						 __func__);
				continue;
			}
			...
			if (priv->join_status <=
			    SSTAR_APOLLO_JOIN_STATUS_MONITOR) {
				Sstar_printk_sta("%s:not join or not ap mode\n",
						 sdata->name);
				continue;
			}
			...
			if (sdata->vif.p2p && (queue_change_chwork == true)) {
				WARN_ON(ieee80211_chw_is_ht40(
					vif_chw(&sdata->vif)));
				Sstar_printk_sta("%s:p2p not suport 40M\n",
						 __func__);
				queue_change_chwork = false;
			}
			...
			if (queue_change_chwork == true) {
				...
				set_channtype.band = (hw_priv->channel->band ==
						      IEEE80211_BAND_5GHZ) ?
								   WSM_PHY_BAND_5G :
								   WSM_PHY_BAND_2_4G;
				set_channtype.flag = 0;
				set_channtype.channelNumber =
					channel_hw_value(hw_priv->channel);
				set_channtype.channelType =
					(u32)hw_priv->channel_type;
				Sstar_printk_sta(
					"%s:chatype(%d),channelNumber(%d)\n",
					__func__, set_channtype.channelType,
					set_channtype.channelNumber);
				if (wsm_set_chantype_func(hw_priv,
							  &set_channtype,
							  priv->if_id)) {
					/*
...
void Sstar_update_filtering(struct Sstar_vif *priv)
{
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_PASSIVE)
		return;
	else if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_MONITOR)
...
#endif
	if (ret)
		Sstar_printk_warn("%s: Update filtering failed: %d.,if(%d)\n",
				  __func__, ret, priv->if_id);
	return;
}
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
void Sstar_update_filtering_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, update_filtering_work);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		return;
	}
	//Sstar_update_filtering(priv);
}
#endif
void Sstar_set_beacon_wakeup_period_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv = container_of(work, struct Sstar_vif,
					      set_beacon_wakeup_period_work);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		//		wsm_unlock_tx(priv->hw_priv);
		return;
	}
	WARN_ON(wsm_set_beacon_wakeup_period(
		priv->hw_priv,
...
u64 Sstar_prepare_multicast(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    struct netdev_hw_addr_list *mc_list)
{
	static u8 broadcast_ipv6[ETH_ALEN] = { 0x33, 0x33, 0x00,
					       0x00, 0x00, 0x01 };
	static u8 broadcast_ipv4[ETH_ALEN] = { 0x01, 0x00, 0x5e,
					       0x00, 0x00, 0x01 };
...
#ifdef P2P_MULTIVIF
	if (priv->if_id == SSTAR_WIFI_GENERIC_IF_ID)
		return 0;
#endif
	...
	netdev_hw_addr_list_for_each (ha, mc_list) {
		...
		memcpy(&priv->multicast_filter.macAddress[count], ha->addr,
		       ETH_ALEN);
...
#endif
void Sstar_configure_filter(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    ...
			    unsigned int *total_flags, u64 multicast)
{
	...
	*total_flags &= FIF_PROMISC_IN_BSS | FIF_OTHER_BSS | FIF_FCSFAIL |
	...
	priv->rx_filter.promiscuous =
		(*total_flags & FIF_PROMISC_IN_BSS) ? 1 : 0;
	priv->rx_filter.bssid =
		(*total_flags & (FIF_OTHER_BSS | FIF_PROBE_REQ)) ? 1 : 0;
	priv->rx_filter.fcs = (*total_flags & FIF_FCSFAIL) ? 1 : 0;
	priv->bf_control.bcn_count =
		(*total_flags & (FIF_BCN_PRBRESP_PROMISC | FIF_PROMISC_IN_BSS |
				 FIF_PROBE_REQ)) ?
			      1 :
			      0;
#if 0
	if (priv->listening ^ listening) {
		...
		wsm_lock_tx(hw_priv);
		Sstar_update_listening(priv, listening);
		wsm_unlock_tx(hw_priv);
	}
#endif
	Sstar_update_filtering(priv);
		  ...
		  u16 queue, const struct ieee80211_tx_queue_params *params)
{
...
#ifdef P2P_MULTIVIF
	if (priv->if_id == SSTAR_WIFI_GENERIC_IF_ID) {
		sta_printk("%s:priv->if_id == SSTAR_WIFI_GENERIC_IF_ID\n",
			   __func__);
		return 0;
	}
#endif
	...
	if (queue < dev->queues) {
		...
		ret = wsm_set_tx_queue_params(
			hw_priv, &priv->tx_queue_params.params[queue], queue,
			priv->if_id);
		if (ret) {
		...
		WSM_EDCA_SET(&priv->edca, queue, params->aifs, params->cw_min,
			     params->cw_max, params->txop, 0xc8, params->uapsd);
		...
		if (priv->mode == NL80211_IFTYPE_STATION) {
			ret = Sstar_set_uapsd_param(priv, &priv->edca);
...
{
	int i;
	struct Sstar_common *priv = dev->priv;

	for (i = 0; i < dev->queues; ++i)
		Sstar_queue_get_stats(&priv->tx_queue[i], &stats[i]);
...
int Sstar_set_pm(struct Sstar_vif *priv, const struct wsm_set_pm *arg)
{
	...
	struct Sstar_vif *other_priv = NULL;
	...
	*/
	if (other_priv->join_status != SSTAR_APOLLO_JOIN_STATUS_STA) {
		sta_printk("Sstar_set_pm(1) set_self_ps(%d)\n", priv->if_id);
		if (other_priv->join_status == SSTAR_APOLLO_JOIN_STATUS_AP) {
			if (pm.pmMode == WSM_PSM_ACTIVE) {
				goto set_self_ps;
			}

			memcpy(&priv->powersave_mode, &pm,
			       sizeof(struct wsm_set_pm));
	...
	*/
	if (!(other_priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA &&
	      other_priv->bss_params.aid && other_priv->setbssparams_done)) {
		sta_printk("Sstar_set_pm(2) set_self_ps(%d)\n", priv->if_id);
...
#define CHANGE_FW_PS_MODE(change_priv, ps)                                     \
	do {                                                                   \
		if (change_priv->firmware_ps_mode.pmMode != ps) {              \
			...
			wsm_set_pm(change_priv->hw_priv,                       \
				   &change_priv->firmware_ps_mode,             \
				   change_priv->if_id);                        \
		...
		sta_printk("%s:self_%d(%d),other_%d(%d)\n", __func__,
			   priv->if_id, self_ps_mod, other_priv->if_id,
			   other_ps_mod);
		...
		if (self_ps_mod ^ other_ps_mod) {
			if (!self_ps_mod) {
				...
				*/
				...
				sta_printk(
					"%s:other_if_%d is in ps ,so self_if_%d change into ps\n",
					__func__, other_priv->if_id,
					priv->if_id);
				return 0;
			}
		} else {
			if (!self_ps_mod) {
				...
				if (((!(priv->join_status ==
					...
					CHANGE_FW_PS_MODE(
						other_priv,
						other_priv->powersave_mode
							.pmMode);
				else {
					sta_printk(
						"%s:all interface ip is not enable,so all interface keep alive\n",
						__func__);
					CHANGE_FW_PS_MODE(other_priv,
							  WSM_PSM_ACTIVE);
					CHANGE_FW_PS_MODE(priv, WSM_PSM_ACTIVE);
		  ...
		  struct ieee80211_key_conf *key)
{
...
#ifdef P2P_MULTIVIF
	WARN_ON(priv->if_id == SSTAR_WIFI_GENERIC_IF_ID);
#endif
	mutex_lock(&hw_priv->conf_mutex);
	...
	//sta_printk( "%s cmd<%d> cipher<%x> key<%s> keyidx %d pairwise %d \n",__func__,cmd,key->cipher,&key->key[0],key->keyidx,(key->flags & IEEE80211_KEY_FLAG_PAIRWISE));
	{
		...
		if (cmd == SET_KEY) {
			u8 *peer_addr = NULL;
			int pairwise =
				(key->flags & IEEE80211_KEY_FLAG_PAIRWISE) ? 1 :
										   0;
			...
			case WLAN_CIPHER_SUITE_WEP104:
				...
				if (pairwise) {
					...
					memcpy(wsm_key->wepPairwiseKey
						       .peerAddress,
					       peer_addr, ETH_ALEN);
					memcpy(wsm_key->wepPairwiseKey.keyData,
					       &key->key[0], key->keylen);
					wsm_key->wepPairwiseKey.keyLength =
						key->keylen;
				} else {
					wsm_key->type =
					...
					memcpy(wsm_key->wepGroupKey.keyData,
					       &key->key[0], key->keylen);
					wsm_key->wepGroupKey.keyLength =
						key->keylen;
					wsm_key->wepGroupKey.keyId =
						key->keyidx;
				}
				break;
			case WLAN_CIPHER_SUITE_TKIP:
				if (pairwise) {
					...
					memcpy(wsm_key->tkipPairwiseKey
						       .peerAddress,
					       peer_addr, ETH_ALEN);
					memcpy(wsm_key->tkipPairwiseKey
						       .tkipKeyData,
					...
					memset(wsm_key->tkipGroupKey
						       .rxSeqCounter,
					       0, 8);
					wsm_key->tkipGroupKey.keyId =
						key->keyidx;
				}
				break;
			...
			case WLAN_CIPHER_SUITE_AES_CMAC: /*add 11W support*/
			{
				...
				wsm_set_protected_mgmt_policy(
					hw_priv, &mgmt_policy, priv->if_id);
				sta_printk(
					"WLAN_CIPHER_SUITE_AES_CMAC,index(%d)\n",
					key->keyidx);
...
#ifdef CONFIG_SSTAR_APOLLO_WAPI_SUPPORT
			case WLAN_CIPHER_SUITE_SMS4:
				if (pairwise) {
					wsm_key->type =
						WSM_KEY_TYPE_WAPI_PAIRWISE;
					memcpy(wsm_key->wapiPairwiseKey
						       .peerAddress,
					       peer_addr, ETH_ALEN);
					memcpy(wsm_key->wapiPairwiseKey
						       .wapiKeyData,
					...
					memcpy(wsm_key->wapiGroupKey.micKeyData,
					       &key->key[16], 16);
					wsm_key->wapiGroupKey.keyId =
						key->keyidx;
				}
				break;
#endif /* CONFIG_SSTAR_APOLLO_WAPI_SUPPORT */
			default:
				WARN_ON(1);
				Sstar_free_key(hw_priv, idx);
				ret = -EOPNOTSUPP;
			...
			ret = WARN_ON(
				wsm_add_key(hw_priv, wsm_key, priv->if_id));
			if (!ret)
				key->hw_key_idx = idx;
			else
				Sstar_free_key(hw_priv, idx);
#ifdef CONFIG_SSTAR_LMAC_FILTER_IP_FRAME
			if (!ret &&
			    (pairwise ||
			     wsm_key->type == WSM_KEY_TYPE_WEP_DEFAULT) &&
			    (priv->filter4.enable & 0x2))
				Sstar_set_arpreply(dev, vif);
#ifdef IPV6_FILTERING
			if (!ret &&
			    (pairwise ||
			     wsm_key->type == WSM_KEY_TYPE_WEP_DEFAULT) &&
			    (priv->filter6.enable & 0x2))
				Sstar_set_na(dev, vif);
#endif /*IPV6_FILTERING*/
#endif
		} else if (cmd == DISABLE_KEY) {
			...
			sta_printk(
				KERN_ERR
				"DISABLE_KEY:idx(%d),key_type(%d),if_id(%d)\n",
...
void Sstar_wep_key_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, wep_key_work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	...
	WARN_ON(wsm_write_mib(hw_priv, WSM_MIB_ID_DOT11_WEP_DEFAULT_KEY_ID,
			      ...
			      priv->if_id));
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
	Sstar_queue_requeue(hw_priv, queue, hw_priv->pending_frame_id, true);
#else
	Sstar_queue_requeue(queue, hw_priv->pending_frame_id, true);
#endif
	wsm_unlock_tx(hw_priv);
}
int Sstar_tool_rts_threshold = 0;
int Sstar_set_rts_threshold(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    u32 value)
{
	struct Sstar_common *hw_priv = hw->priv;
	int ret;
...
#if 0
int Sstar_set_tx_rate_retry_cnt(struct ieee80211_hw *hw,
...
int Sstar_remove_seqlist(struct Sstar_common *hw_priv)
{
	struct Sstar_seq_bit_map *bitmap = NULL, *tmp = NULL;
...
{
	int i, ret;
	struct Sstar_vif *priv = __ABwifi_hwpriv_to_vifpriv(hw_priv, if_id);

	if (Sstar_bh_is_term(hw_priv)) {
		sta_printk("<wifi>Sstar_bh_is_term just drop txpacket\n");
		drop = true;
	}

	if (priv == NULL) {
		Sstar_printk_err("%s:if_id[%d] is already reset\n", __func__,
				 if_id);
		return -1;
	}
	...
	for (;;) {
		...
		if (drop) {
			for (i = 0; i < 4; ++i)
				Sstar_queue_clear(&hw_priv->tx_queue[i], if_id);
#ifdef SSTAR_SDIO_PATCH
			Sstar_remove_seqlist(hw_priv);
#endif
		} else {
			ret = Sstar_wait_event_timeout_stay_awake(
				hw_priv,
				hw_priv->tx_queue_stats.wait_link_id_empty,
				Sstar_queue_stats_is_empty(
		...
		if (!drop && unlikely(ret <= 0)) {
			...
			if (hw_priv->hw_bufs_used == 0) {
				for (i = 0; i < 4; ++i)
					Sstar_queue_clear(&hw_priv->tx_queue[i],
							  if_id);
			} else {
				u8 queueid = 0;
				Sstar_printk_err(
					"__Sstar_flush: ETIMEDOUT.....hw_priv->hw_bufs_used(%d),tx_lock(%d)\n",
					hw_priv->hw_bufs_used,
					atomic_read(&hw_priv->tx_lock));
				for (queueid = 0; queueid < 4; queueid++)
					Sstar_printk_err(
						"%s:queue[%d]-->pending(%d),vif%d_pending(%d),stats->vif_pend(%d)\n",
						__func__, queueid,
						hw_priv->tx_queue[queueid]
							.num_pending,
						if_id,
						hw_priv->tx_queue[queueid]
							.num_pending_vif[if_id],
						hw_priv->tx_queue_stats
							.num_queued[if_id]);
				mdelay(1000);
				ret = -ETIMEDOUT;
				wsm_unlock_tx(hw_priv);
		...
		wsm_vif_lock_tx(priv);
		if (unlikely(!Sstar_queue_stats_is_empty(
			    &hw_priv->tx_queue_stats, -1, if_id))) {
...
void Sstar_flush(struct ieee80211_hw *hw, struct ieee80211_vif *vif, bool drop)
{
	struct Sstar_common *hw_priv = hw->priv;
	struct Sstar_vif *priv = ABwifi_get_vif_from_ieee80211(vif);
	...
	if (!WARN_ON(__Sstar_flush(hw_priv, drop, priv->if_id)))
		wsm_unlock_tx(hw_priv);

	return;
}
#ifdef CONFIG_SSTAR_SUPPORT_P2P
int Sstar_remain_on_channel(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    ...
			    int duration, u64 cookie)
{
	...
	int if_id = priv->if_id;
#ifdef P2P_MULTIVIF
	struct Sstar_vif *p2p_priv = __ABwifi_hwpriv_to_vifpriv(hw_priv, 1);
#endif
	if (Sstar_bh_is_term(hw_priv)) {
		return -1;
	}
	...
	smp_mb();
#ifdef P2P_MULTIVIF
	Sstar_printk_mgmt("%s:p2p_priv(%x)\n", __func__,
			  (unsigned int)p2p_priv);
	if (p2p_priv)
		Sstar_printk_mgmt("%s:p2p_priv->join_status(%d)\n", __func__,
				  p2p_priv->join_status);
	if ((p2p_priv == NULL) ||
	    (p2p_priv->join_status <= SSTAR_APOLLO_JOIN_STATUS_IBSS))
#else
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_PASSIVE)
#endif
	{
		ret = WARN_ON(__Sstar_flush(hw_priv, false, if_id));
		if (!ret) {
			wsm_unlock_tx(hw_priv);
			Sstar_enable_listening(priv, chan);
			Sstar_printk_mgmt(
				"%s:remain channel ready,duration(%d),(%llx)\n",
				__func__, duration, cookie);
		}
		//hw_priv->roc_not_send = 0;
	} else {
		Sstar_printk_err("%s:roc_not_send\n", __func__);
		ret = -1;
		//ret = 0;
...
#ifdef P2P_MULTIVIF
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_MONITOR;
#endif
		ieee80211_ready_on_channel(hw, 100);
	} else {
		hw_priv->roc_if_id = -1;
		atomic_set(&hw_priv->remain_on_channel, 0);
		Sstar_printk_err("Sstar_remain_on_channel,err if_id(%d)\n",
				 priv->if_id);
		Sstar_scan_listenning_restart_delayed(priv);
	}
...
int Sstar_cancel_remain_on_channel(struct ieee80211_hw *hw)
{
	...
	if (atomic_read(&hw_priv->remain_on_channel))
		Sstar_hw_cancel_delayed_work(&hw_priv->rem_chan_timeout, true);
...
void Sstar_event_handler(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, event_handler);
	struct Sstar_vif *priv;
	struct Sstar_wsm_event *event;
	LIST_HEAD(list);
	...
	list_for_each_entry (event, &list, link) {
		...
		case WSM_EVENT_BSS_LOST: {
			...
			priv->bss_loss_status = SSTAR_APOLLO_BSS_LOSS_CHECKING;
			spin_unlock_bh(&priv->bss_loss_lock);
			sta_printk("[CQM] BSS lost.");
			sta_printk("[CQM] BSS lost.\n");
			Sstar_hw_cancel_delayed_work(&priv->bss_loss_work,
						     true);
			Sstar_hw_cancel_delayed_work(
				&priv->connection_loss_work, true);
			Sstar_hw_cancel_delayed_work(&priv->dhcp_retry_work,
						     true);
			if (!down_trylock(&hw_priv->scan.lock)) {
				up(&hw_priv->scan.lock);
				priv->delayed_link_loss = 0;
				Sstar_hw_priv_queue_delayed_work(
					hw_priv, &priv->bss_loss_work, 0);
			} else {
				...
				Sstar_hw_priv_queue_delayed_work(
					hw_priv, &priv->bss_loss_work, 10 * HZ);
			}
#else
		...
		case WSM_EVENT_BSS_REGAINED: {
			...
			priv->delayed_link_loss = 0;
			spin_lock_bh(&priv->bss_loss_lock);
			priv->bss_loss_status = SSTAR_APOLLO_BSS_LOSS_NONE;
			spin_unlock_bh(&priv->bss_loss_lock);
			Sstar_hw_cancel_delayed_work(&priv->bss_loss_work,
						     true);
			Sstar_hw_cancel_delayed_work(
...
#ifdef CONFIG_SSTAR_SUPPORT_RCPI_RSSI
		...
		case WSM_EVENT_INACTIVITY: {
			...
			for (i = 0; i < SSTARWIFI_MAX_STA_IN_AP_MODE + 1; i++) {
				...
				memcpy(deauth->sa,
				       entry->mac /*priv->link_id_db[i].mac*/,
				       ETH_ALEN);
				memcpy(deauth->bssid, priv->vif->addr,
				       ETH_ALEN);
		...
		case WSM_EVENT_PS_MODE_ERROR: {
			...
			if (!priv->uapsd_info.uapsdFlags &&
				...
				ret = Sstar_set_pm(priv, &priv->powersave_mode);
				if (ret)
					priv->powersave_mode = pm;

#ifdef CONFIG_SSTAR_STA_DYNAMIC_PS
...
#ifndef CONFIG_TX_NO_CONFIRM
void Sstar_bss_loss_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, bss_loss_work.work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	...
	spin_lock_bh(&priv->bss_loss_lock);
	if (priv->bss_loss_status == SSTAR_APOLLO_BSS_LOSS_CHECKING) {
		//		spin_unlock(&priv->bss_loss_lock);
		priv->bss_loss_status = SSTAR_APOLLO_BSS_LOSS_CONFIRMED;
	} else if (priv->bss_loss_status == SSTAR_APOLLO_BSS_LOSS_CONFIRMING) {
		priv->bss_loss_status = SSTAR_APOLLO_BSS_LOSS_NONE;
		spin_unlock_bh(&priv->bss_loss_lock);
		return;
	}
...
report:
	...
	Sstar_hw_cancel_delayed_work(&priv->connection_loss_work, true);
	Sstar_hw_priv_queue_delayed_work(hw_priv, &priv->connection_loss_work,
					 timeout * HZ / 10);
...
void Sstar_connection_loss_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, connection_loss_work.work);
...
void Sstar_tx_failure_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, tx_failure_work);
...
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
/**
 * Sstar_device_power_calc- Device power calculation
 ...
 *
 */
void Sstar_device_power_calc(struct Sstar_common *hw_priv, s16 max_output_power,
			     s16 fe_cor, u32 band)
{
	s16 power_calc;
...
*/
static int Sstar_parse_SDD_file(struct Sstar_common *hw_priv)
{
...
#define FIELD_OFFSET(type, field) ((u8 *)&((type *)0)->field - (u8 *)0)
	...
	pElement = (struct Sstar_sdd *)sdd_data;

	pElement = (struct Sstar_sdd *)((u8 *)pElement +
					FIELD_OFFSET(struct Sstar_sdd, data) +
					pElement->length);

	parsedLength +=
		(FIELD_OFFSET(struct Sstar_sdd, data) + pElement->length);
	...
	while (parsedLength <= hw_priv->sdd->size) {
		...
		case SDD_FE_COR_5G_ELT_ID: {
			...
			fe_cor_5G =
				MIN(fe_cor_5G, *((s16 *)(pElement->data + 6)),
				    *((s16 *)(pElement->data + 8)));
		} break;
#endif
		...
		pElement = (struct Sstar_sdd *)((u8 *)pElement +
						FIELD_OFFSET(struct Sstar_sdd,
							     data) +
						pElement->length);
		parsedLength += (FIELD_OFFSET(struct Sstar_sdd, data) +
				 pElement->length);
	}
	...
	Sstar_device_power_calc(hw_priv, max_output_power_2G, fe_cor_2G,
				IEEE80211_BAND_2GHZ);
	/* Max/Min Power Calculation for 5G */
	Sstar_device_power_calc(hw_priv, max_output_power_5G, fe_cor_5G,
				IEEE80211_BAND_5GHZ);
	...
	if (!hw_priv->output_power)
		hw_priv->output_power =
			hw_priv->txPowerRange[IEEE80211_BAND_2GHZ]
				.max_power_level;
...
int Sstar_setup_mac(struct Sstar_common *hw_priv)
{
	...
	if (0) {
		...
		sta_printk(
			"setup mac addr start :[ %02x,%02x,%02x,%02x,%02x,%02x ] \n",
			hw_priv->mac_addr[0], hw_priv->mac_addr[1],
			hw_priv->mac_addr[2], hw_priv->mac_addr[3],
			hw_priv->mac_addr[4], hw_priv->mac_addr[5]);
...
//#define CONFIG_SUPPORT_SDD
		...
		sta_printk(
			"setup mac addr end :[ %02x,%02x,%02x,%02x,%02x,%02x ] \n",
			hw_priv->mac_addr[0], hw_priv->mac_addr[1],
			hw_priv->mac_addr[2], hw_priv->mac_addr[3],
			hw_priv->mac_addr[4], hw_priv->mac_addr[5]);
		/* Parse SDD file for PTA element */
		Sstar_parse_SDD_file(hw_priv);
	} else
#endif
	{
		//const char *sdd_path = sdd;
...
//#define CONFIG_SUPPORT_SDD
		...
		cfg.dpdData_size = 0;
#endif
		sta_printk("Sstar_setup_mac:addr(%pm)\n",
			   &hw_priv->mac_addr[0]);
		ret |= WARN_ON(wsm_configuration(hw_priv, &cfg, 0));
	}
...
#ifdef CONFIG_SSTAR_SUPPORT_P2P
void Sstar_pending_offchanneltx_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv = container_of(work, struct Sstar_vif,
...
void Sstar_offchannel_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, offchannel_work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	u8 queueId = Sstar_queue_get_queue_id(hw_priv->pending_frame_id);
	struct Sstar_queue *queue = &hw_priv->tx_queue[queueId];
	int listening_on = -1;
	...
	if (unlikely(down_trylock(&hw_priv->scan.lock))) {
		int ret;
		sta_printk(KERN_ERR "Sstar_offchannel_work***** drop frame\n");
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
		ret = Sstar_queue_remove(hw_priv, queue,
					 hw_priv->pending_frame_id);
#else
		ret = Sstar_queue_remove(queue, hw_priv->pending_frame_id);
#endif
		if (ret)
			sta_printk(KERN_ERR "Sstar_offchannel_work: "
	...
	if (likely(!priv->join_status)) {
		...
		listening_on = Sstar_enable_listening(priv, hw_priv->channel);
		/* Sstar_update_filtering(priv); */
	}
...
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
		Sstar_queue_requeue(hw_priv, queue, hw_priv->pending_frame_id,
				    false);
#else
		Sstar_queue_requeue(queue, hw_priv->pending_frame_id, false);
#endif
	if (listening_on == 0) {
		Sstar_hw_priv_queue_delayed_work(
			hw_priv, &priv->pending_offchanneltx_work,
			msecs_to_jiffies(5));
	} else {
		sta_printk(
			KERN_ERR
			"Sstar_offchannel_work if_id(%d),cannot in listen state\n",
			priv->if_id);
	}
	mutex_unlock(&hw_priv->conf_mutex);
	if (listening_on) {
...
void Sstar_restart_join_bss(struct Sstar_vif *priv, struct cfg80211_bss *bss)
{
	...
	int ret = 0;
	struct wsm_protected_mgmt_policy mgmt_policy;
	frame.skb = ieee80211_probereq_get(
		hw_priv->hw, priv->vif, NULL, 0,
		vif_to_sdata(priv->vif)->last_scan_ie,
		vif_to_sdata(priv->vif)->last_scan_ie_len, NULL);
	if (!frame.skb)
		return;
	...
	if (ret)
		return;
	ssidie = ieee80211_bss_get_ie(bss, SSTAR_WLAN_EID_SSID);
	dtimie = ieee80211_bss_get_ie(bss, SSTAR_WLAN_EID_TIM);
	if (dtimie)
	...
	if (atomic_read(&priv->enabled) == 1) {
		...
		memcpy(&priv->join_bssid[0], bss->bssid,
...
#endif
			join.basicRateSet =
				Sstar_rate_mask_to_wsm(hw_priv, 0xFF0);
		}
		...
		wsm_set_protected_mgmt_policy(hw_priv, &mgmt_policy,
					      priv->if_id);
		Sstar_printk_sta("%s:ch(%d),chtype(%d),if_id(%d)\n", __func__,
				 join.channelNumber, join.channel_type,
				 priv->if_id);
...
#ifdef SSTAR_P2P_CHANGE
		if (priv->if_id == 0) {
			Sstar_printk_sta("%s:enable combination mode\n",
					 __func__);
...
void Sstar_join_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, join_work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	u8 queueId = Sstar_queue_get_queue_id(hw_priv->pending_frame_id);
	struct Sstar_queue *queue = &hw_priv->tx_queue[queueId];
	...
	BUG_ON(queueId >= 4);
	if (Sstar_queue_get_skb(queue, hw_priv->pending_frame_id, &skb,
				&txpriv)) {
		wsm_unlock_tx(hw_priv);
		return;
	}
	...
	Sstar_hw_cancel_delayed_work(&priv->join_timeout, true);

	bss = ieee80211_Sstar_get_authen_bss(priv->vif, hw_priv->channel, bssid,
					     NULL, 0);
	if (!bss) {
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
		Sstar_queue_remove(hw_priv, queue, hw_priv->pending_frame_id);
	...
	if (atomic_read(&priv->enabled) == 1) {
		...
		Sstar_hw_priv_queue_delayed_work(hw_priv, &priv->join_timeout,
						 3 * HZ);
#ifdef CONFIG_PM
		...
		*/
		...
		if (wsm_join(hw_priv, &join, priv->if_id)) {
			memset(&priv->join_bssid[0], 0,
			       sizeof(priv->join_bssid));
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
			Sstar_queue_remove(hw_priv, queue,
					   hw_priv->pending_frame_id);
#else
			Sstar_queue_remove(queue, hw_priv->pending_frame_id);
#endif /*CONFIG_SSTAR_APOLLO_TESTMODE*/
			...
			/* Upload keys */
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
			Sstar_queue_requeue(hw_priv, queue,
					    hw_priv->pending_frame_id, true);
#else
			Sstar_queue_requeue(queue, hw_priv->pending_frame_id,
					    true);
#endif
			priv->join_status = SSTAR_APOLLO_JOIN_STATUS_STA;
		...
		Sstar_printk_sta("%s:priv is not enable\n", __func__);
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
		Sstar_queue_remove(hw_priv, queue, hw_priv->pending_frame_id);
#else
		Sstar_queue_remove(queue, hw_priv->pending_frame_id);
	...
	mutex_unlock(&hw_priv->conf_mutex);
	ieee80211_Sstar_put_authen_bss(priv->vif, bss);
	wsm_unlock_tx(hw_priv);
}

void Sstar_join_timeout(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, join_timeout.work);
	if (Sstar_bh_is_term(priv->hw_priv)) {
		return;
	}
...
void Sstar_unjoin_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, unjoin_work);
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	...
	if (priv->join_status &&
	    priv->join_status > SSTAR_APOLLO_JOIN_STATUS_STA) {
		Sstar_printk_warn("Unexpected: join status: %d\n",
				  priv->join_status);
		BUG_ON(1);
	}
	if (priv->join_status) {
		Sstar_printk_sta("%s:join_status(%d),if_id(%d)\n", __func__,
				 priv->join_status, priv->if_id);
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
		Sstar_hw_cancel_queue_work(&priv->update_filtering_work, true);
#endif
		Sstar_hw_cancel_queue_work(&priv->set_beacon_wakeup_period_work,
					   true);
		memset(&priv->join_bssid[0], 0, sizeof(priv->join_bssid));
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_PASSIVE;

#ifdef SSTAR_PKG_REORDER
		Sstar_reorder_func_reset(priv,
					 SSTAR_APOLLO_LINK_ID_UNMAPPED - 1);
#endif
		/* Unjoin is a reset. */
		wsm_flush_tx(hw_priv);
		...
		if (priv->vif->p2p == false) {
			sta_printk(
				KERN_ERR
				"Sstar_unjoin_work:chantype_change_work+++++\n");
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
			mutex_unlock(&hw_priv->conf_mutex);
			if (Sstar_hw_cancel_delayed_work(
				    &priv->chantype_change_work, false))
				Sstar_channel_type_change_work(
					&priv->chantype_change_work.work);
			if (Sstar_work_pending(&hw_priv->get_cca_work)) {
				Sstar_hw_cancel_queue_work(
					&hw_priv->get_cca_work, true);
				Sstar_get_cca_work(&hw_priv->get_cca_work);
			}
			mutex_lock(&hw_priv->conf_mutex);
#endif
		...
		WARN_ON(wsm_set_operational_mode(hw_priv, &mode, priv->if_id));
		WARN_ON(wsm_set_output_power(
			hw_priv, hw_priv->output_power * 10, priv->if_id));
		priv->tmpframe_probereq_set = false;
		priv->join_dtim_period = 0;
		priv->cipherType = 0;
		WARN_ON(Sstar_setup_mac_pvif(priv));
		Sstar_free_event_queue(hw_priv);
		Sstar_hw_cancel_queue_work(&hw_priv->event_handler, true);
#ifndef CONFIG_TX_NO_CONFIRM
		Sstar_hw_cancel_delayed_work(&priv->connection_loss_work, true);
#endif
		WARN_ON(wsm_set_block_ack_policy(
			hw_priv, 0, hw_priv->ba_tid_rx_mask, priv->if_id));
			   ...
			   struct ieee80211_channel *chan)
{
...
#endif
		.band = (chan->band == IEEE80211_BAND_5GHZ) ? WSM_PHY_BAND_5G :
								    WSM_PHY_BAND_2_4G,
	...
	if ((priv->vif->type == NL80211_IFTYPE_P2P_CLIENT) ||
	    (priv->vif->type == NL80211_IFTYPE_P2P_GO)) {
		start.mode |= BIT(6);
		Sstar_printk_err("[P2P MODE] SET BIT(6)");
	}
#ifdef P2P_MULTIVIF
	if (priv->if_id != 2) {
		WARN_ON(priv->join_status > SSTAR_APOLLO_JOIN_STATUS_MONITOR);
...
#endif
	Sstar_printk_sta(
		"if(%d):Sstar_enable_listening:channelNumber(%d),channel_type(%d)\n",
		priv->if_id, start.channelNumber, start.channel_type);
	//	#ifdef P2P_MULTIVIF
	return wsm_start(hw_priv, &start, SSTAR_WIFI_GENERIC_IF_ID);
	//	#else
...
int Sstar_disable_listening(struct Sstar_vif *priv)
{
	...
	if (1
		...
		return 0;
	sta_printk("Sstar_disable_listening++++++++\n");

	//#ifdef P2P_MULTIVIF
	ret = wsm_reset(priv->hw_priv, &reset, SSTAR_WIFI_GENERIC_IF_ID);
	Sstar_printk_sta("%s:Sstar_disable_listening(%d)\n", __func__,
			 priv->if_id);
	//#else
...
#endif
#ifdef CONFIG_SSTAR_STA_LISTEN
static int Sstar_sta_enable_listen(struct Sstar_vif *priv)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	int ret = 0;
	...
	if ((priv->join_status != SSTAR_APOLLO_JOIN_STATUS_PASSIVE) &&
	    (priv->join_status != SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)) {
		if (hw_priv->sta_listen_if_save == -1)
			hw_priv->sta_listen_if_save = priv->if_id;
		Sstar_printk_err("%s:join status err(%d)\n", __func__,
				 priv->join_status);
		return -1;
	}
	...
	if (chan == NULL) {
		...
		priv->join_status = SSTAR_APOLLO_JOIN_STATUS_PASSIVE;
		Sstar_printk_err("%s:chan is NULL\n", __func__);
		return -1;
	}

	ret = WARN_ON(__Sstar_flush(hw_priv, false, priv->if_id));
	if (!ret) {
		wsm_unlock_tx(hw_priv);
		Sstar_enable_listening(priv, chan);
	}
	...
	if (ret == 0) {
		...
		if (hw_priv->channel == NULL)
			hw_priv->channel = chan;
		Sstar_printk_err("%s:vif[%d] in listen\n", __func__,
				 priv->if_id);
	}
...
static int Sstar_sta_disable_listen(struct Sstar_vif *priv)
{
	...
	Sstar_printk_err("%s:(%d)(%d)\n", __func__, priv->if_id,
			 priv->join_status);
	WARN_ON(priv->join_status != SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN);
	...
	smp_mb();
	priv->join_status = SSTAR_APOLLO_JOIN_STATUS_PASSIVE;
	Sstar_printk_err("%s:vif[%d] exit listen\n", __func__,
			 hw_priv->sta_listen_if);
	hw_priv->sta_listen_if = -1;
	/*make sure all listen pkg processed*/
	synchronize_rcu();
	return ret;
}
int Sstar_sta_triger_listen(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			    struct ieee80211_channel *chan)
{
	struct Sstar_common *hw_priv = hw->priv;
	struct Sstar_vif *priv = ABwifi_get_vif_from_ieee80211(vif);
	...
	if (hw_priv->sta_listen_if != -1) {
		struct Sstar_vif *listen_priv = __ABwifi_hwpriv_to_vifpriv(
			hw_priv, hw_priv->sta_listen_if);
		if (listen_priv)
			Sstar_sta_disable_listen(listen_priv);
	}
	hw_priv->sta_listen_if_save = -1;
	hw_priv->sta_listen_if = -1;
	smp_mb();
	ret = Sstar_sta_enable_listen(priv);
	mutex_unlock(&hw_priv->conf_mutex);
...
int Sstar_sta_stop_listen(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	if (Sstar_bh_is_term(hw_priv)) {
		Sstar_printk_err("%s:bh is disable\n", __func__);
		return -EOPNOTSUPP;
	}
...
#ifdef CONFIG_SSTAR_BA_STATUS
void Sstar_ba_work(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, ba_work);
	//u8 tx_ba_tid_mask;
	...
	WARN_ON(wsm_set_block_ack_policy(hw_priv, hw_priv->ba_tid_tx_mask,
					 hw_priv->ba_tid_rx_mask,
					 -1)); /*TODO:COMBO*/
...
{
	bool ba_ena;
	struct Sstar_common *hw_priv = (struct Sstar_common *)arg;

	spin_lock_bh(&hw_priv->ba_lock);
	Sstar_debug_ba(hw_priv, hw_priv->ba_cnt, hw_priv->ba_acc,
	...
	if (hw_priv->ba_cnt >= SSTAR_APOLLO_BLOCK_ACK_CNT &&
	    (hw_priv->ba_acc / hw_priv->ba_cnt >= SSTAR_APOLLO_BLOCK_ACK_THLD ||
	     (hw_priv->ba_cnt_rx >= SSTAR_APOLLO_BLOCK_ACK_CNT &&
	      hw_priv->ba_acc_rx / hw_priv->ba_cnt_rx >=
		      SSTAR_APOLLO_BLOCK_ACK_THLD)))
		ba_ena = true;
	else
		ba_ena = false;
	...
	if (ba_ena != hw_priv->ba_ena) {
		if (ba_ena ||
		    ++hw_priv->ba_hist >= SSTAR_APOLLO_BLOCK_ACK_HIST) {
			hw_priv->ba_ena = ba_ena;
			hw_priv->ba_hist = 0;
#if 0
			sta_printk( "[STA] %s block ACK:\n",
				ba_ena ? "enable" : "disable");
			Sstar_queue_work(hw_priv->workqueue, &hw_priv->ba_work);
#endif
		}
...
void Sstar_vif_setup_params(struct Sstar_vif *priv)
{
	...
	SSTAR_INIT_WORK(&priv->join_work, Sstar_join_work);
	SSTAR_INIT_DELAYED_WORK(&priv->join_timeout, Sstar_join_timeout);
	SSTAR_INIT_WORK(&priv->unjoin_work, Sstar_unjoin_work);
	SSTAR_INIT_WORK(&priv->wep_key_work, Sstar_wep_key_work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	SSTAR_INIT_WORK(&priv->offchannel_work, Sstar_offchannel_work);
	SSTAR_INIT_DELAYED_WORK(&priv->pending_offchanneltx_work,
				Sstar_pending_offchanneltx_work);
#endif
#ifndef CONFIG_TX_NO_CONFIRM
	SSTAR_INIT_DELAYED_WORK(&priv->bss_loss_work, Sstar_bss_loss_work);
	SSTAR_INIT_DELAYED_WORK(&priv->connection_loss_work,
				Sstar_connection_loss_work);
	spin_lock_init(&priv->bss_loss_lock);
	SSTAR_INIT_WORK(&priv->tx_failure_work, Sstar_tx_failure_work);
#endif
	spin_lock_init(&priv->ps_state_lock);
	//	SSTAR_INIT_DELAYED_WORK(&priv->set_cts_work, Sstar_set_cts_work);
	SSTAR_INIT_WORK(&priv->set_tim_work, Sstar_set_tim_work);
	SSTAR_INIT_WORK(&priv->multicast_start_work,
			Sstar_multicast_start_work);
	SSTAR_INIT_WORK(&priv->multicast_stop_work, Sstar_multicast_stop_work);
	SSTAR_INIT_WORK(&priv->link_id_work, Sstar_link_id_work);
	SSTAR_INIT_DELAYED_WORK(&priv->link_id_gc_work, Sstar_link_id_gc_work);
#if 0
	SSTAR_INIT_WORK(&priv->linkid_reset_work, Sstar_link_id_reset);
#endif
#ifdef CONFIG_SSTAR_MAC80211_NO_USE
	SSTAR_INIT_WORK(&priv->update_filtering_work,
			Sstar_update_filtering_work);
#endif
	SSTAR_INIT_WORK(&priv->set_beacon_wakeup_period_work,
			Sstar_set_beacon_wakeup_period_work);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 3, 0))
	SSTAR_INIT_WORK(&priv->ht_info_update_work, Sstar_ht_info_update_work);
#endif
#ifdef SSTAR_SUPPORT_WIDTH_40M
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
	SSTAR_INIT_DELAYED_WORK(&priv->chantype_change_work,
				Sstar_channel_type_change_work);
#endif
#endif
	Sstar_init_timer(&priv->mcast_timeout);
	//DHCP
	spin_lock_init(&priv->dhcp_retry_spinlock);
	SSTAR_INIT_DELAYED_WORK(&priv->dhcp_retry_work, Sstar_dhcp_retry_work);
	spin_lock_bh(&priv->dhcp_retry_spinlock);
	priv->dhcp_retry_skb = NULL;
	spin_unlock_bh(&priv->dhcp_retry_spinlock);
	priv->mcast_timeout.data = (unsigned long)priv;
	priv->mcast_timeout.function = Sstar_mcast_timeout;
	priv->setbssparams_done = false;
...
#endif
		/* default EDCA */
		WSM_EDCA_SET(&priv->edca, 0, 0x0002, 0x0003, 0x0007, 47, 0xc8,
			     false);
		WSM_EDCA_SET(&priv->edca, 1, 0x0002, 0x0007, 0x000f, 94, 0xc8,
			     false);
		WSM_EDCA_SET(&priv->edca, 2, 0x0003, 0x000f, 0x03ff, 0, 0xc8,
			     false);
		WSM_EDCA_SET(&priv->edca, 3, 0x0007, 0x000f, 0x03ff, 0, 0xc8,
			     false);
		ret = wsm_set_edca_params(hw_priv, &priv->edca, priv->if_id);
		if (WARN_ON(ret))
			goto out;
...
#ifdef CONFIG_SSTAR_SUPPORT_P2P
void Sstar_rem_chan_timeout(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, rem_chan_timeout.work);
	int ret, if_id;
	struct Sstar_vif *priv;
	...
	Sstar_printk_mgmt("%s:roc_cookie(%llx)\n", __func__,
			  hw_priv->roc_cookie);
...
#ifdef SSTAR_SUPPORT_WOW
/**
 * Sstar_set_macaddrfilter -called when tesmode command
 ...
 */
int Sstar_set_macaddrfilter(struct Sstar_common *hw_priv,
			    struct Sstar_vif *priv, u8 *data)
{
	...
	mac_addr_filter = Sstar_kzalloc(macaddrfiltersize, GFP_KERNEL);
	if (!mac_addr_filter) {
		ret = -ENOMEM;
		goto exit_p;
	}
...
#if 0
/**
 * Sstar_set_multicastaddrfilter -called when tesmode command
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int Sstar_set_multicastfilter(struct Sstar_common *hw_priv, struct Sstar_vif *priv, u8 *data)
{
	u8 i = 0;
	int ret = 0;
...
#ifdef IPV6_FILTERING
/**
 * Sstar_set_ipv6addrfilter -called when tesmode command
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int Sstar_set_ipv6addrfilter(struct ieee80211_hw *hw, u8 *data,
				    int if_id)
{
	...
	ipv6_filter = Sstar_kzalloc(ipaddrfiltersize, GFP_KERNEL);
	if (!ipv6_filter) {
		ret = -ENOMEM;
		goto exit_p;
	}
...
/**
 * Sstar_set_data_filter -configure data filter in device
*
 * @hw: the hardware
 * @vif: vif
 * @data: incoming data
 ...
 *
 */
void Sstar_set_data_filter(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			   void *data, int len)
{
	int ret = 0;
	int filter_id;
	...
	filter_id = *((enum Sstar_data_filterid *)data);

	switch (filter_id) {
#ifdef IPV6_FILTERING
	case IPV6ADDR_FILTER_ID:
		ret = Sstar_set_ipv6addrfilter(hw, &((u8 *)data)[4],
					       priv->if_id);
		break;
#endif /*IPV6_FILTERING*/
	default:
...
/**
 * Sstar_set_arpreply -called for creating and
...
int Sstar_set_arpreply(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	template_frame = Sstar_kzalloc(MAX_ARP_REPLY_TEMPLATE_SIZE, GFP_ATOMIC);
	if (!template_frame) {
	...
	*(u16 *)(++snaphdr) = cpu_to_be16(ETH_P_ARP);
	...
	ret = WARN_ON(wsm_write_mib(
		hw_priv, WSM_MIB_ID_TEMPLATE_FRAME, template_frame,
		(framehdrlen + framebdylen + 4), priv->if_id));
	Sstar_kfree(template_frame);
exit_p:
	return ret;
}
#endif
...
#ifdef ROAM_OFFLOAD
/**
 * Sstar_testmode_event -send asynchronous event
 * to userspace
 *
 * @wiphy: the wiphy
 * @msg_id: STE msg ID
			 ...
			 const void *data, int len, gfp_t gfp)
{
	struct sk_buff *skb = cfg80211_testmode_alloc_event_skb(
		wiphy, nla_total_size(len + sizeof(msg_id)), gfp);
...
#ifdef IPV6_FILTERING
/**
 * Sstar_set_na -called for creating and
 ...
 * Returns: 0 on success or non zero value on failure
 */
int Sstar_set_na(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	struct Sstar_vif *priv = ABwifi_get_vif_from_ieee80211(vif);
	struct Sstar_common *hw_priv = (struct Sstar_common *)hw->priv;
	u32 framehdrlen, encrypthdr, encrypttailsize, framebdylen = 0;
	...
	template_frame = Sstar_kzalloc(MAX_NEIGHBOR_ADVERTISEMENT_TEMPLATE_SIZE,
				       GFP_ATOMIC);
	if (!template_frame) {
		sta_printk("[STA] Template frame memory failed\n");
		ret = -ENOMEM;
	...
	*(u16 *)(++snaphdr) = cpu_to_be16(ETH_P_IPV6);
	...
	ipv6_hdr->version = 6;
	ipv6_hdr->priority = 0;
	ipv6_hdr->payload_len = cpu_to_be16(
		32); /* ??? check the be or le ??? whether to use cpu_to_be16(32)*/
	ipv6_hdr->nexthdr = 58;
...
#endif /*IPV6_FILTERING*/
#endif
int Sstar_tool_shortGi = 0;
int Sstar_tool_use_short_slot = 0;
int Sstar_tool_use_short_preamble = 0;
int Sstar_tool_use_cts_prot = 0;

#if defined(CONFIG_NL80211_TESTMODE) && defined(CONFIG_SSTAR_TEST_TOOL)

/**
 * Sstar_tesmode_reply -called inside a testmode command
 ...
 * Returns: 0 on success or non zero value on failure
 */
int Sstar_tesmode_reply(struct wiphy *wiphy, const void *data, int len)
{
	...
	if (local->hw.vendcmd_nl80211 == 0) {
		...
		struct sk_buff *skb = cfg80211_testmode_alloc_reply_skb(
			wiphy, nla_total_size(len));

		if (!skb) {
			return -ENOMEM;
		}
		ret = nla_put(skb, SSTAR_TM_MSG_DATA, len, data);
		if (ret) {
			Sstar_kfree_skb(skb);
			return ret;
		}
		...
		if (len > 0) {
			...
			memcpy((u8 *)local->hw.vendreturn.respbuff, (u8 *)data,
			       len);
		}
		ret = len;
	}
...
/**
 * Sstar_start_stop_tsm - starts/stops collecting TSM
 *
 * @hw: the hardware
 * @data: data frame
 *
 * Returns: 0 on success or non zero value on failure
 */
int Sstar_start_stop_tsm(struct ieee80211_hw *hw, void *data)
{
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE

	struct Sstar_msg_start_stop_tsm *start_stop_tsm =
		(struct Sstar_msg_start_stop_tsm *)data;
	struct Sstar_common *hw_priv = hw->priv;
	...
	sta_printk("[STA] %s: start : %u: up : %u", __func__,
		   hw_priv->start_stop_tsm.start, hw_priv->start_stop_tsm.up);
	hw_priv->tsm_info.ac = Sstar_1d_to_ac[start_stop_tsm->up];
...
/**
 * Sstar_get_tsm_params - Retrieves TSM parameters
 *
 * @hw: the hardware
 *
 * Returns: TSM parameters collected
 */
int Sstar_get_tsm_params(struct ieee80211_hw *hw)
{
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
	...
	sta_printk("\n [STA] : "
		   ...
		   hw_priv->tsm_stats.bin00, hw_priv->tsm_stats.bin01,
		   hw_priv->tsm_stats.bin02, hw_priv->tsm_stats.bin03,
		   hw_priv->tsm_stats.bin04, hw_priv->tsm_stats.bin05,
		   hw_priv->tsm_stats.bin0, hw_priv->tsm_stats.bin1,
		   hw_priv->tsm_stats.bin2, hw_priv->tsm_stats.bin3);
	...
	spin_unlock_bh(&hw_priv->tsm_lock);
	return Sstar_tesmode_reply(hw->wiphy, &tsm_stats,
...
/**
 * Sstar_get_roam_delay - Retrieves roam delay
 *
 * @hw: the hardware
 *
 * Returns: Returns the last measured roam delay
 */
int Sstar_get_roam_delay(struct ieee80211_hw *hw)
{
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE

	struct Sstar_common *hw_priv = hw->priv;
	u16 roam_delay = hw_priv->tsm_info.roam_delay / 1000;
	sta_printk("[STA] %s: Roam delay : %u", __func__, roam_delay);
	spin_lock_bh(&hw_priv->tsm_lock);
	hw_priv->tsm_info.roam_delay = 0;
	hw_priv->tsm_info.use_rx_roaming = 0;
	spin_unlock_bh(&hw_priv->tsm_lock);
	return Sstar_tesmode_reply(hw->wiphy, &roam_delay, sizeof(u16));
#else
	return -1;
#endif
}

static int Sstar_set_tx_power_level(struct ieee80211_hw *hw, int power)
{
	struct Sstar_common *hw_priv = hw->priv;
	...
	sta_printk("[STA] %s: Power set on Device : %d", __func__, power);
	return 0;
}

static int Sstar_set_wme_uapsd(struct ieee80211_hw *hw, int apsd)
{
	int if_id = 0;
	struct Sstar_common *hw_priv = (struct Sstar_common *)hw->priv;
	struct Sstar_vif *priv;
	...
	ret = Sstar_set_uapsd_param(priv, &arg);
	ret = ret;
	mutex_unlock(&hw_priv->conf_mutex);
	return 0;
}
...
#endif
#ifdef USB_BUS
extern int Sstar_device_txrx_test_init(struct Sstar_common *hw_priv,
				       int caseNum);
int Sstar_device_wakeup_test(struct Sstar_common *hw_priv, int enable)
{
	//Sstar_device_txrx_test_init(hw_priv,enable);
	return 0;
}
#else
int Sstar_device_wakeup_test(struct Sstar_common *hw_priv, int enable)
...
enum altm_Sstar_msg {
	...
	ALTM_SET_CCA = 100,
	SSTAR_MSG_GET_TSM_PARAMS = 102,
	SSTAR_MSG_START_STOP_TSM = 103,
	SSTAR_MSG_GET_ROAM_DELAY = 104,
	SSTAR_START_SMARTCONFIG = 107,
	SSTAR_SET_START_TX = 108,
	SSTAR_SET_STOP_TX = 109,
	SSTAR_SET_START_RX = 110,
	SSTAR_SET_STOP_RX = 111,
	SSTAR_WRITE_REG = 112,
	SSTAR_READ_REG = 113,
};
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
extern void Sstar_set_40M(struct Sstar_common *hw_priv, u32 is40M);
extern void Sstar_set_cca(struct Sstar_common *hw_priv, u8 *buff);
#endif
...
int wsm_stop_tx(struct Sstar_common *hw_priv);
...
int Sstar_altmtest_cmd(struct ieee80211_hw *hw, void *data, int len)
{
	...
	char *const fw_types[] = { "ETF", "WFM", "WSM", "HI test",
				   "Platform test" };
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
	case ALTM_SET_CCA: {
		Sstar_set_cca(hw_priv, (u8 *)(&msg->value));
		break;
	}
#endif
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				...
				WARN_ON(wsm_write_mib(
					hw_priv, WSM_MIB_ID_DOT11_SLOT_TIME,
					&slot_time, sizeof(slot_time),
					vif->if_id));
				Sstar_tool_use_short_slot =
					(slot_time == __cpu_to_le32(9)) ? 1 : 0;
				break;
			}
		}
		break;
	...
	case ALTM_SET_PREAMBLE:
		...
		{
			if (vif != NULL) {
				vif->association_mode.preambleType =
					msg->value ? WSM_JOIN_PREAMBLE_SHORT :
							   WSM_JOIN_PREAMBLE_LONG;
				wsm_set_association_mode(hw_priv,
							 &vif->association_mode,
							 vif->if_id);
				Sstar_tool_use_short_preamble =
					vif->association_mode.preambleType;
				break;
	...
	case ALTM_SET_CTSPROT: {
		...
		{
			if (vif != NULL) {
				wsm_write_mib(hw_priv,
					      WSM_MIB_ID_NON_ERP_PROTECTION,
					      &use_cts_prot,
					      sizeof(use_cts_prot), vif->if_id);
				Sstar_tool_use_cts_prot = value;
				break;
			}
		}
	} break;
	...
	case ALTM_SET_TXQUEUE_PARAM: {
		...
		sta_printk(
			KERN_ERR
			"set_tx_queue:aifs(%d),txop(%d),cw_min(%d),cw_max(%d)\n",
			params.aifs, params.txop, params.cw_min, params.cw_max);
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if ((vif != NULL) && (vif->vif != NULL)) {
				Sstar_conf_tx(hw, vif->vif, altbm_param->queue,
					      &params);
				break;
			}
		}
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
	case ALTM_SET_40M:
		Sstar_set_40M(hw_priv, msg->value);
		break;
#endif
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if ((vif != NULL)) {
				for (queue = 0; queue < 4; queue++)
					sta_printk(
						KERN_ERR
						"vif[%d],queue[%d]: txOpLimit=%d,aifns=%d,cwMin=%d,cwMax=%d;\n",
						i, queue,
						vif->edca.params[queue]
							.txOpLimit,
						vif->edca.params[queue].aifns,
						vif->edca.params[queue].cwMin,
						vif->edca.params[queue].cwMax);
			}
		}
		break;
	case ALTM_SET_WAKEUP:
		...
		//ret = Sstar_device_wakeup_test(hw_priv, msg->value);
	...
	case ALTM_GET_REVINFO:
		...
		if (ret > 0) {
			ret = Sstar_tesmode_reply(hw->wiphy, (void *)buff, ret);
		}
		break;
	case ALTM_GET_SHMEM:
		...
		if (altm_shmem->size <= 4) {
			...
			ret = wsm_read_shmem(hw_priv, altm_shmem->address,
					     &altm_shmem->buff,
					     altm_shmem->size);
			//Sstar_direct_read_reg_32(hw_priv,altm_shmem->address,&altm_shmem->buff);
			if (ret == 0) {
				sta_printk(KERN_ERR "size:0x%x, buff:%x\n",
					   altm_shmem->size, altm_shmem->buff);
				ret = Sstar_tesmode_reply(hw->wiphy,
							  &altm_shmem->buff,
							  altm_shmem->size);
			}
		} else {
	...
	case ALTM_SET_SHMEM:
		...
		if (altm_shmem->size <= 4) {
			sta_printk(KERN_ERR "size:0x%x, buff:%#x\n",
				   altm_shmem->size, altm_shmem->buff);
			//Sstar_direct_write_reg_32(hw_priv, altm_shmem->address,altm_shmem->buff);
			ret = wsm_write_shmem(hw_priv, altm_shmem->address,
					      altm_shmem->size,
					      &altm_shmem->buff);
		} else {
			sta_printk(KERN_ERR "proccess error, size range 1~4\n");
	...
	case ALTM_SET_WME_APSD:
		...
		ret = Sstar_set_wme_uapsd(hw, msg->value);
		break;
	...
	case ALTM_GET_BLOCK_ACK_RX:
		...
		ret = Sstar_tesmode_reply(hw->wiphy, &value, sizeof(value));
		break;
	...
	case ALTM_CLEAR_TSM_STATS:
		...
		spin_lock_bh(&hw_priv->tsm_lock);
		memset(&hw_priv->tsm_stats, 0, sizeof(struct Sstar_tsm_stats));
		memset(hw_priv->Sstar_tsm_stats, 0,
		       4 * sizeof(struct Sstar_tsm_stats));
		memset(&hw_priv->tsm_info, 0, sizeof(struct Sstar_tsm_info));
		spin_unlock_bh(&hw_priv->tsm_lock);
#endif
		break;
	case ALTM_GET_TSM_STATS:
		...
		for (; i < 4; i++) {
			altbeam_tsm_stat[i].msrmt_duration =
				hw_priv->Sstar_tsm_stats[i].msrmt_duration;
			altbeam_tsm_stat[i].tid =
				hw_priv->Sstar_tsm_stats[i].tid;
			altbeam_tsm_stat[i].reporting_reason =
				hw_priv->Sstar_tsm_stats[i].reporting_reason;
			altbeam_tsm_stat[i].txed_msdu_count =
				hw_priv->Sstar_tsm_stats[i].txed_msdu_count;
			altbeam_tsm_stat[i].msdu_discarded_count =
				hw_priv->Sstar_tsm_stats[i].msdu_discarded_count;
			altbeam_tsm_stat[i].msdu_failed_count =
				hw_priv->Sstar_tsm_stats[i].msdu_failed_count;
			altbeam_tsm_stat[i].multi_retry_count =
				hw_priv->Sstar_tsm_stats[i].multi_retry_count;
			altbeam_tsm_stat[i].qos_cfpolls_lost_count =
				hw_priv->Sstar_tsm_stats[i]
					.qos_cfpolls_lost_count;
			altbeam_tsm_stat[i].avg_q_delay =
				hw_priv->Sstar_tsm_stats[i].avg_q_delay;
			altbeam_tsm_stat[i].avg_transmit_delay =
				hw_priv->Sstar_tsm_stats[i].avg_transmit_delay;
		}
#endif
		ret = Sstar_tesmode_reply(hw->wiphy, &altbeam_tsm_stat[0],
					  sizeof(altbeam_tsm_stat));
		break;
	case SSTAR_MSG_GET_TSM_PARAMS:
		ret = Sstar_get_tsm_params(hw);
		break;

	case SSTAR_MSG_START_STOP_TSM:
		ret = Sstar_start_stop_tsm(hw, (u8 *)(&msg->value));
		break;
	case SSTAR_MSG_GET_ROAM_DELAY:
		ret = Sstar_get_roam_delay(hw);
		break;
	case ALTM_GET_STATUS_COMMON:
		...
		seq.cout = 0;
		Sstar_status_show_common((void *)&seq, NULL);
		ret = Sstar_tesmode_reply(hw->wiphy, buff, seq.cout);
		break;
	case ALTM_GET_STATUS_COUNTERS:
		...
		seq.size = BUFFER_SIZE;
		seq.cout = 0;
		Sstar_counters_show((void *)&seq, NULL);
		ret = Sstar_tesmode_reply(hw->wiphy, buff, seq.cout);
		break;
	case ALTM_GET_PACKETCNT:
		...
		seq.size = BUFFER_SIZE;
		seq.cout = 0;
		ret = Sstar_pkt_show((void *)&seq, NULL);
		ret |= Sstar_tesmode_reply(hw->wiphy, buff, seq.cout);
		break;
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				...
				seq.cout += snprintf(
					seq.buff + seq.cout,
					seq.size - seq.cout,
					"0===========Sstar_status_show_priv====%s=====:\n",
					vif_to_sdata(vif->vif)->name);
				Sstar_status_show_priv((void *)&seq, NULL);
			}
		}

		seq.cout +=
			snprintf(seq.buff + seq.cout, seq.size - seq.cout,
				 "=================End==================:\n");
		ret = Sstar_tesmode_reply(hw->wiphy, buff, seq.cout);
		break;
	...
	case ALTM_GET_RSSI:
		...
		seq.size = BUFFER_SIZE;
		seq.cout = 0;
		ret = Sstar_statistics_show((void *)&seq, NULL);
		ret |= Sstar_tesmode_reply(hw->wiphy, buff, seq.cout);
		break;
	case ALTM_SET_PS_MODE:
		sta_printk(KERN_ERR "set ps mode(%d)\n", msg->value);
		ret = Sstar_set_power_save_mode(hw, (u8 *)&msg->value,
						sizeof(msg->value));
		break;
	case ALTM_SET_TX_POWER:
		sta_printk(KERN_ERR "set_tx_power(%d)\n", msg->value);
		ret = Sstar_set_tx_power_level(hw, msg->value);
		break;
	...
	case ALTM_SET_ZERO_COUNTERS_TABLE:
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				WARN_ON(wsm_write_mib(
					hw_priv, WSM_MIB_ID_ZERO_COUNTERS_TABLE,
					&ucZeroCounterTable,
					sizeof(ucZeroCounterTable),
					vif->if_id));
				break;
			}
		}
	...
	case ALTM_GET_DBG_PRINT_TO_HOST:
		...
		WARN_ON(wsm_read_mib(hw_priv, WSM_MIB_ID_DBG_PRINT_TO_HOST,
				     &ucDbgPrintOpenFlag,
				     sizeof(ucDbgPrintOpenFlag), -1));
		ret = Sstar_tesmode_reply(hw->wiphy, &ucDbgPrintOpenFlag,
					  sizeof(ucDbgPrintOpenFlag));
		break;
	case ALTM_SET_FWCMD:
		pFwCmd = (char *)&msg[1];
		length = msg->value;
		Sstar_printk_sta("enter Sstar_set_fwcmd msg[1]:%s\n",
				 (char *)&msg[1]);
		Sstar_printk_sta("set fwcmd(%s)\n", pFwCmd);
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				WARN_ON(wsm_write_mib(hw_priv,
						      WSM_MIB_ID_FW_CMD, pFwCmd,
						      length, vif->if_id));
				break;
			}
		}
	...
	case ALTM_SET_SHORT_GI:
		...
		Sstar_set_shortGI(msg->value);
		Sstar_tool_shortGi = msg->value;
		break;
	...
	case SSTAR_START_SMARTCONFIG:
		Sstar_smartconfig_start(hw_priv, msg->value);
		break;
#endif
	case SSTAR_SET_START_TX:
		//mutex_lock(&hw_priv->conf_mutex);
		//wsm_stop_tx(hw_priv);
		//mutex_unlock(&hw_priv->conf_mutex);

		Sstar_printk_sta("enter Sstar_set_start_tx:value:%d\n",
				 msg->value);
		...
		if (hw->vendcmd_nl80211 == 0) {
			struct nlattr *data_p =
				nla_find(data, len, SSTAR_TM_MSG_DATA);
			if (!data_p)
				return ret;
		...
		break;
	case SSTAR_SET_STOP_TX:
		if (ETF_bStart_Tx == 0) {
			Sstar_printk_sta("please start_tx first, then stop_tx");
			return -EINVAL;
		}
		mutex_lock(&hw_priv->conf_mutex);
		wsm_stop_tx(hw_priv);
		...
		mutex_unlock(&hw_priv->conf_mutex);
		break;
	case SSTAR_SET_START_RX:
		if (ETF_bStart_Rx || ETF_bStart_Tx) {
			Sstar_printk_sta(
				"Error!already ETF_bStartTx/ETF_bStartRx,please stop_rx first!!!\n");
			return -EINVAL;
		}
		str = (char *)&msg[1];
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				ETF_bStart_Rx = 1;
				WARN_ON(wsm_write_mib(hw_priv,
						      WSM_MIB_ID_FW_CMD, str,
						      msg->value, vif->if_id));
			}
			break;
		}
		Sstar_printk_sta("fwcmd:%s\n", str);
		break;
	case SSTAR_SET_STOP_RX:
		if ((0 == ETF_bStart_Rx) || (0 == ch_and_type[0])) {
			Sstar_printk_sta(
				"please start_rx first,then stop_rx!!!\n");
			return -EINVAL;
		}
		str = (char *)&msg[1];
		memset(str + 10, 0, 20);
		...
		Sstar_for_each_vif(hw_priv, vif, i)
		{
			if (vif != NULL) {
				...
				WARN_ON(wsm_write_mib(hw_priv,
						      WSM_MIB_ID_FW_CMD, str,
						      strlen(str), vif->if_id));
			}
			break;
		}
	case SSTAR_WRITE_REG:
		altm_shmem = (struct altm_msg_shmem *)msg;
		if (altm_shmem->size <= 4) {
			Sstar_printk_sta("Write Register %x=%x\n",
					 altm_shmem->address, altm_shmem->buff);
			Sstar_direct_write_reg_32(hw_priv, altm_shmem->address,
						  altm_shmem->buff);
		} else {
			Sstar_printk_sta("proccess error, size range 1~4\n");
		}
		break;
	case SSTAR_READ_REG:
		altm_shmem = (struct altm_msg_shmem *)msg;
		if (altm_shmem->size <= 4) {
			Sstar_direct_read_reg_32(hw_priv, altm_shmem->address,
						 &altm_shmem->buff);
			Sstar_printk_sta("Read Register %x=%x\n",
					 altm_shmem->address, altm_shmem->buff);
		} else {
			Sstar_printk_sta("proccess error, size range 1~4\n");
		}
