...
int Sstar_register_bh(struct Sstar_common *hw_priv)
{
	...
	hw_priv->bh_thread = kthread_create(
		&usb_Sstar_bh, hw_priv,
		ieee80211_alloc_name(hw_priv->hw, "usb_Sstar_bh"));
...
int Sstar_bh_suspend(struct Sstar_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		Sstar_printk_err("BH error -- can't suspend\n");
		return -EINVAL;
	}

	atomic_set(&hw_priv->bh_suspend, SSTAR_APOLLO_BH_SUSPEND);
	wake_up(&hw_priv->bh_wq);
	return Sstar_wait_event_timeout_stay_awake(
		       hw_priv, hw_priv->bh_evt_wq,
		       hw_priv->bh_error || (SSTAR_APOLLO_BH_SUSPENDED ==
...
int Sstar_bh_resume(struct Sstar_common *hw_priv)
{
	bh_printk("[BH] resume.\n");
	if (hw_priv->bh_error) {
		Sstar_printk_err("BH error -- can't resume\n");
		return -EINVAL;
	}
	...
	return Sstar_wait_event_timeout_stay_awake(
		       hw_priv, hw_priv->bh_evt_wq,
		       hw_priv->bh_error || (SSTAR_APOLLO_BH_RESUMED ==
					     atomic_read(&hw_priv->bh_suspend)),
		       1 * HZ, false) ?
			     0 :
			     -ETIMEDOUT;
...
int Sstar_rx_bh_cb(struct Sstar_common *hw_priv, struct sk_buff *skb)
{
	...
	if (unlikely(wsm_id == 0x0800)) {
		wsm_handle_exception(hw_priv, &skb->data[sizeof(*wsm)],
				     wsm_len - sizeof(*wsm));
	...
	if ((hw_priv->wsm_caps.firmwareReady == 0) &&
	    ((wsm_seq != hw_priv->wsm_rx_seq) ||
	     ((wsm_id & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX)) !=
	      WSM_STARTUP_IND_ID))) {
		Sstar_printk_err(
			"rx wsm_seq err1 (rxseq=%d need_rxseq=%d) len(%d) wsm_id %x firmwareReady %d\n",
			wsm_seq, hw_priv->wsm_rx_seq, skb->len, wsm_id,
			hw_priv->wsm_caps.firmwareReady);
		//BUG_ON(1);
		if ((wsm_id & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX)) ==
		    WSM_STARTUP_IND_ID)
			Sstar_bh_halt(hw_priv);
		goto __free;
	} else if (WARN_ON(wsm_seq != hw_priv->wsm_rx_seq)) {
		Sstar_printk_err(
			"rx wsm_seq error wsm_seq[%d] wsm_rx_seq[%d] skb->len[%d] wsm_id[%x],wsm_len[%d]\n",
			wsm_seq, hw_priv->wsm_rx_seq, skb->len, wsm_id,
			wsm_len);
...
int Sstar_rx_single_channel_bh_cb(struct Sstar_common *hw_priv,
				  struct sk_buff *skb)
{
	struct wsm_hdr *wsm;
	u32 wsm_len;
...
static u8 Sstar_rx_need_alloc_skb(int wsm_id)
{
	...
	return (id == WSM_RECEIVE_INDICATION_ID) ||
	       (id == WSM_SMARTCONFIG_INDICATION_ID);
}
...
static bool Sstar_rx_serial(struct Sstar_common *hw_priv, u8 wsm_seq,
			    int wsm_id)
{
	bool serial_rx = true;
	...
	if ((hw_priv->wsm_caps.firmwareReady == 0) &&
	    ((wsm_seq != hw_priv->wsm_rx_seq) ||
	     ((wsm_id & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX)) !=
	      WSM_STARTUP_IND_ID))) {
		Sstar_printk_err(
			"rx wsm_seq err1 (rxseq=%d need_rxseq=%d)  wsm_id %x firmwareReady %d\n",
		...
		Sstar_printk_err(
			"rx wsm_seq error wsm_seq[%d] wsm_rx_seq[%d] wsm_id[%x]\n",
			wsm_seq, hw_priv->wsm_rx_seq, wsm_id);
		//frame_hexdump("rxdata",wsm,64);
		//BUG_ON(1);
		Sstar_hif_status_set(1);
...
static void Sstar_rx_multi_rx(struct Sstar_common *hw_priv, struct sk_buff *skb,
			      ...
			      bool serial_check)
{
#define RX_ALLOC_BUFF_OFFLOAD                                                  \
	((36 + 16) /*RX_DESC_OVERHEAD*/ + 4 /*FCS_LEN*/ - 16 /*WSM_HI_RX_IND*/)
	...
	do {
		...
		BUG_ON((wsm_id & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX)) !=
		       WSM_RECEIVE_INDICATION_ID);
		wsm_seq = (__le32_to_cpu(wsm->id) >> 13) & 7;

		if ((serial_check == true) &&
		    (WARN_ON(Sstar_rx_serial(hw_priv, wsm_seq, wsm_id) ==
			     false))) {
			goto next_skb;
		}
		...
		if (!Sstar_skb_copy) {
			Sstar_printk_err(
				"alloc--skb Error(%d)1,move to next skb\n",
				wsm_len + 16);
			WARN_ON(1);
			goto next_skb;
		}
		...
		Sstar_skb_reserve(Sstar_skb_copy,
				  (8 - (((unsigned long)Sstar_skb_copy->data) &
					7)) /*ALIGN 8*/);
	...
	next_skb:
		...
		RxFrameNum--;
		wsm = (struct wsm_hdr *)((u8 *)wsm +
					 ALIGN((wsm_len + RX_ALLOC_BUFF_OFFLOAD),
					       4));
		wsm_len = __le32_to_cpu(wsm->len);
		wsm_id = __le32_to_cpu(wsm->id) & 0xFFF;
...
static bool Sstar_process_ieee80211_frame(
	...
	int (*rx_func)(struct Sstar_common *hw_priv, struct sk_buff *skb))
{
	...
	if (wsm_id == WSM_MULTI_RECEIVE_INDICATION_ID) {
		...
		*/
		struct wsm_multi_rx *singlech_multirx =
			(struct wsm_multi_rx *)skb->data;

		wsm_seq = (__le32_to_cpu(singlech_multirx->MsgId) >> 13) & 7;

		if (WARN_ON(Sstar_rx_serial(hw_priv, wsm_seq, wsm_id) ==
			    false)) {
			Sstar_printk_err(
				"rx wsm_seq error wsm_seq[%d] wsm_rx_seq[%d] skb->len[%d] wsm_id[%x],wsm_len[%d]\n",
				wsm_seq, hw_priv->wsm_rx_seq, skb->len, wsm_id,
...
static bool Sstar_process_wsm_cmd_frame(
	struct Sstar_common *hw_priv, struct sk_buff *skb,
	int (*rx_func)(struct Sstar_common *hw_priv, struct sk_buff *skb))
{
	struct wsm_hdr *wsm;
...
static bool Sstar_process_raw_frame(struct Sstar_common *hw_priv,
				    struct sk_buff *skb)
{
	if (Sstar_process_ieee80211_frame(
		    hw_priv, skb, Sstar_rx_single_channel_bh_cb) == false) {
				      ...
				      struct sk_buff *skb))
{
	...
	if (WARN_ON(Sstar_rx_serial(hw_priv, wsm_seq,
				    __le32_to_cpu(wsm->id) & 0xFFF) == false)) {
		Sstar_hif_status_set(1);
		Sstar_bh_halt(hw_priv);
		return false;
	}
...
restart:
	...
	while ((skb = __Sstar_skb_dequeue(&local_list)) != NULL) {
		...
		case SSTAR_RX_WSM_CMD_FRAME:
			...
			Sstar_process_wsm_cmd_frame(
				hw_priv, skb, Sstar_rx_single_channel_bh_cb);
			Sstar_skb_queue_tail(&hw_priv->rx_frame_free, skb);
			break;
		...
		case SSTAR_RX_DERICTLY_DATA_FRAME:
			...
			Sstar_dev_kfree_skb(skb);
			break;
		case SSTAR_RX_WSM_FREE:
			bh_printk("SSTAR_RX_WSM_FREE");
			skb->pkt_type = 0;
			Sstar_skb_queue_tail(&hw_priv->rx_frame_free, skb);
			break;
		default:
	...
	while ((skb = Sstar_skb_dequeue(&hw_priv->rx_frame_free)) != NULL) {
		...
		hw_priv->sbus_ops->sbus_read_async(hw_priv->sbus_priv, 0x2, skb,
						   RX_BUFFER_SIZE, NULL);
		hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
	}
...
#define __ALL_HW_BUFS_USED (hw_priv->hw_bufs_used)
	...
	while (1) {
		...
		if (!__ALL_HW_BUFS_USED && powersave_enabled &&
		...
		if (atomic_read(&hw_priv->bh_halt)) {
			...
			if (hw_priv->sbus_ops->lmac_restart(
				    hw_priv->sbus_priv) != 0) {
		...
		if (1) {
			...
			for (i = 0; i < 4; ++i)
				pending |= Sstar_queue_get_xmit_timestamp(
					&hw_priv->tx_queue[i], &timestamp, -1,
					hw_priv->pending_frame_id);

			/* Check if frame transmission is timed out.
			 * Add an extra second with respect to possible
			 * interrupt loss. */
			timeout = timestamp + 2 * HZ - jiffies;

			/* And terminate BH tread if the frame is "stuck" */
			if (pending && timeout < 0) {
				extern void Sstar_usb_urb_map_show(
					struct sbus_priv * self);
				Sstar_printk_err(
					"tx rx schedule timeout................?\n");
		...
		if (!status && __ALL_HW_BUFS_USED) {
			...
			if (pending && timeout < 0) {
				...
				//if(prink_test <10){
				Sstar_printk_err(
					"Sstar_bh: Timeout waiting for TX confirm. hw_bufs_used %d,hw_noconfirm_tx(%d)\n",
					hw_priv->hw_bufs_used,
			...
			if (!hw_priv->tx_queue[0].num_queued &&
				...
				//usb suspend
				hw_priv->sbus_ops->power_mgmt(
					hw_priv->sbus_priv, true);
	...
	if ((!term)) {
		...
		if (!wifi_module_exit) {
			...
			status = wait_event_interruptible_timeout(
				hw_priv->bh_wq, ({
					term = atomic_xchg(&hw_priv->bh_term,
							   0);
	...
	if (term) {
		...
		if (!mutex_trylock(&hw_priv->wsm_cmd_mux)) {
			...
			if (hw_priv->wsm_cmd.cmd != 0xFFFF) {
				...
				Sstar_printk_exit(
					"cancle current pendding cmd,release wsm_cmd.lock\n");
		...
		*/
		...
		if (atomic_read(&hw_priv->scan.in_progress)) {
			struct Sstar_vif *scan_priv = ABwifi_hwpriv_to_vifpriv(
				hw_priv, hw_priv->scan.if_id);
			bool scanto_running = false;
			Sstar_priv_vif_list_read_unlock(&scan_priv->vif_lock);
			mutex_unlock(&hw_priv->conf_mutex);
			scanto_running = Sstar_hw_cancel_delayed_work(
				&hw_priv->scan.timeout, true);
			mutex_lock(&hw_priv->conf_mutex);
			if (scanto_running > 0) {
				u8 scan_fail = 0;
				Sstar_printk_exit(
					"scan is running , stop it right now\n");
				/*
...
/*
				...
				if (scan_fail == 0) {
					mutex_unlock(&hw_priv->conf_mutex);
					Sstar_printk_exit(
						"%s:Sstar_scan_timeout\n",
						__func__);
					Sstar_scan_timeout(
						&hw_priv->scan.timeout.work);
					mutex_lock(&hw_priv->conf_mutex);
				}
			}
		}
...
#define SSTAR_CANCEL_PENDDING_WORK(work, work_func)                            \
	do {                                                                   \
		if (Sstar_hw_cancel_queue_work(work, true) == true) {          \
			work_func(work);                                       \
		}                                                              \
	} while (0)

			if (Sstar_hw_cancel_delayed_work(
				    &hw_priv->scan.probe_work, true))
				Sstar_probe_work(
					&hw_priv->scan.probe_work.work);
			if (Sstar_hw_cancel_delayed_work(&hw_priv->scan.timeout,
							 true))
				Sstar_scan_timeout(&hw_priv->scan.timeout.work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
			if (Sstar_hw_cancel_delayed_work(
				    &hw_priv->rem_chan_timeout, true))
				Sstar_rem_chan_timeout(
					&hw_priv->rem_chan_timeout.work);
#endif
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->scan.work,
						   Sstar_scan_work);
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->event_handler,
						   Sstar_event_handler);
#ifdef CONFIG_SSTAR_BA_STAT
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->ba_work,
						   Sstar_ba_work);
			Sstar_del_timer_sync(&hw_priv->ba_timer);
#endif
#ifdef SSTAR_SUPPORT_WIDTH_40M
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->get_cca_work,
						   Sstar_get_cca_work);
			Sstar_del_timer_sync(&hw_priv->chantype_timer);
#endif
#endif
#ifdef SSTAR_SUPPORT_SMARTCONFIG
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->scan.smartwork,
						   Sstar_smart_scan_work);
			SSTAR_CANCEL_PENDDING_WORK(
				&hw_priv->scan.smartsetChanwork,
				Sstar_smart_setchan_work);
			SSTAR_CANCEL_PENDDING_WORK(&hw_priv->scan.smartstopwork,
						   Sstar_smart_stop_work);
			Sstar_del_timer_sync(
				&hw_priv->smartconfig_expire_timer);
#endif
			...
			Sstar_for_each_vif(hw_priv, priv, i)
			{
				if (priv == NULL)
					continue;
				SSTAR_CANCEL_PENDDING_WORK(&priv->wep_key_work,
							   Sstar_wep_key_work);
				SSTAR_CANCEL_PENDDING_WORK(&priv->join_work,
							   Sstar_join_work);
				SSTAR_CANCEL_PENDDING_WORK(&priv->unjoin_work,
							   Sstar_unjoin_work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
				SSTAR_CANCEL_PENDDING_WORK(
					&priv->offchannel_work,
					Sstar_offchannel_work);
#endif
				SSTAR_CANCEL_PENDDING_WORK(&priv->link_id_work,
							   Sstar_link_id_work);
#ifndef CONFIG_TX_NO_CONFIRM
				SSTAR_CANCEL_PENDDING_WORK(
					&priv->tx_failure_work,
					Sstar_tx_failure_work);
#endif
				SSTAR_CANCEL_PENDDING_WORK(&priv->set_tim_work,
							   Sstar_set_tim_work);
				SSTAR_CANCEL_PENDDING_WORK(
				...
				SSTAR_CANCEL_PENDDING_WORK(
					...
					Sstar_ht_info_update_work);
#endif
#ifdef SSTAR_SUPPORT_WIDTH_40M
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
				//SSTAR_CANCEL_PENDDING_WORK(&priv->chantype_change_work, Sstar_channel_type_change_work);
				if (Sstar_hw_cancel_delayed_work(
					    &priv->chantype_change_work, true))
					Sstar_channel_type_change_work(
						&priv->chantype_change_work
							 .work);
#endif
#endif

				Sstar_hw_cancel_delayed_work(
					&priv->dhcp_retry_work, true);
#ifndef CONFIG_TX_NO_CONFIRM
				if (Sstar_hw_cancel_delayed_work(
					    &priv->bss_loss_work, true))
					Sstar_bss_loss_work(
						&priv->bss_loss_work.work);
				if (Sstar_hw_cancel_delayed_work(
...
#endif
#if 0
				if(Sstar_hw_cancel_delayed_work(&priv->set_cts_work,true))
					Sstar_set_cts_work(&priv->set_cts_work.work);
#endif
				if (Sstar_hw_cancel_delayed_work(
					    &priv->link_id_gc_work, true))
					Sstar_link_id_gc_work(
						&priv->link_id_gc_work.work);
#ifdef CONFIG_SSTAR_SUPPORT_P2P
				if (Sstar_hw_cancel_delayed_work(
					    &priv->pending_offchanneltx_work,
					    true))
...
#endif
				if (Sstar_hw_cancel_delayed_work(
					    &priv->join_timeout, true))
					Sstar_join_timeout(
						&priv->join_timeout.work);
