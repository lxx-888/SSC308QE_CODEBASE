...
struct sbus_ops {
	...
	int (*sbus_memcpy_fromio)(struct sbus_priv *self, unsigned int addr,
				  void *dst, int count); /*rx queue mode*/
	int (*sbus_memcpy_toio)(struct sbus_priv *self, unsigned int addr,
				const void *src, int count); /*tx queue mode*/
	int (*sbus_read_async)(struct sbus_priv *self, unsigned int addr,
			       void *dst, int count,
			       sbus_callback_handler hander); /*rx queue mode*/
	int (*sbus_write_async)(struct sbus_priv *self, unsigned int addr,
				const void *src, int count,
				sbus_callback_handler hander); /*tx queue mode*/
	int (*sbus_read_sync)(
		struct sbus_priv *self, unsigned int addr, void *dst,
		int len); /*read register,download firmware,len <=256*/
	int (*sbus_write_sync)(
		struct sbus_priv *self, unsigned int addr, const void *src,
		int len); /*write register,download firmware,len <=256*/
	...
	int (*sbus_write_data)(struct sbus_priv *self, const void *tx,
			       size_t tx_len);
	int (*sbus_write_firmware)(struct sbus_priv *self, unsigned int addr,
				   const void *src, int count);
	int (*sbus_read_status)(struct sbus_priv *self, u32 *status,
				size_t tx_len);
