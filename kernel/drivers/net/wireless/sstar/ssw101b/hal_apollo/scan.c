...
#define SSTAR_SPLIT_SCAN_MAX_CHANNEL 2
#define SSTAR_SPLIT_NEXT_CHANNEL_TIME (500)
#endif
...
extern void etf_v2_scan_end(struct Sstar_common *hw_priv,
...
static int Sstar_advance_scan_start(struct Sstar_common *hw_priv)
{
	...
	Sstar_hw_priv_queue_delayed_work(
		hw_priv, &hw_priv->advance_scan_timeout, tmo * HZ / 1000);
	return 0;
}
#endif
...
void Sstar_scan_split_work(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, scan.scan_spilt.work);
...
static bool Sstar_scan_split_running(struct Sstar_common *hw_priv)
{
#ifdef CONFIG_SSTAR_SCAN_SPLIT
	...
	Sstar_printk_debug("%s:wait next scan\n", __func__);
	Sstar_hw_priv_queue_delayed_work(
		hw_priv, &hw_priv->scan.scan_spilt,
		msecs_to_jiffies(SSTAR_SPLIT_NEXT_CHANNEL_TIME));
		  ...
		  struct ieee80211_scan_req_wrap *req_wrap)
{
	...
	if (Sstar_work_pending(&priv->offchannel_work) || (roc_if_id != -1)) {
		Sstar_printk_err(
			"[SCAN] Offchannel work pending,ignoring scan work %d\n",
			hw_priv->roc_if_id);
		return -EBUSY;
	}
#endif
	...
	Sstar_printk_debug("[SCAN] Scan request for %d SSIDs.\n",
	...
	frame.skb = ieee80211_probereq_get(
		hw, vif, NULL, 0, req_wrap->req->ie, req_wrap->req->ie_len,
		req_wrap->flags & IEEE80211_SCAN_REQ_NEED_BSSID ?
			      req_wrap->bssid :
			      NULL);
	if (!frame.skb)
		return -ENOMEM;
	...
	if (priv->join_status != SSTAR_APOLLO_JOIN_STATUS_STA) {
		...
		for (i = 0; i < req_wrap->req->n_channels; i++) {
			hw_priv->scan_channels[hw_priv->num_scanchannels + i]
				.number =
				channel_hw_value(req_wrap->req->channels[i]);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0))
			if (req_wrap->req->channels[i]->flags &
			...
			if (req_wrap->req->channels[0]->band ==
			    NL80211_BAND_5GHZ)
				hw_priv->scan_channels[hw_priv->num_scanchannels +
						       i]
					.number |= SSTAR_APOLLO_SCAN_BAND_5G;
		}
		if (req_wrap->req->channels[0]->band == NL80211_BAND_2GHZ)
			...
			if (priv->if_id && (priv->vif->p2p == true)) {
				struct Sstar_ieee80211_mgmt *mgmt =
					(struct Sstar_ieee80211_mgmt *)
						frame.skb->data;
				Sstar_printk_debug(
					"%s:Sstar_clear_wpas_p2p_40M_ie,if_id(%d)\n",
					__func__, priv->if_id);
				Sstar_clear_wpas_p2p_40M_ie(mgmt,
							    frame.skb->len);
			}
#endif
//#endif
		...
		*/
		...
		hw_priv->scan.end =
			&req_wrap->req->channels[req_wrap->req->n_channels];
		hw_priv->scan.output_power = hw_priv->output_power;
		hw_priv->scan.if_id = priv->if_id;
		hw_priv->scan.passive =
			!!(req_wrap->flags & IEEE80211_SCAN_REQ_PASSIVE_SCAN);
		hw_priv->scan.cca =
			!!(req_wrap->flags & IEEE80211_SCAN_REQ_CCA);
#ifdef CONFIG_SSTAR_SCAN_SPLIT
		hw_priv->scan.split =
			!!(req_wrap->flags & IEEE80211_SCAN_REQ_SPILT);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_P2P
#ifdef SSTAR_P2P_CHANGE
		/*
		...
		*/
		...
		for (i = 0; i < req_wrap->req->n_ssids; ++i) {
			...
			BUG_ON(req_wrap->req->ssids[i].ssid_len >
			       sizeof(dst->ssid));
			      ...
			      struct ieee80211_sched_scan_ies *ies)
{
	...
	Sstar_printk_warn("[SCAN] Scheduled scan request-->.\n");
	...
	frame.skb = ieee80211_probereq_get(hw, priv->vif, NULL, 0, ies->ie[0],
					   ies->len[0], NULL);
	if (!frame.skb)
		return -ENOMEM;
	...
	for (i = 0; i < req->n_ssids; ++i) {
		...
		{
			u8 j;
			Sstar_printk_warn("[SCAN] SSID %d\n", i);
			for (j = 0; j < req->ssids[i].ssid_len; j++)
				Sstar_printk_warn("[SCAN] 0x%x\n",
						  req->ssids[i].ssid[j]);
		}
	}
...
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, scan.work);
	...
	if (first_run) {
		...
		if (Sstar_hw_cancel_delayed_work(&priv->join_timeout, true) > 0)
	...
	if (!hw_priv->scan.req || (hw_priv->scan.curr == hw_priv->scan.end)) {
		...
		while (priv->scan_again) {
			int status;
			priv->scan_again = 0;
			scan.ch = Sstar_kzalloc(sizeof(struct wsm_scan_ch[3]),
						GFP_KERNEL);
			if (!scan.ch) {
				break;
			}
			...
			maxChannelTime = (scan.numOfSSIDs *
					  scan.numOfProbeRequests * 15) +
					 10;
			maxChannelTime =
				(maxChannelTime < 35) ? 35 : maxChannelTime;
			scan.ch[0].number = 1;
...
#else
		while ((hw_priv->scan.if_id == 1) &&
		       (atomic_read(&hw_priv->go_bssid_set) == 1) &&
		       (atomic_read(&hw_priv->receive_go_resp) == 0)) {
			int status;
			Sstar_printk_scan(
				"%s:p2p scan again,channel(%d)\n", __func__,
			...
			if (!time_is_after_jiffies(
				    hw_priv->p2p_scan_start_time + 2 * HZ)) {
				atomic_set(&hw_priv->go_bssid_set, 0);
				atomic_set(&hw_priv->p2p_oper_channel, 0);
				break;
			}

			scan.ch = Sstar_kzalloc(sizeof(struct wsm_scan_ch[5]),
						GFP_KERNEL);
			if (!scan.ch) {
				break;
			}
			...
			scan.ch[1].number =
				1; //atomic_read(&hw_priv->p2p_oper_channel);
			scan.ch[1].maxChannelTime = maxChannelTime;
			scan.ch[1].minChannelTime = 35;

			scan.ch[2].number =
				6; //atomic_read(&hw_priv->p2p_oper_channel);
			scan.ch[2].maxChannelTime = maxChannelTime;
			scan.ch[2].minChannelTime = 35;

			scan.ch[3].number =
				11; //atomic_read(&hw_priv->p2p_oper_channel);
		...
		if (hw_priv->scan.status < 0)
			Sstar_printk_debug("[SCAN] Scan failed (%d).\n",
					   hw_priv->scan.status);
		else if (hw_priv->scan.req)
			Sstar_printk_debug("[SCAN] Scan completed.\n");
		else
			Sstar_printk_debug("[SCAN] Scan canceled.\n");
...
#ifdef CONFIG_SSTAR_SCAN_SPLIT
			if ((hw_priv->scan.split == 1) &&
			    (i >= SSTAR_SPLIT_SCAN_MAX_CHANNEL))
				break;
#endif
		...
		if (hw_priv->enable_advance_scan) {
			...
			scan.numOfProbeRequests = (first->flags &
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 11, 0))
						   IEEE80211_CHAN_PASSIVE_SCAN
#else
						   IEEE80211_CHAN_NO_IR
		...
		*/
		...
		scan.ch = Sstar_kzalloc(sizeof(struct wsm_scan_ch) *
						(it - hw_priv->scan.curr),
					GFP_KERNEL);
		if (!scan.ch) {
		...
		for (i = 0; i < scan.numOfChannels; ++i) {
			scan.ch[i].number =
				channel_hw_value(hw_priv->scan.curr[i]);
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
			if (hw_priv->enable_advance_scan) {
...
#else
			WARN_ON(wsm_set_output_power(
				hw_priv, hw_priv->scan.output_power * 10,
				priv->if_id));
#endif
			}
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
...
vif_err:
	...
	up(&hw_priv->scan.lock);
	wsm_unlock_tx(hw_priv);
	Sstar_printk_err("[SCAN] interface removed\n");
...
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, scan.swork);
	struct wsm_scan scan;
	struct wsm_ssid scan_ssid;
	int i;
	struct Sstar_vif *priv =
		ABwifi_hwpriv_to_vifpriv(hw_priv, hw_priv->scan.if_id);
	if (unlikely(!priv)) {
		WARN_ON(1);
		return;
	}
	...
	scan.ch = Sstar_kzalloc(sizeof(struct wsm_scan_ch[scan.numOfChannels]),
				GFP_KERNEL);
	if (!scan.ch) {
		hw_priv->scan.status = -ENOMEM;
		goto fail;
...
static void Sstar_scan_restart_delayed(struct Sstar_vif *priv)
{
	...
	Sstar_for_each_vif(hw_priv, priv_delay, i)
	{
		...
		if (!(
			    ...
			    (priv_delay->join_status ==
			     SSTAR_APOLLO_JOIN_STATUS_MONITOR)))
			continue;

		Sstar_printk_scan(
			"%s:if_id(%d),scan_id(%d),join_status(%d),delayed_unjoin(%d)\n",
			__func__, priv_delay->if_id, hw_priv->scan.if_id,
			priv_delay->join_status, priv_delay->delayed_unjoin);
#ifndef CONFIG_TX_NO_CONFIRM
		if (priv_delay->delayed_link_loss) {
			...
			/* Restart beacon loss timer and requeue
			   BSS loss work. */
			Sstar_printk_debug("[CQM] Requeue BSS loss in %d "
					   "beacons.\n",
					   tmo);
			spin_lock_bh(&priv_delay->bss_loss_lock);
			priv_delay->bss_loss_status =
				SSTAR_APOLLO_BSS_LOSS_NONE;
			spin_unlock_bh(&priv_delay->bss_loss_lock);
			Sstar_hw_cancel_delayed_work(&priv_delay->bss_loss_work,
						     false);
			Sstar_hw_priv_queue_delayed_work(
				hw_priv, &priv_delay->bss_loss_work,
				tmo * HZ / 10);
		}
#endif
		/* FW bug: driver has to restart p2p-dev mode after scan. */
		if (priv_delay->join_status ==
		    SSTAR_APOLLO_JOIN_STATUS_MONITOR) {
		...
		if (priv_delay->delayed_unjoin) {
			...
			if (Sstar_hw_priv_queue_work(
				    hw_priv, &priv_delay->unjoin_work) <= 0)
				wsm_unlock_tx(hw_priv);
		}
	}
}
...
static void Sstar_scan_complete(struct Sstar_common *hw_priv, int if_id)
{
	...
	if (hw_priv->scan.direct_probe) {
		...
		if (priv) {
			Sstar_printk_debug("[SCAN] Direct probe complete.\n");
			Sstar_scan_restart_delayed(priv);
		} else {
			Sstar_printk_debug(
		...
		up(&hw_priv->scan.lock);
		wsm_unlock_tx(hw_priv);
	} else if (Sstar_scan_split_running(hw_priv) == false) {
		Sstar_scan_work(&hw_priv->scan.work);
	} else {
#ifndef CONFIG_SSTAR_SCAN_SPLIT
		BUG_ON(1);
#endif
	}
}
			    ...
			    struct wsm_scan_complete *arg)
{
	...
	if (unlikely(priv->mode == NL80211_IFTYPE_UNSPECIFIED)) {
		/* STA is stopped. */
		Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
		Sstar_printk_err("[SCAN] mode err\n");
		return;
	}
	Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
	...
	if (hw_priv->bStartTx) {
		Sstar_hw_priv_queue_delayed_work(
			hw_priv, &hw_priv->scan.timeout, HZ / 10);
		return;
	}
	...
	if ((hw_priv->scan.status != -ETIMEDOUT) &&
	    (Sstar_hw_cancel_delayed_work(
		     &hw_priv->scan.timeout,
		     false /*can't set to true,because this function is call in bh, must not wait in bh */) >
	     0)) {
...
//#ifdef CONFIG_WIRELESS_EXT
extern int wsm_start_scan_etf(struct Sstar_common *hw_priv,
			      struct ieee80211_vif *vif);
...
void Sstar_scan_timeout(struct Sstar_work_struct *work)
{
	...
	//#ifdef CONFIG_WIRELESS_EXT
	if (hw_priv->bStartTx) {
		struct Sstar_vif *priv = __ABwifi_hwpriv_to_vifpriv(
		...
		if (hw_priv->bStartTxWantCancel == 0) {
			...
			if (hw_priv->etf_test_v2) {
				Sstar_hw_priv_queue_work(
					hw_priv, &hw_priv->etf_tx_end_work);
			}
			//stop etf test
			//up(&hw_priv->scan.lock);
	...
	if (likely(atomic_xchg(&hw_priv->scan.in_progress, 0))) {
		if (hw_priv->scan.status > 0)
			...
			Sstar_printk_warn("Timeout waiting for scan "
					  "complete notification.\n");
			hw_priv->scan.status = -ETIMEDOUT;
			hw_priv->scan.curr = hw_priv->scan.end;
			if (WARN_ON(wsm_stop_scan(
				    hw_priv, hw_priv->scan.if_id ? 1 : 0))) {
				hw_priv->scan.wait_complete = 0;
...
void Sstar_probe_work(struct Sstar_work_struct *work)
{
	...
	if (unlikely(down_trylock(&hw_priv->scan.lock))) {
		...
		if (unlikely(atomic_read(&hw_priv->scan.in_progress))) {
			schedule();
			Sstar_hw_priv_queue_delayed_work(
				hw_priv, &hw_priv->scan.probe_work, HZ / 10);
			mutex_unlock(&hw_priv->conf_mutex);
			...
			BUG_ON(Sstar_queue_remove(hw_priv, queue,
						  hw_priv->pending_frame_id));
#else
			BUG_ON(Sstar_queue_remove(queue,
						  hw_priv->pending_frame_id));
#endif
			wsm_unlock_tx(hw_priv);
			mutex_unlock(&hw_priv->conf_mutex);
			Sstar_printk_scan(
				"%s:listenning or other delete pendding frame\n",
				__func__);
		}
		return;
	}

	if (Sstar_queue_get_skb(queue, hw_priv->pending_frame_id, &frame.skb,
				&txpriv)) {
		up(&hw_priv->scan.lock);
		mutex_unlock(&hw_priv->conf_mutex);
		wsm_unlock_tx(hw_priv);
	...
	ies_len = frame.skb->len - sizeof(struct ieee80211_hdr_3addr);
	if (ies_len) {
		u8 *ssidie = (u8 *)cfg80211_find_ie(SSTAR_WLAN_EID_SSID, ies,
						    ies_len);
