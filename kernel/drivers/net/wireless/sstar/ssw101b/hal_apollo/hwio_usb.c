...
int Sstar_ep0_read(struct Sstar_common *hw_priv, u32 addr, void *buf,
		   u32 buf_len)
{
	BUG_ON(!hw_priv->sbus_ops);
	return hw_priv->sbus_ops->sbus_read_sync(hw_priv->sbus_priv, addr, buf,
						 buf_len);
}

int Sstar_ep0_write(struct Sstar_common *hw_priv, u32 addr, const void *buf,
		    u32 buf_len)
{
	...
	BUG_ON(!hw_priv->sbus_ops);
	return hw_priv->sbus_ops->sbus_write_sync(hw_priv->sbus_priv, addr, buf,
						  buf_len);
}
...
int Sstar_ep0_write_cmd(struct Sstar_common *hw_priv, struct wsm_hdr_tx *wsm_h)
{
	...
	if (size > DOWNLOAD_BLOCK_SIZE) {
		/*  downloading loop */
		for (put = DOWNLOAD_BLOCK_SIZE; put < size;
		     put += DOWNLOAD_BLOCK_SIZE) {
			u32 tx_size;
			...
			ret = Sstar_ep0_write(hw_priv, put + addr, buf,
					      tx_size);
			if (ret < 0) {
				Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
		...
		ret = Sstar_ep0_write(
			hw_priv, addr, buf,
			DOWNLOAD_BLOCK_SIZE); // last step write first block
		if (ret < 0) {
			Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
				  "%s: can't write block at line %d.\n",
				  __func__, __LINE__);
	...
	hw_priv->wsm_tx_seq_ep0 =
		(hw_priv->wsm_tx_seq_ep0 + 1) & WSM_TX_SEQ_MAX;
...
int Sstar_direct_read_reg_32(struct Sstar_common *hw_priv, u32 addr, u32 *val)
{
	...
	ret = Sstar_ep0_read(hw_priv, addr, &local_val, sizeof(local_val));
...
int Sstar_direct_write_reg_32(struct Sstar_common *hw_priv, u32 addr, u32 val)
{
	...
	ret = Sstar_ep0_write(hw_priv, addr, &local_val, sizeof(local_val));
	if (ret < 0) {
		Sstar_dbg(SSTAR_APOLLO_DBG_ERROR,
			  "%s:  can't write "
			  "config register.\n",
...
static struct Sstar_reg_bit_s Sstar_debug_bus_reg_bit[] = {
	{ 0x161000A0, 31, 0, 0x13071307 }, { 0x161000A4, 31, 0, 0x13071307 },
	{ 0x161000A8, 31, 0, 0x13071307 }, { 0x161000AC, 31, 0, 0x13071307 },
	{ 0x161000B0, 31, 0, 0x13071307 }, { 0x161000B4, 31, 0, 0x13071307 },
	{ 0x161000B8, 31, 0, 0x13071307 }, { 0x161000BC, 31, 0, 0x13071307 },
	{ 0x161000C0, 31, 0, 0x13071307 }, { 0x161000C4, 31, 0, 0x13071307 },
	{ 0x161000C8, 31, 0, 0x13071307 }, { 0x161000CC, 31, 0, 0x13001307 },
	{ 0x161000D0, 31, 0, 0x13071300 }, { 0x161000D4, 31, 0, 0x13071307 },
	{ 0x161000D8, 31, 0, 0x13071307 }, { 0x161000DC, 31, 0, 0x13071307 },
	{ 0x161000E0, 31, 0, 0x13071307 }, { 0x161000E4, 31, 0, 0x13071307 },
	{ 0x16100174, 31, 0, 0x13071307 }, { 0xffffffff, 31, 0, 0 }
};
...
int Sstar_usb_write_bit_table(struct Sstar_common *hw_priv,
			      struct Sstar_reg_bit_s *reg_table)
{
	int retval = 0;
...
static struct Sstar_reg_val_s Sstar_debugbus_reg[] = {
	{ 0x1610009c, 0x1a3b },	    { 0x161000A0, 0x13071300 },
	{ 0x161000AC, 0x13071307 }, { 0x161000B8, 0x13071307 },
	{ 0x161000C0, 0x0b070b07 }, { 0x161000C4, 0x0b070b07 },
	{ 0x161000C8, 0x0b070b07 }, { 0x161000CC, 0x0b001307 },
	{ 0x161000D0, 0x00001300 }, { 0x16100174, 0x00000000 },
	{ 0xffffffff, 0 }
};
#endif
#ifdef HW_DOWN_FW

static struct Sstar_reg_val_s Sstar_usb_only_abort_en[] = { { 0x16100008,
							      0xffffffff },
							    { 0x0b000130, 0xf },
							    { 0x0b000134, 0xf },
							    { 0xffffffff, 0 } };

static struct Sstar_reg_val_s Sstar_usb_all_irq_en[] = { { 0x16100008, 0 },
							 { 0x0b000130, 0 },
							 { 0x0b000134, 0 },
							 { 0xffffffff, 0 } };
...
int Sstar_write_reg_table(struct Sstar_common *hw_priv,
			  struct Sstar_reg_val_s *reg_table)
{
	int retval = 0;
	...
	while (preg_table->addr != 0xffffffff) {
		retval = Sstar_direct_write_reg_32(hw_priv, preg_table->addr,
						   preg_table->val);
...
#pragma message("++++++++++++Sstar read reg test++++++++++++\n");
	...
	Sstar_usb_ep0_hw_reset_cmd(hw_priv->sbus_priv, HW_RESET_HIF_SYSTEM_CPU,
				   0);
...
//	u32 val32;
	...
	Sstar_printk_init("0x1610102c=0x%x\n", val32);
	...
	Sstar_printk_init("0x16101000=0x%x\n", val32);
	...
	hw_priv->sbus_ops->sbus_memcpy_fromio(hw_priv->sbus_priv, 0x2, NULL,
					      RX_BUFFER_SIZE);
	hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);

	return hw_priv->sbus_ops->sbus_wait_data_xmited ?
			     hw_priv->sbus_ops->sbus_wait_data_xmited(
			       hw_priv->sbus_priv) :
			     0;
}
...
void Sstar_firmware_init_check(struct Sstar_common *hw_priv)
{
	...
	hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
	status = hw_priv->sbus_ops->sbus_memcpy_toio(hw_priv->sbus_priv, 0x1,
						     NULL, TX_BUFFER_SIZE);
	Sstar_printk_debug("%s : status [%d] \n", __func__, status);
	hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
