...
#define can_not_queue_work(hw_priv) (((hw_priv)->workqueue == NULL))
#define Sstar_hw_priv_queue_work(hw_priv, work)                                \
	(can_not_queue_work(hw_priv) ?                                         \
		       -1 :                                                          \
		       Sstar_queue_work((hw_priv)->workqueue, work))
#define Sstar_hw_priv_queue_delayed_work(hw_priv, dwork, delay)                \
	(can_not_queue_work(hw_priv) ?                                         \
		       -1 :                                                          \
		       Sstar_queue_delayed_work((hw_priv)->workqueue, dwork, delay))
static inline bool Sstar_hw_cancel_queue_work(struct Sstar_work_struct *work,
					      bool sync)
{
	bool retval = false;
	if ((sync == true) || Sstar_work_pending(work)) {
		retval = Sstar_cancel_work_sync(work);
	}
...
static inline bool
Sstar_hw_cancel_delayed_work(struct Sstar_delayed_work *dwork, bool sync)
{
	bool retval = false;
	if (sync == true) {
		retval = Sstar_cancel_delayed_work_sync(dwork);
	} else {
		retval = Sstar_cancel_delayed_work(dwork);
	}

	return retval;
}
#ifdef CONFIG_PM
#define Sstar_hold_suspend(__hw_priv)                                          \
	Sstar_pm_stay_awake_lock(&((__hw_priv)->pm_state))
#define Sstar_release_suspend(__hw_priv)                                       \
	Sstar_pm_stay_awake_unlock(&((__hw_priv)->pm_state))
#else
#define Sstar_hold_suspend(__hw_priv) BUG_ON(__hw_priv == NULL)
#define Sstar_release_suspend(__hw_priv) BUG_ON(__hw_priv == NULL)
#endif

#define Sstar_wait_event_timeout_stay_awake(_hw_priv, _wait_q, _cond,          \
					    _timeout, _awake)                  \
	({                                                                     \
		long ret_timeout = _timeout;                                   \
		if (_awake == true)                                            \
			Sstar_hold_suspend(_hw_priv);                          \
		ret_timeout = wait_event_timeout(_wait_q, _cond, ret_timeout); \
		if (_awake == true)                                            \
			Sstar_release_suspend(_hw_priv);                       \
		ret_timeout;                                                   \
	})
static inline void Sstar_ieee80211_rx(struct ieee80211_hw *hw,
				      struct sk_buff *skb)
{
...
static inline void Sstar_ieee80211_tx_status(struct ieee80211_hw *hw,
					     struct sk_buff *skb)
{
