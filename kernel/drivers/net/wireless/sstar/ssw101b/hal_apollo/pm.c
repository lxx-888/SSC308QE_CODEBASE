				...
				struct Sstar_common *hw_priv)
{
	...
	pm->pm_driver.driver.name =
		ieee80211_alloc_name(hw_priv->hw, "Sstar_power");
...
static int
Sstar_pm_init_stayawake_lock(struct Sstar_pm_stayawake_lock *stayawake_lock)
{
	unsigned long flags;
	...
	wake_lock_init(&stayawake_lock->stayawak_lock, WAKE_LOCK_SUSPEND,
		       ieee80211_alloc_name(stayawake_lock->hw_priv->hw,
					    "Sstar_wlan_stayawake"));
#endif
#endif
...
static int
Sstar_pm_deinit_stayawake_lock(struct Sstar_pm_stayawake_lock *stayawake_lock)
{
#ifdef CONFIG_PM
#ifdef CONFIG_WAKELOCK
	unsigned long flags;
...
int Sstar_pm_init(struct Sstar_pm_state *pm, struct Sstar_common *hw_priv)
{
	int ret = Sstar_pm_init_common(pm, hw_priv);
	if (!ret) {
		wake_lock_init(&pm->wakelock, WAKE_LOCK_SUSPEND,
			       ieee80211_alloc_name(hw_priv->hw, "Sstar_wlan"));
...
void Sstar_pm_stay_awake(struct Sstar_pm_state *pm, unsigned long tmo)
{
	long cur_tmo;
	unsigned long flags;
...
int Sstar_pm_init(struct Sstar_pm_state *pm, struct Sstar_common *hw_priv)
{
	int ret = Sstar_pm_init_common(pm, hw_priv);
	if (!ret) {
		Sstar_init_timer(&pm->stay_awake);
...
void Sstar_pm_deinit(struct Sstar_pm_state *pm)
{
	Sstar_del_timer_sync(&pm->stay_awake);
	Sstar_pm_deinit_common(pm);
	Sstar_pm_deinit_stayawake_lock(&pm->stayawake_lock);
}
...
void Sstar_pm_stay_awake(struct Sstar_pm_state *pm, unsigned long tmo)
{
	...
	if (!Sstar_timer_pending(&pm->stay_awake) || cur_tmo < (long)tmo)
		Sstar_mod_timer(&pm->stay_awake, jiffies + tmo);
	spin_unlock_irqrestore(&pm->lock, flags);
}
...
#endif
#ifdef SSTAR_SUPPORT_WOW
static long Sstar_suspend_work(struct Sstar_delayed_work *work)
{
	int ret = Sstar_cancel_delayed_work(work);
	long tmo;
	if (ret > 0) {
		/* Timer is pending */
...
static int Sstar_resume_work(struct Sstar_common *hw_priv,
			     struct Sstar_delayed_work *work, unsigned long tmo)
{
	if ((long)tmo < 0)
		return 1;
...
static int Sstar_suspend_late(struct device *dev)
{
	...
	if (atomic_read(&hw_priv->bh_rx)) {
		Sstar_printk_pm("Suspend interrupted.\n");
		return -EAGAIN;
	}
	return 0;
}
...
#ifdef SSTAR_SUPPORT_WOW
static int _Sstar_wow_suspend(struct ieee80211_hw *hw,
			      struct cfg80211_wowlan *wowlan)
{
	...
	Sstar_for_each_vif(hw_priv, priv, i)
	{
		...
		ret = __Sstar_wow_suspend(priv, wowlan);
		if (ret) {
			for (; i >= 0; i--) {
				if (!hw_priv->vif_list[i])
					continue;
				priv = (struct Sstar_vif *)hw_priv->vif_list[i]
					       ->drv_priv;
	...
	if (Sstar_bh_suspend(hw_priv)) {
		Sstar_printk_err("Sstar_bh_suspend failed\n");
		Sstar_wow_resume(hw);
		return -EBUSY;
	}
	...
	if (ret) {
		Sstar_printk_err("PM request failed: %d. WoW is disabled.\n",
				 ret);
		Sstar_wow_resume(hw);
		return -EBUSY;
	}
	...
	if (atomic_read(&hw_priv->bh_rx)) {
		Sstar_printk_err("incoming event present - resume\n");
		Sstar_wow_resume(hw);
		return -EAGAIN;
	}
	return 0;
	...
	*/
	...
	Sstar_for_each_vif(hw_priv, priv, i)
	{
		...
		if ((priv->vif->type == NL80211_IFTYPE_STATION) &&
			...
			Sstar_printk_pm("should stop ap\n");
		}
	}
	mutex_unlock(&hw_priv->conf_mutex);
	up(&hw_priv->scan.lock);
			       ...
			       struct cfg80211_wowlan *wowlan)
{
	...
	if (Sstar_work_pending(&priv->join_work))
		goto revert1;
...
#ifdef MCAST_FWDING
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_AP)
		WARN_ON(wsm_set_forwarding_offlad(hw_priv, &fwdoffload,
						  priv->if_id));
#endif
	...
	state->bss_loss_tmo = Sstar_suspend_work(&priv->bss_loss_work);
	state->connection_loss_tmo =
		Sstar_suspend_work(&priv->connection_loss_work);
#endif
	state->join_tmo = Sstar_suspend_work(&priv->join_timeout);
	state->link_id_gc = Sstar_suspend_work(&priv->link_id_gc_work);
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA &&
		...
		wsm_set_beacon_wakeup_period(
			hw_priv, priv->join_dtim_period,
			SSTAR_APOLLO_BEACON_SKIPPING_MULTIPLIER *
				priv->join_dtim_period,
			priv->if_id);
	}

	ret = Sstar_timer_pending(&priv->mcast_timeout);
	if (ret)
		goto revert3;
...
revert3:
#ifndef CONFIG_TX_NO_CONFIRM
	Sstar_resume_work(hw_priv, &priv->bss_loss_work, state->bss_loss_tmo);
	Sstar_resume_work(hw_priv, &priv->connection_loss_work,
			  state->connection_loss_tmo);
#endif
	Sstar_resume_work(hw_priv, &priv->join_timeout, state->join_tmo);
	Sstar_resume_work(hw_priv, &priv->link_id_gc_work, state->link_id_gc);
	Sstar_kfree(state);
revert2:
