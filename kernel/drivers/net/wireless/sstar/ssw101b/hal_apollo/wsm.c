/*
 * WSM host interface (HI) implementation for sigmastar APOLLO mac80211 drivers.
 *
...
extern int smartconfig_start_rx(struct Sstar_common *hw_priv,
				struct sk_buff *skb, int channel);
#endif

extern void etf_v2_scan_rx(struct Sstar_common *hw_priv, struct sk_buff *skb,
			   u8 rssi);
...
#endif /*ROAM_OFFLOAD*/
#endif
//#define CONFIG_SSTAR_APOLLO_WSM_DEBUG
//#if defined(CONFIG_SSTAR_APOLLO_WSM_DEBUG)
//#define wsm_printk  printk
//#else
#define wsm_printk Sstar_printk_wsm
//#endif
...
static int get_interface_id_scanning(struct Sstar_common *hw_priv);
int wsm_write_shmem_confirm(struct Sstar_common *hw_priv,
			    struct wsm_shmem_arg_s *arg, struct wsm_buf *buf);
int wsm_read_shmem_confirm(struct Sstar_common *hw_priv,
			   struct wsm_shmem_arg_s *arg, struct wsm_buf *buf);
int wsm_efuse_change_data_confirm(struct Sstar_common *hw_priv,
				  struct wsm_buf *buf);

static int wsm_cmd_send(struct Sstar_common *hw_priv, struct wsm_buf *buf,
			void *arg, u16 cmd, long tmo, int if_id);

static struct Sstar_vif *wsm_get_interface_for_tx(struct Sstar_common *hw_priv);

/**********************/
//1: Exception  0: Normal
int wifi_run_sta = 0;
void Sstar_wifi_run_status_set(int status)
{
	wifi_run_sta = status;
	return;
}
int Sstar_wifi_run_status_get(void)
{
	return wifi_run_sta;
}
/***********************/

static inline void wsm_cmd_lock(struct Sstar_common *hw_priv)
{
	mutex_lock(&hw_priv->wsm_cmd_mux);
}

static inline void wsm_cmd_unlock(struct Sstar_common *hw_priv)
{
	mutex_unlock(&hw_priv->wsm_cmd_mux);
}
static int wsm_oper_lock_flag = 0;
static inline void wsm_oper_lock(struct Sstar_common *hw_priv)
{
...
void wsm_oper_unlock(struct Sstar_common *hw_priv)
{
...
static int wsm_generic_req_confirm(struct Sstar_common *hw_priv,
				   struct wsm_arg *arg, struct wsm_buf *buf)
{
	u32 req_id = WSM_GET32(buf);
	u32 status = WSM_GET32(buf);

	if (req_id != arg->req_id) {
		Sstar_printk_err("%s:req_id[%d][%d] err\n", __func__,
				 arg->req_id, req_id);
		return -EINVAL;
	}

	if (status != WSM_STATUS_SUCCESS) {
		Sstar_printk_err("%s:status err[%d]\n", __func__, req_id);
		return -EINVAL;
	}
...
int wsm_generic_req(struct Sstar_common *hw_priv, const struct wsm_gen_req *req,
		    void *_buf, size_t buf_size, int if_id)
{
	int ret;
	...
	ret = wsm_cmd_send(hw_priv, buf, &arg, WSM_GENERIC_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
static int wsm_stop_scan_confirm(struct Sstar_common *hw_priv, void *arg,
				 struct wsm_buf *buf)
{
	u32 status = WSM_GET32(buf);

	Sstar_printk_scan("wsm_stop_scan_confirm %x wait_complete %d\n", status,
...
static int wsm_generic_confirm(struct Sstar_common *hw_priv, void *arg,
			       struct wsm_buf *buf)
{
	u32 status = WSM_GET32(buf);
	if (status != WSM_STATUS_SUCCESS) {
...
int wsm_configuration(struct Sstar_common *hw_priv,
		      struct wsm_configuration *arg, int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, arg, WSM_CONFIGURATION_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
static int wsm_configuration_confirm(struct Sstar_common *hw_priv,
...
int wsm_reset(struct Sstar_common *hw_priv, const struct wsm_reset *arg,
	      int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
...
int wsm_read_mib(struct Sstar_common *hw_priv, u16 mibId, void *_buf,
		 size_t buf_size, int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, &mib_buf, WSM_READ_MIB_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	return ret;
...
static int wsm_read_mib_confirm(struct Sstar_common *hw_priv,
...
int wsm_write_mib(struct Sstar_common *hw_priv, u16 mibId, void *_buf,
		  size_t buf_size, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	ret = wsm_cmd_send(hw_priv, buf, &mib_buf, WSM_WRITE_MIB_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
	if (ret == -3) {
		goto disconnect;
	} else if (ret) {
...
nomem:
	Sstar_printk_err("<WARNING> wsm_write_mib fail !!! mibId=%d\n", mibId);
	wsm_cmd_unlock(hw_priv);
	return -ENOMEM;

disconnect:
	Sstar_printk_err(
		"<WARNING> wsm_write_mib fail !!! mibId=%d, HIF disconnect\n",
		mibId);
	wsm_cmd_unlock(hw_priv);
	return 0;
}

static int wsm_write_mib_confirm(struct Sstar_common *hw_priv,
				 struct wsm_mib *arg, struct wsm_buf *buf,
				 int interface_link_id)
{
	...
	if (arg->mibId == WSM_MIB_ID_OPERATIONAL_POWER_MODE) {
		...
		if (!priv)
			return 0;
		Sstar_enable_powersave(priv, (p[0] & 0x0F) ? true : false);
...
ieee80211_probereq_get_etf(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			   const u8 *ssid, size_t ssid_len, size_t total_len);
...
extern struct sk_buff *ieee80211_probereq_get_etf_for_send_result(
	struct ieee80211_hw *hw, struct ieee80211_vif *vif, const u8 *ssid,
	size_t ssid_len, size_t total_len);
#endif
int wsm_start_tx_param_set(struct Sstar_common *hw_priv,
			   struct ieee80211_vif *vif, bool start)
{
	int ret = 0;
	u32 len = 0;
	struct wsm_set_chantype arg = {
		.band = 0, //0:2.4G,1:5G
		.flag = start ? BIT(WSM_SET_CHANTYPE_FLAGS__ETF_TEST_START) :
				      0, //no use
	...
	Sstar_printk_always("etf_channel = %d etf_channel_type %d\n",
	...
	frame.skb = ieee80211_probereq_get_etf(hw_priv->hw, vif, "tttttttt", 8,
					       len);
...
int wsm_start_tx_param_set_v2(struct Sstar_common *hw_priv,
			      struct ieee80211_vif *vif, bool start)
{
	int ret = 0;
	struct wsm_set_chantype arg = {
		.flag = start ? BIT(WSM_SET_CHANTYPE_PRB_TPC) :
				      0, //probreq use tpc
	};
	...
	frame.skb = ieee80211_probereq_get_etf_v2(hw_priv->hw, vif, "tttttttt",
						  0, 1000);
	if (ret == 0)
		ret = wsm_set_template_frame(hw_priv, &frame, 0);
	if (frame.skb)
...
#ifdef CONFIG_SSTAR_PRODUCT_TEST_USE_GOLDEN_LED
int wsm_send_result_param_set(struct Sstar_common *hw_priv,
			      struct ieee80211_vif *vif, bool start)
{
	struct wsm_set_chantype arg = {
		.flag = start ? BIT(WSM_SET_CHANTYPE_PRB_TPC) :
	...
	frame.skb = ieee80211_probereq_get_etf_for_send_result(
		hw_priv->hw, vif, "tttttttt", 0, 1000);
...
#endif
int wsm_start_scan_etf(struct Sstar_common *hw_priv, struct ieee80211_vif *vif)
{
	...
	scan.autoScanInterval =
		(0xba << 24) | (30 * 1024); /* 30 seconds, -70 rssi */
...
int wsm_start_scan_etf_v2(struct Sstar_common *hw_priv,
			  struct ieee80211_vif *vif)
{
	struct wsm_scan scan;
	struct wsm_ssid ssids;
...
#ifdef CONFIG_SSTAR_PRODUCT_TEST_USE_GOLDEN_LED
int wsm_send_result_start_scan_etf(struct Sstar_common *hw_priv,
				   struct ieee80211_vif *vif)
{
	struct wsm_scan scan;
	struct wsm_ssid ssids;
...
int wsm_get_efuse_status(struct Sstar_common *hw_priv,
			 struct ieee80211_vif *vif)
{
	int remainBit = 0;
...
int wsm_start_tx(struct Sstar_common *hw_priv, struct ieee80211_vif *vif)
{
	int ret = 0;
	hw_priv->bStartTx = 1;
...
int wsm_start_tx_v2(struct Sstar_common *hw_priv, struct ieee80211_vif *vif)
{
	//	u8 CodeValue;
	int ret = 0;
	...
	ret = wsm_get_efuse_data(hw_priv, (void *)&efuse_data_etf,
...
#ifdef CONFIG_SSTAR_PRODUCT_TEST_USE_GOLDEN_LED
int wsm_send_result(struct Sstar_common *hw_priv, struct ieee80211_vif *vif)
{
	//hw_priv->bStartTx = 1;
...
int wsm_stop_tx(struct Sstar_common *hw_priv)
{
	int ret = 0;
	struct Sstar_vif *priv =
		__ABwifi_hwpriv_to_vifpriv(hw_priv, hw_priv->scan.if_id);
...
int wsm_scan(struct Sstar_common *hw_priv, const struct wsm_scan *arg,
	     int if_id)
{
	int i;
	int ret;
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_START_SCAN_REQ_ID,
			   WSM_CMD_SCAN_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	if (ret) {
		wsm_oper_unlock(hw_priv);
	}
...
int wsm_stop_scan(struct Sstar_common *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;

	wsm_cmd_lock(hw_priv);
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_STOP_SCAN_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	return ret;
}
struct Sstar_SdioCmd {
	u32 cmd_id;
	u32 lmac_seq;
	u32 hmac_seq;
	u32 data[32];
...
#ifdef SDIO_BUS
extern int Sstar_data_force_write(struct Sstar_common *hw_priv, u8 *buf, int);
//just ARESB have this function
void wsm_sync_channl_reset(struct Sstar_work_struct *work)
{
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, wsm_sync_channl);
	u32 addr = hw_priv->wsm_caps.HiHwCnfBufaddr;
	u32 buf[DOWNLOAD_BLOCK_SIZE / 4];
	struct Sstar_SdioCmd *cmd = (struct Sstar_SdioCmd *)buf;
	...
	int retry = 0;
	u8 forcePacket[16] = { 0 };
	Sstar_printk_err("wsm_recovery++\n");

	if (Sstar_bh_is_term(hw_priv)) {
		Sstar_printk_err("wsm_sync_channl start error \n");
		return;
	}
	wsm_lock_tx_async(hw_priv);
	ret = Sstar_bh_suspend(hw_priv);
	...
	wsm_tx->id = 0x55aa;
	do {
		ret = Sstar_direct_read_unlock(hw_priv, 0xab0016c, &val);
		if (ret < 0) {
			Sstar_printk_err("Error %d\n", __LINE__);
			continue;
		}
		if ((val & 0xe) == 0x0) {
			Sstar_printk_err("[%d]: 0xab0016c=%x\n", __LINE__, val);
			break;
		} else if ((val & 0xe) == 0x8) {
			//Sstar_powerave_sdio_sync(hw_priv);
			Sstar_data_force_write(hw_priv, (void *)&forcePacket[0],
					       sizeof(forcePacket) /
						       sizeof(forcePacket[0]));
			Sstar_printk_err(
				"wait LMAC clear error%d,0xab0016c=%x\n",
				__LINE__, val);
		}
		mdelay(10);
		Sstar_printk_err("wsm_sync_channl_reset 2\n");
	} while (1);
	//step 1--> Reuse softirq intr to reninitial lmac
	Sstar_direct_write_unlock(hw_priv, addr, 0);
	do {
		ret = Sstar_direct_read_unlock(hw_priv, addr, buf);
		if (cmd->cmd_id == 0) {
			Sstar_printk_err(
				"Write Mem Success by DirectMode %x=%x\n", addr,
				cmd->cmd_id);
			break;
		} else {
			Sstar_printk_err(
				"%s Write Mem Fail by DirectMode,what'happend!!! %d\n",
				__func__, __LINE__);
			//force transmit last channId
			//		frame_hexdump("force",forcePacket,16);
			//Sstar_data_force_write(hw_priv,(void*)&forcePacket[0],sizeof(forcePacket)/sizeof(forcePacket[0]));
			Sstar_direct_write_unlock(hw_priv, addr, 0);
			retry++;
		}
		if (retry > 20) {
			Sstar_printk_err("What'happend(RetryTimes) %d\n",
					 retry);
			BUG_ON(1);
		}
		mdelay(100);
	} while (1);
	Sstar_direct_read_unlock(hw_priv, 0x1610000c, &val);
	val |= BIT(0);
	Sstar_direct_write_unlock(hw_priv, 0x1610000c, val);
	do {
		//step 2-->Change directMode to read the Lmac reinital status
		ret = Sstar_direct_read_unlock(hw_priv, addr, buf);
		if (ret) {
			Sstar_printk_err(
				"Direct Mode ReadErr, what'happend !!!\n");
		}
		...
		if (cmd->cmd_id == 0xffffabcd) {
			...
			hw_priv->buf_id_tx = 0;
			ret = Sstar_direct_read_unlock(hw_priv, 0xab00160,
						       &val);
	...
	Sstar_bh_resume(hw_priv);
	wake_up(&hw_priv->wsm_synchanl_done);
	Sstar_printk_err("wsm_sync_channl reset end \n");
}
#endif

#ifdef SSTAR_SDIO_PATCH
static int TRY_UP_LOCK(struct Sstar_common *hw_priv)
{
#ifndef OPER_CLOCK_USE_SEM
...
void wsm_sync_channl(struct Sstar_work_struct *work)
{
	int ret;
	u16 reg;
	int retry = 0;
	...
	Sstar_printk_err("wsm_sync_channl start \n");
	while (retry <= MAX_RETRY) {
		ret = Sstar_reg_read_16(hw_priv, SSTAR_HIFREG_CONTROL_REG_ID,
					&reg);
		if (!ret) {
			if (reg & SSTAR_HIFREG_CONT_WUP_BIT) {
				ret = Sstar_reg_write_16(
					hw_priv, SSTAR_HIFREG_CONTROL_REG_ID,
					(u16)(~SSTAR_HIFREG_CONT_WUP_BIT));
			} else {
				ret = Sstar_reg_write_16(
					hw_priv, SSTAR_HIFREG_CONTROL_REG_ID,
					(u16)SSTAR_HIFREG_CONT_WUP_BIT);
			...
			retry++;
			Sstar_printk_err(
				"Read CONTROL reg  Error ,Retry =%d\n ", retry);
		}
	}
#if 0
	hw_priv->hw_bufs_used=0;
	...
	Sstar_printk_err("wsm_sync_channl end (%d)\n", retry);
}
int Sstar_release_hmac_buffer(struct Sstar_common *hw_priv, u32 packetID,
			      u32 status)
{
	...
	tx_confirm.if_id = Sstar_queue_get_if_id(packetID);
	tx_confirm.link_id = Sstar_queue_get_link_id(packetID);
...
static int wsm_channel_indication(struct Sstar_common *hw_priv,
				  struct wsm_buf *buf)
{
	u32 channle;

	channle = WSM_GET32(buf);
	Sstar_printk_debug("Lmac chanle =%d\n", channle);
	...
	if (hw_priv->dataFlag == IS_CMD) {
		Sstar_printk_err("Here cmd Repeat\n");
		spin_lock_bh(&hw_priv->wsm_cmd.lock);
	...
	return 0;
underflow:
	Sstar_printk_err("wsm_channel_indication:EINVAL\n");
	return -EINVAL;
}
static int wsm_tx_release_bufused(struct Sstar_common *hw_priv,
				  struct wsm_buf *buf, int interface_link_id)
{
	...
	list_for_each_entry_safe (bitmap, tmp, &hw_priv->SeqBitMapList, link) {
		if (loop == 0) {
			TxNum = wsm_calSeqNumOffset(tx_released.LmacEsn,
						    bitmap->bitm.HmacSsn);
		}
		if ((loop) > TxNum) {
			break;
		...
		Offset = wsm_calSeqNumOffset(bitmap->bitm.HmacSsn,
					     tx_released.LmacSsn);
		//sucess tx bitmap
		if (Offset < 64 && ((BitMap_l & ((1) << (Offset))) ||
				    BitMap_h & 1 << (Offset - 32))) {
			if (bitmap->bitm.DataFlag == IS_DATA) { //data
				Sstar_release_hmac_buffer(hw_priv,
							  bitmap->bitm.packetId,
							  WSM_DATA_CRC_ERRO);
				//del the linkNode
			...
			Sstar_printk_err("HmacSsn->LmacEsn = %d:%d Fail\n",
					 bitmap->bitm.HmacSsn,
					 tx_released.LmacEsn);
			//lock tx bh
			...
			list_for_each_entry_safe (
				bitmap, tmp, &hw_priv->SeqBitMapList, link) {
				if (bitmap->bitm.DataFlag ==
				    IS_DATA) { //data release
					Sstar_printk_err(
						"Here data Release %d\n",
						bitmap->bitm.HmacSsn);
					Sstar_release_hmac_buffer(
						hw_priv, bitmap->bitm.packetId,
						WSM_DATA_CRC_ERRO);
				} else {
					Sstar_printk_err(" ToDo cmd Repeat\n");
					hw_priv->dataFlag =
						bitmap->bitm.DataFlag;
			...
			//sync channle
			wsm_lock_tx_async(hw_priv);
			if (Sstar_hw_priv_queue_work(
				    hw_priv, &hw_priv->wsm_sync_channl) <= 0) {
				wsm_unlock_tx(hw_priv);
			}
		}
	}
...
static int wsm_tx_release_cnt(struct Sstar_common *hw_priv, struct wsm_buf *buf,
			      int interface_link_id)
{
	int count;
	count = WSM_GET32(buf);
...
static int wsm_tx_confirm(struct Sstar_common *hw_priv, struct wsm_buf *buf,
			  int interface_link_id)
{
	int packetId;
	struct Sstar_seq_bit_map *bitmap = NULL, *tmp = NULL;
	...
	spin_lock_bh(&hw_priv->SeqBitMapLock);
	list_for_each_entry_safe (bitmap, tmp, &hw_priv->SeqBitMapList, link) {
		packetId = Sstar_seq_to_packetId(hw_priv, bitmap->bitm.HmacSsn);
		//find the packId
			  ...
			  int interface_link_id)
{
	...
	tx_confirm.if_id = Sstar_queue_get_if_id(tx_confirm.packetID);
	tx_confirm.link_id = Sstar_queue_get_link_id(tx_confirm.packetID);
	spin_lock_bh(&hw_priv->tx_com_lock);
...
#endif
static int wsm_multi_tx_confirm(struct Sstar_common *hw_priv,
				struct wsm_buf *buf, int interface_link_id)
{
	struct Sstar_vif *priv;
	int ret;
	...
	priv = ABwifi_hwpriv_to_vifpriv(hw_priv, interface_link_id);
	if (priv) {
		Sstar_debug_txed_multi(priv, count);
		Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
	}
	for (i = 0; i < count; ++i) {
...
int wsm_set_rate_power(struct Sstar_common *hw_priv, int use_flag)
{
	...
	if (get_rate_delta_gain(&rate_txpower[0]) == 0) {
		...
		rate_txpower[11] = use_flag;
		{
			if (hw_priv->wsm_caps.firmwareVersion > 12040)
				err = wsm_write_mib(
					hw_priv, WSM_MIB_ID_SET_RATE_TX_POWER,
					rate_txpower, sizeof(rate_txpower), 0);
			else
				err = wsm_write_mib(
					hw_priv, WSM_MIB_ID_SET_RATE_TX_POWER,
					rate_txpower, 12, 0);
...
static int wsm_join_confirm(struct Sstar_common *hw_priv, struct wsm_join *arg,
			    struct wsm_buf *buf)
{
	if (WSM_GET32(buf) != WSM_STATUS_SUCCESS) {
		Sstar_printk_err("<SSTAR WIFI>: connect FAIL \n");
		return -EINVAL;
	}
...
int wsm_join(struct Sstar_common *hw_priv, struct wsm_join *arg, int if_id)
/*TODO: combo: make it work per vif.*/
{
	...
	ret = wsm_cmd_send(hw_priv, buf, arg, WSM_JOIN_REQ_ID,
			   WSM_CMD_JOIN_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	wsm_oper_unlock(hw_priv);
	return ret;
...
int wsm_set_bss_params(struct Sstar_common *hw_priv,
		       const struct wsm_set_bss_params *arg, int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_SET_BSS_PARAMS_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
int wsm_add_key(struct Sstar_common *hw_priv, const struct wsm_add_key *arg,
		int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_ADD_KEY_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
/* ******************************************************************** */
#if 0
int wsm_remove_key(struct Sstar_common *hw_priv,
...
#else
int wsm_remove_key(struct Sstar_common *hw_priv, const struct wsm_add_key *arg,
		   int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_REMOVE_KEY_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
int wsm_set_tx_queue_params(struct Sstar_common *hw_priv,
			    ...
			    int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_QUEUE_PARAMS_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
int wsm_set_edca_params(struct Sstar_common *hw_priv,
			const struct wsm_edca_params *arg, int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_EDCA_PARAMS_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	return ret;
...
/* ******************************************************************** */
#if 0
int wsm_switch_channel(struct Sstar_common *hw_priv,
...
void Sstar_pm_timer(unsigned long arg)
{
	...
	if (atomic_read(&hw_priv->wsm_pm_running) == 1) {
		...
		Sstar_printk_err("%s:pm timeout\n", __func__);
	}
	spin_unlock_bh(&hw_priv->wsm_pm_spin_lock);
}
#endif
static void Sstar_pm_timer_setup(struct Sstar_common *hw_priv)
{
#ifdef OPER_CLOCK_USE_SEM
	spin_lock_bh(&hw_priv->wsm_pm_spin_lock);
	atomic_set(&hw_priv->wsm_pm_running, 1);
	Sstar_mod_timer(&hw_priv->wsm_pm_timer, jiffies + 2 * HZ);
...
static void Sstar_pm_timer_cancle(struct Sstar_common *hw_priv)
{
#ifdef OPER_CLOCK_USE_SEM
	spin_lock_bh(&hw_priv->wsm_pm_spin_lock);
	atomic_set(&hw_priv->wsm_pm_running, 0);
	Sstar_del_timer(&hw_priv->wsm_pm_timer);
	Sstar_release_suspend(hw_priv);
	spin_unlock_bh(&hw_priv->wsm_pm_spin_lock);
#else
	BUG_ON(hw_priv == NULL);
...
/* ******************************************************************** */
void wsm_wait_pm_sync(struct Sstar_common *hw_priv)
{
	wsm_oper_lock(hw_priv);
	/*
	*wait pm indication
	*/
	wsm_oper_unlock(hw_priv);
	Sstar_printk_err("%s:complete\n", __func__);
}
int wsm_set_pm(struct Sstar_common *hw_priv, const struct wsm_set_pm *arg,
	       int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	Sstar_printk_err(
		"%s:pmMode:%d,fastPsmIdlePeriod:%d,apPsmChangePeriod:%d,minAutoPsPollPeriod:%d\n",
		__func__, arg->pmMode, arg->fastPsmIdlePeriod,
		arg->apPsmChangePeriod, arg->minAutoPsPollPeriod);

	Sstar_pm_timer_setup(hw_priv);
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_SET_PM_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);

	wsm_cmd_unlock(hw_priv);
	if (ret) {
		Sstar_pm_timer_cancle(hw_priv);
		wsm_oper_unlock(hw_priv);
	}
	return ret;
...
int wsm_start(struct Sstar_common *hw_priv, const struct wsm_start *arg,
	      int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
...
#endif
	hw_priv->tx_burst_idx = -1;
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_START_REQ_ID,
			   WSM_CMD_START_TIMEOUT, if_id);
...
*
int wsm_start_find(struct Sstar_common *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
...
int wsm_stop_find(struct Sstar_common *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;

	wsm_cmd_lock(hw_priv);
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_STOP_FIND_ID,
			   WSM_CMD_TIMEOUT, if_id);
	wsm_cmd_unlock(hw_priv);
	return ret;
}
...
int wsm_map_link(struct Sstar_common *hw_priv, const struct wsm_map_link *arg,
		 int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
...
int wsm_update_ie(struct Sstar_common *hw_priv, const struct wsm_update_ie *arg,
		  int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_UPDATE_IE_REQ_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
#ifdef MCAST_FWDING
/* 3.66 */
static int wsm_give_buffer_confirm(struct Sstar_common *hw_priv,
				   struct wsm_buf *buf)
{
...
/* 3.65 */
int wsm_init_release_buffer_request(struct Sstar_common *hw_priv, u8 index)
{
...
/* 3.68 */
static int wsm_request_buffer_confirm(struct Sstar_vif *priv, u8 *arg,
				      struct wsm_buf *buf)
{
	u8 count;
	...
	wsm_printk("CM %x, HM %x, FWM %x\n", change_mask, priv->sta_asleep_mask,
		   sta_asleep_mask);
	...
	if (change_mask) {
		...
		for (i = 0; i < SSTARWIFI_MAX_STA_IN_AP_MODE; ++i) {
			if (SSTAR_APOLLO_LINK_HARD !=
			...
			if (change_mask & mask) {
				...
				sta = ieee80211_find_sta(
					priv->vif, priv->link_id_db[i].mac);
				if (!sta) {
					wsm_printk(
						"[WSM] WRBC - could not find sta %pM\n",
						priv->link_id_db[i].mac);
				} else {
					ret = ieee80211_sta_ps_transition_ni(
						sta, (sta_asleep_mask & mask) ?
								   true :
								   false);
...
/* 3.67 */
int wsm_request_buffer_request(struct Sstar_vif *priv, u8 *arg)
{
	int ret;
	...
	ret = wsm_cmd_send(priv->hw_priv, buf, arg, WSM_REQUEST_BUFFER_REQ_ID,
...
int wsm_set_keepalive_filter(struct Sstar_vif *priv, bool enable)
{
...
#endif
int wsm_set_probe_responder(struct Sstar_vif *priv, bool enable)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
...
static int wsm_startup_indication(struct Sstar_common *hw_priv,
				  struct wsm_buf *buf)
{
	u16 status;
	u16 Resv;
	...
	Sstar_printk_init("firmwareCap %x\n", hw_priv->wsm_caps.firmwareCap);
	...
	if (buf->data + 2 <= buf->end) {
		Resv = WSM_GET16(buf);
		Sstar_printk_debug("wsm_startup_indication : resv[%d] \n",
				   Resv);
	}
	if (buf->data + 2 <= buf->end)
	...
	Sstar_printk_init("firmwareCap2 %x\n", firmwareCap2);
	...
	Sstar_printk_init("wsm_caps.firmwareCap %x",
			  hw_priv->wsm_caps.firmwareCap);
	/*
...
*/
	...
	Sstar_printk_init(
		...
		hw_priv->wsm_caps.numInpChBufs, hw_priv->wsm_caps.sizeInpChBuf,
		hw_priv->wsm_caps.hardwareId, hw_priv->wsm_caps.hardwareSubId,
		fw_types[hw_priv->wsm_caps.firmwareType], &fw_label[0],
		hw_priv->wsm_caps.firmwareVersion,
		hw_priv->wsm_caps.firmwareBuildNumber,
		hw_priv->wsm_caps.firmwareApiVer, hw_priv->wsm_caps.firmwareCap,
		Config[0], Config[1], Config[2],
		hw_priv->wsm_caps.NumOfStations,
		hw_priv->wsm_caps.NumOfInterfaces);
	BUG_ON(hw_priv->wsm_caps.NumOfStations == 0);
	BUG_ON(hw_priv->wsm_caps.NumOfStations > SSTARWIFI_MAX_STA_IN_AP_MODE);
	hw_priv->wsm_caps.firmwareReady = 1;
	hw_priv->wsm_caps.exceptionaddr = Config[1];
	hw_priv->wsm_caps.HiHwCnfBufaddr = Config[2]; //ep0 addr
#if (PROJ_TYPE >= ARES_B)
	Sstar_printk_init("EFUSE(8)				[%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_EFUSE8));
	Sstar_printk_init("EFUSE(I)					[%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_EFUSEI));
	Sstar_printk_init("EFUSE(B)			[%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_EFUSEB));
#endif
	Sstar_printk_init("CAPABILITIES_SSTAR_PRIVATE_IE      [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_SSTAR_PRIVATE_IE));
	Sstar_printk_init("CAPABILITIES_NVR_IPC              [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_NVR_IPC));
	Sstar_printk_init("CAPABILITIES_NO_CONFIRM           [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_NO_CONFIRM));
	Sstar_printk_init("CAPABILITIES_SDIO_PATCH           [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_SDIO_PATCH));
	Sstar_printk_init("CAPABILITIES_NO_BACKOFF           [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_NO_BACKOFF));
	Sstar_printk_init("CAPABILITIES_CFO                  [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap & CAPABILITIES_CFO));
	Sstar_printk_init("CAPABILITIES_AGC                  [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap & CAPABILITIES_AGC));
	Sstar_printk_init("CAPABILITIES_TXCAL                [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_TXCAL));
	Sstar_printk_init("CAPABILITIES_MONITOR              [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_MONITOR));
	Sstar_printk_init("CAPABILITIES_CUSTOM               [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_CUSTOM));
	Sstar_printk_init("CAPABILITIES_SMARTCONFIG          [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_SMARTCONFIG));
	Sstar_printk_init("CAPABILITIES_ETF                  [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap & CAPABILITIES_ETF));
	Sstar_printk_init("CAPABILITIES_LMAC_RATECTL         [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_LMAC_RATECTL));
	Sstar_printk_init("CAPABILITIES_LMAC_TPC             [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_LMAC_TPC));
	Sstar_printk_init("CAPABILITIES_LMAC_TEMPC           [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_LMAC_TEMPC));
	Sstar_printk_init("CAPABILITIES_CTS_BUG              [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_CTS_BUG));
	Sstar_printk_init("CAPABILITIES_USB_RECOVERY_BUG     [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_USB_RECOVERY_BUG));
	Sstar_printk_init("CAPABILITIES_USE_IPC              [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_USE_IPC));
	Sstar_printk_init("CAPABILITIES_OUTER_PA             [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_OUTER_PA));
	Sstar_printk_init("CAPABILITIES_POWER_CONSUMPTION    [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_POWER_CONSUMPTION));
	Sstar_printk_init("CAPABILITIES_RSSI_DECIDE_TXPOWER  [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_RSSI_DECIDE_TXPOWER));
	Sstar_printk_init("CAPABILITIES_RTS_LONG_DURATION    [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_RTS_LONG_DURATION));
	Sstar_printk_init("CAPABILITIES_TX_CFO_PPM_CORRECTION[%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_TX_CFO_PPM_CORRECTION));
#if (PROJ_TYPE >= ARES_B)
	Sstar_printk_init("CAPABILITIES_SHARE_CRYSTAL       [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_NOISE_SET_DCXO));
#else
	Sstar_printk_init("CAPABILITIES_NOISE_SET_DCXO       [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_NOISE_SET_DCXO));
#endif
	Sstar_printk_init("CAPABILITIES_HW_CHECKSUM          [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_HW_CHECKSUM));
	Sstar_printk_init("CAPABILITIES_SINGLE_CHANNEL_MULRX [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_SINGLE_CHANNEL_MULTI_RX));
#if (PROJ_TYPE >= ARES_B)
	Sstar_printk_init("CAPABILITIES_CFO_DCXO_CORRECTION  [%d]\n",
			  !!(hw_priv->wsm_caps.firmwareCap &
			     CAPABILITIES_CFO_DCXO_CORRECTION));
#endif
	Sstar_printk_init("EX_CAPABILITIES_TWO_CHIP_ONE_SOC	[%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_TWO_CHIP_ONE_SOC));
	Sstar_printk_init("EX_CAPABILITIES_MANUAL_SET_AC 	[%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_MANUAL_SET_AC));
	Sstar_printk_init("EX_CAPABILITIES_LMAC_BW_CONTROL	[%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_LMAC_BW_CONTROL));
	Sstar_printk_init("EX_CAPABILITIES_SUPPORT_TWO_ANTENNA	[%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_SUPPORT_TWO_ANTENNA));
	Sstar_printk_init("EX_CAPABILITIES_ENABLE_STA_REMAIN_ON_CHANNEL	[%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_ENABLE_STA_REMAIN_ON_CHANNEL));
	Sstar_printk_init("EX_CAPABILITIES_ENABLE_PS 	   [%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_ENABLE_PS));
	Sstar_printk_init("EX_CAPABILITIES_TX_REQUEST_FIFO_LINK 	   [%d]",
			  !!(hw_priv->wsm_caps.firmeareExCap &
			     EX_CAPABILITIES_TX_REQUEST_FIFO_LINK));

	if (1 ==
	    ((hw_priv->wsm_caps.firmeareExCap & EX_CAPABILITIES_CHIP_TYPE) >>
	     7))
		Sstar_printk_init(
			"EX_CAPABILITIES_CHIP_TYPE		101B2\n");
	else if (2 == ((hw_priv->wsm_caps.firmeareExCap &
			EX_CAPABILITIES_CHIP_TYPE) >>
		       7))
		Sstar_printk_init(
			"EX_CAPABILITIES_CHIP_TYPE		101B\n");
	else if (3 == ((hw_priv->wsm_caps.firmeareExCap &
			EX_CAPABILITIES_CHIP_TYPE) >>
		       7))
		Sstar_printk_init("EX_CAPABILITIES_CHIP_TYPE		\n");
#ifdef CONFIG_TX_NO_CONFIRM
	if ((hw_priv->wsm_caps.firmwareCap & CAPABILITIES_NO_CONFIRM) == 0) {
		Sstar_printk_init("LMAC NOT CAPABILITIES_NO_CONFIRM <ERROR>\n");
		BUG_ON(1);
	}
#else
	if ((hw_priv->wsm_caps.firmwareCap & CAPABILITIES_NO_CONFIRM)) {
		Sstar_printk_init("LMAC SET CAPABILITIES_NO_CONFIRM <ERROR>\n");
		BUG_ON(1);
	}
#endif
	...
	if ((hw_priv->wsm_caps.firmwareCap & CAPABILITIES_VIFADDR_LOCAL_BIT) ==
	    0) {
		Sstar_printk_init(
			"LMAC NOT CAPABILITIES_VIFADDR_LOCAL_BIT <ERROR>\n");
		//		BUG_ON(1);
	}
#else
	if ((hw_priv->wsm_caps.firmwareCap & CAPABILITIES_VIFADDR_LOCAL_BIT)) {
		Sstar_printk_init(
			"LMAC SET CAPABILITIES_VIFADDR_LOCAL_BIT <ERROR>\n");
		//		BUG_ON(1);
	}
#endif

#ifdef SSTAR_PRODUCT_TEST_USE_FEATURE_ID
	Sstar_printk_init("CONFIG_PRODUCT_TEST_USE_FEATURE_ID [1]\n");
#else
	Sstar_printk_init("CONFIG_PRODUCT_TEST_USE_FEATURE_ID [0]\n");
#endif
#ifdef CONFIG_SSTAR_PRODUCT_TEST_USE_GOLDEN_LED
	Sstar_printk_init("CONFIG_PRODUCT_TEST_USE_GOLDEN_LED [1]\n");
#else
	Sstar_printk_init("CONFIG_PRODUCT_TEST_USE_GOLDEN_LED [0]\n");
#endif
...
static int wsm_debug_print_indication(struct Sstar_common *hw_priv,
				      struct wsm_buf *buf)
{
	...
	Sstar_printk_lmac("[lmac]:%s", fw_debug_print);
	return 0;
underflow:
	Sstar_printk_err("wsm_debug_print_indication:EINVAL\n");
	return -EINVAL;
}

static int wsm_smartconfig_indication(struct Sstar_common *hw_priv,
				      int interface_link_id,
				      struct wsm_buf *buf,
				      struct sk_buff **skb_p)
{
	...
	Sstar_skb_pull(*skb_p, hdr_len);
	length_bak = (*skb_p)->len;
	(*skb_p)->len = length;
	data = (*skb_p)->data;
	Sstar_printk_smt(
		"chann=%d,channelType=%d,packNum=%d,rate=%d,len=%d,rx_status0_start=%d,mac=%02x:%02x:%02x:%02x:%02x:%02x\n",
		channelNum, channelType, packNum, rate, length,
		rx_status0_start, data[4], data[5], data[6], data[7], data[8],
		data[9]);
	smartconfig_start_rx(hw_priv, *skb_p, channelNum);
	if (*skb_p) {
		(*skb_p)->len = length_bak;
		Sstar_skb_push(*skb_p, hdr_len);
	}
underflow:
#endif //#ifdef SSTAR_SUPPORT_SMARTCONFIG
#endif //#if (PROJ_TYPE==ARES_A)
	return 0;
}
static int wsm_receive_indication(struct Sstar_common *hw_priv,
				  ...
				  struct sk_buff **skb_p)
{
	...
	if (hw_priv->wsm_cbc.rx) {
		...
		* with interface ID == 2 */
		if (interface_link_id == SSTAR_WIFI_GENERIC_IF_ID) {
			...
			if (hw_priv->auto_scanning) {
				...
				Sstar_printk_scan("%s %d if_id=%d\n", __func__,
						  __LINE__, interface_link_id);
			}
#endif /*ROAM_OFFLOAD*/
#endif
		}
		...
		if (!rx.rcpiRssi &&
		    (ieee80211_is_probe_resp(hdr->frame_control) ||
		     ieee80211_is_beacon(hdr->frame_control))) {
			Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
			Sstar_printk_err("rcpiRssi is zero\n");
			return 0;
		}
#endif
#ifndef CONFIG_RATE_HW_CONTROL
		...
		mac = hdr->addr2;
		rcu_read_lock();
		sta = (struct sta_info *)sta_info_get_rx(
			vif_to_sdata(priv->vif), mac);
		if (sta != NULL) {
	...
	//	}
						rc_sta_info->total_rssi =
							rc_sta_info->total_rssi +
							rssi;

						rc_sta_info->rssi_count++;

						if (rc_sta_info->min_rssi >
						    rssi)
							rc_sta_info->min_rssi =
								rssi;

						if (rc_sta_info->max_rssi <
						    rssi)
							rc_sta_info->max_rssi =
								rssi;
					}
				}
			} else {
				Sstar_printk_rx(
					"wsm_receive_indication: msp is NULL\n");
			}
		}
		rcu_read_unlock();
#endif
		...
		* convert RCPI to RSSI here */
		...
		if (!rx.status && unlikely(ieee80211_is_deauth(fctl))) {
			if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA) {
				...
				if (memcmp(priv->join_bssid, hdr->addr3,
					   ETH_ALEN) == 0) {
					do_unjoin = true;
				}

				rcu_read_lock();
				mfp_sta = (struct sta_info *)sta_info_get_rx(
					vif_to_sdata(priv->vif), hdr->addr2);
				if (mfp_sta &&
				    test_sta_flag(mfp_sta, WLAN_STA_MFP) &&
				...
				if (do_unjoin == true) {
					...
					if (report == false) {
						Sstar_priv_vif_list_read_unlock(
							&priv->vif_lock);
						Sstar_printk_err(
							"unknown bssid[%pM]\n",
							hdr->addr3);
		...
		if (hw_priv->sbus_ops->sbus_rev_giveback)
			hw_priv->sbus_ops->sbus_rev_giveback(hw_priv->sbus_priv,
							     &rx);
...
static int wsm_event_indication(struct Sstar_common *hw_priv,
				struct wsm_buf *buf, int interface_link_id)
{
	int first;
	struct Sstar_wsm_event *event = NULL;
	struct Sstar_vif *priv;
	...
	event = Sstar_kzalloc(sizeof(struct Sstar_wsm_event), GFP_KERNEL);
#else
	event = Sstar_kzalloc(sizeof(struct Sstar_wsm_event), GFP_ATOMIC);
#endif
	...
	if (first)
		Sstar_hw_priv_queue_work(hw_priv, &hw_priv->event_handler);
...
static int wsm_channel_switch_indication(struct Sstar_common *hw_priv,
						struct wsm_buf *buf)
{
...
#endif
static int wsm_set_pm_indication(struct Sstar_common *hw_priv,
				 struct wsm_buf *buf)
{
	Sstar_printk_pm("[PM]:rev ind\n");
#ifdef OPER_CLOCK_USE_SEM
	spin_lock_bh(&hw_priv->wsm_pm_spin_lock);
	if (atomic_read(&hw_priv->wsm_pm_running) == 1) {
		atomic_set(&hw_priv->wsm_pm_running, 0);
...
static int wsm_scan_complete_indication(struct Sstar_common *hw_priv,
					struct wsm_buf *buf)
{
...
static int wsm_find_complete_indication(struct Sstar_common *hw_priv,
					struct wsm_buf *buf)
{
	/* TODO: Implement me. */
	//STUB();
...
static int wsm_suspend_resume_indication(struct Sstar_common *hw_priv,
					 int interface_link_id,
					 struct wsm_buf *buf)
{
...
static int wsm_debug_indication(struct Sstar_common *hw_priv,
				struct wsm_buf *buf)
{
...
#if (PROJ_TYPE < ARES_B)
int wsm_recovery_chip_athena(struct Sstar_common *hw_priv)
{
	if (Sstar_bh_is_term(hw_priv)) {
	...
	Sstar_bh_halt(hw_priv);
	return RECOVERY_BH_HALT;
error:
	return RECOVERY_ERR;
}
#else
int wsm_recovery_chip_ares(struct Sstar_common *hw_priv)
{
	u32 addr = hw_priv->wsm_caps.HiHwCnfBufaddr;
	int ret = 0;
	u32 buf[DOWNLOAD_BLOCK_SIZE / 4];
	int tx_size = 12;
	struct Sstar_EP0Cmd *cmd = (struct Sstar_EP0Cmd *)buf;

	Sstar_wifi_run_status_set(1);

	if (Sstar_bh_is_term(hw_priv)) {
		Sstar_printk_err("wsm_recovery: bh is stop\n");
		return RECOVERY_ERR;
	}
	mdelay(100);
	__Sstar_usb_suspend(hw_priv->sbus_priv);
	wsm_lock_tx_async(hw_priv);
	{
		//lmac may stuck,we need reinitial it
		Sstar_printk_err("step2 lmac may stuck,we need reinitial it\n");
		//printk("wsm_recovery EP0_CMD_RECOVERY %x buf %x tx_size %x addr %x\n",cmd->cmd_id,buf,tx_size,addr);
		mdelay(100);
		Sstar_usb_kill_all_txurb(hw_priv->sbus_priv);
		Sstar_usb_kill_all_rxurb(hw_priv->sbus_priv);

		Sstar_printk_err("wms_recoverycmd_id ARES_B \n");
		Sstar_usb_ep0_hw_reset_cmd(hw_priv->sbus_priv, HW_RESET_HIF, 1);
		hw_priv->wsm_tx_seq = 0;
...
#define USB_RESET 0
		...
		if (!ret) {
			mdelay(1000);
			Sstar_usb_ep0_hw_reset_cmd(hw_priv->sbus_priv,
						   HW_RESET_HIF, 1);
		}
#endif
		mdelay(1000);
		{
			int retryCont = 0;
			while (1) {
				//printk("wsm_recovery Sstar_ep0_read\n");
				Sstar_ep0_read(hw_priv, addr, buf, 4);
				if (cmd->cmd_id == 0xffffabcd) {
					Sstar_printk_err(
						"wsm_recovery ARES_B cmd->cmd_id %x\n",
						cmd->cmd_id);
					break;
				}
				mdelay(100);
				...
				if (retryCont > 20) {
					Sstar_printk_err(
						"wsm_recovery ARES_B Sstar_ep0_read retryCont timeout\n");
					break;
				}
			}
		}
		if (cmd->cmd_id == 0xffffabcd) {
			Sstar_ep0_read(hw_priv, addr + 4, buf + 1, 4);
			Sstar_ep0_read(hw_priv, addr + 8, buf + 2, 4);
			Sstar_ep0_read(hw_priv, addr + 12, buf + 3, 4);
			Sstar_ep0_read(hw_priv, addr + 16, buf + 4, 4);
			Sstar_ep0_read(hw_priv, addr + 20, buf + 5, 4);
			Sstar_printk_err(
				"wsm_recovery hiReq %d,hiConf %d,lmaclastcmd %x\n",
			...
			mdelay(1000);
			wsm_unlock_tx_async(hw_priv);
			__Sstar_usb_resume(hw_priv->sbus_priv);
...
#endif
int wsm_recovery(struct Sstar_common *hw_priv)
{
#if (PROJ_TYPE < ARES_B)
...
int wsm_recovery_done(struct Sstar_common *hw_priv, int type)
{
	return RECOVERY_STEP2_SUCCESS;
}
#else
int wsm_sync_channle_process(struct Sstar_common *hw_priv, int type)
{
	int retFlush;
...
int wsm_recovery(struct Sstar_common *hw_priv)
{
	...
	//sync channle
	hw_priv->syncChanl_done = 0;
	Sstar_schedule_work(&hw_priv->wsm_sync_channl);
	return RECOVERY_STEP2_SUCCESS;
}
int wsm_recovery_done(struct Sstar_common *hw_priv, int type)
{
	...
	if (type == OUT_BH) {
		status = wait_event_interruptible_timeout(
			hw_priv->wsm_synchanl_done, hw_priv->syncChanl_done,
			120 * HZ);
	} else {
		status = wait_event_interruptible_timeout(
			hw_priv->bh_wq,
	...
	if (status <= 0) {
		Sstar_printk_err("sync Channle timeout,what happend !!!\n");
		return RECOVERY_ERR;
	}
	return RECOVERY_STEP2_SUCCESS;
}
...
/* ******************************************************************** */
/* WSM TX								*/
static void wsm_cmd_hif_ximt(struct Sstar_common *hw_priv)
{
...
int wsm_cmd_send(struct Sstar_common *hw_priv, struct wsm_buf *buf, void *arg,
		 u16 cmd, long tmo, int if_id)
{
	...
	int ret;
	if (Sstar_bh_is_term(hw_priv)) {
		Sstar_printk_err("bh_thread %p,bh_error %d pluged %d\n",
				 (hw_priv->bh_thread), (hw_priv->bh_error),
				 (atomic_read(&hw_priv->Sstar_pluged)));
		wsm_buf_reset(buf);
	...
	if (cmd == 0x0006) /* Write MIB */
		wsm_printk(
			"[WSM] >>> 0x%.4X [MIB: 0x%.4X] (%ld)\n", cmd,
			__le16_to_cpu((
				(__le16 *)buf
					->begin)[sizeof(struct wsm_hdr_tx) / 2]),
			buf_len);
	else
...
TxCmdAgain:
	...
	Sstar_ep0_write_cmd(hw_priv, wsm_h);
#else
	wsm_cmd_hif_ximt(hw_priv);
#endif
	if (unlikely(hw_priv->bh_error) || Sstar_bh_is_term(hw_priv)) {
		...
		do {
			...
			if (hw_priv->sdio_status == -1) {
				ret = Sstar_wait_event_timeout_stay_awake(
					hw_priv, hw_priv->wsm_cmd_wq,
					hw_priv->wsm_cmd.done, 100, true);
			} else {
				ret = Sstar_wait_event_timeout_stay_awake(
					hw_priv, hw_priv->wsm_cmd_wq,
					hw_priv->wsm_cmd.done, tmo, true);
			}
			...
			if (!ret && wsm_cmd_runtime < wsm_cmd_max_tmo &&
	...
	if (unlikely(ret == 0)) {
		int ret_flush;
		Sstar_printk_err(
			"wsm_cmd_send cmd(%x) not send to fw wsm_recovery last_send_cmd %x\n",
		...
		Sstar_printk_err("wsm_cmd_send,buffused(%d) ret_flush %x\n",
				 hw_priv->hw_bufs_used, ret_flush);
		if ((ret_flush != RECOVERY_ERR) &&
		    (ret_flush != RECOVERY_BH_HALT)) {
			ret = Sstar_wait_event_timeout_stay_awake(
				hw_priv, hw_priv->wsm_cmd_wq,
				hw_priv->wsm_cmd.done, 3 * HZ, true);

			if ((ret == 0) && (hw_priv->wsm_cmd.ptr == NULL) &&
			    (hw_priv->wsm_cmd.cmd != 0xFFFF)) {
				Sstar_printk_err(
					"wsm_cmd_send cmd(%x) retry to fw\n",
					hw_priv->wsm_cmd.cmd);
				spin_lock_bh(&hw_priv->wsm_cmd.lock);
				//hw_priv->hw_bufs_used = 0;
				//hw_priv->hw_bufs_used_vif[0] = 0;
				...
				ret = Sstar_wait_event_timeout_stay_awake(
					...
					WSM_CMD_DEFAULT_TIMEOUT, true);
			} else {
				//(hw_priv->wsm_cmd.ptr==NULL)
				//bh_wakeup
				Sstar_printk_err(
					"wsm_cmd_send cmd(%x) retry to wakeup usb to send ptr %p\n",
					hw_priv->wsm_cmd.cmd,
					hw_priv->wsm_cmd.ptr);
				wsm_cmd_hif_ximt(hw_priv);
				ret = Sstar_wait_event_timeout_stay_awake(
	...
	if (unlikely(ret == 0)) {
		...
		if (hw_priv->wsm_cmd.ptr) {
			Sstar_printk_err("wsm_cmd_send cmd not send to fw\n");
		}
		Sstar_printk_err("wsm_cmd_send,buffused(%d)\n",
				 hw_priv->hw_bufs_used);
		hw_priv->wsm_cmd.arg = NULL;
		hw_priv->wsm_cmd.ptr = NULL;
		spin_unlock_bh(&hw_priv->wsm_cmd.lock);
		...
		if (raceCheck == 0xFFFF) {
			...
			BUG_ON(Sstar_wait_event_timeout_stay_awake(
				       hw_priv, hw_priv->wsm_cmd_wq,
				       hw_priv->wsm_cmd.done,
				       WSM_CMD_LAST_CHANCE_TIMEOUT, true) <= 0);
		}
		Sstar_printk_err("wsm_cmd_send timeout cmd %x tmo %ld\n", cmd,
				 tmo);

#ifdef SSTAR_SDIO_PATCH
		//sync channle
		wsm_lock_tx_async(hw_priv);
		//Maybe Sstar workqueue die,Here use linux default workqueue
		Sstar_schedule_work(&hw_priv->wsm_sync_channl);
		wait_event_interruptible(hw_priv->wsm_synchanl_done,
					 hw_priv->syncChanl_done);
		{
			Sstar_printk_err("O'My GOD CMD TIMEOUT....Again\n");
			hw_priv->syncChanl_done = 0;
			goto TxCmdAgain;
		}
#endif
	...
	if (ret == WSM_CMD_CRC_ERRO) {
		Sstar_printk_err("O'My GOD CRC ERR....Again\n");
		goto TxCmdAgain;
	} else
#endif
	{
...
void wsm_lock_tx(struct Sstar_common *hw_priv)
{
	wsm_cmd_lock(hw_priv);
...
void wsm_vif_lock_tx(struct Sstar_vif *priv)
{
	struct Sstar_common *hw_priv = priv->hw_priv;
	wsm_cmd_lock(hw_priv);
	if (atomic_add_return(1, &hw_priv->tx_lock) == 1) {
...
void wsm_lock_tx_async(struct Sstar_common *hw_priv)
{
	if (atomic_add_return(1, &hw_priv->tx_lock) == 1)
		wsm_printk("[WSM] TX is locked (async).\n");
}

void wsm_unlock_tx_async(struct Sstar_common *hw_priv)
{
	atomic_sub_return(1, &hw_priv->tx_lock);
}

bool wsm_flush_tx(struct Sstar_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		...
		for (i = 0; i < 4; ++i)
			pending |= Sstar_queue_get_xmit_timestamp(
				&hw_priv->tx_queue[i], &timestamp,
				SSTAR_WIFI_ALL_IFS, 0xffffffff);
		/* It is allowed to lock TX with only a command in the pipe. */
		if (!pending)
			return true;

		timeout = WSM_CMD_LAST_CHANCE_TIMEOUT;
		if (Sstar_wait_event_timeout_stay_awake(
			    hw_priv, hw_priv->bh_evt_wq,
			    !(hw_priv->hw_bufs_used), timeout, true) <= 0) {
			Sstar_printk_err(
				"+++++  bh_error=1 have txframe pending hw_bufs_used %d,timeout =%d\n",
				(u32)hw_priv->hw_bufs_used, (u32)timeout);
			if (!(hw_priv->hw_bufs_used)) {
				/* Get a timestamp of "oldest" frame */
				for (i = 0; i < 4; ++i)
					pending |=
						Sstar_queue_get_xmit_timestamp(
							&hw_priv->tx_queue[i],
							&timestamp,
							SSTAR_WIFI_ALL_IFS,
							0xffffffff);
				...
				Sstar_printk_err(
					"<WARNING hw_bufs_use==0,pending %x,but wait imeout!!!!!!\n",
					pending);
			...
			//
			Sstar_printk_err(
				"bh_error=1 have txframe pending hw_bufs_used %d,hw_noconfirm_tx %d,timeout =%d\n",
				(u32)hw_priv->hw_bufs_used,
				hw_priv->hw_noconfirm_tx, (u32)timeout);
#ifdef SDIO_BUS
...
bool wsm_vif_flush_tx(struct Sstar_vif *priv)
{
	struct Sstar_common *hw_priv = priv->hw_priv;
	unsigned long timestamp = jiffies;
	long timeout;
	...
	list_for_each_entry_safe (bitmap, tmp, &hw_priv->SeqBitMapList, link) {
		if (bitmap->bitm.DataFlag != IS_CMD) { //data
			...
			Sstar_kfree(bitmap);
			Sstar_printk_err(
				"hw_bufs_used_vif[%d] =%d,bitmap->bitm.packetId =%d\n",
				priv->if_id,
	...
	if (hw_priv->bh_error) {
		...
		/* Get a timestamp of "oldest" frame */
		for (i = 0; i < 4; ++i)
			Sstar_queue_get_xmit_timestamp(&hw_priv->tx_queue[i],
						       &timestamp, if_id,
						       0xffffffff);
		/* It is allowed to lock TX with only a command in the pipe. */
		...
		timeout = WSM_CMD_LAST_CHANCE_TIMEOUT;
		if (timeout < 0 || Sstar_wait_event_timeout_stay_awake(
					   hw_priv, hw_priv->bh_evt_wq,
					   !hw_priv->hw_bufs_used_vif[if_id],
					   timeout, true) <= 0) {
			Sstar_printk_err(
				"%s:++  bh_error=1 hw_bufs_used_vif %d,hw_bufs_used %d,timeout %ld\n",
				__func__,
				hw_priv->hw_bufs_used_vif[priv->if_id],
				hw_priv->hw_bufs_used, timeout);
			//
#ifdef SDIO_BUS
			if (wsm_sync_channle_process(hw_priv, OUT_BH) >= 0) {
#else
...
void wsm_unlock_tx(struct Sstar_common *hw_priv)
{
	int tx_lock;
	if (hw_priv->bh_error)
		Sstar_printk_err("fatal error occured, unlock is unsafe\n");
	else {
...
int wsm_handle_exception(struct Sstar_common *hw_priv, u8 *data, u32 len)
{
	...
	Sstar_for_each_vif_safe(hw_priv, priv, if_id)
	{
		if (!priv)
			continue;
	...
	Sstar_printk_debug("wsm_handle_exception :reason[%d] \n ", reason);
	for (i = 0; i < ARRAY_SIZE(reg); ++i)
		reg[i] = WSM_GET32(&buf);
	WSM_GET(&buf, fname, sizeof(fname));

	Sstar_printk_always("Firmware assert at %d,%s, line %d\n",
	...
	i += 4;
	Sstar_printk_always("CPSR: 0x%.8X, SPSR: 0x%.8X\n", reg[i + 0],
			    reg[i + 1]);
...
underflow:
	Sstar_printk_always("Firmware exception.\n");
	print_hex_dump_bytes("Exception: ", DUMP_PREFIX_NONE, data, len);
	Sstar_wifi_run_status_set(1);
	return -EINVAL;
}
#if 0
static int wsm_test_confirm(struct Sstar_common *hw_priv,
				struct wsm_buf *buf)
{
	int ret = 0;
	int count;
...
#endif
extern int Sstar_rx_cnt;
int wsm_handle_rx(struct Sstar_common *hw_priv, int id, struct wsm_hdr *wsm,
		  struct sk_buff **skb_p)
{
...
//	frame_hexdump(__func__,(u32*)wsm,32);
	...
	if (id == WSM_SEQ_BIT_MAP_ID) {
		ret = wsm_tx_release_bufused(hw_priv, &wsm_buf,
					     interface_link_id);
	}
	...
	if (id == WSM_TX_REQ_ID) {
		...
		//			   interface_link_id);
		Sstar_printk_bus("WSM_FIRMWARE_CHECK_CONFIRM_ID \n");
...
#ifdef SSTAR_SDIO_PATCH
		spin_lock_bh(&hw_priv->SeqBitMapLock);
		list_for_each_entry_safe (bitmap, tmp, &hw_priv->SeqBitMapList,
					  link) {
			//find the cmd
		...
		case 0x0400:
			if (likely(wsm_arg))
				ret = wsm_read_shmem_confirm(hw_priv, wsm_arg,
							     &wsm_buf);
			break;
		case 0x0401:
			if (likely(wsm_arg))
				ret = wsm_write_shmem_confirm(hw_priv, wsm_arg,
							      &wsm_buf);
			break;
		case WSM_GENERIC_RESP_ID:
			if (likely(wsm_arg))
				ret = wsm_generic_req_confirm(hw_priv, wsm_arg,
							      &wsm_buf);
			break;
		case WSM_CONFIGURATION_RESP_ID:
			 ...
			 * wsm_cmd_send(). */
			if (likely(wsm_arg))
				ret = wsm_configuration_confirm(
					hw_priv, wsm_arg, &wsm_buf);
			break;
			...
			if (likely(wsm_arg)) {
				Sstar_for_each_vif(hw_priv, priv, i)
				{
					if (priv &&
						...
						ret = wsm_request_buffer_confirm(
							priv, wsm_arg,
							&wsm_buf);
				}
			}
		...
		case WSM_SMARTCONFIG_INDICATION_ID:
			ret = wsm_smartconfig_indication(
				hw_priv, interface_link_id, &wsm_buf, skb_p);
			break;
		...
		case WSM_SCAN_COMPLETE_IND_ID:
			...
			if (hw_priv->auto_scanning && hw_priv->frame_rcvd) {
				...
				priv = ABwifi_hwpriv_to_vifpriv(
					hw_priv, hw_priv->scan.if_id);
				if (unlikely(!priv)) {
					WARN_ON(1);
					return 0;
				}
				...
				wsm_printk("[WSM] Send Testmode Event.\n");
				Sstar_testmode_event(
					priv->hw->wiphy,
					NL80211_CMD_NEW_SCAN_RESULTS, 0, 0,
					GFP_KERNEL);
			}
#endif /*ROAM_OFFLOAD*/
#endif
		...
		case WSM_SUSP_RESUME_TX_IND_ID:
			ret = wsm_suspend_resume_indication(
				hw_priv, interface_link_id, &wsm_buf);
			break;
...
static bool wsm_handle_tx_data(struct Sstar_vif *priv, struct wsm_tx *wsm,
			       ...
			       struct Sstar_queue *queue)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
#ifdef P2P_MULTIVIF
	struct Sstar_vif *p2p_if_vif = NULL;
#endif
	bool handled = false;
	...
	case NL80211_IFTYPE_STATION:
		...
		if (unlikely((priv->join_status ==
			...
			if ((atomic_read(&hw_priv->remain_on_channel) ||
				...
				*/
				Sstar_printk_err(
					"drop offchannel pkg(%x)[%pM][%pM][%pM],bssid[%pM],if_id[%d],join_status[%d]\n",
					fctl, frame->addr1, frame->addr2,
					frame->addr3, priv->join_bssid,
	...
	case NL80211_IFTYPE_AP:
		if (unlikely(!priv->join_status))
			...
			Sstar_printk_warn("A frame with expired link id "
					  "is dropped.\n");
			action = doDrop;
		}
		if (Sstar_queue_get_generation(wsm->packetID) >
		    SSTAR_APOLLO_MAX_REQUEUE_ATTEMPTS) {
			/* HACK!!! WSM324 firmware has tendency to requeue
			 * multicast frames in a loop, causing performance
			 * drop and high power consumption of the driver.
			 * In this situation it is better just to drop
			 * the problematic frame. */
			Sstar_printk_warn("Too many attempts "
					  "to requeue a frame. "
					  "Frame is dropped.\n");
			action = doDrop;
		}
	...
	case NL80211_IFTYPE_ADHOC:
		if (priv->join_status != SSTAR_APOLLO_JOIN_STATUS_IBSS)
			action = doDrop;
		break;
#endif
	...
	case NL80211_IFTYPE_MONITOR:
		if (priv->join_status ==
			...
			txpriv->offchannel_if_id = SSTAR_WIFI_GENERIC_IF_ID;
			action = doTx;
			break;
		}
	...
	if (action == doTx) {
		if (unlikely(ieee80211_is_probe_req(fctl))) {
			if (atomic_read(&hw_priv->scan.in_progress)
				...
				action = doTx;
#endif
			else if (tx_info->flags &
				 IEEE80211_TX_CTL_RATE_CTRL_PROBE) {
				Sstar_printk_err("%s:send probe request\n",
						 __func__);
				action = doTx;
			} else
				action = doProbe;
	...
	case doProbe: {
		...
		wsm_printk("[WSM] Convert probe request to scan.\n");
		Sstar_printk_mgmt(
			"doProbe:[WSM] Convert probe request to scan.\n");
		wsm_lock_tx_async(hw_priv);
		hw_priv->pending_frame_id = __le32_to_cpu(wsm->packetID);
		if (Sstar_hw_priv_queue_delayed_work(
			    hw_priv, &hw_priv->scan.probe_work, 0) <= 0)
			wsm_unlock_tx(hw_priv);
		handled = true;
	} break;
	case doDrop: {
...
#else
		BUG_ON(Sstar_queue_remove(queue, __le32_to_cpu(wsm->packetID)));
	...
	case doJoin: {
		...
		Sstar_printk_mgmt("[WSM] Issue join command.\n");
		wsm_lock_tx_async(hw_priv);
		hw_priv->pending_frame_id = __le32_to_cpu(wsm->packetID);
		if (Sstar_hw_priv_queue_work(hw_priv, &priv->join_work) <= 0)
			wsm_unlock_tx(hw_priv);
		handled = true;
	} break;
	case doOffchannel:
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	{
		Sstar_printk_mgmt("[WSM] Offchannel TX request.\n");
		wsm_lock_tx_async(hw_priv);
		hw_priv->pending_frame_id = __le32_to_cpu(wsm->packetID);
		if (Sstar_hw_priv_queue_work(hw_priv, &priv->offchannel_work) <=
		    0)
			wsm_unlock_tx(hw_priv);
		handled = true;
	}
	...
	case doWep: {
		Sstar_printk_mgmt("[WSM] Issue set_default_wep_key.\n");
		wsm_lock_tx_async(hw_priv);
		priv->wep_default_key_id = tx_info->control.hw_key->keyidx;
		hw_priv->pending_frame_id = __le32_to_cpu(wsm->packetID);
		if (Sstar_hw_priv_queue_work(hw_priv, &priv->wep_key_work) <= 0)
			wsm_unlock_tx(hw_priv);
		handled = true;
	} break;
	case doTx: {
		...
		*/
		if (((priv->if_id != 0) && (priv->vif->p2p == true))) {
			if (ieee80211_is_mgmt(frame->frame_control)) {
				Sstar_clear_wpas_p2p_40M_ie(
					(struct Sstar_ieee80211_mgmt *)frame,
					wsm->hdr.len - txpriv->offset);
			}
		}
#endif
		/*
		...
		*/
		...
		if ((priv->vif->p2p == false) &&
			...
			rcu_read_lock();
			sta = sta_info_get(vif_to_sdata(priv->vif),
					   frame->addr1);
			while (sta && test_sta_flag(sta, WLAN_STA_40M_CH) &&
			       (!test_sta_flag(sta,
					       WLAN_STA_40M_CH_SEND_20M))) {
				const struct Sstar_txpriv *temp_txpriv = NULL;
				...
				wsm->htTxParameters |=
					__cpu_to_le32(WSM_HT_TX_WIDTH_40M);
				break;
			}
			rcu_read_unlock();
		}
...
static int Sstar_get_prio_queue(struct Sstar_vif *priv, u32 link_id_map,
				int *total)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	static u32 urgent;
	...
	int queued;
	int i;
	urgent = BIT(Sstar_dtim_virtual_linkid()) |
		 BIT(Sstar_uapsd_virtual_linkid());

	/* search for a winner using edca params */
	for (i = 0; i < 4; ++i) {
		queued = Sstar_queue_get_num_queued(priv, &hw_priv->tx_queue[i],
						    link_id_map);
		if (!queued)
			continue;
		*total += queued;
		...
		score = ((edca->aifns + edca->cwMin) << 16) +
			(edca->cwMax - edca->cwMin) * (prandom_u32() & 0xFFFF);

		//score = ((edca->aifns) << 8) +
		//		((1<<edca->cwMin) &random32());
//#define EDCA_QUEUE_FIX
#ifdef EDCA_QUEUE_FIX
		if (((hw_priv->wsm_caps.numInpChBufs - hw_priv->hw_bufs_used) <
		     (hw_priv->wsm_caps.numInpChBufs / 2)) &&
		    (hw_priv->tx_queue[i].num_pending_vif[priv->if_id] <=
		     (hw_priv->wsm_caps.numInpChBufs / 8))) {
	...
	if (winner >= 0 && hw_priv->tx_burst_idx >= 0 &&
	    ...
	    !Sstar_queue_get_num_queued(priv, &hw_priv->tx_queue[winner],
					link_id_map & urgent) &&
	    Sstar_queue_get_num_queued(
		    priv, &hw_priv->tx_queue[hw_priv->tx_burst_idx],
		    link_id_map))
		winner = hw_priv->tx_burst_idx;
...
static int wsm_get_tx_queue_and_mask(struct Sstar_vif *priv,
				     struct Sstar_queue **queue_p,
				     u32 *tx_allowed_mask_p, bool *more)
{
	...
	if (priv->tx_multicast) {
		tx_allowed_mask = BIT(Sstar_dtim_virtual_linkid());
		idx = Sstar_get_prio_queue(priv, tx_allowed_mask, &total);
		if (idx >= 0) {
			*more = total > 1;
			goto found;
		}
	...
	tx_allowed_mask |= BIT(Sstar_uapsd_virtual_linkid());
...
int wsm_get_tx(struct Sstar_common *hw_priv, u8 **data, u32 *tx_len, int *burst,
	       int *vif_selected)
{
	struct wsm_tx *wsm = NULL;
	struct ieee80211_tx_info *tx_info;
	struct Sstar_queue *queue = NULL;
	int queue_num;
	u32 tx_allowed_mask = 0;
	struct Sstar_txpriv *txpriv = NULL;
#ifdef P2P_MULTIVIF
	int first = 1;
	int tmp_if_id = -1;
#endif
	...
	int count = 0;
#ifdef P2P_MULTIVIF
	int if_pending = SSTAR_WIFI_MAX_VIFS - 1;
#else
	int if_pending = 1;
#endif
...
#endif //USB_CMD_UES_EP0
	{
		for (;;) {
			...
			/* This can be removed probably: Sstar_vif will not
			...
			* packet needs to be found */
			spin_lock_bh(&priv->ps_state_lock);
			ret = wsm_get_tx_queue_and_mask(
				priv, &queue, &tx_allowed_mask, &more);
			...
			if (Sstar_queue_get(queue, priv->if_id, tx_allowed_mask,
					    &wsm, &tx_info, &txpriv)) {
				Sstar_priv_vif_list_read_unlock(
					&priv->vif_lock);
				if_pending = 0;
			...
			if (wsm_handle_tx_data(priv, wsm, tx_info, txpriv,
					       queue)) {
				Sstar_priv_vif_list_read_unlock(
					&priv->vif_lock);
				if_pending = 0;
...
//			}
			...
			*tx_len = __le16_to_cpu(wsm->hdr.len);
			...
			if (priv->edca.params[queue_num].txOpLimit)
				*burst = min(*burst,
					     (int)Sstar_queue_get_num_queued(
						     priv, queue,
						     tx_allowed_mask) +
						     1);
			else
...
int wsm_txed(struct Sstar_common *hw_priv, u8 *data)
{
...
{
	BUG_ON(buf->begin);
	buf->begin = Sstar_kmalloc(/*SDIO_BLOCK_SIZE*/ MAX_WSM_BUF_LEN,
				   GFP_KERNEL | GFP_DMA);
...
static int wsm_buf_reserve(struct wsm_buf *buf, size_t extra_size)
{
	...
	buf->begin = Sstar_krealloc(buf->begin, size, GFP_KERNEL | GFP_DMA);
	if (buf->begin) {
...
static struct Sstar_vif *wsm_get_interface_for_tx(struct Sstar_common *hw_priv)
{
	struct Sstar_vif *priv = NULL, *i_priv;
	int i = hw_priv->if_id_selected;

	if (1 /*TODO:COMBO*/) {
		Sstar_hw_vif_read_lock(&hw_priv->vif_list_lock);
		i_priv = hw_priv->vif_list[i] ? ABwifi_get_vif_from_ieee80211(
							hw_priv->vif_list[i]) :
						      NULL;
		if (i_priv) {
			priv = i_priv;
			Sstar_priv_vif_list_read_lock(&priv->vif_lock);
		}
		/* TODO:COMBO:
...
static struct Sstar_vif *wsm_get_interface_for_tx(struct Sstar_common *hw_priv)
{
	...
	Sstar_hw_vif_read_lock(&hw_priv->vif_list_lock);
	i_priv = SSTAR_HW_VIF_GET(hw_priv->vif_list[i]);
...
#endif
static inline int get_interface_id_scanning(struct Sstar_common *hw_priv)
{
...
int wsm_read_shmem(struct Sstar_common *hw_priv, u32 address, void *buffer,
		   size_t buf_size)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	WSM_PUT16(buf, buf_size);
	WSM_PUT16(buf, flags);
	ret = wsm_cmd_send(hw_priv, buf, &wsm_shmem_arg, 0x0000,
			   WSM_CMD_TIMEOUT, 0);
...
int wsm_write_shmem(struct Sstar_common *hw_priv, u32 address, size_t size,
		    void *buffer)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	...
	ret = wsm_cmd_send(hw_priv, buf, &wsm_shmem_arg, 0x0001,
			   WSM_CMD_TIMEOUT, 0);
...
int wsm_set_chantype_func(struct Sstar_common *hw_priv,
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
int wsm_req_chtype_indication(struct Sstar_common *hw_priv, struct wsm_buf *buf)
{
	struct wsm_req_chtype_change_ind arg_ind;

	arg_ind.status = WSM_GET32(buf);

	Sstar_printk_mgmt("%s:status(%d)\n", __func__, arg_ind.status);
	return 0;
underflow:
	WARN_ON(1);
	return -EINVAL;
}
int wsm_req_chtype_change_func(struct Sstar_common *hw_priv,
			       struct wsm_req_chtype_change *arg, int if_id)
{
	...
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_SEND_CHTYPE_CHG_REQUEST_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
int wsm_get_cca(struct Sstar_common *hw_priv, struct wsm_get_cca_req *arg,
		struct wsm_get_cca_resp *cca_res, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
	//printk("%s\n",__func__);
	...
	ret = wsm_cmd_send(hw_priv, buf, cca_res, WSM_GET_CCA_ID,
			   WSM_CMD_TIMEOUT, if_id);
...
int wsm_get_cca_confirm(struct Sstar_common *hw_priv,
...
#endif
#if 0
int wsm_test_cmd(struct Sstar_common *hw_priv,u8 *buffer,int size)
{
	int ret;
	struct wsm_shmem_arg_s wsm_shmem_arg = {
		.buf = buffer,
...
*/
int wsm_efuse_change_data_cmd(struct Sstar_common *hw_priv,
			      const struct efuse_headr *arg, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
