...
const char *Sstar_log = SSTAR_TAG;
...
static char chip_type_str[CHIP_NAME_LEN] = "101B";
#elif (PROJ_TYPE < ARES_A)
static char chip_type_str[CHIP_NAME_LEN] = "101A";
#endif
...
void set_chip_type(const char *chip)
{
	if (chip) {
		...
		Sstar_printk_init("current chiptype %s \n", chip_type_str);
	}
}
...
static struct ieee80211_rate Sstar_rates[] = {
	RATETAB_ENT(10, 0, 0),	 RATETAB_ENT(20, 1, 0),
	RATETAB_ENT(55, 2, 0),	 RATETAB_ENT(110, 3, 0),
	RATETAB_ENT(60, 6, 0),	 RATETAB_ENT(90, 7, 0),
	RATETAB_ENT(120, 8, 0),	 RATETAB_ENT(180, 9, 0),
	RATETAB_ENT(240, 10, 0), RATETAB_ENT(360, 11, 0),
	RATETAB_ENT(480, 12, 0), RATETAB_ENT(540, 13, 0),
};
...
#define CHAN5G_2G(_channel, hw_channel, _flags)                                \
	{                                                                      \
		.band = IEEE80211_BAND_5GHZ,                                   \
		.center_freq = 5000 + (5 * (_channel)),                        \
		.hw_value = (hw_channel), .flags = (_flags),                   \
		.max_antenna_gain = 0, .max_power = 30,                        \
	}
#endif
...
static struct ieee80211_channel Sstar_5ghz_chantable[] = {
	...
	CHAN5G(132, 0), CHAN5G(136, 0), CHAN5G(140, 0), CHAN5G(149, 0),
	CHAN5G(153, 0), CHAN5G(157, 0), CHAN5G(161, 0), CHAN5G(165, 0),
	CHAN5G(184, 0), CHAN5G(188, 0), CHAN5G(192, 0), CHAN5G(196, 0),
	CHAN5G(200, 0), CHAN5G(204, 0), CHAN5G(208, 0), CHAN5G(212, 0),
	CHAN5G(216, 0),
};
...
static struct ieee80211_channel Sstar_5ghz_chantable[] = {
	...
	CHAN5G_2G(38, 38, 0), CHAN5G_2G(40, 40, 0),
	CHAN5G_2G(42, 42, 0), CHAN5G_2G(44, 42, 0),
};
/* Sstar channel define */
static struct ieee80211_regdomain
	Sstar_request_regdom = { .n_reg_rules = 4,
				 .alpha2 = "99",
				 .reg_rules = {
					 /* IEEE 802.11b/g, channels 1..11 */
					 REG_RULE(2412 - 10, 2462 + 10, 40, 6,
						  20, 0),
					 /* IEEE 802.11b/g, channels 12..13. */
					 REG_RULE(2467 - 10, 2472 + 10, 40, 6,
						  20, 0 /*NL80211_RRF_NO_IR*/),
					 /* IEEE 802.11 channel 14 - Only JP enables
		 * this and for 802.11b only */
					 REG_RULE(2484 - 10, 2484 + 10, 20, 6,
						  20, 0 /*NL80211_RRF_NO_IR*/),
					 /* IEEE 802.11a, channel 36..48 */
					 REG_RULE(5170 - 10, 5240 + 10, 80, 6,
						  20, 0 /*NL80211_RRF_NO_IR*/),
				 } };
#else
static struct ieee80211_regdomain
	Sstar_request_regdom = { .n_reg_rules = 3,
				 .alpha2 = "99",
				 .reg_rules = {
					 /* IEEE 802.11b/g, channels 1..11 */
					 REG_RULE(2412 - 10, 2462 + 10, 40, 6,
...
#if defined(CONFIG_SSTAR_APOLLO_5GHZ_SUPPORT) ||                               \
	defined(CONFIG_SSTAR_5G_PRETEND_2G)
...
void Sstar_get_drv_version(short *p)
{
	*p = driver_ver;
}
...
int Sstar_req_lmc_to_send_action(struct Sstar_vif *priv,
				 notifyPeerAction_t is40M)
{
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_AP) {
		...
		memcpy(&req_chtype_chg.MacAddr[0], priv->vif->bss_conf.bssid,
		       6);
	} else {
		return -1;
	}
	Sstar_printk_debug("%s: join_status=%d, addr =%pM,is40M = %d\n",
			   ...
			   &req_chtype_chg.MacAddr[0], is40M);
	if (is40M == notifyPeer20M)
		req_chtype_chg.flags =
			WSM_SEND_CHTYPE_CHG_REQUEST__FLAGS_SEN_20M;
	else if (is40M == notifyPeer40M)
		req_chtype_chg.flags =
			WSM_SEND_CHTYPE_CHG_REQUEST__FLAGS_SEN_40M;

	return wsm_req_chtype_change_func(priv->hw_priv, &req_chtype_chg,
					  priv->if_id);
}
...
void Sstar_channel_type_change_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
		container_of(work, struct Sstar_vif, chantype_change_work.work);
	struct Sstar_common *hw_priv = priv->hw_priv;
	...
	if (!atomic_read(&hw_priv->channel_chaging)) {
		Sstar_printk_err(
			"%s:what happend.only if_id 0 can change channel type so return\n",
			__func__);
		goto type_change_work_err;
	}
	...
	if (unlikely(down_trylock(&hw_priv->scan.lock))) {
		...
		if (Sstar_hw_priv_queue_delayed_work(
			    hw_priv, &priv->chantype_change_work, HZ / 3) <=
		    0) {
			Sstar_printk_err(
				"%s: scan locked so requeue the work err\n",
				__func__);
		}
		return;
	}
	...
	*/
	if (test_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANNUM_CHANGE,
		     &hw_priv->change_bit) ||
	    test_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANTYPE_CHANGE,
		     &hw_priv->change_bit)) {
		/*
		...
		*/
		...
		Sstar_printk_debug("%s:chatype(%d),channelNumber(%d)\n",
				   __func__, set_channtype.channelType,
				   set_channtype.channelNumber);
		clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANNUM_CHANGE,
			  &hw_priv->change_bit);
		clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANTYPE_CHANGE,
			  &hw_priv->change_bit);
		if (wsm_set_chantype_func(hw_priv, &set_channtype,
					  priv->if_id)) {
	...
	*/
	if (ieee80211_chw_is_ht40(temp_channel_type) &&
		...
		if (!test_bit(WSM_SET_CHANTYPECHANGE_FLAGS__TOOL_SET_FORCE,
			      &hw_priv->change_bit)) {
			if ((atomic_read(&hw_priv->cca_detect_running) == 0) &&
			    (start_cca_check == 1)) {
				Sstar_printk_debug("%s:start get cca\n",
						   __func__);
				Sstar_start_detect_cca(
					priv, hw_priv->rx_phy_enable_num_req);
				atomic_set(&hw_priv->cca_detect_running, 1);
				Sstar_mod_timer(
					&hw_priv->chantype_timer,
					jiffies +
						(atomic_read(
							 &hw_priv->cca_interval_ms) /
						 1000) * HZ +
						HZ / 1000);
			}
			...
			*/
			...
			atomic_set(&hw_priv->phy_chantype, 1);
			Sstar_del_timer_sync(&hw_priv->chantype_timer);
		...
		if (atomic_read(&hw_priv->tx_20M_lock) == 1) {
			...
			if (test_bit(
				    WSM_SET_CHANTYPECHANGE_FLAGS__TOOL_SET_FORCE,
				    &hw_priv->change_bit)) {
				action = notifyPeer20M;
			}

			if (test_bit(
				    WSM_SET_CHANTYPECHANGE_FLAGS__CHANGE_PHY_CHTYPE,
				    &hw_priv->change_bit)) {
...
type_change_work_err:
	...
	clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANTYPE_CHANGE,
		  &hw_priv->change_bit);
	clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANGE_PHY_CHTYPE,
		  &hw_priv->change_bit);
	if (atomic_sub_return(1, &hw_priv->channel_chaging) < 0)
		atomic_set(&hw_priv->channel_chaging, 0);
	mutex_unlock(&hw_priv->chantype_mutex);
...
static void Sstar_chantype_timer(unsigned long arg)
{
	...
	if (atomic_read(&hw_priv->tx_20M_lock)) {
		can_get = 0;
		Sstar_printk_debug(
			"%s:tx_20M_lock is locked,we cannot get cca\n",
			__func__);
	}

	if (can_get == 0) {
		Sstar_printk_debug("%s:channel_type(%d),tx_20M_lock(%d)\n",
				   __func__, hw_priv->channel_type,
				   atomic_read(&hw_priv->tx_20M_lock));
		return;
	}
	//printk("%s:can_get(%d)\n",__func__,can_get);
	if (start_cca_check == 1) {
		if (Sstar_hw_priv_queue_work(hw_priv, &hw_priv->get_cca_work) <=
		    0) {
			Sstar_printk_err("%s:cmd get cca err", __func__);
		}
	}
}
void Sstar_get_cca_work(struct Sstar_work_struct *work)
{
...
#define CCA_CHANGE_TO_20M_a atomic_read(&hw_priv->chw_sw_20M_level)
	struct Sstar_common *hw_priv =
		container_of(work, struct Sstar_common, get_cca_work);
	u8 can_get = 0;
	u32 flags = 0;
	struct wsm_get_cca_req get_cca;
	struct wsm_get_cca_resp res_cca;
	...
	*/
	if (can_get == 0) {
		...
		if (test_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CCA_LEVEL_CHANGE,
			     &hw_priv->change_bit)) {
			Sstar_printk_err("%s:cca_interval_change\n", __func__);
			clear_bit(
				WSM_SET_CHANTYPECHANGE_FLAGS__CCA_LEVEL_CHANGE,
				&hw_priv->change_bit);
	...
	if (res_cca.status != WSM_STATUS_SUCCESS) {
		...
		if (res_cca.rx_phy_enable_num_cnf <
		    get_cca.rx_phy_enable_num_req) {
			Sstar_printk_always(
				"%s:getcca not good rx_phy_enable_num_cnf (%d) < (%d)\n",
				__func__, res_cca.rx_phy_enable_num_cnf,
				get_cca.rx_phy_enable_num_req);
			goto cca_work_end;
		}
	}
	if (can_get == 1) {
		if (res_cca.pri_channel_idle_cnt > 0xfffff) {
			res_cca.pri_snd_channel_idle_cnt =
				res_cca.pri_snd_channel_idle_cnt >> 8;
			res_cca.pri_channel_idle_cnt =
				res_cca.pri_channel_idle_cnt >> 8;
		}
		if ((res_cca.pri_snd_channel_idle_cnt << 8) >
		    (res_cca.pri_channel_idle_cnt * CCA_CHANGE_TO_40M_a)) {
			if (atomic_read(&hw_priv->tx_20M_lock) == 0) {
				CHW_ACTION_TO_PEER(CHW_IS_40M, action);
			} else {
				CHW_ACTION_TO_PEER(CHW_IS_20M, action);
			}
		} else if ((res_cca.pri_snd_channel_idle_cnt << 8) <
			   (res_cca.pri_channel_idle_cnt *
			    CCA_CHANGE_TO_20M_a)) {
			CHW_ACTION_TO_PEER(CHW_IS_20M, action);
		}
	...
	*/
	if (can_get) {
		Sstar_mod_timer(
			&hw_priv->chantype_timer,
			jiffies +
				(atomic_read(&hw_priv->cca_interval_ms) /
				 1000) * HZ +
				HZ / 1000);
...
void Sstar_clear_wpas_p2p_40M_ie(struct Sstar_ieee80211_mgmt *mgmt, u32 pkg_len)
{
	...
	struct mgmt_filter Sstar_mgmt_filter_table[] = {
		{
			.handle = ieee80211_is_beacon,
			.variable = mgmt->u.beacon.variable,
		},
		{
			.handle = ieee80211_is_probe_req,
			.variable = mgmt->u.probe_req.variable,
		},
		{
			.handle = ieee80211_is_probe_resp,
			.variable = mgmt->u.probe_resp.variable,
		},
		{
			.handle = ieee80211_is_assoc_req,
			.variable = mgmt->u.assoc_req.variable,
		},
		{
			.handle = ieee80211_is_reassoc_req,
			.variable = mgmt->u.reassoc_req.variable,
		},
		{
			.handle = ieee80211_is_assoc_resp,
			.variable = mgmt->u.assoc_resp.variable,
		},
		{
			.handle = ieee80211_is_reassoc_resp,
			.variable = mgmt->u.assoc_resp.variable,
		},
		{
			.handle = NULL,
	...
	u16 clear_bit = IEEE80211_HT_CAP_SUP_WIDTH_20_40 |
			IEEE80211_HT_CAP_DSSSCCK40 | IEEE80211_HT_CAP_SGI_40;
	u8 index = 0;
	...
	for (index = 0; Sstar_mgmt_filter_table[index].handle != NULL;
	     index++) {
		if (!Sstar_mgmt_filter_table[index].handle(
			    mgmt->frame_control)) {
	...
	htcap_ie = (u8 *)cfg80211_find_ie(SSTAR_WLAN_EID_HT_CAPABILITY, ie,
					  ie_len);
...
#define TID_IS_SAFE(tid_index, action)                                         \
	if ((tid_index) > SSTAR_RX_DATA_QUEUES)                                \
	action
#define SEQ_IS_SAFE(seq_index, action)                                         \
	if ((seq_index) >= BUFF_STORED_LEN)                                    \
	action
#define reorder_debug(debug_en, ...)                                           \
	if (debug_en)                                                          \
	Sstar_printk_debug(__VA_ARGS__)
...
#define spinlock_debug(type)                                                   \
	Sstar_printk_debug("%s:tid_params_%s\n", __func__, #type)
#else
#define spinlock_debug(type)
#endif
...
static struct Sstar_ba_tid_params *
Sstar_get_tid_params(struct Sstar_reorder_queue_comm *Sstar_reorder, u8 tid)
{
...
static void Sstar_skb_buff_queue(struct Sstar_ba_tid_params *tid_params,
				 u8 index, struct sk_buff *skb)
{
...
static void Sstar_skb_buff_dequeue(struct Sstar_common *hw_priv,
				   struct Sstar_ba_tid_params *tid_params,
				   u8 index, bool uplayer)
{
	struct sk_buff *skb;
...
int Sstar_reorder_skb_forcedrop(struct Sstar_vif *priv,
				struct Sstar_ba_tid_params *tid_params,
				int need_free)
{
	u8 i = 0;
	...
	for (i = 0; i < need_free; i++) {
		...
		tid_params->start_seq =
			(tid_params->start_seq + 1) & SEQ_NUM_MASKER;
		seq = BUFF_INDEX_IS_SAFE(tid_params->start_seq);
	}

	return 0;
}
int Sstar_reorder_skb_forcefree(struct Sstar_vif *priv,
				struct Sstar_ba_tid_params *tid_params,
				int need_free)
{
	u8 i = 0;
...
int Sstar_reorder_skb_uplayer(struct Sstar_vif *priv,
			      struct Sstar_ba_tid_params *tid_params)
{
...
{
	struct Sstar_ba_tid_params *tid_params =
		(struct Sstar_ba_tid_params *)arg;
	...
	while (i < tid_params->wind_size) {
		if (tid_params->frame_rx_time[index] == 0)
			break;
		if (time_is_before_eq_jiffies(tid_params->frame_rx_time[index] +
					      (5 * HZ / 1000))) {
			reorder_debug(REORDER_DEBUG,
				      "ReOrder:timeout %x>%x sn %x\n",
				      tid_params->frame_rx_time[index], jiffies,
				      tid_params->start_seq);
			Sstar_skb_buff_dequeue(hw_priv, tid_params, index,
					       NULL);
		} else {
			break;
		}
	...
	index = BUFF_INDEX_IS_SAFE(tid_params->start_seq);
	if (tid_params->skb_buffed) {
		Sstar_mod_timer(&tid_params->overtime_timer,
				tid_params->frame_rx_time[index] +
					(tid_params->timeout * HZ) / 1000);
	} else {
		clear_bit(REORDER_TIMER_RUNING, &tid_params->timer_running);
	}
exit:
	tid_params_spin_unlock(&tid_params->skb_reorder_spinlock,
			       spin_unlock_bh);
}

int Sstar_reorder_skb_queue(struct Sstar_vif *priv, struct sk_buff *skb,
			    u8 link_id)
{
	u16 index = 0;
	int res = 0;
	...
	*/
	if (index >= THE_RETRY_PKG_INEDX) {
		reorder_debug(REORDER_DEBUG,
			      "rx a prev frame,seq(%x),start(%x)\n",
			      (frame->seq_ctrl & (IEEE80211_SCTL_SEQ)) >> 4,
			      start_seq);
		res = 0;
		goto exit_reorder;
	}
	...
	*/
	if (index >= tid_params->wind_size) {
		...
		if (need_free >= tid_params->wind_size) {
			reorder_debug(
				REORDER_DEBUG,
				"what a fuck seq,queue to mac80211 handles\n");
			res = 0;
			goto exit_reorder;
			;
		}
...
maybe_send_order:
	...
	*/
	...
	if ((!test_bit(REORDER_TIMER_RUNING, &tid_params->timer_running) ||
	     (start_seq != tid_params->start_seq)) &&
	    (tid_params->skb_buffed) && (tid_params->timeout)) {
		index = BUFF_INDEX_IS_SAFE(tid_params->start_seq);
		...
		clear_bit(REORDER_TIMER_RUNING, &tid_params->timer_running);
		Sstar_del_timer_sync(&tid_params->overtime_timer);
	}
	res = -1;
exit_reorder:
	tid_params_spin_unlock(&tid_params->skb_reorder_spinlock,
...
void Sstar_reorder_func_init(struct Sstar_vif *priv)
{
	...
	for (k = 0; k < SSTAR_APOLLO_LINK_ID_UNMAPPED; k++) {
		...
		for (i = 0; i < SSTAR_RX_DATA_QUEUES; i++) {
			...
			tid_params->overtime_timer.data =
				(unsigned long)tid_params;
			tid_params->overtime_timer.function =
				Sstar_reorder_pkg_timeout;
			Sstar_init_timer(&tid_params->overtime_timer);
...
void Sstar_reorder_tid_buffed_clear(struct Sstar_vif *priv,
				    struct Sstar_ba_tid_params *tid_params)
{
	u8 header = 0;
...
void Sstar_reorder_tid_reset(struct Sstar_vif *priv,
			     struct Sstar_ba_tid_params *tid_params)
{
...
void Sstar_reorder_func_reset(struct Sstar_vif *priv, u8 link_id)
{
	...
	for (; link_start < link_end; link_start++) {
		...
		for (tid = 0; tid < SSTAR_RX_DATA_QUEUES; tid++) {
			if (!test_bit(
				    BAR_TID_EN,
				    &Sstar_reorder->Sstar_rx_tid[tid].tid_en)) {
				continue;
			}
			Sstar_reorder_tid_reset(
				priv, &Sstar_reorder->Sstar_rx_tid[tid]);
			tid_params_spin_lock(&Sstar_reorder->Sstar_rx_tid[tid]
						      .skb_reorder_spinlock,
					     spin_lock_bh);
			clear_bit(BAR_TID_EN,
				  &Sstar_reorder->Sstar_rx_tid[tid].tid_en);
			Sstar_reorder->Sstar_rx_tid[tid].ssn = 0;
			Sstar_reorder->Sstar_rx_tid[tid].wind_size = 0;
			Sstar_reorder->Sstar_rx_tid[tid].start_seq = 0;
			tid_params_spin_unlock(&Sstar_reorder->Sstar_rx_tid[tid]
							.skb_reorder_spinlock,
					       spin_unlock_bh);
		}
		//		mutex_unlock(&Sstar_reorder->reorder_mutex);
...
void Sstar_updata_ba_tid_params(struct Sstar_vif *priv,
				struct Sstar_ba_params *ba_params)
{
	...
	case SSTAR_BA__ACTION_RX_ADDBR: {
		...
		if (tid_params->timeout == 0)
			tid_params->timeout =
				tid_params->wind_size * PER_PKG_TIME_INTERVAL;
	...
	case SSTAR_BA__ACTION_RX_DELBA: {
		...
		reorder_debug(REORDER_DEBUG,
			      "WSM_BA_FLAGS__RX_DELBA_RE,link_id(%d)\n",
			      link_id);
	...
	case SSTAR_BA__ACTION_RX_BAR: {
		...
		if (((ba_params->ssn - pre_start_seq) & SEQ_NUM_MASKER) >
		    THE_RETRY_PKG_INEDX)
		...
		/*
			...
			*/
		if ((((ba_params->ssn - pre_start_seq) & SEQ_NUM_MASKER) <=
		     tid_params->wind_size) &&
		    tid_params->skb_buffed) {
			u8 need_free = ((ba_params->ssn - pre_start_seq) &
					SEQ_NUM_MASKER);

			reorder_debug(REORDER_DEBUG,
				      "RX_BAR,BARsn(%x),Ssn(%x)\n",
				      ba_params->ssn, pre_start_seq);
			reorder_debug(
				REORDER_DEBUG,
				"_RX_BAR case 3,ssn(%x),buffed(%d)need_free %d\n",
				tid_params->start_seq, tid_params->skb_buffed,
				need_free);
			...
			/*
				...
				*/
			Sstar_reorder_skb_forcefree(priv, tid_params,
						    need_free);
			/*
			...
			if (tid_params->skb_buffed) {
				reorder_debug(
					REORDER_DEBUG,
					"_RX_BAR free ALL case 4,(%x)=ssn(%x)-start_seq(%x),buffed(%d)\n",
					(ba_params->ssn - pre_start_seq),
					ba_params->ssn, tid_params->start_seq,
					tid_params->skb_buffed);
				Sstar_reorder_skb_forcefree(
					priv, tid_params,
					tid_params->wind_size);
			}
			tid_params->start_seq = ba_params->ssn;
		...
		if ((!test_bit(REORDER_TIMER_RUNING,
			       &tid_params->timer_running) ||
		     (pre_start_seq != tid_params->start_seq)) &&
		    (tid_params->skb_buffed) && (tid_params->timeout)) {
			u16 index = BUFF_INDEX_IS_SAFE(tid_params->start_seq);

			//reorder_debug(REORDER_DEBUG,"mod_timer,skb_buffed(%d),start_seq(%x)\n",tid_params->skb_buffed,tid_params->start_seq);
			set_bit(REORDER_TIMER_RUNING,
				&tid_params->timer_running);
			WARN_ON(tid_params->frame_rx_time[index] == 0);
			Sstar_mod_timer(&tid_params->overtime_timer,
					tid_params->frame_rx_time[index] +
						(tid_params->timeout * HZ) /
							1000);
		} else if (tid_params->skb_buffed == 0) {
			clear_bit(REORDER_TIMER_RUNING,
				  &tid_params->timer_running);
			Sstar_del_timer_sync(&tid_params->overtime_timer);
		}
		tid_params->ssn = ba_params->ssn;
...
{
	memcpy(hw_priv->addresses[1].addr, hw_priv->addresses[0].addr,
	       ETH_ALEN);
...
int Sstar_set_mac_addr2efuse(struct ieee80211_hw *hw, u8 *macAddr)
{
	...
	if (wsm_get_mac_address(hw_priv, &EfusemacAddr[0]) == 0) {
		if (EfusemacAddr[0] | EfusemacAddr[1] | EfusemacAddr[2] |
		    EfusemacAddr[3] | EfusemacAddr[4] | EfusemacAddr[5]) {
			Sstar_printk_err("MAC addr Writed\n");
			return -1;
		}
	} else {
		Sstar_printk_err("MAC addr disabled\n");
		return -1;
	}

	if (wsm_set_efuse_mac_address(hw_priv, &macAddr[0]) == 0) {
		if (macAddr[0] | macAddr[1] | macAddr[2] | macAddr[3] |
		    macAddr[4] | macAddr[5]) {
...
static int Sstar_set_frame_filter(struct ieee80211_hw *hw,
				  struct ieee80211_vif *vif, u32 n_filters,
				  struct ieee80211_special_filter *filter_table,
				  bool enable)
{
	struct Sstar_common *hw_priv = hw->priv;
	...
	if (n_filters > 10) {
		Sstar_printk_err("%s:n_filters (%d)\n", __func__, n_filters);
		return -EOPNOTSUPP;
	}
	Sstar_printk_debug("%s:n(%d),enable(%d)\n", __func__, n_filters,
			   enable);
	if (n_filters && (enable == true)) {
		memset(&table, 0, sizeof(struct wsm_beacon_filter_table));

		for (index = 0; index < n_filters; index++) {
			table.entry[index].ieId =
				filter_table[index].filter_action;
			memcpy(table.entry[index].oui, filter_table[index].oui,
			       3);

			if (filter_table[index].flags &
			    SPECIAL_F_FLAGS_FRAME_TYPE) {
				table.entry[index].actionFlags =
					WSM_BEACON_FILTER_FRAME_TYPE;
				Sstar_printk_debug("%s:frame [%d]\n", __func__,
						   table.entry[index].ieId);
			} else if (filter_table[index].flags &
				   SPECIAL_F_FLAGS_FRAME_OUI) {
				table.entry[index].actionFlags =
					WSM_BEACON_FILTER_ACTION_ENABLE |
					WSM_BEACON_FILTER_OUI;
				Sstar_printk_debug(
					"%s:ie_oui ie[%d],oui[%d:%d:%d]\n",
					__func__, table.entry[index].ieId,
					table.entry[index].oui[0],
					table.entry[index].oui[1],
					table.entry[index].oui[2]);
			} else if (filter_table[index].flags &
				   SPECIAL_F_FLAGS_FRAME_IE) {
				table.entry[index].actionFlags =
					WSM_BEACON_FILTER_ACTION_ENABLE;
				Sstar_printk_debug("%s:ie [%d]\n", __func__,
						   table.entry[index].ieId);
			} else
				WARN_ON(1);
		}
	...
	ret = wsm_beacon_filter_control(hw_priv, &enabled, priv->if_id);
...
#endif
#ifdef CONFIG_IEEE80211_SPECIAL_FILTER
	.set_frame_filter = Sstar_set_frame_filter,
#endif
};
#ifdef CONFIG_PM
...
void Sstar_get_mac_address(struct Sstar_common *hw_priv)
{
	...
	for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++) {
		memcpy(hw_priv->addresses[i].addr, Sstar_mac_default[i],
		       ETH_ALEN);
	}
...
struct ieee80211_hw *Sstar_init_common(size_t hw_priv_data_len)
{
	...
	hw->extra_tx_headroom = WSM_TX_EXTRA_HEADROOM + 64 + 8 /* TKIP IV */ +
				12 /* TKIP ICV and MIC */;
...
//#ifdef  CONFIG_SSTAR_5G_PRETEND_2G
	...
	wake_lock_init(&hw_priv->hw_wake, WAKE_LOCK_SUSPEND,
		       ieee80211_alloc_name(hw, "wlan_hw_wake"));
	wake_lock_init(&hw_priv->bh_wake, WAKE_LOCK_SUSPEND,
		       ieee80211_alloc_name(hw, "wlan_bh_wake"));
...
#ifdef SDIO_BUS
	mutex_init(&hw_priv->sdio_status_lock);
#endif
	...
	Sstar_init_timer(&hw_priv->wsm_pm_timer);
	...
	hw_priv->workqueue = Sstar_create_singlethread_workqueue(
		ieee80211_alloc_name(hw, "Sstar_wq"));
	sema_init(&hw_priv->scan.lock, 1);
	SSTAR_INIT_WORK(&hw_priv->scan.work, Sstar_scan_work);
#ifdef CONFIG_SSTAR_SUPPORT_SCHED_SCAN
#ifdef ROAM_OFFLOAD
	SSTAR_INIT_WORK(&hw_priv->scan.swork, Sstar_sched_scan_work);
#endif /*ROAM_OFFLOAD*/
#endif
	SSTAR_INIT_DELAYED_WORK(&hw_priv->scan.probe_work, Sstar_probe_work);
	SSTAR_INIT_DELAYED_WORK(&hw_priv->scan.timeout, Sstar_scan_timeout);
#ifdef CONFIG_SSTAR_SCAN_SPLIT
	SSTAR_INIT_DELAYED_WORK(&hw_priv->scan.scan_spilt,
				Sstar_scan_split_work);
#endif
	//#ifdef CONFIG_WIRELESS_EXT
	SSTAR_INIT_WORK(&hw_priv->etf_tx_end_work, etf_scan_end_work);
...
//#endif //#ifdef CONFIG_WIRELESS_EXT
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
	SSTAR_INIT_DELAYED_WORK(&hw_priv->advance_scan_timeout,
				Sstar_advance_scan_timeout);
#endif
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	SSTAR_INIT_DELAYED_WORK(&hw_priv->rem_chan_timeout,
				Sstar_rem_chan_timeout);
#endif
#ifndef CONFIG_RATE_HW_CONTROL
	SSTAR_INIT_WORK(&hw_priv->tx_policy_upload_work, tx_policy_upload_work);
#endif
	spin_lock_init(&hw_priv->event_queue_lock);
	INIT_LIST_HEAD(&hw_priv->event_queue);
	SSTAR_INIT_WORK(&hw_priv->event_handler, Sstar_event_handler);
#ifdef CONFIG_SSTAR_BA_STATUS
	SSTAR_INIT_WORK(&hw_priv->ba_work, Sstar_ba_work);
	spin_lock_init(&hw_priv->ba_lock);
	Sstar_init_timer(&hw_priv->ba_timer);
...
#ifdef SSTAR_SUPPORT_SMARTCONFIG
	SSTAR_INIT_WORK(&hw_priv->scan.smartwork, Sstar_smart_scan_work);
	SSTAR_INIT_WORK(&hw_priv->scan.smartsetChanwork,
			Sstar_smart_setchan_work);
	SSTAR_INIT_WORK(&hw_priv->scan.smartstopwork, Sstar_smart_stop_work);
	Sstar_init_timer(&hw_priv->smartconfig_expire_timer);
	hw_priv->smartconfig_expire_timer.data = (unsigned long)hw_priv;
	hw_priv->smartconfig_expire_timer.function =
		Sstar_smartconfig_expire_timer;
#endif
#ifdef SDIO_BUS
	...
	SSTAR_INIT_WORK(&hw_priv->wsm_sync_channl, wsm_sync_channl);
#else
	SSTAR_INIT_WORK(&hw_priv->wsm_sync_channl, wsm_sync_channl_reset);
#endif
#endif
	if (unlikely(Sstar_queue_stats_init(&hw_priv->tx_queue_stats,
					    WLAN_LINK_ID_MAX, Sstar_skb_dtor,
					    hw_priv))) {
	...
	for (i = 0; i < 4; ++i) {
		if (unlikely(Sstar_queue_init(
			    &hw_priv->tx_queue[i], &hw_priv->tx_queue_stats, i,
			    SSTAR_WIFI_MAX_QUEUE_SZ, Sstar_ttl[i]))) {
			for (; i > 0; i--)
...
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
	Sstar_init_timer(&hw_priv->chantype_timer);
	hw_priv->chantype_timer.data = (unsigned long)hw_priv;
	hw_priv->chantype_timer.function = Sstar_chantype_timer;
	SSTAR_INIT_WORK(&hw_priv->get_cca_work, Sstar_get_cca_work);
#endif
	hw_priv->rx_phy_enable_num_req =
		DEFAULT_CCA_INTERVAL_US / DEFAULT_CCA_UTIL_US / 3 * 2; //
	...
	clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANTYPE_CHANGE,
		  &hw_priv->change_bit);
	clear_bit(WSM_SET_CHANTYPECHANGE_FLAGS__TOOL_SET_FORCE,
		  &hw_priv->change_bit);
...
int Sstar_register_common(struct ieee80211_hw *dev)
{
	...
	Sstar_printk_always("is registered as '%s'\n", wiphy_name(dev->wiphy));
	return 0;
}
...
void Sstar_unregister_common(struct ieee80211_hw *dev)
{
	...
	Sstar_del_timer_sync(&hw_priv->wsm_pm_timer);
	...
	Sstar_destroy_workqueue(hw_priv->workqueue);
				 ...
				 struct ieee80211_hw *hw)
{
	...
	hw_priv->if_combs[0].radar_detect_widths =
		BIT(NL80211_CHAN_NO_HT) | BIT(NL80211_CHAN_HT20) |
		BIT(NL80211_CHAN_HT40MINUS) | BIT(NL80211_CHAN_HT40PLUS);
	hw_priv->if_combs[1].radar_detect_widths =
		BIT(NL80211_CHAN_NO_HT) | BIT(NL80211_CHAN_HT20) |
		BIT(NL80211_CHAN_HT40MINUS) | BIT(NL80211_CHAN_HT40PLUS);
	hw_priv->if_combs[2].radar_detect_widths =
		BIT(NL80211_CHAN_NO_HT) | BIT(NL80211_CHAN_HT20) |
		BIT(NL80211_CHAN_HT40MINUS) | BIT(NL80211_CHAN_HT40PLUS);
#endif
#endif
...
#else
char *strfilename = "/tmp/Sstar_txpwer_dcxo_cfg.txt";
#endif
...
#endif
	if (dst == NULL) {
		Sstar_printk_err("Invalid parameter,dst == NULL\n");
		return -1;
	}
	if (file == NULL) {
		Sstar_printk_err("Invalid parameter,file == NULL\n");
		return -1;
	}
...
void Sstar_get_delta_gain(char *srcData, int *allgain, int *bgain, int *gngain)
{
	int i = 0;
	char *databuf = srcData;
	if ((!databuf) || (!allgain) || (!bgain) || (!gngain)) {
		Sstar_printk_err(
			"Sstar_get_delta_gain fail,invalid parameters\n");
		return;
	}
	...
	sscanf(databuf, "delta_gain1:%d delta_gain2:%d delta_gain3:%d dcxo:%d \
	       ...
	       &allgain[0], &allgain[1], &allgain[2], &allgain[3], &bgain[0],
	       &bgain[1], &bgain[2], &gngain[0], &gngain[1], &gngain[2]);
	/*Sstar_printk_init("gain1:%d gain2:%d gain3:%d dcxo:%d\nb_gain1:%d b_gain2:%d b_gain3:%d\ngn_gain1:%d gn_gain2:%d gn_gain3:%d\n",
			allgain[0], allgain[1], allgain[2], allgain[3],
			bgain[0], bgain[1], bgain[2],gngain[0], gngain[1], gngain[2]);*/
}
#ifdef CONFIG_SSTAR_GET_GPIO4
extern int readInputValueFromGPIO4(struct Sstar_common *hw_priv);
#endif

static struct ieee80211_supported_band Sstar_6012B_band;

int Sstar_core_probe(const struct sbus_ops *sbus_ops, struct sbus_priv *sbus,
		     struct device *pdev, struct Sstar_common **pself)
{
	int err = -ENOMEM;
	...
	*/
	if (hw_priv->sbus_ops->sbus_xmit_func_init)
		err = hw_priv->sbus_ops->sbus_xmit_func_init(
			hw_priv->sbus_priv);
	if (hw_priv->sbus_ops->sbus_rev_func_init)
		err |= hw_priv->sbus_ops->sbus_rev_func_init(
			hw_priv->sbus_priv);

	if (err) {
		Sstar_printk_err("rev and xmit init err\n");
		goto err2;
	}
...
reload_fw:
	...
	//fix mstar CVT suspend bug,in CVT mstar suspend wait_event_interruptible_timeout sometime will not delay
	{
		...
		*/
			...
			if (loop++ > 100) {
				Sstar_printk_init(
					" wsm_startup_done timeout ERROR !!\n");
				Sstar_monitor_pc(hw_priv);
				if (hw_priv->sbus_ops->sbus_reset_chip) {
					Sstar_printk_err("reload fw\n");
					if (hw_priv->sbus_ops->irq_unsubscribe)
						hw_priv->sbus_ops->irq_unsubscribe(
							hw_priv->sbus_priv);
					hw_priv->sbus_ops->sbus_reset_chip(
						hw_priv->sbus_priv);
					goto reload_fw;
				} else {
					err = -ETIMEDOUT;
	...
	}
	{
		...
		err = wsm_get_efuse_data(hw_priv, (void *)&efuse_data,
					 sizeof(efuse_data));
		if (err) {
			WARN_ON(1);
			goto err3;
		...
		Sstar_printk_init(
			"efuse data is [0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x:0x%x:0x%x:0x%x:0x%x:0x%x]\n",
	...
	/*
	only support HT20 when chip is SSTAR6012B
*/

	if (hw_priv->chip_version == ARES_6012B) {
		memcpy(&Sstar_6012B_band, &Sstar_band_2ghz,
		...
		hw_priv->hw->wiphy->bands[IEEE80211_BAND_2GHZ]->ht_cap.cap =
			...
			(1 << IEEE80211_HT_CAP_RX_STBC_SHIFT);
		Sstar_printk_init("chip is 6012B not support HT40! \n");
	} else {
		hw_priv->hw->wiphy->bands[IEEE80211_BAND_2GHZ] =
			&Sstar_band_2ghz;
	}
	...
	gpio4 = sstar_Input_Value_Gpio(hw_priv, 4); //choose gpio you want
#endif
	if (strfilename && gpio4) {
		if (access_file(strfilename, readbuf, sizeof(readbuf), 1) > 0) {
			...
			sprintf(readbuf,
				...
				deltagain[0], deltagain[1], deltagain[2],
				deltagain[3], bgain[0], bgain[1], bgain[2],
				gngain[0], gngain[1], gngain[2]);

			Sstar_printk_init("cmd: %s\n", readbuf);
			err = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf,
	...
	}
	{
		...
		//if(err < 0){
		//	Sstar_printk_err("open cfo fail!!!. \n", err);
		//}

	}

	{
		Sstar_printk_err("get chip id [%x][%x][%x] \n",
		...
		if (hw_priv->chip_version == ARES_6012B)
			...
			set_chip_type(
				chip_str[(hw_priv->wsm_caps.firmeareExCap >> 7) &
					 0x7]);
#endif
	}
...
{
	Sstar_printk_debug("Sstar_set_shortGI,%d\n", shortgi);
	if (shortgi) {
		Sstar_band_2ghz.ht_cap.cap |= IEEE80211_HT_CAP_SGI_20;
		if (Sstar_band_2ghz.ht_cap.cap &
		    IEEE80211_HT_CAP_SUP_WIDTH_20_40)
...
#define SSTAR_40M_FREE ((u32)(-1))
	...
	if ((params == SSTAR_40M_FREE) &&
	    test_bit(WSM_SET_CHANTYPECHANGE_FLAGS__TOOL_SET_FORCE,
		     &hw_priv->change_bit)) {
		...
		set_bit(WSM_SET_CHANTYPECHANGE_FLAGS__TOOL_SET_FORCE,
			&hw_priv->change_bit);
	}
	...
	if (chtype != hw_priv->channel_type) {
		...
		spin_unlock_bh(&hw_priv->spinlock_channel_type);
		set_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CHANTYPE_CHANGE,
			&hw_priv->change_bit);
	...
	if (Sstar_hw_priv_queue_delayed_work(
		    hw_priv, &priv->chantype_change_work, 0) <= 0) {
...
#define SSTAR_CCA_FREE ((u32)(-1))
	...
	struct Sstar_cca_chage {
		...
		.cca_interval_change = 0,
		.cca_20M_level_change = 0,
		.cca_40M_level_change = 0,
	};
	pSstar_cca_val = (struct Sstar_cca_val *)buff;
	Sstar_printk_always(
		KERN_ERR
		"%s:cca_interval(%d),cca_20M_level(%d),cca_40M_level(%d)",
		__func__, pSstar_cca_val->cca_interval,
		pSstar_cca_val->cca_20M_level, pSstar_cca_val->cca_40M_level);
	if (pSstar_cca_val->cca_interval != SSTAR_CCA_FREE)
		cca_change.cca_interval_change = 1;
	if (pSstar_cca_val->cca_20M_level != SSTAR_CCA_FREE)
		cca_change.cca_20M_level_change = 1;
	if (pSstar_cca_val->cca_40M_level != SSTAR_CCA_FREE)
		cca_change.cca_40M_level_change = 1;

	if (cca_change.cca_interval_change &&
	    (atomic_read(&hw_priv->cca_interval_ms) !=
	     pSstar_cca_val->cca_interval)) {
		Sstar_printk_always("%s:cca_interval_change(%d)\n", __func__,
				    pSstar_cca_val->cca_interval);
		atomic_set(&hw_priv->cca_interval_ms,
			   pSstar_cca_val->cca_interval);
		set_bit(WSM_SET_CHANTYPECHANGE_FLAGS__CCA_LEVEL_CHANGE,
			&hw_priv->change_bit);
	}

	if (cca_change.cca_20M_level_change) {
		Sstar_printk_always("%s:cca_20M_level_change(%d)\n", __func__,
				    pSstar_cca_val->cca_20M_level);
		atomic_set(&hw_priv->chw_sw_20M_level,
			   pSstar_cca_val->cca_20M_level);
	}
	if (cca_change.cca_40M_level_change) {
		Sstar_printk_always("%s:cca_40M_level_change(%d)\n", __func__,
				    pSstar_cca_val->cca_40M_level);
		atomic_set(&hw_priv->chw_sw_40M_level,
			   pSstar_cca_val->cca_40M_level);
	}
