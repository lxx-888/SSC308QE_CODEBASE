...
#define SSTAR_UAPSD_VIRTUAL_LINKID (WLAN_LINK_ID_MAX - 1)
#define SSTAR_DTIM_VIRTUAL_LINKID (WLAN_LINK_ID_MAX - 2)
static inline u32 Sstar_uapsd_virtual_linkid(void)
{
	return SSTAR_UAPSD_VIRTUAL_LINKID;
}
static inline u32 Sstar_dtim_virtual_linkid(void)
{
	return SSTAR_DTIM_VIRTUAL_LINKID;
}
...
struct hmac_configure {
	...
	u8 password_len;
	u8 password[64];
	u8 privacy : 1, psk_set : 1, bssid_set : 1, fixrate_set : 1;
	u8 ap_mode;
	u8 hw_fix_rate_id;
	u8 reserve;
...
/*Will be a pointer to a list of VIFs - Dynamically allocated */
	...
	struct Sstar_timer_list smartconfig_expire_timer;
	...
	*/
	...
	struct Sstar_timer_list chantype_timer;
	struct Sstar_work_struct get_cca_work;
	...
	* after the mechanism is clear */
	...
	struct Sstar_timer_list ba_timer; /*TODO: Same as above */
	spinlock_t ba_lock; /*TODO: Same as above */
	bool ba_ena; /*TODO: Same as above */
	struct Sstar_work_struct ba_work; /*TODO: Same as above */
#endif
#ifdef CONFIG_PM
	...
	struct Sstar_delayed_work advance_scan_timeout;
#endif /* CONFIG_SSTAR_APOLLO_TESTMODE */
	...
	struct Sstar_work_struct event_handler;
#ifndef CONFIG_RATE_HW_CONTROL
	/* TX rate policy cache */
	struct tx_policy_cache tx_policy_cache;
	struct Sstar_work_struct tx_policy_upload_work;
#endif
...
#endif
#ifdef SPI_BUS
	struct Sstar_timer_list spi_status_rx_ready_timer;
#endif
	...
	u8 etf_test_v2;
	//struct Sstar_timer_list etf_expire_timer;
	struct Sstar_work_struct etf_tx_end_work;
#ifdef SDIO_BUS
	...
	atomic_t flushed;
#endif
	struct Sstar_work_struct wsm_sync_channl;
	int syncChanl_done;
...
#ifdef SDIO_BUS
	int sdio_status;
	struct mutex sdio_status_lock;
#endif
};

#ifdef SSTAR_SDIO_PATCH
typedef enum { IS_CMD, IS_MGMT, IS_DATA } TxTypeFlag;
#endif
#ifdef SSTAR_SUPPORT_WIDTH_40M
typedef enum { notifyPeer20M, notifyPeer40M, notifyPeerMax } notifyPeerAction_t;
#endif
...
struct Sstar_ba_tid_params {
	...
	struct Sstar_timer_list overtime_timer;
...
struct Sstar_reorder_queue_comm {
	...
	struct Sstar_work_struct reorder_work;
};
...
/*TODO: Split into Common and VIF parts */
	...
	struct Sstar_work_struct update_filtering_work;
#endif
	struct Sstar_work_struct set_beacon_wakeup_period_work;
#ifdef CONFIG_PM
	...
	struct Sstar_work_struct join_work;
	struct Sstar_delayed_work join_timeout;
	struct Sstar_work_struct unjoin_work;

	/* ROC implementation */
#ifdef CONFIG_SSTAR_SUPPORT_P2P
	struct Sstar_work_struct offchannel_work;
	struct Sstar_delayed_work pending_offchanneltx_work;
#endif
	...
	struct Sstar_work_struct wep_key_work;
	unsigned long rx_timestamp;
	u32 cipherType;
	...
	struct Sstar_work_struct link_id_work;
	struct Sstar_delayed_work link_id_gc_work;
	u32 sta_asleep_mask;
	u32 pspoll_mask;
	bool aid0_bit_set;
	...
	bool tx_multicast;
	struct Sstar_work_struct set_tim_work;
	//	struct Sstar_delayed_work	set_cts_work;
	struct Sstar_work_struct multicast_start_work;
	struct Sstar_work_struct multicast_stop_work;
	struct Sstar_timer_list mcast_timeout;
	//dhcp
	struct Sstar_delayed_work dhcp_retry_work;
	struct sk_buff *dhcp_retry_skb;
	spinlock_t dhcp_retry_spinlock;
#ifndef CONFIG_TX_NO_CONFIRM
	/* CQM Implementation */
	struct Sstar_delayed_work bss_loss_work;
	struct Sstar_delayed_work connection_loss_work;
	struct Sstar_work_struct tx_failure_work;
	...
	struct Sstar_work_struct linkid_reset_work;
	u8 action_frame_sa[ETH_ALEN];
	u8 action_linkid;
	bool htcap;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 3, 0))
	u16 ht_info;
	struct Sstar_work_struct ht_info_update_work;
#endif
#ifdef SSTAR_PKG_REORDER
	struct Sstar_reorder_queue_comm
		Sstar_reorder_link_id[SSTAR_APOLLO_LINK_ID_UNMAPPED];
#endif

#ifdef SSTAR_SUPPORT_WIDTH_40M
#ifdef CONFIG_SSTAR_40M_AUTO_CCA
	struct Sstar_delayed_work chantype_change_work;
#endif
#endif
#ifdef SSTAR_SUPPORT_SMARTCONFIG
	u8 scan_expire;
...
static inline struct Sstar_common *
ABwifi_vifpriv_to_hwpriv(struct Sstar_vif *priv)
{
	return priv->hw_priv;
}

static inline struct Sstar_vif *
ABwifi_get_vif_from_ieee80211(struct ieee80211_vif *vif)
{
...
static inline struct Sstar_vif *
ABwifi_hwpriv_to_vifpriv(struct Sstar_common *hw_priv, int if_id)
{
	...
	vif = ABwifi_get_vif_from_ieee80211(
		SSTAR_HW_VIF_GET(hw_priv->vif_list[if_id]));
	WARN_ON(!vif);
	if (vif)
...
#endif
static inline struct Sstar_vif *
__ABwifi_hwpriv_to_vifpriv(struct Sstar_common *hw_priv, int if_id)
{
...
static inline struct Sstar_vif *
ABwifi_get_activevif(struct Sstar_common *hw_priv)
{
...
static inline void Sstar_process_active_vif_queues(void *data, u8 *mac,
						   struct ieee80211_vif *vif)
{
	int lock = (*(int *)data);
...
#define Sstar_for_each_vif(_hw_priv, _priv, _i)                                \
	for (_i = 0; _priv = _hw_priv->vif_list[_i] ?                          \
					   ABwifi_get_vif_from_ieee80211(            \
					     _hw_priv->vif_list[_i]) :         \
					   NULL,                                     \
	    _i < SSTAR_WIFI_MAX_VIFS;                                          \
	     _i++)

#define Sstar_for_each_vif_safe(_hw_priv, _priv, _i)                           \
	for (_i = 0; _priv = ABwifi_get_vif_from_ieee80211(                    \
			     SSTAR_HW_VIF_GET(_hw_priv->vif_list[_i])),        \
	    _i < SSTAR_WIFI_MAX_VIFS;                                          \
	     _i++)
#define SSTAR_VOLATILE_SET(s, v)                                               \
	*((volatile typeof(*(s)) *)(s)) = (v);                                 \
	smp_mb()
...
#endif
#ifdef CONFIG_SSTAR_SUPPORT_P2P
void Sstar_clear_wpas_p2p_40M_ie(struct Sstar_ieee80211_mgmt *mgmt,
				 u32 pkg_len);
#endif
#endif
#ifdef SSTAR_PKG_REORDER
...
extern int Sstar_save_efuse(struct Sstar_common *hw_priv,
			    struct efuse_headr *efuse_save);
extern void Sstar_destroy_wsm_cmd(struct Sstar_common *hw_priv);
extern int Sstar_reinit_firmware(struct Sstar_common *hw_priv);
extern int Sstar_rx_bh_flush(struct Sstar_common *hw_priv);
extern void Sstar_bh_halt(struct Sstar_common *hw_priv);
extern void Sstar_hif_status_set(int status);
extern void Sstar_bh_multrx_trace(struct Sstar_common *hw_priv, u8 n_rx);
