...
void Sstar_enable_powersave(struct Sstar_vif *priv, bool enable)
{
...
void Sstar_monitor_pc(struct Sstar_common *hw_priv)
{
	...
	for (i = 0; i < 10; i++) {
		Sstar_direct_read_reg_32(
			hw_priv, hw_priv->wsm_caps.exceptionaddr + 4 * i + 4,
			&testreg1[i]);
	}
	...
	Sstar_printk_err("ERROR !! pc:[%x],ipc[%x] \n", testreg_pc,
			 testreg_ipc);
	Sstar_printk_err(
		"ERROR !! reg0:[%x],reg1[%x],reg2[%x],reg3[%x],reg4[%x],reg5[%x],reg6[%x] \n",
		testreg1[0], testreg1[1], testreg1[2], testreg1[3], testreg1[4],
		testreg1[5], testreg1[6]);

	Sstar_printk_err("[PC]:0x16101028(%x)\n", val28);
	Sstar_printk_err("[PC]:0x16101020(%x)\n", val20);
}
...
void Sstar_xmit_linearize(struct Sstar_common *hw_priv, struct wsm_tx *wsm,
			  char *xmit, int xmit_len)
{
	...
	while (wsm_id == WSM_TRANSMIT_REQ_MSG_ID) {
		...
		if (Sstar_queue_get_skb(queue, wsm->packetID, &skb, &txpriv) !=
		    0) {
			WARN_ON(1);
			break;
		}
		...
		for (sg = 0; sg < skb_shinfo(skb)->nr_frags; sg++) {
			...
			memcpy(xmit,
			       page_address(frag->bv_page) + frag->bv_offset,
			       frag->bv_len);
		...
		if (tx_info->sg_tailneed) {
			...
			if (sg_len + tx_info->sg_tailneed != xmit_len) {
				Sstar_printk_err(
					"sg_len(%d),xmit_len(%d),sg_tailneed(%d)\n",
...
int Sstar_save_efuse(struct Sstar_common *hw_priv,
		     struct efuse_headr *efuse_save)
{
	...
	*/
	...
	if (iResult == 0) {
		...
		if (memcmp((void *)&efuse_bak, efuse_save,
...
void Sstar_destroy_wsm_cmd(struct Sstar_common *hw_priv)
{
	/*
	*flush work
	*/
	...
	*/
	if (atomic_read(&hw_priv->scan.in_progress)) {
		...
		*/
		...
		if (atomic_read(&hw_priv->scan.in_progress) &&
		    ...
		    (Sstar_hw_cancel_delayed_work(&hw_priv->scan.timeout,
						  true) > 0)) {
	...
	Sstar_flush_workqueue(hw_to_local(hw_priv->hw)->workqueue);
	Sstar_flush_workqueue(hw_priv->workqueue);

	synchronize_net();
}

int Sstar_reinit_firmware(struct Sstar_common *hw_priv)
{
...
void Sstar_bh_halt(struct Sstar_common *hw_priv)
{
#if 0
	if (hw_priv->sdio_status == 1 &&
	    atomic_add_return(1, &hw_priv->bh_halt) == 1) {
	...
	if(hw_priv->wsm_cmd.ptr == NULL){
		if(hw_priv->wsm_cmd.cmd != 0xFFFF){
			Sstar_printk_err("%s:release cmd [%x]\n",__func__,hw_priv->wsm_cmd.cmd);
		...
		Sstar_printk_err("%s:cmd[%x] not send\n",__func__,hw_priv->wsm_cmd.cmd);
	}
	spin_unlock_bh(&hw_priv->wsm_cmd.lock);
#endif
}
void Sstar_bh_multrx_trace(struct Sstar_common *hw_priv, u8 n_rx)
{
