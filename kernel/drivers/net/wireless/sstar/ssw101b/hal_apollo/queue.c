...
static inline void __Sstar_queue_lock(struct Sstar_queue *queue, u8 if_id)
{
	...
	if (queue->tx_locked_cnt[if_id]++ == 0) {
		ieee80211_stop_queue(stats->hw_priv->hw,
				     queue->queue_id + 4 * if_id);
	} else {
		BUG_ON(ieee80211_queue_stopped(stats->hw_priv->hw,
					       queue->queue_id + 4 * if_id) ==
		       0);
	}
}
...
static inline void __Sstar_queue_unlock(struct Sstar_queue *queue, u8 if_id)
{
	...
	if (--queue->tx_locked_cnt[if_id] == 0) {
		ieee80211_wake_queue(stats->hw_priv->hw,
				     queue->queue_id + 4 * if_id);
	}
}
#endif
					     ...
					     u8 if_id, u8 link_id)
{
	...
	return ((u32)item_id << 0) | ((u32)item_generation << 8) |
	       ((u32)queue_id << 16) | ((u32)if_id << 20) |
				...
				struct list_head *gc_list)
{
	...
	while (!list_empty(gc_list)) {
		item = list_first_entry(gc_list, struct Sstar_queue_item, head);
		list_del(&item->head);
					 ...
					 struct Sstar_queue_item *item)
{
	...
	gc_item = Sstar_kmalloc(sizeof(struct Sstar_queue_item), GFP_ATOMIC);
	BUG_ON(!gc_item);
...
static void __Sstar_queue_gc(struct Sstar_queue *queue, struct list_head *head,
			     bool unlock)
{
	struct Sstar_queue_stats *stats = queue->stats;
	...
	while (!list_empty(&queue->queue)) {
		struct Sstar_txpriv *txpriv;
		item = list_first_entry(&queue->queue, struct Sstar_queue_item,
					head);
...
{
	LIST_HEAD(list);
	struct Sstar_queue *queue = (struct Sstar_queue *)arg;
	...
	if (queue->overfull[if_id_clear]) {
		...
		if (queue->num_queued_vif[if_id_clear] <= capacity) {
			...
			if (item->txpriv.if_id == if_id_clear) {
				unsigned long tmo =
					item->queue_timestamp + queue->ttl;
				Sstar_mod_timer(&queue->gc[if_id_clear], tmo);
#ifdef CONFIG_PM
				Sstar_pm_stay_awake(&stats->hw_priv->pm_state,
						    tmo - jiffies);
#endif
				...
				Sstar_printk_err(
					"%s:if_id[%d],id_clear[%d],queued[%d]\n",
					__func__, item->txpriv.if_id,
					if_id_clear,
					queue->num_queued_vif[if_id_clear]);
			}
		}
	}
}
...
static void Sstar_queue_gc(unsigned long arg)
{
	...
	struct Sstar_queue *queue = container_of(
		timer_to_if_id, struct Sstar_queue, timer_to_if_id[0]);
	spin_lock_bh(&queue->lock);
...
#endif
int Sstar_queue_stats_init(struct Sstar_queue_stats *stats, size_t map_capacity,
			   ...
			   struct Sstar_common *hw_priv)
{
	...
	for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++) {
		stats->link_map_cache[i] =
			Sstar_kzalloc(sizeof(int) * map_capacity, GFP_KERNEL);
...
int Sstar_queue_init(struct Sstar_queue *queue, struct Sstar_queue_stats *stats,
		     u8 queue_id, size_t capacity, unsigned long ttl)
{
...
#ifndef SSTAR_WIFI_QUEUE_LOCK_BUG
	Sstar_init_timer(&queue->gc);
	queue->gc.data = (unsigned long)queue;
	queue->gc.function = Sstar_queue_gc;
#else
	for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++) {
		Sstar_init_timer(&queue->gc[i]);
	...
	for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++) {
		queue->link_map_cache[i] = Sstar_kzalloc(
			sizeof(int) * stats->map_capacity, GFP_KERNEL);
...
#ifndef SSTAR_WIFI_QUEUE_LOCK_BUG
	Sstar_del_timer_sync(&queue->gc);
#else
	for (i = 0; i < SSTAR_WIFI_MAX_VIFS; i++) {
		Sstar_del_timer_sync(&queue->gc[i]);
	}
#endif
	INIT_LIST_HEAD(&queue->free_pool);
	Sstar_kfree(queue->pool);
...
size_t Sstar_queue_get_num_queued(struct Sstar_vif *priv,
				  struct Sstar_queue *queue, u32 link_id_map)
{
	size_t ret;
	int i, bit;
...
#endif
int Sstar_queue_put(struct Sstar_queue *queue, struct sk_buff *skb,
		    struct Sstar_txpriv *txpriv)
{
	int ret = 0;
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
	...
	if (!WARN_ON(list_empty(&queue->free_pool))) {
		...
		item->packetID = Sstar_queue_make_packet_id(
			queue->generation, queue->queue_id, item->generation,
			item - queue->pool, txpriv->if_id, txpriv->raw_link_id);
		...
		if (queue->overfull == false &&
		    queue->num_queued >=
			    (queue->capacity - (num_present_cpus()))) {
			queue->overfull = true;
			__Sstar_queue_lock(queue);
			Sstar_mod_timer(&queue->gc, jiffies);
		}
#else
		{
...
#define ALLIF_QUEUE_OVERFLOW(__queue)                                          \
	(__queue->num_queued >= (__queue->capacity - (num_present_cpus())))
			...
			if (queue->overfull[txpriv->if_id] == false) {
				...
				priv = __ABwifi_hwpriv_to_vifpriv(
					stats->hw_priv, txpriv->if_id);
				if (priv)
					capacity = priv->queue_cap;
				else
					capacity = queue->capacity / 2;
				if (xIF_QUEUE_OVERFLOW(queue, txpriv->if_id,
						       capacity) ||
				    ALLIF_QUEUE_OVERFLOW(queue)) {
					queue->overfull[txpriv->if_id] = true;
					__Sstar_queue_lock(queue,
							   txpriv->if_id);
					Sstar_mod_timer(
						&queue->gc[txpriv->if_id],
						jiffies);
				}
			} else {
#if 0
...
int Sstar_queue_get(struct Sstar_queue *queue, int if_id, u32 link_id_map,
		    struct wsm_tx **tx, struct ieee80211_tx_info **tx_info,
		    struct Sstar_txpriv **txpriv)
{
	int ret = -ENOENT;
	struct Sstar_queue_item *item;
	...
	if (!WARN_ON(ret)) {
		...
		*/
		if (queue_generation != (queue->generation & 0xF)) {
			...
			item->packetID |= ((u32)(queue->generation & 0xF))
					  << 28;
			BUG_ON(((item->packetID >> 28) & 0xF) !=
			       queue->generation);
		}
...
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
int Sstar_queue_requeue(struct Sstar_common *hw_priv, struct Sstar_queue *queue,
...
#endif
{
	...
	if (unlikely(queue_generation != queue->generation)) {
		...
		item->packetID =
			Sstar_queue_make_packet_id(queue_generation, queue_id,
...
#endif
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
int Sstar_queue_remove(struct Sstar_common *hw_priv, struct Sstar_queue *queue,
		       u32 packetID)
#else
		...
		if (hw_priv->start_stop_tsm.start) {
			if (queue_id == hw_priv->tsm_info.ac) {
				...
				if (tmval.tv_usec > item->qdelay_timestamp)
					...
					queue_delay = tmval.tv_usec + 1000000 -
						      item->qdelay_timestamp;
				...
				if (tmval.tv_usec > item->mdelay_timestamp)
					...
					media_delay = tmval.tv_usec + 1000000 -
						      item->mdelay_timestamp;
