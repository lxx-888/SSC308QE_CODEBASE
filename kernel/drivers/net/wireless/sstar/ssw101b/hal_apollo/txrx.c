...
void Sstar_tx_udp(struct ieee80211_hw *dev, struct sk_buff *skb)
{
	...
	struct Sstar_vif *priv =
		ABwifi_get_vif_from_ieee80211(tx_info->control.vif);
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		if (IS_BOOTP_PORT(ntohs(udph->source), ntohs(udph->dest))) {
			...
			if (SDATA_IS_CONNECTTING(vif_to_sdata(priv->vif))) {
				priv->dhcp_retry_skb =
					Sstar_skb_copy(skb, GFP_ATOMIC);
				if (priv->dhcp_retry_skb == NULL) {
					spin_unlock_bh(
						&priv->dhcp_retry_spinlock);
					return;
				}
				Sstar_hw_priv_queue_delayed_work(
					priv->hw_priv, &priv->dhcp_retry_work,
					HZ / 2);
				//memcpy(priv->dhcp_retry_skb->data,skb->data,skb->len);
				memcpy(IEEE80211_SKB_CB(priv->dhcp_retry_skb),
				       IEEE80211_SKB_CB(skb),
				       sizeof(struct ieee80211_tx_info));
				Sstar_printk_rx(
					"start Sstar_hw_priv_queue_delayed_work\n");
			} else {
				Sstar_hw_cancel_delayed_work(
					&priv->dhcp_retry_work, false);
			}
...
void Sstar_dhcp_retry_work(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
	...
	if ((priv->dhcp_retry_skb != NULL) &&
		...
		if (skb == NULL) {
			spin_unlock_bh(&priv->dhcp_retry_spinlock);
			Sstar_hw_priv_queue_delayed_work(
				priv->hw_priv, &priv->dhcp_retry_work, HZ / 50);
			return;
		}
		memcpy(IEEE80211_SKB_CB(skb),
		       IEEE80211_SKB_CB(priv->dhcp_retry_skb),
		       sizeof(struct ieee80211_tx_info));
		...
		if (tx_dhcp_retry_cnt < 3) {
			Sstar_hw_priv_queue_delayed_work(
				priv->hw_priv, &priv->dhcp_retry_work, HZ / 2);
		} else {
			Sstar_kfree_skb(priv->dhcp_retry_skb);
			priv->dhcp_retry_skb = NULL;
		}
...
static void Sstar_check_prov_desc_req(struct Sstar_common *hw_priv, u8 *action)
{
	if (action[2] == 0x50 && action[3] == 0x6F && action[4] == 0x9A &&
			    ...
			    struct ieee80211_tx_rate *rates, size_t count)
{
	...
	tx_policy_printk("[TX policy] Policy (%d): "
			 "%d:%d, %d:%d, %d:%d, %d:%d, %d:%d\n",
			 count, rates[0].idx, rates[0].count, rates[1].idx,
			 rates[1].count, rates[2].idx, rates[2].count,
			 rates[3].idx, rates[3].count, rates[4].idx,
			 rates[4].count);
}
...
static int tx_policy_upload(struct Sstar_common *hw_priv)
{
	struct tx_policy_cache *cache = &hw_priv->tx_policy_cache;
	int i;
	struct wsm_set_tx_rate_retry_policy arg = { .hdr = {
							    .numTxRatePolicies =
								    0,
						    } };
	int if_id = 0;
...
void tx_policy_upload_work(struct Sstar_work_struct *work)
{
...
static const u8 Sstar_get_tx_rate_hw_value(const struct Sstar_common *hw_priv,
					   const struct ieee80211_tx_rate *rate)
{
	if (rate->idx < 0) {
		//WARN_ON(1);
...
#endif
static int Sstar_tx_h_calc_link_ids(struct Sstar_vif *priv,
				    struct Sstar_txinfo *t)
{
	...
	if (priv->join_status == SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN) {
		...
		if (priv->enable_beacon) {
			t->txpriv.raw_link_id = 0;
			t->txpriv.link_id = Sstar_dtim_virtual_linkid();
		} else {
			t->txpriv.raw_link_id = 0;
			t->txpriv.link_id = 0;
		}
	} else {
		t->txpriv.link_id = Sstar_find_link_id(priv, t->da);
		...
		if (!(t->txpriv.link_id) &&
			...
			t->txpriv.link_id = 0;
			Sstar_printk_debug("%s:send no id\n", __func__);
		} else {
			if (!t->txpriv.link_id)
				t->txpriv.link_id =
					Sstar_alloc_link_id(priv, t->da);
			if (!t->txpriv.link_id) {
				Sstar_printk_err(
	...
	if (t->tx_info->control.sta &&
		...
		t->txpriv.link_id = Sstar_uapsd_virtual_linkid();
	return 0;
}

static void Sstar_tx_h_pm(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
...
static void Sstar_tx_h_calc_tid(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
...
static int Sstar_tx_h_crypt(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
	u32 iv_len;
	u32 icv_len;
	u8 *newhdr;
	...
	if (t->tx_info->control.hw_key) {
		if (t->tx_info->control.hw_key->cipher ==
		    WLAN_CIPHER_SUITE_AES_CMAC) {
			Sstar_printk_debug("11w WLAN_CIPHER_SUITE_AES_CMAC\n");
			mmie = (struct ieee80211_mmie *)Sstar_skb_put(
				t->skb, sizeof(*mmie));
			memset(mmie, 0, sizeof(struct ieee80211_mmie));
			mmie->element_id = WLAN_EID_MMIE;
			mmie->length = sizeof(*mmie) - 2;
			mmie->key_id =
				cpu_to_le16(t->tx_info->control.hw_key->keyidx);
...
#else
	{
		struct Sstar_ieee80211_mgmt *mgmt =
			(struct Sstar_ieee80211_mgmt *)t->skb->data;
		if (ieee80211_is_deauth(mgmt->frame_control)) {
			...
			if (!memcmp(mgmt->da, broadcast_addr,
				    sizeof(broadcast_addr))) {
				extern u8 aes_mac_key_index;
				Sstar_printk_debug(
					"11w WLAN_CIPHER_SUITE_AES_CMAC,key_index(%d)\n",
					aes_mac_key_index);
				mmie = (struct ieee80211_mmie *)Sstar_skb_put(
					t->skb, sizeof(*mmie));
				memset(mmie, 0, sizeof(struct ieee80211_mmie));
	...
	if (!skb_is_nonlinear(t->skb)) {
		if ((Sstar_skb_headroom(t->skb) + Sstar_skb_tailroom(t->skb) <
			...
			u8 *p;
			if (!(t->tx_info->control.hw_key->flags &
			      IEEE80211_KEY_FLAG_ALLOC_IV)) {
		...
		Sstar_skb_put(t->skb, icv_len);
	} else {
		if (Sstar_skb_headroom(t->skb) <
		    iv_len + WSM_TX_EXTRA_HEADROOM) {
			Sstar_printk_warn(
...
static int Sstar_tx_h_align(struct Sstar_vif *priv, struct Sstar_txinfo *t,
			    u8 *flags)
{
	...
	if (offset & 1) {
		Sstar_printk_err("Bug: attempt to transmit a frame "
	...
	if (Sstar_skb_headroom(t->skb) < offset) {
		Sstar_printk_err("Bug: no space allocated "
				 "for DMA alignment.\n"
				 "headroom: %d\n",
				 Sstar_skb_headroom(t->skb));
		return -ENOMEM;
...
static int Sstar_tx_h_action(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
	struct Sstar_ieee80211_mgmt *mgmt =
...
#endif
/* Add WSM header */
static struct wsm_tx *Sstar_tx_h_wsm(struct Sstar_vif *priv,
				     struct Sstar_txinfo *t)
{
	struct wsm_tx *wsm;
	...
	if (t->skb->len + t->tx_info->sg_tailneed < 1538)
		wsm->hdr.usb_len =
			__cpu_to_le16(1538); //__cpu_to_le16(t->skb->len);
	else
		wsm->hdr.usb_len =
			__cpu_to_le16(t->skb->len + t->tx_info->sg_tailneed);
...
static void Sstar_tx_h_bt(struct Sstar_vif *priv, struct Sstar_txinfo *t,
			  struct wsm_tx *wsm)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	...
	if (unlikely(ieee80211_is_nullfunc(t->hdr->frame_control)))
		...
		if (mgt_frame->u.assoc_req.listen_interval <
			...
			txrx_printk("Modified Listen Interval to %d from %d\n",
				    priv->listen_interval,
...
static inline s8 Sstar_rate_lowest_index(struct ieee80211_supported_band *sband,
...
static bool Sstar_rate_control_send_low(struct Sstar_common *hw_priv,
					struct Sstar_txinfo *t)
{
	struct ieee80211_supported_band *sband =
		hw_priv->hw->wiphy->bands[hw_priv->channel->band];
	struct ieee80211_tx_info *tx_info = t->tx_info;
	struct sta_info *info_sta = NULL;
	bool assoc = false;

	if (tx_info->control.sta)
		info_sta = container_of(tx_info->control.sta, struct sta_info,
					sta);
	if (info_sta)
	...
	if ((assoc == false) ||
		...
		if (!(tx_info->flags & IEEE80211_TX_CTL_NO_CCK_RATE)) {
			tx_info->control.rates[0].idx = Sstar_rate_lowest_index(
				sband, tx_info->control.sta);
		} else {
			tx_info->control.rates[0].idx =
				Sstar_rate_lowest_non_cck_index(
					sband, t->tx_info->control.sta);
		}
		...
		*/
		while ((t->tx_info->control.vif->type ==
			...
			cbss = ieee80211_Sstar_get_bss(hw_priv->hw->wiphy,
						       hw_priv->channel,
						       t->hdr->addr1, NULL, 0,
						       0, 0);
			...
			if (bss == NULL) {
				ieee80211_Sstar_put_bss(hw_priv->hw->wiphy,
							cbss);
				break;
			}
			Sstar_printk_debug(
				"%s:supp_rates_len(%zu),rate_index(%d)\n",
				__func__, bss->supp_rates_len,
				tx_info->control.rates[0].idx);
			...
			Sstar_printk_debug(
				"%s:suport_rates(%x),rate_index(%d)\n",
				__func__, suport_rates,
				tx_info->control.rates[0].idx);
			if (suport_rates == 0) {
				ieee80211_Sstar_put_bss(hw_priv->hw->wiphy,
							cbss);
			...
			if (!(suport_rates &
			      BIT(tx_info->control.rates[0].idx))) {
...
static int Sstar_tx_h_rate_policy(struct Sstar_common *hw_priv,
				  struct Sstar_txinfo *t, struct wsm_tx *wsm)
{
	struct ieee80211_supported_band *sband =
		hw_priv->hw->wiphy->bands[hw_priv->channel->band];
	struct ieee80211_tx_info *tx_info = t->tx_info;
	...
	*/
	...
	if (Sstar_rate_control_send_low(hw_priv, t) == false) {
		if (tx_info->control.sta) {
			...
			*/
			if ((tx_info->control.sta->ht_cap.ht_supported ==
			     true) &&
			    (tx_info->control.sta->ht_cap.mcs.rx_mask[0])) {
				/*
				*only surport one stream
				*/
				if (tx_info->control.sta->ht_cap.mcs
					    .rx_mask[0] == 0xff) {
					tx_info->control.rates[0].idx = 7;
				} else {
					rate_mask = tx_info->control.sta->ht_cap
							    .mcs.rx_mask[0];
					tx_info->control.rates[0].idx =
						fls(rate_mask) - 1;
				}
				Sstar_printk_debug(
					"%s:last set (%d)\n", __func__,
					fls(tx_info->control.sta->ht_cap.mcs
						    .rx_mask[0]) -
						1);
				...
				if (hweight8(tx_info->control.sta->ht_cap.mcs
						     .rx_mask[0]) <= 3) {
					tx_info->flags |=
					   ...
					   ->supp_rates[hw_priv->channel->band]) {
				rate_mask = tx_info->control.sta->supp_rates
				...
				Sstar_printk_debug("%s:last set (%d)\n",
						   __func__,
						   fls(rate_mask) - 1);
#ifdef CONFIG_LMAC_RATECTRL_RATE_BUG_FIX
				if (hweight32(
					    tx_info->control.sta->supp_rates
						    [hw_priv->channel->band]) <
				    5) {
					tx_info->flags |=
						IEEE80211_TX_CTL_USE_MINRATE;
	...
	t->rate = Sstar_get_tx_rate(hw_priv, &t->tx_info->control.rates[0]);
				  ...
				  struct Sstar_txinfo *t, struct wsm_tx *wsm)
{
...
#if OLD_RATE_POLICY
	t->txpriv.rate_id =
		tx_policy_get(hw_priv, t->tx_info->control.rates,
...
#if (OLD_RATE_POLICY == 0)
	wsm->TxRateRetry[0] = Sstar_get_tx_rate_hw_value(
		hw_priv, &t->tx_info->control.rates[1]);
	...
	wsm->TxRateRetry[1] = Sstar_get_tx_rate_hw_value(
		hw_priv, &t->tx_info->control.rates[2]);
	...
	wsm->TxRateRetry[2] = Sstar_get_tx_rate_hw_value(
		hw_priv, &t->tx_info->control.rates[3]);
	...
	wsm->TxRateRetry[3] = Sstar_get_tx_rate_hw_value(
		hw_priv, &t->tx_info->control.rates[4]);
...
__rate_set_end:
	...
	if (time_is_before_jiffies(g_printf_time + 5 * HZ)) {
		...
		Sstar_printk_always("greenfieldMode = %s,",
				    (priv->association_mode.greenfieldMode) ?
						  "greenfield" :
						  "mixed");
		Sstar_printk_always("%s GI\n",
				    (t->tx_info->control.rates[0].flags &
				     IEEE80211_TX_RC_SHORT_GI) ?
						  "short" :
						  "long");
		Sstar_printk_always(
			"queuePending :[VO]<%d>[Vi]<%d>[BE]<%d>[BK]<%d> \n",
	...
	if (tx_policy_renew) {
		...
		if (Sstar_hw_priv_queue_work(
			    hw_priv, &hw_priv->tx_policy_upload_work) <= 0) {
			Sstar_tx_queues_unlock(hw_priv);
			wsm_unlock_tx(hw_priv);
		}
	}
...
#endif
static bool Sstar_tx_h_pm_state(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
	int was_buffered = 1;

	if (t->txpriv.link_id == Sstar_dtim_virtual_linkid() &&
...
static void Sstar_tx_h_ba_stat(struct Sstar_vif *priv, struct Sstar_txinfo *t)
{
	...
	if (!(hw_priv->ba_cnt_rx || hw_priv->ba_cnt)) {
		Sstar_mod_timer(&hw_priv->ba_timer,
...
#endif
static int Sstar_tx_h_skb_pad(struct Sstar_common *priv, struct wsm_tx *wsm,
			      struct sk_buff *skb)
{
...
void __Sstar_tx(struct ieee80211_hw *dev, struct sk_buff *skb)
{
	...
	if (!(t.tx_info->control.vif)) {
		Sstar_printk_err("[TX]:vif is NULL\n");
		goto drop;
	}
	...
	if (ieee80211_is_data_qos(frame->frame_control)) {
		...
		if (skbiphdr->protocol == 1) {
			...
			ack_policy = *p & 0x0078;
			*p++ = ack_policy | tid;
			for (index = 0; index < IEEE80211_TX_MAX_RATES;
			     index++) {
				if ((t.tx_info->control.rates[index].flags &
				     IEEE80211_TX_RC_MCS) &&
				    (t.tx_info->control.rates[index].idx !=
				     -1) &&
				    (t.tx_info->control.rates[index].idx <=
				     7)) {
					u8 tmp_rate_index =
						t.tx_info->control.rates[index]
							.idx;
					t.tx_info->control.rates[index].flags &=
						~IEEE80211_TX_RC_MCS;
					t.tx_info->control.rates[index].idx =
						send_rate[tmp_rate_index];
					t.tx_info->control.rates[index].count =
						10;
				}
			}
		}
	...
	if ((ieee80211_is_action(frame->frame_control)) &&
	    (mgmt->u.action.category == SSTAR_WLAN_CATEGORY_PUBLIC)) {
	...
	if (atomic_xchg(&hw_priv->bh_suspend_usb, 0)) {
		extern int Sstar_usb_pm_async(struct sbus_priv * self,
					      bool auto_suspend);
...
*/
	...
	if (ieee80211_is_auth(frame->frame_control)) {
		Sstar_printk_pm("[TX]:authen, delay suspend\n");
		Sstar_pm_stay_awake(&hw_priv->pm_state, 5 * HZ);
	}
#endif
	if (ieee80211_is_deauth(frame->frame_control)) {
		Sstar_printk_pm("[TX]:[%d]:deatuhen[%pM]\n", priv->if_id,
				ieee80211_get_DA(t.hdr));
	}

	t.txpriv.if_id = priv->if_id;
	t.hdrlen = ieee80211_hdrlen(t.hdr->frame_control);
	t.da = ieee80211_get_DA(t.hdr);
	t.sta_priv = (struct Sstar_sta_priv *)&t.tx_info->control.sta->drv_priv;
	...
	*/
	...
	spin_lock_bh(&priv->ps_state_lock);
	{
		...
		BUG_ON(Sstar_queue_put(&hw_priv->tx_queue[t.queue], t.skb,
				       &t.txpriv));
	}
	spin_unlock_bh(&priv->ps_state_lock);
...
static int Sstar_handle_pspoll(struct Sstar_vif *priv, struct sk_buff *skb)
{
	struct Sstar_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
	struct ieee80211_sta *sta;
	 ...
	 * queued already. */
	for (i = 0; i < 4; ++i) {
		if (Sstar_queue_get_num_queued(priv, &hw_priv->tx_queue[i],
					       pspoll_mask)) {
			Sstar_bh_wakeup(hw_priv);
			drop = 1;
			break;
			 ...
			 struct wsm_tx_confirm *arg)
{
	...
	if (unlikely(!priv)) {
		Sstar_printk_err("[confirm]:arg_if_id %d\n", arg->if_id);
		return;
	}
	...
	hw_priv->Sstar_tsm_stats[queue_id].multi_retry_count +=
		arg->ackFailures;
	...
	if ((arg->status == WSM_REQUEUE) &&
		...
		Sstar_printk_warn("Requeue for link_id %d (try %d)."
				  " STAs asleep: 0x%.8X\n",
				  arg->link_id,
...
#ifdef CONFIG_SSTAR_APOLLO_TESTMODE
		WARN_ON(Sstar_queue_requeue(hw_priv, queue, arg->packetID,
					    true));
#else
		WARN_ON(Sstar_queue_requeue(queue, arg->packetID, true));
#endif
		spin_lock_bh(&priv->ps_state_lock);
		if (!arg->link_id) {
			priv->buffered_multicasts = true;
		...
		spin_unlock_bh(&priv->ps_state_lock);
		Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
	} else if (!WARN_ON(Sstar_queue_get_skb(queue, arg->packetID, &skb,
						&txpriv))) {
		...
		*/
		while (arg->status) {
			...
			WARN_ON(Sstar_queue_requeue(hw_priv, queue,
						    arg->packetID, true));
#else
			WARN_ON(Sstar_queue_requeue(queue, arg->packetID,
						    true));
#endif
		...
		*/
		while ((arg->status) && (arg->status != 1)) {
			...
			*/
			...
			if (Sstar_queue_get_generation(arg->packetID) >= 5) {
				Sstar_printk_err(
					"[confirm]:retry to many times(%x)\n",
					arg->packetID);
				break;
			}

			if (arg->status != WSM_STATUS_RETRY_EXCEEDED) {
				Sstar_printk_err("[confirm]:status(%d)\n",
						 arg->status);
				break;
			}
			...
			if (!time_is_after_jiffies(
				    hw_priv->roc_start_time +
				    msecs_to_jiffies(hw_priv->roc_duration +
						     35))) {
				Sstar_printk_err("[confirm]:timeout\n");
				break;
			}
			...
			Sstar_printk_tx(
				"%s:retry[%d][%x],start[%ld],duration[%ld],now(%ld)\n",
				__func__, arg->status, arg->packetID,
				hw_priv->roc_start_time,
				msecs_to_jiffies(hw_priv->roc_duration + 35),
				jiffies);
			Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
		...
		if (likely(!arg->status)) {
			...
			if (priv->bss_loss_status ==
				...
				Sstar_cancel_delayed_work(&priv->bss_loss_work);
				Sstar_hw_priv_queue_delayed_work(
		...
		Sstar_printk_err("[confirm]:<warning>\n");
		Sstar_priv_vif_list_read_unlock(&priv->vif_lock);
	}
}
...
void Sstar_skb_dtor(struct Sstar_common *hw_priv, struct sk_buff *skb,
		    const struct Sstar_txpriv *txpriv)
{
	struct Sstar_vif *priv =
	...
	if (priv && txpriv->rate_id != SSTAR_APOLLO_INVALID_RATE_ID) {
		Sstar_notify_buffered_tx(priv, skb, txpriv->raw_link_id,
					 txpriv->tid);
...
#ifdef CONFIG_SSTAR_BA_STATUS
static void Sstar_rx_h_ba_stat(struct Sstar_vif *priv, size_t hdrlen,
			       size_t skb_len)
{
	struct Sstar_common *hw_priv = priv->hw_priv;
	if (priv->join_status != SSTAR_APOLLO_JOIN_STATUS_STA)
...
#endif
void Sstar_rx_cb(struct Sstar_vif *priv, struct wsm_rx *arg,
		 struct sk_buff **skb_p)
{
	...
	if (unlikely(priv->mode == NL80211_IFTYPE_UNSPECIFIED)) {
		/* STA is stopped. */
		Sstar_printk_err(
			"priv->mode == NL80211_IFTYPE_UNSPECIFIED,seq(%x)\n",
			frame->seq_ctrl);
		goto drop;
	}
#ifdef CONFIG_SSTAR_SUPPORT_P2P
...
*/
	...
	if (unlikely(arg->status)) {
		...
		sta = (struct sta_info *)sta_info_get_rx(
			vif_to_sdata(priv->vif), frame->addr2);
		if (sta) {
			if (priv->join_status !=
			    SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)
				Sstar_printk_err(
					"[%s Rx]:[%pM],PTK[%p],FLAGS[%lx],skb->len[%d]\n",
					vif_to_sdata(priv->vif)->name,
					frame->addr2, sta->ptk, sta->_flags,
					skb->len);
		} else {
			if (priv->join_status !=
			    SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)
				Sstar_printk_err(
					"[%s RX]:[%pM] is not assoctiated with us\n",
					vif_to_sdata(priv->vif)->name,
					frame->addr2);
		}
		/*
		...
		if (sta && test_sta_flag(sta, WLAN_STA_MFP)) {
			if (Sstar_ieee80211_is_robust_mgmt_frame(skb) == true) {
				rcu_read_unlock();
				Sstar_printk_err("[RX]:drop PMF\n");
				goto drop;
			}
		}
		rcu_read_unlock();
		if (arg->status == WSM_STATUS_MICFAILURE) {
			if (priv->join_status !=
			    SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)
				Sstar_printk_err(
					"[RX] MIC failure. ENCRYPTION [%d][%pM][%pM]\n",
					WSM_RX_STATUS_ENCRYPTION(arg->flags),
			...
			if (priv->join_status !=
			    SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)
				Sstar_printk_err(
					"[RX] No key found.ENCRYPTION [%d][%pM][%pM]\n",
					WSM_RX_STATUS_ENCRYPTION(arg->flags),
					frame->addr1, frame->addr2);
			hdr->flag |= RX_FLAG_UNKOWN_STA_FRAME;
			if (priv->join_status != SSTAR_APOLLO_JOIN_STATUS_AP) {
			...
			if (priv->join_status !=
			    SSTAR_APOLLO_JOIN_STATUS_STA_LISTEN)
				Sstar_printk_err(
					"[RX] Receive failure: %d.ENCRYPTION [%d][%pM][%pM],fc(%x)\n",
					arg->status,
					WSM_RX_STATUS_ENCRYPTION(arg->flags),
					frame->addr1, frame->addr2,
					mgmt->frame_control);
	...
	if (skb->len < sizeof(struct ieee80211_pspoll)) {
		Sstar_printk_err(
			"(skb->len < sizeof(struct ieee80211_pspoll),seq(%x)\n",
			frame->seq_ctrl);
		goto drop;
	}
	...
	if (ieee80211_is_back_req(frame->frame_control)) {
		...
		case WLAN_ACTION_ADDBA_REQ: {
			...
			ba_params.timeout =
				le16_to_cpu(mgmt->u.action.u.addba_req.timeout);
			capab = le16_to_cpu(mgmt->u.action.u.addba_req.capab);
			ba_params.tid =
				(capab & IEEE80211_ADDBA_PARAM_TID_MASK) >> 2;
			ba_params.win_size =
				(capab & IEEE80211_ADDBA_PARAM_BUF_SIZE_MASK) >>
				6;
			ba_params.ssn = le16_to_cpu(mgmt->u.action.u.addba_req
							    .start_seq_num) >>
					4;
			Sstar_updata_ba_tid_params(priv, &ba_params);
			break;
		...
		case WLAN_ACTION_DELBA: {
			...
			ba_params.tid =
				(params & IEEE80211_DELBA_PARAM_TID_MASK) >> 12;
	...
	hdr->band = (arg->channelNumber > 14) ? IEEE80211_BAND_5GHZ :
						      IEEE80211_BAND_2GHZ;
	hdr->freq =
		ieee80211_channel_to_frequency(arg->channelNumber, hdr->band);
	...
	if (WSM_RX_STATUS_ENCRYPTION(arg->flags)) {
		...
		if (skb->len < hdrlen + iv_len + icv_len) {
			Sstar_printk_warn(
				"Mailformed SDU rx'ed.Size is lesser than crypto headers.\n");
	...
	if (WSM_RX_STATUS_ENCRYPTION(arg->flags)) {
		if (ieee80211_has_protected(frame->frame_control))
			...
			Sstar_printk_err("[RX]: open frame decrypted?\n");
		}
	} else {
		if (ieee80211_has_protected(frame->frame_control)) {
			Sstar_printk_err("[RX]:need sw decrypted?\n");
		}
	}
	...
	if (ieee80211_is_beacon(frame->frame_control) && !arg->status &&
...
#endif
		ies = ((struct Sstar_ieee80211_mgmt *)(skb->data))
			      ->u.beacon.variable;
	...
	if (priv->mode == NL80211_IFTYPE_AP &&
		...
		ies = ((struct Sstar_ieee80211_mgmt *)(skb->data))
			      ->u.beacon.variable;
		ies_len = skb->len - (ies - (u8 *)(skb->data));
		ht_cap = Sstar_get_ie(ies, ies_len,
				      SSTAR_WLAN_EID_HT_CAPABILITY);
		if (!ht_cap) {
			priv->ht_info |= 0x0011;
		}
	...
	*/
	if (((priv->vif->p2p == false)) &&
	    (ieee80211_is_action(frame->frame_control)) &&
	    (ieee80211_chw_is_ht40(vif_chw(priv->vif))) && (!arg->status)) {
		if ((mgmt->u.action.category == SSTAR_WLAN_CATEGORY_HT) &&
	...
	if ((ieee80211_is_beacon(frame->frame_control) ||
		...
		if (!memcmp(ieee80211_get_SA(frame), priv->join_bssid,
			    ETH_ALEN)) {
			if (hw_priv->beacon)
			...
			if (!hw_priv->beacon)
				Sstar_printk_err(
					"apollo: sched_scan: own beacon storing failed\n");
		}
	}
#endif /*ROAM_OFFLOAD*/
#endif
	...
	if (ieee80211_is_data_qos(frame->frame_control)) {
		if ((arg->link_id == 0) ||
		    (arg->link_id - 1 >= SSTAR_APOLLO_LINK_ID_UNMAPPED)) {
...
int Sstar_upload_keys(struct Sstar_vif *priv)
{
	...
	for (idx = 0; idx <= WSM_KEY_MAX_IDX; ++idx)
		if (hw_priv->key_map & BIT(idx)) {
			ret = wsm_add_key(hw_priv, &hw_priv->keys[idx],
					  priv->if_id);
			if (ret < 0)
				break;
		}
...
void Sstar_link_id_reset(struct Sstar_work_struct *work)
{
	struct Sstar_vif *priv =
