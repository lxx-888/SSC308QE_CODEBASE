				     ...
				     const struct drm_display_mode *mode)
{
	...
	struct reg_sequence reg_cfg[] = {
		{ 0x824f, 0x80 }, /* [7] = Select ad_txpll_d_clk. */
		{ 0x8250, 0x10 }, /* signal port switch: portA:10;  portB:14 */
		{ 0x8300, LT9611_4LANES },
		/* { 0x8303, 0x00 }, */ /* signal port switch: portA:00;  portB:40 */
#if 0
		{ 0x8302, 0x03 }, /* settle */
		{ 0x8306, 0x03 }, /* settle */
		{ 0x830a, 0x00 }, /* 0x00 = single_port_mipi, 0x03 = dual_port_mipi */
#endif
	};

	regmap_multi_reg_write(lt9611->regmap, reg_cfg, ARRAY_SIZE(reg_cfg));
	if (lt9611->is_settles) {
		regmap_write(lt9611->regmap, 0x8302, (u8)lt9611->settles[0]);
		regmap_write(lt9611->regmap, 0x8306, (u8)lt9611->settles[1]);
	} else {
		regmap_write(lt9611->regmap, 0x8302, 0x03); /* settle */
		regmap_write(lt9611->regmap, 0x8306, 0x03); /* settle */
	}
	if (mode->hdisplay == 3840)
		regmap_write(lt9611->regmap, 0x830a, 0x03); /* 0x03 = dual_port_mipi */
	else
		regmap_write(lt9611->regmap, 0x830a, 0x00); /* 0x00 = single_port_mipi */
...
#ifdef CONFIG_SSTAR_DRM
	regmap_write(lt9611->regmap, 0x82d6, 0x8e); /* HMDI mode: 0x8e = HDMI, 0x0e = DVI */
#else
...
static void __maybe_unused lt9611_enable_test_pattern(struct lt9611 *lt9611)
{
...
static void __maybe_unused lt9611_test_pattern_gcm(struct lt9611 *lt9611, const struct drm_display_mode *mode)
{
	u8 pol = 0;

	pol |= (mode->flags & DRM_MODE_FLAG_NHSYNC) ? 0x10 : 0;
	pol |= (mode->flags & DRM_MODE_FLAG_NVSYNC) ? 0x20 : 0;
	regmap_write(lt9611->regmap, 0x82a3, ((mode->htotal - mode->hsync_start) / 256)); /* de_delay */
	regmap_write(lt9611->regmap, 0x82a4, ((mode->htotal - mode->hsync_start) % 256));
	regmap_write(lt9611->regmap, 0x82a5, ((mode->vtotal - mode->vsync_start) % 256)); /* de_top */
	regmap_write(lt9611->regmap, 0x82a6, (mode->hdisplay / 256));
	regmap_write(lt9611->regmap, 0x82a7, (mode->hdisplay % 256)); /* de_cnt */
	regmap_write(lt9611->regmap, 0x82a8, (mode->vdisplay / 256));
	regmap_write(lt9611->regmap, 0x82a9, (mode->hdisplay % 256)); /* de_line */
	regmap_write(lt9611->regmap, 0x82aa, (mode->htotal / 256));
	regmap_write(lt9611->regmap, 0x82ab, (mode->htotal % 256)); /* htotal */
	regmap_write(lt9611->regmap, 0x82ac, (mode->vtotal  / 256));
	regmap_write(lt9611->regmap, 0x82ad, (mode->vtotal %  256)); /* vtotal */
	regmap_write(lt9611->regmap, 0x82ae, ((mode->hsync_end - mode->hsync_start) / 256));
	regmap_write(lt9611->regmap, 0x82af, ((mode->hsync_end - mode->hsync_start) %  256)); /* hvsa */
	regmap_write(lt9611->regmap, 0x82b0, ((mode->vsync_end - mode->vsync_start) % 256)); /* vsa */
	regmap_write(lt9611->regmap, 0x8247, (pol|0x07)); /* sync polarity */
}
...
#ifdef CONFIG_SSTAR_DRM
	if (lt9611->hpd_polled)
		lt9611->connector.polled = DRM_CONNECTOR_POLL_CONNECT | DRM_CONNECTOR_POLL_DISCONNECT;
#endif
...
#else
	if (0 == of_property_read_u32_array(dev->of_node, "settles", lt9611->settles, ARRAY_SIZE(lt9611->settles)))
		lt9611->is_settles = true;
#endif
			...
			const struct i2c_device_id *id)
{
	...
	if (ret) {
#ifdef CONFIG_SSTAR_DRM
		dev_warn(dev, "failed to request irq, polled instead!\n");
		lt9611->hpd_polled = true;
#else
