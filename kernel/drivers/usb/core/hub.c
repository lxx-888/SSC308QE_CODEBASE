...
void usb_disconnect(struct usb_device **pdev)
{
	...
	dev_info(&udev->dev, "USB disconnect, device number %d\n",
			udev->devnum);
#ifdef CONFIG_ARCH_SSTAR
	if (hdev && hdev->devpath[0] == '0')
		is_root = true;

	if (test_bit(1, &hcd->adjust_async_ladr)) {
		if (hdev && hdev->devpath[0] == '0') {
			clear_bit(1, &hcd->adjust_async_ladr);
			dev_info(&udev->dev, "%s remove adjust_async_ladr flag\n", __FUNCTION__);
		}
	}
#endif
	/*
	...
	*pdev = NULL;
	...
	put_device(&udev->dev);
#ifdef CONFIG_ARCH_SSTAR
	/* roothub ehci reset patch */
	if ((hcd->ehc_base != 0) && is_root && hcd->rh_registered) {
		hcd->driver->stop(hcd);
		...
		dev_info(hcd->self.controller, "reset ehci mac completed\n");
	}
#endif
	...
	if (!hub->error) {
		if (hub_is_superspeed(hub->hdev)) {
			...
			if (hcd->usb_phy && !hdev->parent)
				usb_phy_notify_disconnect(hcd->usb_phy, hub->hdev->speed);
#endif
		}
	}
		...
		u16 portchange)
{
	...
	if (portchange & (USB_PORT_STAT_C_CONNECTION | USB_PORT_STAT_C_ENABLE)) {
		if (hdev->devpath[0] == '0') {
			dev_info(&port_dev->dev, "%s: add adjust_async_ladr flag\n", __FUNCTION__);
			set_bit(1, &hcd->adjust_async_ladr);
		}
	}
#endif
	status = 0;
