...
no_int:
	...
	ss_int_sink_desc.bEndpointAddress =
		fs_int_sink_desc.bEndpointAddress;
#endif
...
static void source_sink_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (ss->buflen_vary)
	{
		...
		if (usb_endpoint_xfer_isoc(ep->desc))
			buflen = ss->iso_buflen;
		else if (usb_endpoint_xfer_bulk(ep->desc))
			buflen = ss->buflen;
		else
			buflen = ss->int_buflen;
		req->length += ss->buflen_vary;
		req->length = req->length % buflen;
		if (req->length == 0)
			req->length = (ss->buflen_vary < buflen) ? ss->buflen_vary : buflen;
	}
#endif
...
static int source_sink_start_ep(struct f_sourcesink *ss, bool is_in,
		bool is_iso, bool is_int, int speed)
#else
static int source_sink_start_ep(struct f_sourcesink *ss, bool is_in,
...
#ifdef CONFIG_ARCH_SSTAR
	ss->iso_buflen = ss_opts->iso_buflen;
	ss->int_buflen = ss_opts->int_buflen;
	ss->int_qlen = ss_opts->int_qlen;
	ss->buflen_vary = ss_opts->buflen_vary;
#endif
...
#ifdef CONFIG_ARCH_SSTAR
static ssize_t f_ss_opts_iso_buflen_show(struct config_item *item, char *page)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int result;
...
static ssize_t f_ss_opts_iso_buflen_store(struct config_item *item,
					   const char *page, size_t len)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int ret;
...
CONFIGFS_ATTR(f_ss_opts_, iso_buflen);

static ssize_t f_ss_opts_int_buflen_show(struct config_item *item, char *page)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int result;
...
static ssize_t f_ss_opts_int_buflen_store(struct config_item *item,
					   const char *page, size_t len)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int ret;
...
CONFIGFS_ATTR(f_ss_opts_, int_buflen);



static ssize_t f_ss_opts_buflen_vary_show(struct config_item *item, char *page)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int result;
...
static ssize_t f_ss_opts_buflen_vary_store(struct config_item *item,
					   const char *page, size_t len)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int ret;
...
CONFIGFS_ATTR(f_ss_opts_, buflen_vary);

static ssize_t f_ss_opts_int_qlen_show(struct config_item *item, char *page)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int result;
...
static ssize_t f_ss_opts_int_qlen_store(struct config_item *item,
					   const char *page, size_t len)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int ret;
...
CONFIGFS_ATTR(f_ss_opts_, int_qlen);
#endif
...
#ifdef CONFIG_ARCH_SSTAR
	&f_ss_opts_attr_iso_buflen,
	&f_ss_opts_attr_int_buflen,
	&f_ss_opts_attr_buflen_vary,
	&f_ss_opts_attr_int_qlen,
#endif
...
#ifdef CONFIG_ARCH_SSTAR
	ss_opts->iso_buflen = GZERO_ISO_BUFLEN;
	ss_opts->int_buflen = GZERO_INT_BUFLEN;
	ss_opts->buflen_vary = GZERO_BULK_BUFLEN_VARY;
	ss_opts->int_qlen = GZERO_SS_INT_QLEN;
#endif
