...
show_trb:
	for (; i < end; i++) {
		...
		seq_printf(s, "%03d %08x%08x,%08x,%s,%d,%d,%d,%d,%d,%d       %c%c\n",
				i, trb->bph, trb->bpl, trb->size,
...
#ifdef CONFIG_ARCH_SSTAR
static int dwc3_ep_trb_stat_show(struct seq_file *s, void *unused)
{
	struct dwc3_ep		*dep = s->private;
	struct dwc3		*dwc = dep->dwc;
	unsigned long		flags;
	...
	spin_lock_irqsave(&dwc->lock, flags);
	seq_printf(s, "total_trb_num: %d  isoc_miss_trb_num: %d isoc_miss_rate: %d%%\n",
		dep->total_trb_num, dep->isoc_miss_trb_num, 100*dep->isoc_miss_trb_num/dep->total_trb_num);
	spin_unlock_irqrestore(&dwc->lock, flags);
...
#ifdef CONFIG_ARCH_SSTAR
DEFINE_SHOW_ATTRIBUTE(dwc3_ep_trb_stat);
#endif
...
#ifdef CONFIG_ARCH_SSTAR
	{ "trb_statistics", &dwc3_ep_trb_stat_fops, },
#endif
};
