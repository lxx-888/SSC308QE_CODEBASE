...
#if defined(CONFIG_ARCH_SSTAR) && defined(_USB_TURN_ON_TT_THROTTLE_MODE_PATCH)
...
#if defined(CONFIG_ARCH_SSTAR) && defined(ENABLE_12US_EOF1)
...
struct usb_hcd  *ehci_sstar_hcd;
struct platform_device *ehci_hcd_sstar_platform_dev;
int vbus_gpio = -1;
#else
...
#ifdef CONFIG_ARCH_SSTAR
static unsigned ehci_sstar_read_frame_index(struct ehci_hcd *ehci)
{
...
static inline unsigned ehci_read_frame_index(struct ehci_hcd *ehci)
{
	...
	if (ehci_to_hcd(ehci)->ms_flag & MS_FLAG_SW_FRM_IDX)
		return ehci_sstar_read_frame_index(ehci);
#endif
...
#if defined(CONFIG_ARCH_SSTAR) && defined(ENABLE_UHC_RUN_BIT_ALWAYS_ON_ECO)
	/* Don't close RUN bit when device disconnect.
	 * set high speed:EOF1:16us, EOF2:133ns.
	 */
	ehci_writel(ehci, ehci_readl(ehci, &ehci->regs->hcmisc) | (BIT7 | BIT4| BIT2), &ehci->regs->hcmisc);
#endif
...
#if defined(CONFIG_ARCH_SSTAR) && defined(CONFIG_DYNAMIC_DEBUG)
	ehci_info(ehci, "USB peridic %px.%x qh %px.%x\n",
		ehci->periodic, (u32)ehci->periodic_dma, ehci->async, (u32)ehci->async->qh_dma);
#endif

#if defined(CONFIG_ARCH_SSTAR) && defined(DMA_OVER4GB_PATCH)
	dma_ehci_enable_over4GB(hcd, ehci->periodic_dma);
#endif
...
restart:
	 ...
	 */
#ifdef CONFIG_ARCH_SSTAR
	masked_status = current_status & (INTR_MASK | STS_FLR | STS_ERR_RXACTIVE);
#else
...
#ifdef FORCE_RESTART_EHCI_PATCH
    if (status & STS_ERR_RXACTIVE) {
        ehci_monitor_status = 1;
		ehci_sstar_hcd = hcd;
        ehci_hcd_sstar_platform_dev = container_of(hcd->self.controller, struct platform_device, dev);
        printk("ERR_INT_SOF_RXACTIVE issue @platform:%p\n", ehci_hcd_sstar_platform_dev);
#ifdef _USB_VBUS_RESET_PATCH
        if (ehci_hcd_sstar_platform_dev)
            vbus_gpio = vbus_gpio_of_parse_and_map(ehci_hcd_sstar_platform_dev->dev.of_node);
...
#ifdef CONFIG_ARCH_SSTAR
#include "ehci-sstar.c"
#define	PLATFORM_DRIVER		ehci_hcd_sstar_driver
	...
	while(1) {
		...
		if (ehci_monitor_status) {
			...
			if (ehci_sstar_hcd) {
				...
				usb_remove_hcd(ehci_sstar_hcd);
				usb_put_hcd(ehci_sstar_hcd);

				if (ehci_hcd_sstar_platform_dev) {
					ehci_hcd_sstar_drv_probe(ehci_hcd_sstar_platform_dev);
				} else
					printk(KERN_EMERG "ehci_hcd_sstar_platform_dev null\n");
			} else {
				printk(KERN_EMERG "ehci_sstar_hcd null\n");
			}
		}
		msleep(100);
	}
...
#endif /* FORCE_RESTART_EHCI_PATCH */
#endif /* CONFIG_ARCH_SSTAR */
...
#ifdef CONFIG_ARCH_SSTAR
	ehci_init_driver(&ehci_sstar_hc_driver, &ehci_sstar_overrides);
#ifdef FORCE_RESTART_EHCI_PATCH
...
#ifdef CONFIG_ARCH_SSTAR
	printk("sstar ehci version:%s\n", EHCI_SSTAR_VERSION);

#ifdef DYNAMIC_MIU_SIZE_MAPPING
...
MODULE_SOFTDEP("pre: phy-sstar-u2phy");
