...
ehci_urb_done(struct ehci_hcd *ehci, struct urb *urb, int status)
{
	...
	ehci_info (ehci,
		"%s %s urb %px ep%d%s status %d len %d/%d\n",
...
#ifdef CONFIG_DYNAMIC_DEBUG
	dump_ehci_statistics("@urb_done", ehci);
	dump_async("@urb_done", ehci);
#endif
#else
	ehci_dbg (ehci,
...
qh_urb_transaction (
	...
	dump_qtd_list("@qh_urb_transaction", ehci, urb, head);
	dump_transaction_msg("@qh_urb_transaction", ehci, urb);
#endif
	return head;
...
static void qh_link_async (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	dump_ehci_statistics("@qh_link_async", ehci);
	dump_async("@qh_link_async", ehci);
#endif
#endif
	enable_async(ehci);
}
...
#ifdef CONFIG_ARCH_SSTAR
		ehci_info(ehci,
			 "%s %s urb %px ep%d%s len %d, qtd %px [qh %px] %x\n",
			 __func__, urb->dev->devpath, urb,
			 epnum & 0x0f, (epnum & USB_DIR_IN) ? "in" : "out",
			 urb->transfer_buffer_length,
			 qtd, urb->ep->hcpriv, (u32)urb->transfer_dma);
#else
		ehci_dbg(ehci,
