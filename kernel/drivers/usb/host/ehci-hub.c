...
int phy_handshake(struct usb_phy* phy, u32 offset, u32 mask, u32 done, int usec)
{
	u32	result;

	do {
		result = usb_phy_io_read(phy, offset);
		result &= mask;
		if (result == done)
			return 0;
		udelay (1);
...
int ehci_hub_control(
	...
	case GetPortStatus:
		...
		/* resume completed */
			...
			temp = ehci_readl(ehci, status_reg);
#ifdef CONFIG_ARCH_SSTAR
			if (hcd->usb_monitor_enable) {
				/* enable usb monitor after resume */
				reg_val = usb_phy_io_read(hcd->usb_phy, 0x34);
				usb_phy_io_write(hcd->usb_phy, reg_val | BIT(7), 0x34);
			}
#endif
...
#ifdef CONFIG_ARCH_SSTAR
			if (hcd->usb_monitor_enable) {
				/* disable usb monitor in suspend */
				reg_val = usb_phy_io_read(hcd->usb_phy, 0x34);
				usb_phy_io_write(hcd->usb_phy, reg_val & (~(BIT(7))), 0x34);
			}

			/* wait dp dpm ready, timeout 5ms */
			retval = phy_handshake(hcd->usb_phy, 0x18, BIT(12) | BIT(13), 0x1 << 12, 5000);
			if (retval)
				ehci_err(ehci, "dp dm is not ready\n");

			/* reset power good */
			reg_val = usb_phy_io_read(hcd->usb_phy, 0x08);
			usb_phy_io_write(hcd->usb_phy, reg_val | BIT(12), 0x08);
			reg_val = usb_phy_io_read(hcd->usb_phy, 0x08);
			usb_phy_io_write(hcd->usb_phy, reg_val & (~(BIT(12))), 0x08);

			/* wait clk ready, timeout 2ms */
			retval = phy_handshake(hcd->usb_phy, 0x7, BIT(6), 0x1 << 6, 2000);
			if (retval)
				ehci_err(ehci, "clk is not ready\n");
#endif
