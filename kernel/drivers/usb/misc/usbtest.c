...
static int simple_check_buf(struct usbtest_dev *tdev, struct urb *urb)
{
	...
	if (pattern == 2)
		return 0;
	ret = check_guard_bytes(tdev, urb);
#else
...
static void complicated_callback(struct urb *urb)
{
	...
	if (urb->status == 0 && ctx->count > (ctx->pending - 1)
		...
		if (ctx->vary) {
			...
			bytes %= ctx->length;
			if (bytes == 0)
				bytes = (ctx->vary < ctx->length) ? ctx->vary : ctx->length;

			urb->transfer_buffer_length = bytes;
			maxp = usb_endpoint_maxp(&urb->ep->desc);
			if (urb->dev->speed >= USB_SPEED_SUPER)
				maxp *= ss_isoc_get_packet_num(urb->dev, urb->pipe);
			else
				maxp *= usb_endpoint_maxp_mult(&urb->ep->desc);
		...
		int pipe, struct usb_endpoint_descriptor *desc, unsigned offset)
{
...
#ifdef CONFIG_ARCH_SSTAR
	context.vary = param->vary;
	context.length = param->length;
#endif
	for (i = 0; i < param->sglen; i++) {
		if (context.is_iso)
	...
	for (i = 0; i < param->sglen; i++) {
		if (urbs[i]) {
#ifdef CONFIG_ARCH_SSTAR
			urbs[i]->transfer_buffer_length = param->length;
#endif
			simple_free_urb(urbs[i]);
        }
	}
...
#ifdef CONFIG_ARCH_SSTAR
		retval = simple_io(dev, urb, param->iterations, param->vary, 0, "test25");
#else
...
#ifdef CONFIG_ARCH_SSTAR
		retval = simple_io(dev, urb, param->iterations, param->vary, 0, "test26");
#else
