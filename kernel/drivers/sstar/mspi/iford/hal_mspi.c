...
static int hal_mspi_full_duplex_buf(struct hal_mspi *hal, u8 *rx_buff, u8 *tx_buff, u16 size)
{
    ...
    int ret        = HAL_MSPI_OK;
    u8  shift      = 0;
    u8  w_msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_WLSB_FIRST) & HAL_MSPI_REG_WLSB_FIRST_BIT);
    u8  r_msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_RLSB_FIRST) & HAL_MSPI_REG_RLSB_FIRST_BIT);
    ...
    for (index = 0; index < size; index++)
    {
        if (index & 1)
        {
            ...
            {
                shift = (w_msb_mode) ? (8 - hal->bits_per_word) : 0;
                value = (tx_buff[index] << (shift + 8)) | ((tx_buff[index - 1] << shift) & 0xff);
            }
            else
            {
                ...
                if (w_msb_mode)
                {
                    value = ((tx_buff[index] << shift) & 0xff) | (tx_buff[index - 1] << 8);
                }
                else
                {
                    // NO LSB
        ...
        {
            shift = (w_msb_mode) ? (8 - hal->bits_per_word) : 0;
            HAL_MSPI_WRITE((HAL_MSPI_REG_WRITE_BUF + (index >> 1)), (tx_buff[index] << shift) & 0xff);
        }
    }
...
static int hal_mspi_read_buf(struct hal_mspi *hal, u8 *data, u16 size)
{
    ...
    int ret = HAL_MSPI_OK;
    u8  shift;
    u8  msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_RLSB_FIRST) & HAL_MSPI_REG_RLSB_FIRST_BIT);
...
static int hal_mspi_write_buf(struct hal_mspi *hal, u8 *data, u16 size)
{
    ...
    int ret      = HAL_MSPI_OK;
    u8  shift    = 0;
    u8  msb_mode = !(HAL_MSPI_READ(HAL_MSPI_REG_WLSB_FIRST) & HAL_MSPI_REG_WLSB_FIRST_BIT);
    ...
    for (index = 0; index < size; index++)
    {
        if (index & 1)
        {
            if (hal->bits_per_word <= 8)
            {
                ...
                value = (data[index] << (shift + 8)) | ((data[index - 1] << shift) & 0xff);
            }
            else
            {
                shift = 16 - hal->bits_per_word;
                if (msb_mode)
                {
                    value = ((data[index] << shift) & 0xff) | (data[index - 1] << 8);
                }
                else
                {
                    // NO LSB
        ...
        else if (index == (size - 1))
        {
            ...
            HAL_MSPI_WRITE((HAL_MSPI_REG_WRITE_BUF + (index >> 1)), (data[index] << shift) & 0xff);
        }
    }
...
int hal_mspi_set_lsb(struct hal_mspi *hal, u8 enable)
{
    ...
    HAL_MSPI_WRITE(HAL_MSPI_REG_RLSB_FIRST, enable);
    HAL_MSPI_WRITE(HAL_MSPI_REG_WLSB_FIRST, enable);
...
int hal_mspi_dma_write(u8 ch, struct hal_mspi *hal, u8 *data, u32 size)
{
    ...
    virt_addr = hal->virt_addr;
    phys_addr = hal_mspi_virt_to_phys(virt_addr);
    ...
    hal_mspi_cache_flush(virt_addr, size);
    hal_mspi_lock();
    ...
    ret = hal_bdma_initialize((u8)dma_ch);
...
int hal_mspi_dma_read(u8 ch, struct hal_mspi *hal, u8 *data, u32 size)
{
    ...
    hal_mspi_cache_invalidate(virt_addr, size);
