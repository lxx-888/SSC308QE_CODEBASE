/*
 * hal_sdmmc_platform_common.c- Sigmastar
 *
...
{
    if (mmc_PMuxInfo.u8_ipOrder >= IP_TOTAL)
    {
        pr_err("sdmmc error ! [%s][%d]\n", __FUNCTION__, __LINE__);
        return;
    }

    if ((0 != drv_padmux_active()) && (mmc_PMuxInfo.u32_PinPWR != PAD_UNKNOWN))
    {
...
void Hal_CARD_ConfigCdzPad(MMCPadMuxInfo mmc_PMuxInfo)
{
    ...
    if ((0 != drv_padmux_active()) && (mmc_PMuxInfo.u32_PinCdzRst != PAD_UNKNOWN))
    {
...
BOOL_T Hal_CARD_GetCdzState(MMCPadMuxInfo mmc_PMuxInfo)
{
    ...
    if (mmc_PMuxInfo.u32_PinCdzRst != PAD_UNKNOWN)
    {
        if (sstar_gpio_pad_read(mmc_PMuxInfo.u32_PinCdzRst, &nLv) != 0)
        {
            printk("sdmmc error ! [%s][%d]\n", __FUNCTION__, __LINE__);
            return FALSE;
        }
    }
...
{
    if (mmc_PMuxInfo.u32_CdzRstMode == PINMUX_FOR_GPIO_MODE)
        return 0;
    return 1;
}
#endif
