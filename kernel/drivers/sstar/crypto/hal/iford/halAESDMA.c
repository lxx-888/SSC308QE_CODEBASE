...
void HAL_AESDMA_MultiRound(u8 u8multiple_num)
{
    RIU[(AESDMA_BASE_ADDR + (0x77 << 1))] = RIU[(AESDMA_BASE_ADDR + (0x77 << 1))] | ((u8multiple_num & 0x3) << 14);
}

void HAL_AESDMA_SCA(u8 enbale)
{
    // HAL_RNG_init();

    if (enbale)
    {
        if (RIU[(AESDMA2_BASE_ADDR + (0x24 << 1))] != 0x1)
        {
            RIU[(AESDMA_BASE_ADDR + (0x51 << 1))] = (1 << 8);
            RIU[(AESDMA_BASE_ADDR + (0x50 << 1))] = 0x80;
            RIU[(AESDMA_BASE_ADDR + (0x50 << 1))] = 0x00;

            while (RIU[(AESDMA2_BASE_ADDR + (0x24 << 1))] != 0x1)
                ;

            RIU[(AESDMA_BASE_ADDR + (0x51 << 1))] = 0x0;
        }
        // Enable sca function used with default value
        RIU[(AESDMA2_BASE_ADDR + (0x20 << 1))] = 0x0021;
    }
    else
    {
        // Disable sca function
        RIU[(AESDMA2_BASE_ADDR + (0x20 << 1))] = 0x8300;
    }
}
...
void HAL_AESDMA_SetCipherKey(u16 *pu16Key, u32 len)
{
    int i;
...
void HAL_AESDMA_SetKeylen(u32 len)
{
    if (len == AES_KEYSIZE_128)
        SETREG16(BASE_RIU_ADDR_AESDMA + ((OFFSET_REG_ENG3_CTRL_S) << 2), KEY_LEN_AES128);
    else if (len == AES_KEYSIZE_192)
        SETREG16(BASE_RIU_ADDR_AESDMA + ((OFFSET_REG_ENG3_CTRL_S) << 2), KEY_LEN_AES192);
    else if (len == AES_KEYSIZE_256)
        SETREG16(BASE_RIU_ADDR_AESDMA + ((OFFSET_REG_ENG3_CTRL_S) << 2), KEY_LEN_AES256);
}
...
u16 HAL_AES_DummyRead(u16 u16Index, u8 dummy)
{
    u8  i;
    u16 data = 0;
    for (i = 0; i < dummy; i++)
...
void HAL_AESDMA_Reset(void)
{
    ...
    RIU[(AESDMA_BASE_ADDR + (OFFSET_REG_ENG3_CTRL_S << 1))] = 0;
    RIU[(AESDMA_BASE_ADDR + (0x79 << 1))] =
        (((RIU[(AESDMA_BASE_ADDR + (0x79 << 1))]) & (~AESDMA_USE_SECRET_KEY_MASK)) | AESDMA_USE_CIPHER_KEY);
    ...
    sstar_miu_module_reset_byid(MIU_CLIENTR_AESDMA, 0);
    sstar_miu_module_reset_byid(MIU_CLIENTW_AESDMA, 1);
}
...
void HAL_AESDMA_Start(u8 u8AESDMAStart)
{
    ...
    /*
     * Phenomena: when open AESDMA MCG or miupll clk too low(200M). sm4 will failure of probability
     * rootcause: After mcg is enabled and sm4 is enabled, wait for dma_done after the internal dma clk starts
     * patch: Polling AESDMA Done and Waiting time for reading the register five times
     */
    while ((HAL_AESDMA_GetStatus() & AESDMA_CTRL_DMA_DONE) == AESDMA_CTRL_DMA_DONE && (i <= 5))
    {
        i++;
        HAL_RSA_DummyRead(0x50, 0);
    }
}
...
void HAL_RSA_LoadSignInverse(u32 *ptr_Sign, u16 u16Signlentgh)
{
...
void HAL_RSA_LoadKeyE(u32 *ptr_E, u16 u16Elentgh)
{
    u32 i;
...
void HAL_RSA_LoadKeyN(u32 *ptr_N, u16 u16Nlentgh)
{
    u32 i;
...
void HAL_RSA_LoadKeyNInverse(u32 *ptr_N, u16 u16Nlentgh)
{
    u32 i;
#ifdef CONFIG_SRAM_DUMMY_ACCESS_RSA
    u16 dummy;
#endif
    u16 lentgh = u16Nlentgh;
...
void HAL_RSA_LoadKeyEInverse(u32 *ptr_E, u16 u16Elentgh)
{
    u32 i;
#ifdef CONFIG_SRAM_DUMMY_ACCESS_RSA
    u16 dummy;
#endif
    u16 lentgh = u16Elentgh;
...
u16 HAL_RNG_init(void)
{
    unsigned long start, timeout;

    RIU[(RNG_BASE_ADDR + (0x09 << 1))] = 0x4321;
    RIU[(RNG_BASE_ADDR + (0x0A << 1))] = 0xFF00;
    RIU[(RNG_BASE_ADDR + (0x0D << 1))] = 0xFFFF;

    // SW set clk_rc set = 12M,if clk_rng=12M. set aesdma2_0x0[15]=1
    // if clk_rng=480M. set aesdma2_0x0[15]=0
    if ((RIU[(RNG_CLK_ADDR + (0x77 << 1))] & 0x40) == 0x040)
        RIU[(RNG_BASE_ADDR + (0x00 << 1))] = RIU[(RNG_BASE_ADDR + (0x00 << 1))] & (0X7FFF);
    else
        RIU[(RNG_BASE_ADDR + (0x00 << 1))] = RIU[(RNG_BASE_ADDR + (0x00 << 1))] | (0X8000);
    HAL_RNG_Reset();
    start   = jiffies;
    timeout = start + msecs_to_jiffies(1);
    do
    {
        if ((RIU[(RNG_BASE_ADDR + (0x23 << 1))] & 0x8000) == 0x8000)
        {
            return 1;
        }
...
u16 HAL_RNG_Read(void)
{
    ...
    do
    {
        if ((RIU[(RNG_BASE_ADDR + (0x23 << 1))]) & HAL_RNG_READY)
        {
            rng = RIU[(RNG_BASE_ADDR + (0x22 << 1))];
            break;
        }
    } while (try_count--);
...
void HAL_RNG_Reset(void)
{
    RIU[(RNG_BASE_ADDR + (0x23 << 1))] = 0x0001; // reset rng
    RIU[(RNG_BASE_ADDR + (0x23 << 1))] = 0x0000; // restart rng
}
