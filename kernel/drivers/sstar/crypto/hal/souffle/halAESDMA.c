...
u16 HAL_RNG_init(void)
{
    ...
    RIU[(AESDMA2_BASE_ADDR + (0x0A << 1))] = 0xFF00;
    ...
    if ((RIU[(RNG_CLK_ADDR + (0x77 << 1))] & 0x40) == 0x040)
        RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] = RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] & (0X7FFF);
    else
        RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] = RIU[(AESDMA2_BASE_ADDR + (0x00 << 1))] | (0X8000);
    HAL_RNG_Reset();
    start   = jiffies;
    timeout = start + msecs_to_jiffies(1);
    do
    {
        if ((RIU[(AESDMA_BASE_ADDR + (0x03 << 1))] & 0x8000) == 0x8000)
        {
            return 1;
        }
...
u16 HAL_RNG_Read(void)
{
    ...
    do
    {
        if ((RIU[(RSA_BASE_ADDR + (0x03 << 1))]) & HAL_RNG_READY)
        {
...
{
    RIU[(AESDMA_BASE_ADDR + (0x03 << 1))] = 0x0001; // reset rng
    RIU[(AESDMA_BASE_ADDR + (0x03 << 1))] = 0x0000; // restart rng
}
