...
static int test_cipher_aes_case(int cfd, CIPHER_AES_CASE_S *pstCase)
{
    ...
    {
        my_perror("ioctl(CIOCGSESSINFO) failed");
        return 1;
    }
    if (debug)
    ...
    {
        my_perror("ioctl(CIOCCRYPT) failed");
        return 1;
    }
    ...
    {
        my_perror("ioctl(CIOCFSESSION) failed");
        return 1;
    }
    ...
    if (memcmp(plaintext, ciphertext, pstCase->dataSize) != 0)
    {
        ...
        if (debug)
        {
            fprintf(stderr, "failed: Decrypted data are different from the input data, case %s\n", szDesc);
            printf("ciphertext:\n0000: ");
...
static int test_crypto(int cfd)
{
    ...
    for (i = 0; i < sizeof(alg) / sizeof(alg[0]); i++)
    {
        for (j = 0; j < sizeof(dataSize) / sizeof(dataSize[0]); j++)
        {
            ...
            {
                printf("\033[0;31mTest %s failed \033[0m\n", szDesc);
            }
...
static int test_aes(int cfd)
{
    ...
    char ciphertext1[BLOCK_SIZE] = {0x27, 0xef, 0x24, 0x95, 0xda, 0xbf, 0x32, 0x38,
                                    0x85, 0xaa, 0xb3, 0x9c, 0x80, 0xf1, 0x8d, 0x8b};
    char iv1[BLOCK_SIZE];
    char key1[KEY_SIZE]              = {0xff, 0xff, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    char ciphertext2[BLOCK_SIZE] = {0x68, 0x3e, 0x21, 0x40, 0x58, 0x5b, 0x18, 0x45,
    ...
    {
        fprintf(stderr, "failed: Decrypted data are different from the input data.\n");
        return 1;
    }
    /* Finish crypto session */
    if (ioctl(cfd, CIOCFSESSION, &sess.ses))
