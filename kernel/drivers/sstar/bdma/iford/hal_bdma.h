...
typedef enum
{
    ...
    HAL_BDMA_MIU0_TO_CRC,
    HAL_BDMA_MIU0_SEARCH
...
typedef struct
{
    bool reflection; // byte reflection
    u32  polynomial; // the polynomial to generate CRC
    u32  seedvalue;  // the seed value for CRC
} hal_bdma_crc;

typedef struct
{
    bool antimatch;   // enable anti match
    bool mobf;        // enable MOBF mode
    u32  patternmask; // the seed value for CRC
    u32  mobfkey;
    u32  patternaddr; // pattern found addr
} hal_bdma_search;
...
typedef struct
{
    ...
    hal_bdma_search *     pstSearchst;
    hal_bdma_crc *        pstCrcst;
    u32                   bEnLineOfst;
