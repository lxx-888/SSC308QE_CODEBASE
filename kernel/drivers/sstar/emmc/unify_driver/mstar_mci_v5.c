...
volatile U32       gu32_SupportEmmc50[EMMC_NUM_TOTAL] = {0};
struct clk_hw *    CLK_fcie_1x_p[EMMC_NUM_TOTAL];
struct clk_hw *    CLK_fcie_2x_p[EMMC_NUM_TOTAL];
...
U64 gu64_Read_spend[5]    = {0};
U64 gu64_Write_spend[5]   = {0};
U8  eMMC_HS200_EN         = 0;
...
static U32 mstar_mci_completed_command(struct mstar_mci_host *pSstarHost_st, U32 u32_err)
{
    ...
    if (u32_err & eMMC_ST_PLAT)
        u32_err_tmp = u32_err;
    else
        u32_err_tmp = u32_err & 0xFF;
...
{
    REG_FCIE(reg_emmcpll_0x03(eIP)) = (u8Skew4 << 12) & BIT_SKEW4_MASK; // cmd & data rx phase
    REG_FCIE(reg_emmcpll_0x6c(eIP)) = BIT_DQS_MODE_1T | ((u8Skew4Inv << SKEW_INV_OFFSET) & R_CLK_SKEW_INV);
}
...
static void mstar_mci_enable(eMMC_IP_EmType emmc_ip)
{
    ...
    g_eMMCDrv[emmc_ip].u16_ClkRegVal = 0;
...
static ssize_t emmc_get_wr_spend_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    printk(KERN_CONT "eMMC Read spend time:");
    for (u8_i = 0; u8_i < 5; u8_i++)
        printk(" %llu(ns) ", gu64_Read_spend[u8_i]);
    printk("\n");

    printk(KERN_CONT "eMMC Write spend time:");
    for (u8_i = 0; u8_i < 5; u8_i++)
...
DEVICE_ATTR(eMMC_get_rw_time, S_IRUSR, emmc_get_wr_spend_show, NULL);
...
static ssize_t eMMC_erase_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    U32            u32_slotNum, u32_temp, u32_err = eMMC_ST_SUCCESS;
    U32            u32_address_start, u32_erase_size;
    eMMC_IP_EmType emmc_ip;

    if (buf == NULL)
        return -EINVAL;

    u32_temp = sscanf(buf, "%u %x %x", &u32_slotNum, &u32_address_start, &u32_erase_size);
    if (u32_temp != 3 || u32_slotNum >= gu8_emmcSlotNums || u32_erase_size == 0)
    {
        return -EINVAL;
    }
    ...
    u32_err = eMMC_EraseBlock(emmc_ip, u32_address_start, u32_address_start + u32_erase_size - 1);
    if (u32_err != eMMC_ST_SUCCESS)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "erase emmc addr 0x%x size 0x%x fail!!\r\n", u32_address_start,
                   u32_erase_size);
    }
    else
    {
        eMMC_debug(0, 0, "erase emmc addr 0x%x size 0x%x blocks success.\r\n", u32_address_start, u32_erase_size);
    }
    eMMC_UnlockFCIE(emmc_ip, (U8 *)__FUNCTION__);
...
DEVICE_ATTR(eMMC_erase_test, S_IWUSR, NULL, eMMC_erase_test_store);
...
#if (defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400) || (defined(ENABLE_eMMC_HS200) && ENABLE_eMMC_HS200)
        CLK_fcie_1x_p[ge_emmcIPOrderSlot[i]] =
            clk_hw_get_parent_by_index(__clk_get_hw(clkdata->clk_fcie[ge_emmcIPOrderSlot[i]]), 14);
        CLK_fcie_2x_p[ge_emmcIPOrderSlot[i]] =
            clk_hw_get_parent_by_index(__clk_get_hw(clkdata->clk_fcie[ge_emmcIPOrderSlot[i]]), 15);
...
LABEL_END1:
    if (pSstarHost_st->adma_buffer)
        dma_free_coherent(&p_emmc_driver->pdev->dev, sizeof(struct _AdmaDescriptor) * FCIE_ADMA_DESC_COUNT,
                          pSstarHost_st->adma_buffer, pSstarHost_st->adma_phy_addr);
...
static U32 mstar_mci_remove_slot(U8 slotNo, struct mstar_mci_driver *p_emmc_driver)
{
    ...
    if (pSstar_mci_host_st->adma_buffer)
        dma_free_coherent(&p_emmc_driver->pdev->dev, sizeof(struct _AdmaDescriptor) * FCIE_ADMA_DESC_COUNT,
                          pSstar_mci_host_st->adma_buffer, pSstar_mci_host_st->adma_phy_addr);
...
#endif
        clk_set_rate(clkdata->clk_fcie[emmc_ip], 12000000); // To update the clock frequency recorded before awakening
        mstar_mci_enable(emmc_ip);
