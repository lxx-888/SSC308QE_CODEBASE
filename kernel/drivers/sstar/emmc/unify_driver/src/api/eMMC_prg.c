...
U32 eMMC_LoadImages(eMMC_IP_EmType emmc_ip, U32 *pu32_Addr, U32 *pu32_SectorCnt, U32 u32_ItemCnt)
{
    U32 u32_err, u32_i;
...
LABEL_BOOT_MODE_START:
    ...
    while (u32_i < u32_ItemCnt)
    {
        ...
        REG_FCIE_W(FCIE_JOB_BL_CNT(emmc_ip), pu32_SectorCnt[u32_i]);
#if FICE_BYTE_MODE_ENABLE
        REG_FCIE_W(FCIE_SDIO_ADDR0(emmc_ip), pu32_Addr[u32_i] & 0xFFFF);
        REG_FCIE_W(FCIE_SDIO_ADDR1(emmc_ip), pu32_Addr[u32_i] >> 16);
#else
        REG_FCIE_W(FCIE_MIU_DMA_15_0(emmc_ip), (pu32_Addr[u32_i] >> 3) & 0xFFFF);
        REG_FCIE_W(FCIE_MIU_DMA_26_16(emmc_ip), (pu32_Addr[u32_i] >> 3) >> 16);
#endif
        REG_FCIE_CLRBIT(FCIE_MMA_PRI_REG(emmc_ip), BIT_DMA_DIR_W);
        ...
        if (0 == u32_i) // stg.1
        {
            REG_FCIE_SETBIT(FCIE_BOOT_CONFIG(emmc_ip), BIT_BOOT_MODE_EN);
        }
        else // stg.2
        {
            // can NOT set BIT_SD_DAT_EN
            // REG_FCIE_SETBIT(FCIE_SD_CTRL, BIT_SD_DAT_EN);
            REG_FCIE_SETBIT(FCIE_BOOT_CONFIG(emmc_ip), BIT_BOOT_STG2_EN);
        }

        u32_err = eMMC_FCIE_PollingEvents(emmc_ip, FCIE_MIE_EVENT(emmc_ip), BIT_MIU_LAST_DONE, HW_TIMER_DELAY_1s);
...
// eMMC_clock_setting(emmc_ip, FCIE_SLOWEST_CLK);
    ...
    // --------------------------------
    // boot end
    REG_FCIE_SETBIT(FCIE_BOOT_CONFIG(emmc_ip), BIT_BOOT_END_EN);
    u32_err = eMMC_FCIE_PollingEvents(FCIE_MIE_EVENT(emmc_ip), BIT_CARD_BOOT_DONE, HW_TIMER_DELAY_1s);
    if (u32_err)
        goto LABEL_LOAD_IMAGE_END;
...
//========================================================
U32 eMMC_GetID(eMMC_IP_EmType emmc_ip, U8 *pu8IDByteCnt, U8 *pu8ID)
{
    eMMC_debug(eMMC_DEBUG_LEVEL_HIGH, 1, "\n");

    *pu8IDByteCnt = g_eMMCDrv[emmc_ip].u8_IDByteCnt;
    memcpy(pu8ID, g_eMMCDrv[emmc_ip].au8_ID, eMMC_ID_BYTE_CNT);

    return eMMC_ST_SUCCESS;
}

// data length is 512 bytes
U32 eMMC_FATAutoSize(eMMC_IP_EmType emmc_ip)
{
    U16          u16_i;
    ...
    for (u16_i = 0; u16_i < pPartInfo->u16_PartCnt; u16_i++)
    {
        if (eMMC_PART_FAT == pPartInfo->records[u16_i].u16_PartType)
            u32_PartSectorCnt = pPartInfo->records[u16_i].u16_StartBlk * pPartInfo->u16_BlkPageCnt;

        else if (u32_PartSectorCnt)
            u32_PartSectorCnt += (pPartInfo->records[u16_i].u16_BlkCnt + pPartInfo->records[u16_i].u16_BackupBlkCnt)
                                 * pPartInfo->u16_BlkPageCnt;
    }

    g_eMMCDrv[emmc_ip].u32_FATSectorCnt = g_eMMCDrv[emmc_ip].u32_SEC_COUNT - u32_PartSectorCnt;
    ...
    for (u16_i = 0; u16_i < pPartInfo->u16_PartCnt; u16_i++)
    {
        if (eMMC_PART_FAT == pPartInfo->records[u16_i].u16_PartType)
        {
            pPartInfo->records[u16_i].u16_BlkCnt = g_eMMCDrv[emmc_ip].u32_FATSectorCnt / pPartInfo->u16_BlkPageCnt;

            u32_PartSectorCnt = pPartInfo->records[u16_i].u16_StartBlk + pPartInfo->records[u16_i].u16_BlkCnt
...
static U32 eMMC_ReadPartitionInfo_Ex(eMMC_IP_EmType emmc_ip)
{
    U32         u32_err;
    U16         u16_i;
    eMMC_PNI_t *pPartInfo = (eMMC_PNI_t *)gau8_eMMC_PartInfoBuf;

    u32_err = eMMC_CheckIfReady(emmc_ip);
    ...
    {
        u32_err = eMMC_CMD17(emmc_ip, eMMC_PNI_BLK_0 + u16_i, gau8_eMMC_PartInfoBuf);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, read Part Info %u fail, %Xh\n", u32_err, u16_i,
                       u32_err);
            continue;
        }

        if (0 == pPartInfo->u32_ChkSum
    ...
    else
    {
        ...
        eMMC_FATAutoSize(emmc_ip);
        // eMMC_debug(0,1,"Total Sec: %Xh, FAT Sec: %Xh \n", g_eMMCDrv[emmc_ip].u32_SEC_COUNT,
        // g_eMMCDrv[emmc_ip].u32_FATSectorCnt);
    }

    g_eMMCDrv[emmc_ip].u32_DrvFlag |= DRV_FLAG_GET_PART_INFO;
    return u32_err;
}

// data length is 512 bytes
U32 eMMC_ReadPartitionInfo(eMMC_IP_EmType emmc_ip, U8 *pu8_Data)
{
    U32 u32_err;
    ...
    if (0 == (g_eMMCDrv[emmc_ip].u32_DrvFlag & DRV_FLAG_GET_PART_INFO))
    {
        u32_err = eMMC_ReadPartitionInfo_Ex(emmc_ip);
        if (eMMC_ST_SUCCESS != u32_err)
        {
...
U32 eMMC_CheckCIS(eMMC_CIS_t *ptCISData)
{
    ...
    u32_ChkSum =
        eMMC_ChkSum((U8 *)&peMMCInfo->u16_SpareByteCnt, (U16)((dma_addr_t)(uintptr_t)(&peMMCInfo->u16_SeqAccessTime)
                                                              - (dma_addr_t)(uintptr_t)(&peMMCInfo->u16_SpareByteCnt)));
...
U32 eMMC_ReadCIS(eMMC_IP_EmType emmc_ip, eMMC_CIS_t *ptCISData)
{
    U32         u32_err;
    U16         u16_i;
    eMMC_NNI_t *peMMCInfo = (eMMC_NNI_t *)&ptCISData->au8_eMMC_nni[0];
    ...
    {
        u32_err = eMMC_CMD17(emmc_ip, eMMC_NNI_BLK_0 + u16_i, (U8 *)peMMCInfo);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: read NNI %u fail, %Xh\n", u16_i, u32_err);
            continue;
        }
        break;
    }
    ...
    for (u16_i = 0; u16_i < eMMC_CIS_PNI_BLK_CNT; u16_i++)
    {
        u32_err = eMMC_CMD17(emmc_ip, eMMC_PNI_BLK_0 + u16_i, (U8 *)pPartInfo);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: read PNI %u fail, %Xh\n", u16_i, u32_err);
            continue;
        }
        break;
    }
...
U32 eMMC_DumpPartitionInfo(eMMC_IP_EmType emmc_ip)
{
    U32         u32_err;
    U16         u16_i;
    ...
    for (u16_i = 0; u16_i < pPartInfo->u16_PartCnt; u16_i++)
    {
        ...
        if (u16_i < pPartInfo->u16_PartCnt - 1)
        {
            ...
            u32_PniBlkCnt += pPartInfo->records[u16_i].u16_BlkCnt + pPartInfo->records[u16_i].u16_BackupBlkCnt;
        }
    }
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "Capacity:    %u MB\n",
               (g_eMMCDrv[emmc_ip].u32_SEC_COUNT - (u32_PniBlkCnt * u32_PniBlkByteCnt >> 9)) >> 11);
...
static U32 eMMC_GetPartitionIndex(eMMC_IP_EmType emmc_ip, U16 u16_PartType, U32 u32_LogicIdx,
...
U32 eMMC_GetPartitionCapacity(eMMC_IP_EmType emmc_ip, U16 u16_PartType, U32 *pu32_Cap)
{
    U32          u32_err;
    ...
    if (eMMC_PART_FAT == u16_PartType)
        *pu32_Cap = g_eMMCDrv[emmc_ip].u32_FATSectorCnt;
    else
    {
        u32_err = eMMC_GetPartitionIndex(emmc_ip, u16_PartType, 0, &u16_PartIdx);
        if (eMMC_ST_SUCCESS != u32_err)
            return u32_err;
...
U32 eMMC_ReadPartition(eMMC_IP_EmType emmc_ip, U16 u16_PartType, U8 *pu8_DataBuf, U32 u32_StartSector,
                       U32 u32_SectorCnt, U32 u32_LogicIdx)
{
    ...
    u32_err = eMMC_GetPartitionIndex(emmc_ip, u16_PartType, u32_LogicIdx, &u16_PartIdx);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
    ...
    // read data
    u32_err = eMMC_ReadData(emmc_ip, pu8_DataBuf, u32_SectorCnt << eMMC_SECTOR_512BYTE_BITS,
                            pPartInfo->records[u16_PartIdx].u16_StartBlk * pPartInfo->u16_BlkPageCnt + u32_StartSector);
    if (eMMC_ST_SUCCESS != u32_err)
    {
    ...
    u32_err = eMMC_BootPartitionHandler_WR(pu8_DataBuf, u16_PartType, u32_StartSector, u32_SectorCnt, eMMC_BOOT_PART_R);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_WritePartition(eMMC_IP_EmType emmc_ip, U16 u16_PartType, U8 *pu8_DataBuf, U32 u32_StartSector,
                        U32 u32_SectorCnt, U32 u32_LogicIdx)
{
    U32          u32_err, u32_PartSecCnt;
    volatile U16 u16_PartIdx;
    ...
    // write data
    u32_err =
        eMMC_WriteData(emmc_ip, pu8_DataBuf, u32_SectorCnt << eMMC_SECTOR_512BYTE_BITS,
                       pPartInfo->records[u16_PartIdx].u16_StartBlk * pPartInfo->u16_BlkPageCnt + u32_StartSector);
    if (eMMC_ST_SUCCESS != u32_err)
    {
    ...
    u32_err = eMMC_BootPartitionHandler_WR(pu8_DataBuf, u16_PartType, u32_StartSector, u32_SectorCnt, eMMC_BOOT_PART_W);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_GetDevInfo(eMMC_IP_EmType emmc_ip, eMMC_INFO_t *peMMCInfo_t)
{
    const char *str = "MSTARSEMIUNFDCIS";

    memcpy(peMMCInfo_t->au8_Tag, str, 16);
    peMMCInfo_t->u8_IDByteCnt = g_eMMCDrv[emmc_ip].u8_IDByteCnt;
    memcpy(peMMCInfo_t->au8_ID, g_eMMCDrv[emmc_ip].au8_ID, g_eMMCDrv[emmc_ip].u8_IDByteCnt);
    peMMCInfo_t->u16_SpareByteCnt = 1;
    peMMCInfo_t->u16_PageByteCnt  = eMMC_SECTOR_512BYTE;
    peMMCInfo_t->u16_BlkPageCnt   = g_eMMCDrv[emmc_ip].u32_EraseUnitSize;
    peMMCInfo_t->u16_BlkCnt       = g_eMMCDrv[emmc_ip].u32_SEC_COUNT / g_eMMCDrv[emmc_ip].u32_EraseUnitSize;
    peMMCInfo_t->u32_ChkSum       = eMMC_ChkSum((U8 *)&peMMCInfo_t->u16_SpareByteCnt, 0x32 - 0x24);

    memcpy(peMMCInfo_t->au8_Vendor, g_eMMCDrv[emmc_ip].au8_Vendor, 16);
    memcpy(peMMCInfo_t->au8_PartNumber, g_eMMCDrv[emmc_ip].au8_PartNumber, 16);

    return eMMC_ST_SUCCESS;
}

U32 eMMC_ErasePartition(eMMC_IP_EmType emmc_ip, U16 u16_PartType)
{
    ...
    {
        u32_err = eMMC_GetPartitionIndex(emmc_ip, u16_PartType, u32_PartLogiIdx, &u16_PartIdx);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            break;
        }
        ...
        u32_eMMCStartBlk = pPartInfo->records[u16_PartIdx].u16_StartBlk
                           * (pPartInfo->u16_PageByteCnt >> eMMC_SECTOR_512BYTE_BITS) * pPartInfo->u16_BlkPageCnt;

        u32_eMMCEndBlk = (pPartInfo->records[u16_PartIdx].u16_StartBlk + pPartInfo->records[u16_PartIdx].u16_BlkCnt)
                         * (pPartInfo->u16_PageByteCnt >> eMMC_SECTOR_512BYTE_BITS) * pPartInfo->u16_BlkPageCnt;

        u32_err = eMMC_EraseBlock(emmc_ip, u32_eMMCStartBlk, u32_eMMCEndBlk - 1);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: eMMC_EraseBlock fail: %Xh, PardIdx:%Xh", u32_err,
                       u16_PartIdx);
            break;
        }
    }
...
U32 eMMC_EraseAllPartitions(eMMC_IP_EmType emmc_ip)
{
    U32          u32_err, u32_PartLogiIdx;
    ...
    while (1) // find all partition copies
    {
        u32_err = eMMC_GetPartitionIndex(emmc_ip, eMMC_PART_NVRAMBAK, u32_PartLogiIdx, &u16_PartIdx_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
            break;
...
U32 eMMC_EraseAll(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err;

    u32_err = eMMC_EraseBlock(emmc_ip, 0, g_eMMCDrv[emmc_ip].u32_SEC_COUNT - 1);
    if (eMMC_ST_SUCCESS != u32_err)
...
U32 eMMC_EraseBootPart(eMMC_IP_EmType emmc_ip, U32 u32_eMMCBlkAddr_start, U32 u32_eMMCBlkAddr_end, U8 u8_PartNo)
{
    ...
    if (u8_PartNo == 1)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT0); // BIT6:ACK
    }
    else if (u8_PartNo == 2)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT1); // still boot from BP1
    }
    if (eMMC_ST_SUCCESS != u32_err)
    {
    ...
    {
        u32_err = eMMC_EraseBlock(emmc_ip, u32_eMMCBlkAddr_start, u32_eMMCBlkAddr_end);
        if (eMMC_ST_SUCCESS != u32_err)
        {
    ...
    else
    {
        ...
        u32_SectorCnt = u32_eMMCBlkAddr_start / g_eMMCDrv[emmc_ip].u32_EraseUnitSize;
        u32_SectorCnt = (u32_SectorCnt + 1) * g_eMMCDrv[emmc_ip].u32_EraseUnitSize;
        u32_SectorCnt -= u32_eMMCBlkAddr_start;
        u32_SectorCnt = u32_SectorCnt > (u32_eMMCBlkAddr_end - u32_eMMCBlkAddr_start)
                            ? (u32_eMMCBlkAddr_end - u32_eMMCBlkAddr_start)
                            : u32_SectorCnt;

        for (u32_i = 0; u32_i < u32_SectorCnt; u32_i++)
        {
            u32_j = ((u32_SectorCnt - u32_i) << eMMC_SECTOR_512BYTE_BITS) > eMMC_SECTOR_BUF_BYTECTN
                        ? eMMC_SECTOR_BUF_BYTECTN
                        : ((u32_SectorCnt - u32_i) << eMMC_SECTOR_512BYTE_BITS);

            u32_err = eMMC_WriteData(emmc_ip, gau8_eMMC_SectorBuf, u32_j, u32_eMMCBlkAddr_start + u32_i);
            if (eMMC_ST_SUCCESS != u32_err)
            {
        ...
        // erase blocks
        u32_i = (u32_eMMCBlkAddr_end - (u32_eMMCBlkAddr_start + u32_SectorCnt)) / g_eMMCDrv[emmc_ip].u32_EraseUnitSize;
        if (u32_i)
        {
            u32_err =
                eMMC_EraseBlock(emmc_ip, (u32_eMMCBlkAddr_start + u32_SectorCnt),
                                (u32_eMMCBlkAddr_start + u32_SectorCnt) + u32_i * g_eMMCDrv[emmc_ip].u32_EraseUnitSize);
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD24 fail 0, %Xh\n", u32_err);
                return u32_err;
        ...
        u32_eMMCBlkAddr_start = (u32_eMMCBlkAddr_start + u32_SectorCnt) + u32_i * g_eMMCDrv[emmc_ip].u32_EraseUnitSize;

        while (u32_eMMCBlkAddr_start < u32_eMMCBlkAddr_end)
        {
            u32_j =
                ((u32_eMMCBlkAddr_end - u32_eMMCBlkAddr_start) << eMMC_SECTOR_512BYTE_BITS) > eMMC_SECTOR_BUF_BYTECTN
                    ? eMMC_SECTOR_BUF_BYTECTN
                    : ((u32_eMMCBlkAddr_end - u32_eMMCBlkAddr_start) << eMMC_SECTOR_512BYTE_BITS);

            u32_err = eMMC_WriteData(emmc_ip, gau8_eMMC_SectorBuf, u32_j, u32_eMMCBlkAddr_start);
            if (eMMC_ST_SUCCESS != u32_err)
            {
...
LABEL_END_OF_ERASE:
    ...
    u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3);
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[179]: %Xh fail\n", u32_err,
                   BIT6 | BIT3);
...
U32 eMMC_WriteBootPart(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    U32 u32_err;
    ...
    if (u8_PartNo == 1)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT0);
    }
    else if (u8_PartNo == 2)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT1); // still boot from BP1
    ...
    u32_err = eMMC_WriteData(emmc_ip, pu8_DataBuf, u16_SecCnt << eMMC_SECTOR_512BYTE_BITS, u32_BlkAddr);
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: write Boot Partition fail, %Xh\n", u32_err);
        return u32_err;
    }
#if 0
    ...
    for (u16_i = (U16)u32_BlkAddr; u16_i < u16_SecCnt; u16_i++)
    {
        ...
        u32_err = eMMC_CMD17(emmc_ip, u16_i, gau8_eMMC_SectorBuf);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: read Boot Partition Sector %u fail, %Xh\n", u16_i,
                       u32_err);
            return u32_err;
        }
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: check Boot Partition Sector %u fail, %Xh\n", u16_i,
                       u32_err);
    ...
    // set Boot Bus
    u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 177, BIT3 | BIT1);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_ReadBootPart(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    U32 u32_err;
    U16 u16_SecCnt;
    ...
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT1);
    }
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[179]: %Xh fail\n", u32_err,
    ...
    u32_err = eMMC_ReadData(emmc_ip, pu8_DataBuf, u16_SecCnt << eMMC_SECTOR_512BYTE_BITS, u32_BlkAddr);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_WriteGPPart(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    U32 u32_err;
    U16 u16_SecCnt;
    ...
    if ((g_eMMCDrv[emmc_ip].u8_u8_ECSD155_PartSetComplete & BIT0) != BIT0)
    {
    ...
    u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 175, BIT0);
    if (eMMC_ST_SUCCESS != u32_err)
    {
    ...
    if (u8_PartNo == 0)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT2);
    }
    else if (u8_PartNo == 1)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT2 | BIT0);
    }
    else if (u8_PartNo == 2)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT2 | BIT1);
    }
    else if (u8_PartNo == 3)
    {
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 179, BIT6 | BIT3 | BIT2 | BIT1 | BIT0);
    }
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[179]: %Xh fail\n", u32_err,
...
U32 eMMC_ReadGPPart(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr, U8 u8_PartNo)
{
    U32 u32_err;
    U16 u16_SecCnt;
...
void eMMC_PrintGPPartition(eMMC_IP_EmType emmc_ip)
{
    U8 u8_i;
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "eMMC Gerneral Purpose Partition\n");
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "No.          Size\n");
    for (u8_i = 0; u8_i < 4; u8_i++)
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "%d          0x%X\n", u8_i, g_eMMCDrv[emmc_ip].GP_Part[u8_i].u32_PartSize);
}

// store partition setting in ram

U32 eMMC_SetGPPartition(eMMC_IP_EmType emmc_ip, U8 u8_PartNo, U32 u32_PartSize, U8 u8_EnAttr, U8 u8_ExtAttr, U8 u8_RelW)
{
    U32 u32_HC_WP_Size =
    ...
    if ((g_eMMCDrv[emmc_ip].u8_ECSD160_PartSupField & BIT0) == 0)
    {
    ...
    if (u8_PartNo < 4)
    {
        ...
        {
            g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u32_PartSize = ((u32_PartSize / u32_HC_WP_Size) + 1) * u32_HC_WP_Size;
            eMMC_debug(0, 1, "Warning GP size: 0x%X does not align to 0x%X, adjust to 0x%X\n", u32_PartSize,
                       u32_HC_WP_Size, g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u32_PartSize);
        }
        else
            g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u32_PartSize = u32_PartSize;
        g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u8_EnAttr  = u8_EnAttr;
        g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u8_ExtAttr = u8_ExtAttr;
        g_eMMCDrv[emmc_ip].GP_Part[u8_PartNo].u8_RelW    = u8_RelW;
        return eMMC_ST_SUCCESS;
    }
...
U32 eMMC_SetEnhanceUserPartition(eMMC_IP_EmType emmc_ip, U32 u32_StartAddr, U32 u32_Size, U8 u8_EnAttr, U8 u8_RelW)
{
    U32 u32_HC_WP_Size =
        g_eMMCDrv[emmc_ip].u8_ECSD224_HCEraseGRPSize * g_eMMCDrv[emmc_ip].u8_ECSD221_HCWpGRPSize * 0x80000;
    ...
    if (g_eMMCDrv[emmc_ip].u8_ECSD160_PartSupField & BIT1)
    {
        if (u32_Size % u32_HC_WP_Size)
        {
            g_eMMCDrv[emmc_ip].u32_EnUserSize = ((u32_Size / u32_HC_WP_Size) + 1) * u32_HC_WP_Size;
            eMMC_debug(0, 1, "Warning Enhance Size: 0x%X does not align to 0x%X, adjust to 0x%X\n", u32_Size,
                       u32_HC_WP_Size, g_eMMCDrv[emmc_ip].u32_EnUserSize);
        }
        else
            g_eMMCDrv[emmc_ip].u32_EnUserSize = u32_Size;

        if (g_eMMCDrv[emmc_ip].u8_IfSectorMode)
            g_eMMCDrv[emmc_ip].u32_EnUserStartAddr = u32_StartAddr >> 9;
        else
            g_eMMCDrv[emmc_ip].u32_EnUserStartAddr = u32_StartAddr;
        g_eMMCDrv[emmc_ip].u8_EnUserEnAttr = u8_EnAttr;
        g_eMMCDrv[emmc_ip].u8_EnUserRelW   = u8_RelW;
        return eMMC_ST_SUCCESS;
    }
    return eMMC_ST_ERR_INVALID_PARAM;
}

U32 eMMC_SetPartitionComplete(eMMC_IP_EmType emmc_ip)
{
    ...
    u32_HC_WP_Size = g_eMMCDrv[emmc_ip].u8_ECSD224_HCEraseGRPSize * g_eMMCDrv[emmc_ip].u8_ECSD221_HCWpGRPSize * 0x80000;
    ...
    if (g_eMMCDrv[emmc_ip].u8_ECSD160_PartSupField & BIT1)
    {
        u64_MaxEnhSize = (U64)((((U32)(g_eMMCDrv[emmc_ip].u8_ECSD159_MaxEnhSize_2) << 16)
                                | ((U32)(g_eMMCDrv[emmc_ip].u8_ECSD158_MaxEnhSize_1) << 8)
                                | (U32)(g_eMMCDrv[emmc_ip].u8_ECSD157_MaxEnhSize_0))
                               * u32_HC_WP_Size);

        u8_tmp  = 0;
        u64_tmp = 0;
        if (g_eMMCDrv[emmc_ip].u8_EnUserEnAttr == 1)
        {
            u64_tmp = (U64)g_eMMCDrv[emmc_ip].u32_EnUserSize;
            u8_tmp |= BIT0;
        }

        for (u8_i = 0; u8_i < 4; u8_i++)
        {
            if (g_eMMCDrv[emmc_ip].GP_Part[u8_i].u8_EnAttr == 1)
            {
                u64_tmp += (U64)g_eMMCDrv[emmc_ip].GP_Part[u8_i].u32_PartSize;
                u8_tmp |= (1 << (u8_i + 1));
            }
        }
        ...
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1,
                       "Sum of all enhance partition size shell be less than 0x%llX, but get 0x%llX\n", u64_MaxEnhSize,
                       u64_tmp);
            return eMMC_ST_ERR_INVALID_PARAM;
        }
...
#if 1
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 156, u8_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
        {
        ...
        if (g_eMMCDrv[emmc_ip].u32_EnUserSize)
        {
            // size enhance user partition size
            u32_tmp = g_eMMCDrv[emmc_ip].u32_EnUserSize / u32_HC_WP_Size;
            ...
            for (u8_i = 3; u8_i > 0; u8_i--)
            {
                ...
                eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "Set Ext_CSD [%d], Enhance User Size, to 0x%X\n",
                           140 + (u8_i - 1), u8_tmp);
#if 1
                u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 140 + (u8_i - 1), u8_tmp);
                if (eMMC_ST_SUCCESS != u32_err)
                {
            ...
            u32_tmp = g_eMMCDrv[emmc_ip].u32_EnUserStartAddr;
            ...
            for (u8_i = 4; u8_i > 0; u8_i--)
            {
                ...
                eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "Set Ext_CSD [%d], Enhance Start Address, to 0x%X\n",
                           136 + u8_i - 1, u8_tmp);
#if 1
                u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 136 + u8_i - 1, u8_tmp);
                if (eMMC_ST_SUCCESS != u32_err)
                {
                    eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err,
                               136 + u8_i - 1, u8_tmp);
    ...
    for (u8_i = 0; u8_i < 4; u8_i++)
    {
        u32_tmp = g_eMMCDrv[emmc_ip].GP_Part[u8_i].u32_PartSize / u32_HC_WP_Size;
        u16_tmp = 143 + u8_i * 3;
...
#if 1
            u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, u16_tmp + (u8_j - 1), u8_tmp);
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err,
                           u16_tmp + (u8_j - 1), u8_tmp);
                return u32_err;
            }
#endif
        }
    ...
    if (g_eMMCDrv[emmc_ip].u8_ECSD160_PartSupField & BIT2)
    {
        u8_tmp = 0;
        if (g_eMMCDrv[emmc_ip].GP_Part[0].u8_ExtAttr > 0 && g_eMMCDrv[emmc_ip].GP_Part[0].u8_ExtAttr < 3)
        {
            u8_tmp |= (g_eMMCDrv[emmc_ip].GP_Part[0].u8_ExtAttr & 0xF);
        }
        if (g_eMMCDrv[emmc_ip].GP_Part[1].u8_ExtAttr > 0 && g_eMMCDrv[emmc_ip].GP_Part[1].u8_ExtAttr < 3)
        {
            u8_tmp |= ((g_eMMCDrv[emmc_ip].GP_Part[1].u8_ExtAttr << 4) & 0xF0);
        }

        eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "Set Ext_CSD [%d], Extend Attr, to 0x%X\n", 52, u8_tmp);

#if 1
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 52, u8_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err, 52,
                       u8_tmp);
        ...
        u8_tmp = 0;
        if (g_eMMCDrv[emmc_ip].GP_Part[2].u8_ExtAttr > 0 && g_eMMCDrv[emmc_ip].GP_Part[2].u8_ExtAttr < 3)
        {
            u8_tmp |= (g_eMMCDrv[emmc_ip].GP_Part[2].u8_ExtAttr & 0xF);
        }
        if (g_eMMCDrv[emmc_ip].GP_Part[3].u8_ExtAttr > 0 && g_eMMCDrv[emmc_ip].GP_Part[3].u8_ExtAttr < 3)
        {
            u8_tmp |= ((g_eMMCDrv[emmc_ip].GP_Part[3].u8_ExtAttr << 4) & 0xF0);
        }

        eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "Set Ext_CSD [%d], Extend Attr, to 0x%X\n", 53, u8_tmp);

#if 1
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 53, u8_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err, 53,
                       u8_tmp);
    ...
    if ((g_eMMCDrv[emmc_ip].u8_ECSD166_WrRelParam & (BIT0 | BIT2))
        == BIT0) // if BIT2 is set -> device support reliable write by default
    {
        u8_tmp = 0;
        if (g_eMMCDrv[emmc_ip].u8_EnUserRelW == 1)
        {
            u8_tmp |= BIT0;
        }
        ...
        for (u8_i = 0; u8_i < 4; u8_i++)
        {
            if (g_eMMCDrv[emmc_ip].GP_Part[u8_i].u8_RelW == 1)
            {
                u8_tmp |= (1 << (u8_i + 1));
            }
        }
...
#if 1
        u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 167, u8_tmp);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err, 167,
                       u8_tmp);
...
#if 1
    u32_err = eMMC_ModifyExtCSD(emmc_ip, eMMC_ExtCSD_WByte, 155, BIT0);
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %Xh, eMMC, set Ext_CSD[%d]: %Xh fail\n", u32_err, 155, BIT0);
        return u32_err;
...
// ==============================================================
void eMMC_DumpSpeedStatus(eMMC_IP_EmType emmc_ip)
{
    if (g_eMMCDrv[emmc_ip].u32_DrvFlag & DRV_FLAG_DDR_MODE)
    {
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "  FCIE Clk: %u.%u MHz, %Xh\n", g_eMMCDrv[emmc_ip].u32_ClkKHz / 1000,
               (g_eMMCDrv[emmc_ip].u32_ClkKHz / 100) % 10, g_eMMCDrv[emmc_ip].u16_ClkRegVal);
    ...
    switch (g_eMMCDrv[emmc_ip].u32_DrvFlag & DRV_FLAG_SPEED_MASK)
    {
...
void eMMC_DumpDriverStatus(eMMC_IP_EmType emmc_ip)
{
    eMMC_debug(0, 1, "\n  eMMCDrvExtFlag: %Xh \n\n", gu32_eMMCDrvExtFlag);
...
// ------------------------------------------------------
    ...
    eMMC_DumpSpeedStatus(emmc_ip);

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "  PwrOff Notification: ");
    switch (g_eMMCDrv[emmc_ip].u32_DrvFlag & DRV_FLAG_PwrOffNotif_MASK)
    {
...
U32 eMMC_Init(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err;
...
#if 1 // def CONFIG_GENERIC_MMC

extern int fbase[];
extern int multipliers[];

U32 eMMC_GetExtCSD(eMMC_IP_EmType emmc_ip, U8 *pu8_Ext_CSD)
{
    return eMMC_CMD8(emmc_ip, pu8_Ext_CSD);
}

U32 eMMC_SetExtCSD(eMMC_IP_EmType emmc_ip, U8 u8_AccessMode, U8 u8_ByteIdx, U8 u8_Value)
{
    return eMMC_ModifyExtCSD(emmc_ip, u8_AccessMode, u8_ByteIdx, u8_Value);
}
...
static ulong eMMC_bread(int dev_num, ulong start, lbaint_t blkcnt, void *dst)
{
    U32 u32_Err;
        if( blkcnt == 0)
            return 0;
    u32_Err = eMMC_ReadData(emmc_ip, dst, blkcnt << 9, start);
...
static ulong eMMC_bwrite(int dev_num, ulong start, lbaint_t blkcnt, const void *src)
{
    ...
    u32_Err = eMMC_WriteData(emmc_ip, pu8_src, u32_blkcnt << 9, u32_start);
...
int eMMC_mmc_Init(struct mmc* mmc)
{
    ...
    mmc->has_init = 1;
    mmc->capacity = (u64)g_eMMCDrv[emmc_ip].u32_SEC_COUNT << 9;


    eMMC_GetExtCSD((U8*)mmc->ext_csd);

    mmc->high_capacity = g_eMMCDrv[emmc_ip].u8_IfSectorMode;
    mmc->bus_width = g_eMMCDrv[emmc_ip].u8_BUS_WIDTH;

    switch (g_eMMCDrv[emmc_ip].u8_SPEC_VERS) {
        case 0:
    ...
    mmc->tran_speed = fbase[( g_eMMCDrv[emmc_ip].u8_Tran_Speed & 0x7)] * multipliers[((g_eMMCDrv[emmc_ip].u8_Tran_Speed >> 3) & 0xf)];
...

    {
        ...
        for(u16_i = 0; u16_i < 4; u16_i ++)
        {
            if(u16_i < 3)
                mmc->cid[u16_i] =
                    g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 1] << 24 |  g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 2] << 16|
                    g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 3] << 8 | g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 4];
            else
                mmc->cid[u16_i] =
                    g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 1] << 24 |  g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 2] << 16|
                    g_eMMCDrv[emmc_ip].au8_CID[u16_i * 4 + 3] << 8;
        }
    }
    ...
    mmc->block_dev.lba = g_eMMCDrv[emmc_ip].u32_SEC_COUNT;
...
extern U32 gu32_BusWidth[];

static U32 eMMC_Init_Ex(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err = 0;
    ...
    // ---------------------------------
    u32_err = eMMC_Init_Device(emmc_ip);
    if (u32_err)
        goto LABEL_INIT_END;
...
LABEL_INIT_END:
    g_eMMCDrv[emmc_ip].u32_LastErrCode = u32_err;

    // ---------------------------------
    // setup ID
    // use first 10 bytes of CID
    memcpy(g_eMMCDrv[emmc_ip].au8_ID, &g_eMMCDrv[emmc_ip].au8_CID[1], eMMC_ID_FROM_CID_BYTE_CNT);
    g_eMMCDrv[emmc_ip].u8_IDByteCnt = eMMC_ID_DEFAULT_BYTE_CNT;
    // add a 11-th byte for number of GB
    g_eMMCDrv[emmc_ip].au8_ID[eMMC_ID_FROM_CID_BYTE_CNT] = (g_eMMCDrv[emmc_ip].u32_SEC_COUNT >> (1 + 10 + 10)) + 1;
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "%u.%u GB [%Xh]\n", g_eMMCDrv[emmc_ip].u32_SEC_COUNT >> 11 >> 10,
               (g_eMMCDrv[emmc_ip].u32_SEC_COUNT >> 11) * 100 / 1024 % 100, g_eMMCDrv[emmc_ip].u32_SEC_COUNT);
// eMMC_dump_mem(g_eMMCDrv[emmc_ip].au8_ID, 0x10);
#if 0
    u32_err = eMMC_ReadPartitionInfo_Ex(emmc_ip);
    if(eMMC_ST_SUCCESS != u32_err)
    {
...
U32 eMMC_Init_Device_Ex(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err;

#if defined(eMMC_RSP_FROM_RAM) && eMMC_RSP_FROM_RAM
    g_eMMCDrv[emmc_ip].u32_DrvFlag |= DRV_FLAG_RSPFROMRAM_SAVE;
#endif
    ...
    u32_err = eMMC_CSD_Config(emmc_ip);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
...
#if defined(eMMC_RSP_FROM_RAM) && eMMC_RSP_FROM_RAM
    u32_err = eMMC_CMD16(emmc_ip, eMMC_SECTOR_512BYTE);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
#endif

    // determine device parameters, from Ext_CSD
    u32_err = eMMC_ExtCSD_Config(emmc_ip);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;

    // eMMC_debug(0,0,"set bus witdth 8 bits\n");
    u32_err = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 0);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
...
U32 eMMC_Init_Device(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err;

    u32_err = eMMC_Init_Device_Ex(emmc_ip);
    if (eMMC_ST_SUCCESS != u32_err)
        goto LABEL_INIT_END;
...
// eMMC_dump_mem(g_eMMCDrv[emmc_ip].au8_AllRsp, 0x100);
LABEL_INIT_END:

    return u32_err;
}

U32 eMMC_CheckIfReady(eMMC_IP_EmType emmc_ip)
{
    if (eMMC_NOT_READY_MARK != sgu32_IfReadyGuard)
        return eMMC_ST_SUCCESS;
    else
        return eMMC_Init_Ex(emmc_ip);
}
...
U32 eMMC_EraseBlock(eMMC_IP_EmType emmc_ip, U32 u32_eMMCBlkAddr_start, U32 u32_eMMCBlkAddr_end)
{
    U32 u32_err, u32_SectorCnt, u32_i, u32_j;

    if (g_eMMCDrv[emmc_ip].u32_eMMCFlag & eMMC_FLAG_TRIM)
    {
        u32_err = eMMC_EraseCMDSeq(emmc_ip, u32_eMMCBlkAddr_start, u32_eMMCBlkAddr_end);
        if (eMMC_ST_SUCCESS != u32_err)
        {
    ...
    {
        for (u32_i = 0; u32_i < eMMC_SECTOR_BUF_BYTECTN; u32_i++)
            gau8_eMMC_SectorBuf[u32_i] = g_eMMCDrv[emmc_ip].u8_ErasedMemContent;
        ...
        if (u32_i)
        {
            u32_err = eMMC_EraseCMDSeq(emmc_ip, (u32_eMMCBlkAddr_start + u32_SectorCnt),
                                       (u32_eMMCBlkAddr_start + u32_SectorCnt)
                                           + u32_i * g_eMMCDrv[emmc_ip].u32_EraseUnitSize);
            if (eMMC_ST_SUCCESS != u32_err)
            {
...
// ok: return 0
U32 eMMC_WriteData(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr)
{
    U32 u32_err;
    eMMC_LockFCIE(emmc_ip, NULL);
#if defined(MSOS_TYPE_ECOS)
    u32_BlkAddr += eMMC_TOTAL_RESERVED_BLK_CNT;
#endif
    u32_err = eMMC_WriteData_Ex(emmc_ip, pu8_DataBuf, u32_DataByteCnt, u32_BlkAddr);
    eMMC_UnlockFCIE(emmc_ip, NULL);
    return u32_err;
}

U32 eMMC_WriteData_Ex(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr)
{
    ...
    if (u32_BlkAddr + (u32_DataByteCnt >> 9) > g_eMMCDrv[emmc_ip].u32_SEC_COUNT)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: invalid data range, %Xh > %Xh \n",
                   u32_BlkAddr + (u32_DataByteCnt >> 9), g_eMMCDrv[emmc_ip].u32_SEC_COUNT);
        return eMMC_ST_ERR_INVALID_PARAM;
    }
    ...
    while (u32_DataByteCnt)
    {
        if (u32_DataByteCnt > eMMC_SECTOR_512BYTE)
        {
            ...
            u32_err = eMMC_CMD25(emmc_ip, u32_BlkAddr, pu8_DataBuf, u16_BlkCnt);
        }
        else
        {
            u16_BlkCnt = 1;
            u32_err    = eMMC_CMD24(emmc_ip, u32_BlkAddr, pu8_DataBuf);
        }
...
// =======================================================
// ok: return 0
U32 eMMC_ReadData(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr)
{
    U32 u32_err;
    eMMC_LockFCIE(emmc_ip, NULL);
#if defined(MSOS_TYPE_ECOS)
    u32_BlkAddr += eMMC_TOTAL_RESERVED_BLK_CNT;
#endif
    u32_err = eMMC_ReadData_Ex(emmc_ip, pu8_DataBuf, u32_DataByteCnt, u32_BlkAddr);
    eMMC_UnlockFCIE(emmc_ip, NULL);
    return u32_err;
}

U32 eMMC_ReadData_Ex(eMMC_IP_EmType emmc_ip, U8 *pu8_DataBuf, U32 u32_DataByteCnt, U32 u32_BlkAddr)
{
    U32          u32_err;
    volatile U16 u16_BlkCnt;
    ...
    if ((uintptr_t)pu8_DataBuf & (eMMC_CACHE_LINE - 1))
    {
        ...
        u32_err = eMMC_CMD17(emmc_ip, u32_BlkAddr, gau8_eMMC_SectorBuf);
        if (eMMC_ST_SUCCESS != u32_err)
        {
    ...
    while (u32_DataByteCnt)
    {
        if (u32_DataByteCnt > eMMC_SECTOR_512BYTE)
        {
            ...
            u32_err = eMMC_CMD18(emmc_ip, u32_BlkAddr, pu8_DataBuf, u16_BlkCnt);
        }
        else
        {
            u16_BlkCnt = 1;
            u32_err    = eMMC_CMD17(emmc_ip, u32_BlkAddr, pu8_DataBuf);
        }
...
U32 eMMC_GetCapacity(eMMC_IP_EmType emmc_ip, U32 *pu32_TotalSectorCnt) // 1 sector = 512B
{
    U32 u32_err;
    ...
    *pu32_TotalSectorCnt = g_eMMCDrv[emmc_ip].u32_SEC_COUNT;
#if defined(MSOS_TYPE_ECOS)
...
U32 eMMC_USER_WriteProtect_Option(eMMC_IP_EmType emmc_ip, U32 dataAddress, U32 u32_size, U8 u8_mode)
{
    U32 u32_err = 0;
    U32 u32_start;
    ...
    // check if u32_DataByteCnt is 512B boundary
    if (dataAddress + (u32_size >> 9) > g_eMMCDrv[emmc_ip].u32_SEC_COUNT)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: invalid data range, %Xh > %Xh \n",
                   dataAddress + (u32_size >> 9), g_eMMCDrv[emmc_ip].u32_SEC_COUNT);
        return eMMC_ST_ERR_INVALID_PARAM;
    }

    if (g_eMMCDrv[emmc_ip].u8_ECSD171_UserWP & (BIT2 | BIT0))
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC is not in tmp WP mode (%X)\n",
                   g_eMMCDrv[emmc_ip].u8_ECSD171_UserWP & (BIT2 | BIT0));
        return -1;
    }
    ...
    switch (u8_mode)
    {
        case 0:
        case 1:
            u32_start = ((dataAddress / g_eMMCDrv[emmc_ip].u32_WP_group_size) * g_eMMCDrv[emmc_ip].u32_WP_group_size);
            while (u32_start <= (dataAddress + u32_size - 1))
            {
                u32_err = eMMC_CMD28_CMD29(emmc_ip, u32_start, u8_mode ? 29 : 28);
                if (u32_err)
                    return u32_err;

                u32_start += g_eMMCDrv[emmc_ip].u32_WP_group_size;
            }
            break;

        case 2:
        case 3:
            u32_err = eMMC_CMD30_CMD31_MIU(emmc_ip, dataAddress, gau8_eMMC_SectorBuf, (u8_mode == 2) ? 30 : 31);
            if (u32_err)
                return -1;

            if (u8_mode == 2)
                u32_err = (gau8_eMMC_SectorBuf[3] & BIT0);
            else
                u32_err = (gau8_eMMC_SectorBuf[7] & 0x03);

            break;

        default:
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC_%d mode(%d) is not support.\n", emmc_ip, u8_mode);
            break;
    }
...
#if defined(eMMC_FCIE_LINUX_DRIVER) && eMMC_FCIE_LINUX_DRIVER
U32 eMMC_SearchDevNodeStartSector(eMMC_IP_EmType emmc_ip)
{
    U32          u32_err;
    volatile U16 u16_PartIdx;
    eMMC_PNI_t * pPartInfo = (eMMC_PNI_t *)gau8_eMMC_PartInfoBuf;
    ...
    u32_err = eMMC_GetPartitionIndex(emmc_ip, eMMC_PART_DEV_NODE, 0, &u16_PartIdx);
    if (eMMC_ST_SUCCESS != u32_err)
    {
    ...
    g_eMMCDrv[emmc_ip].u32_PartDevNodeStartSector = pPartInfo->records[u16_PartIdx].u16_StartBlk;
    g_eMMCDrv[emmc_ip].u32_PartDevNodeStartSector *= pPartInfo->u16_BlkPageCnt;
    g_eMMCDrv[emmc_ip].u16_PartDevNodeSectorCnt = pPartInfo->records[u16_PartIdx].u16_BlkCnt;
    g_eMMCDrv[emmc_ip].u16_PartDevNodeSectorCnt *= pPartInfo->u16_BlkPageCnt;
    eMMC_UnlockFCIE(emmc_ip, NULL);

    return eMMC_ST_SUCCESS;
}

int get_NVRAM_start_sector(eMMC_IP_EmType emmc_ip, unsigned int *u32_startsector)
{
    eMMC_PNI_t *pPartInfo = (eMMC_PNI_t *)gau8_eMMC_PartInfoBuf;
    ...
    if (eMMC_ST_SUCCESS == eMMC_GetPartitionIndex(emmc_ip, eMMC_PART_NVRAM, 0, &u16_PartIdx))
    {
...
int get_DEVNODE_start_sector(eMMC_IP_EmType emmc_ip, unsigned int *u32_startsector)
{
    u32 u32_err;
    if (0 == g_eMMCDrv[emmc_ip].u32_PartDevNodeStartSector)
    {
        u32_err = eMMC_SearchDevNodeStartSector(emmc_ip);
        if (eMMC_ST_SUCCESS != u32_err)
        {
    ...
    *u32_startsector = g_eMMCDrv[emmc_ip].u32_PartDevNodeStartSector;
