...
extern U64 gu64_Read_spend[5];
extern U64 gu64_Write_spend[5];
...
static void eMMC_FCIE_DumpDebugBus(eMMC_IP_EmType emmc_ip)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "\nEMMCPLL DebugBus:\n");
    REG_FCIE_W(reg_emmcpll_0x17(emmc_ip), 0x100);
    eMMC_debug(0, 0, "0x16 = %04Xh \n", REG_FCIE(reg_emmcpll_0x16(emmc_ip)));
    eMMC_debug(0, 0, "0x17 = %04Xh \n", REG_FCIE(reg_emmcpll_0x17(emmc_ip)));
...
static void eMMC_FCIE_DumpRegisters(eMMC_IP_EmType emmc_ip)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "\r\n\r\nCLK_GEN Reg:x%x(BK:x%x)", PADGPIO_BASE, (PADGPIO_BASE & 0x00FFFFFF) >> 9);
    for (u16_i = 0; u16_i < 0x80; u16_i++)
    {
        if (0 == u16_i % 8)
        {
        ...
        REG_FCIE_R(GET_REG_ADDR(PADGPIO_BASE, u16_i), u16_reg);
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "%04Xh ", u16_reg);
    }
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "\r\n");
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "\r\n");
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "[gu16_eMMCMIEEvent_ForInt] = 0x%04X\r\n", eMMC_INT_GetMIEEvent(emmc_ip));
#endif
}
...
static void eMMC_WR_time_count(U64 timer, eMMC_Cmd_EmType e_CmdType)
{
    U8 i;
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        {
            if (timer >= gu64_Read_spend[i])
            {
                gu64_Read_spend[i] = timer;
                break;
            }
        }
        else if (e_CmdType == eMMC_CMD_WRITE)
        {
            if (timer >= gu64_Write_spend[i])
            {
                gu64_Write_spend[i] = timer;
                break;
            }
        }
    }
...
/* Set Read and Write start-bit timeout detect */
static void eMMC_FCIE_start_bit_detect(eMMC_IP_EmType emmc_ip)
{
    volatile U16 u16_value;
    volatile U32 u32_cycle;

    u32_cycle = 1000000 / g_eMMCDrv[emmc_ip].u32_ClkKHz;
    u16_value = 20000000000 / (u32_cycle * 65536);

    REG_FCIE_W(FCIE_RD_SBIT_TIMER(emmc_ip), BIT15 | u16_value);
    REG_FCIE_W(FCIE_WR_SBIT_TIMER(emmc_ip), BIT15 | u16_value);
}
...
LABEL_SEND_CMD:
    ...
    d0      = t2 - t1;
    eMMC_WR_time_count(d0, e_CmdType);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
LABEL_SEND_CMD:
    ...
    eMMC_FCIE_ClearEvents(emmc_ip);
    eMMC_FCIE_start_bit_detect(emmc_ip);
    ...
    if (eMMC_ST_SUCCESS != u32_err)
    {
        ...
        {
            u8_retry_cmd++;
            eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) WARN: wait event error:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u32_err);

            goto SEND_CMD_RETRY;
        }
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) ERR: wait event error:%X \n",
                   __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u32_err);

        if (g_eMMCDrv[emmc_ip].u8_make_sts_err == FCIE_MAKE_WR_TOUT_ERR
            || g_eMMCDrv[emmc_ip].u8_make_sts_err == FCIE_MAKE_RD_TOUT_ERR)
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) make sts ERR: check SD_STS:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, REG_FCIE(FCIE_SD_STATUS(emmc_ip)));
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
                ...
                && eMMC_Error_Retry(emmc_ip))
            {
                ...
                eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1,
                           "LINE_%d eMMC_%d cmd[%d] args(0x%x) WARN: wait event error:%X \n", __LINE__, emmc_ip,
                           gu32Arg, u8_CmdIdx, u32_err);
                goto SEND_CMD_RETRY;
            }
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) ERR: wait event error:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u32_err);
    ...
    if (u16_reg & BIT_SD_FCIE_ERR_FLAGS)
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) WARN: check SD_STS:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u16_reg);
            goto SEND_CMD_RETRY;
        }
        u32_err = u16_reg;
        if (u8_CmdIdx == 13 && 0 != eMMC_IF_TUNING_TTABLE(emmc_ip))
            goto LABEL_SEND_CMD_ERROR;
        else
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) ERR: check SD_STS:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u16_reg);
            goto LABEL_SEND_CMD_ERROR;
        }
    }
    ...
    if ((e_RspType == eMMC_R1) || (e_RspType == eMMC_R1B))
    {
        ...
        if (eMMC_ST_SUCCESS != u32_err)
        {
                ...
                && eMMC_Error_Retry(emmc_ip))
            {
                u8_retry_r1++;
                eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 1,
                           "LINE_%d eMMC_%d cmd[%d] args(0x%x) WARN: check R1 error: %X \n", __LINE__, emmc_ip,
                           u8_CmdIdx, gu32Arg, u32_err);
                goto SEND_CMD_RETRY;
            }
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "LINE_%d eMMC_%d cmd[%d] args(0x%x) ERR: check R1 error:%X \n",
                       __LINE__, emmc_ip, u8_CmdIdx, gu32Arg, u32_err);
            goto LABEL_SEND_CMD_ERROR;
        }
    }
...
void eMMC_PreparePowerSavingModeQueue(eMMC_IP_EmType emmc_ip)
{
    ...
    U16 u16_Reg;
#endif
    U16 u16_RegRst = (IP_BIT_FUNC_ENABLE(emmc_ip) == BIT_EMMC_EN) ? 0x001E : 0x000E;
    U32 u32_Offset = 0;
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x000F);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x34);

    /* Wait CMD Status Idle */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0000);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0xFFFF);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), u16_RegRst);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x34);
#endif
    ...
    /* (6) Reset End */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0001);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x3F);

    /* (7) Set "SD_MOD" */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0021);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x0B);
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0040);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
    ...
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0500);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x0A);

    /* (13) Enable Interrupt, SD_CMD_END */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0002);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x01);

    /* (14) Command Enable + job Start */
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++), 0x0044);
    REG_FCIE_W(GET_REG_ADDR(FCIE_POWEER_SAVE_MODE_BASE(emmc_ip), u32_Offset++),
               PWR_BAT_CLASS | PWR_RST_CLASS | PWR_CMD_WREG | PWR_CMD_BK0 | 0x0C);
    ...
    REG_FCIE_SETBIT(FCIE_PWR_SAVE_CTL(emmc_ip),
                    BIT_POWER_SAVE_MODE | BIT_SD_POWER_SAVE_RIU | BIT_POWER_SAVE_MODE_INT_EN | BIT_SD_POWER_SAVE_RST);
#else
    REG_FCIE_SETBIT(FCIE_PWR_SAVE_CTL(emmc_ip), BIT_POWER_SAVE_MODE);
    REG_FCIE_SETBIT(FCIE_PWR_SAVE_CTL(emmc_ip), BIT_POWER_SAVE_MODE_INT_EN);
#endif
