...
U32 eMMCTest_SingleBlkWRC_IMI_Ex(eMMC_IP_EmType emmc_ip, U32 u32_eMMC_Addr, U8 *pu8_W, U8 *pu8_R)
{
    ...
    Chip_Flush_Cache_Range(pu8_W, eMMC_SECTOR_BYTECNT);
    ...
    Chip_Inv_Cache_Range(pu8_R, eMMC_SECTOR_BYTECNT);
...
U32 eMMC_BootMode(void)
{
    ...
    eMMC_debug(0,1,"ChkSum -: %Xh, Addr: %llxh \n", eMMC_ChkSum((U8*)(uintptr_t)size_Addr[0], 0xC1<<eMMC_SECTOR_512BYTE_BITS), (u64)(uintptr_t)size_Addr[0]);
#else
    u32_SecIdx = 0;
...
U32 eMMCTest_Write_Timeout_Error(eMMC_IP_EmType emmc_ip)
{
    ...
    eMMC_Write_Timeout_Set(emmc_ip);

    eMMC_CMD24_MIU(emmc_ip, eMMC_TEST_BLK_0, pu8_W);

    eMMC_Write_Timeout_Clear(emmc_ip);
...
// ===============================================
    ...
    for (u8_i = 0; u8_i < u8_bus; u8_i++)
    {
        ...
        for (i = 0; i < 12; i++)
        {
            if (eMMC_IPVerify_Main_Ex(emmc_ip, u32_test_pattern[i]) != 0)
                goto err;
        }
...
#if 0 // need clk speed more than 12MHz
    u32_err = eMMCTest_SingleBlkWRC_RIU(emmc_ip, u32_StartSector, u32_DataPattern);
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "Err, eMMCTest_SgWRC_RIU fail: %Xh \n", u32_err);
        goto LABEL_IP_VERIFY_ERROR;
    }
    eMMC_debug(eMMC_DEBUG_LEVEL_HIGH, 1, "[eMMCTest_SgWRC_RIU ok] \n");
#endif
    // ===============================================
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "\n");
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "[OK] \n\n");
