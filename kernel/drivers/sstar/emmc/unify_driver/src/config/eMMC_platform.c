...
static volatile U16 gu16_eMMCMIEEvent_ForInt[EMMC_NUM_TOTAL] = {0}; // MIEEvent for Interrupt
...
U16 eMMC_INT_GetMIEEvent(eMMC_IP_EmType emmc_ip)
{
    return gu16_eMMCMIEEvent_ForInt[emmc_ip];
}
...
U32 eMMC_WaitCompleteIntr(eMMC_IP_EmType emmc_ip, uintptr_t u32_RegAddr, U16 u16_WaitEvent, U32 u32_MicroSec)
{
    ...
    if (wait_event_timeout(fcie_wait, ((gu16_eMMCMIEEvent_ForInt[emmc_ip] & u16_WaitEvent) == u16_WaitEvent),
                           (usecs_to_jiffies(u32_MicroSec) + 1))
        == 0)
    {
#if eMMC_IRQ_DEBUG
...
U32 eMMC_PlatformResetPost(eMMC_IP_EmType emmc_ip)
{
