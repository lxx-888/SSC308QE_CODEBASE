...
void _MSYS_IMPL_ModuleIdToPrefixName(MI_ModuleId_e eModuleId, char *prefix_name)
{
    switch (eModuleId)
    {
        ...
        case E_MI_MODULE_ID_SD:
            ...
            break;
        case E_MI_MODULE_ID_DEBUG:
            strcpy(prefix_name, "mi_debug");
            break;
        default:
...
typedef struct MI_SYS_LogConfigInfo_S
{
    ...
    MI_SYS_LogOutputPos_e eLogPrintPos;
    /*log output location, 0:serial port; 1:network;2:u-disk*/ /*CNcomment:  Ä£¿é´òÓ¡Î»ÖÃ¿ØÖÆ 0:´®¿Ú 1:ÍøÂç 2:UÅÌ */
...
MI_S32 _MSYS_LOG_IMPL_ConfigInfoInit(void)
{
    ...
    for (ModuleIdx = 0; ModuleIdx < (int)E_MI_MODULE_ID_MAX; ModuleIdx++)
    {
        ...
        MSYS_LOG_IMPL_Snprintf(_gpstLogConfigInfo[ModuleIdx].u8ModName, sizeof(_gpstLogConfigInfo[ModuleIdx].u8ModName),
                               "modulemax");
    }
...
MI_S32 MSYS_LOG_IMPL_WriteBuf(MI_U8 *pu8Buf, MI_U32 u32MsgLen, MI_U32 u32UserOrKer)
{
    ...
    {
        if ((_gstLogBufferInfo.u32ReadAddr - _gstLogBufferInfo.u32WriteAddr) < DEBUG_MSG_BUF_SIZE)
        {
            LOGBufferReset();
        }
    }
    ...
    if (u32CopyLen1 > 0)
    {
        ...
        {
            if (copy_from_user((_gstLogBufferInfo.u32WriteAddr + _gstLogBufferInfo.pu8StartAddrVir), pu8Buf,
                               u32CopyLen1))
            {
    ...
    if (u32CopyLen2 > 0)
    {
        ...
        {
            if (copy_from_user(_gstLogBufferInfo.pu8StartAddrVir, (pu8Buf + u32CopyLen1), u32CopyLen2))
            {
                DBG_ERR("copy_from_user error\n");
            }
        }
...
MI_S32 MSYS_LOG_IMPL_ReadBuf(MI_U8 *pu8Buf, MI_U32 u32BufLen, MI_U32 *pu32CopyLen, MI_BOOL bKernelCopy)
{
    ...
    {
        u32BufUsedLen = stCurryMsgInfo.u32BufSize - stCurryMsgInfo.u32ReadAddr + stCurryMsgInfo.u32WriteAddr;
...
MI_S32 MSYS_LOG_IMPL_LogAddModule(const MI_U8 *szProcName, MI_ModuleId_e eModuleID)
{
    ...
    MSYS_LOG_IMPL_Snprintf(_gpstLogConfigInfo[eModuleID].u8ModName, sizeof(_gpstLogConfigInfo[eModuleID].u8ModName),
                           "%s", szProcName);
...
ssize_t MSYS_LOG_IMPL_ProcWrite(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
