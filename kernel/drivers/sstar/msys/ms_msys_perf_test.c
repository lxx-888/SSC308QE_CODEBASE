...
#define STR_CPUINFO_CA32 "ARMv8 CA32"
...
static int print_test_func(void* arg)
{
    ...
    wait_queue_head_t irqctrl_wq;
#endif

#ifdef CONFIG_PM_SLEEP
    init_waitqueue_head(&irqctrl_wq);
    set_freezable();
#endif
    while ((para_tmp.user_time--) && !kthread_should_stop())
    {
        oldt = ktime_get();
#ifdef CONFIG_PM_SLEEP
        while ((CPUCost(&oldt) <= (para_tmp.user_per * 1)) && !pm_freezing)
#else
        ...
        wait_event_freezable_timeout(irqctrl_wq, kthread_should_stop(),
                                     usecs_to_jiffies(10000 - para_tmp.user_per * 1));
#else
...
static int loading_test_func(void* arg)
{
    ktime_t          oldt;
    struct user_para para_tmp = *(struct user_para*)arg;
    char*            dest     = vmalloc(sizeof(char) * mem_count * mem_size);
#ifdef CONFIG_PM_SLEEP
    wait_queue_head_t irqctrl_wq;
#endif
    if (dest == NULL)
    ...
    printk("Cpu-%u vmalloc success\n", para_tmp.cpu);
    while ((para_tmp.user_time--) && !kthread_should_stop())
    {
        oldt = ktime_get();
#ifdef CONFIG_PM_SLEEP
        while ((CPUCost(&oldt) <= (para_tmp.user_per * 10)) && !pm_freezing)
#else
...
#ifdef CONFIG_FREEZER
        wait_event_freezable_timeout(irqctrl_wq, kthread_should_stop(),
                                     usecs_to_jiffies(1000 - para_tmp.user_per * 10));
#else
    ...
    vfree(dest);
fail:
    printk("Cpu:%u, Switch:%u, 4:both, 3:memcpy, 2:loading, others:none, Count: %u\n", para_tmp.cpu,
...
#ifdef CONFIG_FREEZER
        wait_event_freezable_timeout(irqctrl_wq, kthread_should_stop(), usecs_to_jiffies(para_tmp.time_sleep));
#else
        if (para_tmp.time_sleep <= 1000)
        {
            usleep_range(para_tmp.time_sleep, para_tmp.time_sleep);
        }
        else
        {
            msleep_interruptible((para_tmp.time_sleep + 999) / 1000);
        }
#endif
    }
    run_flag = 0;
...
{
    printk(KERN_ERR "msys_perf_test_init\n");
#ifdef CONFIG_CPU_PERF_TEST
