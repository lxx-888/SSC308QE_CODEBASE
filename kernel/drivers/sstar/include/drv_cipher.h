...
typedef struct
{
    ...
    u8                 key[AES_KEYSIZE_256]; // key
    u8                 iv[AES_KEYSIZE_256];  // initial vector
    u32                keylen;               // length of key
    MDRV_AES_ALGO_MODE mode;
    ss_phys_addr_t     in_pa;  // input phy addr
    ss_phys_addr_t     out_pa; // output phy addr
} MDRV_AES_HANDLE;
...
typedef struct
{
    ...
    u32 len;              // length of input, must be less than modulus length
