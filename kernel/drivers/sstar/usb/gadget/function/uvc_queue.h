...
/*Sstar jpeg encode supports output in slice to achieve the purpose of low latency transmission. default is close*/
#ifdef CONFIG_UVC_JPEG_SLICE_FUNC
#define UVC_BUFFER_FLAGS_FRAME_END(flags)                                      \
	((UVC_BUFFER_FLAGS_FRAME_NOEND & flags) == 0)
#else
#define UVC_BUFFER_FLAGS_FRAME_END(flags) 1
#endif
	__u32 reserved;
};
