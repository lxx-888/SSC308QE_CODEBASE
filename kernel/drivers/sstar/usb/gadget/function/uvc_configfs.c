...
static int
uvcg_config_create_children(struct config_group *group,
			    const struct uvcg_config_group_type *type)
{
...
static void uvcg_config_remove_children(struct config_group *group)
{
	...
	list_for_each_entry_safe (child, n, &group->default_groups,
				  group_entry) {
...
static struct uvcg_control_header *
to_uvcg_control_header(struct config_item *item)
{
...
#define UVCG_CTRL_HDR_ATTR(cname, aname, bits, limit)                          \
	...
	{                                                                      \
		...
		struct mutex *su_mutex =                                       \
			&ch->item.ci_group->cg_subsys->su_mutex;               \
		int result;                                                    \
                                                                               \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t uvcg_control_header_##cname##_store(                    \
...
static ssize_t
uvcg_default_processing_bm_controls_show(struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
/* -----------------------------------------------------------------------------
 * control/extension/isp
 */

#define UVCG_ISP_EXTENSION_ATTR(cname, aname, bits)                            \
	static ssize_t uvcg_isp_extension_##cname##_show(                      \
		struct config_item *item, char *page)                          \
	{                                                                      \
		struct config_group *group = to_config_group(item);            \
		struct f_uvc_opts *opts;                                       \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	UVC_ATTR_RO(uvcg_isp_extension_, cname, aname)

UVCG_ISP_EXTENSION_ATTR(b_unit_id, bUnitID, 8);
UVCG_ISP_EXTENSION_ATTR(i_extension, iExtension, 8);

#undef UVCG_ISP_EXTENSION_ATTR

static ssize_t uvcg_isp_extension_ba_source_id_show(struct config_item *item,
						    char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
	...
	mutex_lock(&opts->lock);
	for (result = 0, i = 0; i < xd->bNrInPins; ++i) {
		result += sprintf(pg, "%d\n", xd->baSourceID[i]);
		pg = page + result;
	}
	mutex_unlock(&opts->lock);
...
UVC_ATTR_RO(uvcg_isp_extension_, ba_source_id, baSourceID);

static ssize_t uvcg_isp_extension_bm_controls_show(struct config_item *item,
						   char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
	...
	mutex_lock(&opts->lock);
	for (result = 0, i = 0; i < xd->bControlSize; ++i) {
		result += sprintf(pg, "%u\n", xd->bmControls[i]);
		pg = page + result;
	}
	mutex_unlock(&opts->lock);
...
UVC_ATTR_RO(uvcg_isp_extension_, bm_controls, bmControls);

static struct configfs_attribute *uvcg_isp_extension_attrs[] = {
	&uvcg_isp_extension_attr_b_unit_id,
	&uvcg_isp_extension_attr_ba_source_id,
	&uvcg_isp_extension_attr_bm_controls,
	&uvcg_isp_extension_attr_i_extension,
	NULL,
};

static const struct uvcg_config_group_type uvcg_isp_extension_type = {
	.type = {
		.ct_item_ops	= &uvcg_config_item_ops,
		.ct_attrs	= uvcg_isp_extension_attrs,
		.ct_owner	= THIS_MODULE,
	},
	.name = "isp",
};
...
/* -----------------------------------------------------------------------------
 * control/extension/customer
 */

#define UVCG_CUSTOMER_EXTENSION_ATTR(cname, aname, bits)                       \
	static ssize_t uvcg_customer_extension_##cname##_show(                 \
		struct config_item *item, char *page)                          \
	{                                                                      \
		struct config_group *group = to_config_group(item);            \
		struct f_uvc_opts *opts;                                       \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	UVC_ATTR_RO(uvcg_customer_extension_, cname, aname)

UVCG_CUSTOMER_EXTENSION_ATTR(b_unit_id, bUnitID, 8);
UVCG_CUSTOMER_EXTENSION_ATTR(i_extension, iExtension, 8);

#undef UVCG_CUSTOMER_EXTENSION_ATTR

static ssize_t
uvcg_customer_extension_ba_source_id_show(struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
UVC_ATTR_RO(uvcg_customer_extension_, ba_source_id, baSourceID);

static ssize_t
uvcg_customer_extension_bm_controls_show(struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
...
UVC_ATTR_RO(uvcg_customer_extension_, bm_controls, bmControls);

static struct configfs_attribute *uvcg_customer_extension_attrs[] = {
	&uvcg_customer_extension_attr_b_unit_id,
	&uvcg_customer_extension_attr_ba_source_id,
	&uvcg_customer_extension_attr_bm_controls,
	&uvcg_customer_extension_attr_i_extension,
	NULL,
};

static const struct uvcg_config_group_type uvcg_customer_extension_type = {
	.type = {
		.ct_item_ops	= &uvcg_config_item_ops,
		.ct_attrs	= uvcg_customer_extension_attrs,
		.ct_owner	= THIS_MODULE,
	},
	.name = "customer",
};
...
static const struct uvcg_config_group_type uvcg_extension_grp_type = {
	...
	.name = "extension",
	.children = (const struct uvcg_config_group_type*[]) {
		&uvcg_isp_extension_type,
		&uvcg_customer_extension_type,
		NULL,
	},
};
...
UVCG_DEFAULT_CAMERA_ATTR(w_objective_focal_length_max, wObjectiveFocalLengthMax,
			 16);
UVCG_DEFAULT_CAMERA_ATTR(w_ocular_focal_length, wOcularFocalLength, 16);

#undef UVCG_DEFAULT_CAMERA_ATTR

static ssize_t uvcg_default_camera_bm_controls_show(struct config_item *item,
						    char *page)
{
	struct config_group *group = to_config_group(item);
	struct f_uvc_opts *opts;
...
uvcg_get_ctl_class_arr(struct config_item *i, struct f_uvc_opts *o)
{
	struct uvcg_control_class_group *group =
		container_of(i, struct uvcg_control_class_group, group.cg_item);
...
static ssize_t
uvcg_default_control_b_interface_number_show(struct config_item *item,
					     char *page)
{
	struct config_group *group = to_config_group(item);
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;
...
static struct uvcg_streaming_header *
to_uvcg_streaming_header(struct config_item *item)
{
...
#define UVCG_STREAMING_HEADER_ATTR(cname, aname, bits)                         \
	static ssize_t uvcg_streaming_header_##cname##_show(                   \
		struct config_item *item, char *page)                          \
	{                                                                      \
		struct uvcg_streaming_header *sh =                             \
			to_uvcg_streaming_header(item);                        \
		struct f_uvc_opts *opts;                                       \
		struct config_item *opts_item;                                 \
		struct mutex *su_mutex =                                       \
			&sh->item.ci_group->cg_subsys->su_mutex;               \
		int result;                                                    \
                                                                               \
		mutex_lock(su_mutex); /* for navigating configfs hierarchy */  \
                                                                               \
...
uvcg_streaming_header_make(struct config_group *group, const char *name)
{
	...
	h->desc.bTerminalLink = UVC_OT_ID;
...
struct uvcg_frame {
	...
	struct {
		...
		u32 dw_min_bit_rate;
		u32 dw_max_bit_rate;
		u32 dw_max_video_frame_buffer_size; /* uncompressed/mjpg */
		u32 dw_default_frame_interval;
		u8 b_frame_interval_type;
		u32 dw_bytes_per_line; /* framebase */
...
#define UVCG_FRAME_ATTR(cname, aname, bits)                                    \
	...
	{                                                                      \
		...
		struct config_item *opts_item;                                 \
		struct mutex *su_mutex =                                       \
			&f->item.ci_group->cg_subsys->su_mutex;                \
		int result;                                                    \
                                                                               \
		mutex_lock(su_mutex); /* for navigating configfs hierarchy */  \
                                                                               \
		opts_item =                                                    \
			f->item.ci_parent->ci_parent->ci_parent->ci_parent;    \
		opts = to_f_uvc_opts(opts_item);                               \
                                                                               \
		mutex_lock(&opts->lock);                                       \
		result = sprintf(page, "%u\n", f->frame.cname);                \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t uvcg_frame_##cname##_store(                             \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct uvcg_frame *f = to_uvcg_frame(item);                    \
		struct f_uvc_opts *opts;                                       \
				 ...
				 int (*fun)(char *, void *), void *priv)
{
	...
	while (pg - page < len) {
		i = 0;
		while (i < sizeof(buf) && (pg - page < len) && *pg != '\0' &&
		       *pg != '\n')
			buf[i++] = *pg++;
		if (i == sizeof(buf))
			return -EINVAL;
...
#define UVCG_UNCOMPRESSED_ATTR_RO(cname, aname, bits)                          \
	...
	{                                                                      \
		...
		mutex_lock(su_mutex); /* for navigating configfs hierarchy */  \
                                                                               \
		opts_item =                                                    \
			u->fmt.group.cg_item.ci_parent->ci_parent->ci_parent;  \
		opts = to_f_uvc_opts(opts_item);                               \
                                                                               \
		mutex_lock(&opts->lock);                                       \
		result = sprintf(page, "%u\n",                                 \
...
#define UVCG_UNCOMPRESSED_ATTR(cname, aname, bits)                             \
	...
	{                                                                      \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t uvcg_uncompressed_##cname##_store(                      \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct uvcg_uncompressed *u = to_uvcg_uncompressed(item);      \
		struct f_uvc_opts *opts;                                       \
...
static inline ssize_t
uvcg_uncompressed_bma_controls_store(struct config_item *item, const char *page,
				     size_t len)
{
	struct uvcg_uncompressed *unc = to_uvcg_uncompressed(item);
...
static struct config_group *uvcg_uncompressed_make(struct config_group *group,
						   const char *name)
{
	static char guid[] = { 'Y',  'U',  'Y',	 '2',  0x00, 0x00, 0x10, 0x00,
...
#define UVCG_MJPEG_ATTR_RO(cname, aname, bits)                                 \
	static ssize_t uvcg_mjpeg_##cname##_show(struct config_item *item,     \
						 char *page)                   \
	{                                                                      \
...
#define UVCG_MJPEG_ATTR(cname, aname, bits)                                    \
	...
	{                                                                      \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t uvcg_mjpeg_##cname##_store(                             \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct uvcg_mjpeg *u = to_uvcg_mjpeg(item);                    \
		struct f_uvc_opts *opts;                                       \
...
static inline ssize_t uvcg_mjpeg_bma_controls_show(struct config_item *item,
						   char *page)
{
	struct uvcg_mjpeg *u = to_uvcg_mjpeg(item);
...
static inline ssize_t uvcg_mjpeg_bma_controls_store(struct config_item *item,
						    const char *page,
						    size_t len)
{
	struct uvcg_mjpeg *u = to_uvcg_mjpeg(item);
					    ...
					    const char *name)
{
	...
	config_group_init_type_name(&h->fmt.group, name, &uvcg_mjpeg_type);
...
/* -----------------------------------------------------------------------------
 * streaming/framebase/<NAME>
 */

struct uvcg_framebase {
	struct uvcg_format fmt;
	struct uvc_format_framebase desc;
};

static struct uvcg_framebase *to_uvcg_framebase(struct config_item *item)
{
	return container_of(container_of(to_config_group(item),
					 struct uvcg_format, group),
			    struct uvcg_framebase, fmt);
}

static struct configfs_group_operations uvcg_framebase_group_ops = {
	.make_item = uvcg_frame_make,
	.drop_item = uvcg_frame_drop,
};

static ssize_t uvcg_framebase_guid_format_show(struct config_item *item,
					       char *page)
{
	struct uvcg_framebase *ch = to_uvcg_framebase(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
	struct mutex *su_mutex = &ch->fmt.group.cg_subsys->su_mutex;
...
static ssize_t uvcg_framebase_guid_format_store(struct config_item *item,
						const char *page, size_t len)
{
	struct uvcg_framebase *ch = to_uvcg_framebase(item);
	struct f_uvc_opts *opts;
...
UVC_ATTR(uvcg_framebase_, guid_format, guidFormat);

#define UVCG_FRAMEBASE_ATTR_RO(cname, aname, bits)                             \
	static ssize_t uvcg_framebase_##cname##_show(struct config_item *item, \
						     char *page)               \
	{                                                                      \
		struct uvcg_framebase *u = to_uvcg_framebase(item);            \
		struct f_uvc_opts *opts;                                       \
		struct config_item *opts_item;                                 \
		struct mutex *su_mutex = &u->fmt.group.cg_subsys->su_mutex;    \
		int result;                                                    \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	UVC_ATTR_RO(uvcg_framebase_, cname, aname);

#define UVCG_FRAMEBASE_ATTR(cname, aname, bits)                                \
	static ssize_t uvcg_framebase_##cname##_show(struct config_item *item, \
						     char *page)               \
	{                                                                      \
		struct uvcg_framebase *u = to_uvcg_framebase(item);            \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t uvcg_framebase_##cname##_store(                         \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct uvcg_framebase *u = to_uvcg_framebase(item);            \
		struct f_uvc_opts *opts;                                       \
		...
		return ret;                                                    \
                                                                               ...
                                                                               \
	UVC_ATTR(uvcg_framebase_, cname, aname);

UVCG_FRAMEBASE_ATTR_RO(b_format_index, bFormatIndex, 8);
UVCG_FRAMEBASE_ATTR(b_bits_per_pixel, bBitsPerPixel, 8);
UVCG_FRAMEBASE_ATTR(b_default_frame_index, bDefaultFrameIndex, 8);
UVCG_FRAMEBASE_ATTR_RO(b_aspect_ratio_x, bAspectRatioX, 8);
UVCG_FRAMEBASE_ATTR_RO(b_aspect_ratio_y, bAspectRatioY, 8);
UVCG_FRAMEBASE_ATTR_RO(bm_interface_flags, bmInterfaceFlags, 8);

#undef UVCG_FRAMEBASE_ATTR
#undef UVCG_FRAMEBASE_ATTR_RO

static inline ssize_t uvcg_framebase_bma_controls_show(struct config_item *item,
						       char *page)
{
	struct uvcg_framebase *unc = to_uvcg_framebase(item);
	return uvcg_format_bma_controls_show(&unc->fmt, page);
}

static inline ssize_t
uvcg_framebase_bma_controls_store(struct config_item *item, const char *page,
				  size_t len)
{
	struct uvcg_framebase *unc = to_uvcg_framebase(item);
	return uvcg_format_bma_controls_store(&unc->fmt, page, len);
...
UVC_ATTR(uvcg_framebase_, bma_controls, bmaControls);

static struct configfs_attribute *uvcg_framebase_attrs[] = {
	&uvcg_framebase_attr_b_format_index,
	&uvcg_framebase_attr_guid_format,
	&uvcg_framebase_attr_b_bits_per_pixel,
	&uvcg_framebase_attr_b_default_frame_index,
	&uvcg_framebase_attr_b_aspect_ratio_x,
	&uvcg_framebase_attr_b_aspect_ratio_y,
	&uvcg_framebase_attr_bm_interface_flags,
	&uvcg_framebase_attr_bma_controls,
	NULL,
};

static const struct config_item_type uvcg_framebase_type = {
	.ct_item_ops = &uvcg_config_item_ops,
	.ct_group_ops = &uvcg_framebase_group_ops,
	.ct_attrs = uvcg_framebase_attrs,
	.ct_owner = THIS_MODULE,
};

static struct config_group *uvcg_framebase_make(struct config_group *group,
						const char *name)
{
	static char guid[] = { 'H',  '2',  '6',	 '4',  0x00, 0x00, 0x10, 0x00,
			       0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71 };
	struct uvcg_framebase *h;

	h = kzalloc(sizeof(*h), GFP_KERNEL);
	if (!h)
		return ERR_PTR(-ENOMEM);

	h->desc.bLength = UVC_DT_FORMAT_FRAMEBASE_SIZE;
	h->desc.bDescriptorType = USB_DT_CS_INTERFACE;
	...
	h->fmt.type = UVCG_FRAMEBASE;
	config_group_init_type_name(&h->fmt.group, name, &uvcg_framebase_type);

	return &h->fmt.group;
}

static struct configfs_group_operations uvcg_framebase_grp_ops = {
	.make_group = uvcg_framebase_make,
};

static const struct uvcg_config_group_type uvcg_framebase_grp_type = {
	.type = {
		.ct_item_ops	= &uvcg_config_item_ops,
		.ct_group_ops	= &uvcg_framebase_grp_ops,
		.ct_owner	= THIS_MODULE,
	},
	.name = "framebase",
};
...
{
	struct uvcg_streaming_class_group *group = container_of(
		i, struct uvcg_streaming_class_group, group.cg_item);
...
enum uvcg_strm_type { UVCG_HEADER = 0, UVCG_FORMAT, UVCG_FRAME };
 ...
 */
static int __uvcg_iter_strm_cls(struct uvcg_streaming_header *h, void *priv2,
				void *priv3,
				int (*fun)(void *, void *, void *, int,
			   ...
			   enum uvcg_strm_type type)
{
	...
	case UVCG_FORMAT: {
		...
		if (fmt->type == UVCG_UNCOMPRESSED) {
			struct uvcg_uncompressed *u = container_of(
				fmt, struct uvcg_uncompressed, fmt);
			...
			*size += sizeof(m->desc);
			...
			struct uvcg_framebase *m =
				container_of(fmt, struct uvcg_framebase, fmt);
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_FRAME: {
		...
		/* Because there are differences between uncompressed/mjpg
		 * and framebase, packing method should be different too.
		*/
		if (frm->fmt_type == UVCG_FRAMEBASE) {
			memcpy(*dest, &frm->frame.b_length,
			       sizeof(frm->frame.b_length));
			*dest += sizeof(frm->frame.b_length);
			memcpy(*dest, &frm->frame.b_descriptor_type,
			       sizeof(frm->frame.b_descriptor_type));
			*dest += sizeof(frm->frame.b_descriptor_type);
			memcpy(*dest, &frm->frame.b_descriptor_subtype,
			       sizeof(frm->frame.b_descriptor_subtype));
			*dest += sizeof(frm->frame.b_descriptor_subtype);
			memcpy(*dest, &frm->frame.b_frame_index,
			       sizeof(frm->frame.b_frame_index));
			*dest += sizeof(frm->frame.b_frame_index);
			memcpy(*dest, &frm->frame.bm_capabilities,
			       sizeof(frm->frame.bm_capabilities));
			*dest += sizeof(frm->frame.bm_capabilities);
			memcpy(*dest, &frm->frame.w_width,
			       sizeof(frm->frame.w_width));
			*dest += sizeof(frm->frame.w_width);
			memcpy(*dest, &frm->frame.w_height,
			       sizeof(frm->frame.w_height));
			*dest += sizeof(frm->frame.w_height);
			memcpy(*dest, &frm->frame.dw_min_bit_rate,
			       sizeof(frm->frame.dw_min_bit_rate));
			*dest += sizeof(frm->frame.dw_min_bit_rate);
			memcpy(*dest, &frm->frame.dw_max_bit_rate,
			       sizeof(frm->frame.dw_max_bit_rate));
			*dest += sizeof(frm->frame.dw_max_bit_rate);
			memcpy(*dest, &frm->frame.dw_default_frame_interval,
			       sizeof(frm->frame.dw_default_frame_interval));
			*dest += sizeof(frm->frame.dw_default_frame_interval);
			memcpy(*dest, &frm->frame.b_frame_interval_type,
			       sizeof(frm->frame.b_frame_interval_type));
			*dest += sizeof(frm->frame.b_frame_interval_type);
			memcpy(*dest, &frm->frame.dw_bytes_per_line,
			       sizeof(frm->frame.dw_bytes_per_line));
			*dest += sizeof(frm->frame.dw_bytes_per_line);
			...
			*dest += sz;
			...
			h->bLength = UVC_DT_FRAME_FRAMEBASE_SIZE(
			...
			*dest += sizeof(frm->frame.dw_max_bit_rate);
			memcpy(*dest,
			       &frm->frame.dw_max_video_frame_buffer_size,
			       sizeof(frm->frame
			...
			*dest += sz;
			if (frm->fmt_type == UVCG_UNCOMPRESSED)
				...
				h->bLength = UVC_DT_FRAME_MJPEG_SIZE(
					frm->frame.b_frame_interval_type);
			else if (frm->fmt_type == UVCG_FRAMEBASE)
				h->bLength = UVC_DT_FRAME_FRAMEBASE_SIZE(
					frm->frame.b_frame_interval_type);
		}
	} break;
...
static ssize_t
uvcg_default_streaming_b_interface_number_show(struct config_item *item,
					       char *page)
{
	struct config_group *group = to_config_group(item);
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;
...
#define UVCG_OPTS_ATTR(cname, aname, limit)                                    \
	static ssize_t f_uvc_opts_##cname##_show(struct config_item *item,     \
						 char *page)                   \
	{                                                                      \
		struct f_uvc_opts *opts = to_f_uvc_opts(item);                 \
		int result;                                                    \
		...
		return result;                                                 \
                                                                               ...
                                                                               \
	static ssize_t f_uvc_opts_##cname##_store(                             \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct f_uvc_opts *opts = to_f_uvc_opts(item);                 \
		unsigned int num;                                              \
...
UVCG_OPTS_ATTR(bulk_streaming_ep, bulk_streaming_ep, 1);
#endif

#undef UVCG_OPTS_ATTR

#define UVCG_OPTS_STRING_ATTR(cname, aname)                                    \
	static ssize_t f_uvc_opts_string_##cname##_show(                       \
		struct config_item *item, char *page)                          \
	{                                                                      \
		struct f_uvc_opts *opts = to_f_uvc_opts(item);                 \
		int ret;                                                       \
		...
		return ret;                                                    \
                                                                               ...
                                                                               \
	static ssize_t f_uvc_opts_string_##cname##_store(                      \
		struct config_item *item, const char *page, size_t len)        \
	{                                                                      \
		struct f_uvc_opts *opts = to_f_uvc_opts(item);                 \
		int ret;                                                       \
		...
		if (opts->refcnt) {                                            \
			...
			goto end;                                              \
                                                                               ...
                                                                               \
		ret = snprintf(opts->cname, 32, "%s", page);                   \
	end:                                                                   \
		mutex_unlock(&opts->lock);                                     \
		return ret;                                                    \
	}                                                                      \
                                                                               \
	UVC_ATTR(f_uvc_opts_string_, cname, cname)

#ifndef CONFIG_SS_GADGET_UVC_MULTI_STREAM
UVCG_OPTS_STRING_ATTR(streaming_name, streaming_name);
#endif
...
static struct configfs_attribute *uvc_attrs[] = {
	...
	&f_uvc_opts_string_attr_streaming_name,
	&f_uvc_opts_attr_bulk_streaming_ep,
#endif
	NULL,
};
