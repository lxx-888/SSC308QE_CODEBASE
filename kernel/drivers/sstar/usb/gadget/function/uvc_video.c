...
static unsigned int hdr_err_seq_cnt = 0;
module_param(hdr_err_seq_cnt, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(
	hdr_err_seq_cnt,
	"When meet miss-isoc, header is set continuous err bit to reflect abnormal frame");
#endif
/* --------------------------------------------------------------------------
 * Video codecs
 */
...
#ifdef CONFIG_UVC_STREAM_ERR_SUPPORT
		if (video->queue.xfer_flag != FLAG_UVC_XFER_OK) {
			if (video->queue.xfer_flag & FLAG_UVC_XFER_ERR) {
				// errors occur in transiting and the last payload is prepared here
				data[1] |= UVC_STREAM_ERR;
				video->queue.xfer_flag &= ~FLAG_UVC_XFER_ERR;
				uvc->video.cur_hdr_err_seq_cnt =
					hdr_err_seq_cnt;
			} else if (buf->buf.flags & V4L2_BUF_FLAG_KEYFRAME) {
				// for H.264/H.265, the requested key frame is going to queue to ep
				video->queue.xfer_flag &=
					~FLAG_UVC_WAIT_KEYFRAME;
			} else {
				// for H.264/H.265, keep setting ERR bit to notify host to drop the frame
				data[1] |= UVC_STREAM_ERR;
			}
		}

		if (uvc->video.cur_hdr_err_seq_cnt) {
			data[1] |= UVC_STREAM_ERR;
			uvc->video.cur_hdr_err_seq_cnt--;
		}
#endif
...
static void uvc_video_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case -EAGAIN: /* uncompleted request, resend again */
#ifdef CONFIG_UVC_STREAM_ERR_SUPPORT
		if (queue->use_sg && ureq)
			hdr = &ureq->header[1];
		else if (req->buf)
			hdr = (u8 *)(req->buf + 1);

		spin_lock_irqsave(&queue->irqlock, flags);
		if (!(queue->xfer_flag & FLAG_UVC_XFER_ERR)) {
			queue->xfer_flag = FLAG_UVC_XFER_ERR;

			if ((video->fcc == V4L2_PIX_FMT_H264) ||
			    (video->fcc == V4L2_PIX_FMT_HEVC))
				/* flag to send forceIDR event and wait a key frame queued */
				queue->xfer_flag |= FLAG_UVC_EVENT_KEYFRAME |
						    FLAG_UVC_WAIT_KEYFRAME;
		}
		/* set ERR bit to notify host to drop the whole frame */
		if (hdr && *hdr & UVC_STREAM_EOF) {
			*hdr |= UVC_STREAM_ERR;
			queue->xfer_flag &= ~FLAG_UVC_XFER_ERR;
		}
		spin_unlock_irqrestore(&queue->irqlock, flags);
...
static void uvcg_video_pump(struct work_struct *work)
{
	...
	while (video->ep->enabled) {
		...
		if (queue->xfer_flag & FLAG_UVC_EVENT_KEYFRAME) {
...
#ifdef CONFIG_UVC_STREAM_ERR_SUPPORT
		video->queue.xfer_flag = FLAG_UVC_XFER_OK;
#endif
		return 0;
	}
