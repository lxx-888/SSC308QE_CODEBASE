...
unsigned int MI_AI_SRATE_SUPPORT_LIST[MI_AI_SRATE_SUPPORT_NUM] = { 8000, 16000,
								   32000,
								   48000 };
EXPORT_SYMBOL_GPL(MI_AI_SRATE_SUPPORT_LIST);
#endif
...
static void u_audio_iso_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		 ...
		 */
		pitched_rate_mil =
			(unsigned long long)params->p_srate * prm->pitch;
		div_result = pitched_rate_mil;
		do_div(div_result, uac->p_interval_mil);
		frames = (unsigned int)div_result;

		pr_debug("p_srate %d, pitch %d, interval_mil %llu, frames %d\n",
			 params->p_srate, prm->pitch, uac->p_interval_mil,
			 frames);

		p_pktsize = min_t(unsigned int, uac->p_framesize *frames,
				  ep->maxpacket);

		if (p_pktsize < ep->maxpacket) {
			residue_frames_mil =
				pitched_rate_mil - frames * uac->p_interval_mil;
			p_pktsize_residue_mil =
				uac->p_framesize * residue_frames_mil;
		} else
		...
		if ((unsigned int)div_result >= uac->p_framesize) {
			...
			uac->p_residue_mil -=
				uac->p_framesize * uac->p_interval_mil;
				       ...
				       struct usb_request *req)
{
	...
	u_audio_set_fback_frequency(audio_dev->gadget->speed, audio_dev->out_ep,
				    params->c_srate, prm->pitch, req->buf);
...
static int uac_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
#ifdef SUPPORT_MULTI_PSRATE
		runtime->hw.rate_min = MI_AI_SRATE_SUPPORT_LIST[0];
		runtime->hw.rate_max =
			MI_AI_SRATE_SUPPORT_LIST[MI_AI_SRATE_SUPPORT_NUM - 1];
#else
		...
		runtime->hw.period_bytes_min =
			2 * uac->p_prm.max_psize / runtime->hw.periods_min;
	} else {
		...
		runtime->hw.period_bytes_min =
			2 * uac->c_prm.max_psize / runtime->hw.periods_min;
	}
...
int u_audio_start_playback(struct g_audio *audio_dev)
{
	...
	uac->p_framesize = params->p_ssize * num_channels(params->p_chmask);
	p_interval = factor / (1 << (ep_desc->bInterval - 1));
	uac->p_interval_mil = (unsigned long long)p_interval * 1000000;
	p_pktsize = min_t(unsigned int,
			  uac->p_framesize *(uac->p_prm.rate / p_interval),
			  ep->maxpacket);
...
int u_audio_set_rate(struct g_audio *audio_dev, int playback, u16 val)
{
	struct snd_uac_chip *uac = audio_dev->uac;
	struct uac_rtd_params *prm;
	unsigned long flags;
	...
	snd_ctl_notify(uac->card, SNDRV_CTL_EVENT_MASK_VALUE,
		       &prm->snd_kctl_rate->id);

	return 0;
}
EXPORT_SYMBOL_GPL(u_audio_set_rate);
			     ...
			     struct snd_ctl_elem_info *uinfo)
{
	...
	uinfo->count = 1;
	uinfo->value.integer.min = 0;
	uinfo->value.integer.max = prm->rate_max;
	uinfo->value.integer.step = 1;
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	spin_lock_irqsave(&prm->lock, flags);
	ucontrol->value.integer.value[0] = prm->rate;
	spin_unlock_irqrestore(&prm->lock, flags);
...
static struct snd_kcontrol_new u_audio_controls[] = {
	[UAC_FBACK_CTRL] {
		...
		.name = "Capture Pitch 1000000", .info = u_audio_pitch_info,
		.get = u_audio_pitch_get, .put = u_audio_pitch_put,
	},
	[UAC_P_PITCH_CTRL] {
		.iface = SNDRV_CTL_ELEM_IFACE_PCM,
		.name = "Playback Pitch 1000000", .info = u_audio_pitch_info,
		.get = u_audio_pitch_get, .put = u_audio_pitch_put,
	},
	[UAC_MUTE_CTRL] {
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "", /* will be filled later */
			.info = u_audio_mute_info, .get = u_audio_mute_get,
		.put = u_audio_mute_put,
	},
	[UAC_VOLUME_CTRL] {
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "", /* will be filled later */
			.info = u_audio_volume_info, .get = u_audio_volume_get,
		.put = u_audio_volume_put,
	},
	[UAC_RATE_CTRL] {
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "", /* will be filled later */
			.info = u_audio_rate_info, .get = u_audio_rate_get,
		.put = NULL,
	},
};
		  ...
		  const char *card_name)
{
	...
	err = snd_card_new(&g_audio->gadget->dev, -1, NULL, THIS_MODULE, 0,
			   &card);
	if (err < 0)
		goto fail;
	...
	if ((c_chmask && g_audio->in_ep_fback) ||
	    (p_chmask && params->p_fu.id) || (c_chmask && params->c_fu.id))
	...
	for (i = 0; i <= SNDRV_PCM_STREAM_LAST; i++) {
		...
		snprintf(ctrl_name, sizeof(ctrl_name), "PCM %s Rate",
			 direction);
		...
		if (i == SNDRV_PCM_STREAM_PLAYBACK)
			prm->rate = prm->rate_max = prm->rate_min =
				params->p_srate;
		else
			prm->rate = prm->rate_max = prm->rate_min =
				params->c_srate;
	...
	snd_pcm_set_managed_buffer_all(pcm, SNDRV_DMA_TYPE_CONTINUOUS, NULL, 0,
				       BUFF_SIZE_MAX);
