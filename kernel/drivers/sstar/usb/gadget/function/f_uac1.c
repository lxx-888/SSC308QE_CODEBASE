...
static int interrupt_ep_enable = 0;
...
#define FUIN_EN(_opts) ((_opts)->p_mute_present || (_opts)->p_volume_present)
#define FUOUT_EN(_opts) ((_opts)->c_mute_present || (_opts)->c_volume_present)
 ...
 */
static struct usb_interface_assoc_descriptor audio_iad_descriptor = {
...
#ifdef SUPPORT_MULTI_PSRATE
static struct usb_interface_descriptor as_in_interface_alt_1_desc[MI_AI_SRATE_SUPPORT_NUM] = {
	[0] = {
	.bLength = USB_DT_INTERFACE_SIZE,
	.bDescriptorType = USB_DT_INTERFACE,
	.bAlternateSetting = 1,
...
#ifdef SUPPORT_MULTI_PSRATE
static struct uac_format_type_i_discrete_descriptor_1 as_in_type_i_desc[MI_AI_SRATE_SUPPORT_NUM] = {
	[0] = {
	.bLength = UAC_FORMAT_TYPE_I_DISCRETE_DESC_SIZE(1),
	.bDescriptorType = USB_DT_CS_INTERFACE,
	.bDescriptorSubtype = UAC_FORMAT_TYPE,
...
static struct usb_endpoint_descriptor as_in_ep_desc[MI_AI_SRATE_SUPPORT_NUM] = {
	[0] = {
	.bLength = USB_DT_ENDPOINT_AUDIO_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_IN,
...
static struct usb_ss_ep_comp_descriptor as_in_ep_comp_desc[MI_AI_SRATE_SUPPORT_NUM] = {
	[0] = {
	.bLength = USB_DT_SS_EP_COMP_SIZE,
	.bDescriptorType = USB_DT_SS_ENDPOINT_COMP,
	},
...
static struct usb_descriptor_header *f_audio_desc[50] = {
...
static struct usb_descriptor_header *f_audio_ss_desc[50] = {
	(struct usb_descriptor_header *)&audio_iad_descriptor,
	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc,
...
static int in_rq_min(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	struct usb_request *req = fn->config->cdev->req;
	struct g_audio *audio = func_to_g_audio(fn);
	struct f_uac1_opts *opts = g_audio_to_uac1_opts(audio);
...
static int in_rq_max(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	struct usb_request *req = fn->config->cdev->req;
	struct g_audio *audio = func_to_g_audio(fn);
	struct f_uac1_opts *opts = g_audio_to_uac1_opts(audio);
...
static int in_rq_res(struct usb_function *fn, const struct usb_ctrlrequest *cr)
{
	struct usb_request *req = fn->config->cdev->req;
	struct g_audio *audio = func_to_g_audio(fn);
	struct f_uac1_opts *opts = g_audio_to_uac1_opts(audio);
...
static void out_rq_cur_complete(struct usb_ep *ep, struct usb_request *req)
{
	struct g_audio *audio = req->context;
...
static int ac_rq_in(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
...
static int f_audio_setup(struct usb_function *f,
			 const struct usb_ctrlrequest *ctrl)
{
	struct usb_composite_dev *cdev = f->config->cdev;
...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == uac1->as_out_intf) {
		if (alt && uac1->as_out_alt == alt) {
			printk(KERN_DEBUG "force reset intf %d alt %d", intf,
			       alt);
		...
		u_audio_set_rate(&uac1->g_audio, 0, alt ? opts->c_srate : 0);
	} else if (intf == uac1->as_in_intf) {
		if (alt && uac1->as_in_alt == alt) {
			printk(KERN_DEBUG "force reset intf %d alt %d", intf,
			       alt);
...
#ifdef SUPPORT_MULTI_PSRATE
		{
			ret = usb_ep_disable(uac1->g_audio.in_ep);
			ret = config_ep_by_endp_desc(
				f->config->cdev->gadget,
				&as_in_ep_desc[alt - 1],
				&as_in_ep_comp_desc[alt - 1],
				uac1->g_audio.in_ep);
		}
#else
			...
			u_audio_stop_playback(&uac1->g_audio);
#ifdef SUPPORT_MULTI_PSRATE
		u_audio_set_rate(&uac1->g_audio, 1,
				 alt ? MI_AI_SRATE_SUPPORT_LIST[alt - 1] : 0);
		if (alt)
			ret = u_audio_start_playback(&uac1->g_audio);
#else
		u_audio_set_rate(&uac1->g_audio, 1, alt ? opts->p_srate : 0);
#endif
	} else {
		dev_err(dev, "%s:%d Error!\n", __func__, __LINE__);
		return -EINVAL;
...
/* B.3.2  Class-Specific AC Interface Descriptor */
static struct uac1_ac_header_descriptor *
build_ac_header_desc(struct f_uac1_opts *opts)
{
...
static void setup_descriptor(struct usb_gadget *gadget,
			     struct f_uac1_opts *opts)
{
	...
	for (alt = 0; alt < MI_AI_SRATE_SUPPORT_NUM; alt++) {
		int packet_size = MI_AI_SRATE_SUPPORT_LIST[alt] *
				  opts->p_ssize * num_channels(opts->p_chmask) *
				  (1 << (as_in_ep_desc[0].bInterval - 1)) /
				  8000;

		if (packet_size <= opts->p_mpsize) {
			memcpy(&as_in_interface_alt_1_desc[alt],
			       &as_in_interface_alt_1_desc[0],
			       sizeof(as_in_interface_alt_1_desc[0]));
			memcpy(&as_in_ep_desc[alt], &as_in_ep_desc[0],
			       sizeof(as_in_ep_desc[0]));
			memcpy(&as_in_ep_comp_desc[alt], &as_in_ep_comp_desc[0],
			       sizeof(as_in_ep_comp_desc[0]));
			memcpy(&as_in_type_i_desc[alt], &as_in_type_i_desc[0],
			       sizeof(as_in_type_i_desc[0]));

			as_in_interface_alt_1_desc[alt].bAlternateSetting =
				alt + 1;
			as_in_ep_comp_desc[alt].wBytesPerInterval =
				as_in_ep_desc[alt].wMaxPacketSize = packet_size;
			memcpy(as_in_type_i_desc[alt].tSamFreq[0],
			       &MI_AI_SRATE_SUPPORT_LIST[alt], 3);
		}
	}
#endif
	if (EPOUT_EN(opts))
	...
	i = 0;
	f_audio_desc[i++] = USBDHDR(&audio_iad_descriptor);
	...
	if ((FUOUT_EN(opts) || FUIN_EN(opts)) && interrupt_ep_enable)
	...
	if (EPIN_EN(opts)) {
		...
		for (alt = 0; alt < MI_AI_SRATE_SUPPORT_NUM; alt++) {
			f_audio_desc[i++] =
				USBDHDR(&as_in_interface_alt_1_desc[alt]);
			f_audio_desc[i++] = USBDHDR(&as_in_header_desc);
			f_audio_desc[i++] = USBDHDR(&as_in_type_i_desc[alt]);
			f_audio_desc[i++] = USBDHDR(&as_in_ep_desc[alt]);
	...
	if (gadget_is_superspeed(gadget)) {
		i = 0;
		f_audio_ss_desc[i++] = USBDHDR(&audio_iad_descriptor);
		f_audio_ss_desc[i++] = USBDHDR(&ac_interface_desc);
		f_audio_ss_desc[i++] = USBDHDR(ac_header_desc);

		if (EPOUT_EN(opts)) {
			f_audio_ss_desc[i++] = USBDHDR(&usb_out_it_desc);
			f_audio_ss_desc[i++] = USBDHDR(&io_out_ot_desc);
			if (FUOUT_EN(opts))
				f_audio_ss_desc[i++] =
					USBDHDR(out_feature_unit_desc);
		}

		if (EPIN_EN(opts)) {
			f_audio_ss_desc[i++] = USBDHDR(&io_in_it_desc);
			f_audio_ss_desc[i++] = USBDHDR(&usb_in_ot_desc);
			if (FUIN_EN(opts))
				f_audio_ss_desc[i++] =
					USBDHDR(in_feature_unit_desc);
		}

		if ((FUOUT_EN(opts) || FUIN_EN(opts)) && interrupt_ep_enable)
			f_audio_ss_desc[i++] = USBDHDR(&ac_int_ep_desc);

		if (EPOUT_EN(opts)) {
			f_audio_ss_desc[i++] =
				USBDHDR(&as_out_interface_alt_0_desc);
			f_audio_ss_desc[i++] =
				USBDHDR(&as_out_interface_alt_1_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_out_header_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_out_type_i_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_out_ep_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_out_ep_comp_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_iso_out_desc);
		}
		if (EPIN_EN(opts)) {
			f_audio_ss_desc[i++] =
				USBDHDR(&as_in_interface_alt_0_desc);
#ifdef SUPPORT_MULTI_PSRATE
			for (alt = 0; alt < MI_AI_SRATE_SUPPORT_NUM; alt++) {
				f_audio_ss_desc[i++] = USBDHDR(
					&as_in_interface_alt_1_desc[alt]);
				f_audio_ss_desc[i++] =
					USBDHDR(&as_in_header_desc);
				f_audio_ss_desc[i++] =
					USBDHDR(&as_in_type_i_desc[alt]);
				f_audio_ss_desc[i++] =
					USBDHDR(&as_in_ep_desc[alt]);
				f_audio_ss_desc[i++] =
					USBDHDR(&as_in_ep_comp_desc[alt]);
				f_audio_ss_desc[i++] = USBDHDR(&as_iso_in_desc);
			}
#else
			f_audio_ss_desc[i++] =
				USBDHDR(&as_in_interface_alt_1_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_in_header_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_in_type_i_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_in_ep_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_in_ep_comp_desc);
			f_audio_ss_desc[i++] = USBDHDR(&as_iso_in_desc);
#endif
		}
		f_audio_ss_desc[i] = NULL;
	}
}
...
static int f_audio_validate_opts(struct g_audio *audio, struct device *dev)
{
	...
	if (opts->p_volume_max <= opts->p_volume_min) {
		...
		dev_err(dev,
			"Error: negative/zero playback volume resolution\n");
		return -EINVAL;
	} else if (opts->c_volume_res <= 0) {
		dev_err(dev,
			"Error: negative/zero capture volume resolution\n");
		return -EINVAL;
	}
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	 ...
	 */
	audio_opts->c_mpsize =
		audio_opts->c_mpsize > 1024 ? 1024 : audio_opts->c_mpsize;
	audio_opts->p_mpsize =
		audio_opts->p_mpsize > 1024 ? 1024 : audio_opts->p_mpsize;

	as_out_ep_desc.wMaxPacketSize = cpu_to_le16(audio_opts->c_mpsize);
#ifdef SUPPORT_MULTI_PSRATE
	as_in_ep_desc[0].wMaxPacketSize = cpu_to_le16(audio_opts->p_mpsize);
#else
	as_in_ep_desc.wMaxPacketSize = cpu_to_le16(audio_opts->p_mpsize);
#endif
	/* super-speed */
	if (gadget_is_superspeed(gadget)) {
		as_out_ep_comp_desc.bmAttributes = cpu_to_le16(0);
		as_out_ep_comp_desc.bMaxBurst = cpu_to_le16(0);
		as_out_ep_comp_desc.wBytesPerInterval =
			cpu_to_le16(as_out_ep_desc.wMaxPacketSize);
#ifdef SUPPORT_MULTI_PSRATE
		as_in_ep_comp_desc[0].bmAttributes = cpu_to_le16(0);
		as_in_ep_comp_desc[0].bMaxBurst = cpu_to_le16(0);
		as_in_ep_comp_desc[0].wBytesPerInterval =
			cpu_to_le16(as_in_ep_desc[0].wMaxPacketSize);
#else
		as_in_ep_comp_desc.bmAttributes = cpu_to_le16(0);
		as_in_ep_comp_desc.bMaxBurst = cpu_to_le16(0);
		as_in_ep_comp_desc.wBytesPerInterval =
			cpu_to_le16(as_in_ep_desc.wMaxPacketSize);
#endif
	}
...
#ifdef SUPPORT_MULTI_PSRATE
	as_in_interface_alt_1_desc[0].iInterface = us[STR_AS_IN_IF_ALT1].id;
#else
...
#ifdef SUPPORT_MULTI_PSRATE
	as_in_type_i_desc[0].bNrChannels = num_channels(audio_opts->p_chmask);
	as_in_type_i_desc[0].bSubframeSize = audio_opts->p_ssize;
	as_in_type_i_desc[0].bBitResolution = audio_opts->p_ssize * 8;
	...
	audio_iad_descriptor.bInterfaceCount = 1;
	...
	if (EPOUT_EN(audio_opts)) {
		...
		if (status < 0)
			goto err_free_fu;
		audio_iad_descriptor.bInterfaceCount++;
...
#ifdef SUPPORT_MULTI_PSRATE
		as_in_interface_alt_1_desc[0].bInterfaceNumber = status;
#else
	...
	if (EPIN_EN(audio_opts)) {
#ifdef SUPPORT_MULTI_PSRATE
		ep = usb_ep_autoconfig(cdev->gadget, &as_in_ep_desc[0]);
#else
		...
		audio->in_ep = ep;
#ifdef SUPPORT_MULTI_PSRATE
		audio->in_ep->desc = &as_in_ep_desc[0];
#else
		audio->in_ep->desc = &as_in_ep_desc;
#endif
	}

	setup_descriptor(gadget, audio_opts);
...
#ifdef SUPPORT_MULTI_PSRATE
	audio->in_ep_maxpsize = audio_opts->p_mpsize;
#else
