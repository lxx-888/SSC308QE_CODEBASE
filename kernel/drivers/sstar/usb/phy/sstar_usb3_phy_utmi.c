/*
 * sstar_usb3_phy_utmi.c- Sigmastar
 *
...
#include "sstar_usb3_phy_utmi_reg.h"
#include <linux/regmap.h>
#include <linux/mfd/syscon.h>
#include "sstar_usb3_phy_utmi_debugfs.h"
...
extern int  sstar_phy_edswitch_creat(struct device_node *node, struct sstar_phy_port *port);
...
static int sstar_utmi_init(struct phy *phy)
{
    ...
    struct regmap *     utmi0;
    node = of_parse_phandle(phy->dev.of_node, "syscon-extra-utmi", 0);
    if (node)
    {
        utmi0 = syscon_node_to_regmap(node);
        if (IS_ERR(utmi0))
        {
        ...
        {
            regmap_update_bits(utmi0, (0x0 << 2), BIT(1), 0x01);
            regmap_clear_bits(utmi0, (0x0 << 2), BIT(15));
            dev_info(&phy->dev, "Extra utmi setting\n");
        }
        of_node_put(node);
    }
