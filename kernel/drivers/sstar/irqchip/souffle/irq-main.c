...
static void sstar_main_irq_ack(struct irq_data *d)
{
    ...
    {
        SETREG16_BIT_OP((BASE_REG_INTRCTL_PA + REG_ID_4C + (sstar_fiq / 16) * 4), (1 << (sstar_fiq % 16)));
...
static void sstar_main_irq_mask(struct irq_data *d)
{
    ...
    {
        if (sstar_irq >= 0 && sstar_irq < INT_IRQ_BANK2_BASE)
        {
        ...
        {
            SETREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_54 + ((sstar_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                            (1 << ((sstar_irq - INT_IRQ_BANK2_BASE) % 16)));
            INREG16(BASE_REG_MAILBOX_PA); // read a register make ensure the previous write command was compeleted
        }
    }
    else
...
static void sstar_main_irq_unmask(struct irq_data *d)
{
    ...
    else if (sstar_irq >= 0 && sstar_irq < GIC_SPI_MS_IRQ_NR)
    {
        ...
        {
            CLRREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_54 + ((sstar_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                            (1 << ((sstar_irq - INT_IRQ_BANK2_BASE) % 16)));
            INREG16(BASE_REG_MAILBOX_PA); // read a register make ensure the previous write command was compeleted
        }
    }
...
static int sstar_main_irq_set_type(struct irq_data *data, unsigned int flow_type)
{
    ...
    {
        if (sstar_irq >= 0 && sstar_irq < INT_IRQ_BANK2_BASE) // IRQ 0~31 mapping to int_crtl (1009)
        {
        ...
        else // IRQ 64~95 Mapping to int_crtl2 (new bank 1015)
        {
            if (flow_type & IRQ_TYPE_LEVEL_LOW)
                SETREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_58 + ((sstar_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                                (1 << ((sstar_irq - INT_IRQ_BANK2_BASE) % 16)));
            else
                CLRREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_58 + ((sstar_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                                (1 << ((sstar_irq - INT_IRQ_BANK2_BASE) % 16)));
        }
    }
    else
...
static int sstar_irqchip_suspend(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        if (i < 4) // INTRCTL0
        {
            sstar_main_irq_priv.irq_mask[i]     = INREG16(BASE_REG_INTRCTL_PA + BK_REG(0x54 + i));
            sstar_main_irq_priv.irq_polarity[i] = INREG16(BASE_REG_INTRCTL_PA + BK_REG(0x58 + i));
        }
        else
        {
            sstar_main_irq_priv.irq_mask[i]     = INREG16(BASE_REG_INTRCTL2_PA + BK_REG(0x54 + (i - 4)));
            sstar_main_irq_priv.irq_polarity[i] = INREG16(BASE_REG_INTRCTL2_PA + BK_REG(0x58 + (i - 4)));
        }
    }
...
static void sstar_irqchip_resume(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        if (i < 4)
        {
            OUTREG16(BASE_REG_INTRCTL_PA + BK_REG(0x54 + i), sstar_main_irq_priv.irq_mask[i]);
            OUTREG16(BASE_REG_INTRCTL_PA + BK_REG(0x58 + i), sstar_main_irq_priv.irq_polarity[i]);
        }
        else
        {
            OUTREG16(BASE_REG_INTRCTL2_PA + BK_REG(0x54 + (i - 4)), sstar_main_irq_priv.irq_mask[i]);
            OUTREG16(BASE_REG_INTRCTL2_PA + BK_REG(0x58 + (i - 4)), sstar_main_irq_priv.irq_polarity[i]);
        }
    }
