...
#define HAL_I2C_DMA_TSDONE_CLR (10000)
#define HAL_I2C_POLL_RETRY     (10000000)
#define HAL_I2C_GARB_BUS       (2)
...
static s32 hal_i2c_intr_clr(struct hal_i2c_ctrl *para_hal_ctrl)
{
    ...
    count    = HAL_I2C_READ_REG(HAL_I2C_REG_CNT_HIGH_PERIOD) * 4;
    time_out = count * 1000000 / para_hal_ctrl->match_rate;
    time_out++;
...
static s32 hal_i2c_dma_transfer(struct hal_i2c_ctrl *para_hal_ctrl)
{
    ...
    {
        hal_i2c_dma_trigger(para_hal_ctrl);
        while (poll_count--)
        {
...
static s32 hal_i2c_send_byte(struct hal_i2c_ctrl *para_hal_ctrl, const u8 para_val)
{
    ...
    count = (HAL_I2C_READ_REG(HAL_I2C_REG_CNT_HIGH_PERIOD) + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_LOW_PERIOD)
             + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_BTWN_FALEDGE) + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_DATA_LATCHTIM))
            * 100;
    time_out = (count * 1000000) / para_hal_ctrl->match_rate;
    time_out++;
    for (retry = 0; retry < time_out; retry++)
...
static void hal_i2c_reset_garb(struct hal_i2c_ctrl *para_hal_ctrl, bool cmd)
{
    if (para_hal_ctrl->group == HAL_I2C_GARB_BUS)
    {
        hal_i2c_dma_miu_reset(para_hal_ctrl, 1);

        if (cmd)
            sstar_miu_module_reset_byid(MIU_CLIENTW_MIIC, 1);
        else
            sstar_miu_module_reset_byid(MIU_CLIENTR_MIIC, 0);

        hal_i2c_dma_miu_reset(para_hal_ctrl, 0);
    }
}
...
static s32 hal_i2c_dma_write(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len)
{
    ...
    // set slave addr
    ret |= hal_i2c_dma_slvadr_set(para_hal_ctrl,
                                  para_slvadr); // set dma slave address
    // set cmd data
    ...
    hal_i2c_cache_flush(dma_virt_addr, hal_i2c_cache_line_size_align(para_len));
    ...
    if (ret)
    {
        ...
        hal_i2c_reset_garb(para_hal_ctrl, 1);
                             ...
                             u16 para_flag)
{
    ...
    hal_i2c_intr_clr(para_hal_ctrl);
    if (!(para_flag & HAL_I2C_NO_START))
    {
        // 1.send start
        ret |= hal_i2c_start_bit_send(para_hal_ctrl);
        count = (HAL_I2C_READ_REG(HAL_I2C_REG_CNT_HIGH_PERIOD) + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_LOW_PERIOD)
                 + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_FOR_START) + HAL_I2C_READ_REG(HAL_I2C_REG_CNT_TSU_SET))
                * 50;
        time_out = (count * 1000000) / para_hal_ctrl->match_rate;
        time_out++;
        for (retry = 0; retry < time_out; retry++)
...
static s32 hal_i2c_dma_read(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len)
{
    ...
    if (ret)
    {
        ...
        hal_i2c_reset_garb(para_hal_ctrl, 0);
        return -HAL_I2C_READ;
    }
    ...
    hal_i2c_cache_invalidate(dma_virt_addr, hal_i2c_cache_line_size_align(para_len));
...
#if defined(CONFIG_SSTAR_PM_DMA)
    HAL_I2C_WRITE_WORD_MASK(
        (IO_ADDRESS(BASE_REG_RIU_PA) + (HAL_I2C_PM_RAM_ACCESS_BANK << 0x9) + (HAL_I2C_PM_RAM_ACCESS_OFFSET << 0x2)),
        HAL_I2C_PM_RAM_ACCESS_VAL, HAL_I2C_PM_RAM_ACCESS_MASK);
#endif
...
static u32 hal_i2c_ns_to_cnt(struct hal_i2c_ctrl *para_hal_ctrl, u32 para_ns)
{
    u32 cnt, ns_per_cnt = (1000 / para_hal_ctrl->clkcnt_per_us);
    cnt = para_ns * para_hal_ctrl->clkcnt_per_us / 1000 + !!(para_ns % ns_per_cnt > (ns_per_cnt / 2));
    if (para_ns && !cnt)
    {
        cnt = 1;
    }
...
static s32 hal_i2c_count_calc(struct hal_i2c_ctrl *para_hal_ctrl, u32 para_src_clk,
                              enum hal_i2c_speed_mode para_speed_mode)
{
    ...
    switch (para_speed_mode)
    {
        case HAL_I2C_SM:
            ...
            cnt_data_hd  = HAL_I2C_SM_CNT_THDDAT;
            cnt_start_su = HAL_I2C_SM_CNT_TSUSTA;
            cnt_start_hd = HAL_I2C_SM_CNT_THDSTA;
            cnt_stop_su  = HAL_I2C_SM_CNT_TSUSTO;
            cnt_stop_hd  = HAL_I2C_SM_CNT_THDSTO;
            if (cnt_scl_low < (cnt_data_hd + HAL_I2C_SM_CNT_TSUDAT_MIN))
            {
                cnt_scl_low  = cnt_data_hd + HAL_I2C_SM_CNT_TSUDAT_MIN;
                cnt_scl_high = cnt_perid - cnt_scl_low;
            }
            break;
        case HAL_I2C_FM:
            cnt_perid -= HAL_I2C_CNT_DEC_54M;
            cnt_scl_high = (cnt_perid * 9 / 25);
            cnt_scl_low  = cnt_perid - cnt_scl_high;
            cnt_start_su = HAL_I2C_FM_CNT_TSUSTA;
            cnt_stop_hd  = HAL_I2C_FM_CNT_THDSTO;
            break;
        case HAL_I2C_FM_PLUS:
        case HAL_I2C_HS:
    ...
    else if (para_hal_ctrl->speed > HAL_I2C_SPEED_100KHZ)
    {
        cnt_start_hd = cnt_perid / (250000 / para_hal_ctrl->speed);
        cnt_stop_su  = cnt_perid / (250000 / para_hal_ctrl->speed);
    }
    cnt_start_su = (0 == cnt_start_su) ? cnt_scl_high : cnt_start_su;
    cnt_stop_hd  = (0 == cnt_stop_hd) ? cnt_scl_high : cnt_stop_hd;
    cnt_data_hd  = (0 == cnt_data_hd) ? (cnt_scl_low / 2) : cnt_data_hd;
    cnt_data_su  = cnt_scl_low - cnt_data_hd;

    // Cal the designated values if the timing is specified
    if (para_hal_ctrl->clock_attr_time.ns_start_setup)
    {
        cnt_start_su = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_start_setup);
        cnt_start_su = (cnt_start_su > HAL_I2C_SUPPLY_TSUSTA) ? (cnt_start_su - HAL_I2C_SUPPLY_TSUSTA) : 1;
    }
    if (para_hal_ctrl->clock_attr_time.ns_start_hold)
    {
        cnt_start_hd = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_start_hold);
        cnt_start_hd = (cnt_start_hd > HAL_I2C_SUPPLY_THDSTA) ? (cnt_start_hd - HAL_I2C_SUPPLY_THDSTA) : 1;
    }
    if (para_hal_ctrl->clock_attr_time.ns_stop_setup)
    {
        cnt_stop_su = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_stop_setup);
        cnt_stop_su = (cnt_stop_su > HAL_I2C_SUPPLY_TSUSTO) ? (cnt_stop_su - HAL_I2C_SUPPLY_TSUSTO) : 1;
    }
    if (para_hal_ctrl->clock_attr_time.ns_stop_hold)
    {
        cnt_stop_hd = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_stop_hold);
    }
    if (para_hal_ctrl->clock_attr_time.ns_data_setup)
    {
        cnt_data_su = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_data_setup);
        cnt_data_su = (cnt_data_su >= cnt_scl_low) ? cnt_scl_low : cnt_data_su;
        cnt_data_hd = cnt_scl_low - cnt_data_su + HAL_I2C_SUPPLY_TSUDAT;
    }
    else if (para_hal_ctrl->clock_attr_time.ns_data_hold)
    {
        cnt_data_hd = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_data_hold);
        cnt_data_hd = (cnt_data_hd >= cnt_scl_low) ? cnt_scl_low : cnt_data_hd;
        cnt_data_hd = (cnt_data_hd > HAL_I2C_SUPPLY_THDDAT) ? (cnt_data_hd - HAL_I2C_SUPPLY_THDDAT) : 1;
    }
    if (para_hal_ctrl->clock_attr_time.ns_rck_dly)
    {
        cnt_rck_dly = hal_i2c_ns_to_cnt(para_hal_ctrl, para_hal_ctrl->clock_attr_time.ns_rck_dly);
        cnt_rck_dly = (cnt_rck_dly > HAL_I2C_SUPPLY_RCKDLY) ? (cnt_rck_dly - HAL_I2C_SUPPLY_RCKDLY) : 1;
    }
    ...
    para_hal_ctrl->clock_count.cnt_start_setup = (cnt_start_su >= 0xFFFF) ? 0xFFFF : (u16)cnt_start_su;
    para_hal_ctrl->clock_count.cnt_start_hold  = (cnt_start_hd >= 0xFFFF) ? 0xFFFF : (u16)cnt_start_hd;
    para_hal_ctrl->clock_count.cnt_stop_setup  = (cnt_stop_su >= 0xFFFF) ? 0xFFFF : (u16)cnt_stop_su;
    para_hal_ctrl->clock_count.cnt_stop_hold   = (cnt_stop_hd >= 0xFFFF) ? 0xFFFF : (u16)cnt_stop_hd;
    para_hal_ctrl->clock_count.cnt_sda_hold    = (cnt_data_hd >= 0xFFFF) ? 0xFFFF : (u16)cnt_data_hd;
    para_hal_ctrl->clock_count.cnt_rck_sda_dly = (cnt_rck_dly >= 0xFFFF) ? 0xFFFF : (u16)cnt_rck_dly;
    para_hal_ctrl->clock_count.cnt_rck_scl_dly = (cnt_rck_dly >= 0x7FFF) ? 0xFFFF : (u16)(cnt_rck_dly * 2);

    return 0;
}

static s32 hal_i2c_pushen_disable(u32 group)
{
    switch (group)
    {
        case 0:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL0 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL0);
            break;
        case 1:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL1 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL1);
            break;
        case 2:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL2 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL2);
            break;
        case 3:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL3 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL3);
            break;
        case 4:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL4 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL4);
            break;
        case 5:
            HAL_I2C_WRITE_WORD_MASK(HAL_I2C_BNK_PUSHEN + (HAL_I2C_REG_PUSHEN_SCL5 << 2), 0, HAL_I2C_MASK_PUSHEN_SCL5);
            break;
        default:
            return HAL_I2C_ERR;
    }
...
s32 hal_i2c_cnt_reg_set(struct hal_i2c_ctrl *para_hal_ctrl)
{
    ...
    ret |= hal_i2c_sda_cnt_set(para_hal_ctrl, para_hal_ctrl->clock_count.cnt_sda_hold);
...
s32 hal_i2c_dma_intr_en(struct hal_i2c_ctrl *para_hal_ctrl, u8 para_disenable)
{
    if (para_disenable)
    {
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_CONFIG, (u8)0x00 << HAL_I2C_BIT_SHIFT_2, HAL_I2C_MASK_CFG_ENINT);
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_DMA_CONFIG, para_disenable << HAL_I2C_BIT_SHIFT_2,
                               HAL_I2C_MASK_DMACFG_INTFLAG);
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_CNT_TSUTHD_OPEN, para_disenable << HAL_I2C_BIT_SHIFT_0,
                               HAL_I2C_MASK_INT_BY_PASS);
    }
    else
    {
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_CONFIG, (u8)0x01 << HAL_I2C_BIT_SHIFT_2, HAL_I2C_MASK_CFG_ENINT);
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_DMA_CONFIG, para_disenable << HAL_I2C_BIT_SHIFT_2,
                               HAL_I2C_MASK_DMACFG_INTFLAG);
        HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_CNT_TSUTHD_OPEN, para_disenable << HAL_I2C_BIT_SHIFT_0,
                               HAL_I2C_MASK_INT_BY_PASS);
...
s32 hal_i2c_dma_done_clr(struct hal_i2c_ctrl *para_hal_ctrl, u8 para_val)
{
    ...
    if (ret)
    {
        ...
        for (retry = 1; retry < HAL_I2C_DMA_TSDONE_CLR; retry++)
        {
            if (HAL_I2C_READ_REG(HAL_I2C_REG_DMA_TRANSFER_DONE))
                I2C_DELAY_N_US(1);
            else
                break;
        }
                     ...
                     u16 para_waitcnt)
{
    ...
    if (ret)
    {
        dmsg_i2c_halerr("i2c-%d, trigger wn write failed\n", para_hal_ctrl->group);
        hal_i2c_reset_garb(para_hal_ctrl, 1);
        return -HAL_I2C_WRITE;
    }
...
s32 hal_i2c_dma_async_read(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len)
{
    ...
    if (ret)
    {
        dmsg_i2c_halerr("i2c-%d, trigger dma async read failed\n", para_hal_ctrl->group);
        hal_i2c_reset_garb(para_hal_ctrl, 0);
    }
                           ...
                           u8 para_wnlen, u16 para_waitcnt)
{
    ...
    if (ret)
    {
        dmsg_i2c_halerr("i2c-%d, trigger wn async write failed\n", para_hal_ctrl->group);
        hal_i2c_reset_garb(para_hal_ctrl, 1);
    }
...
s32 hal_i2c_dma_async_write(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len)
{
    ...
    if (ret)
    {
        dmsg_i2c_halerr("i2c-%d, trigger dma async write failed\n", para_hal_ctrl->group);
        hal_i2c_reset_garb(para_hal_ctrl, 1);
    }
...
s32 hal_i2c_write(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len, u16 para_flag)
{
    s32 ret = 0;
    u8  slave_addr;
    ...
    {
        ret |= hal_i2c_riu_write(para_hal_ctrl, slave_addr, para_pdata, para_len, para_flag);
    }
...
s32 hal_i2c_read(struct hal_i2c_ctrl *para_hal_ctrl, u16 para_slvadr, u8 *para_pdata, u32 para_len, u16 para_flag)
{
    s32 ret = 0;
    u8  slave_addr;
    ...
    {
        ret |= hal_i2c_riu_read(para_hal_ctrl, slave_addr, para_pdata, para_len, para_flag);
    }
    if (ret)
    {
        ret = -HAL_I2C_READ;
...
s32 hal_i2c_speed_calc(struct hal_i2c_ctrl *para_hal_ctrl)
{
    ...
    // select source clock
    if (speed > HAL_I2C_MAX_SPEED)
    {
        dmsg_i2c_halerr("i2c-%d limit 1500KHz\n", para_hal_ctrl->group);
        speed = HAL_I2C_MAX_SPEED;
    }
    else if (speed < HAL_I2C_MIN_SPEED)
    {
        dmsg_i2c_halerr("i2c-%d no less than 1KHz\n", para_hal_ctrl->group);
        speed = HAL_I2C_MIN_SPEED;
    }

    para_hal_ctrl->speed = speed;

    hal_i2c_count_calc(para_hal_ctrl, para_hal_ctrl->match_rate, para_hal_ctrl->speed_mode);
...
s32 hal_i2c_init(struct hal_i2c_ctrl *para_hal_ctrl)
{
    ...
    switch (para_hal_ctrl->output_mode)
    {
        case HAL_I2C_OPEN_DRAIN:
            hal_i2c_pushen_disable(para_hal_ctrl->group);
            break;
        case HAL_I2C_OPEN_DRAIN_OEN:
            para_hal_ctrl->config |= HAL_I2C_CONFIG_OEN_PUSH_EN;
            break;
        case HAL_I2C_OPEN_DRAIN_OEN_MULT:
            HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_FORCE_PUSH_PULL, BIT0, HAL_I2C_MASK_MULT_PUSHEN);
            HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_FORCE_PUSH_PULL, (16 << 8), HAL_I2C_MASK_MULT_PUSHEN_NUM);
            break;
        case HAL_I2C_PUSH_PULL:
            HAL_I2C_WRITE_REG_MASK(HAL_I2C_REG_FORCE_PUSH_PULL, BIT1, HAL_I2C_MASK_FORCE_DRIVE_OEN);
            break;
    }
    ...
    // reg cnt default val
    para_hal_ctrl->clock_count.cnt_sda_hold      = 5;
