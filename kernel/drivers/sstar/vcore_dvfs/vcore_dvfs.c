...
static int vcore_dvfs_get_curr_idx(int *idx)
{
    ...
    {
        gpio_values[i] = sstar_gpio_pad_read(vcore_dvfs_info.gpio_pins[i]);
...
static int vcore_dvfs_set_curr_voltage_by_idx(int idx)
{
    ...
    for (i = 0; i < vcore_dvfs_info.gpio_total_num; i++)
    {
        if ((idx >> i) & 0x1)
        {
            sstar_gpio_set_high(vcore_dvfs_info.gpio_pins[i]);
        }
        else
        {
            sstar_gpio_set_low(vcore_dvfs_info.gpio_pins[i]);
        }
    }
        ...
        for (i = 0; i < VCORE_DVFS_DEMANDER_TOTAL; i++)
        {
            for (j = 0; j < rate_voltage_num_table[i]; j++)
            {
                ...
                else if (rate_voltage_all_table[i][j].voltage == VOLTAGE_BY_INIT_1)
                {
                    ...
                    rate_voltage_all_table[i][j].voltage = vcore_dvfs_get_voltage_by_idx(
                        vcore_dvfs_info.voltages_sort_idxs[(vcore_dvfs_info.voltage_init_seq + 1)]);
                }
            }
        }
...
static const struct of_device_id vcore_dvfs_of_match_table[] = {{.compatible = "sstar,vcore_dvfs"}, {}};
...
static ssize_t dbg_rate_vol_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < VCORE_DVFS_DEMANDER_TOTAL; i++)
    {
        ...
        for (j = 0; j < rate_voltage_num_table[i]; j++)
        {
            str += scnprintf(str, end - str, "[%d]rate = %lu\t vol=%d\r\n", j, rate_voltage_all_table[i][j].rate,
                             rate_voltage_all_table[i][j].voltage);
        }
    }
