...
#define CAMFS_CLI_FLASH_CHECK_AVAILABLE FlashIsAvailable
#define CAMFS_CLI_FLASH_ENABLE(a)       FlashAvailableCpuMaskSet(0xF)
...
//#define __VER_CAMFS__
#if defined(__VER_CAMFS__)
static CamFsFd       g_camFSTestFd;
static bool          g_camFSTestOpened;
static s32           g_camFSTestMode;
static unsigned char camfs_buffer[CAMFS_CLI_BUFFER_SIZE + 1] __attribute__((aligned(DATA_CACHE_ENTRY_SIZE)));
...
static s32 CamFs_cli_read(CLI_t *pCli, char *p)
{
    ...
    memset(camfs_buffer, 0, size + 1);
    camfs_buffer[0] = '\n';

    ret = CamFsRead(g_camFSTestFd, camfs_buffer, size);
    ...
    else
    {
        ...
        for (i = 0; i < size; i++)
        {
            CamOsPrintf("%02x", (u32)camfs_buffer[i]);
        }
        CamOsPrintf("\n");
    }
...
static s32 CamFs_cli_write(CLI_t *pCli, char *p)
{
    ...
    if (g_camFSTestMode == 0)
    {
        memcpy(camfs_buffer, content, size);
        ret = CamFsWrite(g_camFSTestFd, camfs_buffer, size);
    ...
    else if (g_camFSTestMode == 1)
    {
        ...
        for (i = 0, j = 0; i < size; i += 2, j++)
        {
            ...
            camfs_buffer[j] = (unsigned char)(hi << 4 | lo);
        }
        ret = CamFsWrite(g_camFSTestFd, camfs_buffer, size / 2);
