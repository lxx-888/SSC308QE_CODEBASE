...
static void hal_uart_force_rx_disable(struct uart_hal *hal, u8 status)
{
    ...
    {
        HAL_UART_WRITE_WORD_MASK(HAL_UART_REG_FORCE_RX_DISABLE, (~status << hal->select), (1 << hal->select));
    }
}
...
void hal_uart_clr_urdma_intr(struct uart_hal *hal)
{
...
static u8 hal_uart_reset(struct uart_hal *hal)
{
    ...
    if (hal->urdma_en)
    {
        ...
        if (hal->rx_timeout)
        {
            HAL_UART_WRITE_WORD(HAL_URDMA_REG_RX_TIMEOUT(hal), hal->rx_timeout);
        }
...
void hal_uart_set_digmux(struct uart_hal *hal)
{
    ...
    for (i = 0; i < HAL_UART_CHIPTOP_SELECT_NUM; i++)
    {
        ...
        if (uart_select_read_val == hal->select)
        {
    ...
    HAL_UART_WRITE_WORD_MASK((uart_select_reg->bank_base + (uart_select_reg->offset << 2)),
                             (hal->select << uart_select_reg->bit_shift), uart_select_reg->mask);
}
...
void hal_uart_init(struct uart_hal *hal)
{
    ...
    if (hal->urdma_en)
    {
        ...
        // mcg off
        HAL_UART_WRITE_WORD_MASK(HAL_URDMA_REG_INTR_THRESHOLD(hal), 0x3000, 0x3000);

        HAL_UART_WRITE_WORD(HAL_URDMA_REG_MIU_ADDR0_SEL(hal), 0x1);

        if (hal->uart_base == HAL_PM_FUART_IO_ADDR)
        {
            HAL_UART_WRITE_BYTE_MASK(HAL_PM_FUART_MIU_ADDR_SEL, 0x1, 0x03);
        }
        else if (hal->uart_base == HAL_PM_FUART1_IO_ADDR)
        {
            HAL_UART_WRITE_BYTE_MASK(HAL_PM_FUART1_MIU_ADDR_SEL, 0x1, 0x03);
        }
    }
...
void hal_uart_deinit(struct uart_hal *hal)
{
    ...
    if (hal->urdma_en)
    {
        HAL_UART_WRITE_WORD(HAL_URDMA_REG_CTRL(hal), HAL_URDMA_SW_RST);
    }
}
...
void hal_uart_config(struct uart_hal *hal)
{
    ...
    if (hal->urdma_en)
    {
        ...
        for (i = 0; ((HAL_URDMA_RX_BUSY | HAL_URDMA_TX_BUSY) & HAL_UART_READ_WORD(HAL_URDMA_REG_CTRL(hal))); i++)
        {
            if (0xFFFF == i)
            {
                printk(KERN_WARNING "urdma busy!!!\n");
                return;
            }
        }
        HAL_UART_WRITE_WORD_MASK(HAL_URDMA_REG_CTRL(hal), 0, HAL_URDMA_DMA_MODE);
    ...
    uart_ier = HAL_UART_READ_BYTE(HAL_UART_REG_DLH_IER(hal));

    HAL_UART_WRITE_BYTE_MASK(HAL_UART_REG_RST(hal), 0, HAL_UART_SW_RST);
    HAL_UART_IMPL_USDELAY(1000);
    HAL_UART_WRITE_BYTE_MASK(HAL_UART_REG_RST(hal), HAL_UART_SW_RST, HAL_UART_SW_RST);
    ...
    HAL_UART_WRITE_BYTE(HAL_UART_REG_DLH_IER(hal), uart_ier);
    if (hal->rtscts_en)
    {
...
void hal_uart_tx_flush(struct uart_hal *hal)
{
    if (hal->urdma_en)
    {
#if HAL_UART_TX_EMPTY_INTERRUPT
        if (hal->tx_empty_int_en)
        {
            while (!hal->tx_empty)
                ;
        }
#else
        while (HAL_UART_READ_WORD(HAL_URDMA_REG_TXBUF_RPTR(hal)) != HAL_UART_READ_WORD(HAL_URDMA_REG_TXBUF_WPTR(hal)))
            ;
#endif
    }
    else
    {
        while ((HAL_UART_READ_BYTE(HAL_UART_REG_LSR(hal)) & HAL_UART_LSR_TEMT) != HAL_UART_LSR_TEMT)
            ;
    }
}
...
u32 hal_uart_get_usr(struct uart_hal *hal)
{
    u16 reg_usr_get    = 0x00;
    u32 reg_usr_status = 0x00;

    reg_usr_get = HAL_UART_READ_BYTE(HAL_UART_REG_USR(hal));
    if (reg_usr_get & HAL_UART_USR_TXFIFO_EMPTY)
    {
        reg_usr_status |= HAL_UART_FIFO_TX_EMPTY;
    }

    return reg_usr_status;
}

u32 hal_uart_change_mode_to_fifo(struct uart_hal *hal)
{
    u32 i = 0;
    /* Note:
     * If you need to receive data at suspend progress,
     * remember enable rx before this api end.
     */
    hal_uart_force_rx_disable(hal, 0);
