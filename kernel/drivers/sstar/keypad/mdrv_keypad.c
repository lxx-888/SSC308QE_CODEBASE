...
static U32 debug_msg = FALSE; // TRUE or FALSE
...
#define MDRV_KEYPAD_ERROR(format, args...) \
    do                                     \
    {                                      \
        printk(format, ##args);            \
    } while (0)
...
#define mdrv_CHECK_TABLE(row, col)                                                            \
    {                                                                                         \
        do                                                                                    \
        {                                                                                     \
...
static ssize_t keypad_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    str += CamOsSnprintf(str, end - str, "row : %hd\n", ddata->data->row);
    str += CamOsSnprintf(str, end - str, "col : %hd\n", ddata->data->col);
    str += CamOsSnprintf(str, end - str, "rawstatus : %hd\n", ddata->data->rawstatus);
    str += CamOsSnprintf(str, end - str, "scan clock : %hd\n", ddata->data->scanclk);
...
static ssize_t keypad_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (NULL != buf)
    {
        ...
        while (1)
        {
            ...
            else if (pName != NULL && strncmp(pName, "scanclk", 7) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                    goto printf_usage;
                ddata->data->scanclk = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
            ...
            else if (pName != NULL && strncmp(pName, "force", 5) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                    goto printf_usage;
                ddata->data->force = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
            ...
            else if (pName != NULL && strncmp(pName, "row", 3) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                {
                ...
                ddata->data->row = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
            ...
            else if (pName != NULL && strncmp(pName, "col", 3) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                {
                ...
                ddata->data->col = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
            ...
            else if (pName != NULL && strncmp(pName, "raw", 3) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                {
                ...
                ddata->data->rawstatus = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
            ...
            else if (pName != NULL && strncmp(pName, "align", 5) == 0)
            {
                p_val = strsep(&pBuf, " ");
                if (NULL == p_val)
                {
                ...
                ddata->data->align = CamOsStrtol(p_val, NULL, 10);
                if (NULL == pBuf)
                {
                    initflag = 1;
                    break;
...
// /sys/bus/platform/devices/1f201400.keypad/   -->P5
// /sys/bus/platform/devices/1f201400.keypad/   -->P3
printf_usage:
    pr_info("keypad_store usage for example:\n");
 ...
 */
static S32 mdrv_keypad_GetPressedkey(struct ms_keypad_date *data, U32 *stagecode, U32 arycnt, U32 type,
                                     U32 s_value[8 * MAX_GROUP])
{
    ...
    MDRV_KEYPAD_DEBUG("\npress  :   %.4x %.4x %.4x %.4x \n", s_value[0 + arycnt * 8], s_value[1 + arycnt * 8],
                      s_value[2 + arycnt * 8], s_value[3 + arycnt * 8]);
    MDRV_KEYPAD_DEBUG("release:   %.4x %.4x %.4x %.4x \n", s_value[4 + arycnt * 8], s_value[5 + arycnt * 8],
                      s_value[6 + arycnt * 8], s_value[7 + arycnt * 8]);
    ...
    for (i = begin; i <= end; i++)
    {
        ...
        {
            tmp = ((s_value[i + arycnt * ONE_GROUP_MEMBER]) >> (REG_ONE_GROUP_MEMBER * j))
                  & 0xf; /* judgment 4bit of register value */
            if (tmp == 0)
                continue;
            *stagecode = i;
            switch (tmp)
...
static void mdrv_keypadreport_event(struct ms_keypad_drvdata *ddata)
{
    ...
    for (i = 0; i < isrcnt; i++)
    {
        ...
        MDRV_KEYPAD_DEBUG("%s %d code=%d type=%d\n", __FUNCTION__, __LINE__, code, type);
        ...
        if (pressed_num <= 4)
        {
            for (key = 1; key <= pressed_num; key++)
            {
                ...
                if (code < 4)
                {
                    ...
                    else if (ddata->data->modesel == E_KP_STATUS_MODE_1 || ddata->data->modesel == E_KP_STATUS_MODE_2)
                    {
                        input_event(input, EV_KEY, keypad_desc[row][col], 1);
                        input_sync(input);
...
S32 mdrv_keypad_init(struct ms_keypad_drvdata *ddata)
{
    ...
    mdrv_keypadMaskAllEN(0, FALSE); // TRUE
    /*select raw status OFFSET:0027*/
    mhal_KeyPad_SetRawSel(ddata->data->rawstatus);

    scanrate = mdrv_keypadcalc_SCANRATE(ddata->data->scanclk);
    ...
    {
        U32 aligncalc = 0;
        aligncalc     = mdrv_keypadcalc_ALIGN(ddata->data->align);
        mhal_KeyPad_KEYPAD_ALIGN_NUM(aligncalc);
    }
    else
    {
    ...
    mdrv_keypadSetForce(0, ddata->data->force);
    // mdrv_keypadSetForce(0, TRUE);
...
static struct ms_keypad_platform_data *mdrv_keypad_get_devtree_pdata(struct device *dev)
{
    ...
    */
    ...
    if (of_property_read_u32(node, "reg", &keypad_io_base))
    {
        dev_err(dev, "Failed to get keypad reg.\n");
    }
    mhal_KeyPad_SetKeypadIOBase(IO_ADDRESS(keypad_io_base));
...
static S32 mdrv_keypad_probe(struct platform_device *pdev)
{
    ...
    ddata->data->scanclk   = KEYPADADJUSTCLK;
    ddata->data->align     = KEYPAD_ALIGNTIMELENGTH;
    ddata->data->force     = 0;
    ddata->data->submode   = 0;
    ddata->data->rawstatus = 0;
...
static int mdrv_keypad_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    if (ddata->pdata->ms_keypadts->keypad_standard == E_KP_STANDARD_MODE_7x7)
    {
        ...
        {
            sstar_gpio_pad_val_set(PAD_KEY7 + i, PINMUX_FOR_GPIO_MODE);
            sstar_gpio_pad_oen(PAD_KEY7 + i);
            sstar_gpio_set_low(PAD_KEY7 + i);
        }
    }
    else if (ddata->pdata->ms_keypadts->keypad_standard == E_KP_STANDARD_MODE_4x4)
    {
        ...
        {
            sstar_gpio_pad_val_set(PAD_KEY4 + i, PINMUX_FOR_GPIO_MODE); //
            sstar_gpio_pad_oen(PAD_KEY4 + i);
            sstar_gpio_set_low(PAD_KEY4 + i);
        }
    }
    else if (ddata->pdata->ms_keypadts->keypad_standard == E_KP_STANDARD_MODE_3x3)
    {
        ...
        {
            sstar_gpio_pad_val_set(PAD_KEY11 + i, PINMUX_FOR_GPIO_MODE);
            sstar_gpio_pad_oen(PAD_KEY11 + i);
            sstar_gpio_set_low(PAD_KEY11 + i);
        }
    }
    return 0;
}
