...
u32 hal_pwm_get_round_cnt(u64 addr, u32 group)
{
...
void hal_pwm_sync_mode_enable(struct hal_pwm_cfg *pwm_cfg, u8 enable)
{
    u32 ret;
...
u64 hal_pwm_get_period_hz(struct hal_pwm_cfg *pwm_cfg)
{
    u32 div;
    u64 period;
    u64 period_reg;
...
u64 hal_pwm_get_duty_pct(struct hal_pwm_cfg *pwm_cfg)
{
    u32 div;
    u64 duty_reg;
    u64 period_reg;
...
u64 hal_pwm_get_shift_pct(struct hal_pwm_cfg *pwm_cfg)
{
    u32 div;
    u64 shift_reg;
    u64 period_reg;
...
void hal_pwm_resume_channel(struct hal_pwm_cfg *pwm_cfg)
{
    ...
    hal_pwm_sync_mode_enable(pwm_cfg, pwm_sync[pwm_cfg->channel]);
...
void hal_pwm_deinit(struct hal_pwm_cfg *pwm_cfg)
{
    ...
    {
        hal_pwm_sync_mode_enable(pwm_cfg, 0);
...
int hal_pwm_init(struct hal_pwm_cfg *pwm_cfg)
{
    ...
    if ((pwm_cfg->group >= HAL_PWM_MAX_GROUP) && (pwm_cfg->group != 0xFFFF))
    {
        pwm_err("pwm group over the maximum\n");
    ...
    if ((pwm_cfg->group >= 0) && (pwm_cfg->group < HAL_PWM_MAX_GROUP))
    {
        ...
        hal_pwm_sync_mode_enable(pwm_cfg, 1);
...
#ifdef CONFIG_SSTAR_PWM_EXTEND
        pwm_cfg->pwm_attr.period_wf =
            (pwm_cfg->pwm_attr.period * pwm_cfg->pwm_attr.divider * pwm_cfg->hz_to_ns) / pwm_cfg->clk_freq;
    ...
    else
    {
        ...
        pwm_cfg->pwm_attr.divider--;
        hal_pwm_set_channel_reset(pwm_cfg, 1);
        /*during initialization, the default value of shift needs to be set to 0 */
        hal_pmw_set_shift(pwm_cfg, 0, 0);
    }
