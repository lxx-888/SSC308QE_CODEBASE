...
//  Implementation
//-------------------------------------------------------------------------------------------------
extern int albany_config_init(void);
...
static int sstar_phy_update_speed_patch(struct phy_device *phydev)
{
    u32 hcd_link_st_ok = 0;
    u32 phy_speed      = 0;
    u32 detect_100m    = 0;
    u32 link_partner   = 0;
    ...
    if (0 == phydev->link)
    {
        NETPHY_PRINT("[%s][%d] link fail! phydev_link:%d\n", __FUNCTION__, __LINE__, phydev->link);
        return 0;
    }

    if (0xffff == (phy_speed = phy_read(phydev, 0x00)))
        return 0;
    if (0xffff == (hcd_link_st_ok = phy_read(phydev, 0x21)))
        return 0;
    if (0xffff == (detect_100m = phy_read(phydev, 0x22)))
        return 0;
    if (0xffff == (link_partner = phy_read(phydev, 0x05)))
        return 0;

    if (!(hcd_link_st_ok & 0x100))
    {
        NETPHY_PRINT("[%s][%d] link fail! hcd_link_st:0x%08x\n", __FUNCTION__, __LINE__, hcd_link_st_ok);
        return 0;
    }

    if (!(phy_speed & 0x2000) && (detect_100m & 0x20)) // in 10M case, but 100M signal
    {
        if (link_partner & 0x180) // link partner ability 100M
        {
            NETPHY_PRINT("[%s][%d] speed change 10 -> 100\n", __FUNCTION__, __LINE__);
            phy_write((phydev), MII_BMCR, 0x1200); // auto negotiation
        }
        else
        {
            // do nothing
    ...
    if (phy_speed & 0x2000)
    {
        ...
        NETPHY_PRINT("[%s] SPEED_100 \n", __FUNCTION__);
    }
    else
    {
        phydev->speed = SPEED_10;
        NETPHY_PRINT("[%s] SPEED_10 \n", __FUNCTION__);
    }

    if (phy_speed & 0x100)
    {
        phydev->duplex = DUPLEX_FULL;
        NETPHY_PRINT("[%s] DUPLEX_FULL \n", __FUNCTION__);
    }
    else
    {
        phydev->duplex = DUPLEX_HALF;
        NETPHY_PRINT("[%s] DUPLEX_HALF \n", __FUNCTION__);
    }
...
static int sstar_phy_read_status(struct phy_device *phydev)
{
    ...
    if ((0 == ret) && (1 == phydev->link))
    {
        ...
        sstar_phy_update_speed_patch(phydev);
    }
    return ret;
}
...
extern void mhal_sstar_100_phy_suspend(void);

static int sstar_phy_suspend(struct phy_device *phydev)
{
    // printk("[%s][%d]\n", __FUNCTION__, __LINE__);
    mhal_sstar_100_phy_suspend();
    return 0;
}

static int sstar_phy_resume(struct phy_device *phydev)
{
    // printk("[%s][%d]\n", __FUNCTION__, __LINE__);
    return albany_config_init();
}
...
static struct phy_driver __maybe_unused sstar_phy_drvs[] = {
    {
        ...
        .suspend     = sstar_phy_suspend,
        .resume      = sstar_phy_resume,
    },
};

module_phy_driver(sstar_phy_drvs);

static struct mdio_device_id __maybe_unused sstar_phy_tbl[] = {{SSTAR_100_PHY_ID, SSTAR_100_PHY_ID_MSK}, {}};
