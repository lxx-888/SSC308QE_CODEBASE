...
static bool parse_mmaheap_config(char *cmdline, struct MMA_BootArgs_Config *heapconfig)
{
    ...
    if (option)
        ...
        sscanf(option, "max_start_off=%llx", &heapconfig->max_start_offset_to_curr_bus_base);
#else
        sscanf(option, "max_start_off=%x", &heapconfig->max_start_offset_to_curr_bus_base);
#endif
    else
    ...
    if (option)
#ifdef CONFIG_PHYS_ADDR_T_64BIT
        sscanf(option, "max_end_off=%llx", &heapconfig->max_end_offset_to_curr_bus_base);
#else
        sscanf(option, "max_end_off=%x", &heapconfig->max_end_offset_to_curr_bus_base);
#endif
    else
...
void __init deal_with_reserved_mmap(void)
{
    int         i = 0;
    int         ret;
    phys_addr_t start;
        ...
        switch (mmap_reserved_config[i].miu)
        {
            ...
            default:
                printk(KERN_ERR "error!!! %s:%d not support miu %d in this chip!!!!\n", __FUNCTION__, __LINE__,
                       mmap_reserved_config[i].miu);
                return;
                break;
        }

        base = memblock_find_in_range(start, end, size, PAGE_SIZE);
        if (base == 0 || base < start || base > end)
        {
            ret = 0xFF0F;
#ifdef CONFIG_PHYS_ADDR_T_64BIT
            printk("%s base:%llx start:%llx end:%llx size:%llx\n", __FUNCTION__, base, start, end, size);
#else
            printk("%s base:%lx start:%lx end:%lx size:%lx\n", __FUNCTION__, (unsigned long)base, (unsigned long)start,
    ...
    RETURN:
        ...
        {
            mmap_reserved_config[i].reserved_start = base;
...
void __init deal_with_reserve_mma_heap(void)
{
    int         i = 0, j = 0, k = 0;
    phys_addr_t base = 0;
    int         ret;
    ...
    for (i = 0; i < mstar_driver_boot_mma_buffer_num - 1; i++)
    {
        ...
        for (j = i + 1; j < mstar_driver_boot_mma_buffer_num; j++)
        {
            ...
            if (0
                == strncmp((char *)(&mma_config[i].name[0]), (char *)(&mma_config[j].name[0]),
                           strlen((char *)(&mma_config[i].name[0])) + 1)) // use strlen() +1
            {
                printk("fail,error  ,find same mma heap!!!! \n");
                printk(
                    "Please do not use different mma heap with same name , though they may in different miu or have "
                    "different size\n");
    ...
    {
        mma_config[i].reserved_start = -1UL; // INVALID_PHY_ADDR;//init with invalid phy addr
        ...
        {
            case 0: // miu0
                start = (mma_config[i].max_start_offset_to_curr_bus_base == -1UL)
                            ? ARM_MIU0_BUS_BASE
                            : mma_config[i].max_start_offset_to_curr_bus_base;
                end   = (mma_config[i].max_end_offset_to_curr_bus_base == -1UL)
                            ? ARM_MIU1_BUS_BASE
                            : mma_config[i].max_end_offset_to_curr_bus_base;
                break;
            case 1: // miu1
                start = (mma_config[i].max_start_offset_to_curr_bus_base == -1UL)
                            ? ARM_MIU1_BUS_BASE
        ...
        if (0 == strncmp(MMU_HEAP_NAME, mma_config[i].name, strlen(MMU_HEAP_NAME)))
        {
            ...
            {
                base = memblock_find_in_range(start + count * MMU_MAX_SIZE, (count + 1) * MMU_MAX_SIZE + start, size,
                                              PAGE_SIZE);
                if (base > 0)
                    break;
                count++;
...
#ifdef CONFIG_PHYS_ADDR_T_64BIT
            printk("0x%llx size:%llx", (unsigned long long)mma_config[i].reserved_start, size);
#else
            printk("0x%lx size:%lx", (unsigned long)mma_config[i].reserved_start, (unsigned long)size);
#endif
            printk("\n");
