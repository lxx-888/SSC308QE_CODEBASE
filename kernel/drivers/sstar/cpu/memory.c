...
#include "memory.h"
#include "sstar_miubase.h"

#ifdef CONFIG_PHYS_ADDR_T_64BIT
#define SSTAR_MEM_MAX ULLONG_MAX // aarch64 or aarch32 with LPAE
#else
#define SSTAR_MEM_MAX ULONG_MAX // aarch32 without LPAE
#endif
...
int __init arm_add_memory(u64 start, u64 size)
{
    ...
    if (aligned_start < SSTAR_MIU0_BUS_BASE)
    {
        if (aligned_start + size <= SSTAR_MIU0_BUS_BASE)
        {
            pr_info("Ignoring memory below MSTAR_MIU0_BUS_BASE: 0x%08llx-0x%08llx\n", aligned_start,
                    aligned_start + size);
            return -EINVAL;
        }

        pr_info("Ignoring memory below MSTAR_MIU0_BUS_BASE: 0x%08llx-0x%08llx\n", aligned_start,
                (u64)SSTAR_MIU0_BUS_BASE);

        size -= SSTAR_MIU0_BUS_BASE - aligned_start;
        aligned_start = SSTAR_MIU0_BUS_BASE;
    }
...
#else /* for ARM32, arm_add_memory() from other file */
...
s64 div64_s64_rem(s64 dividend, s64 divisor, s64 *remainder)
{
    ...
    quot = div64_s64(dividend, divisor);
    /*
     * There is no div64_s64_rem() in kernel,
     * but C99 asks for (a/b)*b + a%b == a,
     * so a%b = a - (a/b)*b.
     */
...
extern unsigned long long __aeabi_uldivmod(unsigned long long dividend, unsigned long long divisor);
extern long long          __aeabi_ldivmod(long long dividend, long long divisor);
...
static unsigned long MMUmemSize   = 0;
unsigned long        mmu_mem_size = 0xFFFFFFFFUL; // default setting

#ifdef CONFIG_ARM_LPAE
static phys_addr_t MMUmemAddr   = 0;
phys_addr_t        mmu_mem_addr = 0xFFFFFFFFUL; // default setting
#else
static unsigned long MMUmemAddr   = 0;
unsigned long        mmu_mem_addr = 0xFFFFFFFFUL; // default setting
#endif

EXPORT_SYMBOL(mmu_mem_addr);
EXPORT_SYMBOL(mmu_mem_size);

static int __init MMU_MEM_setup(char *str)
{
    // printk("MMU_MEM= %s\n", str);
    if (str != NULL)
    {
#ifdef CONFIG_ARM_LPAE
        sscanf(str, "%llx,%lx", &MMUmemAddr, &MMUmemSize);
        // Check_MIU_BUS(&MMUmemAddr);
#else
        sscanf(str, "%lx,%lx", &MMUmemAddr, &MMUmemSize);
#endif
    }
    else
    {
...
#ifdef CONFIG_ARM_LPAE // CONFIG_PHYS_ADDR_T_64BIT

#if defined(CONFIG_ARM_PATCH_PHYS_VIRT)
phys_addr_t lx_mem_addr = SSTAR_MEM_MAX; // UL(CONFIG_PHYS_OFFSET);//UL(CONFIG_MEMORY_START_ADDRESS);
#elif defined(CONFIG_PHYS_OFFSET)
phys_addr_t lx_mem_addr = ULL(CONFIG_PHYS_OFFSET); // UL(CONFIG_MEMORY_START_ADDRESS);
#else
phys_addr_t lx_mem_addr = SSTAR_MEM_MAX; // UL(CONFIG_PHYS_OFFSET);//UL(CONFIG_MEMORY_START_ADDRESS);
#endif
phys_addr_t lx_mem2_addr = SSTAR_MEM_MAX; // default setting
phys_addr_t lx_mem3_addr = SSTAR_MEM_MAX; // default setting
...
phys_addr_t lx_mem3_addr = SSTAR_MEM_MAX; // default setting
#endif
phys_addr_t lx_mem_size  = SSTAR_MEM_MAX; // default setting
phys_addr_t lx_mem2_size = SSTAR_MEM_MAX; // default setting
phys_addr_t lx_mem3_size = SSTAR_MEM_MAX; // default setting

phys_addr_t lx_mem_left_size  = SSTAR_MEM_MAX; // to avoid overflow in va2pa
phys_addr_t lx_mem2_left_size = SSTAR_MEM_MAX; // to avoid overflow in va2pa
#if 0
...
#ifdef CONFIG_ARM64
        /*
         * In aarch32 the memory blocks are added by LX parameter.
         * In aarch64 we use the dts memory node to do this.
         * And the LX parameter is used to inform the MI.
         * These parameter should be same as the memory node of dts.
         * Try to findout a better way to do this, api eg.
         */
        sscanf(str, "%llx,%llx", &lx_mem_addr, &LXmem);
        printk("LX_MEM addr:0x%llx,size:0x%llx\n", lx_mem_addr, LXmem);
#else
#ifdef CONFIG_PHYS_ADDR_T_64BIT
        sscanf(str, "%llx", &LXmem);
#else
        sscanf(str, "%x", &LXmem);
#endif
#endif
...
static int __init LX_MEM2_setup(char *str)
{
    ...
    {
#ifdef CONFIG_PHYS_ADDR_T_64BIT
        sscanf(str, "%llx,%llx", &LXmem2Addr, &LXmem2Size);
#else
        sscanf(str, "%x,%x", &LXmem2Addr, &LXmem2Size);
#endif
    }
    else
    {
...
static int __init LX_MEM3_setup(char *str)
{
    ...
    {
#ifdef CONFIG_PHYS_ADDR_T_64BIT
        sscanf(str, "%llx,%llx", &LXmem3Addr, &LXmem3Size);
#else
        sscanf(str, "%x,%x", &LXmem3Addr, &LXmem3Size);
#endif
    }
    else
    {
...
void get_boot_mem_info(BOOT_MEM_INFO type, phys_addr_t *addr, phys_addr_t *len)
{
    ...
    if (bUseDefMMAP == 0)
    {
        switch (type)
        {
            case LINUX_MEM:
#ifdef CONFIG_ARM64
                *addr = SSTAR_MIU0_BUS_BASE;
#else
                *addr = SSTAR_MIU0_BUS_BASE + PHYS_OFFSET - SOC_LOW_PHYS_START;
#endif
                *len = LXmem;
                break;
            case EMAC_MEM:
        ...
        case MPOOL_MEM:
            ...
            *len = DRAMlen - *addr;
            ...
            case LINUX_MEM3:
                ...
                break;
            case MMU_RESERVED_MEM:
                if (MMUmemAddr != 0 && MMUmemSize != 0)
                {
                    *addr = MMUmemAddr;
                    *len  = MMUmemSize;
                }
...
void __init prom_meminit(void)
{
    ...
    phys_addr_t mmu_memory_address = 0, mmu_memory_length = 0;
    u64         size  = 0;
    u64         start = 0;
    ...
    get_boot_mem_info(MMU_RESERVED_MEM, &mmu_memory_address, &mmu_memory_length);
...
#endif
#ifdef CONFIG_ARM64
    printk("LXmem is 0x%llx PHYS_OFFSET is 0x%llx\n", (u64)LXmem, (u64)SSTAR_MIU0_BUS_BASE);
#else
    printk("LXmem is 0x%llx PHYS_OFFSET is 0x%llx\n", (u64)LXmem,
           (u64)SSTAR_MIU0_BUS_BASE + PHYS_OFFSET - SOC_LOW_PHYS_START);
#endif
...
#endif
#ifdef CONFIG_ARM64
        if (lx_mem_addr == SSTAR_MEM_MAX)
            lx_mem_addr = start = linux_memory_address;
        else
            start = linux_memory_address = lx_mem_addr;
#else
    ...
    if (mmu_memory_length != 0)
    {
        mmu_mem_addr = start = mmu_memory_address;
        mmu_mem_size = size = mmu_memory_length;
        printk("Add mem start 0x%llx size 0x%llx!!!!\n", start, size);
        arm_add_memory(start, size);
    ...
    printk("MMU_MEM = 0x%llx, 0x%llx\n", mmu_memory_address, mmu_memory_length);
    printk("EMAC_LEN= 0x%llX\n", EMACmem);
    printk("DRAM_LEN= 0x%llX\n", DRAMlen);
#else
    printk("LX_MEM  = 0x%zx, 0x%zx\n", linux_memory_address, linux_memory_length);
    printk("LX_MEM2 = 0x%zx, 0x%zx\n", linux_memory2_address, linux_memory2_length);
    printk("LX_MEM3 = 0x%zx, 0x%zx\n", linux_memory3_address, linux_memory3_length);
    printk("MMU_MEM = 0x%zx, 0x%zx\n", mmu_memory_address, mmu_memory_length);
    printk("EMAC_LEN= 0x%X\n", EMACmem);
    printk("DRAM_LEN= 0x%X\n", DRAMlen);
#endif

    /* prevent overflow while converting pa to va for linux memory */
    if (linux_memory_length > (SSTAR_MEM_MAX - PAGE_OFFSET))
    {
        lx_mem_left_size  = SSTAR_MEM_MAX - PAGE_OFFSET;
        lx_mem2_left_size = 0;
    }
    else
    {
        lx_mem_left_size = lx_mem_size;
        if (linux_memory2_length > (SSTAR_MEM_MAX - PAGE_OFFSET - lx_mem_left_size))
        {
            lx_mem2_left_size = SSTAR_MEM_MAX - PAGE_OFFSET - lx_mem_left_size;
        }
        else
        {
